...
from pastor.loader import load_project_meta, convert_relative_project_root_dir
from pastor.parser import parse_data
from pastor.utils import sort_dict_by_custom_order
...
def _convert_validators(validators: List) -> List:
    for v in validators:
        if "check" in v and "expect" in v:
            ...
            if isinstance(v[comparator], List):
...
def _ensure_step_attachment(step: Dict) -> Dict:
    ...
    if "pre_sql" in step:
        test_dict["pre_sql"] = step["pre_sql"]

    if "post_sql" in step:
        test_dict["post_sql"] = step["post_sql"]
...
def ensure_testcase_v3(test_content: Dict) -> Dict:
    ...
    for step in test_content["teststeps"]:
        ...
        elif "location" in step:
            # 新增ui 20241106 liangchunhua
            teststep["location"] = step.pop("location")
        elif "api" in step:
            teststep["testcase"] = step.pop("api")
        # elif "sql" in step:
        #     teststep["testcase"] = step.pop("sql")
...
def _generate_conftest_for_summary(args: List):
    ...
    conftest_content = '''# NOTICE: Generated By pastor.
import json
import os
import time
...
from pastor.utils import get_platform, ExtendJSONEncoder
