...
def is_pastor_testcase(item):
    """ check if a variable is a Pastor testcase class
    """
    ...
    return bool(
        ...
        and issubclass(item, Pastor)
        and item.__name__ != "Pastor"
    )
...
def prepare_locust_tests() -> List:
    ...
    for pytest_file in pytest_files:
        ...
        for name, item in vars(module).items():
            ...
            if not is_pastor_testcase(item):
                continue
...
def main_locusts():
    ...
    from pastor.make import main_make
