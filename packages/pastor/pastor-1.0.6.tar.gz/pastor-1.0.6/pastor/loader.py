...
from pastor import builtin, exceptions, utils
from pastor.exceptions import ParamsError
from pastor.models import ProjectMeta, TestCase, TestSuite
...
def load_testcase_file(testcase_file: Text) -> TestCase:
    ...
    # testcase_content = convert_location_to_obj_list(testcase_content)
    testcase_obj = load_testcase(testcase_content)
    testcase_obj.config.path = testcase_file
    return testcase_obj


def convert_location_to_obj_list(testcase_content: Dict):
    """
    ui元素定位数据转化
    """
    steps = testcase_content.get('teststeps', [])
    for index, step in enumerate(steps):
        locations = step.get('location', [])
        location_list = []
        for location in locations:
            location = re.sub(r'\s+', '', location).replace('，', ',')
            array = location.split(',')
            dict_items = [s.split('=') for s in array]
            try:
                location_dict = {key: str(value) for key, value in dict_items}
                location_list.append(location_dict)
            except ValueError as ve:
                raise ParamsError(f'{location} 设置错误，请按正确的格式设置location！')
        step['location'] = location_list
        steps[index] = step
    if len(steps) != 0:
        testcase_content['teststeps'] = steps
...
def locate_validate_py(start_path: Text) -> Text:
    """
    locate validate.py file
    :param start_path: start locating path
                        maybe testcase file path or directory path
    :return:  validate.py file path, None if not found
    """
    try:
        # locate validate.py file.
        validate_path = locate_file(start_path, "validate.py")
...
def locate_project_root_directory_validate(test_path: Text) -> Tuple[Text, Text]:
    """ locate validate.py path as project root directory

    Args:
        test_path: specified testfile path

    Returns:
        (str, str): validate.py path, project_root_directory_validate

    """
    test_path = prepare_path(test_path)
    # locate validate.py file
    validate_path = locate_validate_py(test_path)

    if validate_path:
        # The folder contains validate.py will be treated as project RootDir.
        project_root_directory_validate = os.path.dirname(validate_path)
    else:
        # validate.py not found, use os.getcwd() as project RootDir.
        project_root_directory_validate = os.getcwd()

    return validate_path, project_root_directory_validate
...
def load_validate_functions() -> Dict[Text, Callable]:
    # load validate.py module
    try:
        imported_module = importlib.import_module("validate")
    except Exception as ex:
        logger.error(f"error occurred in validate.py: {ex}")
        sys.exit(1)
...
def load_project_meta(test_path: Text, reload: bool = False) -> ProjectMeta:
    ...
    validate_path, project_root_directory_validate = locate_project_root_directory_validate(test_path)

    # add project RootDir to sys.path
    sys.path.insert(0, project_root_directory)
    if project_root_directory_validate:
        sys.path.insert(0, project_root_directory_validate)
    ...
    if validate_path:
        # load validate.py functions
        validate_functions = load_validate_functions()
    else:
        validate_functions = {}
    debugtalk_functions.update(validate_functions)
