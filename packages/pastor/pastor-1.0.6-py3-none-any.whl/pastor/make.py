...
from pastor.compat import (convert_variables, ensure_path_sep,
                               ensure_testcase_v3, ensure_testcase_v3_api)
from pastor.loader import (convert_relative_project_root_dir,
                               load_folder_files, load_project_meta,
                               load_test_file, load_testcase, load_testsuite)
from pastor.response import uniform_validator
from pastor.uicore.element import get_uniform_comparator

from pastor.utils import (ga_client, is_support_multiprocessing,
                              merge_variables)
...
__TEMPLATE__ = jinja2.Template(
    """# NOTE: Generated By pastor v{{ version }}
# FROM: {{ testcase_path }}
...
def __ensure_absolute(path: Text) -> Text:
    if path.startswith("./"):
        # Linux/Darwin, prun ./test.yml
        path = path[len("./"):]
    elif path.startswith(".\\"):
        # Windows, prun .\\test.yml
        path = path[len(".\\"):]
...
def __ensure_testcase_module(path: Text):
    ...
    with open(init_file, "w", encoding="utf-8") as f:
        f.write("# NOTICE: Generated By pastor. DO NOT EDIT!\n")
...
def make_config_chain_style(config: Dict) -> Text:
    ...
    if "ignore_cache" in config:
        config_chain_style += f'.ignore_cache({config["ignore_cache"]})'
    ...
    if "datasource" in config:
        config_chain_style += f".datasource('{config['datasource']}')"

    return config_chain_style


def make_location_chain_style(locations: List) -> Text:
    location_chain_style = f".location()"
    for location in locations:
        location_chain_style += f'.with_element(**{location})'
    return location_chain_style
...
def make_teststep_chain_style(teststep: Dict) -> Text:
    ...
    elif teststep.get("location"):
        step_info = f'RunLocation("{teststep["name"]}")'
    ...
    if "type" in teststep:
        step_type = teststep["type"]
        step_info += f".type('{step_type}')"

    if "skip" in teststep:
        skip = teststep["skip"]
        step_info += f".skip('{skip}')"
    ...
    if "pre_sql" in teststep:
        sql_datas = teststep["pre_sql"]
        step_info += f".pre_sql()"
        for sql_data in sql_datas:
            if sql_data['datasource']:
                step_info += f".datasource('{sql_data['datasource']}')"
            for dml in sql_data['dml']:
                step_info += f".dml('{dml.strip()}')"

    if teststep.get("request"):
        step_info += make_request_chain_style(teststep["request"])
    elif teststep.get("location"):
        # 新增ui步骤
        step_info += make_location_chain_style(teststep["location"])
    elif teststep.get("testcase"):
        testcase = teststep["testcase"]
        call_ref_testcase = f".call({testcase})"
        step_info += call_ref_testcase

    if "post_sql" in teststep:
        sql_datas = teststep["post_sql"]
        step_info += f".post_sql()"
        for sql_data in sql_datas:
            if sql_data['datasource']:
                step_info += f".datasource('{sql_data['datasource']}')"
            for dml in sql_data['dml']:
    ...
    if teststep.get("location"):
        # 新增UI
        for v in teststep["validate"]:
            comparator = list(v.keys())[0]
            compare_values = v[comparator]
            assert_method = get_uniform_comparator(comparator)
            step_info += f".validate('{assert_method}', **{compare_values})"
    else:
...
def main_make(tests_paths: List[Text]) -> List[Text]:
    ...
    ga_client.track_event("ConvertTests", "pmake")
