...
from pastor import __description__, __version__
from pastor.compat import ensure_cli_args
from pastor.ext.har2case import init_har2case_parser, main_har2case
from pastor.make import init_make_parser, main_make
from pastor.scaffold import init_parser_scaffold, main_scaffold
from pastor.utils import init_sentry_sdk, ga_client
...
def init_parser_run(subparsers):
    sub_parser_run = subparsers.add_parser(
        "run", help="Make Pastor testcases and run with pytest."
    )
    return sub_parser_run


def main_run(extra_args) -> enum.IntEnum:
    ga_client.track_event("RunAPITests", "prun")
    ...
    logger.info(f"start to run tests with pytest. Pastor version: {__version__}")
