...
class AsofBitemporalJoinFullAggSparkNode(AsofJoinFullAggSparkNode):
    ...
    def _to_dataframe(self, spark: pyspark.sql.SparkSession) -> pyspark.sql.DataFrame:
        ...
        2. Union the output of (1) with the spine and partition by join_keys AND spine.timestamp
        """

        spine_df = self.spine.to_dataframe(spark)
        partial_agg_df = self.partial_agg_node.to_dataframe(spark)

        join_key_conditions = [spine_df[key] == partial_agg_df[key] for key in self.fdw.join_keys]

        bitemporal_join_conditions = [
            spine_df[self.fdw.timestamp_key] >= partial_agg_df[effective_timestamp()],
            spine_df[anchor_time()] >= partial_agg_df[anchor_time()],
        ]
        if not self.fdw.has_lifetime_aggregate:
            ...
            bitemporal_join_conditions.append(
                spine_df[anchor_time()] - max_aggregation_window <= partial_agg_df[anchor_time()]
            )

        partial_agg_column_names_to_keep = (
            set(partial_agg_df.columns) - set(self.fdw.join_keys) - {effective_timestamp()}
        )
        partial_agg_columns_to_keep = [partial_agg_df[col] for col in partial_agg_column_names_to_keep]
        join_key_columns = [spine_df[key] for key in self.fdw.join_keys]
        cols_to_keep = [*join_key_columns, spine_df[self.fdw.timestamp_key], *partial_agg_columns_to_keep]

        # use inner join instead of left join to avoid performance problems of merge sort join associated with left join.
        # we union spine_df with joined_df afterwards so spine rows with no matching partial_agg rows are not lost.
        joined_df = (
            partial_agg_df.join(spine_df, on=join_key_conditions + bitemporal_join_conditions, how="inner")
        ).select(*cols_to_keep)
        ...
        if self.fdw.aggregation_secondary_key:
            ...
            # If the aggregation secondary key appears, we add an indicator column to the partial aggregation result to
            # help identify all secondary keys that are present in each distinct time window.
            joined_df = partial_agg_df.withColumn(tecton_secondary_key_aggregation_indicator_col(), F.lit(1))
