...
class CalculationSqlBuilder(ABC):
    ...
    def _operation_node_to_query_term(self, operation_node: OperationNodeSpec) -> Term:
        ...
        elif operation_node.operation == calculation_node_pb2.OperationType.STRPTIME:
            return self._build_strptime_query(operation_node)
        else:
            msg = f"In Calculation sql generation, calculation operation {OperationType.Name(operation_node.operation)} not supported."
            raise TectonInternalError(msg)

    def _build_strptime_query(self, operation_node: OperationNodeSpec) -> Term:
        if len(operation_node.operands) != 2:
            msg = "Calculation function Strptime must have exactly 2 operands."
            raise TectonInternalError(msg)
        date_string = self.ast_node_to_query_term(operation_node.operands[0])
        format_string = self.ast_node_to_query_term(operation_node.operands[1])
        return LiteralValue(f"STRPTIME({date_string}, {format_string})")
