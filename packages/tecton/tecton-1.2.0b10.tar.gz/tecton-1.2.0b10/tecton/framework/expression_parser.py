...
VALID_STRPTIME_DIRECTIVES = {
    ...
    "f",  # Microsecond as decimal number, zero-padded (000000-999999)
    "g",  # Millisecond as a decimal number, zero-padded (000-999), note: not supported in python
    "G",  # ISO 8601 year with century representing the year that contains the greater part of the ISO week (see %V) (0001, 0002, ..., 9999)
    "H",  # Hour (24-hour clock) as zero-padded decimal number (00-23)
    "I",  # Hour (12-hour clock) as zero-padded decimal number (01-12)
    "j",  # Day of year as zero-padded decimal number (001-366)
    "m",  # Month as zero-padded decimal number (01-12)
    "M",  # Minute as zero-padded decimal number (00-59)
    "n",  # nanosecond as a decimal number, zero-padded (000000000-999999999), note: not supported in python
    "p",  # Locale's AM/PM indicator (AM/PM)
    "S",  # Second as zero-padded decimal number (00-59)
    "u",  # ISO 8601 weekday as decimal number (1-7, 1=Monday)
    "U",  # Week number of year, week 01 starts on first Sunday of year (00-53, Sunday first day), not ISO 8601 compliant
    "V",  # ISO 8601 week number, monday is first day of week and week 01 is the week containing January 4 (01-53), not compatible with %Y (use %G instead)
    "w",  # Weekday as decimal number (0-6, 0=Sunday)
    "W",  # Week number of year, week 01 starts on the first Monday of the year (00-53), not ISO 8601 compliant
    "x",  # ISO date representation (1992-03-02)
    "X",  # ISO time representation (10:30:20)
    "y",  # Year without century, zero-padded (00-99)
    "Y",  # Year with century as a decimal number (2013, 2019, etc)
    "z",  # UTC offset in form +HHMM or -HHMM
    "Z",  # Time zone name
    "%",  # Literal % character
}
...
class _ToAbstractSyntaxTree(Transformer):
    ...
    def strptime(self, values: List[AbstractSyntaxTreeNode]) -> AbstractSyntaxTreeNode:
        timestamp_ast, format_ast = values
        # check data types
        if timestamp_ast.dtype.type != DataTypeEnum.DATA_TYPE_STRING:
            msg = f"Error parsing expression: STRPTIME requires a string timestamp value, {self.original_expression}"
            raise ExpressionParsingError(msg, self.calculation_name)

        # validate format value
        self._validate_strptime_format(format_ast)
        ...
        if timestamp_ast.HasField("literal_value"):
            ...
            msg = f"Error parsing expression: STRPTIME requires an ISO 8601 formatted UTC timestamp string, {self.original_expression}"
            try:
                # TODO: update the format here to be from the expression rather than the hardcoded ISO_8601_FORMAT
                dt = datetime.strptime(timestamp_str, ISO_8601_FORMAT)
            except ValueError:
                raise ExpressionParsingError(msg, self.calculation_name)
        ...
        return AbstractSyntaxTreeNode(
            operation=Operation(operation=OperationType.STRPTIME, operands=[timestamp_ast, format_ast]),
            dtype=TimestampType().proto,
        )

    def _validate_strptime_format(self, format_ast: AbstractSyntaxTreeNode):
        if format_ast.dtype.type != DataTypeEnum.DATA_TYPE_STRING:
            msg = f"Error parsing expression: STRPTIME requires a string format value, {self.original_expression}"
            raise ExpressionParsingError(msg, self.calculation_name)
        format_str = format_ast.literal_value.string_value
        if "%" not in format_str:
            msg = f"Error parsing expression: no format directives specified in STRPTIME format string: {format_str}"
            raise ExpressionParsingError(msg, self.calculation_name)
        ...
        while i < len(format_str):
            if format_str[i] == "%":
                if i + 1 >= len(format_str):
                    msg = f"Error parsing expression: Incomplete format directive at position {i} in {format_str}"
                    raise ExpressionParsingError(msg, self.calculation_name)
                if format_str[i + 1] not in VALID_STRPTIME_DIRECTIVES:
                    msg = f"Error parsing expression: Invalid format directive '{format_str[i:i + 2]}' at position {i} in {format_str}"
                    raise ExpressionParsingError(msg, self.calculation_name)
                i += 2
            else:
                i += 1
        ...
        if format_str != ISO_8601_FORMAT:
            msg = f"Error parsing expression: STRPTIME currently only supports ISO 8601 format ('{ISO_8601_FORMAT}'), {self.original_expression}"
            raise ExpressionParsingError(msg, self.calculation_name)
