...
使用 ``pip`` 安装托管在 `pypi`_ 上的 ``ccob`` 项目, 运行以下命令:
...
可以在 ``gitee`` 代码托管平台上下载 ``ccob`` 项目的源代码, 地址: `gitee repo`_ .

可以使用 ``git`` 克隆项目源代码:

.. code-block:: console

    $ git clone https://gitee.com/gooker_young/ccob.git
...
.. code-block:: console
    ...
    $ uv build
    $ cd dist && pip install ccob --no-index --find-links .

.. _gitee repo: https://gitee.com/gooker_young/ccob
