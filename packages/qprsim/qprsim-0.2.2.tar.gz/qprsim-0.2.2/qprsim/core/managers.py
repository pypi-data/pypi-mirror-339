...
from qprsim.model.model_traits import WithResourceManager, WithActivityManager
from qprsim.shared.base_classes import permuted, weighed_permutation
...
class ActivityResourceCorrespondence:
    ...
    def __init__(self, ar_mapping: dict[
        cm.ResourceUser, Set[cm.ResourceProvider]],
                 assignment_propensities: Dict[
                     cm.ResourceUser, Dict[cm.ResourceProvider, Number]] = None) -> None:
    ...
    @property
    def activities(self) -> KeysView[cm.ResourceUser]:
        return self.__assignable_resources_map.keys()

    @property
    def resources(self) -> KeysView[cm.ResourceProvider]:
        return self.__assignable_activities_map.keys()

    def activity_propensities(self, resource: cm.ResourceProvider):
        return self.__resource_propensity_map[resource]

    def resource_propensities(self, activity: cm.ResourceUser):
        return self.__activity_propensity_map[activity]

    def assignable_activities(self, resource: cm.ResourceProvider) -> Set[
        cm.ResourceUser]:
        return self.__assignable_activities_map[resource]

    def assignable_resources(self, activity: cm.ResourceUser) -> Set[cm.ResourceProvider]:
...
class ActivityManager(WithResourceManager):
    ...
    def __init__(self, event_queue: ev_sys.EventQueue, resource_manager: ResourceManager = None) -> None:
        ...
        self.activity_demand: dict[cm.ResourceUser, bool] = {}
    ...
    def weighed_activities_for_resource(self, resource: cm.ResourceProvider):
    ...
    def demanding_activities(self, resource: cm.ResourceProvider = None):
    ...
    def _demanding_activities_perm(self, resource: cm.ResourceProvider = None):
        return self.rng.permutation(list(self.demanding_activities(resource)))

    def activity_demand_change(self, activity: cm.ResourceUser, new_state: bool) -> None:
        self.activity_demand[activity] = new_state
        logging.info(f'{activity} changed its demand to {new_state}')
        if new_state:
            self.event_queue.offer_end_of_timestep_task(self.whip_workers)

    def distribute_resource(self, resource: cm.ResourceProvider) -> bool:
        ...
        if len(activities_for_resource) > 0:
            ...
            activity.take_resource_from(resource)
            return True
        else:
            return False
            # weigh ordering here
...
class ResourceManager(WithActivityManager):

    def __init__(self, event_queue: ev_sys.EventQueue, activity_manager: ActivityManager = None) -> None:
        super().__init__(activity_manager=activity_manager)
        self.event_queue = event_queue
        self.__activity_manager = activity_manager
        self.__ar_correspondence = None
        self.resource_supply: dict[cm.ResourceProvider, bool] = {}
        self.rng = np.random.default_rng()
    ...
    def weighed_resources_for_activity(self, activity: cm.ResourceUser):
    ...
    def supplying_resources(self, activity: cm.ResourceUser = None):
    ...
    def resource_supply_change(self, resource: cm.ResourceProvider, new_state: bool) -> None:
        self.resource_supply[resource] = new_state
        logging.info(f'{resource} changed its supply to {new_state}')
        if new_state:
            self.event_queue.offer_end_of_timestep_task(self.redistribute_the_means)

    def redistribute_the_means(self) -> None:
    ...
    def provide_resource_for(self, activity: cm.ResourceUser) -> bool:
    ...
    def is_supplying(self, resource: cm.ResourceProvider) -> bool:
