...
if TYPE_CHECKING:
    from qprsim.execution.simulation import SimulationModel
...
class ButWhatIf:
    ...
    def __init__(self, sim_model: SimulationModel) -> None:
        self.sim_model = sim_model
        self.scheduling_manager = sim_model.scheduling_manager
        self.working_set = {}
        self.scheduled_changes = []
        self.activity_datas = dict(sim_model.scheduling_manager.activity_datas) # {str(am): am.data for am in sim_model.activity_manager.activity_demand}
        self.resource_datas = dict(sim_model.scheduling_manager.resource_datas) # {str(rm): rm.data for rm in sim_model.resource_manager.resource_supply}
