...
def __get_event_log():
    global _EVT_LOG
    if _EVT_LOG is None:
        _EVT_LOG = EventLogger(capacity=10000, pager=page)
    return _EVT_LOG
...
# Useful functions
# (not magic commands but accessible from CLI as normal python functions)
...
def init_pytango(ip):
    ...
    ip.set_hook('complete_command', dp_completer, re_key = r".*DeviceProxy[^\w\.]+")
    ip.set_hook('complete_command', dp_completer, re_key = r".*Device[^\w\.]+")
    ip.set_hook('complete_command', attr_completer, re_key = r".*AttributeProxy[^\w\.]+")
    ip.set_hook('complete_command', attr_completer, re_key = r".*Attribute[^\w\.]+")
...
def init_db(parameter_s=''):
    ...
    for klass, devices in klass_dict.items():
        ...
        if not exists or klass in old_junk:
            ...
            ip.set_hook('complete_command', c, re_key = r".*" + klass + r"[^\w\.]+")
...
def load_config(config):
    ...
    if common.get_ipython_version() < Version('5.0'):
