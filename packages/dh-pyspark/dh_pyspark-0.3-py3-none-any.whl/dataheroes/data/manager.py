...
class DataManagerBase:
    ...
    def read_file(
        ...
        file_path: file/directory or an iterator of file/directory
                path to a local files or local directories
        ...
        def _to_files(p):
            return helpers.file_path_to_files(p, sort_by_name=sort_by_name)
    ...
    def _estimate_expected_n_features(self, dataset, return_counts=False) -> Union[int, Tuple[int, Dict]]:
        ...
        if not self.has_categorical_features() and not self.has_array_features():
            if not return_counts:
                ...
                return self.n_features, defaultdict(int, {'numeric': self.n_features - bool_features_total,
                                                          'boolean': bool_features_total})
        ...
        arrays_features_total = self._get_expected_num_array_features(dataset, array_features)

        if return_counts:
            # get ohe and te counts
            cat_features_total, feature_counts = self._get_expected_num_cat_features(dataset, cat_features,
                                                                                     return_counts=True)
            # count bool features
            bool_features_total = len(
                [feature_idx for feature_idx in range(dataset.shape[1])
            ...
            feature_counts['numeric'] = n_num_features - bool_features_total
            feature_counts['array'] = arrays_features_total
            feature_counts['boolean'] = bool_features_total
            return n_num_features + cat_features_total + arrays_features_total, feature_counts
        else:
            cat_features_total = self._get_expected_num_cat_features(dataset, cat_features)
            return n_num_features + cat_features_total + arrays_features_total
    ...
    def _get_expected_num_cat_features(self, dataset, cat_features,
                                       return_counts: bool = False) -> Union[int, Tuple[int, Dict]]:
