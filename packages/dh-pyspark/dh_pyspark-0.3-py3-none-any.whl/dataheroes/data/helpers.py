...
from typing import Callable, Union, Iterator, Optional, List, Tuple
...
def file_path_to_files(file_path, *, include_dirs=False, sort_by_name=True) -> Iterator:
    """
    ...
    def _to_files(path):
        ...
        files_ = path.iterdir() if path.is_dir() else [path]
        for f in files_:
            if not include_dirs and not f.is_file():
                continue
            yield f
