...
class CoresetServiceBase(abc.ABC):
    ...
    def _save(
        ...
        assert (not dir_path.exists() or dir_path.is_dir()), f"`dir_path` is not a directory ({dir_path})"
        dir_path.mkdir(exist_ok=True, parents=True)

        # create subdirectory where data will be stored
        # when override, remove existing directory
        if override:
            save_dir = dir_path.joinpath(name)
            if save_dir.exists():
                shutil.rmtree(save_dir)
            save_dir.mkdir()
        else:
            save_dir = dir_path.joinpath(f'{name}_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}')
            assert not save_dir.exists(), f'save_dir already exist ({save_dir})'
            save_dir.mkdir()

        model_path = save_dir.joinpath('model.pickle')
        service_params_path = save_dir.joinpath("service_params.json")
        ...
        with service_params_path.open('w') as fp:
            json.dump(service_params, fp, cls=JSONEncoderExtra)
    ...
    def _load_params(
        ...
        load_dir = cls._resolve_load_dir(dir_path, name)
        service_params_path = load_dir.joinpath('service_params.json')

        with service_params_path.open() as fp:
            service_params = json.load(fp)
    ...
    def _fit(
        self, tree_idx: int, model=None, model_params=None, coreset_params=None, sparse_threshold=None, **params
    ):
    ...
    def _build_from_file(
        ...
        datasets = self.data_manager.read_file(file_path, reader_f=reader_f, reader_kwargs=reader_kwargs, sort_by_name=sort_by_name)
        if target_file_path is not None:
            target_datasets = self.data_manager.read_file(target_file_path, reader_f=reader_f, reader_kwargs=reader_kwargs, sort_by_name=sort_by_name)
        else:
    ...
    @classmethod
    def _resolve_load_dir(cls, dir_path: Union[str, os.PathLike], name: str = None):
        dir_path = pathlib.Path(dir_path)
        name = name or cls._get_save_name()
        candidates = [
            f for f in dir_path.glob(f'{name}*')
            if f.is_dir() and (
                    f.name == name
                    or re.match(r'%s_\d{8}_\d{4}' % name, f.name)
            )
        ]
        load_dir = sorted(candidates, reverse=True)[0] if candidates else dir_path.joinpath(name)
        return load_dir
