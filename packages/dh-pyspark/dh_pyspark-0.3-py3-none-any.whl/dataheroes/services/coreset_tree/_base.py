...
from ...utils import add_telemetry_attribute, telemetry, check_feature_for_license
...
class CoresetTreeService(CoresetServiceBase):
    ...
    def save(
        ...
        for i, tree in enumerate(self.trees):
            tree_name = f"tree_{i}"
            if allow_pickle:
                tree_data_path = save_dir.joinpath(f'{tree_name}.pkl')
                joblib.dump(tree_data, tree_data_path)
            else:
                tree_data_path = save_dir.joinpath(f'{tree_name}.json')
                with tree_data_path.open('w') as fp:
                    json.dump(tree_data, fp)

            if tree_models:
                tree_nodes_models_dir = save_dir.joinpath(f'{tree_name}_nodes_models')
                tree_nodes_models_dir.mkdir(exist_ok=True)
                for model_data in tree_models:
                    if model_data:
                        model_id, model = model_data
                        joblib.dump(model, tree_nodes_models_dir.joinpath(f"{model_id}"))

        if save_buffer:
            buffer_path = save_dir.joinpath('buffer.npz')
            assert self.tree_group_manager is not None
            buffer = self.tree_group_manager.chunk_layer.get_buffer()
            if buffer:
                np.savez(buffer_path, *buffer)
    ...
    def load(
        ...
        for i, tree_params in enumerate(trees_params):
            tree_name = f"tree_{i}"
            # load the tree data
            tree_data_path = load_dir.joinpath(f'{tree_name}.json')
            if tree_data_path.exists():
                with tree_data_path.open() as fp:
                    tree_data = json.load(fp)
            else:
                tree_data_path = load_dir.joinpath(f'{tree_name}.pkl')
                tree_data = joblib.load(tree_data_path)

            # load node level models
            tree_nodes_models_dir = load_dir.joinpath(f'{tree_name}_nodes_models')
            if tree_nodes_models_dir.exists():
                models = []
                for model_path in tree_nodes_models_dir.iterdir():
                    if model_path.is_file():
                        models.append((model_path.stem, joblib.load(model_path)))
        ...
        # load and set buffer
        buffer_path = load_dir.joinpath('buffer.npz')
        if load_buffer and buffer_path.exists():
            container = np.load(str(buffer_path.absolute()), allow_pickle=True)
    ...
    def get_coreset(
        ...
        """
        if 'get_coreset_size' not in [s.function for s in inspect.stack()]:
            # get_coreset_size should work without license
            check_feature_for_license("get_coreset")
        self._requires_tree()
        return self._get_tree_coreset(
            tree_idx=tree_idx,
            level=level,
            seq_from=seq_from,
            seq_to=seq_to,
    ...
    @telemetry
    def save_coreset(self, file_path: Union[str, os.PathLike], level: int = 0,
                     preprocessing_stage: Union[str, None] = 'user', with_index: bool = True):
        """
        ...
        data = self.get_coreset(level=level,
                                preprocessing_stage=preprocessing_stage, as_df=True)
        export_data = data['X']
        y = data.get('y', None)
    ...
    @telemetry
    def plot(self, dir_path: Union[str, os.PathLike] = None, tree_indices: List[int] = None) -> Dict[int, pathlib.Path]:
        """
        ...
        """
        self._requires_tree()
        dir_path = dir_path or self.working_directory
        ...
        for tree_idx in tree_indices:
            ...
            output[tree_idx] = pathlib.Path(res) if res else None
        return output
    ...
    def _set_reader_chunk_size_param(self, file_path, reader_f, reader_kwargs, reader_chunk_size_param_name,
        ...
        if not chunk_size or chunk_size <= 0:
            n_features = self.data_manager.get_file_n_columns(
                next(file_path_to_files(file_path)),
                reader_f,
                reader_kwargs,
    ...
    def _build_internal(self, datasets, *, partial=False, chunk_size=None, chunk_by=None, **kwargs):
        ...
        # Extract first dataset for peaking into it for estimation purposes.
        first = next(datasets)
        datasets = itertools.chain([first], datasets)
        ...
        expected_n_features = self.data_manager._estimate_expected_n_features(first.X)
        ...
        for i, size in enumerate(self.data_tuning_params.coreset_size):
            if auto_chunk_size or not size:
                ...
                if size is None:
                    ...
                    # self.params['coreset_size'] = coreset_size_tree_type

        self.params['chunk_size'] = chunk_size

        # Define n_jobs.
        user_defined_n_jobs = kwargs.get('n_jobs')
        if chunk_size and (n_instances * expected_n_features) > get_min_data_cells_for_parallel_build():
            max_batch_size = evaluate_max_batch_size(expected_n_features, available=True, dtype=self._get_dtype())
    ...
    def _print_model_warning(self, model):
        if model is None and self.user_set_model is False:
            ...
            example_model = "CatBoostRegressor" if model_name == "CatBoostRegressor" else "LGBMRegressor"
            message += (
    ...
    def _split_into_folds(nodes, levels, indexes, buffer_node, n_folds):
        ...
        if buffer_node is not None:
            folds_nodes[0].append(buffer_node)
            folds_levels[0].append(-1)
            folds_indexes[0].append(-1)
