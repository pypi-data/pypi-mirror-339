...
1. Sample weight support was added throughout, according to the following source:
   https://github.com/scikit-learn/scikit-learn/blob/0b04adcb8f518b9bd5a26ccc2d058a54370a01f0/sklearn/preprocessing/_target_encoder.py
   Based on:
   https://github.com/scikit-learn/scikit-learn/issues/28881
   https://github.com/scikit-learn/scikit-learn/pull/29110/files
2. All imports replaced to point to a local frozen-in-time sklearn copy (based off 1.5.1).
3. fit_transform adapted to support retro-fitting from a pre-fit encoders rebuilt from values saved in the past for
   the sake of transforming in the context of prediction.
"""
from numbers import Integral, Real

import numpy as np

from .sklearn_1_5_1_mask.base import OneToOneFeatureMixin, _fit_context
from .sklearn_1_5_1_mask.utils._param_validation import Interval, StrOptions
from .sklearn_1_5_1_mask.utils.multiclass import type_of_target
from .sklearn_1_5_1_mask.utils.validation import (
...
from .sklearn_1_5_1_mask.preprocessing._encoders import _BaseEncoder
# from ._target_encoder_fast import _fit_encoding_fast, _fit_encoding_fast_auto_smooth

from ..numpy_extra import unique  # DH Specific
...
class TargetEncoder(OneToOneFeatureMixin, _BaseEncoder):
    ...
    def __init__(
            ...
            shuffle=True,
            random_state=None,
            # DH specific: attributes of a pre-built encoder for using in a predict-only context.
            pre_fit_classes=None,
            pre_fit_target_mean=None,
            pre_fit_encodings=None,
    ):
        ...
        # DH specific: remembering attributes of a pre-built encoder for using in a predict-only context.
        self.pre_fit_classes = pre_fit_classes
        self.pre_fit_target_mean = pre_fit_target_mean
        self.pre_fit_encodings = pre_fit_encodings

    @_fit_context(prefer_skip_nested_validation=True)
    def fit_transform(self, X, y, sample_weight=None):
        """
        DH Specific handling:
        Replacing the original fit_transform with a forked decision on whether to call the original, or "retro-fit"
        the attributes with pre-fit values - something that can happen only in the context of prediction.
        Even if we decide to call the original, there is a very specific edge case we handle, as an addition in the
        original method, in order to circumvent failure in cross-validation for extremely small datasets.
        """
        if y is not None:
            X_out = self.fit_transform_orig_safe(X=X, y=y, sample_weight=sample_weight)
            return X_out
        else:
            if sample_weight is not None:
                raise ValueError("Non-targeted fit_transform called in a wrong (non transform-only) context")
            # "_fit" rebuilds "self.n_features_in_" and "self.categories_" using values passed in the constructor.
            self._fit(X, handle_unknown="ignore", force_all_finite="allow-nan")
            # Assign values directly as-if they were always there, and call "transform" straight away.
            self.target_type_ = self.target_type
            # It is essential that the pre-fit lists will be in the form of ndarrays (-> we may have them as lists).
            self.classes_ = np.array(self.pre_fit_classes) if self.pre_fit_classes is not None else None
            self.target_mean_ = self.pre_fit_target_mean
            if self.pre_fit_encodings is not None:
                self.encodings_ = []
                for pre_fit_encoding in self.pre_fit_encodings:
                    self.encodings_.append(np.array(pre_fit_encoding))
            else:
    ...
    @_fit_context(prefer_skip_nested_validation=True)
    def fit_transform_orig_safe(self, X, y, sample_weight=None):  # DH Specific: method name change + "cv" safety
        ...
        # DH Specific "cv"-safety: our own addition here prevents the situation where cross-validation is applied
        # on an input where we have exactly one member for each class in classification tasks.
        # In this extremely rare input, seems like the only way to prevent an error is here, inside the
        # fit_transform() method - which, upon having encountered such situation, will simply run fit() + transform(),
        # preventing the usage of cross-validation whatsoever.
        if self.classes_ is not None and len(y) <= len(self.classes_):
            _, class_counts = unique(y, return_counts=True)
            if np.all(self.cv > class_counts):
                X_out = self.transform(X)
                return X_out
