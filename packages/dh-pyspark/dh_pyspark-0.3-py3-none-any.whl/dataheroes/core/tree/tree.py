...
class CoresetTree:
    ...
    def _update_chunks_params(self, chunk_size, coreset_size):
        ...
        if coreset_size is not None and self.is_mutable_coreset_size:
    ...
    def add_leaf(self, chunk_node: ChunkNode, coreset=None, random_sample_indexes=None, leaf_index=None):
        ...
        if self.data_manager.n_jobs is not None and self.data_manager.n_jobs > 1:
            ...
            def _check_if_children_exist(node_level, node_idx):
                max_batch_size = evaluate_max_batch_size(n_features=self.n_features, available=True, dtype=dtype)
    ...
    def plot(self, path: Union[str, os.PathLike] = None, name: str = None) -> os.PathLike:
        """
        Produce a tree graph plot and save the figure in the OS.
        ...
        Input:
            path: {str} -- OS Path to save the plot figure in; if not provided, or if isn't valid/doesn't exist, the figure will be saved in the current directory (from which this method is called).
        ...
        full_path = path.joinpath(f_name)
        # TODO:
        ...
        # plt.show()  # uncomment to pop-up a plot
        return full_path

    def safe_plot(self, path: Union[str, os.PathLike] = None, name: str = None) -> Optional[os.PathLike]:
        try:
