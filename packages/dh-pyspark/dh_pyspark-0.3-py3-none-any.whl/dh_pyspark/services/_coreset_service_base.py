...
class CoresetTreeServiceBase:
    ...
    def _get_categories_lengths(self):
        ...
        full_categories_loaded = pd.read_parquet(
            os.path.join(self.service_params.dhspark_path, 'string_indexer', 'all', 'categories.parquet')
        )
    ...
    def _process_features(self, df, categories_lengths, sparse=True, categorical_features_suffix=False):
        ...
        combine_udf = f.udf(
            lambda *cols: combine_features(
                numeric_values=cols[:len(self.service_params.numeric_columns)],
                categorical_values=cols[len(self.service_params.numeric_columns):],
    ...
    def has_categorical(self):
        return self.service_params.categorical_columns is None or len(self.service_params.categorical_columns) > 0
