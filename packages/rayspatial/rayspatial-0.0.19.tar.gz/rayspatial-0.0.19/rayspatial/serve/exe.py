...
class ServeExecute:
    ...
    def update_header(header):
        ...
        if engine_config.scale is not None:
            ...
            print(f"scale:{engine_config.scale}")
        ...
        for index, executeOpera in tqdm(
            ...
            if executeOpera.order in cur_res_map.keys():
                ...
                try:
                    ...
                    logger.error(f"execute error :router:{executeOpera.router}, order:{executeOpera.order}, error: params not valid, error:{e}, inputParams:{inputParams}")
                    raise e
