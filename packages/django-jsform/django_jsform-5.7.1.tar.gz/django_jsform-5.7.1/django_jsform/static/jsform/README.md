...
### jsformuploadprogress

This event is fired periodically during an upload when the request receives more data. `event.detail` will be an object with the following properties:

- **loaded**: the number of bytes that have been uploaded
- **total**: the total number of bytes to be uploaded
