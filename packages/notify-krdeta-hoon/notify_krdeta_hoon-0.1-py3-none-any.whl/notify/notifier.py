...
def notify_on_completion(slack_webhook_url='https://hooks.slack.com/services/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', email_recipients=None, smtp_config=None):
    def decorator(func):
...
def just_notify(message='',slack_webhook_url='https://hooks.slack.com/services/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'):
    """Send a desktop notification using notify.sh."""
    # Get the correct path of notify.sh inside the installed package
    with pkg_resources.path("notify", "notify.sh") as script_path:
        subprocess.run([str(script_path), message], shell=False)
