...
class ControlType(BaseEnum):
    ...
    TABLE_ROW = ('TableRow', '表格行') # TableRow
    DATA_ITEM = ('DataItem', '数据项') # DataItem
    ...
    HEADER = ('Header', '表格标题')  # Header
    FILE = ('File', '文件') # 文件操作，虚拟控件
    ALL = ('*', '全部') # 全部控件
    ...
    def get_by_element(cls, element):
        ...
        return ControlType.of_code(type_name)

    def is_composite(self):
        """
        判定是否为复合控件
        """
        return self in {ControlType.MENU, ControlType.COMBO_BOX,
