...
class ASTInterpreter:
    def __init__(
        ...
        self.loop = None  # Do not initialize with asyncio.get_event_loop() to avoid DeprecationWarning
    ...
    async def visit_Module(self, node: ast.Module, wrap_exceptions: bool = True) -> Any:
        ...
        for stmt in node.body:
            last_value = await self.visit(stmt, wrap_exceptions=wrap_exceptions)
        return last_value
