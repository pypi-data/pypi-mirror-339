...
class Snowflake:
    ...
    def next_id(self, worker_id, sequence, timestamp):
        with self.lock:
            sequence = sequence + 1
...
# 按雪花算法生成随机数
def gen_snowflake(snowflake: Snowflake, worker_id: int, sequence, redis_client_util: RedisClientUtil):
    return snowflake.next_id(worker_id, sequence, redis_client_util.get_redis_time_mill_second())
