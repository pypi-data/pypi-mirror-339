...
class BaseJobExecutor(base.Base):
    ...
    def action_btn(self, data, css_class=None):
        return web.button_js(data.get('title', "Execute"), 'exec_job('+json.dumps(data)+');', css_class=css_class)

    def action_btn_parametric(self, title, data, on_success='null', css_class=None):
    ...
    def ui_runner_link(self, type, spec, params={}):
...
class MqttSend(BaseJobExecutor):
    ...
    def part_index(self, p: base.Page, params={}):
        ...
        p(self.action_btn_parametric("Subscribe", {'type': self.type, 'topic': '#topic', 'message': '#message'}))
        p.hr()
