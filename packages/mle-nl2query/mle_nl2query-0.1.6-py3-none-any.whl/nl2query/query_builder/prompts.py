...
def get_query_builder_prompt(
    query, prompt=None, intent_json=None, examples=None, schema_name=None
):
    ...
    if intent_json:
        prompt_parts.append(f"intent_JSON:\n{json.dumps(intent_json, indent=2)}")
        examples_text = "\n".join(examples) if examples else EXAMPLE_WITH_INTENT_JSON
        prompt_parts.append(f"Examples:\n{examples_text}")
        prompt_parts = [
            part.replace("{", "{{").replace("}", "}}") for part in prompt_parts
        ]
    else:
        prompt_parts.append(f"Natural language query: {query}")
