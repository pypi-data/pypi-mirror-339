...
def kms_new_key(alias: str) -> str:
    ...
    """
    logger.info(f"Creating KMS key with alias {alias}")
    logger.debug(f"KMS Client: {kms_client}")
    # Create the KMS key
    try:
        ...
        logger.debug(f"Create Key Response: {response}")
        ...
        logger.info(f"Created KMS key with alias alias/{alias} and key ID {key_id}")
        return key_id
