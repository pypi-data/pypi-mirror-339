...
class MutationService:
    ...
    def get_gene_mutation(self, input_hgvs: str) -> GeneMutation:
        ...
        try:
            ...
            transcript, mutation_detail = self.extract_and_validate_transcript(input_hgvs)
            ...
            logger.error(f"Error processing mutation {input_hgvs}: {e} with warnings: {validation_warnings}")
            raise Exception(
                f"VariantValidator error: {e} with warnings: {validation_warnings}"
            )
        except Exception as e:
            validation_warnings = unwrapped_initial.get("validation_warnings", [])
            logger.error(f"Unexpected error processing mutation {input_hgvs}: {e} with warnings: {validation_warnings}")
            raise Exception(
                f"Unexpected error: {e} with warnings: {validation_warnings}"
            ) from e



    def get_gene_mutation_unique(self, input_hgvs: str) -> GeneMutation:
        """
        Given an input HGVS mutation, obtain a mapped and normalized GeneMutation
        using only the primary transcript.

        Steps:
        1. Extract the transcript and mutation details from the input.
        2. Call VariantValidator with the input transcript to normalize the mutation.
        3. Unwrap the response and extract the genomic coordinate.
        4. Map the API response to a TranscriptMutation for the primary transcript.
        5. Build and return a GeneMutation with secondary_transcript left empty.
        """
        # Initialize an empty dict for use in exception handling.
        unwrapped_response = {}
        try:
            logger.info(f"Processing mutation: {input_hgvs}")
            transcript, mutation_detail = self.extract_and_validate_transcript(input_hgvs)
            ...
            # Single API call using the extracted transcript.
            response = self.variantvalidator_adapter.normalize_mutation(
                variant_description=f"{transcript}:{mutation_detail}",
                select_transcripts=transcript
            )
            if response.get("messages"):
                raise Exception(f"VariantValidator error: {response['messages']}")
            
            # Unwrap the response and extract the genomic coordinate.
            unwrapped_response = self.mapper.unwrap_response(response)
            genomic_coordinate = self.mapper.extract_genomic_coordinate(unwrapped_response, self.target_assembly)
            
            # Map the gene variant using the current mapper.
            mapped_variant = self.mapper.map_gene_variant(unwrapped_response)
            
            primary_transcript_obj = TranscriptMutation(
                hgvs_transcript_variant=mapped_variant["hgvs_transcript_variant"],
                protein_consequence_tlr=mapped_variant["predicted_protein_consequence_tlr"],
                protein_consequence_slr=mapped_variant["predicted_protein_consequence_slr"]
            )
            ...
            return GeneMutation(
                ...
                genomic_coordinate=genomic_coordinate,
                primary_transcript=primary_transcript_obj,
                secondary_transcript=None  # Secondary transcript is left empty.
            )
        except VariantValidatorNormalizationError as e:
            validation_warnings = unwrapped_response.get("validation_warnings", [])
            logger.error(f"Error processing mutation {input_hgvs}: {e} with warnings: {validation_warnings}")
            raise Exception(f"VariantValidator error: {e} with warnings: {validation_warnings}")
        except Exception as e:
            validation_warnings = unwrapped_response.get("validation_warnings", [])
            logger.error(f"Unexpected error processing mutation {input_hgvs}: {e} with warnings: {validation_warnings}")
            raise Exception(f"Unexpected error: {e} with warnings: {validation_warnings}") from e
