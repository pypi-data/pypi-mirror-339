...
class QueryBuilder:
    ...
    def join(
        ...
        if isinstance(table_or_query, QueryBuilder):
            ...
            alias = alias or f"sub_{len(self._joins)}"
            self._joins.append(
                f"JOIN ({subquery_sql}) AS {alias} ON {left} = {right}"
            )
        else:
            self._joins.append(
                f"JOIN {table_or_query} ON {left} = {right}"
            )
        return self
    ...
    def left_join(
        ...
        if isinstance(table_or_query, QueryBuilder):
            subquery_sql = table_or_query._build_query()
            alias = alias or f"left_sub_{len(self._joins)}"
            self._joins.append(
    ...
    def right_join(
        ...
        if isinstance(table_or_query, QueryBuilder):
            subquery_sql = table_or_query._build_query()
            alias = alias or f"right_sub_{len(self._joins)}"
            self._joins.append(
    ...
    def cross_join(
        ...
        if isinstance(table_or_query, QueryBuilder):
            subquery_sql = table_or_query._build_query()
            alias = alias or f"cross_sub_{len(self._joins)}"
            self._joins.append(
                f"CROSS JOIN ({subquery_sql}) AS {alias}"
            )
        else:
            self._joins.append(f"CROSS JOIN {table_or_query}")
        return self
