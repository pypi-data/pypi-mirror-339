...
from .config import (
    STORAGE_DIR, STRUCTURE_FILE, CALLS_FILE, CALL_GRAPH_FILE, 
    MERMAID_HEADER, load_settings
)
...
def generate_call_graph(base_path: str = "."):
    ...
    for root, _, files in os.walk(base_path):
        ...
        for file in files:
            ...
            calls = extract_call_relations(abs_path, defined_funcs=defined, exclude_builtins=exclude_builtins)
...
def extract_structure_hierarchy(filepath):
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef):
            ...
            hierarchy.append(("method" if is_method else "function", node.name, ast.get_docstring(node) or "",
...
def extract_defined_names(filepath):
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        tree = ast.parse(f.read(), filename=filepath)
    return {n.name for n in ast.walk(tree) if
            isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef)) and not n.name.startswith("__")}
