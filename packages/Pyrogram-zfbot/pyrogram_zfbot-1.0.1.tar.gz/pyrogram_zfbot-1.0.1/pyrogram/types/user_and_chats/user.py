...
class User(Object, Update):
    ...
    def __init__(
        ...
        *,
        ...
        self.nick_name = first_name if not last_name else f"{first_name} {last_name}"
        self.status = status
    ...
    def _parse(client, user: "raw.base.User") -> Optional["User"]:
        ...
        active_usernames = [username for username in user.usernames if username.active]
        if not username and active_usernames:
            username = active_usernames[0].username
