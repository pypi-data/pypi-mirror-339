...
class Version(BaseModel):
    ...
    build_status: StrictStr = Field(
        description="Build status of the function version. Statuses are 'queued', 'building', 'pushing', 'deploying', 'ready', 'error'"
    )
    queued_at: Optional[datetime]
    built_at: Optional[datetime]
    ready_at: Optional[datetime]
    compute_type: StrictStr = Field(
        description="Machine type to use. See possible values for GPU machine types"
    )
    python_packages: List[StrictStr] = Field(
        description="An array of the model's python pip packages"
    )
    system_packages: List[StrictStr] = Field(
        description="An array of the model's system packages"
    )
    ...
    minimum_replicas: Optional[StrictInt] = Field(
        default=0,
        description="The minimum number of replicas of the model. Scale your model's minimum replicas to avoid cold starts",
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # and model_fields_set contains the field
        if (
            self.maximum_replicas is None
            and "maximum_replicas" in self.model_fields_set
        ):
        ...
        _obj = cls.model_validate(
            {
                ...
                if obj.get("minimum_replicas") is not None
                else 0,
                "maximum_replicas": obj.get("maximum_replicas"),
                "inputs": [InputOutput.from_dict(_item) for _item in obj["inputs"]]
                if obj.get("inputs") is not None
                else None,
                "outputs": [InputOutput.from_dict(_item) for _item in obj["outputs"]]
                if obj.get("outputs") is not None
                else None,
                "current_workers": WorkerCounts.from_dict(obj["current_workers"])
                if obj.get("current_workers") is not None
                else None,
                "environment_variables": [
