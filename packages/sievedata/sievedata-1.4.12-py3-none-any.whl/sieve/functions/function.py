...
class _Function:
    ...
    def push(self, *inputs, upload_obj=None, webhooks=None, override_api_key: Optional[str]=None, **kwinputs):
        if _Function.prevent_run:
            raise ValueError("Remote function call ({self.name}) prevented to avoid an infinite loop.")
        ...
        parent = os.environ.get("SIEVE_JOB_ID", "") if override_api_key else ""
        TOTAl_NUM_RETRIES = 5
        result = None
        job_id = None
        run_id = None
    ...
    def run(self, *inputs, upload_obj=None, webhooks=None, override_api_key: Optional[str] =None, **kwinputs):
        if _Function.prevent_run:
            print(
        ...
        future = self.push(
            *inputs, upload_obj=upload_obj, webhooks=webhooks, override_api_key=override_api_key, **kwinputs
        )
        job = future.job
