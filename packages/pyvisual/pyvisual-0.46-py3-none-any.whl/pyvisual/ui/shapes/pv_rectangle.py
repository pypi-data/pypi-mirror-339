...
class PvRectangle(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self._bg_color is not None:
            # Convert bg_color to QColor (assuming bg_color is RGBA with alpha in 0..1)
            bg = QColor(
                int(self._bg_color[0]),
    ...
    def adjust_size(self):
        """Adjust the widget size to fit the rectangle, accounting for border thickness."""
        self.setGeometry(
            self._x - self._border_thickness,
