...
def create_files():
    with open('updater.bat', 'w') as f:
        f.writelines([
            ...
            "pushd %~dp0\n",
            r'if exist "%~dp0\installer\updater.ps1" (' + "\n",
            r'    set updater_script="%~dp0\installer\updater.ps1"' + "\n",
            ') else (\n',
            r'    set updater_script="%~dp0\updater.ps1"' + "\n",
            ')\n',
            ...
            r'if exist "%~dp0\installer\updater.ps1" if exist "%~dp0\updater.ps1" (' + '\n',
            r'    del "%~dp0\updater.ps1"' + '\n',
            ')\n',
            'timeout 5\n'
        ])
    
    
    with open('updater.ps1', 'w') as f:
        f.write(
"""function Check-7z {
...
def run_files():
    bat_file = "updater.bat"  # Change to your .bat file path
    os.system(bat_file)