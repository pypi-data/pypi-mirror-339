...
class n_valveServerQuery:
    ...
    已解决并发问题, 无需考虑可直接调用
    简单用法
        apikey = "your_steamWebApikey"  # 如果使用 A2S 则传入None即可
        timeout = 5
        nvq = n_valveServerQuery(timeout,'utf-8', apikey)
        info = nvq.query_servers([('server1_ip',server2_port),('server2_ip',server2_port)], query_type.A2S)
    ...
    def _batched(self, iterable, n):
        ...
        while batch := list(islice(it, n)):
            yield batch
    def query_servers(self, addresses: list, q_type: query_type, interval:float = 0.0) -> list[server_info]:
        # 根据查询类型选择适当的处理函数
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            # 分组处理
            for group in self._batched(addresses, group_size):
                # 提交当前组的任务
