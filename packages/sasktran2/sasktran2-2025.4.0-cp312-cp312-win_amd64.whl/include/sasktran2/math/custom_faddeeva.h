...
#define C(a, b) cmplx(a, b)
    ...
    static inline double sinc(double x, double sinx) {
        return fabs(x) < 1e-4 ? 1 - (0.1666666666666666666667) * x * x
                              : sinx / x;
    }
    ...
    inline double faddeeva_re_erfcx(double x) {
        if (x >= 0) {
    ...
    inline cmplx faddeeva_w(double xs, double y) {
        ...
        if (ya > 7 || (x > 6 // continued fraction is faster
            ...
            /* Poppe & Wijers suggest using a number of terms
               ...
               that the estimated nu be >= minimum nu to attain machine
        ...
        /* Note: The test that seems to be suggested in the paper is x <
           ...
           below.  Therefore, we use x < 10 here. */
            ...
            /* Somewhat ugly copy-and-paste duplication here, but I see
               significant speedups from using the special-case code with the
               precomputed exponential, and the x < 5e-4 special case is needed
               for accuracy. */

            if (x < 5e-4) { // compute sum4 and sum5 together as sum5-sum4
                const double x2 = x * x;
                expx2 = 1 - x2 * (1 - 0.5 * x2); // exp(-x*x) via Taylor
                // compute exp(2*a*x) and exp(-2*a*x) via Taylor, to double
                // precision
            ...
            // (round instead of ceil as in original paper; note that x/a > 1
            // here)
            double n0 =
                floor(x / a + 0.5); // sum in both directions, starting at n0
            double dx = a * n0 - x;
    ...
    finish:
        return ret + C((0.5 * c) * y * (sum2 + sum3),
                       (0.5 * c) * copysign(sum5 - sum4, x));
    }

    inline double faddeeva_w_real(double xs, double y) {
        double a, a2, c;
        a = 0.518321480430085929872;  // pi / sqrt(-log(eps*0.5))
        c = 0.329973702884629072537;  // (2/pi) * a;
        a2 = 0.268657157075235951582; // a^2
        ...
        if (y > 7 || (x > 6 // continued fraction is faster
            ...
            if (x + y > 4000) {                                  // nu <= 2
                if (x + y > 1e7) { // nu == 1, w(z) = i/sqrt(pi) / z
                    ...
                    if (x > y) {
                        double yax = y / xs;
                        double denom = ispi / (xs + yax * y);
                        return denom * yax;
                    } else {
                        double xya = xs / y;
                        double denom = ispi / (xya * xs + y);
                        return denom;
                    }
                } else { // nu == 2, w(z) = i/sqrt(pi) * z / (z*z - 0.5)
                    double dr = xs * xs - y * y - 0.5, di = 2 * xs * y;
                    double denom = ispi / (dr * dr + di * di);

                    return denom * (xs * di - y * dr);
                }
            } else { // compute nu(z) estimate and do general continued fraction
                const double c0 = 3.9, c1 = 11.398, c2 = 0.08254, c3 = 0.1421,
                             c4 = 0.2023; // fit
                double nu = floor(c0 + c1 / (c2 * x + c3 * y + c4));
                double wr = xs, wi = y;
                for (nu = 0.5 * (nu - 1); nu > 0.4; nu -= 0.5) {
                    // w <- z - nu/w:
                    double denom = nu / (wr * wr + wi * wi);
        ...
        /* Note: The test that seems to be suggested in the paper is x <
           ...
           below.  Therefore, we use x < 10 here. */
            ...
            if (y > 5) { // imaginary terms cancel
                ...
                return (expx2erfcxy - c * y * sum1) * cos(2 * x * y);
            } else {
                const double sinxy = sin(xs * y);
                const double sin2xy = sin(2 * xs * y), cos2xy = cos(2 * xs * y);
                const double coef1 = expx2erfcxy - c * y * sum1;
                const double coef2 = c * xs * expx2;
                return coef1 * cos2xy + coef2 * sinxy * sinc(xs * y, sinxy);
            }
        } else { // x large: only sum3 & sum5 contribute (see above note)
            // (round instead of ceil as in original paper; note that x/a > 1
            // here)
            ...
            for (dn = 1; n0 - dn > 0; ++dn) { // loop over n0-dn and n0+dn terms
                ...
                sum5 += a * (np * tp + nm * tm);
                if (a * (np * tp + nm * tm) < DBL_EPSILON * sum5)
                    return (0.5 * c) * y * (sum2 + sum3) + exp(-x * x);
            }
            while (1) { // loop over n0+dn terms only (since n0-dn <= 0)
                double np = n0 + dn++;
                double tp = exp(-sqr(a * dn + dx)) / (a2 * (np * np) + y * y);
