...
namespace sasktran2::math {
    const double TWOPI =
        6.283185307179586476925287; //@define Value of 2.PI to 24 decimal places
    const double TwoPi =
        6.283185307179586476925287; //@define Value of 2.PI to 24 decimal places
    const double Pi =
        3.141592653589793238462643; //@define Value of PI to 24 decimal places
    const double PiOver2 =
    ...
    inline double cosd(double x) { return cos(x * ONE_DEGREE); }
    ...
    inline double sind(double x) { return sin(x * ONE_DEGREE); }
    ...
    inline double PowerOf10Magnitude(double x, int* firstdigit) {
        ...
        if (x == 0) {
            ...
            p10 =
                pow(10.0,
                    floor(log10(ax))); // Get the power of 10 of the data range
            digit = (int)(ax / p10);   // get the first digit.
            if (digit ==
                10) // Numerical round off might make 9.99999999999 equal to 10
            {       // if it does
                p10++;     // then increase the power of 10
                digit = 1; // and set the digit equal to 0
            }
            if (digit ==
                0) // Numerical round off may truncate 0.99999999999 to zero
            {      // if it does
