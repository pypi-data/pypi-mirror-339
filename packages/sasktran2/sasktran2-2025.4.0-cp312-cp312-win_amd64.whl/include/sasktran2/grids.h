...
namespace sasktran2::grids {
     ...
     *
     * 'constant' indicates that all grid points are evenly spaced.  If this is
     * the case interpolation speed can be greatly improved
     *
     * 'variable' indicates that the spacing may be variable between grid
     * points.  If this is the case then usually a binary search has to be
     * performed to find interpolation indicies.
     *
     */
    enum gridspacing { constant, variable, automatic };
     ...
     *
     */
    enum outofbounds { extend, setzero };

    /** Generic class that implements all of the standard functionality for a
     * one-dimensional grid to interpolate over.
     */
    class Grid {
      private:
        ...
        double m_x0; /**< Set when m_grid spacing is constant, set to the first
                        value of the grid */
         ...
         */
        void interpolate_constant_spacing(double x, std::array<int, 2>& index,
         ...
         * @param num_contributing Number of constributing interpolation points
         */
        void interpolate_varying_spacing(double x, std::array<int, 2>& index,
                                         std::array<double, 2>& weight,
                                         int& num_contributing) const;
      ...
      public:
         ...
         *
         * @param grid_values The vector of points defining the grid, it is
         * moved into the class upon creation
         * @param spacing The spacing mode, either constant or varying.  Set to
         * constant if the grid is uniformly spaced
         * @param out_of_bounds_mode Out of bounds mode, either extend or
         * setzero.  Extend will always use the last/first value for out of
         * bounds values, setzero will set it to 0.
         * @param interp Interpolation mode, either linear or constant.
         */
        Grid(Eigen::VectorXd&& grid_values, gridspacing spacing,
             outofbounds out_of_bounds_mode, interpolation interp);

        /** Calculates the interpolation indicies and weights.  The number of
         * contributing indices is stored in num_contributing, the maximum
         * number of contributing points is 2.  Note that it may be 0 if the
         * interpolated value is always 0.
         *
         * @param x Value to interpolate to
         * @param index The indicies in the grid
         * @param weight The corresponding weights to the indicies
         * @param num_contributing The number of contributing points to the
         * interpolation
         */
    ...
    class AltitudeGrid : public Grid {
      public:
         ...
         * @param interp Interpolation mode, either linear or constant.
         */
        AltitudeGrid(Eigen::VectorXd&& grid_values, gridspacing spacing,
                     outofbounds out_of_bounds_mode, interpolation interp)
            : Grid(std::forward<Eigen::VectorXd&&>(grid_values), spacing,
                   out_of_bounds_mode, interp) {}
    };

    /** A convenience class which combines multiple grids to provide
     * interpolation scheme for source functions. Note this includes the
     * location component of the source function but not the directional
     * component.
     *
     *  Examples are, in a spherically symmetric atmosphere the source function
     * depends only on solar zenith angle and altitude.
     *
     *  In a plane parallel horizontally homogenous atmosphere, the source
     * function depends only on altitude.
     *
    ...
    class SourceLocationInterpolator {
      ...
      public:
        ...
        grid_location(const sasktran2::Coordinates& coords,
                      int location_index) const = 0;
        virtual Eigen::Vector3d
        ground_location(const sasktran2::Coordinates& coords,
                        int ground_index) const = 0;
        virtual void interior_interpolation_weights(
            const sasktran2::Coordinates& coords,
            const sasktran2::Location& location,
            std::vector<std::pair<int, double>>& weights, int& num_interp) = 0;
        virtual void ground_interpolation_weights(
            const sasktran2::Coordinates& coords,
            const sasktran2::Location& location,
            std::vector<std::pair<int, double>>& weights,
            int& num_interp) const = 0;
    };

    class AltitudeSZASourceLocationInterpolator
