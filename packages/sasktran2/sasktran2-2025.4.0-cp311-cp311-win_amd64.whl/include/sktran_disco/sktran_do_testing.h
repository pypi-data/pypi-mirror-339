...
namespace sasktran_disco {
    namespace testing {
        ...
        struct TestLayerSpecRayleigh : public TestLayerSpecBase {
            ...
            TestLayerSpecRayleigh(double optical_depth, double ssa,
                                  double depol) {
                this->optical_depth = optical_depth;
                this->ssa = ssa;
                this->depol = depol;
        ...
        template <int NSTOKES, int CNSTR = -1> class TestLayer {
          public:
            TestLayer(uint NSTR, const TestLayerSpecBase& spec) {
                if (dynamic_cast<const TestLayerSpecHG*>(&spec) != nullptr) {
                    assignHGLayer(NSTR,
                                  dynamic_cast<const TestLayerSpecHG&>(spec));
                } else if (dynamic_cast<const TestLayerSpecRayleigh*>(&spec) !=
                           nullptr) {
                    assignRayleighLayer(
                        NSTR, dynamic_cast<const TestLayerSpecRayleigh&>(spec));
                } else if (dynamic_cast<const TestLayerSpecSiewert*>(&spec) !=
                           nullptr) {
                    assignSiewertLayer(
                        NSTR, dynamic_cast<const TestLayerSpecSiewert&>(spec));
                }
            }
          ...
          private:
            ...
            void assignRayleighLayer(uint NSTR,
                                     const TestLayerSpecRayleigh& spec);
            void assignSiewertLayer(uint NSTR,
                                    const TestLayerSpecSiewert& spec);
        };
        ...
        template <int NSTOKES, int CNSTR = -1> class TestAtmosphere {
          public:
            TestAtmosphere(std::vector<TestLayerSpecHG>&& layers) {
                m_layershg.reserve(layers.size());
                for (TestLayerSpecHG& layer : layers)
                    m_layershg.emplace_back(layer);
            }
            TestAtmosphere(std::vector<TestLayerSpecRayleigh>&& layers) {
                m_layersray.reserve(layers.size());
                for (TestLayerSpecRayleigh& layer : layers)
                    m_layersray.emplace_back(layer);
            }
            TestAtmosphere(std::vector<TestLayerSpecSiewert>&& layers) {
                m_layersray.reserve(layers.size());
                for (TestLayerSpecSiewert& layer : layers)
                    m_layerssiewert.emplace_back(layer);
            }
        ...
        template <int NSTOKES, int CNSTR = -1> struct TestCase {
            TestCase(uint NSTR, TestSolarSpec& solar_spec,
                     TestAtmosphere<NSTOKES> atmo, double ALBEDO,
