...
namespace sasktran2 {
    /** Enum defining the type of geometry used in the calculation.  Possible
     * options are plane parralel, spherical, and ellipsoidal.  Note that
     ...
     * radius.  The location can also hold optional information to aid in
     *  interpolation.
     */
    struct Location {
        Eigen::Vector3d position; /**< x,y,z location coordinates */

        bool on_exact_altitude; /**< OPTIONAL.  True if the location is on an
                                   exact grid point of the global altitude grid
                                 */
        int lower_alt_index;    /**< OPTIONAL.  the lower altitude index of this
    ...
    /** Defines the coordinate system used for the radiative transfer
     * calculation.  We define Local Coordinates as a cartesian coordinate
     * system defined by x=reference_plane unit vector, z=reference_point unit
     * vector, and y = z cross x.  Local cartesian coordinates are used when
     * using plane parallel mode.
     *
     *  Local angular coordinates are defined as, for a positional vector v, |r|
     * = |v| - reference point radius, theta = angle of the point projected into
     * the plane formed by x and z, phi = angle of the point projected into the
     * plane formed by y and z
     *
     *  All radiative transfer calculations internally happen on this local
     * coordinate system, which includes specification of the atmosphere.
     *
     *  For most radiative transfer calculations we need a singular sun
     * position, the Coordinates object is also responsible for creating this
     * sun unit vector.
     *
     *  The preferred method of constructing the Coordinates is to only specify
     * the solar zenith angle and solar azimuth angle at the
     */
    class Coordinates {
      private:
      ...
      public:
         ...
         * constructing the geometry.  Here solar azimuth angle is defined
         * relative to the reference plane direction.  For 1d calculations this
         * is typically ambiguous, and it may be preferred to simply set the saa
         * to 0, then the relative azimuth angle between any line of sight and
         * the sun is determined from the line of sight azimuth angles.  If all
         * lines of sight are expected to have the same relative azimuth angle
         * you may want to set the solar azimuth here to a non-zero value and
         * leave the line of sight viewing azimuth at 0.
         *
         *  For 2d/3d calculations it is necessary to properly set the solar
         * azimuth angle relative to the viewing plane, since here the viewing
         * plane also influences the definition of the 2d/3d atmosphere. For
         * example in a 2d calculation where all lines of sight are within the
         * direction the atmosphere varies, you would specify the line of sight
         * azimuth of 0, and set the solar azimuth to be the relative azimuth.
         *
         *
         * @param cos_sza Cosine of solar zenith angle
         * @param saa Solar azimuth angle in [radians]
         * @param earth_radius Earth radius in [m]
         * @param geotype Type of geometry (spherical or plane parallel)
         * defaults to spherical
         * @param force_sun_z Set to true to force the sun unit vector to be (0,
         * 0, 1)
         */
        Coordinates(double cos_sza, double saa, double earth_radius,
                    geometrytype geotype = geometrytype::spherical,
                    bool force_sun_z = false);

        /** Constructs the coordinates by manually specifying the unit vectors
         * for the reference point and planes.
         *
         * @param ref_point_unit Unit vector to the reference point, becomes m_z
         * @param ref_plane_unit Unit vector along the reference plane, becomes
         * m_x
        ...
        /** Calculates a unit vector in the local coordinate system from the
         * angles (theta, phi).
         *
        ...
        local_x_y_from_angles(double theta, double phi) const;
        ...
        angles_from_unit_vector(const Eigen::Vector3d& uv) const;
         ...
         */
        Eigen::Vector3d solar_coordinate_vector(double cos_sza, double saa,
     ...
     * object contains a reference to the coordinate transform, as well as
     * defines the geometry grids for the calculation.  For example, a 1d
     * geometry object will contain an altitude grid.
     */
    class Geometry {
      private:
      ...
      public:
        ...
        virtual void assign_interpolation_weights(
            const Location& loc,
            std::vector<std::pair<int, double>>& index_weights) const = 0;
    };

    /** Derived Geometry object which contains a 1D altitude grid.  Usually
     * serves as the Base geometry object in most places since almost all
     * calculations require at least an altitude grid to be implemented.
     */
