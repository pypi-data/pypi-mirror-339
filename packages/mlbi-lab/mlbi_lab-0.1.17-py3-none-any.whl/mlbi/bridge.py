...
def evaluate_roc_auc( X, y, vlst, classifier, param_grid, 
                      NCV = 5, target_cls = 'Cancer', title_t = None,
                      fpr_th = 0.01, bsr = 0.8, bsn = 40, vbs = 0,
    ...
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.xlabel("False Positive Rate = FP/(FP+TN)", fontsize = label_fs)
    plt.ylabel("True Positive Rate = TP/(TP+FN)", fontsize = label_fs)    
    if title is not None: plt.title(title_t, fontsize = title_fs )

    if show_legend:
        plt.legend(loc=legend_loc, bbox_to_anchor = bbox_to_anchor, fontsize = legend_fs)
    ...
    df_res = pd.DataFrame({'Case': groups, 'AUC': auc_all, 'Sensitivity': sens_all})
