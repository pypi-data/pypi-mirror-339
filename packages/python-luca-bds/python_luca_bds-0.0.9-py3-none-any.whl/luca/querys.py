...
def query(conn, environment, name=None, id=None, input_variables={}, page=1, size=25, commit=True, return_pd=True):
    ...
    enviroment_id = [env[0] for env in query['environments'] if env[1] == environment][0]
    if input_variables:
        try:
        ...
        input_variables = []
    return execute_query(conn=conn, id=id, environment_id=enviroment_id, input_variables=input_variables, page=page, size=size, commit=commit, return_pd=return_pd)