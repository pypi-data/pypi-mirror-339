...
class PQDataClient(object):
    ...
    def __call__(self, method, *args, **kwargs):
        ...
        max_retry = args[0] if len(args) > 0 else self.request_attempt_count  # 最多尝试次数
        result_on_except = args[1] if len(args) > 1 else None  # 如果尝试失败的返回默认结果
        retry_count = 0
        for _ in range(self.request_attempt_count):
            retry_count += 1
            try:
