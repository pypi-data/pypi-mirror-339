...
def plot_data(ax, data, title, cmap, vmin, vmax):
    """
    Helper function to plot a single dataset on a given axis.
    """
...
def plotValplusnan(epoch=0, *datasets, title=["input", "prediction", "truth"], saveDir="plotValplus", cmap='summer'):
    """
    Main function to plot multiple datasets in a grid layout.
    """
    plt.ioff()  # 禁用交互模式
    ...
    for i, (data, t) in enumerate(zip(datasets, title)):
        if i != 0:
            ...
            vmin, vmax = 150, 300
            cmap_used = cmap  #        
        plot_data(axes[i], data, t, cmap_used, vmin, vmax)    
    # Remove unused subplots
    for j in range(i + 1, len(axes)):
        fig.delaxes(axes[j])    
    # Adjust layout and save the figure
