...
class Neo4jConnection(Connection):
    ...
    def create_node(self, payload):
        ...
        if not label or not properties:
            raise ValueError({"error":"label and atleast one property must be provided to create a Node"})
        query = f"CREATE (n:{label} $props) RETURN n"
        
        with self._driver.session() as session:
            session.run(query, props=properties).single()
            return {"message":f"Node created for {label}"}
    ...
    def get_nodes(self, payload=None):
        ...
        with self._driver.session() as session:
            return [record["n"] for record in session.run(query, properties)]
