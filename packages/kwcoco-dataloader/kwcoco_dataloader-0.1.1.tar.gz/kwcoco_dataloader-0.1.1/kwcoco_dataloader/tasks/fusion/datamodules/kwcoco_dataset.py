...
class KWCocoVideoDatasetConfig(scfg.DataConfig):
    ...
    dynamic_channels = scfg.Value(None, type=str, group='sensorchan', help=ub.paragraph(
        '''
        A YAML specification to dynamically compute additional channels. An
        example might be:
...
class GetItemMixin(TruthMixin):
    ...
    def _cached_sample_sensorchan_matching_sensor(self, sensor_coarse):
        ...
        matching_sensorchan = matching_sensor(self.input_sensorchan, sensor_coarse)
    ...
    def _sample_one_frame(self, gid, sampler, coco_dset, target_, with_annots,
        ...
        for input_stream in modality_streams:
            ...
            if input_stream.spec in self._special_inputs:
                dynamic_task = self._special_inputs[matching_sensorchan.spec]
            ...
            if dynamic_task is not None:
                ...
                new_channel_lut = dict(zip(_sample_chan_names, einops.rearrange(sample['im'], 't h w c -> c t h w')))
                for dynamic_chan in dynamic_task['dynamic_chans']:
                    dynamic_info = self._dynamic_channel_lut[dynamic_chan]
                    dynamic_args = {}
                    for arg_chan in dynamic_info['args']:
                        arg_idx = _sample_chan_names.index(arg_chan)
                        arg_data = sample['im'][..., arg_idx]
                        dynamic_args[arg_chan] = arg_data
                    dynamic_args.update(dynamic_info['ns'])
                    # TODO: be safer about this
                    # Use kwutil.safeeval, but that requires explicit
                    # passlisting
                    result = eval(dynamic_info['expr'], dynamic_args)
                    new_channel_lut[dynamic_chan] = result
    ...
    def getitem(self, index):
        ...
        self._robust_normalize_frame_items(frame_items)
    ...
    def _robust_normalize_frame_items(self, frame_items):
        """
        SeeAlso:
        ...
        Example:
            ...
            >>> kwplot.show_if_requested()
        """
        if self.robust_normalizer is not None:
            # print(f'self.robust_normalizer._normalizer_items = {ub.urepr(self.robust_normalizer._normalizer_items, nl=1)}')
            self.robust_normalizer.normalize(frame_items, _debug=0)

        if self.config['normalize_perframe']:
            assert not self.robust_normalizer, 'normalize_perframe cannot be used with robust_normalize'
            # DEPRECATED
            for frame_item in frame_items:
                frame_modes = frame_item['modes']
                for mode_key in list(frame_modes.keys()):
        ...
        if self.config['normalize_peritem'] is not None and self.config['normalize_peritem'] is not False:
            assert not self.robust_normalizer, 'normalize_peritem cannot be used with robust_normalize'
            # DEPRECATED
...
class KWCocoVideoDataset(data.Dataset, GetItemMixin, BalanceMixin,
    ...
    def _init_sensorchan(self):
        ...
        self._dynamic_channels = copy.deepcopy(kwutil.Yaml.coerce(self.config['dynamic_channels']))
        # self._dynamic_channels = copy.deepcopy(self._dynamic_channels)
        self._dynamic_channel_lut = {}
        ...
        if self._dynamic_channels is not None:
            dynamic_channel_names = [r['name'] for r in self._dynamic_channels]
            for r in self._dynamic_channels:
                r['args'] = get_free_varnames(r['expr'])
                all_args = set(r['args'])
                r['args'] = list(all_args - allowed_funcnames)
                funcnames = list(all_args & allowed_funcnames)
                r['ns'] = {n: getattr(np, n) for n in funcnames}
                # TODO: can we check for unavailable names or be more robust in
                # general here?
                self._dynamic_channel_lut[r['name']] = r
        ...
        _input_sensorchans = []
        _sample_sensorchans = []
        self._special_inputs = {}  # Unused? Remove?
        for fused_sensorchan in self.sensorchan.streams():
            sensor = fused_sensorchan.sensor
            chans = fused_sensorchan.chans
            ...
            if dynamic_chans:
                ...
                _other_req = sorted(set(ub.flatten(self._dynamic_channel_lut[n]['args'] for n in dynamic_chans)))
    ...
    def _init_robust_normalizers(self):
        ...
        if self.config['robust_normalize'] is not None:
