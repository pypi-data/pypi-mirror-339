...
class EnvManager:
    ...
    def __init__(self, env_dirs = [ "./", "/content/drive/MyDrive/env/" ], env_files = [ "settings_ven.env" ] ):
        ...
        初期設定
        args:
            env_dir( str ) : .env ファイルが格納されているフォルダ( "/content/drive/MyDrive/env" を推奨)
            env_files( str ) : 読み込む .env ファイルのリスト（settings_git.env を含めることを推奨）
        """
        ...
        self.debug.log_step( f"env_dirs  :{ self.env_dirs  }", success = None )
        self.debug.log_step( f"env_files :{ self.env_files }", success = None )
        self.load_all_envs()
        ...
        for env_dir in self.env_dirs:
            for env_file in self.env_files:
                ...
                if env_path.exists():
                    ...
                    self.debug.log_step(f"{ env_path } が正常にロードされました。", success=True)
                else:
                    self.debug.log_step(f"{ env_path } が見つかりません。", success=False)
