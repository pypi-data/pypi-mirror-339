...
class EXTRACT:
            ...
            for self.TableName in self.Parameters["Objetos_Candidatos"]:
                ...
                for row in rows:
                    ...
                    if self.Origem["tipo_objeto"].upper() == "QUERY":
                        ...
                        self.QUERY = etl.getQueryExtract(obj=self.Parameters, name=self.TableName, pkgDB=con, qry=self.QUERY)
                    else:
                        self.QUERY = etl.getQueryExtract(obj=self.Parameters, name=self.TableName, pkgDB=con)

                    log.setLogEvent(content=f"""Definindo PLACEHOLDERS!""", level=lvl.INFO)
                    self.PLACEHOLDER = self.setPlaceHolders(self.QUERY)
                    ...
                    # alterando o objeto datetime para str (json)
                    if self.Delta["start_value"]:
                        self.Delta["start_value"] = self.Delta["start_value"].strftime(dtu.MILLISECONDS_FORMAT_PYTHON)
                    if self.Delta["end_value"]:
                        self.Delta["end_value"] = self.Delta["end_value"].strftime(dtu.MILLISECONDS_FORMAT_PYTHON)
                    #endregion
    ...
    def setError(self, value):
        msg, result = None, True
        try:
            self._error = gen.nvl(value, True)
        except Exception as error:
            msg = error
            result = msg
        finally:
    ...
    def getIterator(self, iterator):
        ...
        try:
            if isinstance(iterator, int):
                ...
                __qryText = etl.getQueryExtract(obj=self.Parameters, name=self.TableName, pkgDB=con.CONNECTION, qry=__qry)
                # if self.DataBaseConnection["driver_conexao"].upper() == "SQLALCHEMY":
    ...
    # ---------------------------------
    def setPlaceHolders(self, stmt: str) -> dict:
        msg, result = None, {}
        try:
            __text = None
            for key in self.RowIterator:
                ...
                if __dummy > 0:
                    result[key] = self.RowIterator[key]
        except Exception as error:
            msg = error
            result = msg
            log.setLogEvent(content=f"""{result}""", level=lvl.ERROR)
