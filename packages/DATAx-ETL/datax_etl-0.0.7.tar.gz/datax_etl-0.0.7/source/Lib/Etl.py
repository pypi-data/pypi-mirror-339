...
class ETL:
    ...
    def getTableWhere(self, origem: dict, delta: dict, filters: dict, pkgDB: object, qry:str=None):
        msg, result = None, None
        try:
            ...
            if __startValue is not None:
                if __endValue is not None:
                    if delta["type"].upper() == "DATETIME":
                        __where = f"""where {origem["alias"]}.{origem["quoted"]}{delta["coluna"]}{origem["quoted"]} 
                        between {self.DataBaseConvertDateTime(pkgDB.NOME_DATABASE, __startValue, delta["type"])}
                        and {self.DataBaseConvertDateTime(pkgDB.NOME_DATABASE, __endValue, delta["type"])}"""
                    if delta["type"].upper() == "DATE":
                        __where = f"""where {origem["alias"]}.{origem["quoted"]}{delta["coluna"]}{origem["quoted"]} 
                                                between {self.DataBaseConvertDateTime(pkgDB.NOME_DATABASE, __startValue.date(), delta["type"])}
                                                and {self.DataBaseConvertDateTime(pkgDB.NOME_DATABASE, __endValue.date(), delta["type"])}"""
            ...
            for f in filters:
                if filters[f]["flg_ativo"] == "S":
                    if filters[f]["datatype"].upper() == "STRING":
                        ...
                        __f.append(f"""{origem["alias"]}.{origem["quoted"]}{f}{origem["quoted"]} = '{self.DataBaseConvertDateTime(pkgDB.NOME_DATABASE, filters[f]["valor"], delta["type"])}'""")
                    elif filters[f]["datatype"].upper() == 'DATE':
                        __value = filters[f]["valor"]
                        __f.append(f"""{origem["alias"]}.{origem["quoted"]}{f}{origem["quoted"]} = {self.DataBaseConvertDateTime(pkgDB.NOME_DATABASE, filters[f]["valor"], delta["type"])}""")
                    else:
                        #__f.append(f"""{origem["alias"]}.{origem["quoted"]}{f}{origem["quoted"]} = '{filters[f]["valor"]}'""")
                        __f.append(f"""{origem["alias"]}.{origem["quoted"]}{f}{origem["quoted"]} = {self.DataBaseConvertDateTime(pkgDB.NOME_DATABASE, filters[f]["valor"])}""")
    ...
    def getQueryExtract(self, obj, name, pkgDB, qry:str=None):  # df, origem, delta, filters, pkgDB):
        msg, result = None, True
        try:
            ...
            if __origem["tipo_objeto"].upper() == "TABLE":
                ...
                __md = md.getMetadados(table=__origem["name"], owner=__origem["schema"], con=pkgDB.CONNECTION, driver=pkgDB.DATABASE_DRIVER)
                #__md = pkgDB.getMetadados(__origem["name"], __origem["schema"], pkgDB.CONNECTION)
                ...
                # --------------------------------------------------------------
                __where = self.getTableWhere(origem=__origem, delta=__delta, filters=__filters, pkgDB=pkgDB)
        ...
        try:
            ...
            if delta["scopo"].upper() == "ANUAL":
                if not delta["end_value"]:
                    ...
                    __lano = dt.datetime.strptime(delta["end_value"], dtu.MILLISECONDS_FORMAT_PYTHON) + dt.timedelta(
                        microseconds=1)
                    delta["start_value"] = __lano
                    delta["end_value"] = delta["start_value"] + rd.relativedelta(years=__inc) - dt.timedelta(
                        microseconds=1)
            elif delta["scopo"].upper() == "MENSAL":
                if not delta["end_value"]:
                    ...
                    delta["end_value"] = dtu.getUltimoDiaMes(__lmes) + rd.relativedelta(
                        months=__inc - 1) - dt.timedelta(microseconds=1)
                else:
                    __lmes = dt.datetime.strptime(delta["end_value"], dtu.MILLISECONDS_FORMAT_PYTHON) + dt.timedelta(
                        microseconds=1)
                    delta["start_value"] = __lmes
                    delta["end_value"] = delta["start_value"] + rd.relativedelta(months=__inc) - dt.timedelta(
                        microseconds=1)
            elif delta["scopo"].upper() == "DIARIO":
                if not delta["end_value"]:
                    __ldia = __now - dt.timedelta(days=__inc)
                    delta["start_value"] = dtu.getPrimeiraHoraDia(__ldia)
                    delta["end_value"] = dtu.getUltimaHoraDia(__ldia) + dt.timedelta(days=__inc - 1) - dt.timedelta(
                        microseconds=1)
                else:
                    __ldia = dt.datetime.strptime(delta["end_value"], dtu.MILLISECONDS_FORMAT_PYTHON) + dt.timedelta(
                        microseconds=1)
                    delta["start_value"] = __ldia
                    delta["end_value"] = delta["start_value"] + dt.timedelta(days=__inc) - dt.timedelta(microseconds=1)
            elif delta["scopo"].upper() == "HORA":
                if not delta["end_value"]:
                    __lhora = __now - dt.timedelta(hours=__inc)
                    delta["start_value"] = dtu.getPrimeiroMinutoHora(__lhora)
                    delta["end_value"] = delta["start_value"] + dt.timedelta(hours=__inc) - dt.timedelta(microseconds=1)
                else:
                    __lhora = dt.datetime.strptime(delta["end_value"], dtu.MILLISECONDS_FORMAT_PYTHON) + dt.timedelta(
                        microseconds=1)
                    delta["start_value"] = __lhora
                    delta["end_value"] = delta["start_value"] + dt.timedelta(hours=__inc) - dt.timedelta(microseconds=1)
            elif delta["scopo"].upper() == "MINUTO":
                if not delta["end_value"]:
                    __lmin = __now - dt.timedelta(minutes=__inc)
                    delta["start_value"] = dtu.getPrimeiroSegundoMinuto(__lmin)
                    delta["end_value"] = delta["start_value"] + dt.timedelta(minutes=__inc) - dt.timedelta(
                        microseconds=1)
                else:
                    __lmin = dt.datetime.strptime(delta["end_value"], dtu.MILLISECONDS_FORMAT_PYTHON) + dt.timedelta(
                        microseconds=1)
            ...
            if delta["end_value"] >= __now:
                __lmin = __now - dt.timedelta(minutes=__inc)
                delta["end_value"] = dtu.getUltimoSegundoMinuto(__lmin) + dt.timedelta(microseconds=999999)
    ...
    def getEnviroment(self):
        ...
        try:
            if os.environ.get("token") is None or os.environ.get("token_parametros"):
                __fileEnv = "C:\cloud\OneDrive\Trabalho\Projetos\Python\DataEng\DATAx\DATAx-ETL\config\env\.env"
