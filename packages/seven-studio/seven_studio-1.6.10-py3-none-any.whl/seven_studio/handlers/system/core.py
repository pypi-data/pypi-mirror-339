...
:LastEditTime: 2025-04-07 16:30:37
...
class VerifyCodeHandler(StudioBaseHandler):
    ...
    def get_async(self):
        ...
        if user_info and user_info["Phone"]:
            if check_level == "verify_code":
                return self.response_common("Check VerifyCode", "验证码验证模式")
            elif check_level == "safety" or check_level == "safety_strong":
                return self.response_common("Check All", f"安全验证模式 ip:[{self.get_remote_ip()}]")
...
class LoginPlatformHandler(StudioBaseHandler):
    ...
    def get_async(self):
        ...
        if check_level in ["safety", "safety_strong"]:
            ...
            if not user_info:
                return self.response_common("AccountNotExist", "用户不存在")
    ...
    def login(self, account="", password="", verify_code=""):
        ...
        manage_context_key = config.get_value(
            "manage_context_key", "db_sevenstudio")
        base_manage_context_key = config.get_value(
            "base_manage_context_key", "db_sevenstudio")
        ...
        if password:
            ...
            if not verify:
                ...
                invoke_result.ResultMessage = f"密码错误,尝试次数：{str(user_info.FaildLoginCount)}"
