...
class PostgresConversationClient(DatabaseClientBase):
    ...
    async def create_conversation(self, conversation_id, user_id, title=""):
        with tracer.start_as_current_span("PostgresConversationClient.create_conversation"):
            ...
            try:
                ...
                if conversation:
                    ...
                    logger.warning(f"Failed to create conversation with id: {conversation_id}")
                    return False
            except Exception as e:
                logger.error(f"Error creating conversation with id: {conversation_id}: {e}", exc_info=True)
                raise

    async def upsert_conversation(self, conversation):
        with tracer.start_as_current_span("PostgresConversationClient.upsert_conversation"):
            query = """
                INSERT INTO conversations (id, conversation_id, type, "createdAt", "updatedAt", user_id, title)
            ...
            try:
                ...
                if updated_conversation:
                    ...
                    logger.warning(f"Failed to upsert conversation with id: {conversation['id']}")
                    return False
            except Exception as e:
                logger.error(f"Error upserting conversation with id: {conversation['id']}: {e}", exc_info=True)
                raise

    async def delete_conversation(self, user_id, conversation_id):
        with tracer.start_as_current_span("PostgresConversationClient.delete_conversation"):
            ...
            try:
                ...
                logger.info(f"Conversation deleted with conversation_id: {conversation_id} and user_id: {user_id}")
                return True
            except Exception as e:
                logger.error(f"Error deleting conversation with conversation_id: {conversation_id} and user_id: {user_id}: {e}", exc_info=True)
                raise
    ...
    async def delete_messages(self, conversation_id, user_id):
        with tracer.start_as_current_span("PostgresConversationClient.delete_messages"):
            ...
            try:
                ...
                logger.info(f"Messages deleted for conversation_id: {conversation_id} and user_id: {user_id}")
                return [dict(message) for message in messages]
            except Exception as e:
                logger.error(f"Error deleting messages for conversation_id: {conversation_id} and user_id: {user_id}: {e}", exc_info=True)
                raise

    async def get_conversations(self, user_id, limit=None, sort_order="DESC", offset=0):
        with tracer.start_as_current_span("PostgresConversationClient.get_conversations"):
            try:
            ...
            if limit is not None:
                try:
                    ...
                    logger.info(f"Retrieved conversations for user_id: {user_id} with limit: {limit} and offset: {offset}")
                except ValueError:
                ...
                logger.info(f"Retrieved conversations for user_id: {user_id} without limit and offset")
            return [dict(conversation) for conversation in conversations]

    async def get_conversation(self, user_id, conversation_id):
        with tracer.start_as_current_span("PostgresConversationClient.get_conversation"):
            ...
            try:
                ...
                if conversation:
                    logger.info(f"Retrieved conversation with id: {conversation_id} and user_id: {user_id}")
                    return dict(conversation)
                else:
                    logger.warning(f"No conversation found with id: {conversation_id} and user_id: {user_id}")
                    return None
            except Exception as e:
                logger.error(f"Error retrieving conversation with id: {conversation_id} and user_id: {user_id}: {e}", exc_info=True)
                raise
        ...
        with tracer.start_as_current_span("PostgresConversationClient.create_message"):
            ...
            try:
                ...
                if message:
                    ...
                    logger.info(f"Message created with id: {message_id} in conversation: {conversation_id}")
                    return dict(message)
                else:
                    logger.warning(f"Failed to create message with id: {message_id} in conversation: {conversation_id}")
                    return False
            except Exception as e:
                logger.error(f"Error creating message with id: {message_id} in conversation: {conversation_id}: {e}", exc_info=True)
                raise

    async def update_message_feedback(self, user_id, message_id, feedback):
        with tracer.start_as_current_span("PostgresConversationClient.update_message_feedback"):
            ...
            try:
                ...
                if message:
                    logger.info(f"Message feedback updated for message_id: {message_id} and user_id: {user_id}")
                    return dict(message)
                else:
                    logger.warning(f"Failed to update message feedback for message_id: {message_id} and user_id: {user_id}")
                    return False
            except Exception as e:
                logger.error(f"Error updating message feedback for message_id: {message_id} and user_id: {user_id}: {e}", exc_info=True)
                raise
    ...
    async def get_messages(self, user_id, conversation_id):
        with tracer.start_as_current_span("PostgresConversationClient.get_messages"):
            ...
            try:
                messages = await self.conn.fetch(query, conversation_id, user_id)
                logger.info(f"Retrieved messages for conversation_id: {conversation_id} and user_id: {user_id}")
                return [dict(message) for message in messages]
            except Exception as e:
                logger.error(f"Error retrieving messages for conversation_id: {conversation_id} and user_id: {user_id}: {e}", exc_info=True)
                raise