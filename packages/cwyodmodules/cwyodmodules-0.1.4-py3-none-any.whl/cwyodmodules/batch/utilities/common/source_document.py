...
class SourceDocument:
    def __init__(
        ...
        logger.debug(f"SourceDocument initialized with id: {self.id}, source: {self.source}")
    ...
    def from_json(cls, json_string):
        with tracer.start_as_current_span("SourceDocument.from_json"):
    ...
    def from_dict(cls, dict_obj):
        with tracer.start_as_current_span("SourceDocument.from_dict"):
    ...
    def from_metadata(
        ...
        with tracer.start_as_current_span("SourceDocument.from_metadata"):
            logger.debug(f"Creating SourceDocument from metadata. document_url:{document_url}, metadata:{metadata}")
    ...
    def get_filename(self, include_path=False):
        with tracer.start_as_current_span("SourceDocument.get_filename"):
            ...
            logger.debug(f"Extracted filename: {filename}, include_path: {include_path}")
            return filename
