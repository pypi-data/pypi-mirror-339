...
class TestIntegration(unittest.TestCase):
    ...
    def test_update_method_update_seq_table_run(self):
        try:
            ...
            loc1 = FiftyFourVialPlate(plate=Plate.ONE, letter=Letter.A, num=Num.THREE)
            ...
            report_vial_locations = [r.vial_location for r in report]
            self.assertEqual({loc, loc1}, set(report_vial_locations))
        except Exception:
            self.fail("Failed")
    ...
    def test_run_method_immidiate_return(self):
        ...
        try:
            for _ in range(trys):
                ...
                current_run_file = os.path.isdir(
                    self.hplc_controller.get_last_run_method_file_path()
                )
                ...
                self.assertTrue(len(set(vial_locations)) == 1)
                for path in paths:
        ...
        try:
            ...
            seq_table = SequenceTable(
                ...
                rows=[
                    ...
                    SequenceEntry(
                        vial_location=FiftyFourVialPlate.from_str("P1-F2"),
                        sample_name="run1",
                        data_file="run1",
                        method=DEFAULT_METHOD,
                        inj_source=InjectionSource.HIP_ALS,
            ...
            self.hplc_controller.edit_sequence(seq_table)
            loc1 = FiftyFourVialPlate.from_str("P1-F3")
