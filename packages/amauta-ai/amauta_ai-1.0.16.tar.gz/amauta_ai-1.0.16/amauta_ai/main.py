...
{format_requirements}
            ...
            try:
                ...
                if "timeout" in error_msg.lower() or "timed out" in error_msg.lower():
                    print_warning(f"Request timed out. The PRD file may be too large for {selected_provider}.")
                    
                    if selected_provider == "perplexipy" and num_tasks > 15:
                        print_info("Trying again with a reduced number of tasks...")
                        fallback_tried = True
                        # Try again with reduced complexity
                        reduced_prompt = ai_prompt.replace(f"approximately {num_tasks} items", "approximately 10 items")
                        try:
                            response = ai_service.query_llm(
                            ...
                            if "anthropic" in provider_selection_service.get_provider_names():
                                # Try with Claude instead
                                print_warning(f"Second attempt failed: {str(inner_e)}")
                                print_info("Trying with Claude (Anthropic) instead...")
                                try:
                                    response = ai_service.query_llm(
                                        prompt=reduced_prompt,
                                        system_prompt=system_prompt,
                                    ...
                                    print_error(f"All attempts failed. Error: {str(final_e)}")
                                    print_info("Suggestions:")
                                    print_info("1. Try a smaller PRD file")
                                    print_info("2. Use --provider=anthropic directly")
                                    print_info("3. Reduce --num-tasks (try 5-10)")
                                    raise ValueError("Failed to process PRD after multiple attempts") from final_e
                            else:
                                print_error(f"Second attempt also failed: {str(inner_e)}")
                                print_info("Suggestions:")
                                print_info("1. Try a smaller PRD file")
                                print_info("2. Reduce --num-tasks (try 5-10)")
                                print_info("3. Install the anthropic package (pip install anthropic) and try again")
                                raise ValueError("Could not process PRD - the file may be too large") from inner_e
                    else:
                        # Try Anthropic if available
                        if "anthropic" in provider_selection_service.get_provider_names() and selected_provider != "anthropic":
                            print_info("Trying with Claude (Anthropic) instead...")
                            try:
                                response = ai_service.query_llm(
                                    prompt=ai_prompt,
                                ...
                                print_error(f"Anthropic attempt also failed: {str(anthro_e)}")
                                print_info("Suggestions:")
                                print_info("1. Try a smaller PRD file")
                                print_info("2. Reduce --num-tasks (try 5-10)")
                                raise ValueError("Failed to process PRD with multiple providers") from anthro_e
                        else:
                            print_error("Request timed out and no fallback providers are available")
                            print_info("Suggestions:")
                            print_info("1. Try a smaller PRD file")
                            print_info("2. Reduce --num-tasks (try 5-10)")
                            print_info("3. Install the anthropic package (pip install anthropic) and try again")
                            raise ValueError("Could not process PRD - the file may be too large") from e
                else:
                    # For non-timeout errors
                    print_error(f"Error calling AI service: {error_msg}")
                    print_info("Suggestions:")
                    print_info("1. Check your API keys in .env file")
                    print_info("2. Verify your internet connection")
                    print_info("3. Try another provider with --provider=anthropic")
                    raise ValueError(f"Failed to query AI service: {error_msg}") from e
            
            if response is None:
                print_error("No response received from AI service")
                raise ValueError("Failed to get response from AI service")
                
            if fallback_tried:
                print_info("Successfully processed PRD with fallback approach")
            ...
            def clean_json_str(json_str):
                ...
                # Check for incomplete JSON arrays
                if json_str.strip().endswith(","):
                    json_str = json_str.strip()[:-1]
                
                # Check if array is incomplete and missing closing bracket
                if json_str.count('[') > json_str.count(']'):
                    json_str = json_str + ']' * (json_str.count('[') - json_str.count(']'))
                
                # Check if object is incomplete and missing closing brace
                if json_str.count('{') > json_str.count('}'):
                    json_str = json_str + '}' * (json_str.count('{') - json_str.count('}'))
                
                # Remove unbalanced quotes - this is dangerous, use with caution
                # This attempts to fix the most common case of incomplete strings
                if json_str.count('"') % 2 != 0:
                    last_quote_pos = json_str.rfind('"')
                    if last_quote_pos > 0:
                        # If there's an unbalanced quote, try to add one at the end of a value
                        # but only if we find an easy pattern
                        search_area = json_str[last_quote_pos:]
                        if "," in search_area:
                            insert_pos = last_quote_pos + search_area.find(",")
                ...
                # Special case: if we have no content but there should be a JSON array,
                # return an empty array that will parse correctly
                if not json_str or json_str == '[]' or (json_str.startswith('[') and json_str.endswith(']') and len(json_str) < 5):
                    return '[]'
                    
                return json_str
            
            # If the content is very large, log only a reasonable portion
            if len(response) > 10000:
                logger.debug(f"Response size is large: {len(response)} characters")
                short_response = f"{response[:5000]}...{response[-5000:]}"
            else:
            ...
            extraction_methods = [
                ...
                # Method 5: Find the start of JSON array and try to extract as much as possible
                lambda resp: resp[resp.find('['):] if resp.find('[') != -1 else None,
            ]
            ...
            if not json_str:
                ...
                if array_start != -1:
                    ...
                    # If we couldn't find a proper closing bracket, try to recover as much as possible
                    if level > 0:
                        print_warning("Incomplete JSON detected - attempting recovery")
                        # Take everything from the array start to the end and try to fix it
                        partial_json = response[array_start:]
                        # Add missing closing brackets/braces
                        json_str = partial_json + ']' * level
                        print_debug(f"Attempted recovery of partial JSON by adding {level} closing brackets")
            ...
            if not json_str:
                ...
                print_info("Suggestions:")
                print_info("1. Try using a different provider with --provider=anthropic")
                print_info("2. Reduce the number of tasks with --num-tasks=10")
                print_info("3. Try a smaller PRD file")
            ...
            # Additional validation step: Check if JSON can be loaded
            try:
                json.loads(json_str)
            except json.JSONDecodeError as je:
                print_warning(f"Initial JSON parsing failed: {str(je)}")
                print_info("Attempting additional fixes...")
                
                # Try additional fixes for common JSON error patterns
                try:
                    # Fix 1: Try to extract just the outermost array
                    if json_str.startswith('[') and ']' in json_str:
                        last_bracket = json_str.rfind(']')
                        if last_bracket > 0:
                            cleaner_json = json_str[:last_bracket+1]
                            print_debug(f"Trying to extract outer array: {cleaner_json[:100]}...")
                            json.loads(cleaner_json)  # Test if it parses
                            json_str = cleaner_json
                            print_info("Fixed JSON by extracting outer array")
                except Exception:
                    pass
                    
                try:
                    # Fix 2: Try to extract the first complete object
                    if json_str.startswith('[{') and '}' in json_str and '},{' in json_str:
                        first_comma = json_str.find('},{')
                        if first_comma > 0:
                            single_obj_json = f"[{json_str[1:first_comma+1]}]"
                            print_debug(f"Trying first object only: {single_obj_json[:100]}...")
                            # Test if it parses
                            parsed = json.loads(single_obj_json)
                            if parsed and len(parsed) == 1:
                                json_str = single_obj_json
                                print_info("Extracted first complete object as fallback")
                except Exception:
                    pass
