...
import mrftools.Utilities.twixtools.twixprot as twixprot
import mrftools.Utilities.twixtools.helpers as helpers
import mrftools.Utilities.twixtools.mdb as twixtools_mdb
import mrftools.Utilities.twixtools.hdr_def as hdr_def
import mrftools.Utilities.twixtools.geometry as twixtools_geometry
...
def read_twix(infile, include_scans=None, parse_prot=True, parse_data=True,
    ...
    for s in range(NScans):
        ...
        if parse_geometry:
            out[-1]['geometry'] = twixtools_geometry.Geometry(out[-1])
        ...
        while pos + 128 < scanEnd:  # fail-safe not to miss ACQEND
            fid.seek(pos, os.SEEK_SET)
            try:
                mdb = twixtools_mdb.Mdb(fid, version_is_ve)
            except ValueError:
    ...
    with open(outfile, 'xb') as fid:
        ...
        for scan in scanlist:
            ...
            if not mdb.is_flag_set('ACQEND'):
                ...
                acqend = twixtools_mdb.Mdb_local()
