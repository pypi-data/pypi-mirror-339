...
class Histogram(object):
    ...
    If no initial contents are provided, all axes track under/overflow
    by default.  If contents are provided, an axis tracks
    under/overflow by default if the provided contents has two more
    bins in that dimension than axis.nbins. You can specify that
    ...
    Args:
        edges (Axes or array): Definition of bin edges, as anything
    ...
    def interp(self, *values, kind="linear"):
        ...
        If all axes of the histogram have log scale, multilinear
        interpolation is performed in the log domain. Hence, for
        example, interpolating a value halfway between two bin centers
        along a log-scale axis returns the *geometric* mean of the
        values in those bins. If log-domain interpolation is
        requested, the histogram's contents should all be > 0 to avoid
        warnings or errors.

        Interpolation will raise an error if called on a histogram
        containing both log- and linear/symmetric-scale axes, as the
        result of multilinear interpolation is ill-defined in this
        case.
        ...
        Args:
            ...
            kind (string): "linear" (default) if multilinear interpolation
               ...
               "log" if it is to be done on the log of the contents
               and then converted back to the linear domain.
        Return:
    ...
    def fill(self, *values, weight = None, warn_overflow = True):
        ...
        Args:
            ...
            weight (float): Value weight in histogram. Defaults to 1 in whatever units
                the histogram has
            warn_overflow (bool): Enable/disable warnings when an underflow or overflow occurs
                --i.e. when one or more of the input values falls beyond the range of the
                corresponding axis.

        Note:
            Note that weight needs to be specified explicitly by key; otherwise
            it will be considered a value, and an IndexError will be thrown.
        '''
    ...
    def _project(self, axis, project_out = False):
        ...
        def _project_contents(contents, keep_axes, sum_axes):
            ...
            def to_ranks(v):
                # turn a 1-D array v into an array
                # containing the ranks of its elements
                return np.argsort(np.argsort(v))

            if sum_axes == (): # sparse does not handle this case
                new_contents = contents.copy()
            else:
                new_contents = contents.sum(axis = sum_axes)

            # Transpose the contents to match the order of the axes
            # provided by the the user, which are currently sorted
            new_contents = np.transpose(new_contents, to_ranks(keep_axes))
        ...
        axes = np.asarray(self._axes.label_to_index(axis))
    ...
    def concatenate(edges, histograms, label = None, track_overflow = None):
        """
        Generate a Histogram H from a list of histograms h_1 ... h_n. We
        create a new first axis of length equal to the list and set
        H[i] = h_i.
        ...
        dimension than the input. The new axis has index 0.  If histograms
        can be subclassed, all of them must have the same class type.
        ...
        Args:
            ...
            track_overflow (bool): Track underflow and overflow on the newly created axis.
                Defaults to True if number of histograms is 2 + # bins on new axis,
                or False otherwise.
        Return:
    ...
    def _sumw2_product_op(self, self_sumw2, other_sumw2,
        ...
        if other_sumw2 is None:
            ...
            new_sumw2 = operation(operation(self_sumw2, other), other)
        else:
    ...
    def _inplace_operation(self, other, other_sumw2, operation,
        ...
        new_contents = operation(self._contents, other)
        ...
        if self._sumw2 is not None:
            ...
            if optype == Histogram.OpType.SUM:
                if other_sumw2 is not None:
                    self_sumw2_contents += other_sumw2
    ...
    def plot(self, ax = None,
        ...
        if self.ndim == 1:
            ...
            # Plot
            plot = ax.errorbar(without_units(xdata),
                               ydata,
                               yerr = yerr,
                               drawstyle = drawstyle,
                               **kwargs)
            ...
            # No under/overflow
            plot = ax.pcolormesh(without_units(self._axes[0].edges),
                                 without_units(self._axes[1].edges),
