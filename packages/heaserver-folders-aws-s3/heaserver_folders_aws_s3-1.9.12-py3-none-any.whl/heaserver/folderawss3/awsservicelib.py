...
async def list_object_versions(s3: S3Client, bucket_id: str, prefix: str | None = None,
                               ...
                               delimiter: str | None = None) -> AsyncIterator[Mapping[str, Any]]:
    """
    ...
    :param filter_folder: flag to filter out folders and only show object versions
    ...
    while (page := await loop_.run_in_executor(None, next, pages, None)) is not None:
        ...
        for obj in page.get('Versions', []):
            if (filter_folder and is_folder(obj.get("Key"))) or (filter_deleted and obj.get('Key') in dm_keys and not obj['IsLatest']):
                continue
            yield obj
...
async def get_latest_object_version(s3: S3Client, bucket_id: str, key: str,
    ...
    async for obj in list_object_versions(s3, bucket_id=bucket_id, prefix=key, loop=loop, max_keys=1):
        if obj['IsLatest']:
            return obj
    return None
...
async def _delete_object(request: web.Request, s3_client: S3Client, mongo_client: Mongo, volume_id: str, bucket_name: str, key: str,
    ...
    try:
        ...
        async def gen() -> AsyncIterator[KeyAndVersion]:
            ...
            async for obj in list_object_versions(s3_client, bucket_name, prefix=key, loop=loop, filter_deleted=True):
