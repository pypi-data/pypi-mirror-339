...
class image:
    ...
    class trackbar_:
        ...
        def __init__(self, name, coos, min=0, max=100, val=0, step=1) -> None:
            self.name, self.coos = name, coos
            self.values = [min, max, val, step]
            self.changing, self.chg_val = False, None
        def is_clicked(self, coos) -> bool: return clicked_in(coos, self.coos)
        def draw(self, col1=COL.red, col2=COL.darkRed, thickness=3, lineType=2, fontSize=1, interligne=0) -> None:
            self.img.rectangle(*self.coos, col1, 0, lineType)
            self.img.rectangle(*self.coos, col2, thickness, lineType)
            p1, p4 = self.coos
            x, y = p4[0]-p1[0], p4[1]-p1[1]
            g, d = coosCircle(coosCircle(p1, y/2, 90), x/30, 0), coosCircle(coosCircle(p4, y/2, -90), x/30, 180)
            self.img.line(g, d, col2, thickness, lineType)
            t = Text(self.name, False)
            X, Y = t.get_size(fontSize, 0, interligne)
            t.draw(self.img, (p1[0]+X/2, p1[1]+Y/2), col2, thickness, fontSize, interligne, lineType, 0, True, False)
        def get(self) -> number: return self.val
    def trackbar(self, name="TrackBar", coos=[[100, 200], [500, 300]], min=0, max=100, val=0, step=1, *args, **kwargs) -> trackbar_:
        trkb = self.trackbar_(name, coos, min, max, val, step)
        trkb.defImg(self)
        trkb.draw(*args, **kwargs)
    ...
    def show_(self, wait=1, destroy=False, built_in_functs=True) -> int:
        ...
        if not self.disable_callback and self.mouse.new:
            ...
            for b in self.buttons:
                if b.is_clicked(pos):
                    b.clicked(v)
            for t in self.trackbars: ## TODO trackbars' handling
                if v[0] == cv2.EVENT_LBUTTONDOWN:
                    ...
                    if t.changing:
                        t.val = t.chg_val
                        t.changing, t.chg_val = False, None
...
if __name__ == "__main__":
    ...
    t = img.trackbar("TEST", fontSize=fs, thickness=tk)
    img.build()
    while img.is_opened():
        wk = img.show()
        if wk == -1: pass