...
def execute_commands(
    ...
    if config_template_content:
        ...
        # Also fix the keys_zone name - either with or without domain
        content = re.sub(
            r'keys_zone=[^\s:]+:', 
        ...
        # Fix any limit_req_zone references
        content = re.sub(
            r'zone=[^\s:]+:', 
            f'zone={unique_id}_limit:',
            content
        )
        
        # Fix any limit_req directives
        content = re.sub(
            r'limit_req\s+zone=[^\s;]+', 
            f'limit_req zone={unique_id}_limit',
            content
        )
