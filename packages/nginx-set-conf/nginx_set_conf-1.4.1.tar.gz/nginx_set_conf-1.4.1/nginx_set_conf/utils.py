...
def execute_commands(
    ...
    if config_template_content:
        ...
        # Replace any proxy_cache_path that isn't already domain-specific
        generic_path_pattern = r'proxy_cache_path\s+/var/cache/nginx/[^_]*\s+'
        domain_safe = domain.replace('.', '_')
        replacement = f'proxy_cache_path /var/cache/nginx/{service_name}_{domain_safe} '
        
        if re.search(generic_path_pattern, content):
            print(f"FIXING: Found generic cache path pattern, replacing with domain-specific path")
            content = re.sub(generic_path_pattern, replacement, content)
            
            # Also fix the keys_zone name
            content = re.sub(
                r'keys_zone=([^_:]*):',
                f'keys_zone={service_name}_{domain_safe}_cache:',
                content
            )
            
            # Fix limit_req_zone too
            content = re.sub(
                r'zone=([^_:]*):',
                f'zone={service_name}_{domain_safe}_limit:',
                content
            )
            ...
            with open(file_path, "r") as f:
                for line in f:
                    if 'proxy_cache_path' in line or 'keys_zone=' in line or 'limit_req_zone' in line:
