...
def run_on_hpc(raw_files, output_dir, kraken_db_dir=None, corn_db_dir=None, config_file=None, partition="normal", threads=32, memory=200, time="144:00:00", setup_kraken_db=False, setup_corn_db=False):
    ...
    try:
        ...
        if setup_kraken_db:
            ...
            # Create a script to download and extract the database
            kraken_setup_script = f"""
#!/bin/bash
cd {remote_kraken_db_dir}
if [ ! -d "kraken2-microbial-fatfree" ]; then
    echo "Downloading Kraken2 database..."
    wget -q --show-progress https://genome-idx.s3.amazonaws.com/kraken/k2_standard_20240112.tar.gz
...
"""
            ...
            temp_script = "/tmp/setup_kraken.sh"
            with open(temp_script, "w") as f:
                f.write(kraken_setup_script)
            
            remote_script = f"{remote_output_dir}/setup_kraken.sh"
            hpc.upload_file(temp_script, remote_script)
            hpc.execute_command(f"chmod +x {remote_script}")
            hpc.execute_command(f"bash {remote_script}")
        ...
        if setup_corn_db:
            ...
            corn_setup_script = f"""
#!/bin/bash
cd {remote_corn_db_dir}
if [ ! -f "corn_db.1.bt2" ]; then
    echo "Downloading corn genome database..."
    wget -q --show-progress https://glwasoilmetagenome.s3.us-east-1.amazonaws.com/corn_db.zip
    echo "Extracting corn genome database..."
    unzip -o corn_db.zip -d .
    rm corn_db.zip
    echo "Corn genome database setup complete"
else
    echo "Corn genome database already exists"
fi
"""
            # Upload and execute the script
            temp_script = "/tmp/setup_corn.sh"
            with open(temp_script, "w") as f:
                f.write(corn_setup_script)
            
            remote_script = f"{remote_output_dir}/setup_corn.sh"
            hpc.upload_file(temp_script, remote_script)
            hpc.execute_command(f"chmod +x {remote_script}")
            hpc.execute_command(f"bash {remote_script}")
            os.remove(temp_script)
