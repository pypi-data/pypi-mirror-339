...
class URLRequest():
    ...
    def __init__(self, file_path, cookies: list = None):
        ...
        if cookies is not None and len(cookies) > 0:
            ...
            self.header_info['headers']['Host'] = domain
            
            cookies_str_list = [f'{cookie["name"]}={cookie["value"]}' for cookie in cookies]
            cookies_str = '; '.join(cookies_str_list)
            self.header_info['headers']['Cookie'] = cookies_str
