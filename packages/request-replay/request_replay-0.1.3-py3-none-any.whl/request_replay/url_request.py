...
class URLRequest():
    ...
    def update_cookies_jar(self, json_path):
        cookies_jar = json.loads(open(json_path, 'r', encoding='utf-8').read())
        for cookie in cookies_jar:
            if cookie['domain'] == self.header_info['headers']['Host']:
                self.header_info['cookies'][cookie['name']] = cookie['value']
    ...
    def request(self, client = None, override_scheme = None):
        ...
        resp = client.request(
            ...
            method= method,
            url=url,
            cookies=self.header_info['cookies'],
