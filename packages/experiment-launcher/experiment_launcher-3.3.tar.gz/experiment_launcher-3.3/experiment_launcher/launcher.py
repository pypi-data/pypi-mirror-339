...
class Launcher(object):
    ...
    def __init__(
            ...
            conda_env=None,
            gres=None, constraint=None, partition=None, use_job_array_seeds=False,
            begin=None,
            use_timestamp=True,
            compact_dirs=False,
        ...
        Args:
            ...
            project_name (str): name of the project for slurm. This is important if you have
                different projects (e.g. in the hrz cluster)
            base_dir (str): path to directory to save the results (in hrz results are saved to /work/scratch/$USER)
            n_exps_in_parallel (int): number of experiment configurations to run in parallel.
                ...
                If use_seed_job_array is True, experiment configurations are run in parallel using the same seed.
                If use_seed_job_array is False, a mix of seeds and experiment configurations are run in parallel.
            ...
            use_job_array_seeds (bool): if True, the slurm job will be submitted as a job array.
                This is useful for running multiple seeds in the TUDa HRZ cluster, since jobs get allocated faster.
        ...
        self._experiment_py_file = exp_file
        ...
        self._use_job_array_seeds = use_job_array_seeds
        self._begin = begin

        self._experiment_list = list()

        if use_timestamp:
            self._experiment_name += datetime.datetime.now().strftime('_%Y-%m-%d_%H-%M-%S')

        base_dir = './logs' if base_dir is None else base_dir
        self._experiment_dir_local = os.path.join(base_dir, self._experiment_name)

        # tracks the results directories
        self._results_dir_l = []
        self._check_results_directories = check_results_directories

        self._experiment_dir_slurm = self._experiment_dir_local
        if os.getenv("USER"):
            scratch_dir = os.path.join('/work', 'scratch', os.getenv('USER'))
            if os.path.isdir(scratch_dir):
                self._experiment_dir_slurm = os.path.join(scratch_dir, self._experiment_name)
        os.makedirs(self._experiment_dir_slurm, exist_ok=not self._check_results_directories)

        # directories for slurm sbatch files and logs
        self._experiment_dir_slurm_files = os.path.join(self._experiment_dir_slurm, "slurm_files")
        self._experiment_dir_slurm_logs = os.path.join(self._experiment_dir_slurm, "slurm_logs")
    ...
    def generate_slurm(self, command_line_args_list=None):
        project_name_option = ''
        partition_option = ''
        begin_option = ''
        gres_option = ''
        ...
        seed_code = f'\t\t--seed $(({self._start_seed} + $SLURM_ARRAY_TASK_ID)) \\' if self._use_job_array_seeds else ''
...
#SBATCH -J {self._experiment_name}
"""
        if self._use_job_array_seeds:
            sbatch_code += f"#SBATCH -a 0-{self._n_seeds - 1}\n"
...
#SBATCH -o {self._experiment_dir_slurm_logs}/%A_%a.out
#SBATCH -e {self._experiment_dir_slurm_logs}/%A_%a.err
...
"""
        for command_line in command_line_args_list:
            sbatch_code += f"""\
                
{python_code}
"""
            if len(seed_code) > 0:
                sbatch_code += f"""{seed_code}\n"""

            sbatch_code += f"""\t\t{command_line}  &
"""
...
"""
    ...
    def save_slurm(self, command_line_args_list=None, idx: str = None):
        code = self.generate_slurm(command_line_args_list)

        label = f"_{idx}" if idx is not None else ""
        script_name = f'slurm_{self._experiment_name}{label}.sh'
        full_path = os.path.join(self._experiment_dir_slurm_files, script_name)
    ...
    def _run_slurm(self, test):
        ...
        os.makedirs(self._experiment_dir_slurm_files, exist_ok=True)
        os.makedirs(self._experiment_dir_slurm_logs, exist_ok=True)

        experiment_list_to_run = deepcopy(self._experiment_list)

        # If we do not use job arrays, we add the seed as part of the experiment configuration.
        # This allows for correct parallelization across seeds and experiment configurations if n_exps_in_parallel > 1.
        if not self._use_job_array_seeds:
            experiment_list_to_run_with_seed = []
            for exp_d in experiment_list_to_run:
                for i in range(self._start_seed, self._start_seed + self._n_seeds):
                    exp_d_with_seed = deepcopy(exp_d)
                    exp_d_with_seed['seed'] = i
                    experiment_list_to_run_with_seed.append(exp_d_with_seed)
            experiment_list_to_run = experiment_list_to_run_with_seed

        # Generate and save slurm files
        slurm_files_path_l = []
        experiment_list_chunked = []
        for i in range(0, len(experiment_list_to_run), self._n_exps_in_parallel):
            experiment_list_chunked.append(experiment_list_to_run[i:i + self._n_exps_in_parallel])
        ...
        for i, exps in enumerate(experiment_list_chunked):
            ...
            for exp in exps:
                ...
                results_dir = self._generate_results_dir(self._experiment_dir_slurm, exp)
                command_line_args_l.append(f'--results_dir {results_dir} {command_line_arguments}')
            slurm_files_path_l.append(
                self.save_slurm(command_line_args_l, str(i).zfill(len(str(len(experiment_list_chunked)))))
            )

        # Launch slurm files with sbatch
    ...
    def _run_local(self, test):
        if not test:
            os.makedirs(self._experiment_dir_local, exist_ok=True)

        module = import_module(self._experiment_py_file)
        ...
        if test:
            ...
            def experiment_wrapper(params):
                try:
                    ...
                    print(f"Experiment failed with parameters:\n{params}")
                    traceback.print_exc()
    ...
    def _check_experiments_results_directories(self):
        ...
        for exp in self._experiment_list:
            results_dir = self._generate_results_dir(self._experiment_dir_local, exp)
    ...
    def _generate_exp_params(self, default_params_dict):
        ...
        for exp in self._experiment_list:
            ...
            params_dict['results_dir'] = self._generate_results_dir(self._experiment_dir_local, exp)
            for seed in seeds:
    ...
    def log_dir(self, local=True):
        if local:
            return self._experiment_dir_local
        else:
            return self._experiment_dir_slurm
