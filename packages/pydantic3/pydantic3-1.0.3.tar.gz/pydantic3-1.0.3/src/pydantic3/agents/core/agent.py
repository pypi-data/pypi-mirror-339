...
class FormAgent:
    ...
    def __init__(
        self,
        api_key: str,
        model_name: str = "openai/gpt-4o-2024-11-20-mini-2024-07-18",
    ...
    async def _extract_form_info(
        ...
        try:
            ...
            history_data = []
            last_messages = message_history[-20:]
            ...
            blocks = [
                ['CURRENT_PROGRESS', f"{form_data.session_info.metadata.progress}%"],
                ['USER_MESSAGE', message],
                ['FORM_SCHEMA', json.dumps(form_schema, indent=2, ensure_ascii=False)],
                ['USER_FORM', Helper.data_to_yaml(user_form_data)],
                ['METADATA', Helper.data_to_yaml(metadata_data)],
                ['MESSAGE_HISTORY', Helper.data_to_yaml(history_data)],
            ]
            prompt_blocks = []
            for block in blocks:
                prompt_blocks.append(f"[{block[0]}]")
                prompt_blocks.append(block[1])
                prompt_blocks.append("[/" + block[0] + "]")

            prompt_extra = """
            🎯 TASK:
            Your goal is to extract useful information from the [USER_MESSAGE] and [MESSAGE_HISTORY], improve the form, and move the conversation forward.
    ...
    async def _generate_analytics(
        ...
        try:
            ...
            if session_id:
                ...
                if latest_form_data_dict:
                    ...
                    if latest_user_form_data:
                        ...
                        try:
                            # Просто создаем новый экземпляр формы и заполняем его данными из БД
                            factory = UniversalModelFactory(form_class, fill_data=False)
                            new_form = factory.build()
            ...
            if hasattr(form_data.session_info.user_form, "model_dump"):
                try:
                    # Также используем фабрику вместо валидации
                    factory = UniversalModelFactory(form_class, fill_data=False)
                    new_form = factory.build()

                    # Копируем текущие данные
                    for field_name, field_value in current_user_form_dump.items():
                        if hasattr(new_form, field_name):
                            setattr(new_form, field_name, field_value)

                    form_data.session_info.user_form = new_form
                except Exception as e:
                    self.logger.error(f"Failed to update form data for analytics: {e}")
