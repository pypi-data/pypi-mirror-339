...
class HomeLanUtils:
    ...
    def net_check_shutdown(cls, url: str) -> None:
        ...
        20250404 这个功能基本上被 hlan_checkurl_shutdown_bj 替代。
    ...
    def hlan_checkurl_shutdown_bj(
        ...
        *,
        ...
        def cb(rss: List[str | None]) -> bool:
            ...
            if shutdown:
                if verbose:
                    print(f"{ts} connect url:{url} shutdown {cmd}")
                # cmd = "shutdown -P now"
                run_simple_a(cmd, verbose=verbose)
                if verbose:
                    print(f"shutdown end")
                return False
            else:
                if verbose:
                    print(f"{ts} connect url:{url} {rss[-1:]}")
                return True

        dtstr = AT.sdf_logger_format_datetime()
        print(f"{dtstr} hlan_checkurl_shutdown_bj start version:{AT.VERSION}")
