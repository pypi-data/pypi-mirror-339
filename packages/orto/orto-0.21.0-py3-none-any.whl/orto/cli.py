...
def extract_sf_energies_func(uargs):
    ...
    all_data = extractor.SpinFreeEnergyExtractor().extract(
        uargs.output_file
    )
...
def extract_so_energies_func(uargs):
    ...
    all_energies = extractor.SpinOrbitEnergyExtractor().extract(
        uargs.output_file
    )
...
def extract_gmatrix_func(uargs):
    '''
    Wrapper for cli call to extract gmatrix
    '''

    choices = {
        'total': extractor.GMatrixExtractor,
        'L': extractor.GMatrixLExtractor,
        'S': extractor.GMatrixSExtractor,
        'eff': extractor.GMatrixEffectiveExtractor,
    }
...
def print_gmatrix_func(uargs):
    ...
    choices = {
        'total': extractor.GMatrixExtractor,
        'L': extractor.GMatrixExtractor,
        'S': extractor.GMatrixExtractor,
        'eff': extractor.GMatrixExtractor,
    }

    # Extract data
    data = choices[uargs.type]().extract('nev-ailft.out')[0]
...
def gen_job_func(uargs):
    ...
    for input_file in uargs.input_files:
        ...
        except extto.DataFormattingError:
            ut.red_exit(
                f'%PAL block is malformed, perhaps missing END?\n in {oj.input_file}' # noqa
            )
        ...
        try:
            ...
            ut.red_exit(f'Missing max per-core memory in {oj.input_file}')
        ...
        if len(moinp):
            ...
            if not moinp.suffix:
                ut.red_exit(f'Error: Orbital file {moinp} has no extension')
            if not moinp.exists():
                ut.red_exit(f'Error: Orbital file {moinp} cannot be found')
...
def print_pop_func(uargs) -> None:
    ...
    if uargs.flavour in ['loewdin', 'lowdin']:
        ...
        try:
            data = extractor.MullikenPopulationExtractorDensities.extract(
                uargs.output_file
            )
        except extto.DataNotFoundError:
            data = extractor.MullikenPopulationExtractorPopulations.extract(
                uargs.output_file
            )
