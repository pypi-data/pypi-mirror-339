...
from xingshu.包_事件引擎 import 类_事件,类_事件引擎
from xingshu.包_交易核心.模块_主引擎 import 基础引擎,类_主引擎
...
from xingshu.包_交易核心.模块_事件类型 import (
    事件类型_行情,
    事件类型_订单,
    事件类型_成交
)
...
from xingshu.包_交易核心.模块_工具 import 加载json文件, 保存json文件, 提取合约代码, 四舍五入到指定值
from xingshu.包_交易核心.模块_基础数据库 import 类_基础数据库, 获取数据库, 数据库时区
from xingshu.包_交易核心.模块_数据服务 import 类_基础数据服务, 获取数据服务
...
class 类_CTA引擎(基础引擎):
    ...
    配置文件名: str = "cta_strategy_setting.json"
    数据文件名: str = "cta_strategy_data.json"
    ...
    def 加载策略类(self) -> None:
        """加载策略类文件"""
        路径1 = Path(__file__).parent.joinpath("strategies")
        self.从目录加载策略类(路径1, "vnpy_ctastrategy.strategies")

        路径2 = Path.cwd().joinpath("strategies")
        self.从目录加载策略类(路径2, "strategies")
