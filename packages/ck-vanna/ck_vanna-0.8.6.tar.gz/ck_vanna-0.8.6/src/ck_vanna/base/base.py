...
class VannaBase(ABC):
    def __init__(self, config=None):
        ...
        参数:
            config (dict, optional): 配置字典，包含以下可选项：
                - dialect: SQL方言，默认为"SQL"
                - language: 响应语言，默认为None
                - max_tokens: 最大令牌数，默认为14000
        """
        if config is None:
            config = {}
    ...
    def get_training_data(self, **kwargs) -> pd.DataFrame:
        ...
        返回:
            pd.DataFrame: 包含所有训练数据的DataFrame
        """
        """
        Example:
        ```python
