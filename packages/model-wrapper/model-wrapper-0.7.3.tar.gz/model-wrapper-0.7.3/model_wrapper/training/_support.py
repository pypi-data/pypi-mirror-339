...
def _is_same_params(model, batch):
    return len(batch) == len(inspect.signature(model.module.forward if isinstance(model, nn.DataParallel) else model.forward).parameters)  

def _forward_y(model, batch, device):
    # 判断所传参数个数是否与方法一致
    same_params = _is_same_params(model, batch)
    batch = [x.to(device) if torch.is_tensor(x) else x for x in batch]
    if same_params:
