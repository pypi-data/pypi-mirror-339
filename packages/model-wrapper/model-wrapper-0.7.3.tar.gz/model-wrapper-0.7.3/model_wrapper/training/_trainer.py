...
class Trainer:
    ...
    def _begin_train(self, model):
        log_utils.info(f'Model parameters: {sum(param.numel() for param in model.parameters() if param.requires_grad)} / {sum(param.numel() for param in model.parameters())}, use {self.device} training...\n')
    ...
    def train(
        ...
        if isinstance(X, Tuple):
            if all([torch.is_tensor(x) for x in X]):
                ...
                return super().train(
                    model, ListDataset(*X, y), collate_fn if collate_fn else ListTensorCollator(), show_progress, eps=eps
                )
        ...
        if isinstance(X, Tuple):
            if all([torch.is_tensor(x) for x in X]):
                ...
                return super().train(
                    ...
                    collate_fn if collate_fn else ListTensorCollator(),
                    show_progress,
                    eps,
                )
        else:
