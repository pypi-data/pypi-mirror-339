...
class HistogramPlot(MultiDataPlot):
    ...
    def resolve_comparison_options(
        ...
        plot_format = comparison_options.pop('plot_format', self.config['plot_format'])
        error_format = comparison_options.pop('error_format', self.config['error_format'])
        ...
        for component in components:
            ...
            if inherit_color:
                ...
                for style_type in ['styles', 'error_styles']:
                    ...
                    styles['color'] = styles.get('color') or get_target_color(reference, style_type)
    ...
    def resolve_ylabel(
        ...
        if not all(h.uniform_binning for h in histograms):
            return ylabel.format(unit=unit, bin_width='bin', space=space)
