...
class XMLWSCombiner(XMLWSBase):
    ...
    def __init__(self, source:Union[str, Dict],
        ...
        self.cms_disc_set_keyword = DiscreteNuisance().config['disc_set_keyword']
        self.cms_nuis_pdf_keyword = "CONSTRAINTPDF"
        self.cms_nuis_pdf_prefix = "nuisancePdf"
        self.cms_nuis_pdf_suffix = "withconstr"
    ...
    def load_cache_ws(self, ws_type:str):
        ...
        if ws_type == "rename":
            ...
            if discrete_parameters:
                self.discrete_parameters = discrete_parameters.snapshot()
            else:
                self.discrete_parameters = ROOT.RooArgSet(self.cms_disc_set_keyword)
    ...
    def validate_channel_input(self, channel:str):
        ...
        if model.model_config.GetPdf().ClassName() == "RooSimultaneousOpt":
            self.use_cms_opt_pdf = True # force to use CMS optimized PDF when there is CMS WS
    ...
    def rename_channel(self, channel:str) -> None:
        ...
        if self.use_cms_opt_pdf:
            ...
            for i in range(cat_pdf.size()):
                ...
                pdf.addPdf(pdf_i, cat_pdf.getLabel())
            
            # get list of constraint pdfs
            constrpdf_list = ROOT.RooArgSet()
            for arg in model.model_config.GetGlobalObservables():
                globName = arg.GetName()
                constrs = arg.clients()
                if constrs.size() == 1:
                    constr = constrs[0]
                elif constrs.size() > 1:
                    raise RuntimeError(f"Global observable {globName} has more than one constraint PDF")
                else:
                    self.stdout.warning(f"Global observable {globName} does not have constraint PDF")
                    continue
                
                # harmonize the pdf class
                constr_name = constr.GetName()
                constr_class = constr.ClassName()
                if constr_class == "RooGaussian" or constr_class == "SimpleGaussianConstraint":
                    if self.use_cms_gaussian:
                        constrpdf = ROOT.SimpleGaussianConstraint(constr, constr_name)
                    else:
                        constrpdf = ROOT.RooGaussian(constr, constr_name)
                else:
                    constrpdf = constr
        ...
        if simplified_import:
            ...
            if constrpdf_list.getSize() > 0:
                self._append_channel_suffix_to_args(channel, constrpdf_list, "variable")
        ...
        if constrpdf_list.getSize() > 0:
            ...
            nuis_pdf_name = f"{self.cms_nuis_pdf_prefix}_{channel}"
    ...
    def combine_channel(self, channel:str):
        ...
        for i in range(n_cat):
            ...
            if self.constrpdf_list_all.getSize() > 0 and not pdf_i.dependsOnValue(self.constrpdf_list_all):
                self.stdout.debug(f'Add CMS nuisance pdf for category "{new_cat_name_i}"')
                nuis_pdf = self.tmp_ws.pdf(f"{self.cms_nuis_pdf_prefix}_{channel}")
                pdf_i_new_name = f"{pdf_i.GetName()}_{self.cms_nuis_pdf_suffix}"
