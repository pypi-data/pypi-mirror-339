...
def generate_plot_for_subsample(value, ref_values):
    ...
    plt.tight_layout()
    plot_filename = 'step3_sub_range.png'
...
def generate_plot_for_subsample_barcode(value, ref_values):
    ...
    plt.legend(loc="lower right")
    plt.tight_layout()
    plot_filename = 'step3_sub_barcode.png'
    plt.savefig(plot_filename)
    plt.close()
    return plot_filename
...
def generate_plot_for_dist_barcode(value, ref_values):
    ...
    plt.title('Test Barcode Distribution')
    plt.grid(True)
    plot_filename = 'step3_hist_barcode.png'
    plt.tight_layout()
    plt.savefig(plot_filename)
    plt.close()
    return plot_filename
...
def generate_qc_report(name_temp, metrics_data, plot_metric, other_metric, suggestions_data, target_metric):
    pdf_filename = f"qc_{name_temp}_step3.pdf"
    c = canvas.Canvas(pdf_filename, pagesize=letter)

    c.setFont("Helvetica-Bold", 16)
    c.drawString(50, 750, "Quality Control Report for RNA(cDNA) Counting Step")
    ...
    y_position -= 5
    text = "Low values in above metrics may indicate insufficient sequencing depth or low reverse transcription efficiency. If so, check the reverse transcription experiment firstly. Insufficient total RNA used for reverse transcription is a common issue in this step."
    y_position = draw_multiline_string(c, 50, y_position, text, max_width=500)

    plot_filename = generate_plot_for_subsample(plot_metric['oligo_coverage'][0], plot_metric['oligo_coverage'][1])
    c.drawImage(plot_filename, 120, 100, width=310, height=230)
    ...
    y_position  = 200
    text = "The distribution of the tested barcodes should resemble one of the reference distributions, or the overall distribution should be more skewed to the right. If a long-tail effect is significant here despite qualified QC in the previous step, it likely indicates uneven cDNA distribution due to issues in the reverse transcription process. If so, it is recommended to check the reverse transcription process and refer to the previous page of report to make sure the sequencing depth is enough. It is strongly recommend ensuring that the total amount of RNA used for reverse transcription is sufficient when encountering issues in this step."
    y_position = draw_multiline_string(c, 50, y_position, text, max_width=500)
    ...
    text = "barcode_less_1/5/10_ratio: The proportion of barcodes detected fewer than 1/5/10 times. A high value may indicate insufficient sequencing depth or low reverse transcription efficiency."
    y_position = draw_multiline_string(c, 50, y_position, text, max_width=500)
    ...
    text = "barcode_detected_200: The number of barcodes detected more than 200 times. This value usually represents outliers and may be associated with highly uneven plasmid distribution or not qualified reverse transcription."
    y_position = draw_multiline_string(c, 50, y_position, text, max_width=500)
    ...
    text = "not_qualified_oligo_ratio/_with_bc_count3/5/10: The ratio of oligos that do not link to a sufficient amount (default is 3) of barcodes when using barcodes detected at least 1/3/5/10 times. A high value may indicate insufficient sequencing depth or low reverse transcription efficiency."
    y_position = draw_multiline_string(c, 50, y_position, text, max_width=500)
...
def main():
    ref_file = pkg_resources.resource_filename("esMPRA", "data/step3_refdata.pkl")
    ...
    file_path = os.path.join(folder_path, 'down_sample_for_rna_oligo.txt')
    if os.path.isfile(file_path):
        data = np.genfromtxt(file_path, skip_header=1)
        sub_ratio = data[:, 0]
        counts = data[:, 1]
    ...
    file_path = os.path.join(folder_path, 'down_sample_for_rna.txt')
    if os.path.isfile(file_path):
        data = np.genfromtxt(file_path, skip_header=1)
        sub_ratio = data[:, 0]
        counts = data[:, 1]
    ...
    key_metric['barcode_detected_1_ratio'] = (barcode_detected_1_temp/(total_barcodes_type_temp),(0.08,np.inf,0.03,np.inf))
    min_value = np.min(barcode_detected_5_ratio)
    max_value = np.max(barcode_detected_5_ratio)
    key_metric['barcode_detected_5_ratio'] = (barcode_detected_5_temp/(total_barcodes_type_temp),(0.05,np.inf,0.03,np.inf))
    min_value = np.min(barcode_detected_10_ratio)
    max_value = np.max(barcode_detected_10_ratio)
    key_metric['barcode_detected_10_ratio'] = (barcode_detected_10_temp/(total_barcodes_type_temp),(0.03,np.inf,0.014,np.inf))

    min_value = np.min(mean_barcodes_detected_counts)
    max_value = np.max(mean_barcodes_detected_counts)
    key_metric['mean_barcodes_detected_counts'] = (mean_barcodes_detected_counts_temp,(5,np.inf,3,np.inf))

    min_value = np.min(qualified_oligo_ratio)
    max_value = np.max(qualified_oligo_ratio)
    key_metric['qualified_oligo_ratio'] = (qualified_oligo_num_temp/total_oligo,(0.8,np.inf,0.65,np.inf))

    min_value = np.min(qalified_oligo_ratio_with_bc_count3)
    max_value = np.max(qalified_oligo_ratio_with_bc_count3)
    key_metric['qualified_oligo_ratio_with_bc_count3'] = (qalified_oligo_num_with_bc_count3_temp/total_oligo,(0.78,np.inf,0.5,np.inf))
    min_value = np.min(qalified_oligo_ratio_with_bc_count5)
    max_value = np.max(qalified_oligo_ratio_with_bc_count5)
    key_metric['qualified_oligo_ratio_with_bc_count5'] = (qalified_oligo_num_with_bc_count5_temp/total_oligo,(0.7,np.inf,0.4,np.inf))
    min_value = np.min(qalified_oligo_ratio_with_bc_count10)
    max_value = np.max(qalified_oligo_ratio_with_bc_count10)
    key_metric['qualified_oligo_ratio_with_bc_count10'] = (qalified_oligo_num_with_bc_count10_temp/total_oligo,(0.5,np.inf,0.2,np.inf))

    min_value = np.min(mean_barcodes_count_for_oligo)
    max_value = np.max(mean_barcodes_count_for_oligo)
    key_metric['mean_barcodes_count_for_oligo'] = (mean_barcodes_count_for_oligo_temp,(10,300,7.5,600))
    ...
    other_metric['barcode_less_1_ratio'] = (barcode_less_1_temp/total_barcodes_type_temp,(-np.inf,0.96))
    ...
    other_metric['barcode_less_5_ratio'] = (barcode_less_5_temp/total_barcodes_type_temp,(-np.inf,0.97))
    ...
    other_metric['not_qualified_oligo_ratio'] = (not_qualified_oligo_num_temp/total_oligo,(-np.inf,0.2))
    ...
    other_metric['qualified_barcodes_ratio'] = (qualified_barcodes_num_temp/total_barcodes_type_temp,(0.05,np.inf))
    ...
    other_metric['not_qalified_oligo_ratio_with_bc_count3'] = (not_qalified_oligo_num_with_bc_count3_temp/total_oligo,(-np.inf,0.3))
    ...
    other_metric['not_qalified_oligo_ratio_with_bc_count5'] = (not_qalified_oligo_num_with_bc_count5_temp/total_oligo,(-np.inf,0.35))
    ...
    other_metric['not_qalified_oligo_ratio_with_bc_count10'] = (not_qalified_oligo_num_with_bc_count10_temp/total_oligo,(-np.inf,0.6))
    ...
    other_metric['cv_barcodes_count_for_oligo'] = (cv_barcodes_count_for_oligo_temp,(0.6,1.5))
    min_value = np.min(mode_barcodes_count_for_oligo)
    max_value = np.max(mode_barcodes_count_for_oligo)
    other_metric['mode_barcodes_count_for_oligo'] = (mode_barcodes_count_for_oligo_temp,(3,300))
    min_value = np.min(mean_barcodes_counts)
    max_value = np.max(mean_barcodes_counts)
    other_metric['mean_barcodes_counts'] = (mean_barcodes_counts_temp,(4,65))
    min_value = np.min(cv_barcodes_for_counts)
    max_value = np.max(cv_barcodes_for_counts)
    other_metric['cv_barcodes_for_counts'] = (cv_barcodes_for_counts_temp,(0.6,2.1))
    min_value = np.min(mode_barcodes_for_counts)
    max_value = np.max(mode_barcodes_for_counts)
    other_metric['mode_barcodes_for_counts'] = (mode_barcodes_for_counts_temp,(1,40))
