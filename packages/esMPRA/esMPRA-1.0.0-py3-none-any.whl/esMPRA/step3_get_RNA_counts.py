...
def read_barcode(args):
    ...
    with open(os.path.join('./',args.run_name+'_step3','log.txt'),'w') as f_out:
        f_out.write(f'total reads count: {reads_count}\n')

    np.savetxt(os.path.join('./',args.run_name+'_step3','RNA_total_counts.txt'),np.array([reads_count]), fmt='%d')

    with open(os.path.join('./',args.run_name+'_step3','RNA_reads_counts.pkl'), 'wb') as f:
...
def barcode_qc1(args, barcode_count):
    ...
    with open(os.path.join('./',args.run_name+'_step3','log.txt'),'a') as f_out:
    ...
    plt.savefig(os.path.join('./',args.run_name+'_step3','dist_of_barcode_reads_counts.png'))
    plt.close()

    frequency = Counter(all_counts)
    frequency_list = [(item, count) for item, count in frequency.items()]
    np.savetxt(os.path.join('./',args.run_name+'_step3','dist_of_barcode_reads_counts.txt'), frequency_list, fmt='%d', delimiter='\t', header='barcode_counts\tfrequency')
...
def barcode_qc2(args, barcode_count, barcode_to_seq):
    ...
    plt.xlabel('value')
    plt.ylabel('Frequency')
    plt.savefig(os.path.join('./',args.run_name+'_step3','dist_of_barcode_counts_for_oligos_after_plasmid_construct.png'))
    plt.close()

    frequency = Counter(all_list_length)
    frequency_list = [(item, count) for item, count in frequency.items()]
    np.savetxt(os.path.join('./',args.run_name+'_step3','dist_of_barcode_counts_for_oligos_after_plasmid_construct.txt'), frequency_list, fmt='%d', delimiter='\t', header='barcode_counts\tfrequency')
...
def qc_rna(args, barcode_count, barcode_to_seq):
    ...
    plt.ylabel('coverage')
    plt.savefig(os.path.join('./',args.run_name+'_step3','down_sample_for_rna.png'))
    plt.close()

    combined_list = list(zip(np.log10(down_ratio), coverage))
    np.savetxt(os.path.join('./',args.run_name+'_step3','down_sample_for_rna.txt'), combined_list, fmt='%f %d', delimiter='\t', header='log10_ratio\tcoverage')
    ...
    plt.xlabel('log10 ratio')
    plt.ylabel('coverage')
    plt.savefig(os.path.join('./',args.run_name+'_step3','down_sample_for_rna_oligo.png'))
    plt.close()

    combined_list = list(zip(np.log10(down_ratio), coverage))
    np.savetxt(os.path.join('./',args.run_name+'_step3','down_sample_for_rna_oligo.txt'), combined_list, fmt='%f %d', delimiter='\t', header='log10_ratio\tcoverage')
...
def main():    
    ...
    # Main function
    os.makedirs(f'./{args.run_name}_step3')
    ...
    qc_rna(args, barcode_count, barcode_to_seq)
