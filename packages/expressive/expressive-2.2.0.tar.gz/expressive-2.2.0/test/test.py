...
def patch_config(field, new, debug=False):
    ...
    value = expressive.CONFIG
    if debug:  # pragma nocover FUTURE show diffs
        print("original config")
        print(text_indent(json.dumps(expressive.CONFIG, sort_keys=False, indent=4), "    "))
...
class TestGuess_dtype(unittest.TestCase):
    ...
    def test_signature_automatic(self):
        ...
        E = expressive.Expressive("a * b")
        r = expressive.signature_automatic(E._expr_sympy, data)
        ...
        E = expressive.Expressive("cos(a[i+1]) * b[i-1]^c")

        # generate as if internally-building
        # FIXME make part of regular path
        offset_values, offset_ranges = expressive.indexed_offsets_from_expr(E._expr_sympy)
        data, dtype_result_prepare, sig_prepare, result_passed = E._prepare(data, dtype_result=None)
        fn_jit, signature_auto, dtype_result_auto = expressive.signature_automatic(E._expr_sympy, data, offset_ranges)
        ...
        E = expressive.Expressive("cos(a[i+10]) * b[i-1]^c")
        offset_values, offset_ranges = expressive.indexed_offsets_from_expr(E._expr_sympy)
        data, dtype_result_prepare, sig_prepare, result_passed = E._prepare(data, dtype_result=None)
        with self.assertRaisesRegex(ValueError, re.escape("data[a] with shape (3,) doesn't have enough data to match combined offset(11)")):
            expressive.signature_automatic(E._expr_sympy, data, offset_ranges)
...
class TestSingleValues(unittest.TestCase):
    ...
    def test_lhs_indexed_mixed_single_array(self):
        ...
        E = expressive.Expressive("r[i] = a[i] + b")
...
class TestSumCompiler(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        #   >>> expr.doit()
        #   10*b**2 + 110*b + 385
        E = expressive.Expressive("Sum((x+b)**2, (x, 1, 10))")
        expr, symbols, indexers, results, block_sum_functions = expressive.breakout_Sum_to_loop(
            E._expr_sympy.rhs,
            E._symbols,
            E._indexers,
            E._results,
    ...
    def test_builds_over_Sum(self):
        ...
        # from direct (1->50)
        E = expressive.Expressive("Sum(log(x) + b, (x, 1, 50))")
        E.build(data, verify=True)
        E(data)
        # if comparing later
        # d = {
        ...
        # from indirect (1->m)
        E = expressive.Expressive("Sum(log(x) + b, (x, 1, m))")
    ...
    def test_single_values(self):
        ...
        E = expressive.Expressive("Sum(log(x) * m, (x, a, b))")
        E.build(data)
        E(data)  # TODO check result

        data["c"] = 5
        E = expressive.Expressive("Sum(log(x * c) * m, (x, a, b))")
        E.build(data)
        E(data)  # TODO check result
    ...
    def test_result_passed(self):
        ...
        E = expressive.Expressive("r = Sum(x + log(b), (x, a, b))")
    ...
    def test_vectorized_limits(self):
        ...
        E = expressive.Expressive("Sum(log(x) * a, (x, 1, m))")
        E.build(data)
        ...
        E = expressive.Expressive("Sum(log(x) * a, (x, m, 1000))")
        E.build(data)
    ...
    def test_nested_SumSum(self):
        ...
        with self.assertRaisesRegex(NotImplementedError, r"nested Sum instances not yet supported.*wraps"):
            E = expressive.Expressive("Sum( log(Sum(a*log(x),(x,1,n))) , (a, 1, 100) )")
    ...
    def test_summation_warns(self):
        ...
        #     "m": numpy.arange(10),
        # }
        # E = expressive.Expressive("Sum(x + b, (x, 1, m))")
        ...
        # E = expressive.Expressive("Sum(x + b, (x, 1, m)) + x")  # dummy var is used
        ...
        with self.assertRaisesRegex(NotImplementedError, re.escape("only exactly 1 Sum() limits is supported for now")):
            E = expressive.Expressive("Sum(a + b, (a, 1, m), (b, 1, m))")
        ...
        with self.assertRaisesRegex(ValueError, re.escape("fixed Sum() limits start(3) > stop(2) represents a zero or negative range")):
            E = expressive.Expressive("Sum(log(a) + b, (a, 3, 2))")
        ...
        E = expressive.Expressive(expr_full)
        with patch_config("translate_simplify.build.sum.try_algebraic_convert", False):
        ...
        #     E = expressive.Expressive("Sum(a + b, (a, 1, m), integer=True)")
        ...
        #       "b": 11,
        #   }
        #   # E = expressive.Expressive("Sum(sin(x**a) + (a*b), (x, 1, 100))")  # hangs forever [ISSUE 99]
        #   E = expressive.Expressive("Sum(log(x**a) + (a*b), (x, 1, 100))")  # can be decomposed
        #   data["c"] = numpy.arange(10)
        #   E = expressive.Expressive("Sum(log(x) + (a*b), (x, 1, 100)) + c")
        # FIXME test works fine now without this, is the error still relevant?
        # data = {
        #     "m": numpy.arange(1000),
        # }
        # E = expressive.Expressive("Sum(log(x), (x, 1, m))")
    ...
    def test_warn_thread(self):
        ...
        E = expressive.Expressive("Sum(a + log(m), (a, 1, m))")
