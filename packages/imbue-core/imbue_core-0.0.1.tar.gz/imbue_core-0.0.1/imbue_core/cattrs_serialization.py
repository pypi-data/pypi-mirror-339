...
def _serialize_attr_class_without_dont_serialize_fields(cls: type, converter: Converter) -> Callable[[Any], Any]:
    # Handle a pydantic model
    if _safe_issubclass(cls, BaseModel):
        return lambda x: {k: converter.unstructure(v) for k, v in get_pydantic_model_attributes(x).items()}
...
def _deserialize_using_type_marker(
    ...
    if _is_special_mapping_type(type_of_obj):
        ...
        obj = {k: _deserialize_serialized_object(v, _ShouldDeserialize, converter) for k, v in obj.items()}
        return cast(T, type_of_obj(**obj))
...
def _should_serialize_as_serialized_exception(t: type) -> bool:
    return _safe_issubclass(get_origin(t) or t, BaseException) and not attr.has(t)
    ...
    def get_converter(
        ...
        if exclude_dont_serialize_fields:
            converter.register_unstructure_hook_factory(
                ...
                partial(_serialize_attr_class_without_dont_serialize_fields, converter=converter),
            )
        else:
            converter.register_unstructure_hook_factory(
                _should_serialize_without_type_key, partial(_serialize_attr_class_factory, converter=converter)
            )
...
def force_serialize_to_dict(obj: Any) -> Mapping[str, Any]:
    ...
    if obj is None:
        return {}
    if _is_obj_supported_primitive(obj):
