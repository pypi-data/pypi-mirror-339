...
#Блок - Нейросети
def format_models(models_data):
    '''Форматирование информации о моделях в текстовый список'''
    formatted_lines = []
    for model in models_data:
        line = f"• {model['name']}: {model['description']}"
...
class Ai:
    ...
    def generate_voice_otvet(self, prompt: str = None, filename: str = "ozv.mp3", voice: str = 'nova') -> Union[bool, None]:
        '''Генерация аудио, доступные голоса: alloy, echo, fable, onyx, nova, shimmer, coral, verse, ballad, ash, sage, amuch, dan'''
        try:
            if prompt is None:
                raise TypeError("Забыли указать prompt")
            else:
                payload = {"model": "openai-audio", "message": [{"role": "user", "content": prompt}], "voice": voice}
                response = requests.post(f"https://text.pollinations.ai", headers={"Content-Type": "application/json"}, json=payload)
                if response.status_code == 200:
    ...
    def generate_image(self, prompt: str = None, width: int = 1024, height: int = 1024, seed: int = None, enhance: bool = True, private: bool = True, nologo: bool = True, model: str = 'flux', save: bool = False, filename: str = 'image.jpg') -> Union[str, bool]:
        ...
        try:
            if prompt is None:
                ...
                if save:
                    response = requests.get(f"https://pollinations.ai/p/{prompt}?width={width}&height={height}&seed={seed}&enhance={enhance}&private={private}&nologo={nologo}&model={model}", timeout=300)
                    if response.status_code == 200:
