...
class AlchemyEngine:
    ...
    def __init__(
        ...
        **engine_kwargs: Any,
        ...
        self._cache_registry = ModelCacheRegistry(self)

    @property
    def cache_registry(self) -> ModelCacheRegistry:
        """A cache registry that can be used to load and retrieve predefined model instances.
    ...
    def register_model(self, model_cls: Type[M], key_fn: KeyFunction):
        """Register a model with the :class:`cache repository <ModelCacheRepository>`.

        Parameters
        ----------
        model_cls : Type[M]
            The model type to register.
        key_fn : KeyFunction
            The key function used to extract the key values from an instance.
        """
        self._cache_registry.register(model_cls, key_fn)
...
class Repository(Generic[M]):
    ...
    def __init__(self, engine: AlchemyEngine, model_class: Type[M]):
        self.engine: AlchemyEngine = engine
...
class ModelCache(Mapping, Generic[M]):
    """A generic cache for preloading and accessing predefined model instances."""

    def __init__(self, model_cls: Type[M], key_fn: KeyFunction):
        self.model_cls = model_cls
        self.key_fn = key_fn
        self._cache: dict[Any, M] = {}

    def load(self, repository: Repository[M]):
        """Load the instances into the cache."""
        dtos = repository.find_all()
        self._cache = {self.key_fn(dto): self.model_cls.from_dto(dto) for dto in dtos}

    def __getitem__(self, key: Any) -> M:
        """
...
class ModelCacheRegistry:
    """A project-wide management class for a collection of :class:`ModelCache' instances."""

    def __init__(self, engine: AlchemyEngine):
        self._engine = engine
        self._registry: dict[Type[BaseModel], ModelCache[Any]] = {}

    def register(self, model_cls: Type[M], key_fn: KeyFunction):
        """Register a new :class:`ModelCache` if one of this type is not already registered.

        If a cache already exists for the given ``model_cls`` this method
        does nothing.
        ...
        engine : AlchemyEngine
            An engine that can be used to initialize a :class:`Repository`.
        """
        if model_cls in self._registry:
            return

        cache = ModelCache(model_cls, key_fn)
        cache.load(Repository(self._engine, model_cls))
        self._registry[model_cls] = cache

    def find(self, model_cls: Type[M], key: Any) -> M:
        ...
        """
        return self._registry[model_cls][key]

    def get_cache(self, model_cls: Type[M]) -> ModelCache[M]:
