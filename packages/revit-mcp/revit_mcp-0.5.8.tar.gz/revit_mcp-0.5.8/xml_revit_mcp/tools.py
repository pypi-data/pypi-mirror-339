...
def create_sheets(ctx: Context, method: str = "CreateSheets", params: List[dict] = None,
    ...
    try:
        ...
        # 发送请求并获取响应
        ctx.log("info", f"发送图纸创建请求，共{len(validated_params)}个图纸")
        from .server import get_revit_connection
        revit = get_revit_connection()
        response = revit.send_command(method, validated_params)
        ...
        if isinstance(response, dict):
            ...
            if "error" in response:
                ...
                if isinstance(errors, list) and errors:  # 只有当error列表非空时才处理
                    for error in errors:
                        if isinstance(error, dict):
                            ctx.log("error", f"图纸创建错误: {error.get('message', '未知错误')}")
                        else:
                            ctx.log("error", f"图纸创建错误: {error}")
                elif isinstance(errors, dict):
                    ctx.log("error", f"图纸创建错误: {errors.get('message', '未知错误')}")

            # 处理成功结果
            if "result" in response and response["result"]:
                result = response["result"]
                if isinstance(result, list):
                    ctx.log("info", f"图纸创建成功，返回{len(result)}个元素")
                    # 可以选择记录每个创建的元素
                    for elem in result:
                        if isinstance(elem, dict):
                            elem_info = f"ID: {elem.get('elementId', '?')}, 名称: {elem.get('name', '?')}"
                            ctx.log("debug", f"创建元素: {elem_info}")
                else:
                    ctx.log("info", "图纸创建成功")
        else:
            ctx.log("warning", f"收到非字典类型的响应: {type(response)}")
