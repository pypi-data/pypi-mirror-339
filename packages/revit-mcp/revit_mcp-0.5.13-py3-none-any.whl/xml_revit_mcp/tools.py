...
def call_func(ctx: Context, method: str = "CallFunc", params: List[dict[str, any]] = []) -> dict:
    ...
    - 自动验证参数有效性
    - 完善的错误处理机制
    - 系统支持功能:清理重复实例,清理已删除房间,清理房间方案,轴网两道尺寸线
    - 其他都为本地MCP文件夹查询是否存在相应用户自定义功能

    参数:
        ctx (Context): FastMCP上下文对象
        method (str): JSON-RPC方法名，默认为"CallFunc"
        params (List[Dict]): 功能参数列表，必须提供至少一个函数名，每个字典包含:
            - name (str): 要调用的功能名称(清理重复实例,清理已删除房间,清理房间方案,轴网两道尺寸线,及其他外部MCP文件夹调用的功能)
...
def get_view_data(ctx: Context, method: str = "GetViewData", params: List[dict[str, any]] = None) -> dict:
    """
    提取当前视图中所有信息，包括文本和图形实体数据，遵循JSON-RPC 2.0规范。

    特性:
    - 提取所有文本内容及其位置信息
    - 提取所有图形对象（线、弧、圆等）的几何信息,图形坐标点的数据都是毫米单位
    - 按图层组织返回的数据
    - 完善的错误处理机制
...
def create_room_separation_lines(ctx: Context, method: str = "CreateRoomSeparationLines",
                                 params: List[dict] = None) -> dict:
    """
    创建房间分割线，便于房间方案确定
    方案更新时可使用call_func[清理房间方案]来清理重新设计方案
    ...
    参数:
        ctx (Context): FastMCP上下文对象
        method (str): JSON-RPC方法名，默认为"CreateRoomSeparationLines"
        params (List[dict]): 线条数据列表，每个字典应包含:
            - p0 (str/dict): 起点坐标，格式为 "X,Y,Z"
            - p1 (str/dict): 终点坐标，格式为 "X,Y,Z"
            - pointOnArc (str/dict, optional): 弧上点坐标，仅用于创建弧线，格式为 "X,Y,Z"

    返回:
        dict: JSON-RPC 2.0格式的响应，成功时包含创建的元素信息
    ...
    示例:
        ...
        params = [
            ...
            },
            {
                "p0": "0,0,0",
                "p1": "3000,0,0",
                "pointOnArc": "1500,1000,0"
            }
        ]

        response = create_room_separation_lines(ctx, "CreateRoomSeparationLines", params)

        # 创建简单直线
        response = create_room_separation_lines(ctx,"CreateRoomSeparationLines", [
            {"p0": "0,0,0", "p1": "1000,0,0"},
            {"p0": "1000,0,0", "p1": "1000,1000,0"}
        ])
    """
    try:
        # 验证参数
        if not params or not isinstance(params, list):
            raise ValueError("必须提供至少一个线条数据")

        # 确保每个线条数据都有必要的坐标
        for i, line in enumerate(params):
            if not isinstance(line, dict):
                raise ValueError(f"第{i + 1}个线条数据必须是字典")

            if not line.get("p0") or not line.get("p1"):
                raise ValueError(f"第{i + 1}个线条数据必须包含p0和p1坐标")

        # 发送请求到Revit服务器
        from .server import get_revit_connection
        revit = get_revit_connection()
        response = revit.send_command(method, params)
        return response
        ...
        ctx.log("error", f"创建房间分割线时发生错误: {str(e)}")
        return {
            "jsonrpc": "2.0",
            "error": {
                "code": -32603,  # Internal error
                "message": f"创建房间分割线时发生错误: {str(e)}",
                "data": None
            },
            "id": ctx.request_id if hasattr(ctx, "request_id") else None
        }
