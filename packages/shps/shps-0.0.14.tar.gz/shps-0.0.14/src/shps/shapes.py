...
class Shape:
    ...
    def create_fibers(self, **kwds):
        return self.create_shape().fibers(**kwds)
...
class Channel(Shape):
    ...
    def create_mesh(self):
        ...
        return create_mesh(mesh_size=min(w,t)/4.0, patches=[
...
class Angle(Shape):
    ...
    def create_mesh(self):
        ...
        b = self.b
        d = self.d
        return create_mesh(mesh_size=t/2.5, patches=[
...
def load_aisc(SectionName, props="", units=None)->dict:
    ...
    for k, scale in UNITS:
        if k in SectData and SectData[k] is not None:
            SectData[k] *= scale
