...
cc = set()
def gen(EmailType):
    r = requests.get(f'http://185.158.132.66:80/gmail-temp/api-v2-iq/gen={EmailType}')
    print(f'{G26}{G1}{r.text}{G27}')
def message(EmailCheck,whileTrue=False):
    iso = [G1,G2,G3,G4,G5,G6,G7,G8,G9,G10,G11,G12,G13,G14,G15,G16,G17,G18,G19,G20,G21,G22,G23,G24,G25]
    while True:
        ...
        try:
            ...
            go = r.text
        if isinstance(go,dict) and "emails" in go:
            for email in go["emails"]:
                code = email.get("Code")
                if code not in cc:
                    color = random.choice(iso)
                    ee2 = {k: v.encode('utf-8').decode('unicode-escape') if isinstance(v,str) else v for k,v in email.items()}
                    print(f'\n{G26}{color}{json.dumps(ee2,indent=4,ensure_ascii=False)}{G27}')
                    cc.add(code)
