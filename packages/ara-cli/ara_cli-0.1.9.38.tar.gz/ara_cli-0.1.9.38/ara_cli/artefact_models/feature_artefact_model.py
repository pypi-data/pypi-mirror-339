...
class FeatureIntent(Intent):
    ...
    def deserialize_from_lines(cls, lines: List[str], start_index: int = 0) -> 'FeatureIntent':
        ...
        if not as_a:
            raise ValueError(f"Could not find {as_a_prefix}line")
        if not i_want_to:
            raise ValueError(f"Could not find {i_want_to_prefix}line")
        if not so_that:
            raise ValueError(f"Could not find {so_that_prefix}line")
...
class Example(BaseModel):
    ...
    @classmethod
    def from_row(cls, headers: List[str], row: List[str], has_title: bool) -> 'Example':
        """Create an Example instance from a table row."""
        ...
        return cls(title=title, values=values)
...
class Scenario(BaseModel):
    ...
    def validate_title(cls, v: str) -> str:
        """Ensure title is non-empty and stripped."""
        v = v.strip()
        if not v:
...
class ScenarioOutline(BaseModel):
    ...
    def validate_examples(cls, v: List[Example]) -> List[Example]:
        """Ensure examples list is non-empty."""
        if not v:
    ...
    def from_lines(cls, lines: List[str], start_idx: int) -> Tuple['ScenarioOutline', int]:
        ...
        if idx < len(lines) and lines[idx].strip() == 'Examples:':
            ...
            has_title = headers[0] == 'descriptive scenario title'
            idx += 1
            while idx < len(lines) and lines[idx].strip():
                row = [cell.strip()
                       for cell in lines[idx].split('|') if cell.strip()]
                example = Example.from_row(headers, row, has_title)
...
class FeatureArtefact(Artefact):
    ...
    def validate_intent(cls, v):
        if not v:
            raise ValueError("intent must be set in a FeatureArtefact")
        return v
    ...
    def _serialize_scenario_outline(self, scenario: ScenarioOutline) -> str:
        ...
        if scenario.examples:
            ...
            # Determine if any example has a title
            has_titles = any(
                example.title is not None for example in scenario.examples)

            # Set headers based on presence of titles
            headers = ["descriptive scenario title"] + \
                placeholders if has_titles else placeholders
            rows = [headers]
            ...
            for example in scenario.examples:
                if has_titles:
                    row = [example.title or ""] + \
                        [str(example.values.get(ph, ""))
                         for ph in placeholders]
                else:
                    row = [str(example.values.get(ph, ""))
                           for ph in placeholders]
                rows.append(row)
