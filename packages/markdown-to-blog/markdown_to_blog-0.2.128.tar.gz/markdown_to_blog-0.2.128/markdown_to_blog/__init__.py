...
@add_options(["service", "tui"])
def run_upload_images(input_, service: Optional[str] = None, tui: bool = False):
...
@click.option("--input", "-i", "input_", required=True, help="변환할 마크다운 파일 경로")
...
@mdb.command("set_client_secret", help="Google API 클라이언트 시크릿 파일을 설정합니다.")
...
@click.option(
    "--after",
    "-af",
    type=click.Choice(["now", "1m", "10m", "1h", "1d", "1w", "1M"], case_sensitive=True),
    default=None,
    prompt=True,
...
@click.argument("filename", type=click.Path(exists=True))
def run_publish(title, filename, is_draft, after, after_hour, blogid, labels, description):
    ...
    datetime_string = (
        ...
        else (get_datetime_after(after) if after is not None else get_datetime_after("now"))
    )
...
@click.option("--interval", "-i", default=1, help="발행 간격 (시간 단위, 기본값: 1시간)")
@click.option(
    "--draft",
    is_flag=True,
    default=False,
...
def run_publish_folder(blogid, interval, service, tui, draft, labels, folder_path):
    ...
    try:
        ...
        with click.progressbar(file_list, label=f"폴더 내 {total_files}개 파일 처리 중", show_pos=True) as files:
            for idx, file in enumerate(files, 1):
                try:
                    ...
                    if not file_title:
                        logger.warning(f"제목을 찾을 수 없음: {file_name}, 파일 이름을 제목으로 사용합니다.")
                    ...
                    # 포스트 업로드
                    logger.info(f"Publishing '{file_title}' to blog ID: {blog_id} at {datetime_string}")
                    upload_to_blogspot(
                        file_title,
                        file_path,
                        blog_id,
        ...
        if success_count == total_files:
            click.echo(f"✅ 모든 파일이 성공적으로 처리되었습니다. (총 {total_files}개)")
        else:
            click.echo(f"⚠️ 처리 완료: {success_count}개 성공, {error_count}개 실패 (총 {total_files}개)")
    except Exception as e:
