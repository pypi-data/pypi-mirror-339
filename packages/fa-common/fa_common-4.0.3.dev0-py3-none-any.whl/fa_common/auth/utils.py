...
from fa_common.routes.user.enums import AccessLevel
    ...
    if "sub" in payload:
        if get_profile and "aud" in payload and len(payload["aud"]) > 1 and "userinfo" in payload["aud"][1]:
            try:
                ...
                LOG.warning(
                    "Something went wrong retieving user profile falling back to creating user from " + f"the payload. Error: {err}"
                )
...
async def get_standalone_user() -> AuthUser:
    ...
    return AuthUser(sub="standalone", name="Default User", scopes=["read:me"])
...
async def get_api_key_user(api_key=Depends(get_api_key)) -> AuthUser | None:
    ...
    if api_key is not None and api_key != "" and get_settings().MASTER_API_KEY is not None and api_key == get_settings().MASTER_API_KEY:
        ...
        return AuthUser(sub="standalone", name="Default User", email="default-user@app.au", email_verified=False, scopes=["read:me"])
    return None
