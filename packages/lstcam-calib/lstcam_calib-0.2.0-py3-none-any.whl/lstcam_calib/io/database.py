...
class CalibrationDB(Component):
    ...
    data_tree_root = traits.Path(
        default_value=Path("/fefs/aswg/data/real"),
        help=(
    ...
    def _db_name_default(self):
        return f"pixel_calibration_tel_{self.tel_id:03d}"
    ...
    def find_document_by_file(self, type, path: Path):
        ...
        try:
            rel_path = path.resolve().absolute().relative_to(self.data_tree_root)
        except ValueError:
            raise ValueError(
    ...
    def add_calibration_file(
        ...
        **kwargs,
        ...
        if (
            drs4_baseline_path
            and self.find_document_by_file(
                CalibrationType.DRS4_BASELINE, drs4_baseline_path
            )
            is None
        ):
        ...
        if ffactor_systematics_path and (
            self.find_document_by_file(
    ...
    def _add_file(self, type_, path, provenance_path, **kwargs):
        ...
        path = path.resolve().relative_to(self.data_tree_root)
        provenance_path = provenance_path.resolve().relative_to(self.data_tree_root)
    ...
    def invalidate_file_quality(self, type, file):
        ...
        query_new_doc = {
            "path": str(file.resolve().absolute().relative_to(self.data_tree_root))
        }
        update = {
    ...
    def remove_file(self, type, file):
        ...
        rel_path = Path(file).resolve().absolute().relative_to(self.data_tree_root)
