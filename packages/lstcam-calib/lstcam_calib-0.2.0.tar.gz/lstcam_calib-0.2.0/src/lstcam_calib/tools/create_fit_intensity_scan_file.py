...
class FitIntensityScan(Tool):
    ...
    def setup(self):
        ...
        Provenance().add_output_file(
            str(self.output_path), role=PROV_OUTPUT_ROLES["fit_intensity_scan"]
        )
        channel = ["HG", "LG"]
            ...
            with PdfPages(self.plot_path) as pdf:
                ...
                for chan in self.gain_channels:
                    ...
                    for pix in [0, 600, 1200, 1800]:
                        ...
                        mask = self.unusable_pixels[chan][pix]
                        sig = np.ma.array(
                            self.signal[chan][pix], mask=mask
                        ).compressed()
                        var = np.ma.array(
                            self.variance[chan][pix], mask=mask
                        ).compressed()
