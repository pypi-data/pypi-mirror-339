...
from descartcan.llm.strategy import (
    ModelStrategy, ModelMetrics, ChatResponse, ModelConfig, TokenUsage, ChatStreamResponse, ChatRequest
)
...
class BaseStrategy(ModelStrategy):
    ...
    def _initialize_client_pool(self):
        ...
        for key_id, key_info in self._keys.items():
            try:
                ...
                logger.warning(f"Failed to initialize client with key_id {key_id}: {str(e)}")
    ...
    def _gen_messages(self, question: str, system: Optional[str] = None, history: Optional[List[Dict]] = None) -> List[Dict[str, str]]:
        messages = []
        if system:
    ...
    def calculate_messages_token_count(self, messages: List[Dict[str, str]]) -> TokenUsage:
        prompt_tokens = 0
    ...
    def _create_chat_request(
        ...
        **kwargs
        ...
        return ChatRequest(
            ...
            stream=stream,
            presence_penalty=kwargs.get('presence_penalty', self._property.presence_penalty),
            frequency_penalty=kwargs.get('frequency_penalty', self._property.frequency_penalty),
...
class BaseImplementStrategy(BaseStrategy):
    """OpenAI API 兼容的基础策略类"""
    async def chat(
        self, question: str, system: Optional[str] = None, 
        history: Optional[List[Dict]] = None, **kwargs
    ) -> ChatResponse:
    ...
    async def chat_stream(
        ...
        return ChatStreamResponse(content=stream_content(), prompt_tokens=token_usage.prompt_tokens)
