...
class Conv2DTranspose(Conv2D):
    ...
    pad_top, pad_bottom : int
        The number of data points to be added to the top and right bottom of the data, respectively.
        Corresponds to each half of `pad_height`, with `pad_top` being the first to increment.
    out_pad_height, out_pad_width : int 
        The total number of data points to be added to the output array as padding on each axis before being
        returned. Represents the padding for height and width, respectively
    ...
    Examples
    --------
    >>> layer1 = Conv2DTranspose(ReLU(), 2, 3, (5, 9, 8), strides=1)
    >>> in_arr = np.random.uniform(0.0, 1.0, (5, 9, 8))
    ...
    def __init__(self, funct: Activation, layers: int, kernel_size: tuple[int, int] | int, 
                 ...
                 out_padding: tuple[int, int] | int = 0,
                 biases: bool = True, optimizer: Optimizer = StandardGD()) -> None:
        """
        Initializes a `Conv2DTranspose` layer using given parameters.
        ...
        padding : tuple[int, int] | int, default: 0
            ...
            be added in a standard `Conv2D` layer.
        out_padding : tuple[int, int] | int, default: 0
    ...
    def forward(self, data: np.ndarray, training: bool = False) -> np.ndarray:
        ...
        p_w = self.out_size[2] - self.out_pad_width + self.pad_width + self.kernel_width - (dil_data.shape[3] + 1)
        ...
        conv_window = np.lib.stride_tricks.as_strided(pad_data, 
                                                      data_win_shape, 
                                                      new_strides)
        conv_val = np.einsum("nfchwxy,fchw->nfxy", conv_window, self.kernel_weights)
        conv_val = conv_val[:, :, 
    ...
    def backward(self, cost_err: np.ndarray) -> np.ndarray:
        ...
        opt_view = np.lib.stride_tricks.as_strided(opt_pad, 
                                                   shape=opt_win_shape, 
                                                   strides=opt_strides)
        self.kernel_weights += np.einsum("nfchwxy,nfxy->fchw", opt_view, opt_grad_pad)
    ...
    def save_to_file(self, filename: str = None) -> str | None:
        ...
        """
        write_ret_str = f"Conv2DTranspose\u00A0{repr(self.funct)}\u00A0{self.kernel_weights.shape[1]}" + \
    ...
    @staticmethod
    def from_save(context: str, file_load: bool = False) -> 'Conv2DTranspose':
        """
        ...
        -------
        Conv2DTranspose
            A new `Conv2DTranspose` layer containing all of the information encoded in the string or file provided.
        """
    ...
    def from_kernel(funct: Activation, input_size: tuple[int, int, int], 
        ...
        ------
        AssertionError
            If the input channel sizes are not equal between the kernel and the inputs, or if the
        ...
        -------
        Conv2DTranspose
            A new `Conv2DTranspose` layer containing all of the information given and interpreted from the input kernel.
        """
        assert len(kernel.shape) == 4, f"Kernel shape must be of length 4 instead of length {len(kernel.shape)}."
