...
class FeatureHandler:
    ...
    def avg_trait_values(self, data_type, id1, id2):
        ...
        and non-standardized data if possible.
            ...
            if data_type == "trait":
                if len(data_avg) > 0:
                    ...
                    self.data_trait[std_type].loc[
                        self.data_trait[std_type].ObsDataID.isin(
                            update_data.ObsDataID), data_val_col
                    ] = update_data.avg
            elif data_type == "year":
                if len(data_avg) > 0:
                    ...
                    self.data_year[std_type].loc[
                        self.data_year[std_type].ObsDataID.isin(
                            update_data.ObsDataID), data_val_col
                    ] = update_data.avg
            else: # data_type == "lonlat"
                for l in ["lon", "lat"]:
                    if len(data_avg[l]) > 0:
                        ...
                        self.data_lonlat[l][std_type].loc[
                            self.data_lonlat[l][std_type].ObsDataID.isin(
                                update_data.ObsDataID), data_val_col
                        ] = update_data.avg
