...
class AnalysisTreeNode(AnyNode):
    ...
    def _execute_algorithm(self, algorithm_name, args):
        ...
        algorithm_metadata = concordance.available_algorithms.get(algorithm_name)
    ...
    def add_subset_node(self, algo_tuple, keep_arrangement=True):
        ...
        target_algo = {"subset": {"algorithm_name": algorithm_name, "args": args}}
    ...
    def add_arrangement_node(self, ordering=[], grouping=None):
        ...
        target_algos = {
            "ordering": [{"algorithm_name": name, "args": a} for name, a in ordering],
            "grouping": {"algorithm_name": grouping[0], "args": grouping[1]} if grouping is not None else None
        }
    ...
    def view(self) -> dict:
        ...
        if hasattr(self, "grouping_result"):
            grouping = None
            if isinstance(self.grouping_result, dict):
                if "partitions" in self.grouping_result:
                    grouping = self.grouping_result["partitions"]
                elif "clusters" in self.grouping_result:
                    grouping = self.grouping_result["clusters"]
            if grouping is not None:
                view_dict["grouping"] = grouping
        ...
        if hasattr(self, "ordering_result") and "rank_keys" in self.ordering_result:
            rk_dict = self.ordering_result["rank_keys"]
            # Build line_info from rank_keys, but include only entries for lines in selected.
            for algo_key, rank_map in rk_dict.items():
                for line_id, rank_value in rank_map.items():
                    if line_id in selected:
                        if line_id not in line_info:
                            line_info[line_id] = {"rank_keys": {}}
                        line_info[line_id]["rank_keys"][algo_key] = rank_value
        if line_info:
            view_dict["line_info"] = line_info

        # 6. Token spans.
        if hasattr(self, "token_spans") and self.token_spans is not None:
            try:
                if isinstance(self.token_spans, pd.DataFrame):
                    view_dict["token_spans"] = self.token_spans.to_dict(orient="records")
                else:
                    view_dict["token_spans"] = self.token_spans
            except ImportError:
                view_dict["token_spans"] = self.token_spans
    ...
    def available_algorithms(self):
        ...
        for algo_name, algo_metadata in concordance.available_algorithms.items():
            ...
            if matches_conditions:
                available_algos[algo_name] = algo_metadata
    ...
    def _enrich_schema(self, schema: dict) -> dict:
        """
        Enriches the base args schema by evaluating any 'x-eval' expressions.
        The expressions are evaluated in a namespace where only the Concordance (as 'conc') is provided.
        The expression's result can be:
          - A dict: in which case the keys/values are merged into the property definition.
          - A list: in which case it is assigned to the "enum" key.
          - Other types: in which case it might be set as the default.

        Parameters:
            schema (dict): The base args schema.
        ...
        for prop_name, prop in properties.items():
            if "x-eval" in prop:
                expr = prop["x-eval"]
                try:
                    # Evaluate the expression with a limited namespace.
                    result = eval(expr, {"conc": self.concordance(), "node": self})
                    # If result is a dict, merge it into the property.
                    if isinstance(result, dict):
                        prop.update(result)
                except Exception as e:
                    # Optionally log the error; here we just add an error key.
                    prop["x-eval-error"] = str(e)
    ...
    def summary(self) -> str:
        """
        Returns a textual summary of an analysis tree node.

        If a non-null label is present, it is included.
        The algorithms for the node are listed (for both subset and arrangement nodes) with each algorithm's name and its parameters.

        Returns:
            str: A textual summary of the node.
        """
        parts = []
        # Include label if present and non-null
        if hasattr(self, "label") and self.label:
            parts.append(f"Label: {self.label}")

        # List algorithms if available
        if hasattr(self, "algorithms") and self.algorithms:
            algo_summaries = []
            for algo_type, algo_val in self.algorithms.items():
                if algo_val is None:
                    continue
                # Algorithms may be stored as a single dict or a list
                if isinstance(algo_val, list):  # for multiple ordering algorithms
                    for algo in algo_val:
                        summary_str = f"{algo.get('algorithm_name', 'Unknown')}: {algo.get('args', {})}"
                        algo_summaries.append(summary_str)
                elif isinstance(algo_val, dict): # for the single subset/grouping algorithm
                    summary_str = f"{algo_val.get('algorithm_name', 'Unknown')}: {algo_val.get('args', {})}"
                    algo_summaries.append(summary_str)
            if algo_summaries:
...
class Concordance:
    ...
    def __init__(
        ...
        if any([metadata, tokens, matches, info]):
            self.load(metadata, tokens, matches, info)
    ...
    def add_annotation(self, algo_tuple, column_names=None):
        ...
        if scope not in ["token", "line"]:
            raise ValueError(
                f"Invalid scope '{scope}' for algorithm '{algorithm_name}'. Must be 'token' or 'line'.")
