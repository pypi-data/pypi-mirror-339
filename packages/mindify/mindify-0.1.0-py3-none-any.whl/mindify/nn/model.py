...
class MLP(nn.Module):
    def __init__(self, input_dim, out_dim, hidden_dim: int = None, num_blocks: int = None, dropout: float = None):
        super().__init__()

        hidden_dim = input_dim if hidden_dim is None else hidden_dim
        num_blocks = 0 if num_blocks is None else num_blocks
        dropout = 0.1 if dropout is None else dropout

        self.model = torch.nn.Sequential(
            LinearBlock(input_dim, hidden_dim, dropout),
            torch.nn.Sequential(
                *[LinearBlock(hidden_dim, hidden_dim, dropout) for _ in range(num_blocks)]
            ),
...
class ClassifierModel(nn.Module):
    def __init__(self, hidden_size: int, num_labels: int, dropout: float = 0.1):
        ...
        self.model = nn.Sequential(
            nn.Tanh(),
            MLP(hidden_size, out_dim=num_labels, num_blocks=1, dropout=dropout)
        )
