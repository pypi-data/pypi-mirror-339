...
pub struct TransformResult {
    result: Option<(Array4<f32>, usize, Option<f32>)>,
}

impl TransformResult {
    pub(crate) fn new(result: Option<(Array4<f32>, usize, Option<f32>)>) -> Self {
        Self { result }
    }

    pub fn to_preprocessed_bytes(self, minimum_element: f32) -> Option<Vec<Vec<u8>>> {
        self.result.map(|(mut result, frame_count, start_idx)| {
            let shape = result.shape();

            let nframes = shape[3];
            let mut index: f32 = start_idx.unwrap_or(0.0);
            // let steps: f32 = 1.0 / 25.0 * 80.0;

            // padding
            let target_nframes = (((nframes as f32 / MEL_STEP_IN_FRAME) as u32) as f32
                * MEL_STEP_IN_FRAME) as usize
                + 16;
            if target_nframes > nframes {
                let padding_size = target_nframes - nframes;

                let padding = ndarray::Array4::from_elem([1, 1, 80, padding_size], minimum_element);

                result = ndarray::concatenate(ndarray::Axis(3), &[result.view(), padding.view()])
...
impl Audio2MelSpectrogram {
    ...
    pub fn transform<F>(
        ...
        audio: Vec<f32>,
        norm_fn: F,
    ) -> Option<(Array4<f32>, usize, Option<f32>)>
    where
        F: Fn(&Array2<f32>) -> Array2<f32>,
    {
        ...
        if mel_outputs.is_empty() {
            ...
            Some((stacked_mel, frame_count, None))
        }
        // // .map(|v| {
        // //     let raw_data = v.as_slice().unwrap();
...
impl Audio2MelSpectrogramV2RT {
        ...
        F: Fn(&Array2<f32>) -> Array2<f32>,
    {
        ...
        if mel_outputs.is_empty() {
            ...
            Some((stacked_mel, frame_count, Some(start_idx)))
        }
        // // .map(|v| {
        // //     let raw_data = v.as_slice().unwrap();
        // //     cast_slice(raw_data).to_vec()
