...
pub fn calculate_frame_count(length: usize, start_idx: f32, step_size: f32, window_size: usize) -> usize {
    if length <= window_size {
        return 0;
    }
    
    // Calculate the available space for frames
    let available_length = length - window_size;
    let effective_start = (start_idx + 0.5) as usize;
    
    if effective_start >= available_length {
        return 0;
    }
    
    // Calculate how many complete steps fit in the remaining space
    let remaining_space = available_length - effective_start;
    ((remaining_space as f32 / step_size).floor() as usize).max(0)
}
