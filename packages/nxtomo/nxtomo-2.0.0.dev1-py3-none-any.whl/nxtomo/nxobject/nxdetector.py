...
from nxtomo.utils.io import deprecated, deprecated_warning, ignore_deprecation_warning
...
class NXdetector(NXobject, _ObjectWithPixelSizeMixIn):
    ...
    def y_rotation_axis_pixel_position(self) -> float:
        ...
        .. warning::
            ...
            This field is not handled at the moment by tomotools. Only x position is handled.
        """
    ...
    def set_transformation_from_x_flipped(self, flipped: bool | None):
        ...
        self.transformations.add_transformation(DetZFlipTransformation(flip=flipped))
    ...
    def y_flipped(self):
        ...
        return DetYFlipTransformation(flip=True) in self.transformations.transformations
    ...
    def _concatenate_except_data(nx_detector, nx_objects: tuple):
        ...
        for nx_obj in nx_objects[1:]:
            ...
            if nx_detector.distance is not None and not numpy.isclose(
                nx_detector.distance.magnitude, nx_obj.distance.magnitude
            ):
                _logger.warning(
                    f"found different distance value. ({nx_detector.distance.magnitude} vs {nx_obj.distance.magnitude}). Pick the first one"
                )
            if (
...
class NXdetectorWithUnit(NXdetector):
    ...
    def data(self) -> pint.Quantity | None:
        """
        Detector data.
        Can be a numpy array, list of DataUrl, VirtualSource, or a pint.Quantity.
        """
        return self._data
