...
@numba.jit
def cross_correlation (y1, y2, maxlag=None):
    """
    Compute cross correlation.
    """
    if maxlag is None:
        maxlag = y2.size
    maxlag = int (min(y2.size, maxlag))
    ccf = np.zeros(maxlag, dtype=np.int64)
    for lag in range (ccf.size):
        y1_max = min (y2.size - lag, y1.size)
        y2_max = min (y2.size, y1.size + lag)
        ccf[lag] = np.sum (y1[:y1_max] * y2[lag:y2_max])
    return ccf
...
def compute_shift (sub1, sub2, maxlag, freq_init, shiftmax, lags=None, plot=False,
  ...
  corr_vec_1 = cross_correlation (sub2, sub1, maxlag=maxlag)
  corr_vec_1 = np.delete (corr_vec_1, 0)
  corr_vec_1 = corr_vec_1 [::-1]
  corr_vec_2 = cross_correlation (sub1, sub2, maxlag=maxlag)
