...
def ampl_mode (l, m, i, sini, cosi, instr='geometric') :
  '''
  ...
  if (instr=='kepler') | (instr=='geometric') :
    ...
    if (l == 3) & (abs(m) == 1) : 
       R = (3.0/64.0)*((5.0*np.cos(2.0*i))+3)**2*sini**2
    if (l == 3) & (abs(m) == 2) : 
       R = (15.0/8.0)*cosi**2*sini**4
    if (l == 3) & (abs(m) == 3) : 
       R = (5.0/16.0)*sini**6
    if (l == 4) & (abs(m) == 0) : 
       R = (1.0/64.0)*((35.0*cosi**4)-(30.0*cosi**2)+3)**2
    if (l == 4) & (abs(m) == 1) : 
       R = (5.0/256.0)*(((7.0/2.0)*np.sin(4.0*i))+(np.sin(2.0*i)))**2
    if (l == 4) & (abs(m) == 2) : 
       R = (5.0/128.0)*((7.0*np.cos(2.0*i))+5)**2*sini**4
    if (l == 4) & (abs(m) == 3) : 
       R = (35.0/16.0)*cosi**2*sini**6
    if (l == 4) & (abs(m) == 4) : 
       R = (35.0/128.0)*sini**8
  ...
  if R==-1 :
    print ('Amplitude ratio within degree returned -1 for l =', l, ', m =', m)
