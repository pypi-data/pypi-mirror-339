...
from .fit_tools import a2z_to_pkb, cornerplot_wrapper, check_a2z
...
def likelihood_mode (param_pkb, freq, psd, back, free=1, use_sinc=False, 
                     asym_profile='nigam-kosovichev', param_wdw=None, instr='geometric',
                     fit_amp=False, projected_splittings=False) :
  '''
  Compute likelihood of the selected mode.
  '''
  log_l_sw = wrapped_log_l (freq, psd, back, free, param_pkb, 
                            instr, param_wdw, use_sinc, asym_profile, 
                            fit_amp, projected_splittings)
  log_l_s = wrapped_log_l (freq, psd, back, free, param_pkb[param_pkb[:,1]<2, :], 
                           instr, param_wdw, use_sinc, asym_profile,
                           fit_amp, projected_splittings)

  return log_l_sw, log_l_s

def likelihood_h0 (psd) :
  '''
  Compute likelihood of the H0 hypothesis.
  '''
  model_h0 = np.full (psd.size, np.mean (psd))
  aux = psd / model_h0 + np.log (model_h0)
  log_l_h0 = np.sum (aux)

  return log_l_h0

def model_likelihood_h1 (param_pkb, freq, psd, back, free=1, use_sinc=False, 
                         asym_profile='nigam-kosovichev', param_wdw=None, instr='geometric') :
  '''
  Compute the likelihood of the different models to compare.

  :return: in the following order, negative logarithmic likelihood of 
    the model with two modes, of the model with only the strong mode
    and of the H0 model. 
...
def wrapper_loop_sequential (flatchain, labels, degrees, order, add_ampl, instr, use_sinc, 
  ...
  for param in tqdm (flatchain) :
    df = chain_element_to_a2z (param, labels, degrees, order, 
                               add_ampl=add_ampl, instr=instr) 
    pkb = a2z_to_pkb (df)
    ...
    #print (log_l_h0, log_l_sw, log_l_s)
...
def bayes_factor (freq, psd, back, df_a2z, n, strategy='pair', l02=True, dnu=None, 
                  do_not_use_dnu=True, size_window=None,  
                  thin=10, discard=0, instr='geometric', use_sinc=False, 
                  asym_profile='nigam-kosovichev', hdf5Dir='./',
  ...
  if parallelise :
    ...
    for param in flatchain :
      arg.append ([param, labels, degrees, order, add_ampl, instr, use_sinc,
                  asym_profile, param_wdw, log_l_h0, aux_freq, aux_psd, aux_back,
                  fit_amp, projected_splittings])
    p = ProcessPool (nodes=nodes) 
    chunksize = flatchain.size // nodes
    a_proba = list (tqdm (p.imap (wrapper_loop_parallel, arg, chunksize=chunksize), 
                    total=flatchain.shape[0])) 
...
def compute_lnK (psw, ps, p0) :
  '''
  Compute quality assurance parameter ln K for strong-weak mode detection 
  and strong only mode detection.
