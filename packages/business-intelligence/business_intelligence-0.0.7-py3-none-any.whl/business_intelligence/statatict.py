...
class config_table:
    ...
    """
    Los intervalos de clase son segmentos o rangos definidos de valores numéricos en los que se agrupan los datos para facilitar su análisis y visualización en estadística. Son utilizados principalmente cuando se trabaja con datos numéricos continuos, como edades, ingresos, temperaturas, entre otros.

    Características de los Intervalos de Clase

    Agrupación de Datos: Los intervalos de clase agrupan valores similares de datos dentro de rangos específicos. Esto ayuda a resumir grandes conjuntos de datos y a simplificar su interpretación.

    Mutuamente Excluyentes: Cada dato en el conjunto debe pertenecer a exactamente un intervalo de clase. No puede pertenecer a más de un intervalo ni quedar fuera de todos los intervalos definidos.

    Exhaustivos: Todos los datos deben estar cubiertos por los intervalos de clase definidos. No debe haber datos que no estén incluidos en al menos uno de los intervalos.

    Definición de Límites: Cada intervalo de clase tiene límites inferiores y superiores bien definidos. Por convención, el límite inferior de un intervalo es inclusivo (se incluye en el intervalo) y el límite superior es exclusivo (no se incluye en el intervalo).

    Ancho Uniforme o Variable: Los intervalos de clase pueden tener anchos uniformes, donde todos los intervalos tienen el mismo tamaño, o anchos variables, donde los intervalos pueden tener tamaños diferentes dependiendo de la distribución de los datos.
    ...
    # Función para calcular los intervalos de clase     
    ...
    def tabla_frecuencia_discreta(dato):
    ...
    def estadistica_descriptiva(variable):
    ...
    def graficar_histograma_con_curva(datos, columna):
        """
        ...
        #LLamando a la función creada previamente
        print("======================================")
        print("Estadística descriptiva")
        print("======================================")
        estadistica_descriptiva(data[columna])

df = pd.read_csv('dataset_food.csv', low_memory = False)

tr = config_table()

print(tr.intervalos_clase(df.venta))