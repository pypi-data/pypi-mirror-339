...
class _CognitiveServiceAsyncRestAPI():
    ...
    """
    _rest_client: CognitiveServiceAsyncRestClient

    def __init__(self):
        self._rest_client = CognitiveServiceAsyncRestClient()

    async def fetch_translation(self, path, payload):
        response = await self._rest_client.post(path, json=payload)
        return await response.json()

    async def translate_text_async(self, texts: Union[str, List[str]], to_lang: List[str], from_lang: Optional[str] = None):
        path = self._rest_client._get_default_base_url() + "texttranslation/translate?api-version=3.0"
        for lang in to_lang:
            path += f"&to={lang}"
        ...
        for chunk in chunks:
            ...
            tasks.append(self.fetch_translation(path, payload))
        responses = await tqdm_asyncio.gather(*tasks)
        await self._rest_client.close_session()
