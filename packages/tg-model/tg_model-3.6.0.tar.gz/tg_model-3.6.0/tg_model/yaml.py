...
class CollectionMixin(object):
    ...
    @staticmethod
    def _create_property(key, parent_prop):
    ...
    def basic_classifications(self):
        return self._get_or_set_multi_item("basic_classifications")

    @basic_classifications.setter
    def basic_classifications(self, value):
        self._get_or_set_multi_item("basic_classifications", value)

    @property
    def gnd_subjects(self):
        return self._get_or_set_multi_item("gnd_subjects")

    @gnd_subjects.setter
    def gnd_subjects(self, value):
        self._get_or_set_multi_item("gnd_subjects", value)

    @property
    def rights_holder(self):
        return self._get_or_set_multi_item("rights_holder")

    @rights_holder.setter
    def rights_holder(self, value):
        self._get_or_set_multi_item("rights_holder", value)

    @property
    def collector(self):
        return self._get_or_set_multi_item("collector")

    @collector.setter
    def collector(self, value):
        self._get_or_set_multi_item("collector", value)
        ...
        def _dump_value_recursive(
            ...
            if isinstance(value, dict):
                ...
                for key, val in value.items():
                    if key == "comments":  # comments-Einträge überspringen
                        continue
                    ...
                    for item in value:
                        if isinstance(item, dict):
                            _dump_value_recursive(
                                item, level + 1, path_parts, list_indent=indent
                            )
                        else:
...
class CollectionConfigTemplate(YAMLConfigTemplate, CollectionMixin):
    ...
    def _set_or_initialize_multi_items(
        ...
        def _set_helpr(item, value):
            # we need this to avoid syntax errors, as it is not possible to
            # set something like: `getattr(self, key) = value`
            item = value
            return item

        if collection_config.content and getattr(collection_config, key)():
            _set_helpr(getattr(self, key), getattr(collection_config, key))
        else:
            _set_helpr(getattr(self, key), default_value)

    def hits_in_files(self, xpath):
    ...
    def process_proposals(self, attribs, orig_content=None):
        project_config = ProjectConfig(self.projectpath)
        ...
        for attrib in attribs:
            ...
            if attribs[attrib]:
                if "proposals" in attribs[attrib]:
                    ...
                    min_hitrate = project_config.min_hitrate  # in %
                    winner = None
...
%s percent hitrate"
    ...
    def render(self, overwrite=False):
        ...
        if (
            overwrite
            or not collection_config.content
            or not collection_config.short_title
        ):
            self.short_title = self.subproject["name"]
        else:
            self.short_title = collection_config.short_title

        if (
            overwrite
            or not collection_config.content
            or not collection_config.long_title()
        ):
            if self.projectname:
                self.long_title(
            ...
            self.long_title(collection_config.long_title())
