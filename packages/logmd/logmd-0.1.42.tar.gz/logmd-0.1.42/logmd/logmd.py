...
from logmd.utils import is_dev, get_fe_base_url, get_run_id, get_upload_url, update_pdb_positions, fix_pdb_bfactor_string
...
class LogMD:
    ...
    def __call__(self, atoms, dyn=None, data_dict=None, calc=False):
        ...
        if type(atoms) == str: 
            ...
            if calc:
                ...
                atoms = ase.io.read(io.StringIO(atom_string), format='proteindatabank')
                atoms.calc = calc
    ...
    def notebook(self, width=900, height=600, label=None, suffix=''):
        ...
        rich.print(f"{LOGMD_PREFIX}Embedded Url=[blue][link={self.url}?{suffix}]{self.url}?{suffix}[/link][/] 🚀")
