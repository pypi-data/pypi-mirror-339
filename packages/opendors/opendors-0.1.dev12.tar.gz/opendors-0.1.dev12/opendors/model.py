...
class SvnStructure(
    namedtuple(
        "Structure", ["url", "trunk", "branches", "tags", "compliance", "sub_dirs"]
    )
):
    def __hash__(self):
...
LAX_SHA1_PATTERN = r"[0-9a-f]{5,40}"
SWH_ID_PATTERN = (
    r"^swh:[1-9]:(rel|rev):[0-9a-f]+(;(origin|visit|anchor|path|lines)=\S+)*$"
)
...
class LatestVersion(NoExtraFieldsModel):
    ...
    commit_sha: Optional[Annotated[str, Field(pattern=LAX_SHA1_PATTERN)]] = None
    """The commit sha of the latest version tag or commit if the latest version is of a git repository."""

    tag_sha: Optional[Annotated[str, Field(pattern=LAX_SHA1_PATTERN)]] = None
