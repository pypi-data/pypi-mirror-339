...
class Data:
    ...
    def median_2d(self, time_window_s, position_window_m) -> npt.ArrayLike:
        ...
        time_window_samples = int(time_window_s / self.parameters.time_step_s)
        position_window_samples = int(
    ...
    def energy_bands(
        ...
        fft = 2 * np.abs(np.fft.rfft(reshaped, axis=1)) / np.shape(reshaped)[1]
        ...
        for energy_band in energy_bands_Hz:
            ...
            energy_band[1] = min(energy_band[1], frequencies[-1])
            stop_index = int(np.ceil(energy_band[1] / frequencies[1])) + 1
            energy[f"{energy_band[0]}-{energy_band[1]} Hz"] = np.sum(
                np.square(fft[:, start_index:stop_index, :]), axis=1
            ) / (energy_band[1] - energy_band[0])
