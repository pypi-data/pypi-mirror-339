        ...
        for key, value in element.events.__dict__.items():
            if value is not None:
                if isinstance(value, str) or callable(value):
                    html += f" {key}='{self.__event_id(value) if callable(value) else value}'"
            ...
            root.childs[0].childs.extend(
                [
                    self.__create_default_element(
                        tag='script',
                        content=f"window.PYWEBER_WS_PORT = {os.environ.get('PYWEBER_WS_PORT', None) or config.get('websocket', 'port')}",
