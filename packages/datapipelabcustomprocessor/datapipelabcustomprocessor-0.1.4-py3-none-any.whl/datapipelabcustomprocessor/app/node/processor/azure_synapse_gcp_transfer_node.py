...
class AzureSynapseToGCPBucketNode(TNode):
    ...
    def __init__(self, spark, tnode_config):
        ...
        self.gcp_bucket_name = tnode_config['options']['gcp_bucket_name']
        self.source_directory_path = tnode_config['options']['source_directory_path']
        self.file_extension = tnode_config['options'].get('file_extension', None)
        self.subdirectory = tnode_config['options'].get('subdirectory', None)
        self.spark = spark
    ...
    def copy_and_upload_file(self, adls_path, relative_path, local_base_dir, gcp_bucket, keep_local_file=False):
        ...
        # Define the subdirectory within the GCP bucket
        subdirectory = self.subdirectory  # Modify this to your desired subdirectory
        if subdirectory:
            gcp_blob_name = os.path.join(subdirectory, relative_path).replace('\\', '/')  # Unix-style
        else:
            gcp_blob_name = relative_path.replace('\\', '/')  # Unix-style
    ...
    def _process(self):
        ...
        gcp_bucket_name = self.gcp_bucket_name # os.environ['GCP_BUCKET_NAME']
        ...
        source_directory_path = self.source_directory_path # os.environ['SOURCE_PATH']
        file_extension = self.file_extension # os.environ.get('FILE_EXTENSION', None)  # optional
