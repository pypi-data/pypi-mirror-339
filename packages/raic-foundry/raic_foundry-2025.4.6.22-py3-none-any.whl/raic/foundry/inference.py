...
from shapely import Point, from_wkb, from_wkt
...
class InferenceRun():
    ...
    def fetch_predictions_as_dataframe(self, include_embeddings: bool = True) -> pd.DataFrame:
        ...
        def get_centroid(centroid: str | None) -> Point | None:
            return None if centroid is None else from_wkb(centroid).centroid
        ...
        with tempfile.NamedTemporaryFile(mode='w+t', delete=True) as tmpfile:
            ...
            for record in iterator:
                ...
                complete_row = {field: json.get(field, '') for field in fieldnames}
    ...
    def delete(self):
        if self._is_raic_vision:
            return CascadeVisionClient().update_run(self.id, is_deleted=True)
        else:
