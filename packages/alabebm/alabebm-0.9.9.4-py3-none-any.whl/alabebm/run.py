...
def run_ebm(
    ...
    Args:
        ...
        true_order_dict (Optional[Dict[str, int]]): biomarker name: the initial correct order of it (if known)
    ...
    if true_order_dict:
        original_order = dict(sorted(true_order_dict.items(), key=lambda item:item[1]))
    else:
        original_order = true_order_dict
    ...
    ml_stages = [
        # get the index of the highest prob
        max(enumerate(probs), key=lambda x: x[1])[0]
        # sort the dict by int(key) to ensure participants are ordered correctly.
        for pid, probs in sorted(final_stage_post.items(), key=lambda x: int(x[0]))
    ]
