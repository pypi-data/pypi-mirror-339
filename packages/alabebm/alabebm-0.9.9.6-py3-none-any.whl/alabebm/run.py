...
def run_ebm(
    ...
    # Making sure the true order is sorted by the order (asending)
    if true_order_dict:
    ...
    mse = None
    rmse = None
    # If 'hard_kmeans", final_stage_post will be empty
    if true_stages and algorithm != 'hard_kmeans':
    ...
    results = {
        ...
        "kendalls_tau2": tau2,
        "p_value2": p_value2,
        "ml_order": dict(sorted(most_likely_order_dic.items(), key=lambda item:item[1])),
