...
def delete_components(top_components, to_delete = None, min_distance = 0.98, how = 'one', dry_run = True, verbose = True):
    ...
    Args:
        ...
        how (str): either 'all' (deletes all the component) or 'one' (leaves one image and delete the rest of the duplicates)
    ...
    try:
        ...
        assert len(top_components), "top_components should not be empty"
        assert 'len' in top_components.columns, "top_components dataframe should contain a len column with an integer value of the number of images in this component"
        ...
        for _, row in top_components.iterrows():
            ...
            if how == 'one':
                ...
                inner_delete(files_to_delete, how='delete', dry_run=dry_run, verbose=verbose)
                total_deleted += files_to_delete
            else:  # how == 'all'
                # Delete all files in the component
        ...
        print(f"Error in delete_components: {ex}")
        return []
