...
def job_prepare_for_output(
    api: ToolforgeClient, job, headers: dict[str, Any], suppress_hints=True
):
    ...
    if health_check is not None:
        ...
        path = health_check.get("path", None)
        if script:
            job["health_check"] = f"script: {script}"
        elif path:
            job["health_check"] = f"http: {path}"
    else:
...
def get_api_payload(args: argparse.Namespace) -> Dict[str, Any]:
    mount = args.mount or (
        MountOption.NONE if _image_is_buildservice(args.image) else MountOption.ALL
    )
    filelog = (
        args.filelog
    ...
    if args.health_check_script:
        health_check = {"type": "script", "script": args.health_check_script}
