...
class ConfigTool:
    ...
    def set_values(self, section: str, values: Dict[str, Any]) -> bool:
        """批量设置配置值"""
        try:
            if not self.config.has_section(section):
                self.config.add_section(section)
    ...
    def add_listener(self, section: str, key: str, callback: Callable[[str, str, Any], None]):
        """添加配置变更监听器"""
        key = f"{section}.{key}"
    ...
    def remove_listener(self, section: str, key: str, callback: Callable[[str, str, Any], None]):
        """移除配置变更监听器"""
        key = f"{section}.{key}"
    ...
    def with_config(func):
        ...
        def wrapper(*args, **kwargs):
            # 确保配置已加载
            ConfigTool.get_instance().load_config()
