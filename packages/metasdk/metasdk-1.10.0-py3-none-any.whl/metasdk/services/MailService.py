...
class Message(ABC):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        :param message_extra: Дополнительные параметры сообщения. Примеры таких параметров:
...
class TelegramMessage(Message):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        Инициализирует экземпляр класса TelegramMessage.
...
class MailService:
    ...
    def submit_message(self, message: Message) -> int | None:
        """
...
class TelegramUtils:
    ...
    def fetch_user_telegram_id(
        ...
        if user_id is None and user_email is None:
            raise ValueError("Необходимо передать либо user_id, либо user_email")

        if user_email is not None:
            user_info_query_result = self.__metadb.one(
                """
                SELECT id
                FROM public.users pu
                WHERE
                    pu.info->'intranet'->>'domain_id' = :domain_id::text
                    OR pu.email = :email::text
                """,
                {"domain_id": user_email.split("@")[0], "email": user_email},
            )
            target_user_id = user_info_query_result.get("id") if user_info_query_result else None
        else:
        ...
        if all([
            ...
            target_user_id is None or user_id != target_user_id,
        ]):
            raise ValueError("Переданные user_id и user_email не соответствуют одному пользователю")

        if not target_user_id:
            return None

        telegram_id_query_result = self.__metadb.one(
            """
            SELECT telegram_id
            FROM public.users pu
            WHERE pu.id=:user_id::bigint
            """,
            {"user_id": target_user_id},
        )
        return telegram_id_query_result.get("telegram_id") if telegram_id_query_result else None
