    ...
    def preprocess(self, fillna=True, segcheck=False,
        ...
        if dup.any():
            if "frequency" in paradigms.columns:
                paradigms.loc[dup, 'frequency'] = paradigms.loc[dup].groupby(subset_cols, observed=True).sum("frequency")
        ...
        paradigms[form_col] = paradigms[['form_id', form_col]].apply(
            lambda x: Form(x[form_col], x.form_id, resegment=resegment), axis=1)
