...
class Pattern(object):
    ...
    def __lt__(self, other):
        ...
        Example:
            >>> Inventory.initialize("tests/data/frenchipa.csv")
            >>> cells = ("prs.1.sg", "prs.2.pl")
            >>> forms = (Form("a m E n"), Form("a m ə n E"))
            >>> forms2 = (Form("b E n"), Form("b ə n E"))
    ...
    def __eq__(self, other):
        ...
        Example:
            ...
            >>> p2 = Pattern(('A','B'), (Form("a m E n"), Form("a m ə n E")), aligned=False)
            >>> p1 == p2
            True
            >>> p1 == "E_ ⇌ Ø_E / am_n_ <0>"
            False
...
class ParadigmPatterns(dict):
    ...
    def find_patterns(self, paradigms, *args, method="edits", disable_tqdm=False,
        ...
        self.cells = list(paradigms.cells_dedup)
        ...
        # Create empty dfs of patterns
        # This is to avoid threads depending on a shared paradigms object !
        tasks = [(paradigms.get_empty_pattern_df(*pair), pair, self.insert_cost, self.sub_cost, self.optim_mem)
                        for pair in combinations(self.cells, 2)]
        if cpus > 1:
            with Pool(cpus) as pool:  # Create a multiprocessing Pool
                self.update(pool.starmap(_find_cellpair_patterns, tqdm(tasks)))
        else:
            self.update((_find_cellpair_patterns(*task) for task in tqdm(tasks)))
    ...
    def from_file(self, patterns_md, paradigms, force=False, **kwargs):
        ...
        for r in patterns_md.package.resources:
            if ('cells' in r.custom
                    and 'kind' in r.custom
                    and r.custom['kind'] == "machine_readable"):
                pattern_files[tuple(r.custom['cells'])] = patterns_md.get_resource_path(r.name)

        # Parse patterns for each pair of cells
        log.info('Reading patterns...')
        cell_pairs = [p if p in pattern_files else p[::-1]
                      for p in combinations(cells, 2)] if cells else list(pattern_files)
    ...
    def from_csv(self, path, pair, patterns_map, collection,
        ...
        table[['lexeme', 'form_x']] = pd.merge(table, paradigms, right_index=True,
                                               left_on='form_x')[['lexeme', 'form']]
        table['form_y'] = table.form_y.map(paradigms.form)
        ...
        if (
            ...
            raise ValueError("It looks like you ignored defective rows"
                             ...
                             f"Occured when reading cellpair {pair}")
    ...
    def unmerge_columns(self, paradigms):
        ...
        Arguments:
            paradigms (qumin.representations.Paradigms): a Paradigms object.
        """

        # Variables to store asynchronous changes.
        mapping = {i: k for k, v in paradigms.cells_dedup.items() for i in v}
        new_pairs = []

        # Find which pairs need to be restored
        for cell, dupl_cells in paradigms.cells_dedup.items():
            if dupl_cells == []:
                continue
            new_pairs.extend(
                list(product(dupl_cells,
                             set(paradigms.cells) - set(dupl_cells) - {cell})))

        # We create the new pairs and delete the old ones.
        for pair in new_pairs:
            old_pair = (mapping.get(pair[0], pair[0]),
                        mapping.get(pair[1], pair[1]))
            if old_pair not in self.keys():
                old_pair = old_pair[::-1]
                pair = pair[::-1]

            new_df = paradigms.get_empty_pattern_df(*pair)
            cols = ["lexeme", "form_x", "form_y"]
            self[pair] = pd.merge(new_df[cols], self[old_pair], on=cols, how="left")

        # We create identity patterns
        for cell, dupl_cells in paradigms.cells_dedup.items():
            identical_cells = [cell] + dupl_cells
            if len(identical_cells) < 2:
                continue

            for pair in combinations(identical_cells, 2):
...
def _find_cellpair_patterns(df, pair, insert_cost, sub_cost, optim_mem):
    ...
    for alt in collection:
        ...
        for alt_type in collection[alt]:
            log.debug(f"\tType {alt_type}, found {len(collection[alt][alt_type])} patterns:\n\t {collection[alt][alt_type]}")
            ...
            log.debug(f"Resulted in {len(collection[alt][alt_type])} patterns : {collection[alt][alt_type]}")
        log.debug(f"2.Generalizing across types {types}")
        ...
        log.debug(f"Resulted in {len(collection[alt])} patterns : {collection[alt]}")
        # Score
