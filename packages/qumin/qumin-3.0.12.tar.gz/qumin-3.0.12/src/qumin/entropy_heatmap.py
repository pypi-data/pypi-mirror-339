...
def zones_heatmap(results, md, features, cell_order=None, cols=None):
    ...
    def zone_table_one(clusters, ax):
        ...
        palette = sns.cubehelix_palette(maxi)
        ax = sns.heatmap(table, cmap=palette, square=True, xticklabels=True, yticklabels=True,
                         cbar=False, annot=True, linewidths=0, ax=ax)
        ax.tick_params(axis='x', labelbottom=False, labeltop=True,
                       bottom=False, top=False, labelrotation=90)
        ax.set_title(f"Entropy threshold: {t} ({maxi} zones)", y=-0.1)
...
def get_features_order(features, results, sort_order=None, cells=None):
    """Returns an ordered list of the cells from a Paralex compliant
    cell file.

    Arguments;
        features (pandas.DataFrame): Paralex compliant features table
        results (pandas.DataFrame): Results from entropy computations.
        sort_order (List[str]): Order of the features to sort.
        cells (List[str]): List of cells to draw on the heatmap.
            If no sort_order, will be used to define the order.

    """
    if features is not None:
        df = results.reset_index()
        result_cells = sorted(set(df.predictor.to_list() + df.predicted.to_list()))
        if cells is None:
            # Handle multiple predictor format ('cellA&cellB')
            cells = sorted(set(sum([x.split('&') for x in result_cells], [])))
        else:
            cells = [c for c in cells if c in result_cells]

        df_c = decompose(features, list(cells))
        if 'canonical_order' in features:
        ...
        if sort_order is not None:
            ...
            df = results.reset_index()
            cells = set(df.predictor.unique()) - set(cells) if cells \
                else set(df.predictor.unique())
            return sorted(list(cells))


def _draw_heatmap(*args, cmap=None, cmap_freqs=None,
                  pred_freqs=None, target_freqs,
    ...
    Arguments:
        ...
        pred_freqs (pandas.Series): frequencies of the predictors
        target_freqs (pandas.DataFrame): frequencies of the targets
    ...
    if 'Number of pairs' in types:
        ...
        hm_cmap = cmap
        fmt = ".2f"
        cell_colors = (pred_freqs is not None) and (target_freqs is not None)
    ...
    if feat_order:
        ...
        df = df.loc[sort.index, feat_order]
    ...
    if cell_colors:
        row_freqs = pred_freqs.loc[df.index]
        col_freqs = pd.DataFrame(target_freqs.loc[df.columns]).T
...
def entropy_heatmap(results, md, cmap_name=False, freq_margins=True,
    ...
    target_freqs = None
    cmap_freqs = None
    if freq_margins and (results.proba_source == "tokens").all():
        pred_freqs = results.reset_index('predicted').pred_proba
        pred_freqs = pred_freqs[~pred_freqs.index.duplicated()]
        pred_freqs.name = "frequency"
        target_freqs = results.reset_index('predictor').target_proba
        target_freqs = target_freqs[~target_freqs.index.duplicated()]
        target_freqs.name = "frequency"
    ...
    path = md.get_path(f"vis/{filename}")
    log.info("Writing heatmap to: %s", path)
    cg.savefig(path, pad_inches=0.1)
    md.register_file(f"vis/{filename}", description="Entropy heatmap")
...
def ent_heatmap_command(cfg, md):
    ...
    feat_order = get_features_order(features, results, cfg.heatmap.order, cells=cfg.cells)
    # Keep only required rows
    results = results.loc[(feat_order, feat_order), :]
    ...
    if 1 not in results.n_preds.values:
        return
    ...
    if len(distillation) > 1:
        ...
        entropy_heatmap(result_subset, md,
                        ...
                        n_pairs=cfg.heatmap.display.n_pairs,
                        debug=verbose,
                        freq_margins=False,  # No freq margins, we would need to accumulate across distillation
