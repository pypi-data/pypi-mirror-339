    ...
    def counts(self, prefix=""):
        ...
        if self.trig_effs:
            ...
            for n_bin in bin_nums:
                tistos = helpers.bin_to_ufloat(
                    self._counts[sel_hist_name.replace("sel", "tistos")], n_bin
                )  # d_i in Equation 11

                tis = helpers.bin_to_ufloat(
                    self._counts[sel_hist_name.replace("sel", "tis")], n_bin
                )  # b_i + d_i in Equation 11
                tis_only = helpers.bin_to_ufloat(
                    self._counts[sel_hist_name.replace("sel", "tis_only")], n_bin
                )  # b_i in Equation 11

                tos = helpers.bin_to_ufloat(
                    self._counts[sel_hist_name.replace("sel", "tos")], n_bin
                )  # c_i + d_i in Equation 11
                tos_only = helpers.bin_to_ufloat(
                    self._counts[sel_hist_name.replace("sel", "tos_only")], n_bin
                )  # c_i in Equation 11
