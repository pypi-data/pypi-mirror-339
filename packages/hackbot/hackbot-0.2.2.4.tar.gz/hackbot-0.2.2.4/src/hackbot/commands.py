...
def add_common_arguments(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        default="plus",
        help=f"The profile to use (e.g. {YELLOW}starter{END}, {YELLOW}plus{END}) Default: {YELLOW}plus{END}",
    )
    parser.add_argument(
        "--model",
        type=str,
        required=False,
        help=f"The backing LLM model to use (e.g. {YELLOW}gpt-4o-mini{END}, {YELLOW}o1-mini{END}, {YELLOW}claude-3-5-sonnet{END} etc.)",
    )
...
def add_api_key_argument(parser):
...
def setup_parser():
    ...
    issue_parser.add_argument(
        "--issues_repo",
        type=str,
        help=f"The repository to generate issues in (format: {YELLOW}username/repo{END}). By default empty and so no issues are generated",
    )
    issue_parser.add_argument(
        "--github_api_key",
        type=str,
...
def check_common_args(args: argparse.Namespace) -> Union[argparse.Namespace, int]:
    ...
    if args.profile == "starter" and args.model:
        log.error(
            f"❌ Error: Using the starter profile (--profile starter) and --model arguments together is not allowed, go to {GREEN}https://hackbot.co/pricing{END} to upgrade your subscription"
        )
        return 1
