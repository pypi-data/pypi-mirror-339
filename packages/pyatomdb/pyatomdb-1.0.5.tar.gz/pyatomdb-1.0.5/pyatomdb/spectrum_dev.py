...
#-----------------------------------------------------------------------
  ...
  def return_spectrum(self, Te, teunit='keV', nearest = False,\
    ...
    for Z in elements:
      ...
      if abund > 0:
        ...
        if self.doBroadenPseudo:
          if self.doBroadenCont:
            # FUCK THIS SHIT
            self.broaden_continuum(Z, ss['cont']+ss['pseudo'], Tb, broaden_object)
          else:
             self.broaden_continuum(Z, ss['pseudo'], Tb, broaden_object)
        else:
          if self.doBroadenCont:
            self.broaden_continuum(Z, ss['cont'], Tb, broaden_object)
            
        
        for key in ['lines', 'cont', 'pseudo']:
          s[key]+=ss[key]
    ...
    if do_eebrems:
      ...
      s['cont']+= eespec/(nel/nH)

    #  now broaden the continuum if required
    if self.broadenpseudo:
        

    return s

  def broaden_continuum(self, s, Tb, Z, broaden_object):
    """
    Broaden a continuum array
    
    Parameters
    ----------
    s : array(float)
      The spectrum to broaden (on self.ebins) array)
    Tb : float
      broadening temperature (keV)
    Z : int
    ...
    -------
    float(array)
      The flux of s, broadened appropriately.
    """  
    
    # get mass
    mass = atomic.Z_to_mass(Z)
    
    # centroid of energy bins, use as broadening baseline
    ecent = (self.ebins[1:]+self.ebins[:-1])/2
  ...
  def _merge_spectra_temperatures(self, f, spec1, spec2, log_interp):
    ...
    for lbl in ['lines','cont','pseudo']:
      if log_interp:
        
        out = numpy.log(spec1[lbl]+const.MINEPSOFFSET) * f[0]+\
              numpy.log(spec2[lbl]+const.MINEPSOFFSET) * f[1]
        spec[lbl] = numpy.exp(out)-const.MINEPSOFFSET

      else:
        spec[lbl] = spec1[lbl]*f[0] + spec2[lbl]*f[1]

      spec[lbl][spec[lbl]<0]=0.0
...
#      if ebins_checksum == False:
    ...
    spec={}
    spec['lines'] = numpy.zeros(len(eedges)-1)
    spec['cont'] = numpy.zeros(len(eedges)-1)
    spec['pseudo'] = numpy.zeros(len(eedges)-1)

    if dolines:
      spec['lines']=self.lines.return_spec(eedges, T, ebins_checksum=ebins_checksum,\
                                  thermal_broadening=thermal_broadening,\
                                  broaden_limit=broaden_limit,\
                                  velocity_broadening=velocity_broadening,\
                                  broaden_object=broaden_object)
    if dopseudo:
      spec['pseudo']=self.continuum.return_spec(eedges, ebins_checksum = ebins_checksum,\
                                       dopseudo=True, docont=False)
    if docont:
      spec['cont']=self.continuum.return_spec(eedges, ebins_checksum = ebins_checksum,\
                                       dopseudo=False, docont=True)
