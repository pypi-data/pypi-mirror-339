...
class YTeva_direct:
    ...
    async def fetch_audio_link_direct(self, video_id: str):
        return await verif_auido_direct(self.api_key, video_id)

    async def play_audio_direct(self, video_id: str):
        result = await self.fetch_audio_link_direct(video_id)  
        if result["type"] == "live":
            return result["url"]
        donwload_link = result["url"]
        song_name = result["song_name"].replace(" ", "_")
        download_folder = "downloads"
        if not os.path.exists(download_folder):
           os.makedirs(download_folder)
        save_path = f"downloads/{song_name}.m4a"
        try:
           response = requests.get(donwload_link, stream=True)
