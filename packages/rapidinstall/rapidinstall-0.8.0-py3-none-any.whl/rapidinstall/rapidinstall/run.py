    ...
    while active_processes:
        ...
        if verbose and update_interval and (iteration_count % update_interval == 0):
            ...
            if waiting_on: # Only print status if someone is actually waiting
                ...
                # Aggregate all *new* stdout collected in this cycle from waiting processes
                aggregated_stdout_this_cycle = []
                for name in waiting_on_names:
                    aggregated_stdout_this_cycle.extend(
                        output_collected_this_cycle.get(name, {}).get('stdout', [])
                    )

                # --- Print Status Update Block ---
                status_title = f"*** Waiting for: {', '.join(waiting_on_names)}"
                runtime_line = f"*** Approx Runtime: {current_runtime:.1f} sec"
                status_header = f"{SEPARATOR}\n{status_title}\n{runtime_line}\n{SEPARATOR}"
                status_output = "".join(aggregated_stdout_this_cycle).strip()
                status_footer = f"\n{SEPARATOR}" if status_output else "" # Footer only if output exists
                status_block = f"{status_header}\n{status_output}{status_footer}\n\n"

                _print_locked(print_lock, status_block)
        ...
        for process, data in list(active_processes.items()): # Iterate copy keys
            ...
            if return_code is not None: # Process has finished
                ...
                final_stdout = "".join(data['final_output']['stdout'])
                final_stderr = "".join(data['final_output']['stderr'])
                ...
                if verbose:
                    ...
                    output_block = _format_output_block(f"*** Output of {name}:", final_stdout)
                    error_block = _format_output_block(f"*** Error in {name}:", final_stderr)
