...
MAX_ROWS_TO_DELETE = 999999  # Maximum number of rows to delete at once
        ...
        for row_id in test_data_list:
            for result in results:
                ...
                if result[1].endswith('table'):
                    gcrml1.default_view_table_name = result[1].replace("table", "with_deleted_and_test_data_view")
                if result[1].endswith('old'):
                    continue
                if is_safe_mode:
                    global to_delete
                    try:
                        id_column_name = generate_id_column_name(result[1])
                        ...
                        if e.errno == 1054:
                            ...
                            to_delete = gcrml1.select_multi_value_by_column_and_value(select_clause_value="is_test_data",
                                                                                      column_name=id_column_name,
                                                                                      column_value=row_id,
                                                                                      view_table_name=gcrml.default_view_table_name,
                                                                                      limit=MAX_ROWS_TO_DELETE,)
                            continue
