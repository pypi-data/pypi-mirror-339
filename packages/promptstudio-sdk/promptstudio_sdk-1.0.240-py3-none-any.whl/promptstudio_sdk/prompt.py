...
def modify_messages_openai_supported_models(messages):
    ...
    for message in messages:
        ...
        # Handle assistant and user messages (text format)
        if role in ["assistant", "user"]:
            if isinstance(content, list):
                text_content = " ".join(
                    item["text"] for item in content if item["type"] == "text" and "text" in item
                )
        ...
        # Handle system messages (no change)
        if role == "system":
            if isinstance(content, str) and content.strip():
                modified_messages.append({"role": "system", "content": content})
            continue

        # Handle image messages for user
        if isinstance(content, list):
            modified_content = []
            for item in content:
                if item["type"] == "file" and "file_url" in item and "url" in item["file_url"]:
                    image_url = item["file_url"]["url"]
                    _, extension = os.path.splitext(image_url)
                    if extension not in supported_extensions:
                        raise ValueError(f"Unsupported image extension: {extension}. We support PNG, JPEG, WEBP, and JFIF.")
                    modified_content.append({"type": "image_url", "image_url": {"url": image_url}})
                else:
                    modified_content.append(item)
            modified_messages.append({"role": role, "content": modified_content})
        else:
            modified_messages.append({"role": role, "content": content})
