...
def get_center_line_points(road, step_size):
    ...
    if road.elevation_profile.elevations:
        for i, elevations in enumerate(road.elevation_profile.elevations):
            ...
            center_line_points[idxs, 4] = (
                elevations.a
                + elevations.b * delta_s
                + elevations.c * np.power(delta_s, 2)
                + elevations.d * np.power(delta_s, 3)
            )
    ...
    if road.lateral_profile.superelevations:
        for i, supelev in enumerate(road.lateral_profile.superelevations):
            ...
            # get angle in rad
            center_line_points[idxs, 5] = (
                supelev.a + supelev.b * delta_s + supelev.c * np.power(delta_s, 2) + supelev.d * np.power(delta_s, 3)
            )
    ...
    for i, lane_offset in enumerate(road.lanes.lane_offset):
        ...
        lane_offset_value = (
            lane_offset.a
            + lane_offset.b * delta_s
            + lane_offset.c * np.power(delta_s, 2)
            + lane_offset.d * np.power(delta_s, 3)
        )
        # add offset to center line points
        center_line_points[idxs, 1] = center_line_points[idxs, 1] + lane_offset_value * np.cos(
            center_line_points[idxs, 3] + np.pi / 2
        )
        center_line_points[idxs, 2] = center_line_points[idxs, 2] + lane_offset_value * np.sin(
            center_line_points[idxs, 3] + np.pi / 2
        )
        # add superelevation to z coordinate
        center_line_points[idxs, 4] = (
            center_line_points[idxs, 4] + np.sin(center_line_points[idxs, 5]) * lane_offset_value
        )
...
def sample_points(geometry_entry, step_size):
    ...
    delta_s = np.arange(0, geometry_entry.length, step_size)
    ...
    if geometry_entry.line.line is not None:
        ...
        center_line_points = np.stack([xs, ys, delta_phi + geometry_entry.hdg])
        ...
        spiral = EulerSpiral.createFromLengthAndCurvature(
            geometry_entry.length, geometry_entry.spiral.curv_start, geometry_entry.spiral.curv_end
        )

        center_line_points = np.stack(
            spiral.calc(
                delta_s, geometry_entry.x, geometry_entry.y, geometry_entry.spiral.curv_start, geometry_entry.hdg
            )
        )
        ...
        u = delta_s
        v = (
            geometry_entry.poly3.a
            + geometry_entry.poly3.b * u
            + geometry_entry.poly3.c * u**2
        ...
        center_line_points = np.stack(
            [
                geometry_entry.x
                + np.sqrt(u**2 + v**2) * np.cos(rotation_total)
                - origin_offset * np.cos(origin_offset_hdg),
                geometry_entry.y
                + np.sqrt(u**2 + v**2) * np.sin(rotation_total)
