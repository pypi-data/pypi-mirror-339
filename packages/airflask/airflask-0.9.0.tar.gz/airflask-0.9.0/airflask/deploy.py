...
def optimal_host(venv_path, app_path):
    ...
    # Step 3: Measure Gunicorn RAM usage
    ...
    print(f"Gunicorn RAM usage: {gunicorn_ram:.2f} MB")
    process.terminate()
    process.wait()
...
def run_deploy(app_path, domain, apptype, ssl, noredirect):
    ...
    workers, threads, connections  = optimal_host(venv_path, app_path)
