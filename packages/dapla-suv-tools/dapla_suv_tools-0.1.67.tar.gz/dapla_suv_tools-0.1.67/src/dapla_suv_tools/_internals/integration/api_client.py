...
#
# # Following two functions are used as a temporary way of handling the env vars for Jupyter and DaplaLab in the transition phase
# def select_end_user_url() -> str:
#     url = os.getenv("SUV_DAPLA_API_URL", "")
#     if not url:
#         url = os.getenv("SUV_END_USER_API_URL")
...
#     base = os.getenv("SUV_DAPLA_API_URL", "")
#     if base:
#         return base.replace("admin-api", "prefill-api.stat").replace("/public/", "")
#     base = os.getenv("SUV_END_USER_API_URL")
#     if base:
#         return base.replace("altinn3-admin-api", "altinn-prefill-api-stat")
#     return ""
#
#
# END_USER_API_BASE_URL = select_end_user_url()
#
...
class SuvApiClient:
    ...
    def put(self, path: str, body_json: str, context: SuvOperationContext) -> str:
        headers = self._get_headers(context)
        response = requests.put(url=f"{self.base_url}{path}", headers=headers, data=body_json)
        return self._handle_response(response=response, context=context)
