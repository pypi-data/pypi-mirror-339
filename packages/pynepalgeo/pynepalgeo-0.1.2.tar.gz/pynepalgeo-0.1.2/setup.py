...
setup(
    ...
    url="https://github.com/smaheshacharya/nepal-geoinfo",  # Replace with the actual URL of your project
    ...
    include_package_data=True,  # Include non-code files listed in MANIFEST.in
      package_data={
        'py_nepal_geo': [
            'dataset/province/*.json',
            'dataset/district/*.json',
            'dataset/municipality/*.json',
            'dataset/overall/*.json'
        ],
    },
    entry_points={  # Optional: for adding command-line scripts
        'console_scripts': [
            'py_nepal_geo=py_nepal_geo',  # Replace with actual script path if applicable
        ],
    },
)
