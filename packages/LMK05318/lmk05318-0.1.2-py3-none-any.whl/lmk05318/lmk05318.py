...
class LMK05318:
    ...
    def get_vendor_id(self):
        ...
        vendor_id = (vendor_id_high << 8) \
                  | vendor_id_low
        self.log.info("Vendor ID Readback: 0x{:X}".format(vendor_id))
        return vendor_id
    ...
    def get_product_id(self):
        ...
        self.log.info("Product ID Readback: 0x{:X}".format(prod_id))
        return prod_id
    ...
    def write_cfg_regs_to_eeprom(self, method, eeprom_data=None):
        ...
        def _wait_for_busy(self, value):
            ...
            while datetime.datetime.now() < wait_until:
                self.log.info("wait till busy bit becomes {}".format(value))
        ...
        if method == self.LMK_EEPROM_REG_COMMIT:
            self.log.info("write current device register content to EEPROM")
        ...
        self.log.info("programming EEPROM done, power-cycle or hard-reset to take effect")
