...
def gblur_skimage(x, s):
	import skimage.filters   # pip install scikit-image
	y = skimage.filters.gaussian(x, sigma=s, channel_axis=-1)
	return y

@boundarize
def gblur_torch(x, s):
	import torchvision.transforms  # pip install torchvision
	n = 2 * round(s * 3) + 1
...
def gblur_gimp(x, s):
	...
	f = f"{tempfile.NamedTemporaryFile().name}.png"
	g = f"{tempfile.NamedTemporaryFile().name}.png"
	c = f'gimp -i -b "(define img (car (gimp-file-load RUN-NONINTERACTIVE \\\"{f}\\\" \\\"{f}\\\"))) (define layer (car (gimp-image-get-active-layer img))) (plug-in-gauss 1 img layer {s} {s} 0) (gimp-file-save RUN-NONINTERACTIVE img layer \\\"{g}\\\" \\\"{g}\\\") (gimp-quit 0)" 2>/dev/null >/dev/null'
	iio.write(f, x)
	os.system(c)
	y = iio.read(g)
	os.system(f"rm {f} {g}")
