...
from src.Framework.Shared.NodeStructure import NodeStructure
...
class NodeStructureImpl(NodeStructure):
    ...
    def addDefaultLink(self, src_node, sink_link, category, activation,
                       removal_threshold):
        if not src_node in self.getConnectedsrcs(sink_link):
            self.addDefaultLink_(src_node.getId(),
                                 sink_link.getCategory("id"), category,
                                 activation, removal_threshold)

    def addDefaultLink_(self, src_id, sink_id, category, activation,
                       removal_threshold):
        self.addLink(DEFAULT_LINK_TYPE, src_id, sink_id, category,
                     activation, removal_threshold)
    ...
    def addLink(self, link_type, src_id, sink_id, category, activation,
                removal_threshold):
        link = LinkImpl()
    ...
    def getConnectedSinks(self, node):
        sink_links = []
        for link in self.links:
            if link.getsrc() == node.getId():
                sink_links.append(link)
        return sink_links

    # Returns a list of all node objects connected to link as a src
    def getConnectedsrcs(self, link):
        sink_nodes = []
        for node in self.nodes:
            if link.getsrc() == node.getId():
