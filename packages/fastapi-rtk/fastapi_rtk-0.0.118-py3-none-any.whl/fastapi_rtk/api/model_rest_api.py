...
from ..const import EXCLUDE_ROUTES, logger
...
class ModelRestApi(BaseApi):
    ...
    def _init_properties(self) -> None:
        ...
        search_cols = self.datamodel.get_search_column_list()
        ...
        self.search_columns = use_default_when_none(
            self.search_columns,
            [x for x in search_cols if x not in self.search_exclude_columns],
        )
    ...
    def _init_schema(self) -> None:
        ...
        self.add_schema = self.add_schema or self._create_request_schema(
            self.add_obj_schema, name=f"{self.__class__.__name__}-AddSchema"
        )
    ...
    async def upload_headless(
        ...
        return await self._import_data(file, query, session, delimiter)
    ...
    async def _generate_info_schema(
        ...
        if self._is_in_base_permissions("post"):
        ...
        if self._is_in_base_permissions("put"):
    ...
    async def _import_data(
        ...
        list_data = await smart_run(CSVJSONConverter.csv_to_json, csv_data, delimiter)
