...
class Plugin(ad.Plugin):
    ...
    def before_analyze(self):
        ...
        self.site.models.uploads.add_shortcut(
            'presto.Client',
            'income_certificate',
            _("Income certificate"),
    ...
    def walk_invoice_entries(self, obj):
        # used by trading/service_report.weasy.html
        ...
        def collect(pl):  # calls itself
            places.add(pl)
            for spl in self.site.models.countries.Place.objects.filter(
                    parent=pl):
                collect(spl)
