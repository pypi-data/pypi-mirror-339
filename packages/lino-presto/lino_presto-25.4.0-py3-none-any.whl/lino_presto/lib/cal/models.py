...
class Room(Room, Referrable, Printable):
    ...
    def print_actions(self, ar):
        if ar is None:
            return ''
        elems = [ar.instance_action_button(self.print_roster)]
...
class Event(Event, InvoiceGenerator):
    ...
    def as_summary_item(self, ar, text=None, **kwargs):
        if text is None:
            text = str(self)
        if iselement(text):
            kwargs.setdefault('title', html2rst(text))
        else:
    ...
    def get_invoiceable_partner(self):
        ...
        if isinstance(ord, rt.models.orders.Order):
            return ord.get_invoiceable_partner()
    ...
    def get_generators_for_plan(cls, plan, partner=None):
        ...
        if partner is None:
            # only events with a project can get invoiced
            qs = qs.filter(project__isnull=False)
        else:
            pm = settings.SITE.project_model
            if issubclass(pm, rt.models.contacts.Partner):
                qs = cls.filter_by_invoice_recipient(qs, partner, 'project')
                # q1 = models.Q(
            ...
            elif issubclass(pm, ContactRelated):
                fldname = pm.get_partner_filter_field(partner)
                qs = cls.filter_by_invoice_recipient(qs, partner,
                                                     'project__' + fldname)
                # qs = pm.add_partner_filter(qs, partner, 'project__')
            else:
                raise Exception("Cannot filter {} using {}".format(
                    partner, settings.SITE.project_model))
    ...
    def get_wanted_invoice_items(self, info, invoice, ar):
        ...
        for i in super().get_wanted_invoice_items(info, invoice, ar):
            # print(i.qty)
            yield i
            for oi in self.owner.get_wanted_invoice_items(info, invoice, ar):
                yield oi
    ...
    def setup_parameters(cls, params):
        ...
        params['project__municipality'] = make_remote_field(
            cls, 'project__municipality')
...
class FindReplacement(dd.ChangeStateAction):
    ...
    parameters = dict(new_partner=dd.ForeignKey(dd.plugins.cal.partner_model),
...
dd.inject_action('cal.Guest',
                 find_replacement=FindReplacement(GuestStates.invited))
