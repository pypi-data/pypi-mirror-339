...
async def _merge_nodes_then_upsert(
    ...
    """Get existing nodes from knowledge graph using name. If exists, merge data, else create, then upsert."""
    timings = {}
    ...
    timings["upsert_node"] = time.perf_counter() - start
    ...
    logger.info(f"[TIMING] _merge_nodes_then_upsert for '{entity_name}': {timings}")
...
async def _merge_edges_then_upsert_batched(
    src_id: str,
    tgt_id: str,
    edges_data: list[dict],
    ...
    total_weight = sum(dp.get("weight", 0.0) for dp in edges_data)
    descriptions = {dp["description"] for dp in edges_data if dp.get("description")}
    keywords = {dp["keywords"] for dp in edges_data if dp.get("keywords")}
    source_ids = {dp["source_id"] for dp in edges_data if dp.get("source_id")}
    file_paths = {dp["file_path"] for dp in edges_data if dp.get("file_path")}

    existing_edge = existing_edges.get((src_id, tgt_id))
    if existing_edge:
        total_weight += existing_edge.get("weight", 0.0)
        source_ids.update(split_string_by_multi_markers(existing_edge.get("source_id", ""), [GRAPH_FIELD_SEP]))
        file_paths.update(split_string_by_multi_markers(existing_edge.get("file_path", ""), [GRAPH_FIELD_SEP]))
        if existing_edge.get("description"):
            descriptions.add(existing_edge["description"])
        if existing_edge.get("keywords"):
            keywords.update(split_string_by_multi_markers(existing_edge["keywords"], [GRAPH_FIELD_SEP]))

    description_str = GRAPH_FIELD_SEP.join(sorted(descriptions))
    keywords_str = GRAPH_FIELD_SEP.join(sorted(keywords))
    source_id_str = GRAPH_FIELD_SEP.join(sorted(source_ids))
    file_path_str = GRAPH_FIELD_SEP.join(sorted(file_paths))

    enriched_description = await _handle_entity_relation_summary(
        f"({src_id}, {tgt_id})", description_str, global_config
    )
...
def timed_block(name: str):
    ...
    try:
        ...
        logger.info(f"TIMING: {name} took {elapsed:.3f} seconds")
...
async def extract_entities(
    ...
    with timed_block("Total extract_entities execution"):
        use_llm_func: callable = global_config["llm_model_func"]
        entity_extract_max_gleaning = global_config["entity_extract_max_gleaning"]
        enable_llm_cache_for_entity_extract: bool = global_config[
            "enable_llm_cache_for_entity_extract"
        ...
        async def _user_llm_func_with_cache(
            ...
            with timed_block("LLM call with cache"):
                if enable_llm_cache_for_entity_extract and llm_response_cache:
                    if history_messages:
                        history = json.dumps(history_messages, ensure_ascii=False)
                        _prompt = history + "\n" + input_text
        ...
        async def _process_extraction_result(
            ...
            Returns:
                tuple: (nodes_dict, edges_dict) containing the extracted entities and relationships
            """
            with timed_block("Process extraction result"):
                maybe_nodes = defaultdict(list)
                maybe_edges = defaultdict(list)
        ...
        async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
            ...
            """
            nonlocal processed_chunks
            with timed_block(f"Process chunk {processed_chunks + 1}/{total_chunks}"):
                chunk_key = chunk_key_dp[0]
                chunk_dp = chunk_key_dp[1]
                content = chunk_dp["content"]
                # Get file path from chunk data or use default
        ...
        with timed_block("Process all chunks"):
            tasks = [_process_single_content(c) for c in ordered_chunks]
            results = await asyncio.gather(*tasks)
        ...
        # Ensure that nodes and edges are merged and upserted atomically
        with timed_block("Graph DB operations"):
            async with graph_db_lock:
                node_ids = list(maybe_nodes.keys())
                edge_pairs = list(maybe_edges.keys())
                unique_edge_node_ids = list(set([x for pair in edge_pairs for x in pair]))
                existing_nodes = await knowledge_graph_inst.get_nodes_bulk(node_ids + unique_edge_node_ids)
                # logger.info(f"Existing nodes: {existing_nodes}")
                # logger.info(f"Edge pairs passed to get_edges_bulk: {edge_pairs}")
                existing_edges = await knowledge_graph_inst.get_edges_bulk(edge_pairs)
                ...
                all_relationships_data = await asyncio.gather(
                    *[
                        _merge_edges_then_upsert_batched(k[0], k[1], v, existing_edges, existing_nodes, knowledge_graph_inst, global_config)
                        for k, v in maybe_edges.items()
                    ]
                )
                # logger.info(f"All relationships data: {all_relationships_data}")
                await knowledge_graph_inst.upsert_edges_batch(all_relationships_data)
        ...
        with timed_block("Vector DB operations"):
            # Entity VDB upsert
            if entity_vdb is not None:
                try:
                    data_for_vdb = {
                    ...
                    logger.info(f"\033[1;34mUpserting {len(data_for_vdb)} entities to entity VDB...\033[0m")
                    await entity_vdb.upsert(data_for_vdb)
                    logger.info("\033[1;32m✅ Entity VDB upsert successful.\033[0m")
                except Exception as e:
                    logger.error(f"\033[1;31m❌ Failed to upsert entity data to VDB: {str(e)}\033[0m", exc_info=True)

            # Relationships VDB upsert
            if relationships_vdb is not None:
                try:
                    data_for_vdb = {
                        compute_mdhash_id(dp["src_id"] + dp["tgt_id"], prefix="rel-"): {
                    ...
                    logger.info(f"\033[1;34mUpserting {len(data_for_vdb)} relationships to relationship VDB...\033[0m")
                    await relationships_vdb.upsert(data_for_vdb)
                    logger.info("\033[1;32m✅ Relationship VDB upsert successful.\033[0m")
                except Exception as e:
                    logger.error(f"\033[1;31m❌ Failed to upsert relationship data to VDB: {str(e)}\033[0m", exc_info=True)
...
async def kg_query(
    ...
    args_hash = compute_args_hash(query_param.mode, query, db_name, query_param.graph_algorithm, cache_type="query")
