...
async def measure_time(task_name, task):
    start_time = time.monotonic()
    result = await task
    end_time = time.monotonic()
    print(f"⏰ Task {task_name} took {end_time - start_time:.4f} seconds")
    return result
...
class LightRAG:
    ...
    def __post_init__(self):
        from or_lib.kg.shared_storage import (
            ...
            while True:
                ...
                async def process_document(
                    ...
                    try:
                        ...
                        logger.info(f"⏰ total chunking time: {total_chunk_time}")
                        ...
                        tasks = [
                            measure_time("⏰ chunks_vdb_task", chunks_vdb_task),
                            measure_time("⏰ entity_relation_task", entity_relation_task),
                            measure_time("⏰ full_docs_task", full_docs_task),
                            measure_time("⏰ text_chunks_task", text_chunks_task),
                        ]
                        await asyncio.gather(*tasks)
