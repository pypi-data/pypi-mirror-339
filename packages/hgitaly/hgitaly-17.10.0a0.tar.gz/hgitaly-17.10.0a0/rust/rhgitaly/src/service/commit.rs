...
use crate::gitaly::{
    list_commits_by_ref_name_response, CommitIsAncestorRequest, CommitIsAncestorResponse,
    CountCommitsRequest, CountCommitsResponse, FindCommitRequest, FindCommitResponse,
    FindCommitsRequest, FindCommitsResponse, GetTreeEntriesRequest, GetTreeEntriesResponse,
    ...
    ListLastCommitsForTreeRequest, ListLastCommitsForTreeResponse, Repository, TreeEntryRequest,
    TreeEntryResponse,
};
...
fn revlog_err_from_graph_err(err: GraphError) -> HgError {
    HgError::CorruptedRepository(format!("{:?}", err))
}
