...
# -*- coding: utf-8 -*-
"""
Created on Tue May 14 12:49:14 2024
...
color_list = ['#bfd3e6', '#9ebcda', '#8c96c6', '#8c6bb1', '#88419d', '#810f7c', '#49006a', '#fff7bc', '#ffeda0', '#fed976', '#feb24c', '#fd8d3c', '#fc4e2a', '#e31a1c', '#bd0026', '#800026']
colour_list = color_list
QLMblue = '#0c6485'
QLMred = '#aa2c4b'
QLMgold = '#9ea364'
new_colour = [color_list[13], color_list[10], color_list[1], QLMblue, color_list[4]]

plt.style.use('default')

####################################################################################
#   various general functions for fitting and such
####################################################################################
...
def gaussian(x, a, b, c, d):
    """For fitting the data- maybe want to fit not a Gaussian at some point?"""
    return a*np.exp(-(x-b)**2/(2*c**2)) + d

####################################################################################
# functions involved in extracting the data
####################################################################################

def ExtractDataSetpoint(filename, FindResiduals):
...
def ExtractDataSetpointTWOPEAK(filename, FindResiduals):
...
def PlotAverageTOF(PMTfilename, fitGaussian, initialguess=0):
...
def DealwithPMTData(setpointfilename, PMTfilename, FindResiduals):
...
def AveragePeakPositionSD(p2, avg):
...
def AveragePeakPosition(p2, avg):
    """To give us something to plot against, also average every 'avg' peak 2 positions"""
    print('p2 len')
    print(len(p2))
    peak = p2[0:-2]
...
def AveragePeakDifference(p1, p2, avg):
...
def PlotAverageDataIntegral(initialguess, PMTfilename, checkfitparams, plotMaximum, plotQuadintegral, plotTrapzintegral, SaveData, upperlimit):
...
def DealWithAllData(PMTfilename, avg, SaveData, FindResiduals):
...
def UnpickleProcessedData(filename):
...
def UnpickleUnprocessedData(file):
...
def GenerateFrequencySpectra(filename, windowsize, starttime, endtime, PlotFreqSpec, HFfixed):
...
def MakeVelocityDistribution(angledfilename, perpfirstpeak, perpfirstpeakerr, angledfirstpeak, angledfirstpeakerr, arrivaltimeold):
...
def GenerateVelocityDistribution(filepathperp, perpdatatype, perpinitialguess, perpcutoff, perpforceHF, filepathangled, windowsize, starttime, endtime, PlotFreqSpec, HFfixedangle):
