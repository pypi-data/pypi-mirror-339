...
def test_get_model_from_server_successful():
    ...
    response_mock.json.return_value = {"vfg": "data"}

    with patch("genius_client_sdk.agent.send_http_request", return_value=response_mock):
        agent.get_model_from_server()
        assert agent.model is not None and agent.model.json_model is not None
...
def test_structure_learning_successful():
    ...
    response_mock = MagicMock()
    response_mock.status_code = 200
    response_mock.json.return_value = {"result": {"model": "data"}}
    ...
    with patch("builtins.open", mock_open(read_data=csv_data)):
        with patch(
            ...
            with patch("genius_client_sdk.agent.send_http_request") as mock_request:
                ...
                assert agent.model.json_model == {"model": "data"}
...
def test_structure_learning_calls_parameter_learning():
    ...
    response_mock = MagicMock()
    response_mock.status_code = 200
    response_mock.json.return_value = {"result": {"model": "data_import"}}
    ...
    with patch("builtins.open", mock_open(read_data=csv_data)):
        with patch(
            ...
            with patch(
                ...
                agent.structure_learning(csv_path="dummy_path.csv", verbose=False)
                assert agent.model.json_model == {"model": "data_import"}
...
def test_act(agent):
    with patch("genius_client_sdk.agent.send_http_request") as mock_request:
        mock_request.return_value.status_code = 200
        mock_request.return_value.text = r'{"result": {"updated_vfg": {"model": "data_act"}, "action_data": {}, "belief_state": {}, "policy_belief": {}, "efe_components": {}}, "success": true, "error": null, "warnings": null, "metadata": {"execution_time": 0.5, "memory_used": [0.125, "MB"]}}'
        agent.model = GeniusModel()
        result = agent.act(10)
...
def test_learn_with_mocked_file(mock_send_http_request, mock_file):
    ...
    mock_response.json.return_value = {
        "result": {"updated_model": {"model": "data_learn"}}
    }
    ...
    with patch.object(agent.logger, "info") as mock_logger_info:
        ...
        mock_logger_info.assert_called_with(
            json.dumps(
                mock_response.json().get("result").get("updated_model"), indent=4
            )
        )
