...
def handle_import_annotations(config):
    ...
    def monitor_import_progress(project_id, import_id, file_path, format_id):
        ...
        ws_url = f"wss://app-import.scematics.xyz/api/v1/imports/import-progress/{import_id}"
    ...
    def print_result(results):
        """Display a properly aligned summary table of import results"""
        ...
        # Check if we have valid dictionaries
        for i, r in enumerate(results):
            if not isinstance(r, dict):
                print(f"Error during import: '{type(r).__name__}' object has no attribute 'get'")
                return
        
        # Calculate column widths based on content plus padding
        f_width = max(20, max(len(str(r.get("file_path", ""))) for r in results) + 2)
        i_width = 12  # Import ID
        s_width = 9   # Status
        m_width = max(20, max(len(str(r.get("message", ""))) for r in results) + 2)
        p_width = 12  # Processed items
        t_width = 10  # Processing time
        
        # Create horizontal separator with proper width
        separator = "+" + "-" * f_width + "+" + "-" * i_width + "+" + "-" * s_width + "+" + "-" * m_width + "+" + "-" * p_width + "+" + "-" * t_width + "+"
        
        # Print the table header
        print(separator)
        header = f"|{'File Path':^{f_width}}|{'Import ID':^{i_width}}|{'Status':^{s_width}}|{'Message':^{m_width}}|{'Items':^{p_width}}|{'Time (s)':^{t_width}}|"
        print(header)
        print(separator)
        ...
        for result in results:
            ...
            import_id = str(result.get("import_id", "N/A"))
            message = str(result.get("message", "N/A"))
            processed = result.get("processed_items", 0)
            total = result.get("total_items", 0)
            processing_time = str(result.get("processing_time", "N/A"))
            
            # Format items info
            if total > 0:
                items_display = f"{processed}/{total}"
            else:
                items_display = "N/A"
            
            # Truncate file_path and message if too long and add "..." indicator
            if len(file_path) > f_width - 3:
                file_path = file_path[:f_width - 5] + "..."
            if len(message) > m_width - 3:
                message = message[:m_width - 5] + "..."
            
            # Create the row with proper alignment
            row = f"|{file_path:<{f_width}}|{import_id:^{i_width}}|{status_display:^{s_width}}|{message:<{m_width}}|{items_display:^{p_width}}|{processing_time:^{t_width}}|"
            print(row)
