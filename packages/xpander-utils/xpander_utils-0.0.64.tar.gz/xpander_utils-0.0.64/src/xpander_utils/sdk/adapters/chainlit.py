...
class ChainlitXpanderDataLayer(BaseDataLayer):
    ...
    async def delete_thread(self, thread_id: str):
        ...
        await asyncio.to_thread(Memory.delete_thread_by_id, {"configuration": xpander.configuration}, thread_id=thread_id)
    ...
    async def list_threads(self, pagination: Pagination, filters: ThreadFilter) -> PaginatedResponse[ThreadDict]:
        ...
        threads = await asyncio.to_thread(Memory.fetch_user_threads,
    ...
    async def get_thread(self, thread_id: str) -> ThreadDict:
        ...
        all_user_threads = await asyncio.to_thread(Memory.fetch_user_threads,
    ...
    async def update_thread(
        ...
        if name and isinstance(name, str) and len(name) != 0:
            await asyncio.to_thread(Memory.rename_thread_by_id,{"configuration": xpander.configuration}, thread_id=thread_id, name=name)
...
class ChainlitAdapter(SDKAdapter):
    ...
    async def add_task(
        ...
        if self.agent.execution and self.agent.messages:
            ...
            if is_edit:
                ...
                await asyncio.to_thread(self.agent.memory.update_messages,updated_thread)

        # Add the new task to the agent
        await asyncio.to_thread(super().add_task,input=input, files=files, use_worker=use_worker, thread_id=thread_id)
    ...
    async def process_tool_calls(self,run_id: str, tool_calls: Dict[int, ChainlitToolCallWithLLMResponse], is_sequence_agent: bool = False, sequence_agent_graph_items:List[GraphItem] = [], reported_sequence_sub_agent_ids: set = set(), on_node_enter: Optional[Callable] = None, on_node_error: Optional[Callable] = None, agent_custom_functions: list[GraphItem] = [], local_tools: Dict[str, Callable] = None) -> str | None:
        ...
        await asyncio.to_thread(self.agent.add_messages,
            messages=[
                {
                    "role": "assistant",
                    "content": "",
        ...
        for raw_tool_call in tool_calls.values():
            ...
            if tool_call.type == ToolCallType.LOCAL:
                ...
                await asyncio.to_thread(self.agent.add_tool_call_results,tool_call_results=[tool_call_result])
            else:
                # failures
                if is_max_tool_call_reached:
                    ...
                    tool_call_result = await asyncio.to_thread(self.agent.run_tool,tool=tool_call, payload_extension={"headers":{"x-xpander-source-node-type": SourceNodeType.ASSISTANT.value.lower()}})
            ...
            if is_debug_mode_active and not is_sub_agent_root:
                ...
                if tool_call_result.result == "STOP":
                        current_step.name = f"Waiting for human approval ({tool_name})"
                        current_step.output = None
