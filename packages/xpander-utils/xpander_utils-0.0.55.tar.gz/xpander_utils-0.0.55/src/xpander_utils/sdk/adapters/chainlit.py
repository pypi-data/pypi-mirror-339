...
class ChainlitAdapter(SDKAdapter):
    ...
    async def process_tool_calls(self,run_id: str, tool_calls: Dict[int, ChainlitToolCallWithLLMResponse], is_sequence_agent: bool = False, sequence_agent_graph_items:List[GraphItem] = [], reported_sequence_sub_agent_ids: set = set(), on_node_enter: Optional[Callable] = None, on_node_error: Optional[Callable] = None, agent_custom_functions: list[GraphItem] = [], local_tools: Dict[str, Callable] = None) -> str | None:
        ...
        if execution_id in self.execution_tool_calls:
            ...
            # Check if the last 3 tool calls are the same
            if len(self.execution_tool_calls[execution_id]) >= MAX_SAME_TOOL_IN_A_ROW:
                last_three_calls = self.execution_tool_calls[execution_id][-MAX_SAME_TOOL_IN_A_ROW:]
                if len(set(last_three_calls)) == 1:  # All three are the same
                    is_repeating_tool_call = True
        ...
        for raw_tool_call in tool_calls.values():
            ...
            if tool_call.type == ToolCallType.LOCAL:
                ...
                # failures
                if is_max_tool_call_reached or is_repeating_tool_call:
                    ...
                    elif is_repeating_tool_call:
                        error = f"Oops! The last {MAX_SAME_TOOL_IN_A_ROW} tool calls were the same ({last_three_calls[0]}). The agent cannot proceed with this tool."
                    ...
                    tool_call_result = self.agent.run_tool(tool=tool_call, payload_extension={"headers":{"x-xpander-source-node-type": SourceNodeType.ASSISTANT.value.lower()}})
