...
def get_stats(data):
    ...
    This function calculates basic statistics (minimum, maximum, mean, median, standard deviation,
    and variance) as well as advanced metrics (skewness, kurtosis, various percentiles, interquartile range, 
    and range). It also counts the number of valid (non-NaN) data points and the number of NaN values.
    
    Parameters:
        data (np.ndarray): 
            A numerical array (1D or multi-dimensional) that may contain NaN values. The function 
            automatically filters out NaN values and performs all statistical calculations on the valid data.
    
    Returns:
        pd.DataFrame: 
            A single-row DataFrame containing the following statistics:
              - Minimum: The minimum value in the data.
              - Mean: The average value.
              - Maximum: The maximum value in the data.
              - Median: The median value.
              - Standard Deviation: The standard deviation.
              - Variance: The sample variance (with degrees of freedom = 1).
              - Skewness: The skewness of the data distribution (indicating asymmetry).
              - Kurtosis: The kurtosis of the data distribution (indicating tailedness).
              - Range: The range (difference between maximum and minimum).
              - Count / Valid Count: The number of valid (non-NaN) data points.
              - NaN Count: The number of NaN values in the original data.
              - Interquartile Range (IQR): The difference between the 75th and 25th percentiles.
              - 1th Percentile: The 1st percentile of the data.
              - 5th Percentile: The 5th percentile of the data.
              - 10th Percentile: The 10th percentile of the data.
              - 90th Percentile: The 90th percentile of the data.
              - 95th Percentile: The 95th percentile of the data.
              - 99th Percentile: The 99th percentile of the data.
              - Mean - 3*StdDev: The value of the mean minus three times the standard deviation.
              - Mean + 3*StdDev: The value of the mean plus three times the standard deviation.
              - Sum: The sum of all valid data points.
    ...
    The output is a DataFrame with one row containing the computed statistical metrics.
    """
    ...
    std_dev = np.std(clean_data)  # Standard deviation
    ...
    stats_dict = {
        ...
        'NaN Count': nan_count,
        'Interquartile Range (IQR)': np.percentile(clean_data, 75) - np.percentile(clean_data, 25),
        '1th Percentile': np.percentile(clean_data, 1),
        '5th Percentile': np.percentile(clean_data, 5),
        '10th Percentile': np.percentile(clean_data, 10),
        '90th Percentile': np.percentile(clean_data, 90),
        '95th Percentile': np.percentile(clean_data, 95),
        '99th Percentile': np.percentile(clean_data, 99),
        'Mean - 3*StdDev': mean - 3 * std_dev,
        'Mean + 3*StdDev': mean + 3 * std_dev,
        'Sum': np.sum(clean_data),
    }
