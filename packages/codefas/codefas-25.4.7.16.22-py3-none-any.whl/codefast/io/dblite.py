...
class DBLite(object):
    ...
    def set(self, key: str, value: str, eager_mode: bool = False) -> 'self':
        ...
        """
        cmd = "DELETE FROM db WHERE trim(key) = ?"
        self.cur.execute(cmd, (key.strip(),))

        cmd = "INSERT OR REPLACE INTO db (key, value, date) VALUES (?, ?, ?)"
        self.cur.execute(cmd, (key.strip(), str(value).strip(), time.time()))
        if eager_mode:
            self.sync()
        return self
    ...
    def get(self, key: str) -> Union[str, None]:
        cmd = "SELECT value FROM db WHERE TRIM(key) = ?"
        resp = self.cur.execute(cmd, (key.strip(),)).fetchone()
        if resp:
            return resp[0]
        return None
    ...
    def exists(self, key: str) -> bool:
        return self.get(key.strip()) is not None

    def keys(self, shuffle:bool=False) -> List[str]:
    ...
    def values(self, shuffle:bool=False) -> List[str]:
    ...
    def __getitem__(self, key: str) -> Union[str, None]:
        return self.get(str(key).strip())

    def __setitem__(self, key: str, value: str) -> None:
        self.set(str(key).strip(), str(value).strip())

    def items(self, shuffle=False) -> List[Tuple[str, str]]:
