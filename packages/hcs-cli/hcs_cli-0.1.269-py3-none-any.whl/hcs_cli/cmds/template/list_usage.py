...
def _format_template_table(data):
    for d in data:
        ...
        _colorize(d, "addedVmHours", lambda n: "bright_red" if n > 0 else None)
        _colorize(d, "reducedVmHours", lambda n: "green" if n > 0 else None)
        _colorize(d, "offloadVmHours", lambda n: "bright_yellow" if n > 0 else None)
    fields_mapping = {
        "id": "Id",
        "name": "Name",
...
def list_usage(org: str, **kwargs):
    ...
    ret = admin.template.list(org_id=org_id, limit=100, **kwargs)
    ...
    for template in ret:
        ...
        item = {
            ...
            "historyVmUtilizationPercent": f'{summary.get("historyVmUtilizationPercent")}%',
            "predictionVmUtilizationPercent": f'{summary.get("predictionVmUtilizationPercent")}%',
        }
