...
def list(org: str, template: str, task: str, namespace: str, meta: str, data: str, **kwargs):
    ...
    if data:
        kwargs["data"] = data
    return scm.plan.list(org_id=org, **kwargs)
...
def get(org: str, name: str, **kwargs):
    ...
    org = cli.get_org_id(org)
    name = recent.require(name, "scm.plan")
...
@cli.org_id
@cli.limit
@click.option("--day", required=False, help="Optional. Search by day-identifier. Example: Monday")
@click.option("--time", required=False, help="Optional. Search by time. Example: 13:30")
@click.option("--slot", required=False, help="Optional. Search by time slot. Example: Mon/13:30")
@click.option(
    "--state",
    "-s",
    required=False,
    type=str,
    help="Optional. Search by task state, as comma-separated values. E.g. 'init,running,success,error'.",
)
@click.argument("name", required=False)
...
def task(org: str, name: str, task_key: str, **kwargs):
    ...
    org = cli.get_org_id(org)
    name = recent.require(name, "scm.plan")
    task_key = recent.require(task_key, "scm.task_key")
...
@cli.org_id
@click.argument("name", required=False)
@click.option("--slot", "-s", required=True, type=str, help="Example: Mon/13:30")
...
def update(org: str, name: str, slot: str, config: str, delete: bool, **kwargs):
