...
def process_command_input(
    ...
    logger.info(f"Starting RootCA migration:")
    ...
    logger.info(f"Ended RootCA migration")


def migrate_edges(
    target_org_id: str, edge_ids: str, poweredon_vms_only: bool, skip_active_session_vms: bool, verbose: bool
):
    ...
    if not org:
        ...
        return
    if not edge_ids:
        logger.error(f"edge_ids is not specified")
        return
    ...
    for edge in edges:
        ...
        if edge["providerLabel"].lower() not in allowed_providers:
            logger.warn(
                f"Pool migration is not supported on {edge.get('providerLabel')} provider edge {edge_id}, org: {target_org_id}"
            )
            continue
...
def migrate_template_vms(
    ...
    while True:
        raw_list = inventory.vm.raw_list(
            template_id, org_id=target_org_id, vm_ids=vm_ids, size=common_dict["BATCH_SIZE"], page=page_num
        )
        page_num += 1
        ...
        if update_power_policy_once_only and len(vms) > 0:
            ...
            if not poweredon_vms_only:
                ...
                logger.info(
                    f"powerPolicy on template {template_id} is un-changed since migration is run on poweredon_vms_only"
                )
        ...
        try:
            ...
            if power_policy_disabled_now:
                ...
                logger.info(
                    f"powerPolicy on template {template_id} was un-changed in the beginning, so not changed now."
                )
            return
        ...
        if page_num >= total_pages:
            if len(invalid_lcs_vms) > 0:
                logger.warn(
                    f"migration skipped on these vms since their lifecycleStatus is not PROVISIONED {json.dumps(invalid_lcs_vms)}"
                )
            if len(un_avail_agent_status_vms) > 0:
                logger.warn(
                    f"migration skipped on these vms since their agentStatus is not AVAILABLE {json.dumps(un_avail_agent_status_vms)}"
                )
            if len(already_migrated_vms) > 0:
                logger.warn(f"Skipped migration on already migrated VMs: {already_migrated_vms}")
            break

    if len(prolonged_powering_on_vms) > 0:
        logger.warn(
            f"these vms are taking prolonged time to powerOn {list(prolonged_powering_on_vms.keys())}. making another final attempt to migrate these."
        )
        logger.info(f"sleep for {SLEEP_TIME_POWERON_VMS} minutes for Vms to powerOn")
        time.sleep(60 * SLEEP_TIME_POWERON_VMS)
        try:
            # perform migration only on prolonged powerOn vms
            perform_migration(prolonged_powering_on_vms.values(), target_org_id, template_id, page_num, verbose)
        except Exception as error:
            logger.error(error)

    if len(prolonged_powering_on_vms) > 0:
        logger.warn(
            f"migration skipped on these vms. these vms are not poweringOn {list(prolonged_powering_on_vms.keys())}"
        )
...
def process_file(file: str, verbose: bool):
    ...
    for target_org_id in migrate_edges_dict.keys():
        counter += 1
        logger.info(
            f"migrate_edges:{counter}: target_org_id = {target_org_id}, edges = {migrate_edges_dict.get('target_org_id')}"
        )
        ...
        if not edge_ids or len(edge_ids) <= 0:
            logger.warn(
                f"migrate_edges:{counter}: edges are not specified for org {target_org_id}. skipped migration for this org"
            )
            continue
        migrate_edges(target_org_id, edge_ids, poweredon_vms_only, skip_active_session_vms, verbose)
    logger.info(f"Ended RootCA migration")
...
def get_template_and_edge_details(template: dict, org_id: str, verbose: bool):
    ...
    if edge["providerLabel"].lower() not in allowed_providers:
        logger.warn(
            f"Pool migration is not supported on {edge.get('providerLabel')} provider edge {edge['id']}, org: {org_id}"
        )
        return
...
def power_policy_on_template(template: dict, enable: bool, verbose: bool):
    ...
    for i in range(3):
        try:
            ...
            logger.warn(
                f"powerPolicy update on template failed due to {str(ex)}. will auto retry after 90 seconds sleep"
            )
            time.sleep(90)
...
def perform_migration(vms: dict, target_org_id: str, template_id: str, batch_num: int, verbose: bool):
    ...
    for vm in migrate_vms:
        ...
        try:
            ...
            logger.error(
                f"{target_org_id}/{template_id}: failed to send get-mqtt-command to agent on vm {vm['id']} due to {str(error)}"
            )
            """
            vmm.hoc_event.send(
                u_vm,
    ...
    for vm in migrate_vms:
        ...
        if vm.get("regionalMqttEndpoint") is None or template_key_dict[REGIONAL_MQTT_URL] not in vm.get(
            "regionalMqttEndpoint"
        ):
...
def log_vm_info(target_org_id: str, template_id: str, vm_id: str):
    ...
    logger.info(
        f"vm %s - powerState: %s, haiAgentVersion: %s, agentStatus: %s, lifecycleStatus: %s, activeSessions: %s, regionalMqttEndpoint: %s, edgeMqttEndpoint: %s, agentCertIssuedByCALabel: %s",
        vm["id"],
        vm["powerState"],
