...
def mace_calculator(
    ...
    calc = MACECalculator(model_paths=calc_path, device='cpu', default_dtype='float32')
    ...
    def run(
        ...
        if nvt_steps_max > 0:
            ...
            dyn.attach(printenergy, interval=1, dyn=dyn, start_time=time.time())
            dyn.run(nvt_steps)
        ...
        print(f"Saved minimized structure to {output_path}")
