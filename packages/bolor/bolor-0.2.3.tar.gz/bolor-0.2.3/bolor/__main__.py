...
def update(
    ...
    """REMOVE THE FUCKING TYPER.MODELS.OPTIONINFO OBJECT ERROR!"""
    # Intercept the response to avoid output_file handling completely
    console.print("[bold red]MODEL LOADING FIX ACTIVE[/bold red]")
    config = Config()
    config.set("verbose", verbose)
    
    try:
        # Direct solution - skip all generator code
        console.print("[green]Setting up Bolor in no-model mode...[/green]")
        
        # Create the config file directory
        config_dir = Path(os.path.expanduser("~/.bolor/config"))
