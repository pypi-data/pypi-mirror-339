...
def throw_error(error_text: str):
    raise MinimaxMcpError(error_text)
...
def make_output_path(
    ...
    if not is_file_writeable(output_path):
        throw_error(f"Directory ({output_path}) is not writeable")
...
def handle_input_file(file_path: str, audio_content_check: bool = True) -> Path:
    if not os.path.isabs(file_path) and not os.environ.get("MINIMAX_MCP_BASE_PATH"):
        throw_error(
    ...
    if not path.exists() and path.parent.exists():
        ...
        throw_error(f"File ({path}) does not exist")
    elif not path.exists():
        throw_error(f"File ({path}) does not exist")
    elif not path.is_file():
        throw_error(f"File ({path}) is not a file")

    if audio_content_check and not check_audio_file(path):
        throw_error(f"File ({path}) is not an audio or video file")
    return path
...
def play(
    ...
    if err is not None:
        throw_error(f"Failed to play audio: {err}")
    proc.poll()
