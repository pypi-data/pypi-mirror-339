...
from .errors import CrsError, InputError, SlopeError
...
from .utils.smoother import cast_smoothed_rays, determine_ray_direction
...
class ShorelineAnalyzer:
    ...
    def _remove_intersections(self):
        self.trimmed = remove_intersecting_lines(self.trimmed)
...
class AnalysisResult:
    ...
    def visualize_coastal_slopes(self, plot_indices=True):
        ...
        min_x, max_x, min_y, max_y = self._get_expanded_bounds(500)
        ...
        breaking = self.breaking.plot(  # noqa: F841
            ax=ax1, color="red", lw=0.5, label="Breaking", zorder=9
        )

        ax1.set_title(
            f"HAT {self.metadata['hat']}, LAT {self.metadata['lat']} , Period: {self.metadata['wave_period']} s, Wave height: {self.metadata['wave_height']} m"
        )
        ax1.axis("off")
        ...
        ax1.set_title(
            f"Coastal Profile with Sampling Rays (Ray resolution: {self.metadata['ray_resolution']} m)"
        )
            ...
            pd.DataFrame(
                {
                    "Metric": [
                        ...
                        "Mean Angle (°)",
                        "Median Angle (°)",
                        "Basic Intertidal Gradient (Short and Wright)",
                        "Basic Intertidal Angle (°)",
                        "Weighted Intertidal Gradient (Plant et al)",
                        "Weighted Intertidal Angle (°)",
                        "Mean Segment Gradient (Masselink & Hegge)",
                        "Mean Segment Angle (°)",
                        "Area Weighted Gradient (Hegge et al)",
                        "Area Weighted Angle (°)",
