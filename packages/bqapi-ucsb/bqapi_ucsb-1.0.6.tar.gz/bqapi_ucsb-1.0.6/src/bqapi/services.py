...
class BaseServiceProxy(object):
    ...
    def request(self, path=None, params=None, method="get", render=None, **kw):
        ...
        try:
            ...
            # self.session.log.error ("xml parse error in %s",
            #                             response.content)
...
class AdminProxy(BaseServiceProxy):
    def login_as(self, user_name):
        ...
        userxml = data.fetch(
            "user",
            params={"wpublic": "1", "resource_name": user_name},
            render="xml",
        )
...
class BlobProxy(BaseServiceProxy):
    def _resource_element(
        self,
        args_tag_file=None,
        args_resource_type=None,
        args_srcpath=None,
        **kw,
    ):
        """Check the args and create a compatible resource element  for
        posting or linking"""
        if args_tag_file:
            # Load file into resource
            try:
                resource = etree.parse(args_tag_file).getroot()
            except etree.ParseError as _:
    ...
    def path_link(
        ...
        params = {}
        resource = self._resource_element(
            args_srcpath=srcpath,
            args_resource_type=resource_type,
            args_tag_file=tag_file,
        )
    ...
    def path_delete(self, srcpath, alias=None):
        url = urllib.parse.urljoin(
            self.session.service_map["blob_service"], "paths/remove"
        )
        params = {"path": srcpath}
        if alias:
            params["user"] = alias
    ...
    def path_rename(self, srcpath, dstpath, alias=None):
        url = urllib.parse.urljoin(
            self.session.service_map["blob_service"], "paths/move"
        )
    ...
    def path_list(self, srcpath, alias=None):
        url = urllib.parse.urljoin(
            self.session.service_map["blob_service"], "paths/list"
        )
        params = {"path": srcpath}
        if alias:
            params["user"] = alias
...
class ImageProxy(BaseServiceProxy):
    def get_thumbnail(self, image_uniq, **kw):
        url = urllib.parse.urljoin(
            self.session.service_map["image_service"], image_uniq, "thumbnail"
        )
        r = self.get(url)
        return r
...
def test_module():
    ...
    xml = data.get(
        "user", params={"wpublic": "1", "resource_name": "admin"}, render="xml"
    )
