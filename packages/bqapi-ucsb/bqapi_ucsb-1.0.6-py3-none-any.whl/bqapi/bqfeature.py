...
class Feature(object):
    def fetch(self, session, name, resource_list, path=None):
        """
        Requests the feature server to calculate features on provided
        resources.
        ...
        @param: path - the location were the hdf5 file is stored. If None
        is set the file will be placed in a tempfile and the pytables file
        handle will be returned. (default: None)

        @return: returns either a pytables file handle or the file name when
        ...
        if path is None:
            f = tempfile.NamedTemporaryFile(
                suffix=".h5", dir=tempfile.gettempdir(), delete=False
            )
            f.close()
            session.c.push(
                url,
                content=etree.tostring(resource),
                headers={
    ...
    def fetch_vector(self, session, name, resource_list):
        """
        Requests the feature server to calculate features on provided
        resources. Designed more for requests of very view features.
        ...
        @exception: FeatureError - if any part of the request has an error the
        FeatureError will be raised on the
        first error.
        note: You can use fetch and read from the status table for the error.
        warning: fetch_vector will not return response if an error occurs
        within the request
        """
...
class ParallelFeature(Feature):
    ...
    class BQRequestThread(Thread):
        ...
        def __init__(self, request_queue, errorcb=None):
            ...
            @param: errorcb - a call back that is called if a BQCommError is
                    raised
            """
    ...
    def calculate_request_plan(self, rl):
        ...
        if len(rl) > self.MaxThread * self.MaxChunk:
            ...
            if len(rl) / float(self.MaxThread) >= self.MinChunk:
                return (
                    self.MaxThread,
                    ceil(self.MaxChunk / float(self.MaxThread)),
                )
            else:
    ...
    def fetch(self, session, name, resource_list, path=None):
        ...
        @param: name - the name of the feature one wishes to extract
        @param: resource_list - list of the resources to extract. format:
        [(image_url, mask_url, gobject_url),...] if a parameter is not required
        just provided None
        @param: path - the location were the hdf5 file is stored. If None is
        set the file will be placed in a tempfile and the pytables file handle
        ...
        def request_factory(partial_resource_list):
            def request():
                ...
                while True:
                    try:
                        path = super(ParallelFeature, self).fetch(
                            session, name, partial_resource_list, path=f.name
                        )
                        ...
                        if e.errno == errno.WSAECONNRESET:
                            attempts += 1
                            log.debug(
                                "Connection fail: Attempting to reconnect \
                                    (try: %s)"
                                % attempts
                            )
                    try:
                        ...
                        # if fail gets corrupts during download
                        if attempts > MAX_ATTEMPTS:
                            log.debug(
                                "Failed to open hdf5 file: Reached max \
                                    attempts"
                            )
                            break
                        attempts += 1
                        log.debug(
                            "HDF5 file may be corrupted: Attempted to \
                                redownload (try: %s)"
                            % attempts
                        )
        ...
        self.request_thread_pool(
            request_queue, errorcb=self.errorcb, thread_count=int(thread_num)
        )
