...
from .util import sum_components
...
class Component:
    ...
    def __init__(self, model, description='', ID=-1):
        ...
        if isinstance(model, str) and (model in Component.valid_strs):
            ...
            if model == 'Constant':
                ...
                self.model = Moffat1D(bounds={'alpha': (1, None), 'gamma': (1e-30, None)})
    ...
    def getParamsLists(self):
        ...
        for p in self.model.param_names:
            val_str = f'{self.model.__getattribute__(p).value:.3f}'
            if self.model.fixed[p]:
                ...
                if self.model.bounds[p][0] is None:
                    ...
                    lb = f'{self.model.bounds[p][0]:.3f}'
                
                if self.model.bounds[p][1] is None:
                    ub = '+∞'
                else:
                    ub = f'{self.model.bounds[p][1]:.3f}'
...
class FunctionFitter:
    ...
    def fit(self):
        """ Run the fitter """
        try :
            fit = self.fitter(self.function.model, self.x, self.y, weights=self._fitter_weights)
