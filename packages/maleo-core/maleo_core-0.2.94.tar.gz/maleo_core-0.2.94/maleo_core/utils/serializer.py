...
class BaseSerializer:
    ...
    def recursive_prune(obj: Any, expandable_fields: set[str], expand_map: dict[str, set[str]], current_field: str = "") -> Any:
        ...
        result = {}
        # First pass: determine which fields are expanded
        expanded_fields = set()
        for key, value in obj.items():
            full_key = f"{current_field}.{key}" if current_field else key
            
            # Track expanded relation fields
            if isinstance(value, dict) and (key in expand_map or full_key in expand_map):
                expanded_fields.add(key)

        # Second pass: process the fields
        for key, value in obj.items():
            full_key = f"{current_field}.{key}" if current_field else key
            
            # Skip foreign key fields for expanded relations
            if key.endswith('_id') and key[:-3] in expanded_fields:
                continue
            ...
            if key in expand_map or full_key in expand_map:
                nested_expansion = expand_map.get(full_key, set()).union(expand_map.get(key, set()))
                if isinstance(value, dict):
                    result[key] = BaseSerializer.recursive_prune(
                        value, 
                        expandable_fields, 
                        BaseSerializer.build_nested_expand_structure(nested_expansion), 
                        full_key
                    )
                else:
