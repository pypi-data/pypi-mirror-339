...
class MaleoAccessUserTypeHTTPService:
    ...
    async def get_user_types(
        ...
        MaleoAccessHTTPClientUserTypeServiceResults.MultipleData
    ]:
        """Fetch user types from maleo-access"""
        result = await MaleoAccessUserTypeHTTPController.get_user_types(parameters=parameters)
        if not result.success:
            return MaleoAccessHTTPClientUserTypeServiceResults.Fail.model_validate(result.content)
        else:
            return MaleoAccessHTTPClientUserTypeServiceResults.MultipleData.model_validate(result.content)
