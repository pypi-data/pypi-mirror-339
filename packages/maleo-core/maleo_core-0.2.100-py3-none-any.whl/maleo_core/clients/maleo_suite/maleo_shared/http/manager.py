...
class MaleoSharedHTTPClientManager(HTTPClientManager):
    ...
    def initialize(cls, base_url:Optional[str] = None) -> None:
        ...
        cls._base_url = base_url or os.getenv("MALEO_SHARED_API_BASE_URL")
        if cls._base_url is None:
            raise ValueError("MALEO_SHARED_API_BASE_URL environment variable must be set if no base_url is provided")
