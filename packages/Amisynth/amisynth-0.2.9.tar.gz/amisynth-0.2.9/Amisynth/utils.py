...
class ContextAmisynth:
    ...
    def set_context(self, ctx):
        ...
        elif isinstance(ctx, tuple) and len(ctx) == 3 and isinstance(ctx[0], discord.TextChannel) and isinstance(ctx[1], discord.User) and isinstance(ctx[2], int):
            ctx.channel_id = ctx[0].id
            ctx.obj_channel = ctx[0]
...
class VariableManager:
    ...
    def set_value(self, level, key, value, guild_id=None, user_id=None, channel_id=None):
        ...
        self._save_to_file()  # Guarda cambios en el JSON
