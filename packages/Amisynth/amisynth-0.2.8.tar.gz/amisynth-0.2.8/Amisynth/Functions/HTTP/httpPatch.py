...
async def httpPatch(url: str, json: str, *args, **kwargs):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.patch(url, json=json, headers=utils.http_data["headers"]) as response:
