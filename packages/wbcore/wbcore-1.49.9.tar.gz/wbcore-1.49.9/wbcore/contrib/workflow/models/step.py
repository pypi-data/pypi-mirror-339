...
class Step(WBModel):
    ...
    workflow = models.ForeignKey(
        to="workflow.Workflow", verbose_name=_("Workflow"), on_delete=models.CASCADE, related_name="associated_steps"
    )
    ...
    def execute_single_next_step(self, process_step: ProcessStep):
        ...
        if len(valid_transitions := self.get_all_valid_outgoing_transitions(process_step)) == 1:
            ...
            self.set_failed(process_step, gettext("No valid outgoing transitions found for this step!"))
        else:
            self.set_failed(process_step, gettext("More than one possible outgoing transition found for this step!"))

    def run(self, process_step: ProcessStep):
        self.set_finished(process_step)

    def finish(self, process_step: ProcessStep):
        if process_step.state in [ProcessStep.StepState.FINISHED, ProcessStep.StepState.CANCELED]:
            # Set finished field
...
class DecisionStep(Step):
    ...
    def run(self, process_step: ProcessStep):
        if transition := self.get_first_valid_transition(process_step):
            ...
            self.set_failed(process_step, gettext("No valid outgoing transition found for this step!"))
...
class EmailStep(Step):
    ...
    cc = models.ManyToManyField(
        "directory.EmailContact", related_name="workflow_steps_cc", verbose_name=_("CC"), blank=True
    )
    bcc = models.ManyToManyField(
        "directory.EmailContact", related_name="workflow_steps_bcc", verbose_name=_("BCC"), blank=True
    )
    ...
    def run(self, process_step: ProcessStep):
        ...
        if (
            ProcessStep.objects.filter(
                process=process_step.process, state__in=[ProcessStep.StepState.WAITING, ProcessStep.StepState.ACTIVE]
            )
            .exclude(id=process_step.pk)
            .exists()
        ):
            self.set_failed(process_step, gettext("There are process steps still running for this workflow!"))
        else:
            self.set_finished(process_step)
    ...
    # Otherwise we need to check if all unfinished join steps that need several inputs can still be reached
        ...
        waiting_join_steps = JoinStep.objects.filter(
            ...
            & Q(
                ...
                | Q(
                    Q(process_steps__state__in=[ProcessStep.StepState.ACTIVE, ProcessStep.StepState.WAITING])
