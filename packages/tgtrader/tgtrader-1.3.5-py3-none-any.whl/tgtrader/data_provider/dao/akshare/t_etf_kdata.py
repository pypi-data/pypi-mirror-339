...
from tgtrader.data_provider.dao.models.t_etf_kdata_model import T_ETF_KData_Model


@register_model(DataSource.Akshare, 't_etf_kdata')
class T_ETF_KData(T_ETF_KData_Model):
    class Meta:
        database = main_db
    ...
    def init_table(cls):
        # 初始化表
        with main_db:
            table_exists = T_ETF_KData.table_exists()
            if not table_exists:
                main_db.create_tables([T_ETF_KData])  # 如果表不存在，创建表
