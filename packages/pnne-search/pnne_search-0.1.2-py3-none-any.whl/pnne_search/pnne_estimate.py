...
def pnne_estimate(Y, Xp, Xa, Xc, consumer_idx, checks=True, se=False, use_parallel=True):
    ...
    k = np.argsort(rng.random_sample(n))

    i = ((k[:, None]) * J + np.arange(J)).flatten()
    ...
    if n <= nne['dim']['n'][1]:
        ...
        if use_parallel:
            results = Parallel(n_jobs=-1)(
                delayed(split_moments)(b, blocks, J, Y, Zp, Za, Zc, consumer_idx, nne)
            ...
            all_mmt = []; all_sens = []
            for b in range(1, blocks + 1):
                mmt, sens = split_moments(b, blocks, J, Y, Zp, Za, Zc, consumer_idx, nne)
                all_mmt.append(mmt)
    ...
    if se:
        ...
        draws = np.floor(rng.random_sample(size=(se_repeats,n)) * n).T.astype(int)

        if n <= nne['dim']['n'][1]:

            if use_parallel:
                results = Parallel(n_jobs=-1)(
                    delayed(bootstrap_moments)(r, draws, J, Y, Zp, Za, Zc, consumer_idx, nne)
                ...
                all_mmt = []
                for r in range(se_repeats):
                    mmt = bootstrap_moments(r, draws, J, Y, Zp, Za, Zc, consumer_idx, nne)
                    all_mmt.append(mmt)
                all_mmt = np.squeeze(np.array(all_mmt), axis=1)
            ...
            par['pred_mat'] = pred[:,i_par]
        else: # large data
            all_mmt = []
            if use_parallel:
                all_mmt = Parallel(n_jobs=-1)(
                    delayed(bootstrap_split_moments)(blocks, r, draws, J, Y, Zp, Za, Zc, consumer_idx, nne)
                    for r in range(se_repeats)
                    )
                all_mmt = np.squeeze(np.array(all_mmt), axis=2)
            else:
                for r in range(se_repeats):
                    mmt = bootstrap_split_moments(blocks, r, draws, J, Y, Zp, Za, Zc, consumer_idx, nne)
                    all_mmt.append(mmt)
                all_mmt = np.array(all_mmt)
                all_mmt = np.squeeze(np.array(all_mmt), axis=2)
            pred_net = []
    ...
    if 'pred_mat' in par:
        alpha0 = par['pred_mat'][:,0].reshape(se_repeats, 1)
        alpha = par['pred_mat'][:,1:a+1].reshape(se_repeats, a)
        eta0 = par['pred_mat'][:,a+1].reshape(se_repeats, 1)
        eta = par['pred_mat'][:,a+2:a+2+c].reshape(se_repeats, c)
        beta = par['pred_mat'][:,a+2+c:a+2+c+p].reshape(se_repeats, p)

        val_mat = np.concatenate([
            alpha0 - np.sum(alpha / sigma_a * mu_a),
            alpha / sigma_a,
            eta0 - np.sum(eta / sigma_c * mu_c) + np.sum(beta / sigma_p * mu_p),
            eta / sigma_c,
...
def bootstrap_moments(r, draws, J, Y, Zp, Za, Zc, consumer_idx, nne):
    k = draws[:,r]
    i = ((k[:, None]) * J + np.arange(J)).flatten()
...
def split_moments(b, blocks, J, Y, Zp, Za, Zc, consumer_idx, nne):
    n = len(Zc)
    m = int(np.ceil(n / blocks))
    k = np.arange((b-1) * m, min(b * m, n))
...
def bootstrap_split_moments(blocks, r, draws, J, Y, Zp, Za, Zc, consumer_idx, nne):
    k = draws[:,r]
    i = ((k[:, None]) * J + np.arange(J)).flatten()
    all_mmt = []
    for b in range(1, blocks + 1):
        mmt,_ = split_moments(b, blocks, J, Y[i], Zp[i], Za[i], Zc[k], consumer_idx,nne)
        all_mmt.append(mmt)
