...
class LocalSyncClient:
    ...
    def __init__(
        ...
        self._last_remote_repo_state = None
    ...
    async def _send_message(self, message: LocalSyncClientMessage) -> LocalSyncServerMessage:
        ...
        async with self._make_post_request(
            ...
            logger.debug(f"Received message response on local sync client")
    ...
    async def _sync_branch(
        ...
        logger.debug(f"Assuming target branch is {target_branch_name}")
    ...
    async def _sync_branch_while_active(
        ...
        async for line in content_iterator:
            ...
            if isinstance(message, SyncedMessage):
                ...
                last_save_version = message.version
                break
            else:
        ...
        try:
            while True:
                    ...
                    if data != "":
                        ...
                        if isinstance(message, NewRemoteRepoStateMessage):
                            ...
                            if message.version is None:
                                logger.error(
                                    "Server encountered unexpected error while processing save message, re-syncing branch..."
                                )
                                return
                            logger.debug("Enqueued new remote repo state message")
                            new_remote_state_messages.append(message)
                ...
                if self.current_save_task is not None and self.current_save_task.done():
                    ...
                    if isinstance(save_result, FailedToSaveMessage):
                        ...
                        if not save_result.is_containing_new_changes:
                            last_save_version = save_result.version
                        else:
                ...
                if conflict_version is not None:
                    ...
                    if is_using_local_changes:
                        ...
                        if latest_message.version < conflict_version:
                            # if not, just continue -- we'll get it from the server soon enougn
                            logger.debug(
                                f"Waiting to get remote state version={conflict_version} before resolving conflict"
                            )
                            continue
                        new_repo_state = VersionedLocalRepoState(
                ...
                if self.want_to_save_flag.is_set():
                    ...
                    new_file_change_event_task = await self._handle_save(self._last_remote_repo_state, project)
                ...
                if latest_message is not None:
                    ...
                    if latest_message.version > last_save_version or (
                        latest_message.version == last_save_version and is_different_commit
                    ):
                        logger.debug("Applying new remote state")
                        await self._process_new_repo_state_message(latest_message, project)
    ...
    async def _process_new_repo_state_message(self, message: NewRemoteRepoStateMessage, project: Project) -> None:
    ...
    async def _handle_save(self, remote_repo_state: VersionedLocalRepoState, project: Project) -> asyncio.Task:
        ...
        if is_local_repo_states_equal(current_local_repo_state, remote_repo_state):
        ...
        async with self._get_local_repo_for_sync() as repo:
            ...
            await repo.run_git(
                [
                    ...
                    *([f"{commit_hash}:refs/heads/{branch_name}"] if target_branch_name is not None else []),
                    *([f"{target_branch_name}"] if target_branch_name is not None else []),
                ]
            )
...
async def _get_target_branch_name(
    ...
    proc = await asyncio.create_subprocess_shell(
        ...
        f'git log --decorate=full --oneline {local_state.branch_name} | grep "refs/heads/" | head -n 10',
        stdin=subprocess.DEVNULL,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        executable="/bin/bash",
        cwd=str(local_sync_repo_path),
    )
    stdout_bytes, stderr_bytes = await proc.communicate()
    stdout = stdout_bytes.decode("UTF-8")
    _stderr = stderr_bytes.decode("UTF-8")
    assert proc.returncode == 0
    for line in stdout.splitlines():
        branch_list = line.split(" (", maxsplit=1)[-1].split(")", maxsplit=1)[0]
        branch_name_strings = branch_list.split(", ")
        branch_names = []
        for branch_name_string in branch_name_strings:
            branch_name = branch_name_string.rsplit(" ")[-1]
            if branch_name.startswith("refs/heads"):
                branch_names.append(branch_name)
        branch_names = [b for b in branch_names if b != f"refs/heads/{local_state.branch_name}"]
        if not branch_names:
            continue
        # if there are multiple candidates, select the ones that matches the previous remote state
        if remote_state is not None:
            if f"refs/heads/{remote_state.branch_name}" in branch_names:
                return remote_state.branch_name
        # otherwise, just return the first one
        if len(branch_names) > 0:
            return sorted(branch_names)[0]
    # and if we fail to find anything, fine, the current branch is the target
