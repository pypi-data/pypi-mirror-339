...
class RRuleValueParser(ValueParser):
    LEXICON = {
        ...
        'BYDAY': ByDayParser,
        'BYMONTHDAY': integer_list_parser(1, 31, exception=[0]),
        'BYYEARDAY': integer_list_parser(1, 366, exception=[0]),
        'BYWEEKNO': integer_list_parser(1, 53, exception=[0]),
        'BYMONTH': integer_list_parser(1, 12),
        'BYSETPOS': integer_list_parser(1, 366, exception=[0]),
    ...
    def tokenize(self):
        self.tokens = RRuleParser(self.txt, self.LEXICON, self.MANDATORY_KEYS).run()
