...
class ApiFluid:
    ...
    To be used in the `FlashCalculationInput.fluid` attribute.
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Returns the `ApiFluid` attributes as a dict."""
    ...
    def from_dict(cls: Type[T], src_dict: Dict[str, Any]) -> T:
        """Create an instance of `ApiFluid` from a dict representation."""
        d = src_dict.copy()
    ...
    def __str__(self) -> str:
        ...
        parts.append(f"  Comment: {self.comment if not isinstance(self.comment, Unset) else "N/A"}")
        parts.append(f"  EOS: {self.eos if not isinstance(self.eos, Unset) else "N/A"}")
        parts.append(f"  Property Reference Point: {self.property_reference_point if not isinstance(self.property_reference_point, Unset) else 'N/A'}")
        parts.append(f"  Solvent Cp: {self.solvent_cp if not isinstance(self.solvent_cp, Unset) else 'N/A'}")
        parts.append(f"  Polymer Cp: {self.polymer_cp if not isinstance(self.polymer_cp, Unset) else 'N/A'}")
