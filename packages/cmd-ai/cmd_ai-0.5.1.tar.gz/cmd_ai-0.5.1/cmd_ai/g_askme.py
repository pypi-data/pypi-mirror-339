...
# --------------------------------------------------------------
#
def g_ask_gptchat_new(prompt,  model, temp=0,
                  dontadd=False):
#def main():
    """
...
#                 model="claude-3-5-sonnet-20241022",
    ...
    # ** init
    clienta = anthropic.Anthropic(api_key=api_key.get_api_key_anthropic())
...
# ------------------------------------------------------------
#
def g_ask_gemini(prompt , temp=0,
                 model="gemini-2.5-pro-exp-03-25",
                 max_tokens=1000, role=None,
                 dontadd=False):
    STOPPED_BY_LENGTH = False
    # ** init
    clientg = openai.OpenAI( api_key= api_key.get_api_key_gemini(),
...
def g_ask_anyone(
    ...
    if config.MODEL_TO_USE.find("claude") >= 0:
        ...
        #print("G")
        resdi = g_ask_gemini( prompt, temp, config.MODEL_TO_USE, estimate_tokens)
        ...
        resdi = g_ask_gptchat_new( prompt, config.MODEL_TO_USE )
        #
    elif config.MODEL_TO_USE.find("bigthree") >= 0 :
        resdi = g_ask_claude( prompt, temp, "claude-3-7-sonnet-20250219", estimate_tokens, dontadd=True)
        print(texts.color_of_model("claude"), resdi[0], fg.default)
        resdi = g_ask_gemini( prompt, temp, "gemini-2.5-pro-exp-03-25"  , estimate_tokens, dontadd=True)
        print(texts.color_of_model("gemini"),resdi[0], fg.default)
        resdi = g_ask_gptchat_new( prompt,  "gpt-4o-2024-11-20", dontadd=True )
        print(texts.color_of_model("gpt"),  resdi[0], fg.default)
        #
        resdi = "dialog is not recorded", "select", "bigthree"
    else:
        resdi = None
