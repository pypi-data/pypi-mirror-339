...
class PtInsearcher:
    ...
    def run(self, args: argparse.Namespace):
        ...
        results_raw = self.process_targets(self.url_list)

        # Filter None values out
        self.result_list = [res for res in results_raw if res]

        # Print output continuously
        for result in self.result_list:
            results.print_result([result], args)

        # If grouping_complete, print again grouped
        if self.result_list and args.grouping_complete:
    ...
    def _get_domain(self, domain: str) -> str:
        if domain and not re.match(r"https?://", domain):
