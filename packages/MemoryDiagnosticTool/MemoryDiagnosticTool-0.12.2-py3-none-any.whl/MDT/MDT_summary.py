...
def MDT_summary_table(mdt_html, rrw_file, esid_file):
    ...
    rrw_data  = {}
    rrw_count = {}
    esid_data = defaultdict(lambda: {"log_files": [], "pass_count": 0})
    memtest_result = {}
    pmu_train_result = ""
    try:
        ...
        if os.path.exists(esid_file):
            ...
            summary_table = "<tr><th>System</th><th>RRW IOD0 Pass/Total</th><th>RRW IOD1 Pass/Total</th><th>ESID Pass/Total</th><th>Memtest Absent Channel</th></tr>"
            # Check systems that meet the condition of 8 log files and all "Pass"
            for system, esid_count in esid_data.items():
                if rrw_count:
                    summary_table += f"<tr><td>{system}</td><td>{rrw_count[system][0]['pass']}/{rrw_count[system][0]['total']} Pass</td><td>{rrw_count[system][1]['pass']}/{rrw_count[system][1]['total']} Pass</td><td>{esid_count['pass_count']}/{len(esid_count['log_files'])} Pass</td><td>{memtest_result[system]}</td></tr>"
                else:
                    summary_table += f"<tr><td>{system}</td><td> NA </td><td> NA </td><td>{esid_count['pass_count']}/{len(esid_count['log_files'])} Pass</td><td>{memtest_result[system]}</td></tr>"

            if not pmu_train_result:
                pmu_train_result = "No PMU training error detected."
            else:
                pmu_train_result = "PMU failed channel:<br>" + pmu_train_result
