...
def parse_memtest_umcs(file_name, system_name, bios, output_file):
    ...
    # s1_dimms = []
    s0_d0_memtest_umcs = {0: False, 1: False, 2: False, 3: False, 4: False, 5: False, 6: False, 7: False, 8: False, 9: False, 10: False, 11: False, 12: False, 13: False, 14: False, 15: False}
    s0_d1_memtest_umcs = {0: False, 1: False, 2: False, 3: False, 4: False, 5: False, 6: False, 7: False, 8: False, 9: False, 10: False, 11: False, 12: False, 13: False, 14: False, 15: False}
    # s1_memtest_umcs = []
    for line in iod0:
    ...
    if(os.path.splitext(file_name)[1] == ".log"):
        ...
        miscompare_iods = []
        if miscompare_match0: miscompare_iods.append("IOD0")
        if miscompare_match1: miscompare_iods.append("IOD1")

        df_am = {"Log File":[fl],"System":[system_name],"BIOS":[bios],"Training Status":[pmu_train_status],"PMU Training Failures Channel":[pmu_train_fails], "IOD0 ESID Pass":["Pass" if esid_complete_match0 else "Fail"],
                 "IOD1 ESID Pass":["Pass" if esid_complete_match1 else "Fail"], "Any Miscompares":[miscompare_iods],
        ...
        if crop_started:
            if end_marker in line:
                ...
                section2_lines.append(
                    line
                )  # Add all lines between the markers to section2
        else:
...
if __name__ == '__main__':
    ...
    if os.path.isdir(args.input):
        ...
        for file in log_files:
            base = os.path.splitext(os.path.basename(file))[0]
            parse_memtest_umcs(file, args.system, args.bios, out_csv)
        ...
        if os.path.exists(args.input):
            ...
            df.to_csv(out_csv, mode="w", header=True, index=False)
            parse_memtest_umcs(args.input, args.system, args.bios, out_csv)
            print("Memtest parsing results are saved in the file: ", out_csv)
        else:
