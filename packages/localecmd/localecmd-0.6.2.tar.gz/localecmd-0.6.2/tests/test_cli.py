...
def test_distributor_finding():
    flist = "help list_cmds list_funcs".split()
    assert find_distributor_functions(flist) == set("list".split())

    # Distributors with more than 1 underscore not implemented
    flist = "help list_cmds list_funcs_now list_func_later".split()
    dist = find_distributor_functions(flist)
    assert dist == set("list".split())
