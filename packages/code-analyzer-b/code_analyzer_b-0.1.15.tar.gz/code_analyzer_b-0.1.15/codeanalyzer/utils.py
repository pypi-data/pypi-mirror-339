...
tqdm = partial(tqdm, lock_args=(None,))
...
def scan_files(directory):
    return [
        ...
        for file in files
        if (os.path.splitext(file)[1].lower() in Config.SUPPORTED_EXTENSIONS
            and os.path.getsize(os.path.join(root, file)) <= Config.MAX_FILE_SIZE)
    ]
...
def write_report(report, filename, format=None, include_details=True):
    if format:
        file_ext = f'.{format}'
    else:
    ...
    if file_ext == '.txt':
        _write_txt_report(report, filename, include_details)
    elif file_ext == '.md':
        _write_markdown_report(report, filename, include_details)
    elif file_ext == '.html':
        _write_html_report(report, filename, include_details)
    elif file_ext == '.json':
        _write_json_report(report, filename, include_details)
    elif file_ext == '.sarif' or format == 'sarif':
        _write_sarif_report(report, filename, include_details)
    else:
        raise ValueError(f"Unsupported file format: {file_ext}")


def _write_txt_report(report, filename, include_details):
    content = [
    ...
    if include_details and report.get('detailed_findings'):
        content.append("\nDetailed Findings:")
...
def _write_sarif_report(report, filename, include_details=True):
    sarif_template = {
    ...
    findings = report['detailed_findings'] if include_details else []

    for idx, finding in enumerate(findings):  # Use filtered list
