...
class CodeAnalyzer:
    ...
    async def analyze_file(self, file_path):
        try:
            ...
            self.stats['errors'] += 1  # Add error counter to stats
            print(f"::warning file={file_path}::Analysis error: {str(e)}")
        finally:
    ...
    async def generate_report(self, include_details=True, stream=False):
        stats_summary = self._generate_stats_summary()
        ai_summary = await self._generate_ai_summary(stream=stream)
    ...
    async def _generate_ai_summary(self, stream=False):
        ...
        async with self.client as client:
            if stream:
                ...
                stream = client.generate_summary_stream(findings_text)
                async for chunk in stream:
                    print(chunk, end='', flush=True)
                    full_summary.append(chunk)
            else:
                summary = await client.generate_summary(findings_text)
                full_summary.append(summary)

        return "AI Analysis:\n" + ''.join(full_summary) if full_summary else ""
