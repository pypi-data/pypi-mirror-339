...
async def analyze_command_async(args):
    ...
    try:
        ...
        # Create analyzer with explicit async context
        async with CodeAnalyzer() as analyzer:
            await analyzer.analyze_project(files)
            report = await analyzer.generate_report(include_details=not args.no_details)
        ...
        # Explicit cleanup if context manager fails
        if analyzer and not analyzer.closed:
