...
def format_notification_response(data: Dict) -> Dict:
    ...
    try:
        ...
        for notification in data["notifications"]:
            ...
            username = notification["data"].get(
                "display_username", "") or notification.get("acting_user_name", "")
            title = notification.get(
                "fancy_title", "") or notification["data"].get("topic_title", "")
...
@mcp.tool()
async def top_topic(
    period: Optional[Literal["daily", "weekly",
                             "monthly", "quarterly", "yearly", "all"]] = None,
    page: int = 1,
    per_page: int = 10
) -> Dict:
    """
...
async def new_notification(
    limit: int = 10,
    read: bool = False,
    filter_by_types: List[Literal["reply", "like", "other"]] = []
) -> Dict:
    """
    Name:
        获取Linux.do您最近的未读通知
    ...
    try:
        ...
        if filter_by_types:
            mapped_types = []  # Initialize as empty list
            ...
            # Remove duplicates that might arise from the map
            unique_mapped_types = sorted(list(set(mapped_types)))

            if unique_mapped_types:
                api_params["filter_by_types"] = ",".join(unique_mapped_types)
