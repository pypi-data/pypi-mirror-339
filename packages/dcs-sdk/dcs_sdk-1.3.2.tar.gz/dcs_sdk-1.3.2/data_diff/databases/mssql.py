...
class MsSQL(ThreadedDatabase):
    ...
    def __init__(self, host, port, user, password, *, database, thread_count, **kw) -> None:
        ...
        if "timeout" not in self._args:
            self._args["timeout"] = 30
        try:
    ...
    def create_connection(self):
        ...
        try:
            conn_dict = {
                ...
                "UID": self._args.get("user"),
                "PWD": self._args.get("password"),
                "TrustServerCertificate": "yes",
            }
            conn_dict["SERVER"] = f"{self._args.get('server')},{self._args.get('port')}"
