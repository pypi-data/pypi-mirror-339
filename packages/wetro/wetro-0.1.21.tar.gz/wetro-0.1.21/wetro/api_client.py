...
class WetrocloudClient:
    ...
    def _upload_file(self,collection_id: CollectionID, resource: Union[str, bytes, Any]) -> str:
        """
        Uploads a file to the file-upload service and returns the URL.
        If resource is a string that doesn't start with "http", it's assumed to be a file path.
        Otherwise, resource can be a bytes-like object or a file-like object.
        """
        upload_url = "https://file-upload-service-python.vercel.app/upload/"
        files = {}
        if isinstance(resource, str) and not resource.startswith("http"):
            # Assume resource is a file path.
            resource = Path(resource).expanduser().resolve().as_posix()
            # print(resource)
            if not os.path.exists(resource):
                raise ValueError(f"File {resource} does not exist.")
            files["file"] = open(resource, "rb")
        else:
            # If resource is already a file-like object or bytes.
            files["file"] = resource

        try:
            response = requests.post(upload_url,data={"collection_id":collection_id}, files=files)
            response.raise_for_status()
            json_response = response.json()
            file_url = json_response.get("url")
            if not file_url:
                raise ValueError("File upload did not return a URL.")
            return file_url
        finally:
            if isinstance(resource, str) and not resource.startswith("http"):
                files["file"].close()
    
    def _get_resource(self,collection_id: CollectionID, resource: Union[str, bytes, Any]) -> str:
        if not (isinstance(resource, str) and resource.startswith("http")):
            resource = self._upload_file(collection_id,resource)
        return resource
    ...
    def list_collection(self) -> ListCollectionResponse:
        """
            ...
            Endpoint:
                POST /v1/collection/get/<collection_id>/ (&#8203;:contentReference[oaicite:6]{index=6})
        """
        endpoint = f"/v1/collection/all/"
        
        return self._request("GET", endpoint, response_model=ListCollectionResponse)
    ...
    def insert_resource(
        ...
        """
        resource = self._get_resource(collection_id,resource)
        payload = {
            "collection_id": collection_id,
            "resource": resource,
            "type": type
    ...
    def remove_resource(
        ...
        return self._request("DELETE", "/v1/resource/remove/", data=form_data, response_model=RemoveResponse)
...
if __name__ == "__main__":
    ...
    # client = WetrocloudClient(api_key="your_api_key_here")   
    try:
        pass
        # Create a new collection
        # create_response = client.create_collection("sdk_unique_id_3")
        # print("Collection Created: %s", create_response)
        ...
        #     print("Resource Inserted: %s", insert_response)
        ...
        #         chat_history=chat_history
        #     )
        #     print("Chat Response: %s", chat_response)
