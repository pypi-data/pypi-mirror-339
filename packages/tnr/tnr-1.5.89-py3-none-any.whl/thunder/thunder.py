...
TEMPLATE_NAMES = [template['name'] if template['default'] else template['displayName'] for template in sorted_templates]
NICE_TEMPLATE_NAMES = {template['name']: template['displayName'] for template in TEMPLATES}
OPEN_PORTS = {template['name']: template['openPorts'] for template in TEMPLATES}
AUTOMOUNT_FOLDERS = {template['name']: template['automountFolders'] for template in TEMPLATES}
...
def setup_shell_completion():
    ...
    if IS_WINDOWS:
        ...
        if 'zsh' in shell:
            ...
            completion_line = 'eval "$(_TNR_COMPLETE=zsh_source tnr)" > /dev/null 2>&1'
        elif 'bash' in shell:
            rc_file = os.path.expanduser('~/.bashrc')
            completion_line = 'eval "$(_TNR_COMPLETE=bash_source tnr)" > /dev/null 2>&1'
        elif 'fish' in shell:
            rc_file = os.path.expanduser('~/.config/fish/completions/tnr.fish')
            completion_line = '_TNR_COMPLETE=fish_source tnr | source > /dev/null 2>&1'
        else:
            ...
            if loading:
                ...
                for instance_id, metadata in instances.items():
                    ...
                    if not template_key.startswith("client-image-"):
                        ...
                        template_name = NICE_TEMPLATE_NAMES.get(template_key, template_key)
                    else:
...
@click.option('--template', type=click.Choice(TEMPLATE_NAMES),
              help='Environment template: base (standard ML tools), comfy-ui, comfy-ui-wan, ollama, webui-forge, or custom snapshots')
@click.option('--gpu', type=click.Choice(['t4', 'a100', 'a100xl', 'h100']),
...
def create(vcpus, template, gpu, num_gpus, disk_size_gb):
    ...
    if vcpus is None:
        vcpus = click.prompt(
            f"Number of vCPUs (RAM is 4x vCPUs)",
            type=click.Choice(['4', '8', '16', '32']),
    ...
    if gpu is None:
        gpu = click.prompt(
            "GPU type",
            type=click.Choice(['t4', 'a100', 'a100xl', 'h100']),
            default=default_gpu,
            show_default=True,
            show_choices=False
        )
...
@click.option('--list', 'list_flag', is_flag=True, help='List all snapshots.') # Renamed variable
@click.option('--delete', 'delete_name', type=str, help='Delete a snapshot by name.') # Renamed variable
def snapshot(instance_id, snapshot_name, list_flag, delete_name):
    ...
    if is_list_mode and (instance_id is not None or snapshot_name is not None):
         raise click.ClickException("Cannot provide instance ID or snapshot name when using --list.")
    if is_delete_mode and (instance_id is not None or snapshot_name is not None):
         raise click.ClickException("Cannot provide instance ID or snapshot name when using --delete.")
    ...
    if is_list_mode:
        ...
        initial_table = _create_snapshot_table([], show_timestamp=True, loading=True)

        try:
            # Use Live to manage the updating table display
            with Live(initial_table, refresh_per_second=4, transient=True) as live:
                current_snapshots = _fetch_snapshot_data(token) # Initial fetch
                while True:
                    ...
                    table = _create_snapshot_table(current_snapshots, show_timestamp=True, changed=not pending)
                    ...
                    if new_snapshots != current_snapshots:
                         ...
                         if not _snapshots_pending(current_snapshots):
                             ...
                             table = _create_snapshot_table(current_snapshots, show_timestamp=True, changed=True)
                             final_table = table
                             live.update(table)
                             break
    ...
    # DELETE Action
        ...
        if not re.match(r'^[a-z0-9-]+$', delete_name):
            raise click.ClickException("Snapshot name for --delete can only contain lowercase letters, numbers, and hyphens.")

        click.echo(click.style(f"Attempting to delete snapshot '{delete_name}'...", fg="cyan")) # User feedback
        if utils.delete_snapshot(delete_name, token):
            click.echo(click.style(f"Successfully deleted snapshot '{delete_name}'.", fg="green"))
        else:
            # Improve error message if possible (e.g., distinguish not found vs other error)
            raise click.ClickException(f"Failed to delete snapshot '{delete_name}'. Does the snapshot exist or do you have permissions?")
    ...
    # CREATE Action (or invalid arguments)
        ...
        if has_create_args:
            ...
            if not re.match(r'^[a-z0-9-]{1,62}$', snapshot_name):
                raise click.ClickException("Snapshot name can only contain lowercase letters, numbers, and hyphens (max 62 characters).")
            ...
            if not success:
                if "Unauthorized" in error:
                    ...
                    raise click.ClickException(f"Failed to get instance details: {error}")

            instance_id_str = str(instance_id)
            if instance_id_str not in instances:
                 raise click.ClickException(f"Instance with ID {instance_id} not found.")

            if instances[instance_id_str]["status"] != "STOPPED":
                raise click.ClickException(f"Instance {instance_id} must be stopped to create a snapshot. Use `tnr stop {instance_id}` and try again.")

            # Call create snapshot utility
            click.echo(click.style(f"Creating snapshot '{snapshot_name}' from instance {instance_id}...", fg="cyan"))
            success, response = utils.make_snapshot(token, snapshot_name, instances[instance_id_str]["name"])
            if success:
                click.echo(click.style("Successfully initiated snapshot creation.", fg="green"))
                # Start list command
                click.echo('\n')
                ctx = click.get_current_context()
                ...
                if "already exists" in response:
                    raise click.ClickException(f"Snapshot with name '{snapshot_name}' already exists. Please choose a different name.")
                else:
                    raise click.ClickException(f"Failed to initiate snapshot creation. An error occurred on the backend:\n {response}")
             ...
             raise click.ClickException("Both instance ID and snapshot name are required to create a snapshot.")
...
def _create_snapshot_table(snapshots, show_timestamp=False, changed=False, loading=False):
    ...
    if loading:
        ...
        for snapshot in snapshots:
            ...
            status_text = Text("READY", style="green") if snapshot.get("ready", False) else Text("PENDING", style="yellow")
            snapshots_table.add_row(
                str(snapshot.get("displayName", "--")),
                status_text,
                str(snapshot.get("defaultSpecs", {}).get("template", "--")),
                str(snapshot.get("defaultSpecs", {}).get("gpuType", "--")).upper(),
                f"{str(int(snapshot.get('defaultSpecs', {}).get('storage', '--')))}GB",
                f"{str(int(snapshot.get('archiveSize', 0)))}GB" if snapshot.get("archiveSize") else "--"
            )
