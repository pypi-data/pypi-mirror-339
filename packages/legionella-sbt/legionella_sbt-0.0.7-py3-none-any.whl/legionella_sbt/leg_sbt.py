...
__all__ = ['execute_cmd_and_log', 'setup_logger', 'run_sbt_typing', 'parse_vcf', 'get_border_reads', 'get_positions_from_sam',
           ...
           'find_best_ST_match_to_alleles', 'cli']
...
def run_sbt_typing(
    ...
    ---------------------------------------------------------------------------------------\n"""
        ...
        print(f"SBT profile printet to {sbt_results_file}")
    logging.shutdown()
    return
...
@call_parse
def cli(
    assembly_file: Path = None,  # A name
    r1_file: Path = None,  # A name
    r2_file: Path = None,  # A name
    output_folder: Path = None,
    output_prefix: str = "legionella",
    config_file: Path = None,  # config file to set env vars from
) -> None:
    """
    Run legionella SBT
    """
    resources_dir = core.get_resources_dir("legionella_sbt")
    print(resources_dir)
    ...
    run_sbt_typing(
        ...
        r1_file=r1_file,
        r2_file=r2_file,
        mompS2_reference_fasta=resources_dir.joinpath(
            config["resources"]["mompS2_reference_fasta"]
        ),
        mompS2_allele_fasta=resources_dir.joinpath(
            config["resources"]["mompS2_allele_fasta"]
        ),
        sbt_allele_blast_db=resources_dir.joinpath(
            config["resources"]["sbt_allele_blast_db"]
        ),
        sbt_profiles_tsv=resources_dir.joinpath(
            config["resources"]["sbt_profiles_tsv"]
        ),
