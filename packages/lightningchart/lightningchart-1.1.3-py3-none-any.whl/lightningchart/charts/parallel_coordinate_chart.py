...
class ParallelCoordinateChart(ChartWithSeries, TitleMethods, GeneralMethods):
    ...
    def set_lut(self, axis_key: str, interpolate: bool, steps: list):
        ...
        """
        for step in steps:
            step['color'] = step['color'].get_hex()

        lut_config = {'interpolate': interpolate, 'steps': steps}
        self.instance.send(
            self.id, 'setParallelAxisLUT', {'axisId': axis_key, 'lut': lut_config}
        )
        return self
    ...
    def set_series_stroke_thickness(self, thickness: int | float):
        ...
        """
        self.instance.send(
            self.id, 'setSeriesStrokeThickness', {'thickness': thickness}
        )
        return self
    ...
    def set_unselected_series_color(self, value: lightningchart.Color):
        """Set the color for unselected series.

        Args:
            value (lc.Color): Color to apply to unselected series.

        Returns:
            The instance of the chart for fluent interface.
        """
        self.instance.send(
            self.id, 'setUnselectedSeriesColor', {'color': value.get_hex()}
        )
        return self
...
class ParallelCoordinateAxis(GenericAxis):
    ...
    def add_range_selector(self):
        ...
        return ParallelCoordinateAxisRangeSelector(
            self.chart, self.axis_key, selector_id
        )
    ...
    def set_palette_stroke(
        self, thickness: int | float, interpolate: bool, steps: list
    ):
    ...
    def set_solid_stroke(self, thickness: int | float, color: lightningchart.Color):
        """Set a solid stroke style for the axis.

        Args:
            thickness (int | float): Thickness of the stroke in pixels.
            color (lc.Color): Solid color for the stroke.
        ...
        self.chart.instance.send(
            self.chart.id,
            'setSolidStroke',
            {'axisId': self.axis_key, 'thickness': thickness, 'color': color.get_hex()},
        )
        return self
    ...
    def set_mouse_interaction_range_selectors(self, enabled: bool):
        """Enable or disable mouse interactions for range selectors.
...
class ParallelCoordinateAxisRangeSelector:
    ...
    def set_interval(
        self, a: float, b: float, stop_axis_after: bool = False, animate: bool = False
    ):
    ...
    def set_mouse_interactions(self, state: bool):
        """Enable or disable mouse interactions for the selector.
