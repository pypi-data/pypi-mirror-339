...
class Chart3D(GeneralMethods, TitleMethods, ChartWithXYZAxis, ChartWithSeries):
    ...
    def set_bounding_box(
        self, x: int | float = 1.0, y: int | float = 1.0, z: int | float = 1.0
    ):
    ...
    def add_surface_grid_series(
        ...
        series = SurfaceGridSeries(
            chart=self, columns=columns, rows=rows, data_order=data_order
        )
        self.series_list.append(series)
        return series
    ...
    def add_surface_scrolling_grid_series(
        ...
        series = SurfaceScrollingGridSeries(
            chart=self, columns=columns, rows=rows, scroll_dimension=scroll_dimension
        )
        self.series_list.append(series)
        return series
    ...
    def set_camera_automatic_fitting_enabled(self, enabled: bool):
        ...
        """
        self.instance.send(
            self.id, 'setCameraAutomaticFittingEnabled', {'enabled': enabled}
        )
        return self
