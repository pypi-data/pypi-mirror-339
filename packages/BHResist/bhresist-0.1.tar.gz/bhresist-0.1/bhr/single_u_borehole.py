...
class SingleUBorehole(UTube):
    ...
    def update_beta(self, mass_flow_rate: float, temperature: float) -> float:
        """
        ...
        pipe_resist = self.calc_pipe_resist(mass_flow_rate, temperature)
        self.pipe_resist = pipe_resist
        beta = self.two_pi_kg * pipe_resist

        return beta

    def calc_direct_coupling_resistance(self, mass_flow_rate, temperature) -> tuple:
        r_a = self.calc_total_internal_bh_resistance(mass_flow_rate, temperature)
        r_b = self.calc_local_bh_resistance(mass_flow_rate, temperature)
    ...
    def calc_local_bh_resistance(self, mass_flow_rate, temperature) -> float:
        """
        ...
        """
        beta = self.update_beta(mass_flow_rate, temperature)
    ...
    def calc_total_internal_bh_resistance(self, mass_flow_rate, temperature) -> float:
        """
    ...
    def calc_grout_resistance(self, mass_flow_rate, temperature) -> float:
        """
        ...
        resist_bh_grout = self.calc_local_bh_resistance(mass_flow_rate, temperature) - self.pipe_resist / 2.0
    ...
    def calc_effective_bh_resistance_uhf(self, mass_flow_rate: float, temperature: float) -> float:
        ...
        pt_1 = 1 / (3 * r_a)
        pt_2 = (self.bh_length / (self.fluid.cp(temperature) * mass_flow_rate)) ** 2
    ...
    def calc_effective_bh_resistance_ubwt(self, mass_flow_rate: float, temperature: float) -> float:
        ...
        r_a = self.calc_total_internal_bh_resistance(mass_flow_rate, temperature)  # R_a
        r_b = self.calc_local_bh_resistance(mass_flow_rate, temperature)  # R_b
        r_v = self.bh_length / (mass_flow_rate * self.fluid.cp(temperature))  # (K/(w/m)) thermal resistance factor
        n = r_v / (r_b * r_a) ** 0.5
