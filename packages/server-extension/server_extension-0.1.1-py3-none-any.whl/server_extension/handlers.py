    ...
    async def post(self):
        try:
            ...
            if prompt_type == "summary":
                ...
                print(f"Successfully fetched URL content. Title: {webpage_content['title']}")
            ...
            if current_prompt == prompt_question:
                try:
                    ...
                    if question_type != "conceptual":
                        ...
                        if notebook_code_cells:
                            execution_result = execute_code(
                                "\n\n".join(cell["content"] for cell in notebook_code_cells),
                                notebook_directory
                            )
                    ...
                    self.set_status(500)
                    self.finish(json.dumps({"error": f"Code execution failed: {str(e)}"}))
                    return
                ...
                try:
                    ...
                    traceback.print_exc()
                    self.set_status(500)
                    self.finish(json.dumps({"error": f"Failed to construct prompt: {str(e)}"}))
                    return
            ...
            try:
                ...
                print(f"Question Type: {question_type if prompt_type == 'question' else 'N/A'}")
                print(f"Using Selected Cells: {input_data.get('useSelectedCells', False)}")
...
def setup_handlers(web_app):
    ...
    route_pattern = url_path_join(base_url, "server-extension", "message")
    handlers = [
        (route_pattern, MessageHandler),
    ]
