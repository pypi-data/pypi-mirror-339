...
class RunnerCleanup:
    ...
    def cleanup_all(self) -> dict[str, tuple[float, float, float]]:
        """Perform full cleanup and return before/after disk space stats"""
        print("Cleaning up ...")
        before = self.get_disk_space()
        print(f"Before - Total: {before[0]:.2f}GB, Used: {before[1]:.2f}GB, Free: {before[2]:.2f}GB")

        self.cleanup_docker()
        self.cleanup_temp()

        after = self.get_disk_space()
        print(f"After - Total: {after[0]:.2f}GB, Used: {after[1]:.2f}GB, Free: {after[2]:.2f}GB")
