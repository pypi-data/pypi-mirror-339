...
def run(context, flow_context, project_code, asset_id, company_id, repository_dir, send_to_flow, custom_sca_tags,
    ...
    if not from_ast:
        ...
        params_to_copy = [
            ...
            'experimental', 'asset_name', 'vulnerability_auto_close', 'cleanup'
        ]
...
def perform_command(
        ...
        deploy_id, experimental, from_ast, cleanup
):
    ...
    try:
        ...
        if send_to_flow:
            ...
            if experimental:
                deploy_results_to_conviso_beta(flow_context, conviso_beta_api, results_filepaths, asset_id, company_id)
            else:
