...
class AzureBaseTTSService(TTSService):
    ...
    def __init__(
        ...
        *,
        ...
        params: InputParams = InputParams(),  # For SSML prosody/style settings ONLY
        **kwargs,
    ):
...
class AzureTTSService(AzureBaseTTSService):
    ...
    async def start(self, frame: StartFrame):
        ...
        self._speech_config = SpeechConfig(
            ...
            region=self._region,
            # Set recognition language based on primary language - might not be strictly necessary for synthesis config
            # but aligns with how STT service might be configured.
            speech_recognition_language=self._primary_language,
        )
        # self._speech_config.set_property(PropertyId.Speech_SegmentationSilenceTimeoutMs, "300")
        ...
        # IMPORTANT: Speech synthesis language/voice are set dynamically within run_tts via SSML,
        # so we don't set them globally on the config here.
    ...
    async def clear_azure_audio(self):
        """Flushes the current audio synthesis, stopping any ongoing generation and clearing buffered audio."""
        self.logger.debug("Flushing audio")
        self._clear_audio = True
        if self._speech_synthesizer is not None:
            # Get the ResultFuture
            future = self._speech_synthesizer.stop_speaking_async()

            # Create a coroutine that will run the blocking operation in an executor
            async def wait_for_future_completion():
                # Since ResultFuture.get() is blocking, we should run it in a separate thread
        ...
        # Clear the audio queue to discard any pending chunks
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, StartInterruptionFrame):
            await self.clear_azure_audio()

    async def run_tts(self, text: str) -> AsyncGenerator[Frame, None]:
        text = text.lstrip()
        self.logger.debug(f"Generating TTS: [{text}]")
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(f"Ending TTS:[{text}]")
...
class AzureHttpTTSService(AzureBaseTTSService):
    ...
    async def run_tts(self, text: str) -> AsyncGenerator[Frame, None]:
        ...
        if result.reason == ResultReason.SynthesizingAudioCompleted:
            ...
            self.logger.warning(f"Speech synthesis canceled: {cancellation_details.reason}")
            if cancellation_details.reason == CancellationReason.Error:
                self.logger.error(f"{self} error: {cancellation_details.error_details}")
...
class AzureSTTService(STTService):
    def __init__(
        ...
        **kwargs,
        ...
        # Store primary and additional languages separately for potential use in detection setup
        self._primary_language = language
        self._additional_languages = additional_languages
        # Set the primary language on the config initially
    ...
    async def start(self, frame: StartFrame):
        ...
        if self._additional_languages:
            ...
            # Convert language enum values to Azure language codes
            ...
            # Add additional languages, ensuring no duplicates
            ...
            # Create auto-detect language config with all languages
            self._auto_detect_source_language_config = AutoDetectSourceLanguageConfig(
                languages=all_languages
            )
            # Configure for "at-start" detection
            ...
            # Create standard speech recognizer with primary language only
            self.logger.debug(
        ...
        # Connect events and start recognition
        self._speech_recognizer.recognized.connect(self._on_handle_recognized)
        # self._speech_recognizer.recognizing.connect(self._on_handle_recognizing) # Uncomment if interim results needed
