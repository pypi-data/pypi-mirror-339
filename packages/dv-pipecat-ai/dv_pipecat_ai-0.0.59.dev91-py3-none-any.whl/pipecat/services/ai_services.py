...
class AIService(FrameProcessor):
    ...
    async def _update_settings(self, settings: Mapping[str, Any]):
        ...
        for key, value in settings.items():
            self.logger.debug("Update request for:", key, value)

            if key in self._settings:
                self.logger.info(f"Updating LLM setting {key} to: [{value}]")
                self._settings[key] = value
            elif key in SessionProperties.model_fields:
                self.logger.debug("Attempting to update", key, value)
                ...
                try:
                    ...
                    self.logger.warning(f"Unexpected error updating session property {key}: {e}")
            elif key == "model":
                self.logger.info(f"Updating LLM setting {key} to: [{value}]")
                self.set_model_name(value)
            else:
                self.logger.warning(f"Unknown setting for {self.name} service: {key}")
...
class TTSService(AIService):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(**kwargs)
        self._text_formatter: Optional[Callable[[str], str]] = text_formatter
    ...
    async def _update_settings(self, settings: Mapping[str, Any]):
        for key, value in settings.items():
            if key in self._settings:
                self.logger.info(f"Updating TTS setting {key} to: [{value}]")
                self._settings[key] = value
                if key == "language":
                ...
                self.logger.warning(f"Unknown setting for TTS service: {key}")
...
class STTService(AIService):
    ...
    async def start(self, frame: StartFrame):
        ...
        if "voicemail_detect" in frame.metadata:
