...
#     await weaviate_client_manager.connect()
#     yield
#     # Cleanup connection
#     await weaviate_client_manager.close()
...
@app.post("/pc/inbound_plivo_hangup")
...
async def make_plivo_inbound_call(request: Request):
    try:
        ...
        wss_ngrok_url = ngrok_url.replace("https:", "wss:") + "/pc/ws"
...
async def make_call(request: Request):
    try:
        ...
        if update_call_status_url:
            ...
            answer_url = f"{ngrok_url}/pc/start_call?callback_call_id={encoded_call_id}&max_call_length={max_call_length}&record={record}"
            hangup_url = f"{ngrok_url}/pc/plivo_hangup_callback?update_call_status_url={encoded_update_call_status_url}&callback_call_id={encoded_call_id}"
            ring_url = f"{ngrok_url}/pc/ring_call?update_call_status_url={encoded_update_call_status_url}&callback_call_id={encoded_call_id}"
        else:
            await set_call_config(call_id, call_details.get("call_config"))
            answer_url = (
                f"{ngrok_url}/pc/start_call?callback_call_id={encoded_call_id}&record={record}"
            )
            hangup_url = f"{ngrok_url}/pc/plivo_hangup_callback?callback_call_id={encoded_call_id}"
            ring_url = f"{ngrok_url}/pc/ring_call?callback_call_id={encoded_call_id}"
...
@app.post("/pc/plivo_hangup_callback")
...
@app.get("/pc/transcript_url/{call_id}")
...
async def make_exotel_call(request: Request):  # Renamed function to avoid conflict
    try:
        ...
        payload = {
            ...
            "StatusCallback": f"{ngrok_url}/pc/exotel/callback?update_call_status_url={encoded_update_call_status_url}&callback_call_id={encoded_call_id}",
...
async def handle_sigterm(signum, frame):
    ...
    if not shutting_down:
        ...
        # Wait for active connections to close, respecting K8s grace period (minus some buffer)
        # terminationGracePeriodSeconds is 270 in values.yaml
        grace_period = 260  # Slightly less than terminationGracePeriodSeconds
        start_time = time.time()
        while time.time() - start_time < grace_period:
            if not websocket_connections:
                logger.info("All WebSocket connections closed.")
                break
            logger.info(f"Waiting for {len(websocket_connections)} active connections to close...")
            await asyncio.sleep(5)  # Check every 5 seconds
        else:
            logger.warning(
                f"{len(websocket_connections)} connections did not close within grace period."
            )

        logger.info("Graceful shutdown process complete. Uvicorn will now exit.")
        # Uvicorn should handle the actual exit after this handler returns


if __name__ == "__main__":
    # Register signal handler for graceful shutdown
    signal.signal(
        signal.SIGTERM, lambda signum, frame: asyncio.create_task(handle_sigterm(signum, frame))
    )
    ...
    uvicorn.run(
        ...
        reload=reload,
        workers=workers,
        timeout_graceful_shutdown=10,  # Uvicorn's internal timeout before force kill
    )
