...
from no_llm.config.metadata import ModelMetadata, ModelPricing, PrivacyLevel, TokenPrices
...
def test_registry_model_listing(base_registry):
    ...
    vision_models = list(base_registry.list_models(capabilities={ModelCapability.VISION}))
...
def test_registry_model_privacy_filtering(base_registry):
    ...
    hipaa_models = list(base_registry.list_models(
    ...
    all_mode_models = list(base_registry.list_models(
        privacy_levels=SetFilter({PrivacyLevel.BASIC, PrivacyLevel.HIPAA}, mode='all')
    ))
    ...
    basic_streaming_models = list(base_registry.list_models(
        privacy_levels={PrivacyLevel.BASIC},  # Simple set usage
        capabilities=SetFilter({ModelCapability.STREAMING}, mode='any')  # Explicit SetFilter usage
    ))
...
"""
    custom_config.write_text(override_yaml.strip())  # Remove leading/trailing whitespace
...
def test_registry_model_listing_filters():
    ...
    models = list(registry.list_models(
        privacy_levels=SetFilter({PrivacyLevel.SOC2}, mode="any")
    ))
    assert len(models) == 0
