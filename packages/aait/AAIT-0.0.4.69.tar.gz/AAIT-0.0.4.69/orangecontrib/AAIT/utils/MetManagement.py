...
def download_aait_store_file_or_directory(server_path: str = "", target: str = "") -> None:
    """
    ...
    if not compressed:
        if not is_url(aait_store_remote):
            source=get_aait_store_remote_ressources_path()+file_or_directory
            ...
            if os.name=='nt':
                ...
                try:
                    dialog_progress = SimpleDialogQt.ProgressDialog(title="Synchronsation please wait",content=f"synchronization of the file {source}\nthis operation may take a few minutes")
            ...
            try:
                aait_repo_file.download_from_folder_server(base_url=aait_store_remote,
...
def GetFromRemote(name_to_download):
    ...
    remote_version = get_aait_store_remote_version(get_aait_store_remote_ressources_path())
    ...
    ia_store_path=get_aait_store_remote_ressources_path()
            ...
            if key in element:
                ...
                for subpath in element[key]:
                    ...
                    if not is_url(ia_store_path + "/" + subpath):
                        download_aait_store_file_or_directory(target=subpath)
                    else:
