...
class Application:
    ...
    def read_config_from_file(
        config: google.protobuf.message.Message,
        file: os.PathLike,
    ):
        ...
        with config_file.open(mode="r") as config_stream:
            config_text = config_stream.read()
            return google.protobuf.json_format.Parse(config_text, config, ignore_unknown_fields=True)
    ...
    def write_config_to_file(
        ...
        """
        config_text = google.protobuf.json_format.MessageToJson(
            config,
            always_print_fields_with_no_presence=True,
        )
        config_file = pathlib.Path(file)
        config_file.parent.mkdir(parents=True, exist_ok=True)
        with config_file.open(mode="w") as config_stream:
    ...
    def create_logger(
        ...
        file_name: os.PathLike | None = None,
        file_size: int = 10,
        file_count: int = 30,
        ...
        Args:
            ...
            file_name (Optional[os.PathLike], optional): File name of the log files. Defaults to None.
