...
from .datatypes import Stack, Station
...
def plot_waveform(
    ...
    if ts not in timespans:
        raise ValueError(
            "no data for timespan: %s in [%s-%s]"
            % (ts, timespans[0].start_datetime, timespans[-1].end_datetime)
        )
    ...
    if number_of_channels == 1:
        ...
        plt.title(
            "T\u2080:%s   %s.%s   @%5.3f-%5.2f Hz" % (tr[0].stats.starttime, net, sta, freqmin, freqmax)
        )
...
def plot_substack_all(
    ...
    ax[0].set_title(
        "%s dist:%5.2f km filtered at %4.2f-%4.2fHz" % (sfile.split("/")[-1], dist, freqmin, freqmax)
    )
    ax[0].set_xlabel("time [s]")
...
def plot_all_moveout(
    ...
    if ndata.shape[0] >= 10:
        ...
        ax.imshow(
            ndata,
            cmap="RdBu",
            extent=[-disp_lag, disp_lag, ndist[0], ndist[-1]],
            aspect="auto",
            origin="lower",
        )
