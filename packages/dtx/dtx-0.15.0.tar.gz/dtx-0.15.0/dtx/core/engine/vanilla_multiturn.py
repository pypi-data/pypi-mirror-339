...
from dtx.core.models.evaluator import TypeAndNameBasedEvaluator
...
class VanillaMultiTurnScanner:
    ...
    def _process_prompt(
        ...
        self._update_stats(stats_builder, evaluation_results)
        yield EvalResult(
            run_id=run_id,
            prompt=prompt_with_values,
            evaluation_method=evaluation_method,
            responses=response_evaluation_statuses,
            stats=stats_builder.get_stats(),
        )
    ...
    def _update_stats(self, stats_builder, evaluation_results):
        """
    ...
    def _get_response(
        ...
        try:
            ...
            raise UnknownAgentException(f"Connection error: {str(e)}")
