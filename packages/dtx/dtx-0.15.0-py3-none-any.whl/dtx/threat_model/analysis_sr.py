...
from ..core.models.scope import RedTeamScope
from ..core.repo.plugin import PluginRepo
from .stringray.modules import (
    StingrayModuleManager,
    StringRayModule,
    StringRayThreatModules,
)
...
class AppRiskAnalysis_SR:
    ...
    def analyze(self, scope: RedTeamScope) -> AnalysisResult:
        ...
        max_plugins = self._determine_max_plugins(
            scope.redteam.max_plugin, len(plugin_ids)
        )
    ...
    ) -> Tuple[List[StringRayModule], List[RiskItem]]:
        """
        Collects modules and risks from plugins.
        """
        collected_modules: List[StringRayModule] = []
    ...
    def _create_risk_item(
        self, plugin_id: str, plugin_descriptions: dict, modules: List[StringRayModule]
    ) -> RiskItem:
        """
    ...
    def _build_threat_modules(
        self, modules: List[StringRayModule]
    ) -> StringRayThreatModules:
        """
        Builds a StringRayThreatModules object from a list of modules.
        """
        if not modules:
            return StringRayThreatModules(
                modules=[],
                worst_scenarios=[],
    ...
    def _build_threat_model(
        self, scope: RedTeamScope, threat_modules: StringRayThreatModules
    ) -> ThreatModel:
        """
