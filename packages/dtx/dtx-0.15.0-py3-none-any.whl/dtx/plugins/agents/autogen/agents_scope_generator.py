...
from dtx.core.models.scope import Agent, RedTeamScope
...
1. **Extract Key Information**  
   - Identify the primary **purpose** of the Agent.  
...
2. **Generate AI Agent Attributes**  
   Using the extracted details, structure an AI Agent profile with the following attributes:
   
   - **Name**: Assign a relevant name based on Agent's branding.  
...
async def main(
    ...
    chain = rt_scope_gen.get_chain(Agent)
    agent_details = str(result.messages[-2:-1])
    agent_data: Agent = chain.invoke({"agent_details": agent_details})

    rtscope = RedTeamScopeBuilder().set_agent(agent_data).build()
