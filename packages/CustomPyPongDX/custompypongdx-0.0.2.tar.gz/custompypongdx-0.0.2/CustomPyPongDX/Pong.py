...
def CreatePing(font, colorleft, colorright,
    ...
    while running:
        ...
        # Movement
        if keys[eval("pygame.K_" + controls[1])] and Rect1.bottom <= 480:
            Rect1.y += 7
        if keys[eval("pygame.K_" + controls[0])] and Rect1.bottom >= 100:
            Rect1.y -= 7
        pygame.draw.rect(screen, colorright, Rect2, 0,
                         4)
        if keys[eval("pygame.K_" + controls[3])] and Rect2.bottom <= 480:
            Rect2.y += 7
        if keys[eval("pygame.K_" + controls[1])] and Rect2.bottom >= 100:
            Rect2.y -= 7
