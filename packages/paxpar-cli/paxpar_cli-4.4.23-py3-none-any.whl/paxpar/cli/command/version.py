...
from paxpar.cli.tools import PaxparCLI_ObjCtx, call, call_text_output, root_command_callback
import sys
import yaml
import toml
...
def version_get(
    ...
    branch = call_text_output('git branch --show-current')
    if ctx_obj.verbose:
        print(f'{branch} is the current git branch')
    # see https://semver.org/
    build_prefix = f"+{branch}"if branch is not main_git_branch else ""
