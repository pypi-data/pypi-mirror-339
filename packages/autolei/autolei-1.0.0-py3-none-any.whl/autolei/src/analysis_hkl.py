...
slice_list = [
    ...
    0.87, 0.84, 0.82, 0.80, 0.78, 0.76, 0.74, 0.72, 0.70, 0.68, 0.66, 0.64,
    0.62, 0.60, 0.58, 0.56, 0.54, 0.52, 0.50, 0.48, 0.46, 0.44, 0.42, 0.41,
...
def combine_rules(rules: dict) -> tuple:
    ...
    for rule_key, rule_expression in rules.items():
        rule_expression = rule_expression.replace(", ", " and ")
...
def generate_unique_reflections(refls: list, sg_no: int, uc: list) -> list:
    ...
    for r in unique_reflections_keys:
        ...
        if has_intensity_sigma:
            intensities = unique_reflections_dict[r][0]
            sigmas = unique_reflections_dict[r][1]
            mean_intensity = np.mean(intensities) if intensities else 0.0
            mean_sigma = np.sqrt(np.mean(np.square(sigmas)) / len(sigmas)) if sigmas else 0.0
            results.append((h, k, l, d, intensities, sigmas, mean_intensity, mean_sigma))
        else:
...
def generate_slice_report(ideal_refl: list, refls: list, half1: pd.DataFrame, half2: pd.DataFrame) -> list:
    ...
    for i in range(len(resolution_slices) - 1):
        ...
        half1_slice = half1[(half1['d'] >= high_res) & (half1['d'] < low_res)]
        half2_slice = half2[(half1['d'] >= high_res) & (half1['d'] < low_res)]

        cc12, cc_crit = calculate_cc_half(half1_slice, half2_slice)
...
def accumulate_statistics(refls: list, reso: float, half1: pd.DataFrame, half2: pd.DataFrame) -> tuple:
    ...
    return (len(temp_refls), calculate_mean_i_over_sigma(temp_refls), calculate_r_factors(temp_refls),
            calculate_cc_half(half1[(half1['d'] >= reso)], half2[(half1['d'] >= reso)]))


def calculate_resolution_limit(unique_refls: list, half1: pd.DataFrame, half2: pd.DataFrame, p: float = 0.01) -> float:
    ...
    for i in range(1, len(_list) - 1):
        cc12, cc_crit = calculate_cc_half(half1[(half1['d'] >= _list[i + 1]) & (half1['d'] < _list[i])],
                                          half2[(half1['d'] >= _list[i + 1]) & (half1['d'] < _list[i])], p)
        if cc12 >= cc_crit:
            reso = _list[i + 1]
        elif cc12 < cc_crit:
            break
...
def calculate_cc_half(half1: pd.DataFrame, half2: pd.DataFrame, p_value: float = 0.01) -> tuple:
    """Calculates the correlation coefficient (CC1/2) between two halves of the data.

    Args:
        half1 (pd.DataFrame): Data for the first half of reflections.
        half2 (pd.DataFrame): Data for the second half of reflections.
        p_value (float, optional): p-value threshold for the correlation test. Defaults to 0.01.

    Returns:
        tuple: Correlation coefficient (CC1/2) and critical correlation coefficient.
    """
    ...
    corr_coeff, corr_p_value = pearsonr(mean_I1, mean_I2)
    ...
    return np.round(100 * corr_coeff, 2), np.round(100 * cc_critical, 2)
...
def calculate_mean_i_over_sigma(refls: list) -> float:
    ...
    valid_entries = [
        ...
        for _, _, _, _, intensities, sigmas, mean_intensity, mean_sigma in refls
...
def make_cc_half_list(refls: list) -> tuple:
    ...
    half1, half2 = [], []
    for reflection in refls:
        h, k, l, d, intensities, sigmas, _, _ = reflection
        if len(intensities) >= 2:
            shuffled_indices = np.random.permutation(len(intensities))
            mid = len(shuffled_indices) // 2
            half1_indices = shuffled_indices[:mid]
            half2_indices = shuffled_indices[mid:]
            half1.append([h, k, l, d, np.mean([intensities[i] for i in half1_indices])])
            half2.append([h, k, l, d, np.mean([intensities[i] for i in half2_indices])])

    half1_df = pd.DataFrame(half1, columns=['h', 'k', 'l', 'd', 'I'])
    half2_df = pd.DataFrame(half2, columns=['h', 'k', 'l', 'd', 'I'])

    return half1_df, half2_df
...
def get_unique_ideal_reflection(cell: list, sg: int, resolution: float, rule=None, MM: bool = False) -> list:
    ...
    a, b, c, al, be, ga = cell
    if abs(a - b) < 0.01 and abs(b - c) < 0.01 and abs(al - be) < 0.1 and abs(be - ga) < 0.1:
        is_R = True
    else:
        is_R = False
...
def analysis_xds_hkl(hkl_file: str, merge: bool = False, reso: float = None, output: bool = False, exclude: list = None,
    ...
    df1, df2 = make_cc_half_list(unique_reflections)
    reso_limit = calculate_resolution_limit(unique_reflections, df1, df2)
    ...
    num_reso, isa_reso, r_values, cc12_reso = accumulate_statistics(unique_reflections, reso_limit, df1, df2)
    ...
    slice_report = generate_slice_report(ideal_reflections, unique_reflections, df1, df2)
    if output:
...
def generate_unique_no_d(reflections: np.ndarray, symmetry_ops: list) -> list:
    ...
    for (h, k, l), (intensities, sigmas) in unique_reflections.items():
        if has_intensity_sigma and intensities:
            ...
            mean_sigma = np.sqrt(np.mean(np.square(sigmas)) / len(sigmas))
            unique_list.append((h, k, l, 0, intensities, sigmas, mean_intensity, mean_sigma))
        else:
    ...
    for value in test_list:
        ...
        for sg in bravais_lattice_sg.get(bravais, []):
            ...
            result_list.append({
                ...
                "diff": distance * 20,
                "sg_no": sg,
                "cc12": calculate_cc_half(*make_cc_half_list(unique))[0],
                "bravais_lattice": bravais,
                **value
            })
