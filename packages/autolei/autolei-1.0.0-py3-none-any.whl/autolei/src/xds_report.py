...
This module provides a comprehensive set of functions for processing XDS data, visualizing diffraction patterns,
...
and generating HTML reports summarizing the analysis and visualizations.

Typical usage example:
    # Visualize lattice from an XDS directory
...
Functions:
    ...
    process_data(xds_dir: str, root: Tk) -> None:
        Processes XDS data for visualization by extracting and rotating reflection coordinates.
    ...
    visualise_lattice(xds_dir: str) -> None:
        Initiates the lattice visualization process by setting up the GUI and processing data.

    create_html_file(report_path: str, mode: str) -> str:
        Generates an HTML report summarizing the XDS analysis and visualizations.
...
def parse_xds_inp(fn):
    ...
    return ((float(params["ORGX"][0]), float(params["ORGY"][0])), float(params["OSCILLATION_RANGE"][0]), pixelsize,
            float(params["X-RAY_WAVELENGTH"][0]), omega_current, float(params["STARTING_ANGLE"][0]),
            int(params["STARTING_FRAME"][0]), np.array([rotx, roty, rotz]))


def process_data(xds_dir, root):
    """Processes XDS data for visualization by extracting and rotating reflection coordinates.

    Args:
        xds_dir (str): Directory containing the XDS output files.
        root (Tk): Tkinter root window for embedding the plot.

    Raises:
        FileNotFoundError: If required files (`SPOT.XDS`, `IDXREF.LP`, `XDS.INP`) are missing.
    ...
    try:
        ...
        raise FileNotFoundError(f"The file {spot_xds} was not found.")
    except Exception as e:
        raise Exception(f"An error occurred while reading {spot_xds}: {e}")
    ...
    try:
        beam_center, osc_angle, pixelsize, wavelength, omega_current, starting_angle, starting_frame, rotation_axis = (
            parse_xds_inp(xds_inp))
    except Exception as e:
        raise Exception(f"An error occurred while parsing XDS.INP: {e}")
    ...
    # Pass data to the plot function
    plot_data(X_rot, Y_rot, Z_rot, hkl, intensity, a_star, b_star, c_star, root)


def set_view_direction(ax, a_star):
    """Adjusts the view direction of the 3D plot to align with a specified reciprocal lattice vector.

    Args:
        ax (plt.Axes): The 3D plot axis.
        a_star (np.ndarray): The reciprocal lattice vector to align the view with.
    """
    norm_a_star = a_star / np.linalg.norm(a_star)
    elev = np.degrees(np.arctan2(norm_a_star[2], np.sqrt(norm_a_star[0] ** 2 + norm_a_star[1] ** 2)))
    azim = np.degrees(np.arctan2(norm_a_star[1], norm_a_star[0]))
    ax.view_init(elev=elev, azim=azim)


def plot_data(X_rot, Y_rot, Z_rot, hkl, intensity, a_star, b_star, c_star, root):
    """Generates a 3D scatter plot of rotated reflection coordinates with interactive controls.

    Args:
        X_rot (np.ndarray): Rotated X coordinates.
        Y_rot (np.ndarray): Rotated Y coordinates.
        Z_rot (np.ndarray): Rotated Z coordinates.
        hkl (np.ndarray): Miller indices for each reflection.
        intensity (np.ndarray): Intensity values of reflections.
        a_star (np.ndarray): Reciprocal lattice vector a*.
        b_star (np.ndarray): Reciprocal lattice vector b*.
        c_star (np.ndarray): Reciprocal lattice vector c*.
        root (Tk): Tkinter root window for embedding the plot.
    """
    fig = Figure(figsize=(5, 6.5))
    ax = fig.add_subplot(111, projection='3d', proj_type='ortho')
    button_font = font.Font(family="Liberation Sans", size=16, )
    ...
    def update_plot():
        ...
        if show_intensity:
            sizes = np.sqrt(intensity)
            sizes = 50 * (sizes / np.max(sizes))  # Normalize and scale sizes
        else:
            sizes = 20

        if current_mode == 'indexed' and a_star is not None:
            indexed_mask = (hkl[:, 0] != 0) | (hkl[:, 1] != 0) | (hkl[:, 2] != 0)
            colors = np.where(indexed_mask, 'k', 'none')
        elif current_mode == 'unindexed' and a_star is not None:
            unindexed_mask = (hkl[:, 0] == 0) & (hkl[:, 1] == 0) & (hkl[:, 2] == 0)
            colors = np.where(unindexed_mask, 'r', 'none')
        elif current_mode in ['hk0', 'h0l', '0kl'] and a_star is not None and b_star is not None:
            coords = np.column_stack((X_rot, Y_rot, Z_rot))
            if current_mode == 'hk0':
                distances = distance_to_plane(coords, a_star, b_star)
            elif current_mode == 'h0l':
                distances = distance_to_plane(coords, a_star, c_star)
            else:  # '0kl'
                distances = distance_to_plane(coords, b_star, c_star)
        ...
        ax.scatter(X_rot, Y_rot, Z_rot, c=colors, s=sizes)

        max_range = min(0.6, 1.2 * np.max(np.abs([X_rot, Y_rot, Z_rot])))

        if a_star is not None:
            ax.quiver(0, 0, 0, a_star[0], a_star[1], a_star[2], color='blue', length=2, normalize=False, label='a*')
            ax.quiver(0, 0, 0, b_star[0], b_star[1], b_star[2], color='green', length=2, normalize=False, label='b*')
            ax.quiver(0, 0, 0, c_star[0], c_star[1], c_star[2], color='red', length=2, normalize=False, label='c*')
            # Add text labels at the end of the arrows
            ax.text(a_star[0] * 2, a_star[1] * 2, a_star[2] * 2, 'a*', color='blue')
            ax.text(b_star[0] * 2, b_star[1] * 2, b_star[2] * 2, 'b*', color='green')
            ax.text(c_star[0] * 2, c_star[1] * 2, c_star[2] * 2, 'c*', color='red')
    ...
    # Set up the Tkinter canvas and toolbar
    canvas = FigureCanvasTkAgg(fig, master=root)
    canvas.get_tk_widget().pack(side='top', fill='both', expand=True)  # Fill the window with the canvas
    ...
    def toggle_intensity(show):
        nonlocal show_intensity
        show_intensity = show
        update_plot()

    def set_view_to_vector(vector):
        set_view_direction(ax, vector)
        canvas.draw()

    button_frame_top = tk.Frame(root)
    button_frame_bottom = tk.Frame(root)

    button_frame_top.pack(side='top', fill='x', padx=0, pady=0)  # No padding to remove gaps
    button_frame_bottom.pack(side='top', fill='x', padx=0, pady=0)  # No padding to remove gaps

    # First Line (Left): "All", "Indexed", "Unindexed"
    btn_all = tk.Button(button_frame_top, text='All', command=lambda: set_mode('all'), font=button_font)
    btn_indexed = tk.Button(button_frame_top, text='Indexed', command=lambda: set_mode('indexed'), font=button_font)
    btn_unindexed = tk.Button(button_frame_top, text='Unindexed', command=lambda: set_mode('unindexed'),
                              font=button_font)
    ...
    btn_show_intensity = tk.Button(button_frame_top, text='Show Intensity', command=lambda: toggle_intensity(True),
                                   font=button_font)
    btn_hide_intensity = tk.Button(button_frame_top, text='Hide Intensity', command=lambda: toggle_intensity(False),
                                   font=button_font)
    ...
    btn_hk0 = tk.Button(button_frame_bottom, text='HK0', command=lambda: set_mode('hk0'), font=button_font)
    btn_h0l = tk.Button(button_frame_bottom, text='H0L', command=lambda: set_mode('h0l'), font=button_font)
    btn_0kl = tk.Button(button_frame_bottom, text='0KL', command=lambda: set_mode('0kl'), font=button_font)
    ...
    btn_view_a_star = tk.Button(button_frame_bottom, text='View [100]', command=lambda: set_view_to_vector(a_star),
                                font=button_font)
    btn_view_b_star = tk.Button(button_frame_bottom, text='View [010]', command=lambda: set_view_to_vector(b_star),
                                font=button_font)
    btn_view_c_star = tk.Button(button_frame_bottom, text='View [001]', command=lambda: set_view_to_vector(c_star),
                                font=button_font)
...
def visualise_lattice(xds_dir):
    ...
    """
    root = Tk()
    root.title("Lattice Visualization")

    # Run data processing and plotting in a separate thread to avoid blocking the GUI
    threading.Thread(target=process_data, args=(xds_dir, root)).start()
...
def create_plotly_figure_cc12(df):
    ...
    fig.add_trace(go.Scatter(
        ...
        y=df["CC_crit"],
        mode='lines',
        name='CC<sub>1/2</sub> crit. (p=0.01)',
        hovertemplate='%{y}',
...
def create_plotly_figure_reso(plot_type, df):
    ...
    if plot_type == 'isa':
        x_data = df["high_res"]
        y_data = df["Isa_meas"]
        title = 'I/Sa vs Resolution'
        yaxis_title = 'ISa_meas'
