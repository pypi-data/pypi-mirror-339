...
if __name__ == "__main__":
    ...
    table_name = os.path.basename(os.path.abspath(sys.argv[0])).strip(".py")
    config = SparkSinkConfig(
        kafka_broker="redpanda:29092",
        s3_endpoint="http://host.docker.internal:9000",
        schema_registry_url="http://schema-registry.de.data.snapp.tech:8081",
        kafka_topic="dbz.snapp_backoffice_mysql.backoffice.tags",
        table_path=f"s3a://{table_name}-refactored-bucket2/{table_name}",
        checkpoint_path=f"s3a://{table_name}-refactored-bucket2/checkpoints/{table_name}",
        hudi_options={
    ...
    def run_batch(batch_df: DataFrame, batch_id: int):
        ...
        connector.get_logger().info(f"✅ Data written to {config.table_path}  Hudi.")
