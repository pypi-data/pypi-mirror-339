...
class SparkSinkConnector:
    ...
    def foreach_batch(self, foreach_batch_fn: Optional[Callable[[DataFrame, int], None]] = None) -> 'SparkSinkConnector':
        """
    ...
    # def write_delta_to_s3(self, partition_key: str = None, output_mode: ConnectorOutputMode = None) -> 'SparkSinkConnector':
    #     """
    ...
    #             .option("path", self.config.table_path)
    #
    #     self.logger.info(f"✅ Configured Delta write to {self.config.table_path}")
    #     return self
    #
    # def write_hudi_to_s3(self,
    #                      partition_key: str = None,
    #                      output_mode: ConnectorOutputMode = None,
    #                      hudi_options: dict[str,str] = None) -> 'SparkSinkConnector':
    #     """
    ...
    #         self.writer = self.writer.trigger(availableNow=True)
    #     else:
    #         self.writer = self.dataframe.write.format("hudi")
    #         if partition_key is not None:
    #             self.writer = self.writer.partitionBy(partition_key)
    #         if hudi_options is not None:
    #             self.writer = self.writer.options(**hudi_options)
    #         if output_mode is not None:
    #             self.writer = self.writer.mode(output_mode)
    #         self.writer.save(self.config.table_path)
    #         self.logger.info(f"✅ Data written to {self.config.table_path} in Hudi format.")
    #
    #     return self
    ...
    def write_to_s3(self,
                    ...
                    partition_key: str = None,
                    output_mode: ConnectorOutputMode = None) -> 'SparkSinkConnector':
        """
        Configure writing data to S3 in Hudi format.
        ...
        if partition_key is not None:
            self.writer = self.writer.partitionBy(partition_key)
        if self.config.open_table_format_options is not None:
            self.writer = self.writer.options(**self.config.open_table_format_options)
        if self.config.checkpoint_path is not None:
            self.writer = self.writer.option("checkpointLocation", self.config.checkpoint_path)
        if output_mode is not None:
            self.writer = self.writer.mode(output_mode)
