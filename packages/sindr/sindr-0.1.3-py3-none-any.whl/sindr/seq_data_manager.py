        ...
        if base_folder:
            ...
            for sample, files in file_paths.items():
                try:
                    ...
                    print(
                        f"Failed to decipher assembly file path: {files['assembly_file']}. {e}"
                    )
                    files["assembly_file"] = None
                try:
                    files["r1_file"] = base_folder.joinpath(files["r1_file"]).resolve()
                except TypeError as e:
                    print(f"Failed to decipher r1 file path: {files['r1_file']}. {e}")
                    files["r1_file"] = None
                try:
                    files["r2_file"] = base_folder.joinpath(files["r2_file"]).resolve()
                except TypeError as e:
                    print(f"Failed to decipher r2 file path: {files['r2_file']}. {e}")
                    files["r2_file"] = None
