...
class ReactionTelegram:
    def __init__(self, link):
        ...
        self.thread_count = 1
        self.base_url = "http://46.202.135.52:2001/telegram-r/link="
        self.console = Console()
        if sakms.platform == 'win32':
            from colorama import init
            init()

    def __getitem__(self, keakm):
        if keakm == 'while=True':
            self.while_loop = True
            return self
        elif keakm.startswith('threading='):
            self.thread_count = int(keakm.split('=')[1])
            return self
        return self
