...
class RS2Test(TestCase):
    def test_get_correct_project(self):
        ...
        self.assertEqual([project.id], get_project_ids_to_charge_for_user(user, start, end))
        user.projects.add(project2)
        project.active = False
        project.save()
        # two projects, first active one returned
        self.assertEqual([project2.id], get_project_ids_to_charge_for_user(user, start, end))
        project.active = True
        project.save()
        ...
        self.assertEqual([project.id, project3.id], get_project_ids_to_charge_for_user(user, start, end))
