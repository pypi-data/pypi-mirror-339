...
class FakeDaikinServer:
    ...
    def _setup_brp069_routes(self):
        ...
        self.app.router.add_get(
            '/aircon/get_sensor_info', self._handle_brp069_sensor_info
        )
        self.app.router.add_get(
            '/aircon/get_control_info', self._handle_brp069_control_info
        )

    def _setup_airbase_routes(self):
        """Set up routes for AirBase device."""
        self.app.router.add_get(
            '/skyfi/common/basic_info', self._handle_airbase_basic_info
        )
        self.app.router.add_get(
            '/skyfi/aircon/get_control_info', self._handle_airbase_control_info
        )
        self.app.router.add_get(
            '/skyfi/aircon/get_model_info', self._handle_airbase_model_info
        )
        self.app.router.add_get(
            '/skyfi/aircon/get_sensor_info', self._handle_airbase_sensor_info
        )
        self.app.router.add_get(
            '/skyfi/aircon/get_zone_setting', self._handle_airbase_zone_setting
        )
    ...
    async def _handle_brp069_basic_info(self, request):
        ...
        return web.Response(
            text="ret=OK,type=aircon,reg=eu,dst=1,ver=1_2_54,rev=203DE8C,pow=1,err=0,location=0,name=%4e%6f%74%74%65,icon=3,method=home only,port=30050,id=,pw=,lpw_flag=0,adp_kind=3,pv=3.20,cpv=3,cpv_minor=20,led=1,en_setzone=1,mac=409F38D107AC,adp_mode=run,en_hol=0,ssid1=Mock Device,radio1=-35,grp_name=,en_grp=0"
        )
    ...
    async def _handle_brp069_control_info(self, request):
        ...
        return web.Response(
            text="ret=OK,pow=1,mode=3,stemp=25.0,shum=0,f_rate=A,f_dir=0"
        )
    ...
    async def _handle_airbase_control_info(self, request):
        ...
        return web.Response(
            text="ret=OK,pow=1,mode=2,adv=,stemp=M,shum=50,f_rate=A,f_dir=0,f_auto=1"
        )

    async def _handle_airbase_model_info(self, request):
        """Handle AirBase model_info request."""
        return web.Response(
            text="ret=OK,model=NOTSUPPORT,type=N,pv=3.20,cpv=3,cpv_minor=20,mid=NA,humd=0,s_humd=0,acled=0,land=0,elec=1,temp=1,temp_rng=0,m_dtct=1,ac_dst=--,disp_dry=0,dmnd=1,en_scdltmr=1,en_frate=1,en_fdir=1,s_fdir=3,en_rtemp_a=0,en_spmode=7,en_ipw_sep=1,en_mompow=0,hmlmt_l=10.0"
        )
    ...
    async def _handle_skyfi_zones_cgi(self, request):
        ...
        return web.Response(
            text="nz=8&zone1=Zone%201&zone2=Zone%202&zone3=Zone%203&zone4=Zone%204&zone5=Zone%205&zone6=Zone%206&zone7=Zone%207&zone8=Zone%208"
        )
