...
class Theme:
    ...
    def __init__(
        self, term_fg: Color, term_bg: Color, term_palette: list[Optional[Color]]
    ):
...
def _print_welcome_message(theme: Theme) -> None:
    print("╭─────────────────────────────────╮")
    print(
        f"│ ✨ Welcome to {bold_seq()}{fg_color_seq(theme.exponent_green)}Exponent \x1b[4mSHELL{reset_attrs_seq()} ✨ │"
    )
...
class LiveView(BaseView):
    ...
    def _handle_command_event(self, event: dict[str, Any]) -> None:
        ...
        seqs: list[Any] = []
        command_name = (
            data["commandName"] if data["type"] == "PROTOTYPE" else data["type"].lower()
        )

        if self.buffer.event_uuid != event_uuid:
            self.buffer = CommandBuffer(event_uuid)

            if command_name == "file_read":
                path = data["filePath"]
                seqs.append(self._render_command_block_start(f"Read file: {path}"))
            elif command_name == "search_files":
                regex = data["contentJson"]["regex"]
                seqs.append(
                    self._render_command_block_start(f"Search file content: {regex}")
                )
            elif command_name == "glob":
                glob = data["contentJson"]["glob"]
                seqs.append(
                    self._render_command_block_start(f"Search file names: {glob}")
                )
            else:
        ...
        if command_name not in ["file_read", "search_files", "glob"]:
            formatted_content = self._render_command_block_content(data)
        ...
        if event["requireConfirmation"]:
            if command_name in ["file_read", "search_files", "glob"]:
                seqs.append(
                    [
                        self._render_block_footer(
                            f"Confirm with {bold_seq()}<ctrl+y>{not_bold_seq()}, "
                            "send a new message to cancel."
                        ),
                        "\n",
                    ]
                )
                ...
                seqs.append(
                    [
                        self._render_block_footer(
                            f"Confirm {bold_seq()}{type_}{not_bold_seq()} command with {bold_seq()}<ctrl+y>{not_bold_seq()}, "
                            "send a new message to cancel."
                        ),
                        "\n",
                    ]
...
class Shell:
    ...
    def _confirm_execution(self) -> bool:
        ...
        success = self.chat.pending_event is not None
        self._ensure_connected()
...
def fg_color_seq(c: Union[int, Color]) -> str:
    if isinstance(c, int):
        return f"\x1b[{30 + c}m"
    else:
        if TRUE_COLOR:
...
def bg_color_seq(c: Union[int, Color]) -> str:
    if isinstance(c, int):
        return f"\x1b[{40 + c}m"
    else:
        if TRUE_COLOR:
...
def get_term_colors() -> tuple[Color, Color, list[Optional[Color]]]:
    fg = DEFAULT_TERM_PALETTE[7]
    bg = DEFAULT_TERM_PALETTE[0]
    palette: list[Optional[Color]] = [None, None, None, None, None, None, None, None]

    if not POSIX_TERMINAL or not sys.stdin.isatty():
        return (fg, bg, palette)  # Not supported on Windows or when stdin is not a TTY
    ...
    with RawMode(sys.stdin.fileno()):
        ...
        if stdout_fd in select.select([], [stdout_fd], [])[1]:
            ...
            #   CSI c - query device attributes (DA)
            # os.write(stdout_fd, b"\x1b]10;?\x07\x1b]11;?\x07\x1b[c")
            os.write(
                stdout_fd,
            ...
            # If the terminal doesn't support the color query then adding the
            # device attributes (DA) query forces it to respond anyway (it's
            # widely supported), which let's us check if the response includes
            # the colors without timing out.
