...
def _get_upgrade_command_str(version: str) -> str:
    ...
    return f'{sys.executable} -m pip install --upgrade "exponent-run=={version}"'
...
def upgrade_exponent(
    *,
    ...
    result = subprocess.run(upgrade_command, capture_output=True, text=True)
...
def upgrade_exponent_in_background(
    ...
    click.secho(
        f"\nUpgrading Exponent from {current_version} to {new_version}\n",
        fg="cyan",
        bold=True,
    )
