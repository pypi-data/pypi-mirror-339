...
def create_project_structure(project_name):
    """Create a base Python project structure"""
    base_path = Path(project_name)
    base_path.mkdir(parents=True, exist_ok=True)
    
    # Create standard Python project directories
...
def create_kotlin_project_structure(project_name):
    ...
    # Create build.gradle.kts files with more realistic content
...
"""
    (app_path / "build.gradle.kts").write_text(app_build_gradle_content)
...
"""
    (core_path / "build.gradle.kts").write_text(core_build_gradle_content)
...
include(":core")
"""
    (base_path / "settings.gradle.kts").write_text(settings_gradle_content)
...
"""
    (base_path / "build.gradle.kts").write_text(root_build_gradle_content)
...
"""
    ...
    # Update build.gradle.kts with coroutines dependencies
    app_gradle_path = base_path / "app" / "build.gradle.kts"
...
@click.group()
def cli():
    """Kotlin CLI - Tool for Kotlin project management"""
    pass

@cli.group()
def create():
    """Create different types of Kotlin project structures"""
    pass
...
@cli.command()
def version():
    """Display the version of the creator tool"""
    from . import __version__
    click.echo(f"Kotlin creator version: {__version__}")
