...
def test_tiled_mm_swiglu(dims):
    ...
    with torch.no_grad():
        ...
        out_koi = torch.zeros((M // 16, N // 16, 16, 8), device=dev, dtype=dtype)
        ...
        input = input.view((M // 16, 16, K // 8, 8)).transpose(1, 2).contiguous()
        weights = (
            weights.view((N // 32, 16, 2, K // 8, 8))
            .permute((0, 2, 3, 1, 4))
            .contiguous()
            .view((N // 16, K // 8, 16, 8))
        )
        ...
        out_koi2 = out_koi.permute((0, 2, 1, 3)).contiguous().view((M, N // 2))
