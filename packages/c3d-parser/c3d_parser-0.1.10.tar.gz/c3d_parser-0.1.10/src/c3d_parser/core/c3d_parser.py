...
# TODO: Talk to Britney and Elyse about the missing PROCESSING sections in their cropped static trials.
#   May need to adjust when the pop-up is called.
#   May also need to create an additional pop-up in the medial marker method that asks for missing knee-width values.
#   NOTE: a pop-up in that function will also have issues with the exec thread...
def extract_static_data(file_path):
    with open(file_path, 'rb') as handle:
        ...
        # if processing_group is not None and 'BODYMASS' in processing_group and 'HEIGHT' in processing_group:
        ...
        # if processing_group is not None and  'LKNEEWIDTH' in processing_group and 'RKNEEWIDTH' in processing_group:
