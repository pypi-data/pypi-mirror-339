# coding=utf-8
# *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import copy
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from . import _utilities

__all__ = ['NsxtGlobalDefaultSegmentProfileTemplateArgs', 'NsxtGlobalDefaultSegmentProfileTemplate']

@pulumi.input_type
class NsxtGlobalDefaultSegmentProfileTemplateArgs:
    def __init__(__self__, *,
                 vapp_networks_default_segment_profile_template_id: Optional[pulumi.Input[str]] = None,
                 vdc_networks_default_segment_profile_template_id: Optional[pulumi.Input[str]] = None):
        """
        The set of arguments for constructing a NsxtGlobalDefaultSegmentProfileTemplate resource.
        :param pulumi.Input[str] vapp_networks_default_segment_profile_template_id: Global Default Segment Profile
               Template ID for all vApp Networks
        :param pulumi.Input[str] vdc_networks_default_segment_profile_template_id: Global Default Segment Profile
               Template ID for all VDC Networks
        """
        if vapp_networks_default_segment_profile_template_id is not None:
            pulumi.set(__self__, "vapp_networks_default_segment_profile_template_id", vapp_networks_default_segment_profile_template_id)
        if vdc_networks_default_segment_profile_template_id is not None:
            pulumi.set(__self__, "vdc_networks_default_segment_profile_template_id", vdc_networks_default_segment_profile_template_id)

    @property
    @pulumi.getter(name="vappNetworksDefaultSegmentProfileTemplateId")
    def vapp_networks_default_segment_profile_template_id(self) -> Optional[pulumi.Input[str]]:
        """
        Global Default Segment Profile
        Template ID for all vApp Networks
        """
        return pulumi.get(self, "vapp_networks_default_segment_profile_template_id")

    @vapp_networks_default_segment_profile_template_id.setter
    def vapp_networks_default_segment_profile_template_id(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "vapp_networks_default_segment_profile_template_id", value)

    @property
    @pulumi.getter(name="vdcNetworksDefaultSegmentProfileTemplateId")
    def vdc_networks_default_segment_profile_template_id(self) -> Optional[pulumi.Input[str]]:
        """
        Global Default Segment Profile
        Template ID for all VDC Networks
        """
        return pulumi.get(self, "vdc_networks_default_segment_profile_template_id")

    @vdc_networks_default_segment_profile_template_id.setter
    def vdc_networks_default_segment_profile_template_id(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "vdc_networks_default_segment_profile_template_id", value)


@pulumi.input_type
class _NsxtGlobalDefaultSegmentProfileTemplateState:
    def __init__(__self__, *,
                 vapp_networks_default_segment_profile_template_id: Optional[pulumi.Input[str]] = None,
                 vdc_networks_default_segment_profile_template_id: Optional[pulumi.Input[str]] = None):
        """
        Input properties used for looking up and filtering NsxtGlobalDefaultSegmentProfileTemplate resources.
        :param pulumi.Input[str] vapp_networks_default_segment_profile_template_id: Global Default Segment Profile
               Template ID for all vApp Networks
        :param pulumi.Input[str] vdc_networks_default_segment_profile_template_id: Global Default Segment Profile
               Template ID for all VDC Networks
        """
        if vapp_networks_default_segment_profile_template_id is not None:
            pulumi.set(__self__, "vapp_networks_default_segment_profile_template_id", vapp_networks_default_segment_profile_template_id)
        if vdc_networks_default_segment_profile_template_id is not None:
            pulumi.set(__self__, "vdc_networks_default_segment_profile_template_id", vdc_networks_default_segment_profile_template_id)

    @property
    @pulumi.getter(name="vappNetworksDefaultSegmentProfileTemplateId")
    def vapp_networks_default_segment_profile_template_id(self) -> Optional[pulumi.Input[str]]:
        """
        Global Default Segment Profile
        Template ID for all vApp Networks
        """
        return pulumi.get(self, "vapp_networks_default_segment_profile_template_id")

    @vapp_networks_default_segment_profile_template_id.setter
    def vapp_networks_default_segment_profile_template_id(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "vapp_networks_default_segment_profile_template_id", value)

    @property
    @pulumi.getter(name="vdcNetworksDefaultSegmentProfileTemplateId")
    def vdc_networks_default_segment_profile_template_id(self) -> Optional[pulumi.Input[str]]:
        """
        Global Default Segment Profile
        Template ID for all VDC Networks
        """
        return pulumi.get(self, "vdc_networks_default_segment_profile_template_id")

    @vdc_networks_default_segment_profile_template_id.setter
    def vdc_networks_default_segment_profile_template_id(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "vdc_networks_default_segment_profile_template_id", value)


class NsxtGlobalDefaultSegmentProfileTemplate(pulumi.CustomResource):
    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 vapp_networks_default_segment_profile_template_id: Optional[pulumi.Input[str]] = None,
                 vdc_networks_default_segment_profile_template_id: Optional[pulumi.Input[str]] = None,
                 __props__=None):
        """
        Create a NsxtGlobalDefaultSegmentProfileTemplate resource with the given unique name, props, and options.
        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[str] vapp_networks_default_segment_profile_template_id: Global Default Segment Profile
               Template ID for all vApp Networks
        :param pulumi.Input[str] vdc_networks_default_segment_profile_template_id: Global Default Segment Profile
               Template ID for all VDC Networks
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: Optional[NsxtGlobalDefaultSegmentProfileTemplateArgs] = None,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        Create a NsxtGlobalDefaultSegmentProfileTemplate resource with the given unique name, props, and options.
        :param str resource_name: The name of the resource.
        :param NsxtGlobalDefaultSegmentProfileTemplateArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(NsxtGlobalDefaultSegmentProfileTemplateArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 vapp_networks_default_segment_profile_template_id: Optional[pulumi.Input[str]] = None,
                 vdc_networks_default_segment_profile_template_id: Optional[pulumi.Input[str]] = None,
                 __props__=None):
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = NsxtGlobalDefaultSegmentProfileTemplateArgs.__new__(NsxtGlobalDefaultSegmentProfileTemplateArgs)

            __props__.__dict__["vapp_networks_default_segment_profile_template_id"] = vapp_networks_default_segment_profile_template_id
            __props__.__dict__["vdc_networks_default_segment_profile_template_id"] = vdc_networks_default_segment_profile_template_id
        super(NsxtGlobalDefaultSegmentProfileTemplate, __self__).__init__(
            'vcd:index/nsxtGlobalDefaultSegmentProfileTemplate:NsxtGlobalDefaultSegmentProfileTemplate',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None,
            vapp_networks_default_segment_profile_template_id: Optional[pulumi.Input[str]] = None,
            vdc_networks_default_segment_profile_template_id: Optional[pulumi.Input[str]] = None) -> 'NsxtGlobalDefaultSegmentProfileTemplate':
        """
        Get an existing NsxtGlobalDefaultSegmentProfileTemplate resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[str] vapp_networks_default_segment_profile_template_id: Global Default Segment Profile
               Template ID for all vApp Networks
        :param pulumi.Input[str] vdc_networks_default_segment_profile_template_id: Global Default Segment Profile
               Template ID for all VDC Networks
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _NsxtGlobalDefaultSegmentProfileTemplateState.__new__(_NsxtGlobalDefaultSegmentProfileTemplateState)

        __props__.__dict__["vapp_networks_default_segment_profile_template_id"] = vapp_networks_default_segment_profile_template_id
        __props__.__dict__["vdc_networks_default_segment_profile_template_id"] = vdc_networks_default_segment_profile_template_id
        return NsxtGlobalDefaultSegmentProfileTemplate(resource_name, opts=opts, __props__=__props__)

    @property
    @pulumi.getter(name="vappNetworksDefaultSegmentProfileTemplateId")
    def vapp_networks_default_segment_profile_template_id(self) -> pulumi.Output[Optional[str]]:
        """
        Global Default Segment Profile
        Template ID for all vApp Networks
        """
        return pulumi.get(self, "vapp_networks_default_segment_profile_template_id")

    @property
    @pulumi.getter(name="vdcNetworksDefaultSegmentProfileTemplateId")
    def vdc_networks_default_segment_profile_template_id(self) -> pulumi.Output[Optional[str]]:
        """
        Global Default Segment Profile
        Template ID for all VDC Networks
        """
        return pulumi.get(self, "vdc_networks_default_segment_profile_template_id")

