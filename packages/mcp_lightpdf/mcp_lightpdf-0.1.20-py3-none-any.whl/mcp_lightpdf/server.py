...
def generate_report(results: List[ConversionResult], is_watermark_removal: bool = False) -> str:
    """生成转换报告
    ...
    # 生成报告头部，根据任务类型显示不同文案
    task_type = "水印去除" if is_watermark_removal else "转换"
    report_lines = [
    ...
    # 添加成功的文件信息
    if success_count > 0:
        success_action = "去除水印成功" if is_watermark_removal else "转换成功"
    ...
    # 添加失败的文件信息
    if failed_count > 0:
        failed_action = "水印去除失败" if is_watermark_removal else "转换失败"
...
async def process_files(file_paths: List[str], format: str, logger: Logger, converter: Converter) -> List[ConversionResult]:
    """处理文件转换
    ...
    if len(file_paths) > 1:
        operation_desc = "批量去除水印" if is_watermark_removal else f"批量转换为 {format} 格式"
...
async def handle_call_tool(name: str, arguments: dict | None) -> list[types.TextContent | types.ImageContent | types.EmbeddedResource]:
    ...
    if name == "convert_document":
        ...
        # 生成报告
        report_msg = generate_report(results, is_watermark_removal=False)
        ...
        # 处理去除水印，直接传递doc-repair格式
        results = await process_files(file_paths, "doc-repair", logger, converter)
        
        # 生成报告，标记为水印去除任务
        report_msg = generate_report(results, is_watermark_removal=True)
