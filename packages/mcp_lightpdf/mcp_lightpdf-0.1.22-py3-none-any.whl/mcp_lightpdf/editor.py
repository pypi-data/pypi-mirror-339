...
class Editor(BaseApiClient):
    ...
    async def split_pdf(self, file_path: str, pages: str = "all", password: Optional[str] = None, split_type: str = "page", merge_all: int = 1) -> EditResult:
        """拆分PDF文件
        
        Args:
            file_path: 要拆分的PDF文件路径
            pages: 拆分页面规则，例如 "1,3,5-7" 表示提取第1,3,5,6,7页，"all"表示所有页面，默认为"all"
    ...
    async def rotate_pdf(self, file_path: str, angle: Dict[str, str], password: Optional[str] = None) -> EditResult:
        """旋转PDF文件的页面
        
        Args:
            file_path: 要旋转的PDF文件路径
            angle: 旋转参数，格式为{旋转角度(90/180/270): 页面范围}
                  例如 {"90": "1-5"} 表示将第1-5页旋转90度
                  页面范围为空字符串表示所有页面
            password: 文档密码，如果文档受密码保护，则需要提供（可选）
        ...
        if not angle or not any(key in valid_angles for key in angle.keys()):
            await self.logger.error("无效的旋转参数。角度必须是: 90, 180, 270")
            return EditResult(success=False, file_path=file_path, error_message="无效的旋转参数。角度必须是: 90, 180, 270")
        ...
        # 记录操作描述
        angle_desc = ", ".join([f"{k}°: {v or '所有页面'}" for k, v in angle.items()])
    ...
    async def add_watermark(
        ...
        range: str = "all",  # 与API保持一致，使用range而非pages
        ...
        Args:
            ...
            range: 指定页面范围，例如 "1,3,5-7" 或 "all" 表示所有页面
