...
class Converter:
    ...
    async def convert_file(self, file_path: str, format: str, extra_params: dict = None) -> ConversionResult:
        ...
        # 如果是去除水印操作，检查是否PDF文件
        if extra_params and extra_params.get("remove_watermark") and input_format != InputFormat.PDF:
        ...
        if output_format not in available_formats:
            # 特殊情况：如果是去除水印，输入和输出都是PDF，则允许
            if not (extra_params and extra_params.get("remove_watermark") and 
                   input_format == InputFormat.PDF and output_format == OutputFormat.PDF):
        ...
        output_path = self.file_handler.get_unique_output_path(file_path, format, output_dir)

        operation_desc = "去除水印" if extra_params and extra_params.get("remove_watermark") else f"将 {input_format.value.upper()} 转换为 {output_format.value.upper()} 格式"
