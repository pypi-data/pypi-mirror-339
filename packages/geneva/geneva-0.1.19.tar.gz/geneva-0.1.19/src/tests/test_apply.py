...
def test_create_plan(tmp_path: Path) -> None:
    ...
    plans = list(plan_read(tbl.uri, ["a"], batch_size=16))
    assert len(plans) == 1
    plan = plans[0]
...
def test_create_plan_with_diverse_shuffle(tmp_path: Path) -> None:
    ...
    plans = list(plan_read(ds.uri, ["a"], batch_size=1, task_shuffle_diversity=4))
...
def test_plan_with_filter(tmp_path: Path) -> None:
    ...
    tasks = list(plan_read(tbl.uri, ["a"], filter="a > 100 AND a % 2 == 0"))
    assert len(tasks) == 3
...
def test_plan_with_row_address(tmp_path: Path) -> None:
    ...
    tasks = list(plan_read(tbl.uri, ["a"]))
    assert len(tasks) == 1
...
def test_udf_with_arrow_params(tmp_path: Path) -> None:
    ...
    key = applier.run(next(plan_read(tbl.uri, ["a", "b"], batch_size=16)))
    batch = store[key]
