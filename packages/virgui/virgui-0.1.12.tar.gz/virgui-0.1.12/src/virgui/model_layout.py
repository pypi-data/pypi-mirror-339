...
class ModelLayout(QtWidgets.QWidget):
    ...
    def _make_node_overview_graphviz(
        ...
        network = virgui.GLOBAL_MODEL.optical_network
        # use the undirected network to make it easier to retrieve node neighbors
        undirected_network = virgui.GLOBAL_MODEL.optical_network.to_undirected()
        # select all nodes that are part of the component or directly connnected to it
        comp_plus_nbs = set()

        comp_nodes = [
            node for node in network.nodes if str(node).startswith(f"{comp.name}.")
        ]
        for node in comp_nodes:
            comp_plus_nbs.add(node)
            comp_plus_nbs = comp_plus_nbs.union(undirected_network.neighbors(node))
        # convert to pygraphviz graph for better rendering
        A: pygraphviz.AGraph = nx.drawing.nx_agraph.to_agraph(
            network.subgraph(comp_plus_nbs)
        )
        ...
        # we put all the component nodes in a subgraph cluster so we can draw a box
        # around them. Name needs to start with cluster!!
        A.add_subgraph(
            comp_nodes,
        ...
        # horizontal layout fits better in UI
        A.graph_attr["rankdir"] = "LR"
        path = tempfile.NamedTemporaryFile(suffix=".svg", delete=False).name
        A.draw(path=path, format="svg", prog="dot")
        svg_item = QtSvgWidgets.QGraphicsSvgItem(path)
        return svg_item
