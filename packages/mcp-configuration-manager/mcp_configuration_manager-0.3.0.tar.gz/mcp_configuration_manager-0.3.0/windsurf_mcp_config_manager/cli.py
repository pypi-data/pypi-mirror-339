...
# Constants
DEFAULT_CONFIG_DIR = os.path.expanduser("~/.codeium/mcm")
...
@click.pass_context
def add(ctx, name, server_url, url, package, pipx, npx, cursor, proxy, command, args):
...
@click.option('--command', help='Command to run for proxy servers (Windsurf only)')
@click.option('--args', help='Arguments for the command (Windsurf only)')
@click.option('--env', help='Environment variables in KEY=VALUE format (Windsurf only)')
@click.option('--cursor', is_flag=True, help='Configure for Cursor')
@click.pass_context
def update(ctx, name, server_url, url, command, args, env, cursor):
            ...
            if env:
                try:
                    ...
                    if "env" not in server:
                        server["env"] = {}
                    server["env"][key.strip()] = value.strip()
                except ValueError:
                    click.echo(f"Warning: Invalid environment variable format: {env}")
