...
from kyutil.config import BUILD_PATH, SIG_KEY, FILE_SCHEMA, REPODATA_PATH, APACHE_READ_MODEL, SENSITIVE_KEYWORDS, HOST_IP
from kyutil.config import SOURCE_PATH, PYTHON_MINOR_CMD
...
class ReleaseOSBase(ReleaseDependency):
    ...
    def __init__(self, **kwargs):
        ...
        self.build_logger = zero_log("build", self.build_logfile)
    ...
    def init_mock_env(self):
        """初始化mock环境"""
        self.process = 25
        self._update_status("初始化mock构建环境", self.process, None)
        self.mock_cfg_file = generate_mock_config(self.params, self.build_mocktag, self.build_logger, config_root_dir=f"{SOURCE_PATH}/src/build/mock/")
        if self.params.get('pungi_url') or self.params.get('lorax_url'):
            cmd = f"sh {SOURCE_PATH}/src/build/mock/mock_.sh  {self.lorax_templates_url} {self.build_mocktag} " \
