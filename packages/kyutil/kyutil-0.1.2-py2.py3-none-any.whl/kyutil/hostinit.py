...
from kyutil.base import ok_data, error_out_put
...
def host_init(ip_addr, remote_user, remote_user_password, task, queue, host_type, server, ansible_root_path="/opt/ctdy/build_iso_server/src/app/utils/"):
    """
    ip_addr:         待执行的目标主机的 IP地址
    remote_user:     目标主机的用户名
    remotePassword: 目标主机的密码（有root权限）或普通用户的sudo密码
    task:           任务类型
    queue:          队列类型
    host_type:      主机类型
    server:         测试环境/正式环境
    ...
    if rc == 0:
        return ok_data(code=0, msg="Playbook执行成功")
    else:
        return error_out_put(code=70002, msg="Playbook执行失败")
