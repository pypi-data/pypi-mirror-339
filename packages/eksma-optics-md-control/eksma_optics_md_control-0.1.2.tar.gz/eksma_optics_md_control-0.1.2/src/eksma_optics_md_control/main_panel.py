...
import wx
import wx.dataview
from eksma_optics_motorized_devices.command import IdentificationValue, PresetFlags, PresetValue, SystemFlagsValue
...
class MainPanel(_MainPanel):
    ...
    def on_wavelength_update_ui(self, event: wx.UpdateUIEvent) -> None:
        ...
        if event_object.GetCount() == 0:
            for wavelength in self._device_parameters.wavelengths:
                event_object.Append(str(wavelength), wavelength)
            event_object.SetStringSelection(str(self._device_parameters.wavelength))
    ...
    def on_presets_update_ui(self, event: wx.UpdateUIEvent) -> None:
        ...
        if item_count != len(self._device_parameters.presets):
            event_object.DeleteAllItems()
            for preset in self._device_parameters.presets:
    ...
    def on_preset_remove_update_ui(self, event: wx.UpdateUIEvent) -> None:
        ...
        if index != wx.NOT_FOUND and self._device_parameters is not None:
            preset = self._device_parameters.presets[index]
            enabled = enabled and not (preset.flags & PresetFlags.PROTECTED)
        else:
            enabled = False
    ...
    def on_wavelength_choice(self, _event: wx.CommandEvent) -> None:
        ...
        schedule_task(self._wavelength_command_task, self._task_error, self._wavelength_command_resume)

    def _wavelength_command_task(self, value: DecimalWithDimension) -> DecimalWithDimension:
        if self._control is None:
            raise ControlError

        self._control.set_wavelength(int(value))
        return self._control.get_wavelength()

    def _wavelength_command_resume(self, value: DecimalWithDimension) -> None:
        if self._device_parameters is None:
            raise DeviceParametersError

        self._device_parameters.wavelength = value
    ...
    def _add_preset_command_task(self) -> list[PresetValue]:
        if self._control is None:
            raise ControlError

        self._control.save_preset()
        return self._control.get_presets()

    def _add_preset_command_resume(self, value: list[PresetValue]) -> None:
        if self._device_parameters is None:
            raise DeviceParametersError

        self._state = State.IDLE

        self._device_parameters.presets = value
    ...
    def _remove_preset_command_task(self, value: int) -> list[PresetValue]:
        if self._control is None:
            raise ControlError
    ...
    def _remove_preset_command_resume(self, value: list[PresetValue]) -> None:
        if self._device_parameters is None:
            raise DeviceParametersError
    ...
    def _present_error(self, error: Exception) -> None:
        dialog = wx.MessageDialog(self, message=str(error), caption="Error", style=wx.OK | wx.ICON_ERROR)
        dialog.ShowModal()
        dialog.Destroy()
