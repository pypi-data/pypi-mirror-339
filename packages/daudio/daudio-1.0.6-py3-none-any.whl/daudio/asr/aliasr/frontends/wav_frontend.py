...
class WavFrontend(nn.Module):
    ...
    def __init__(self, cmvn_file: str=None, fs: int=16000, window: str='hamming', n_mels: int=80, frame_length: int=25, frame_shift: int=10, filter_length_min: int=-1, filter_length_max: int=-1, lfr_m: int=1, lfr_n: int=1, dither: float=1.0, snip_edges: bool=True, upsacle_samples: bool=True, **kwargs):
        super().__init__()
        self.fs = fs
        self.window = window
        self.n_mels = n_mels
    ...
    def forward(self, input: torch.Tensor, input_lengths, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(batch_size):
            ...
            mat = kaldi.fbank(waveform, num_mel_bins=self.n_mels, frame_length=min(self.frame_length, waveform_length / self.fs * 1000), frame_shift=self.frame_shift, dither=self.dither, energy_floor=0.0, window_type=self.window, sample_frequency=self.fs, snip_edges=self.snip_edges)
    ...
    def forward_fbank(self, input: torch.Tensor, input_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(batch_size):
            ...
            waveform = waveform * (1 << 15)
            waveform = waveform.unsqueeze(0)
            mat = kaldi.fbank(waveform, num_mel_bins=self.n_mels, frame_length=self.frame_length, frame_shift=self.frame_shift, dither=self.dither, energy_floor=0.0, window_type=self.window, sample_frequency=self.fs)
            feat_length = mat.size(0)
            feats.append(mat)
            feats_lens.append(feat_length)
        feats_lens = torch.as_tensor(feats_lens)
...
class WavFrontendOnline(nn.Module):
    ...
    def forward_fbank(self, input: torch.Tensor, input_lengths: torch.Tensor, cache: dict={}, **kwargs) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        batch_size = input.size(0)
    ...
    def forward_lfr_cmvn(self, input: torch.Tensor, input_lengths: torch.Tensor, is_final: bool=False, cache: dict={}, **kwargs):
        batch_size = input.size(0)
        feats = []
        feats_lens = []
        ...
        for i in range(batch_size):
            mat = input[i, :input_lengths[i], :]
            if self.lfr_m != 1 or self.lfr_n != 1:
                (mat, cache['lfr_splice_cache'][i], lfr_splice_frame_idx) = self.apply_lfr(mat, self.lfr_m, self.lfr_n, is_final)
    ...
    def forward(self, input: torch.Tensor, input_lengths: torch.Tensor, **kwargs):
        ...
        (waveforms, feats, feats_lengths) = self.forward_fbank(input, input_lengths, cache=cache)
        if feats.shape[0]:
            ...
            if feats_lengths[0] + cache['lfr_splice_cache'][0].shape[0] >= self.lfr_m:
                ...
                frame_from_waveforms = int((cache['waveforms'].shape[1] - self.frame_sample_length) / self.frame_shift_sample_length + 1)
