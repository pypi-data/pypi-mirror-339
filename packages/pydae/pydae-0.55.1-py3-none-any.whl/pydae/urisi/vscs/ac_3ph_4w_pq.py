...
def ac_3ph_4w_pq(grid,vsc_data):
    ...
    v_dc  = sym.Symbol(f'v_dc_{bus_dc_name}', real = True) 
    ...
    grid.dae['u_ini_dict'].update({f'v_dc_{bus_dc_name}':800.0}) 
    grid.dae['u_ini_dict'].update({f'{str(q_a)}':0.0,f'{str(q_b)}':0.0,f'{str(q_c)}':0.0}) 

    grid.dae['u_run_dict'].update({f'v_dc_{bus_dc_name}':800.0}) 
