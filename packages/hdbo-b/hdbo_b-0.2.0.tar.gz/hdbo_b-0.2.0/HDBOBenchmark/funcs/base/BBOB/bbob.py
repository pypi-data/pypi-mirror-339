# from https://github.com/google/vizier/blob/main/vizier/_src/benchmarks/experimenters/synthetic/bbob.py
#
...
def Rastrigin(arr: np.ndarray, seed: int = 0) -> float:
    ...
    return float(10 * (dim - np.sum(np.cos(2 * math.pi * z))) + np.sum(z * z, axis=0))
...
def Discus(arr: np.ndarray, seed: int = 0) -> float:
    ...
    return float(10**6 * z_vec[0] * z_vec[0]) + sum([z * z for z in z_vec[1:].flat])
