...
class DesignSpace:
    def __init__(self):
        ...
        self.register_para_type(
            "bool", CategoricalPara
        )  # consider boolean as categorical for combinatorial opt.
        self.paras = {}
        self.para_names = []
        self.numeric_names = []
        self.enum_names = []
    ...
    def parse(self, rec):
        ...
        self.named_para_config = {}
        self.paras = {}
        self.para_names = []
        for item in rec:
            if item["type"] not in self.para_types:
                raise ValueError(
                    f"item {item['type']} not in para_types {self.para_types}"
                )
            ...
            self.named_para_config[item["name"]] = item
    ...
    def transform(self, data: pd.DataFrame) -> (Tensor, Tensor):
        ...
        if xc.ndim == 1:
            for i, name in enumerate(self.numeric_names):
                xc[None, i] = self.paras[name].transform(xc[None, i])
        else:
            for i, name in enumerate(self.numeric_names):
                xc[:, i] = self.paras[name].transform(xc[:, i])
        if xe.ndim == 1:
            for i, name in enumerate(self.enum_names):
                xe[None, i] = self.paras[name].transform(xe[None, i])
        else:
            for i, name in enumerate(self.enum_names):
    ...
    def inverse_transform(self, x: Tensor, xe: Tensor) -> pd.DataFrame:
        ...
        with torch.no_grad():
            inv_dict = {}
            for i, name in enumerate(self.numeric_names):
                inv_dict[name] = self.paras[name].inverse_transform(
                    x.detach().double().numpy()[:, i]
                )
            for i, name in enumerate(self.enum_names):
                inv_dict[name] = self.paras[name].inverse_transform(
                    xe.detach().numpy()[:, i]
