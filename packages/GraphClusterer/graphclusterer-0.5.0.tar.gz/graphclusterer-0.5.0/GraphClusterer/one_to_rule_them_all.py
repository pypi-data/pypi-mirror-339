...
def run_summarization(_name: str, _vertices, aspects, _print_info, sum_model, title_model) -> object:
    """
    ...
    :param _print_info: whether to print the outputs.
    :param sum_model: the model to use for the summarization.
    :param title_model: the model to use for the title generation.
    :return:
    """
    # load the graph.
    ...
    titles = summarize.summarize_per_color(_subgraphs, _name, _vertices, aspects, _print_info, sum_model, title_model)
    return titles
...
def plot_bar(name: str, metrics_dict: dict, vertices: pd.DataFrame = None):
    ...
    values = [
        ...
        metrics_dict.get('purity_score', None),
...
def the_almighty_function(pipeline_kwargs: dict, summary_model=None, refinement_model=None):
    ...
    print(f"Summary model: '{summary_model.model_name}', Refinement model: '{refinement_model.model_name}'")
    ...
    if 'summary' not in vertices.columns:
        ...
        vertices['summary'] = [summarize.summarize_text(s, summary_model) for s in vertices['abstract']]
