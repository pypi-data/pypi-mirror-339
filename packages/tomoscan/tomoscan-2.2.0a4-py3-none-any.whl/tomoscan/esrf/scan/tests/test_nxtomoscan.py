...
class TestHDF5Scan(HDF5TestBaseClass):
    ...
    def testDarkRefUtils(self):
        ...
        self.assertTrue(
            numpy.isclose(
                self.scan.x_pixel_size * _ureg.meter,
                0.05 * _ureg.millimeter,
            )
        )
        self.assertTrue(
            numpy.isclose(
                self.scan.y_pixel_size * _ureg.meter,
                0.05 * _ureg.millimeter,
            )
        )
...
def test_save_and_load_dark(tmp_path):
    ...
    with NXtomoMockContext(
        ...
        output_file_path = scan._format_file_path(
            url=scan.REDUCED_DARKS_DATAURLS[0],
            entry=scan.entry,
            idx=0,
...
def test_get_bounding_box(tmp_path):
    ...
    test_dir = tmp_path / "test_dir"
    test_dir.mkdir()
    x_pos = 0.0 * _ureg.millimeter
    y_pos = 1.0 * _ureg.millimeter
    z_pos = 3.6 * _ureg.millimeter
    magnification = 2
    ...
    scan = MockNXtomo(
        ...
        z_pos=z_pos.magnitude,
    ).scan

    pixel_size = MockNXtomo.PIXEL_SIZE * _ureg.micrometer
    frame_dim = 200

    # note: pixel size is provided with magnificaton. To get
    min_x = x_pos.to(_ureg.meter).magnitude - (
        frame_dim / 2.0 * pixel_size.to(_ureg.meter).magnitude
    )
    max_x = x_pos.to(_ureg.meter).magnitude + (
        frame_dim / 2.0 * pixel_size.to(_ureg.meter).magnitude
    )
    assert scan.get_bounding_box(axis="x") == (min_x, max_x)

    min_y = y_pos.to(_ureg.meter).magnitude - (
        frame_dim / 2.0 * pixel_size.to(_ureg.meter).magnitude
    )
    max_y = y_pos.to(_ureg.meter).magnitude + (
        frame_dim / 2.0 * pixel_size.to(_ureg.meter).magnitude
    )
    assert scan.get_bounding_box(axis="y") == (min_y, max_y)

    min_z = z_pos.to(_ureg.meter).magnitude - (
        frame_dim / 2.0 * pixel_size.to(_ureg.meter).magnitude
    )
    max_z = z_pos.to(_ureg.meter).magnitude + (
        frame_dim / 2.0 * pixel_size.to(_ureg.meter).magnitude
    )
    assert scan.get_bounding_box(axis="z") == (min_z, max_z)
...
def test_build_drac_metadata(tmp_path):
    ...
    nx_tomo.instrument.detector.x_pixel_size = 2.3 * _ureg.micrometer
