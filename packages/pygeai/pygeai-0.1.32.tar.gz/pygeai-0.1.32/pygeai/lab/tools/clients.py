...
from pygeai.lab.tools.endpoints import CREATE_TOOL_V2, LIST_TOOLS_V2, GET_TOOL_V2, UPDATE_TOOL_V2, UPSERT_TOOL_V2, \
    PUBLISH_TOOL_REVISION_V2, GET_PARAMETER_V2, SET_PARAMETER_V2
...
class ToolClient(BaseClient):
    ...
    def create_tool(
        ...
        :param description: str - Description of the tool.
        :param scope: str - Scope of the tool (e.g., "builtin").
        :param parameters: list - List of parameter dictionaries defining inputs and configurations for the tool.
        :param open_api_json: dict - OpenAPI JSON specification for the tool, required for tools with scope 'api'.
            This dictionary is serialized to a JSON string before being sent in the request payload.
            If None, the existing OpenAPI specification (if any) is not updated.
        :param automatic_publish: bool - Whether to automatically publish the tool after updating. Defaults to False.
        ...
        if open_api_json:
            ...
            data["openApiJson"] = open_api_str.replace('"', r'\"')
    ...
    def list_tools(
        ...
        if scope not in valid_scopes:
            raise ValueError("Scope must be one of 'builtin', 'external', or 'api'.")
    ...
    def update_tool(
        ...
        :param project_id: str - Unique identifier of the project.
        :param tool_id: str - Unique identifier of the tool to update.
        :param open_api_json: dict - OpenAPI JSON specification for the tool, required for tools with scope 'api'.
            This dictionary is serialized to a JSON string before being sent in the request payload.
            If None, the existing OpenAPI specification (if any) is not updated.
        :param name: str - Updated name of the tool.
        :param description: str - Updated description of the tool.
        :param scope: str - Updated scope of the tool, must be "builtin", "external", or "api".
        :param parameters: list - Updated list of parameter dictionaries defining inputs and configurations for the tool.
        :param automatic_publish: bool - Whether to automatically publish the tool after updating. Defaults to False.
        :param upsert: bool - Whether to insert the tool if it does not exist (upsert) instead of just updating. Defaults to False.
        :return: dict or str - JSON response containing the updated tool details if successful, otherwise the raw response text.

        :raises ValueError:
            If `scope` is not one of "builtin", "external", or "api".
        :raises JSONDecodeError:
            Caught internally if the response cannot be parsed as JSON; in this case, the raw response text is returned.
        :raises Exception:
            May be raised by `self.api_service.put` if the request fails due to network issues, authentication errors,
            or server-side problems (not explicitly caught here; depends on `api_service` implementation).
        """
        valid_scopes = ["builtin", "external", "api"]
        if scope not in valid_scopes:
            raise ValueError("Scope must be one of 'builtin', 'external', or 'api'.")
