...
def create_tool(option_list: list):
    ...
    for option_flag, option_arg in option_list:
        ...
        if option_flag.name == "open_api_json":
            try:
                ...
                raise WrongArgumentError(
                    "open_api_json must be a valid JSON object in the format: "
                    "'{\"openapi\": \"3.0.0\", \"info\": {\"title\": \"example\", \"version\": \"1.0.0\"}, ...}'"
                )
        if option_flag.name == "automatic_publish":
            automatic_publish = get_boolean_value(option_arg)
    ...
    if scope == "api" and open_api_json is None:
        raise MissingRequirementException(
            "For tools with scope 'api', open_api_json must be provided."
        )
...
create_tool_options = [
    ...
    Option(
        "automatic_publish",
        ["--automatic-publish", "--ap"],
        "Define if tiik must be published besides being created. 0: Create as draft. 1: Create and publish.",
        True
    ),
]
...
update_tool_options = [
    ...
    Option(
        "open_api_json",
        ["--open-api-json", "--oaj"],
        "OpenAPI JSON specification for the tool in JSON format: "
        "'{\"openapi\": \"3.0.0\", \"info\": {\"title\": \"example\", \"version\": \"1.0.0\"}, ...}' "
        "Required for tools with scope 'api'.",
        True
    ),
    Option(
        "name",
    ...
    Option(
        "scope",
        ["--scope", "-s"],
        "Scope of the tool (e.g., builtin, external, api)",
        True
    ),
    Option(
        "parameter",
    ...
    Option(
        "automatic_publish",
        ["--automatic-publish", "--ap"],
        "Define if tool must be published after being updated. 0: Update as draft. 1: Update and publish.",
        True
    ),
    Option(
        "upsert",
        ["--upsert"],
        "Define if tool must be created if it doesn't exist. 0: Update only if it exists. 1: Insert if it doesn't exist.",
        True
    ),
]
...
create_task_options = [
    ...
    Option(
        "title_template",
        ["--title-template", "--tt"],
        "Title template for the task (optional)",
        True
    ),
    Option(
        "automatic_publish",
...
update_task_options = [
    ...
    Option(
        "name",
        ["--name", "-n"],
        "Name of the task (optional for update)",
        True
    ),
    Option(
        "description",
        ["--description", "-d"],
        "Description of the task (optional for update)",
        True
    ),
    Option(
        "title_template",
        ["--title-template", "--tt"],
        "Title template for the task (optional for update)",
        True
    ),
    Option(
        "automatic_publish",
