...
def main():
    ...
    if isinstance(instance_result, ProcessInstance):
        print(f"Success: Started Instance: Process: {instance_result.process_name}, ID: {instance_result.id}")
    ...
    if isinstance(instance_info, ProcessInstance):
        print(f"Success: Instance Info: Process: {instance_info.process_name}, Subject: {instance_info.subject}, Variables: {[var.to_dict() for var in instance_info.variables]}")
    else:
