...
class TestCFFClient(unittest.TestCase):
    ...
    def test_get_pipelines(self, mock_get):
        mock_get.return_value.json.return_value = {"pipelines": ["TestPipeline"]}
        response = self.client.get_pipelines(server_ip="http://example.com")
        self.assertEqual(response, {"pipelines": ["TestPipeline"]})
        mock_get.assert_called_once_with("http://127.0.0.1:8000/metadata/workflow/pipelines", params={"server_ip": "http://example.com"})
