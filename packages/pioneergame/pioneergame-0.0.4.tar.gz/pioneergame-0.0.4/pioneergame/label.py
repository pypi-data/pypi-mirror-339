...
class Label:
    def __init__(self, window: Window, x, y, color: str | pg.Color | tuple[int, int, int],
                 text: str = 'Sample Text', size: int = 25, font: str = "", italic: bool = False):
        self.window = window
        self.x = x
        self.y = y
        self.color = color
        self.text = text
        self.font = pg.font.SysFont(font, size, False, italic) if font else pg.font.Font('Fixedsys.ttf', size)
