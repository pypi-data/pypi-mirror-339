...
def read_file(f, file_path, filters={}):
    ...
    unit = int(os.path.split(os.path.splitext(file_path)[0])[1].split('_')[1])
    file_id = int(file_name_parts[-3] + file_name_parts[-2] + file_name_parts[-1])
    X['file_id'] = file_id
    X['unit'] = unit
    ...
    for cycle in Xd.Cycle_Index.unique():
        ...
        # X.loc[X.Cycle_Index == cycle, 'capacity'] = -1 * np.sum(time_diff * current / 3600)
...
def read_data(file_path, task: dict = None, filters: dict = None, params={}):
    ...
    for unit in C.unit.unique():
        ...
        C.loc[C.unit == unit, 'soc'] = C.loc[C.unit == unit, 'capacity'] / 1.1
