        ...
        for note_file in os.listdir(folder_path):
          ...
          with open(note_path, "r") as f:
            ...
            if first_line.lower().startswith("tags: "):
              ...
              note_tags = [tag.strip().lower() for tag in tags_str.split(", ")]
              if query in note_tags:
                if note_name not in found_notes_by_tag:
                  found_notes_by_tag[note_name] = folder

    if found_notes_by_tag:
      console.print("\n[bold green]Notes found by tag:[/bold green]")
      for name, folder in found_notes_by_tag.items():
        console.print(f"├── [bold]{name}[/bold] in [italic]{folder}[/italic]")
      console.print("└── Type '[bold]o + note name in which it was found[/bold]' to open or '[bold]c[/bold]' to cancel.")
