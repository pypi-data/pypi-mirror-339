...
def search_rich(name):
  ...
  prompt_text = f"[bold {theme_color}]Folder:[/bold {theme_color}]"
  choice = Prompt.ask(
    f"\nType the folder name to open it or '[italic]c[/italic]' to cancel",
    default="c",
  )
...
def read_note_rich(folder, name):
  """Reads and displays a note within a Rich Panel."""
  note_path = os.path.join(BASE_DIR, folder, f"{name}.txt")
  if not os.path.exists(note_path):
    console.print(
      Panel(
        f"[bold red]Note '{name}' not found in folder '{folder}'.[/bold red]",
  ...
  try:
    with open(note_path, "r") as file:
      content = file.read()
    panel = Panel(content, title=f"[bold blue]{name}[/bold blue]")
    console.print(panel)
  except Exception as e:
    console.print(
      Panel(
        f"[bold red]Error reading note '{name}': {e}[/bold red]",
        title="[bold red]Error[/bold red]",
      )
    )
