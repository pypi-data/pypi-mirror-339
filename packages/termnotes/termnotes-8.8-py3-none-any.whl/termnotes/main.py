...
def search(query):
  ...
  if found_notes_by_tag:
    results_content = "[bold green]Notes found by tag:[/bold green]\n"
    for name, folder in found_notes_by_tag.items():
      results_content += f"├── [bold]{name}[/bold] in [italic]{folder}[/italic]\n"
    results_content += "└── Type '[bold]o + note name in which it was found[/bold]' to open or '[bold]c[/bold]' to cancel."
    results_panel = Panel(results_content, title="[bold blue]Tag Search Results[/bold blue]")
    console.print("\n")
    console.print(results_panel)
    choice = Prompt.ask("\n[bold blue]Option:[/bold blue]").strip().lower()
    if choice != 'c' and choice.startswith('o '):
      note_to_open_with_folder = choice[2:].strip().split(' in ')
      if len(note_to_open_with_folder) == 2:
        note_name_to_open = note_to_open_with_folder[0].strip()
        folder_of_note = note_to_open_with_folder[1].strip()
        if os.path.exists(os.path.join(BASE_DIR, folder_of_note, f"{note_name_to_open}.txt")):
          read_note(folder_of_note, note_name_to_open)
          in_folder = folder_of_note
          return
        else:
          console.print("[bold red]Note not found in the specified folder.[/bold red]")
          return
      else:
        console.print("[bold red]Invalid open format.[/bold red]")
        return
  elif choice == 'c':
    console.print("[bold yellow]\nTag search canceled.[/bold yellow]\n")
    return
  else:
