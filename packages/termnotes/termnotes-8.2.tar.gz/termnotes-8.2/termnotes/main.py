...
def search(name):
  ...
  found_notes_with_tag = []
  found_notes_with_name = []
  found_folders = [
        ...
        if note_file.endswith(".txt"):
          ...
          with open(note_path, "r") as f:
            ...
            if first_line.startswith("Tags:"):
              ...
              if name.lower() in tags_list:
                found_notes_with_tag.append((folder, note_name))
                is_tag_search = True
            if name in note_name:
              found_notes_with_name.append((folder, note_name))

  found_notes = list(set(found_notes_with_tag + found_notes_with_name))
  ...
  if is_tag_search:
    choice_prompt = f"\nType 'o' + filename or 'c' to cancel search"
  else:
    choice_prompt = f"\nType 'o' to open, or 'c' to cancel search"

  choice = Prompt.ask(choice_prompt).lower()

  if choice == "o" and not is_tag_search:
    if found_folders:
      if len(found_folders) == 1:
        ...
        console.print("[bold yellow]\nMultiple folders found. Please be more specific or use 'o' + foldername.[/bold yellow]\n")
    elif found_notes:
      if len(found_notes) == 1:
        folder, note = found_notes[0]
        read_note(folder, note)
        in_folder = folder
        return
      else:
        console.print("[bold yellow]\nMultiple notes found. Please be more specific or use 'o' + foldername/notename.[/bold yellow]\n")
    ...
    for folder in found_folders:
      if folder == filename:
        ...
        return
    for folder, note in found_notes:
      if f"{folder}/{note}" == filename or note == filename:
        read_note(folder, note)
        in_folder = folder
        return
    console.print("[bold red]\nNo matching folder or note found with that name in search results.[/bold red]\n")
