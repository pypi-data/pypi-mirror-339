...
def edit_note_or_folder(name):
  """Edits a note or renames a folder with Rich formatting (simple panel)."""
  global in_folder
  global auto_complete_names

  if in_folder:
    note_path = os.path.join(BASE_DIR, in_folder, f"{name}.txt")
    title = f"[bold blue]Editing Note: {name}[/bold blue]"

    if not os.path.exists(note_path):
      panel = Panel("[bold red]Note not found.[/bold red]", title=title)
      console.print(panel)
      return

    console.print(Panel("\nPress Enter to keep the current name, or type a new name:", title=title))
    ...
    if new_name and new_name != name and check_name(new_name):
      ...
      console.print(Panel(f"[green]Note renamed to '[bold]{new_name}[/bold]'.[/green]", title=f"[bold blue]Rename Note[/bold blue]"))
      name = new_name
      note_path = new_path
    ...
    try:
      ...
      panel = Panel(f"[bold red]Error reading note content:\n{e}[/bold red]", title=title)
      console.print(panel)
      return

    content_text = Text()
    for i, line in enumerate(old_content, 1):
      content_text.append(f"{i}: ", style=f"bold blue")
      content_text.append(line.strip() + "\n")

    console.print(Panel(content_text, title=f"[bold blue]Current Content[/bold blue]"))

    new_content = list(old_content)

    while True:
      prompt_text = Text("\nEnter:\n", style=f"bold blue")
      prompt_text.append("'line number' to edit\n", style="default")
      prompt_text.append("'a' to append\n", style="default")
      prompt_text.append("'d + line number' to delete\n", style="default")
      prompt_text.append("'c + line number' to copy line\n", style="default")
      prompt_text.append("'save' to save:\n\n", style="default")
      prompt_text.append("cmd: ", style=f"bold blue")
      command = console.input(prompt_text).strip()

      if command.lower() == "save":
        break
      elif command.lower() == "a":
        console.print(Panel("\nType new lines (enter 'save' when finished):", title=title))
        while True:
        ...
        if 0 <= line_number < len(new_content):
          console.print(Panel(f"Current: {new_content[line_number].strip()}", title=title))
          new_text = console.input("New text: ").strip()
          if new_text:
            new_content[line_number] = new_text + "\n"
        else:
          console.print(Panel("[bold red]Invalid line number.[/bold red]", title=title))
      elif command.startswith("d ") and command[2:].isdigit():
        line_number = int(command[2:]) - 1
        if 0 <= line_number < len(new_content):
          del new_content[line_number]
          console.print(Panel(f"[green]Line {line_number + 1} deleted.[/green]", title=title))
        else:
          console.print(Panel("[bold red]Invalid line number.[/bold red]", title=title))
      elif command.startswith("c ") and command[2:].isdigit():
        line_number = int(command[2:]) - 1
        if 0 <= line_number < len(new_content):
          copied_line = new_content[line_number]
          pyperclip.copy(copied_line)
          console.print(Panel(f"[green]Line {line_number + 1} copied to clipboard.[/green]", title=title))
        else:
          console.print(Panel("[bold red]Invalid line number.[/bold red]", title=title))
      else:
        console.print(Panel("[bold red]Invalid command.[/bold red]", title=title))

    with open(note_path, "w") as file:
      file.writelines(new_content)
    console.print(Panel("[green]Note updated successfully.[/green]", title=f"[bold green]Save Successful[/bold green]"))

  else:
    folder_path = os.path.join(BASE_DIR, name)
    title = f"[bold blue]Renaming Folder: {name}[/bold blue]"
    if not os.path.exists(folder_path):
      panel = Panel(f"[bold red]Folder not found.[/bold red]", title=title)
      console.print(panel)
      return

    console.print(Panel("\nEnter a new name for the folder:", title=title))
    new_name = console.input().strip()
    ...
    if new_name and new_name != name and check_name(name):
      ...
      console.print(Panel(f"[green]Folder renamed to '[bold]{new_name}[/bold]'.[/green]", title=f"[bold green]Rename Folder[/bold green]"))
...
def run():
  ...
  while True:
    choice = console.input("cmd ", style="bold blue").strip()
