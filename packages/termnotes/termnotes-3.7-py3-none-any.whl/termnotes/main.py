...
def edit_note_or_folder(name):
    """Edits a note (rename and modify content) or renames a folder with Rich formatting."""
    global in_folder
    global auto_complete_names
    ...
    if in_folder:  # Editing a note
        ...
        if not os.path.exists(note_path):
            panel = Panel("[bold red]Note not found.[/bold red]", title=f"[bold blue]{name}[/bold blue]")
            console.print(panel)
            return
        ...
        try:
            ...
            panel = Panel(f"[bold red]Error reading note content:\n{e}[/bold red]", title=f"[bold red]Error[/bold red]")
            console.print(panel)
            return

        console.print(Panel(Text("\n".join(line.strip() for line in old_content), style=f"default"), title=f"[bold blue]Current Content[/bold blue]"))

        new_content = list(old_content)  # Copy old content
        ...
        while True:
            ...
            if command.lower() == "save":
                ...
                if 0 <= line_number < len(new_content):
                    console.print(f"Current: {new_content[line_number].strip()}")
                    new_text = console.input("New text: ").strip()
                    if new_text:
                        new_content[line_number] = new_text + "\n"  # Modify the line
                else:
                    console.print("[bold red]Invalid line number.[/bold red]")
                ...
                if 0 <= line_number < len(new_content):
                    ...
                    console.print(f"[green]Line {line_number + 1} deleted.[/green]")
                else:
                    console.print("[bold red]Invalid line number.[/bold red]")
                ...
                if 0 <= line_number < len(new_content):
                    ...
                    console.print(f"[green]Line {line_number + 1} copied to clipboard.[/green]")
                else:
                    console.print("[bold red]Invalid line number.[/bold red]")
            else:
        ...
        try:
            ...
            panel = Panel(f"[bold red]Error saving note content:\n{e}[/bold red]", title=f"[bold red]Error[/bold red]")
            console.print(panel)

    else:  # Renaming a folder
        folder_path = os.path.join(BASE_DIR, name)
        if not os.path.exists(folder_path):
            panel = Panel(f"[bold red]Folder not found.[/bold red]", title=f"[bold blue]{name}[/bold blue]")
            console.print(panel)
            return
...
def run():
  ...
  while True:
    choice = input(f"[bold blue]cmd: [/bold blue]")
