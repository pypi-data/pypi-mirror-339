  ...
  if in_folder:  # Editing a note
    ...
    while True:
      ...
      if command.lower() == "save":
        ...
        print("\nEdit tags (enter 'save' when finished):")
        while True:
          new_line = input()
          if new_line.lower() == "save":
            break
          new_tags.append(new_line + "\n")  # Append new tags
        ...
        line_number = int(command[2:]) - 1
        if 0 <= line_number < len(new_tags):
            copied_line = new_tags[line_number]  # Copy the specified line
    ...
    if len(new_tags) > 0:
      ...
      for tag in new_tags:
        ...
        # Ensure there's exactly one '#' at the beginning
        if stripped_tag.startswith('#'):
          processed_tags.append(f"[bold pale_violet_red1]{stripped_tag}[/bold pale_violet_red1]")
        elif stripped_tag:  # Only add '#' if the tag is not empty
          processed_tags.append(f"[bold pale_violet_red1]#{stripped_tag}[/bold pale_violet_red1]")
