...
def search_by_tag(tag):
  """Searches for notes by tag (without regex)."""
  found_notes = []
  for folder in os.listdir(BASE_DIR):
    folder_path = os.path.join(BASE_DIR, folder)
    if os.path.isdir(folder_path):
      for note_file in os.listdir(folder_path):
        if note_file.endswith(".txt"):
          ...
          with open(note_path, "r") as f:
            content = f.read()
            #  Find the tag without regex
            if f"Tags: {tag}" in content or f"Tags: {tag}," in content:
              found_notes.append((folder, note_file.replace(".txt", "")))

  if not found_notes:
    console.print(f"\n[bold red]No notes found with tag '{tag}'.[/bold red]\n")
    return

  search_results = ["[bold green]Notes found with tag:[/bold green]"]
  for folder, note in found_notes:
    search_results.append(f"└── [bold]{folder}/{note}[/bold] (n)")
  results_panel = Panel("\n".join(search_results))
  console.print("\n")
  ...
  choice = Prompt.ask(
    f"\nType 'o' to open a note, 'l' to list notes in folder or 'c' to cancel:"
  )
  if choice.lower() == "o":
    note_to_open = Prompt.ask("Enter note name (e.g., folder/note_name):")
    folder_name, note_name = note_to_open.split("/")
    read_note(folder_name, note_name)
  elif choice.lower() == "l":
    folder_to_list = Prompt.ask("Enter folder name:")
    list_notes(folder_to_list)
  elif choice.lower() == "c":
...
def run():
  ...
  while True:
    ...
    elif choice.startswith("st "):  #Search by tag
      tag = choice[3:]
      search_by_tag(tag)
