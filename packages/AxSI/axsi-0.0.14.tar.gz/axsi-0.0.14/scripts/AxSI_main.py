...
# Run with the default values of argparse
def run_from_python(subj_folder, data_path, mask_path, bval_path, bvec_path, small_delta=15, big_delta=45, gmax=7.9,
                    gamma_val=4257):
    subj_folder = assert_path(subj_folder)
    file_names = build_files_dict(data_path, mask_path, bval_path, bvec_path)

    if subj_folder:
        logger.info(f"subj_folder: {subj_folder} \nfilenames: {file_names} \nsmall_delta: {small_delta} \nbig_delta: "
                    f"{big_delta} \ngmax: {gmax} \ngamma_val: {gamma_val}")
...
def run_from_terminal():
    ...
    parser.add_argument('--run-name', type=Path, help='Specify the name for the run', required=True)
    ...
    parser.add_argument('--small-delta', type=float, default=15, help='Gradient duration in miliseconds (default: %(default)s)')
    parser.add_argument('--big-delta', type=float, default=45, help='Time to scan (time interval) in milisecond (default: %(default)s)')
    parser.add_argument('--gmax', type=float, default=7.9, help='Gradient maximum amplitude in G/cm (default: %(default)s)')
...
# Full input files:
# =================
# python AxSI_main.py --subj-folder full-output-temp --run-name test_run --bval '/Users/user/Library/CloudStorage/GoogleDrive-refaelkohen@mail.tau.ac.il/My Drive/TLV-U-drive/BrainWork/AxSI-pipeline/input-data/sub-CLMC10/ses-202407110849/dwi/sub-CLMC10_ses-202407110849_space-dwi_desc-preproc_dwi.bval' --bvec '/Users/user/Library/CloudStorage/GoogleDrive-refaelkohen@mail.tau.ac.il/My Drive/TLV-U-drive/BrainWork/AxSI-pipeline/input-data/sub-CLMC10/ses-202407110849/dwi/sub-CLMC10_ses-202407110849_space-dwi_desc-preproc_dwi.bvec' --mask '/Users/user/Library/CloudStorage/GoogleDrive-refaelkohen@mail.tau.ac.il/My Drive/TLV-U-drive/BrainWork/AxSI-pipeline/input-data/sub-CLMC10/ses-202407110849/dwi/sub-CLMC10_ses-202407110849_space-dwi_desc-brain_mask.nii.gz' --data '/Users/user/Library/CloudStorage/GoogleDrive-refaelkohen@mail.tau.ac.il/My Drive/TLV-U-drive/BrainWork/AxSI-pipeline/input-data/sub-CLMC10/ses-202407110849/dwi/sub-CLMC10_ses-202407110849_space-dwi_desc-preproc_dwi.nii.gz' --debug-mode --num-processes 35 --num-threads 1

# 4-timeframes (0,1,5,8) input files (bvals: 0,0, 1,1):
# ======================================================
# python AxSI_main.py --subj-folder tests/short-output --run-name test_run --bval 'tests/test_data/part_times/test_vol_0_1_5_8_bval.txt' --bvec 'tests/test_data/part_times/test_vol_0_1_5_8_bvec.txt' --data 'tests/test_data/part_times/test_vol_0_1_5_8_data.nii' --mask 'tests/test_data/part_times/test_vol_0_1_5_8_mask.nii'

# 6-timeframes (0,1,5,8,80,81) input files (bvals: 0,0, 1,1, 2,2):
# ======================================================
# python AxSI_main.py --subj-folder tests/short-output --run-name test_run --bval 'tests/test_data/part_times/test_vol_0_1_5_8_80_81_bval.txt' --bvec 'tests/test_data/part_times/test_vol_0_1_5_8_80_81_bvec.txt' --data 'tests/test_data/part_times/test_vol_0_1_5_8_80_81_data.nii' --mask 'tests/test_data/part_times/test_vol_0_1_5_8_80_81_mask.nii'
...
# python AxSI_main.py --subj-folder 15-voxels-output --run-name test_run --bval "toy_tests/test_data/part_voxels/voxels_${s}_${e}_bval.txt" --bvec "toy_tests/test_data/part_voxels/voxels_${s}_${e}_bvec.txt" --data "toy_tests/test_data/part_voxels/voxels_${s}_${e}_data.nii" --mask "toy_tests/test_data/part_voxels/voxels_${s}_${e}_mask.nii"
