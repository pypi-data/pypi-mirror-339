...
from AxSI.toolbox import cart2sph_env, parallel_batches
...
class Predictions():
    ...
    def __init__(self, scan: Scan, dti1000: DTI_Values) -> None:
        ...
        if config.NUM_PROCESSES_PRED == 1:  # in multiprocessing init it with shared memory in predict_hindered() method
    ...
    def predict_hindered(self, bshell: np.ndarray, shell_dti: list[DTI_Values]) -> None:
        """
        Main function to compute MD0 and predict hindered for each voxel.
        Parallellism depends on hardcoded value of num_of_processed.

        :param bshell:      bvalues of the shell
        :param shell_dti:   dti values of the shell
        """
        # sMDi[j,k] is the biggest eigval of voxel j under the k'th DTI, dims: (n_vols, len(shell_dti))
        sMDi = np.stack([dti.get("eigval")[:, -1] for dti in shell_dti], axis=-1)  # stack on the columns
        if config.NUM_PROCESSES_PRED > 1:
            with Manager() as manager:
                self.md0 = manager.list([None] * self.num_of_vols)  # Create a shared list

                parallel_batches(self.num_of_vols, config.NUM_PROCESSES_PRED, config.NUM_THREADS_PRED,
                                 self.predict_md0_for_voxel, bshell, sMDi)

                # Copy values from the shared object to self
                self.md0 = np.array(self.md0)
        elif config.NUM_THREADS_PRED > 1:
            parallel_batches(self.num_of_vols, config.NUM_PROCESSES_PRED, config.NUM_THREADS_PRED,
                             self.predict_md0_for_voxel, bshell, sMDi)
        else:
            # calculate MD0 in sequence
            for i in range(self.num_of_vols):
                self.predict_md0_for_voxel(bshell, sMDi, i)

        # calculate hindered decay altogether. The decay values are stored in self.hindered.
    ...
    def predict_hindered_decay(self, sMDi: np.ndarray) -> None:
        ...
        :param sMDi: sMDi[j, k] is the biggest eigval of voxel j under the k'th DTI, dims: (n_vols, len(shell_dti))
        """
        every_fac = self.md0 / sMDi[:, 0]  # sMDi[:, 0] is max eignval for bvalue=1
        every_fac = every_fac.reshape((self.num_of_vols, 1, 1))  # adjust to DT1000_maps dims
        every_d_mat = every_fac * 1000 * self.dt1000_maps  # dims: (n_vols, 3, 3)
        # a series of matrix multiplications
        # self.grad_dirs.shape = (n_frames, 3), every_d_mat.shape=(n_vols, 3, 3), decay.shape=(num_of_vols, n_frames)
        decay = exp_vec_mat_multi(self.grad_dirs, every_d_mat, -self.max_bval, axis=1)
        self.hindered = decay  # set result

    def predict_md0_for_voxel(self, bshell: np.ndarray, smdi: np.ndarray, i: int) -> None:
        """
        For a specific voxel, Use `curve_fit` to model eigvals as a function of `bshell` values,
        where eigvals are the maximum eigenvalue for each DTI with respective bshell.

        :param smdi: biggest eigval of voxel i for each bshell. dims: (len(shell_dti), )
        :return:       first parameter of the model
        """
        fit, covariance = curve_fit(exp1, bshell, smdi[i, :], method='trf', loss='soft_l1')
        fit_a = fit[0]  # there are 2 parameters in exp1.
        self.md0[i] = fit_a

    def init_predict_restricted(self) -> None:
        """
        Calculate the parallel and perpendicular sq values for each voxel and timeframe.
        Save the results in self.q_par_sq_arr and self.q_perp_sq_arr.
    ...
    def predict_restricted(self, i: int) -> np.ndarray:
        """
        Predict restricted decay for the ith voxel for all timeframes.
        Use data computed to all voxels.
        output dims:

        :param i:  index of the voxel
        :return:   restricted decay values: (n_frames, N_SPEC)
        """
        signal_0 = self.signal_0[i]  # mean of voxel i signal with bvalue=0
        q_par_sq_arr = self.q_par_sq_arr[i, :]  # a value for each timeframe
        q_perp_sq_arr = self.q_perp_sq_arr[i, :]
        md = self.md0[i]  # MD0 from predict_hindered calculations (float).
    ...
    def calc_factor_angle(self) -> np.ndarray:
        ...
        dim eigvecs: (n_vols, ), dim factor_angle: (n_vols, n_frames)

        :return: factor_angle for each voxel and timeframe. dims: (n_vols, n_frames)
        """
        theta_q = self.params['theta']  # dims: (n_frames, )
        phi_q = self.params['phi']  # dims: (n_frames, )
        _, phi_n, theta_n = cart2sph_env(self.eigvec1000.T)  # dims: (n_vols, )
        tmp_cos = np.outer(np.cos(theta_n), np.cos(theta_q))  # dims: (n_vols, n_frames)
        tmp_sin = np.outer(np.sin(theta_n), np.sin(theta_q))  # dims: (n_vols, n_frames)
        factor_angle = abs(tmp_cos * np.cos(phi_q - phi_n[np.newaxis].T) + tmp_sin)  # dims: (num_of_vols, n_frames)
        return factor_angle

    def calc_sq(self, factor_angle: np.ndarray) -> np.ndarray:
        """
        :param factor_angle: factor_angle: (n_vols, n_frames)
        :return: sq of factor_angle (n_vols, n_frames)
        """
        return (self.params['r'] * factor_angle) ** 2  # self.params['r']: (n_frames,)

    def calc_E(self, q_perp: np.ndarray, q_par: np.ndarray, MD: float) -> np.ndarray:
        """
        Calculate E matrix for predict restricted for one voxel for all timeframes.
        qprep: (n_frames, ), qpar: (n_frames, )

        :param q_perp: perpendicular sq values
        :param q_par:  parallel sq values
        :param MD:     md0 value
        :return:
        """
        bigdel = self.params['big_delta']  # float
        smldel = self.params['small_delta']  # float
        q_par = q_par[np.newaxis].T  # adjust dims to (n_frames, 1)
        # ADD_VALS is defined in toolbox
        tmp = np.multiply.outer(q_perp, (config.ADD_VALS / 2) ** 2)  # dims: (n_frames, N_SPEC)
        tmp = tmp + (q_par * (bigdel - smldel / 3) * MD)  # dims: (n_frames, N_SPEC)
        E = np.exp(-4 * np.pi ** 2 * tmp)  # dims: (n_frames, N_SPEC)
        return E

    def predict_csf(self, shell_dti: list[DTI_Values]):
        """
        Calculate CSF decay for all voxels at once.
        Use gaussian mixture model.

        :param shell_dti:
        :return: decay for voxels with nonzero md value
        """
        # TODO: Refael: the comment is wrong: shell_dti[0] is for bvalue=1 not bvalue=0
        md_non0 = shell_dti[0].get("md")  # dim: (n_vols,). get md values from shellDTI with bvalue=0
    ...
    def compute_pdf_array(self, md_nonzero: np.ndarray, gm: GaussianMixture) -> list:
        """
        Probability to get md value (for all voxels with nonzero md) for each gaussian
        from the mixed gaussian model (we use 3 components).

        :param md_nonzero:  md values for all voxels
        :param gm:          fitted gaussian mixture
        :return:            3 probabilities for voxels with nonzero md
        """
    ...
    def fit_gaussian_mixture(self, md_nonzero: np.ndarray) -> GaussianMixture:
        """
        Use scipy gaussian mixture fit data.

        :param md_nonzero: md values of the voxels (only nonzero)
        :return:
        """
        ...
        ).fit(md_nonzero)  # not precisely as in matlab (predict_csf.m)
        return gm


def exp1(x, a, b):
    """ Function for curve fit in MD0 calculation.
    The learned parameters are: a, b
    """
    y = a * np.exp(b * x)
    return y


def predict(scan: Scan, dti1000: DTI_Values, shell_dti: list[DTI_Values]) -> Predictions:
    """
    Calculate predictions decays for each environment

    :param scan:        scan object
    :param dti1000:     DTI_Values object for dti1000
    :param shell_dti:   list of DTI_Values objects for shell_dti
    :return object with decays prediction for AxSI
    """

    decays = Predictions(scan, dti1000)  # init object
    # dims hindered_decay: (n_vols, len(shell_dti)),   dims md0: (n_vols, ). Stored in self.hindered.
    tic = time.time()
    decays.predict_hindered(scan.get_shell(), shell_dti)
    toc = time.time()
    logger.debug("Computation time md0 all voxels " + (
        "with multi-processes/multi-threads " if config.NUM_PROCESSES_PRED > 1 or config.NUM_THREADS_PRED > 1 else "") + "= " + str(
        (toc - tic)) + " sec.")
    ...
    toc = time.time()
    logger.debug("Computation time restricted decay = " + str((toc - tic)) + " sec.")

    # calculate CSF decay, dims: (n_vols)
