...
class XmlUiStringIntermediate(Intermediate):
    ...
    def transform_to(self, content: Any, chunk_size: int = None) -> ET.Element:
        ...
        # Process all strings through translation function
        for s_node in root.findall('.//s'):
            original_text = s_node.text or ''
            translated_text = self.process_func(original_text, self.source_lang, self.language)
            s_node.text = translated_text
