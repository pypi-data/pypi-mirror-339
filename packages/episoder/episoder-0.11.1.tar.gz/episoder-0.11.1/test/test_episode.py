...
class TestEpisode(TestCase):
    ...
    def test_construct(self):
        ...
        ep = Episode("First", 3, 8, date(2017, 1, 1), "0XOR", 117)
        ...
        self.assertEqual(ep.title, "First")
        self.assertEqual(ep.totalnum, 117)
        self.assertEqual(ep.prodnum, "0XOR")

    def test_str_and_repr(self):

        show = Show("TvShow", "")
        ep = Episode("First", 1, 1, date(2017, 1, 1), "http://", 1)
        ep.show = show

        self.assertEqual(str(ep), "TvShow 1x01: First")
        self.assertEqual(repr(ep), 'Episode("First", 1, 1, '
