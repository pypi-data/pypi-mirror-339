...
from episoder.sources import parser_for, setup_sources
from episoder.sources.thetvdb import TVDBShowNotFoundError
from episoder.output import ColorfulRenderer, ColorlessRenderer, EmailNotifier
from episoder.output import NewEpisodesNotification
from episoder.version import __version__


class EpisoderOptions:
    ...
    def _add_options(self):
        ...
        parser = commands.add_parser("add", help="Add a show")
        ...
        self._parser.add_argument("-c", metavar="file", action="store",
                                  default=path.join(environ["HOME"],
                                                    ".episoder"),
    ...
    def _replace_parsed_date(self, args):
        if args.date.isdigit():
            ...
            self._parser.error(f"{args.date}: Wrong date format")
...
class Episoder:
    ...
    def add(self):
        ...
        if not parser_for(url):
            stderr.write(f"Invalid show url/id: {url}\n")
            return
    ...
    def _set_show_enabled(self, database, enabled):
        ...
        if not show:
            stderr.write(f"There is no show with id={show_id}\n")
            return
    ...
    def episodes(self):
        ...
        if self._args.nocolor:
            out = ColorlessRenderer(self._args.format, self._args.dateformat)
        else:
        ...
        out.render(episodes, self._args.date)
    ...
    def search(self):
        ...
        try:
            print("ID\tName\n-------\t--------------------")
            for show in tvdb.lookup(self._args.keyword):
...
def main():
    ...
    setup_sources(options.args.agent, options.args.tvdb_key)
