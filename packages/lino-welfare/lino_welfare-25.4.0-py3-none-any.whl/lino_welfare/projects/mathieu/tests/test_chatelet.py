...
class TestCase(TestCase):
    ...
    def test_cv_obstacle(self):
        ...
        colnames.sort()
        self.assertEqual('detected_date id person remark type user',
                         ' '.join(colnames))
        ...
        response = self.client.post(url, post_data, REMOTE_USER='robin')
        result = self.check_json_result(
            response, 'rows success message navinfo master_data')
        ...
        self.assertEqual(result['rows'], [[
            'Alcohol', 1, 'robin', 1, '22.05.2014', '', 1, 'LAST First (100)', 100, {
                'id': True
            }, False, {
                'meta': True
            }
