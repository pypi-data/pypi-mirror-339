...
class TestCase(TestCase):
    ...
    def test_aids(self):
        ...
        cli = self.create_obj(Client, first_name="First", last_name="Client")

        pt = self.create_obj(ClientContactType, name="Apotheke")

        ct = ConfirmationTypes.get_by_value('aids.RefundConfirmation')
        aid_type = self.create_obj(AidType,
                                   name="foo",
                                   confirmation_type=ct,
                                   pharmacy_type=pt)
        grant = self.create_obj(Granting, client=cli, aid_type=aid_type)

        ar = RefundConfirmations.create_request(user=robin)
        ...
        obj = ar.create_instance(granting=grant,
                                 start_date=i2d(20180331),
                                 end_date=i2d(20180331))
        with translation.override('en'):
            try:
                ...
                self.assertEqual(
                    str(e),
                    "['Date range 31/03/2018...31/03/2018 lies outside of granted period 01/04/2018....']"
                )
