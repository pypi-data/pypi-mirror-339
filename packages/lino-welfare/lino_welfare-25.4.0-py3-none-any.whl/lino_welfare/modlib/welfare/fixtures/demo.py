...
COACHING_STORIES[ClientStates.former] = Cycler([(-1000, -500, True, CT_GSS)],
...
def objects():
    ...
    DEBTS = rt.models.coachings.CoachingType(does_gss=False,
                                             does_integ=False,
                                             **dd.babelkw(
                                                 'name',
    ...
    melanie = person(first_name="Mélanie",
                     last_name="Mélard",
    ...
    melanie = User(username="melanie",
                   ...
                   coaching_type=DSBE,
                   language='fr',
                   newcomer_consultations=False,
                   newcomer_appointments=False)
    yield melanie

    hubert = person(first_name=u"Hubert",
                    last_name=u"Huppertz",
                    email=settings.SITE.demo_email,
                    city=kettenis,
                    country='BE',
    ...
    alicia = User(username="alicia",
                  ...
                  coaching_type=DSBE,
                  newcomer_consultations=True,
                  newcomer_appointments=True)
    yield alicia

    theresia = person(first_name="Theresia",
                      last_name="Thelen",
                      email=settings.SITE.demo_email,
                      city=eupen,
                      country='BE',
    ...
    yield cal.GuestRole(**dd.babelkw(
        'name',
        de=u"Vorsitzender",
        fr=u"Président",
        en=u"Chairman",
    ...
    contractType = Instantiator(jobs.ContractType, "ref", exam_policy=3).build
    yield contractType(
        'art60-7a',
        **dd.babelkw(
            'name',
    ...
    contractType = Instantiator(isip.ContractType, "ref", exam_policy=1).build
    yield contractType("vsea",
    ...
    yield company(name="Die neue Alternative V.o.G.",
                  city=eupen,
                  country=belgium)
    ...
    yield proaktiv_dir
    cardijn = company(name="Werkstatt Cardijn V.o.G.",
                      city=eupen,
                      country=belgium)
    yield cardijn
    yield company(name="Behindertenstätten Eupen", city=eupen, country=belgium)
    bwe = company(name="Beschützende Werkstätte Eupen",
                  city=eupen,
                  country=belgium)
    yield bwe
    ...
    yield company(
        name="Solidaris - Mutualité socialiste et syndicale de la province de Liège",
        **kw)
    ...
    yield company(name="Apotheke Reul",
                  street='Klosterstraße',
                  street_no=20,
                  **kw)
    yield company(name="Apotheke Schunck",
                  street='Bergstraße',
                  street_no=59,
                  **kw)
    yield company(name="Pharmacies Populaires de Verviers",
                  street='Aachener Straße',
                  street_no=258,
                  **kw)
    yield company(name="Bosten-Bocken A",
                  street='Haasstraße',
                  street_no=6,
                  **kw)
    ...
    yield cct
    kw = dict(client_contact_type=cct, country=belgium, city=eupen)
    yield company(name=u"Brüll Christine",
                  street=u'Schilsweg',
                  street_no=4,
                  **kw)
    yield company(name=u"Brocal Catherine",
                  street=u'Neustraße',
                  street_no=115,
                  **kw)
    yield company(name=u"Bourseaux Alexandre",
                  street=u'Aachener Straße',
                  street_no=21,
                  **kw)
    yield company(name=u"Baguette Stéphanie",
                  street=u'Gospertstraße',
                  street_no=24,
                  **kw)
    ...
    yield company(name="Schmitz Marc",
                  street='Rodter Straße',
                  street_no=43,
                  street_box="B",
                  **kw)
    ...
    sc.signer2 = jean
    yield sc
    yield role(company=cpas, person=michael, type=sc.signer1_function)
    yield role(company=cpas, person=jean, type=sc.signer2_function)
    ...
    cct = ClientContactType(**kw)
    yield cct
    kw = dict(client_contact_type=cct,
              country=belgium,
              city=eupen,
              title="Dr.")
    ...
    COACHINGTYPES = Cycler(
        rt.models.coachings.CoachingType.objects.filter(does_gss=False,
                                                        does_integ=False))
        ...
        if not person.birth_date:  # not those from humanlinks
            if User.objects.filter(partner=person).count() == 0:
                if contacts.Role.objects.filter(person=person).count() == 0:
                    birth_date = settings.SITE.demo_date(-170 * count -
                                                         16 * 365)
    ...
    def langk(person=None, language=None, **kwargs):
        ...
        try:
            obj = LanguageKnowledge.objects.get(person=person,
                                                language=language)
            for k, v in kwargs.items():
                setattr(obj, k, v)
        except LanguageKnowledge.DoesNotExist:
            obj = LanguageKnowledge(person=person, language=language, **kwargs)
        obj.full_clean()
        obj.save()
        return obj
    ...
    yield study(person=luc,
                type=schule,
                content=abi,
                start_date='19740901',
                end_date='19860630')
    yield study(person=gerd,
                type=schule,
                content=abi,
                start_date='19740901',
                end_date='19860630')
    ...
    obj = jobs.Offer(name="Übersetzer DE-FR (m/w)",
                     remark="""\
...
Flexibilität: die Termine sind je nach Kandidat anpassbar.""",
    ...
    AGENTS_SCATTERED = Cycler(alicia, hubert, melanie, caroline, hubert,
                              melanie, hubert, melanie)
    ...
    RECEPTION_CLIENTS = Cycler(reception.Clients.create_request(user=theresia))
