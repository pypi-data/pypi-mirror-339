...
def default_signer1():
    return settings.SITE.get_config_value('signer1')


def default_signer2():
    return settings.SITE.get_config_value('signer2')
...
class ContractTypeBase(mixins.BabelNamed):
    ...
    exam_policy = dd.ForeignKey("isip.ExamPolicy",
...
class ContractBase(Signers, Certifiable, EventGenerator, UserAuthored,
    ...
    date_decided = models.DateField(blank=True,
                                    null=True,
                                    verbose_name=_("date decided"))
    ...
    ending = dd.ForeignKey("isip.ContractEnding",
                           related_name="%(app_label)s_%(class)s_set",
                           blank=True,
                           null=True)
    ...
    def client_changed(self, ar):
        ...
        if self.client_id is not None:
            ...
            qs = self.client.get_coachings(
                (self.applies_from, self.applies_until), type__does_gss=True)
            if qs.count() == 1:
                user_asd = qs[0].user
    ...
    def update_reminders(self, ar):
        ...
        update_auto_task(self.TASKTYPE_CONTRACT_APPLIES_UNTIL, self.user, au,
                         _("Contract ends in a month"), self)
        return rv
...
class ContractPartnerBase(AbstractContactRelated, UploadController):
    ...
    reference_person = models.CharField(_("reference person"),
                                        max_length=200,
                                        blank=True)
        ...
        for (p2, con2) in self.actives:
            if con1 != con2 and overlap2(ap, p2):
                if con1.type.overlap_group and con1.type.overlap_group == con2.type.overlap_group:
                    ...
                    msg = msg.format(ctype=con2.__class__._meta.verbose_name,
                                     id=con2.pk)
                    return msg
        return None
...
class ContractBaseTable(dd.Table):
    ...
    def get_request_queryset(cls, ar):
        ...
        if ce and period is not None:
            if ce == ContractEvents.ended:
                ...
                qs = qs.filter(date_ended__gte=period[0],
                               applies_from__lte=period[1])
            else:
    ...
    def get_title_tags(self, ar):
        ...
        if pv.start_date is None or pv.end_date is None:
            ...
            if oe is not None:
                yield "%s %s-%s" % (str(oe.text), dd.dtos(
                    pv.start_date), dd.dtos(pv.end_date))
