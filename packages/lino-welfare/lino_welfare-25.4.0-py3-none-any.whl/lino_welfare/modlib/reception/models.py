...
class FindDateByClientTable(ButtonsTable):
    ...
    def get_data_rows(self, ar=None):
        ...
        for user in appointable_users(newcomer_appointments=True):
            ...
            sar = extensible.CalendarPanel.create_request(subst_user=user,
...
be selected manually."""
    ...
    parameters = dict(user=dd.ForeignKey(settings.SITE.user_model),
                      summary=models.CharField(verbose_name=_("Reason"),
                                               blank=True))
    params_layout = """
    user
    summary
    ...
    def run_from_ui(self, ar, **kw):
        ...
        kw.update(subst_user=ar.action_param_values.user,
...
class CreateClientVisit(dd.Action):
    ...
    parameters = dict(user=dd.ForeignKey(settings.SITE.user_model),
                      summary=models.CharField(verbose_name=_("Reason"),
                                               blank=True),
                      waiting_number=models.CharField(
                          verbose_name=_("Waiting number"), blank=True))
    params_layout = """
    user
    summary
    waiting_number
    ...
    def create_visit(self, ar, client):
        ...
        def msg():
            ...
            subject = subject.format(client=client,
                                     user=pv.user,
                                     number=pv.waiting_number)
            return (subject, '')
        ...
        rt.models.notify.Message.emit_notification(ar, None, mt, msg,
                                                   recipients)
...
class CreateNote(dd.Action):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        ar.confirm(
            ok,
            _("Client has no valid eID data!",
...
if False:
    ...
    # how it would look
    dd.inject_action('pcsw.Client', find_date_dlg=FindDateByClientDlg())
dd.inject_action('pcsw.Client',
                 find_date=dd.ShowSlaveTable(FindDateByClientTable))
    ...
    def get_data_rows(self, ar=None):
        ...
        if mi.client_state == ClientStates.coached:
            ...
            for u in appointable_users(cnd):
                ...
                yield coachings.Coaching(client=mi,
                                         user=u,
    ...
    def actions(cls, obj, ar):
        ...
        if client.client_state == ClientStates.coached \
           or user.newcomer_consultations:
            ...
            if False:  # apv are ignored, and it's ugly
                ...
                sar = ba.create_request(action_param_values=apv)
                sar.setup_from(ar)
        ...
        if client.client_state == ClientStates.coached \
            ...
            if not dd.plugins.extensible.is_hidden():
                sar = extensible.CalendarPanel.create_request(
                    subst_user=user, current_project=client.pk)
                elems += [
                    ar.href_to_request(
                        sar,
                        _("Find date"),
...
class CoachingsByClient(CoachingsByClient):
    ...
    def actions(cls, obj, ar):
        ...
        elems = []
        elems += [
            ar.instance_action_button(obj.create_visit,
                                      _("Visit"),
                                      icon_name=CreateClientVisit.icon_name),
            ' '
        ]
        ...
        if obj.user.user_type is not None:
            sar = extensible.CalendarPanel.create_request(subst_user=obj.user,
...
WaitingVisitors.column_names = (
    'since:15 client event__user:10 event__user__coaching_type:10 waiting_number:5 '
