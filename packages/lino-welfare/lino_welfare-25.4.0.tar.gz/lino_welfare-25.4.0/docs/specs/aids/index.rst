.. doctest docs/specs/aids/index.rst
.. _welfare.specs.aids:
.. _weleup.specs.aids:
...
The :mod:`lino_welfare.modlib.aids` plugin provides functionality for managing
"social aid" or "aid grantings".
...
.. glossary::
  ...
  aid granting
    ...
    The decision that a given client gets a given aid during a given
    period.  See `Aid grantings`_.

    German: Hilfebeschluss

  aid type

    Every :term:`aid granting` has a type, and the list of aid types is
    configurable. See `Aid types`_.
...
The list of :term:`aid type` can be modified by end users with the required
permissions.
...
.. class:: AidType
    ...
    The Django model representing an :term:`aid type`.
...
.. class:: Granting
    ...
    The Django model representing an :term:`aid granting`.
...
>>> rt.show(clients.ContactsByClient, granting.client)
==================== =============== =================== =============
...
There is only one pharmacy in above list, but in a confirmation I can manually
choose any other pharmacy:

>>> ContentType = rt.models.contenttypes.ContentType
>>> mt = ContentType.objects.get_for_model(rt.models.aids.Granting).id
>>> url = '/choices/aids/RefundConfirmationsByGranting/pharmacy?mt={0}&mk={1}'.format(mt, granting.id)
...
...     language='',
...     remark='')
>>> result = post_json_dict('rolf', url, data)  #doctest: +ELLIPSIS
>>> result.success
False
...
The text of a date range **with open end** can differ in certain languages
depending on whether it is in the future or in the past.
...
:mod:`lino.sphinxcontrib.help_texts_extractor` but aren't yet translated:
...
.. py2rst::
    ...
    url_tpl = ":srcref:`source code <lino_welfare/modlib/aids/config/aids/Confirmation/{}>`"
