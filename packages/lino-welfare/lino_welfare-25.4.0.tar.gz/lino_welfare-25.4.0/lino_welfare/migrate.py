...
- Fields Partner.bic and Partner.iban no longer exist. Check whether
  sepa.Account exists.
        ...
        if dd.is_installed('sepa'):
            ...
            def create_contacts_partner(id, modified, created, country_id,
                                        city_id, zip_code, region_id, addr1,
                                        street_prefix, street, street_no,
                                        street_box, addr2, name, language,
                                        email, url, phone, gsm, fax, remarks,
                                        is_obsolete, activity_id,
                                        client_contact_type_id, iban, bic):
            ...
            def check_sepa_accounts(loader):
                ...
                for pk, bic, iban in self.sepa_accounts:
                    try:
                        ...
                        obj.full_clean()
                        obj.save()
                        logger.info("20150825 created %s for partner #%s", obj,
                                    pk)
    ...
    def migrate_from_1_1_24(self, globals_dict):
        """
- ignore accounting, finan, vatless and sepa
...
"""
        ...
        def create_pcsw_client(
                person_ptr_id, national_id, nationality_id, card_number,
                card_valid_from, card_valid_until, card_type, card_issuer,
                noble_condition, group_id, birth_place, birth_country_id,
            ...
            return create_mti_child(
                contacts_Person,
                person_ptr_id,
                pcsw_Client,
                person_ptr_id=person_ptr_id,
                ...
                noble_condition=noble_condition,
                group_id=group_id,
                ...
                residence_until=residence_until,
                unemployed_since=unemployed_since,
                needs_residence_permit=needs_residence_permit,
                needs_work_permit=needs_work_permit,
                work_permit_suspended_until=work_permit_suspended_until,
                aid_type_id=aid_type_id,
                declared_name=declared_name,
                is_seeking=is_seeking,
                unavailable_until=unavailable_until,
                unavailable_why=unavailable_why,
                obstacles=obstacles,
                skills=skills,
                job_office_contact_id=job_office_contact_id,
                ...
                income_wg=income_wg,
                income_kg=income_kg,
                income_rente=income_rente,
                income_misc=income_misc,
                job_agents=job_agents,
                broker_id=broker_id,
                faculty_id=faculty_id,
                seeking_since=seeking_since)
        ...
        from lino_welfare.modlib.accounting.fixtures.std_journals import objects \
            as std_journals
        ...
        def create_accounts_account(id, ref, seqno, name, chart, group_id,
                                    type, sales_allowed, purchases_allowed,
                                    wages_allowed, clearings_allowed,
                                    clearable, required_for_household,
                                    required_for_person, periods,
                                    default_amount):
...
- remove all checkdata messages because checker names have changed
  (please run yourself checkdata after migration)
        ...
        def create_cal_eventtype(id, seqno, name, attach_to_email,
                                 email_template, description, is_appointment,
                                 all_rooms, locks_user, start_date,
                                 event_label, max_conflicting, invite_client,
                                 fse_field):
            kw = dict()
            kw.update(id=id)
            kw.update(seqno=seqno)
        ...
        def create_pcsw_client(
                ...
                obstacles, skills, job_office_contact_id, client_state,
                refusal_reason, remarks2, gesdos_id, tim_id, is_cpas,
                is_senior, health_insurance_id, pharmacy_id, income_ag,
                income_wg, income_kg, income_rente, income_misc, job_agents,
                broker_id, faculty_id, has_fse):
            return create_mti_child(
                contacts_Person,
                person_ptr_id,
                pcsw_Client,
        ...
        if not dd.is_installed('accounting'):
            # installed in weleup, not installed in welcht
            return '1.1.26'

        ledger_Journal = rt.models.accounting.Journal

        @override(globals_dict)
        def create_ledger_journal(id, ref, build_method, template, seqno, name,
                                  trade_type, voucher_type, journal_group,
                                  auto_check_clearings, force_sequence,
                                  account_id, printed_name, dc):
            kw = dict()
            kw.update(id=id)
            kw.update(ref=ref)
            ...
            if printed_name is not None:
                kw.update(bv2kw('printed_name', printed_name))
            kw.update(dc=dc)
    ...
    def migrate_from_1_1_26(self, globals_dict):
        ...
        if dd.is_installed('xcourses'):
            globals_dict.update(
                ...
                courses_CourseProvider=resolve_model(
    ...
    def migrate_from_18_11_0(self, globals_dict):
        ...
        Checkers.items_dict['pcsw.SSINChecker'] = Checkers.get_by_value(
            'pcsw.IdentityChecker')
        if dd.is_installed('properties'):
            # bv2kw = globals_dict['bv2kw']
            properties_PersonProperty = rt.models.cv.PersonProperty

            @override(globals_dict)
            def create_properties_personproperty(id, group_id, property_id,
                                                 value, person_id, remark):
                kw = dict()
                kw.update(id=id)
                kw.update(group_id=group_id)
