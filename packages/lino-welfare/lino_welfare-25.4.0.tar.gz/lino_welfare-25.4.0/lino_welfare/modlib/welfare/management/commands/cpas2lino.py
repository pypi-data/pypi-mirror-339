...
def wanted_accounts():
    ...
    AccountTypes = rt.models.accounting.AccountTypes
...
class TimLoader(TimLoader):
    ...
    def unused_load_jnl_alias(self, row, **kw):
        if row.alias == 'IMP':
            ...
            if idgrj == "AAW":
                ...
                kw.update(trade_type=accounting.TradeTypes.purchases)
            elif idgrj == "TRE":
                if row.compte.strip():
                ...
                return None, kw
            kw.update(
                journal_group=accounting.JournalGroups.get_by_name(idgrj.lower()))
            return vcl, kw
    ...
    def row2account(self, row):
        ...
        acc = accounting.Account.get_by_ref(ref, None)
        if acc is None:
    ...
    def unused_load_bud(self, row, **kw):
        ...
        if row.dc == "A":
            kw.update(type=accounting.AccountTypes.expenses)
        else:
            kw.update(type=accounting.AccountTypes.incomes)
        self.babel2kw('name', 'name', row, kw)
        obj = accounting.Account(**kw)
        # dd.logger.info("20160302 %s", dd.obj2str(obj))
        yield obj

    def fiscal_year(self, per):
        return periods.StoredYear.get_by_ref(per[:2])
    ...
    def load_imp(self, row, **kw):
        jnl = accounting.Journal.get_by_ref(row.idjnl.strip(), None)
        if jnl is None:
            return
        if jnl.journal_group != accounting.JournalGroups.anw:
            return
        ...
        if username:
            try:
                ...
                if row.idusr.strip() not in self.unknown_users:
                    ...
                    dd.logger.warning("%s %s : Unknown username '%s'",
        ...
        if issubclass(voucher_model, vatless.AccountInvoice):
            ...
            if compte:
                if row.compte1.strip() not in self.ignored_bank_accounts:
                    ...
                    dd.logger.warning("%s %s : Ignoring bank account '%s'",
                                      row.idjnl, row.iddoc, row.compte1)
        else:
            raise Exception("Unknown voucher_model {}".format(voucher_model))
    ...
    def load_iml(self, row, **kw):
        ...
        jnl = accounting.Journal.get_by_ref(idjnl, None)
        if jnl is None:
    ...
    def write_report(self):
        ...
        for k in ('ignored_journals', 'ignored_accounts', 'rows_by_year',
                  'ignored_matches', 'ignored_partners', 'missing_partners',
                  'missing_clients', 'unknown_users',
                  'undefined_bank_accounts', 'ignored_bank_accounts'):
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # from lino_xl.lib.accounting.mixins import VoucherItem
        # models = list(rt.models_by_base(rt.models.accounting.Voucher))
        ...
        # # models.append(rt.models.accounting.Journal)
        # models.append(rt.models.accounting.Voucher)
        models.append(rt.models.accounting.Movement)
        if True:
            for m in models:
                qs = m.objects.all()
