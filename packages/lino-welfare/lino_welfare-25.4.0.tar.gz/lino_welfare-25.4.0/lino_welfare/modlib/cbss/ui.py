...
class IdentifyPersonRequestInsert(dd.InsertLayout,
                                  IdentifyPersonRequestDetail):
    window_size = (60, 'auto')
    ...
    def check_permission(cls, obj, ar):
        ...
        if obj.user != u:
            if not u.user_type.has_required_roles([SecurityAdvisor]):
                ...
                raise Warning(_("Confidential data"))
...
class IdentifyPersonResult(ConfidentialResultsTable):
    ...
    def get_data_rows(self, ar):
        ...
        for obj in results:
            data = dict()
            data.update(national_id=nodetext(
            ...
            data.update(birth_date=cbss2date(
                nodetext(obj.childAtPath('/Basic/BirthDate'))))
            data.update(civil_state=cbss2civilstate(
                obj.childAtPath('/Extended/CivilState')))
            data.update(birth_location=nodetext(
