...
from lino.mixins.periods import rangefmt, DateRange
...
class RevokeConfirmation(dd.Action):
    ...
    def get_action_permission(self, ar, obj, state):
        user = ar.get_user()
        if obj.signer != user and not user.user_type.has_required_roles(
            [AidsStaff]):
            return False
...
class Confirmable(DateRange):
    ...
    def on_analyze(cls, site):
        cls.CONFIRMED_FIELDS = dd.fields_list(cls,
    ...
    def setup_parameters(cls, fields):
        fields.update(
            signer=dd.ForeignKey(settings.SITE.user_model,
                                 verbose_name=pgettext("aids", "Signer"),
                                 blank=True,
                                 null=True))
    ...
    def get_printable_context(self, ar=None, **kw):
        ...
        return super().get_printable_context(ar, **kw)
...
class Confirmation(Confirmable, UserAuthored, ContactRelated, mixins.Created,
                   Certifiable):
    ...
    def get_date_range_veto(obj):
        ...
        if obj.start_date or obj.end_date:
            ...
            if not encompass(gp, cp):
                return _("Date range %(p1)s lies outside of granted "
                         "period %(p2)s.") % dict(p2=rangefmt(gp),
                                                  p1=rangefmt(cp))
    ...
    def get_excerpt_options(self, ar, **kw):
        ...
        return super().get_excerpt_options(ar, **kw)
