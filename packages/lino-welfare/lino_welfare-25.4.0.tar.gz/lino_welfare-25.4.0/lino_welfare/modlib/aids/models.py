...
class AidType(ContactRelated, ExcerptTitle):
    ...
    confirmed_by_primary_coach = models.BooleanField(
        _("Confirmed by primary coach"),
        default=True,
        help_text=
        _("Whether grantings for this aid type are to be signed by the client's primary coach."
          ))

    pharmacy_type = dd.ForeignKey('clients.ClientContactType',
                                  blank=True,
                                  null=True)
    ...
    body_template = models.CharField(
        ...
        blank=True,
        help_text="The body template to use instead of the "
...
class Granting(Confirmable, BoardDecision):
    ...
    request_date = models.DateField(_("Date of request"),
                                    blank=True,
                                    null=True)
    ...
    def custom_actions(self, ar, **kw):
        ...
        if not ct:
            return ''
        sar = ct.table_class.insert_action.request_from(ar,
...
dd.update_field(ConfirmationsByGranting,
                'detail_link',
...
class AllSimpleConfirmations(SimpleConfirmations):
    required_roles = dd.login_required(AidsStaff)
    column_names = ("id client start_date end_date granting "
...
class IncomeConfirmationsByGranting(IncomeConfirmations):
    ...
    """,
                                    window_size=(70, 20),
                                    hidden_elements='client granting')
...
dd.inject_field(
    'clients.ClientContactType', 'can_refund',
    models.BooleanField(_("Can refund"),
                        default=False,
...
class RefundConfirmation(Confirmation):
    ...
    doctor_type = dd.ForeignKey('clients.ClientContactType',
                                verbose_name=_("Doctor type"),
                                blank=True)
