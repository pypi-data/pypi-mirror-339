...
class Group(mixins.BabelNamed):
    ...
    ref = models.CharField(max_length=settings.SITE.plugins.debts.ref_length,
                           blank=True,
                           null=True,
                           unique=True)
...
class Account(mixins.BabelNamed, mixins.Sequenced, mixins.Referrable):
    ...
    required_for_household = models.BooleanField(_("Required for Households"),
                                                 default=False)
    required_for_person = models.BooleanField(_("Required for Persons"),
                                              default=False)
...
class Budget(UserAuthored, Certifiable, mixins.Duplicable):
    ...
    print_todos = models.BooleanField(_("Print to-do list"),
                                      default=False,
                                      help_text=_("""\
...
Vielleicht mit Fußnoten?"""))
    ...
    dist_amount = dd.PriceField(_("Distributable amount"),
                                default=120,
                                help_text=_(
    ...
    def unused_entries_by_group(self, ar, group, **kw):
        """Return an action request showing the entries of this budget for the
        ...
        ar = ar.spawn(t,
                      ...
                      title=str(group),
                      filter=models.Q(account__group=group),
                      **kw)
...
class Entry(SequencedBudgetComponent):
    ...
    distribute = models.BooleanField(_("Distribute"),
                                     default=False,
                                     help_text=u"""\
Ob diese Schuld in die Schuldenverteilung aufgenommen wird oder nicht.""")
    todo = models.CharField(verbose_name=_("To Do"),
                            max_length=200,
                            blank=True)
...
bei jährlichen Ausgaben 12.""")
    monthly_rate = dd.PriceField(_("Monthly rate"),
                                 default=0,
                                 help_text=u"""
    ...
    def full_clean(self, *args, **kw):
        ...
        if self.distribute and self.monthly_rate:
            raise ValidationError(
                ...
                _("Cannot set 'Distribute' when 'Monthly rate' is %r") %
                self.monthly_rate)
...
dd.inject_field(
    'clients.ClientContactType', 'is_bailiff',
    models.BooleanField(_("Debt collection agency"), default=False))
...
dd.inject_field(
    'system.SiteConfig', 'master_budget',
    dd.ForeignKey("debts.Budget",
                  blank=True,
                  null=True,
...
def site_setup(site):
    for T in (  # site.modules.contacts.Partners,
            site.modules.contacts.Persons, site.modules.pcsw.Clients,
...
# There are no message extractors for `.odt` files. One workaround is to
# manually repeat translatable messages here so that :command:`inv mm` finds
# them.
