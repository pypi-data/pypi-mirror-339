...
class EntryGroup(TableRow):
    ...
    def __init__(self, budget, group, ar):
        self.budget = budget
        self.group = group
        self.action_request = ar.spawn(PrintEntriesByBudget,
...
class ResultByBudget(SummaryTable):
    ...
    def get_summary_numbers(self, ar):
        ...
        yield [
            _("Monthly installment for running credits"),
    ...
    def get_summary_numbers(self, ar):
        ...
        for eg in budget.entry_groups(ar, 'L'):
            ...
            for acc in eg.group.account_set.all():
                yield [
                    _("%s (distributable)") % dd.babelattr(acc, 'name'),
                    budget.sum('amount', account=acc, distribute=True)
                ]
                yield [
                    dd.babelattr(acc, 'name'),
...
""")
    ...
    @classmethod
    def override_column_headers(self, ar, **headers):
        headers.update(partner=_("Creditor"))
        headers.update(amount=_("Debt"))
        return headers
