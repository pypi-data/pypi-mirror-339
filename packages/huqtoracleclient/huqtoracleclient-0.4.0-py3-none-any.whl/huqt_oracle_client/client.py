...
class AdvancedTrader(ABC):
    def __init__(self, account, api_key, url, secure = True):
        ...
        self.eStub = exchange_pb2_grpc.ExchangeStub(self.channel)
    ...
    def submit_order(self, symbol: str, logging: str, size: int, price: int, side: OrderSide, tif: OrderTif) -> Response:
        return submit_order(self.eStub, symbol, logging, size, price, side, tif, self.account, self.api_key)
    
    def cancel_order(self, order_id: str) -> Response:
        return cancel_order(self.eStub, order_id, self.account, self.api_key)
    
    def cancel_orders(self, order_ids: List[str]) -> ResponseList:
        return cancel_orders(self.eStub, order_ids, self.account, self.api_key)
    
    @abstractmethod
    def handle_md_update(self, data: MarketData):
        pass
    
    @abstractmethod
    def handle_open_orders_update(self, data: List[Order]):
        pass
    
    @abstractmethod
    def handle_positions_update(self, data: List[Position]):
        pass
    
    @abstractmethod
    def handle_fill_update(self, data: Fill):
        pass
    
    @abstractmethod
    def handle_recent_fills(self, data: List[Fill]):
        pass
        ...
        try:
            ...
            async for market_data in stream:
                ...
                if field == "orderbook":
                    ...
                    self.handle_md_update(MarketData(symbol, tape, book, int(orderbook.volume)))
                elif field == "orders":
                    ...
                    self.handle_open_orders_update(orders)
...
def submit_order(stub: exchange_pb2_grpc.ExchangeStub, symbol: str, logging: str, size: int, price: int, side: OrderSide, tif: OrderTif, account: str, api_key: str) -> Response: 
