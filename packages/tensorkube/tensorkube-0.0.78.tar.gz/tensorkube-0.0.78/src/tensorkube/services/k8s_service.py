...
def set_stop_event_post_ksvc_ready(service_name, namespace, stop_event, context_name: Optional[str] = None):
    ...
    max_wait_seconds = 600  # maximum time to wait (adjust as needed)
...
def generate_kubeconfig():
    ...
    kubeconfig_data = {"apiVersion": "v1", "clusters": [
        {"cluster": {"server": endpoint, "certificate-authority-data": cluster_ca_certificate},
            "name": f'arn:aws:eks:us-east-1:{get_aws_account_id()}:cluster/{cluster_name}'}], "contexts": [{
        "context": {"cluster": f'arn:aws:eks:us-east-1:{get_aws_account_id()}:cluster/{cluster_name}',
        ...
        "current-context": f'arn:aws:eks:us-east-1:{get_aws_account_id()}:cluster/{cluster_name}', "kind": "Config",
        "users": [{"name": f'arn:aws:eks:us-east-1:{get_aws_account_id()}:cluster/{cluster_name}', "user": {
            "exec": {"apiVersion": "client.authentication.k8s.io/v1beta1", "command": "aws",
                "args": ["eks", "get-token", "--cluster-name", cluster_name],
                "env": [{"name": "AWS_DEFAULT_REGION", "value": REGION}]}}}]}
