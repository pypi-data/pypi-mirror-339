import os
import shutil
from .AOT_Acoustic import load_fieldKWAVE_XZ, hex_to_binary_array
import numpy as np
import re
import time
...
def getAOsignal(laser, acoustic_envelope_squaredMATRIX):
    AOsignal = np.zeros((acoustic_envelope_squaredMATRIX.shape[0], acoustic_envelope_squaredMATRIX.shape[1]))
    for i in range (acoustic_envelope_squaredMATRIX.shape[0]):
        for t in range(acoustic_envelope_squaredMATRIX.shape[1]):
            interaction = laser.T * acoustic_envelope_squaredMATRIX[i, t, :, :]
            AOsignal[i, t] = np.sum(interaction)
    return AOsignal

def getSaveAOsignal(listFieldPathHDR, laser, data_path, save_directory):
    #vider le dossier save_directory
    if os.path.exists(save_directory):
        shutil.rmtree(save_directory)
    os.makedirs(save_directory)
    os.makedirs(os.path.join(save_directory, "system_matrix"))
    new_listFieldPathHDR = []
    for num, file in enumerate(listFieldPathHDR):
        src_hdr = os.path.join(data_path, file)
        src_img = os.path.join(data_path, file.replace('.hdr', '.img'))
        # Chemin du fichier de destination
        dest_hdr = os.path.join(os.path.join(save_directory, "system_matrix"), file.replace('KWAVE', 'field'))
        dest_img = os.path.join(os.path.join(save_directory, "system_matrix"), file.replace('KWAVE', 'field').replace('.hdr', '.img'))
        new_listFieldPathHDR.append(dest_hdr)
        # Copier les fichiers
        if not os.path.exists(dest_hdr):
            shutil.copy(src_hdr, dest_hdr)
            while not (os.path.exists(dest_hdr)):
                time.sleep(0.1)  # Attendre un peu avant de revérifier
        if not os.path.exists(dest_img):
            shutil.copy(src_img, dest_img)
            while not (os.path.exists(dest_img)):
                time.sleep(0.1)  # Attendre un peu avant de revérifier

        with open(dest_hdr, 'r') as file:
            contenu = file.read()
            
        # Remplacer la séquence "system_matrix/KWAVE_" par "system_matrix/field_"
        contenu_modifie = contenu.replace("system_matrix/KWAVE_", "system_matrix/field_")

        # Écrire les modifications dans le fichier copié
        with open(dest_hdr, 'w') as file:
            file.write(contenu_modifie) 
    
    acoustic_envelope_squared = []
    for nScan in range(len(new_listFieldPathHDR)):  
        acoustic_envelope_squared.append(load_fieldKWAVE_XZ(os.path.join(save_directory,new_listFieldPathHDR[nScan])))
        print(f"shape du {nScan} field {acoustic_envelope_squared[nScan].shape}")
    acoustic_envelope_squared = np.array(acoustic_envelope_squared)
    print(f"acoustic enveloppe shape : {acoustic_envelope_squared.shape}")
    print(f" optique shape : {laser.shape}")
    AOsignal = getAOsignal(laser, acoustic_envelope_squared)
    ...
    # Extraire l'active list
        start = listFieldPathHDR[i].find("KWAVE_") + len("KWAVE_")
    ...
    save_AOsignal(AOsignal, save_directory, angles, acoustic_envelope_squared.shape[1], activeListarray)
...
def save_AOsignal(AOsignal, save_directory, angles, Nt, active_list):
    ...
    nScan = AOsignal.shape[0]  # Nombre de scans ou d'événements
