...
{str(additional_args)}."""
    ...
    def save(self, output_dir: str, relative_path: Optional[str] = None):
        ...
        app_template = textwrap.dedent("""
            import yaml
            import os
            from AgentEngine.smolagent import GradioUI, {{ class_name }}, {{ agent_dict['model']['class'] }}
    ...
    def from_folder(cls, folder: Union[str, Path], **kwargs):
        ...
        for managed_agent_name, managed_agent_class in agent_dict["managed_agents"].items():
            agent_cls = getattr(importlib.import_module("AgentEngine.framework.agents"), managed_agent_class)
        ...
        model_class: Model = getattr(importlib.import_module("AgentEngine.framework.models"), agent_dict["model"]["class"])
    ...
    def push_to_hub(
        ...
        metadata_update(
            repo_id,
            {"tags": ["AgentEngine.framework", "agent"]},
            repo_type="space",
            token=token,
            overwrite=True,
        )
