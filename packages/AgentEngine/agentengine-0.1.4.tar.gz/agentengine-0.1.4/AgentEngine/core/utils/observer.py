...
# class AgentNewRunTransformer(MessageTransformer):
#     TEMPLATES = {
#         "zh": ""
#         # "zh": "当前 `{0}` 正在执行 `{1}` 任务",
#         # "en": "Current `{0}` is executing `{1}` task"
#     }
#
#     def transform(self, **kwargs: Any) -> str:
#         """转换agent新运行的消息"""
...
#         template = self.TEMPLATES.get(lang, self.TEMPLATES["zh"])
#         return template.format(agent_name, content)
...
class MessageObserver:
    ...
    def _init_message_transformers(self):
        ...
        self.transformers = {
            ...
            ProcessType.TOKEN_COUNT: default_transformer,
