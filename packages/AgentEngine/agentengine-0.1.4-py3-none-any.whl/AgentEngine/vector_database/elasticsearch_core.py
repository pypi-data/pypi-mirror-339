...
from AgentEngine.vector_database.utils import format_size, format_timestamp, print_index_info, format_search_results
...
class ElasticSearchCore:
    ...
    def __init__(
        ...
        embedding_model: Optional[JinaEmbedding] = None,
        ...
        self.host = host or os.getenv('ELASTICSEARCH_HOST')
        self.api_key = api_key or os.getenv('ELASTICSEARCH_API_KEY')
        
        if not all([self.host, self.api_key]):
            raise ValueError("Missing Elasticsearch configuration. Check host and API key.")
        ...
        self.embedding_model = embedding_model or JinaEmbedding()
    ...
    def get_active_tasks_status(self, index_name: str) -> Dict[str, str]:
        """
        Get active tasks status from data cleanse service for a specific index.
        ...
        try:
            ...
            # Get data cleanse service URL from environment
            data_cleanse_service = os.getenv('DATA_CLEANSE_SERVICE', 'http://localhost:8001')
            url = urljoin(data_cleanse_service, f'/indices/{index_name}/tasks')

            # Make request to data cleanse service
...
def main():
    """
    Main function to demonstrate ElasticSearchCore capabilities.
    """
    # Initialize the ElasticSearchCore (test KB created automatically)
    es_core = ElasticSearchCore()
    
    # Index names for our operations
    index_names = ["sample_articles"]
    
    # List all user indices
    print("\nListing all user indices...")
    user_indices = es_core.get_user_indices()
    for idx in user_indices:
        print(f"  - {idx}")
    
    # Demonstrate search capabilities
    if all(idx in user_indices for idx in index_names):
        # Perform accurate search
        print("\nPerforming accurate text search...")
        query = "Doctor"
        accurate_results = es_core.accurate_search(index_names, query, top_k=2)
        format_search_results(accurate_results, f"Accurate search: '{query}'")
        
        # Perform semantic search
        print("\nPerforming semantic vector search...")
        query = "medical professionals in London"
        semantic_results = es_core.semantic_search(index_names, query, top_k=2)
        format_search_results(semantic_results, f"Semantic search: '{query}'")

        # Perform hybrid search
        print("\nPerforming hybrid vector search...")
        query = "medical professionals in London"
        hybrid_results = es_core.hybrid_search(index_names, query, top_k=2)
        format_search_results(hybrid_results, f"Hybrid search: '{query}'")

        # Get consolidated index info
        print("\nGetting comprehensive index information...")
        stats = es_core.get_index_stats(index_names)
        mappings = es_core.get_index_mapping(index_names)
        
        for index_name in index_names:
            print(f"\n=== Index: {index_name} ===")
            print(f"\nBase Info:")
            for metric_name, value in stats[index_name]["base_info"].items():
                print(f"  {metric_name}: {value}")

            print(f"\nSearch Performance:")
            for metric_name, value in stats[index_name]["search_performance"].items():
                print(f"  {metric_name}: {value}")

            print(f"\nFields: {', '.join(mappings[index_name])}")
    
    print("\nExample completed successfully!")


if __name__ == "__main__":
    # Set init_test_kb to False if you don't want to create test knowledge base
    # via environment variable
    init_test_kb = os.getenv("CREATE_TEST_KB", "True").lower() in ("true", "1", "yes")
    es_core = ElasticSearchCore(init_test_kb=init_test_kb)
