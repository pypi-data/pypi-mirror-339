...
class HistoryAPIEndpoints:
    ...
    def __init__(self, api: NinjaAPI) -> None:
        ...
        @api.get("/history/skills", response=SkillHistoryResponse, tags=self.tags)
        def skill_history(request, character_id: int):
            if not request.user.has_perm("incursions.waitlist_history_view"):
                logger.warning(f"User {request.user} denied access to skill history for character {character_id}")
                return 403, {"error": "Permission denied"}

            history_list: list[SkillHistoryResponseLineSchema] = []
            skills = SkillHistory.objects.filter(character_id=character_id).select_related("skill__group__category").order_by("-pk")
            for skill in skills:
            ...
            ids = {
                skill.name: skill.type_id
                for skill in EveItemType.objects.filter(group__category_id=EVECATEGORY_SKILLS).only("name", "type_id")
            }

            logger.info(f"Skill history returned for character {character_id} with {len(history_list)} entries")
            return {"history": history_list, "ids": ids}
