...
class AuthAPIEndpoints:
    ...
    def __init__(self, api: NinjaAPI) -> None:
        ...
        def whoami(request) -> tuple[int, WhoamiResponse | dict]:
            ...
            return 200, WhoamiResponse(account_id=main_character.character_id, access=access_levels, characters=character_schemas)

        # @api.get("/auth/logout", tags=self.tags)
        # def logout(request) -> dict:
        #     logger.debug(f"Logout endpoint hit for user: {request.user}")
        #     return {"status": "Logged out"}

        # @api.get("/auth/login_url", tags=self.tags)
        # def login_url(request, alt: bool = False, fc: bool = False) -> dict:
        #     state: str = "alt" if alt else "normal"
        #     scopes: list[str] = ["publicData", "skills.read_skills", "clones.read_implants"]

        #     if fc:
        #         scopes += ["fleets.read_fleet", "fleets.write_fleet", "ui.open_window", "search"]
        ...
        #     logger.debug(f"Generated login URL for user: {request.user}, fc={fc}, alt={alt}")
