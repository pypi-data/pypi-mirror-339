...
class BadgesAPIEndpoints:
    ...
    def __init__(self, api: NinjaAPI) -> None:
        ...
        def list_badges(request) -> tuple[int, list[BadgeSchema] | dict]:
            ...
            logger.info(f"Returning {len(badges)} badges to user {request.user}")
            return 200, badges
        ...
        def get_badge_members(request, badge_id: int) -> tuple[int, list[CharacterBadgesSchema] | dict]:
            if not (request.user.has_perm("incursions.waitlist_badges_manage") or request.user.has_perm("incursions.waitlist_badges_view")):
                logger.warning(f"Permission denied to view badge members for user {request.user}")
                return 403, {"error": "Permission denied"}

            characters = CharacterBadges.objects.filter(badge_id=badge_id).select_related("character", "granted_by", "badge")
            if not characters.exists():
                logger.error(f"No members found for badge:{badge_id}")
                return 200, []
