...
class AnnouncementAPIEndpoints:
    ...
    def __init__(self, api: NinjaAPI) -> None:
        ...
        def create_announcement(request, payload: RequestPayloadSchema) -> tuple[int, dict]:
            if not request.user.has_perm("incursions.manage_announcements"):
        ...
        def update_announcement(request, announcement_id: int, payload: RequestPayloadSchema) -> tuple[int, dict]:
            ...
            announcement = Announcement.objects.filter(id=announcement_id, revoked_at__isnull=True).first()
            if not announcement:
            ...
            sse_client.submit([SSEEvent.new_json(topic="announcements", event="announcement;updated", data=AnnouncementSchema.from_orm(announcement))])
        ...
        def revoke_announcement(request, announcement_id: int) -> tuple[int, dict]:
            ...
            announcement.revoked_by = request.user
            announcement.revoked_at = datetime.now(timezone.utc)
            announcement.save()
