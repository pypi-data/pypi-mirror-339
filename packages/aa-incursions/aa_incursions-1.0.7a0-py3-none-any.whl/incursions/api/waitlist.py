...
from incursions.models.waitlist import (
    ActiveFleet, ApprovedFitting, Fitting, FleetSquad, ImplantSet, Waitlist,
    WaitlistCategory, WaitlistCategoryRule, WaitlistEntry, WaitlistEntryFit,
)
    ...
    def __init__(self, api: NinjaAPI) -> None:
        ...
        def get_waitlist(request: HttpRequest) -> WaitlistResponse | tuple[int, dict]:
            ...
            for waitlist_entry in waitlist_entries:
                ...
                for wl_fit in WaitlistEntryFit.objects.filter(waitlist_entry=waitlist_entry).select_related("implant_set","fit", "category", "character"):
                    ...
                    # try:
                    #     fit_analysis_data = json.loads(wl_fit.fit_analysis)
                    # except Exception as e:
                    #     logger.error("Error parsing fit_analysis for fit %s: %s", wl_fit.pk, e)
                    ...
                    fits_data.append(
                        WaitlistEntryFitSchema(
                            ...
                            ship=HullSchema(
                                id=wl_fit.fit.ship.type_id if (is_self or context_wl_approver or context_ship) else 0,
        ...
        def xup(request: HttpRequest, payload: XupRequest) -> tuple[int, dict]:
            ...
            waitlist = get_object_or_404(Waitlist, pk=payload.waitlist_id)
            ...
            with transaction.atomic():
                ...
                waitlist_entry, _ = WaitlistEntry.objects.get_or_create(main_character=character, waitlist=waitlist, defaults={"joined_at": now()})
        ...
        def remove_x(request: HttpRequest, payload: RemoveXRequest) -> tuple[int, dict]:
            logger.info("Removing waitlist entry x with id %s", payload.id)
            waitlist_entry = get_object_or_404(WaitlistEntry, pk=payload.id)
            waitlist = get_object_or_404(Waitlist, pk=waitlist_entry.waitlist.pk)
            if not (
                waitlist_entry.main_character == get_main_character_from_user(request.user)
                or request.user.has_perm("incursions.manage_waitlist")
            ):
        ...
        def remove_fit(request: HttpRequest, payload: RemoveFitRequest) -> tuple[int, dict]:
            logger.info("Removing fit with id %s", payload.id)
            entry = get_object_or_404(WaitlistEntryFit, pk=payload.id)
            waitlist_entry = get_object_or_404(WaitlistEntry, pk=entry.waitlist_entry.pk)
            waitlist = get_object_or_404(Waitlist, pk=waitlist_entry.waitlist.pk)
            ...
            with transaction.atomic():
                entry.delete()
                if not WaitlistEntryFit.objects.filter(waitlist_entry=waitlist_entry).exists():
                    waitlist_entry.delete()
            sse_payload = WaitlistUpdateSchema(waitlist_id=waitlist.pk).model_dump()
        ...
        def invite(request: HttpRequest, payload: InviteRequest) -> dict:
            logger.info("Inviting character %s for waitlist entry fit id %s", payload.character_id, payload.id)
            ...
            invited_char = get_main_character_from_evecharacter(waitlist_entry_fit.character)
            sse_client.submit([SSEEvent.new(
                topic=f"account;{invited_char.character_id}",
                event="wakeup",
                data=f"{active_fleet.fleet.boss.character_name} has invited your {waitlist_entry_fit.fit.ship.name} to fleet.",
            )])
            logger.info("Character %s invited for waitlist entry %s", payload.character_id, payload.id)
        ...
        def approve_fit(request: HttpRequest, payload: ApproveRequest) -> dict:
            ...
            if not (
                request.user.has_perm("incursions.manage_waitlist")
        ...
        def set_open(request: HttpRequest, payload: SetOpenRequest) -> tuple[int, str | dict]:
            ...
            if not request.user.has_perm("incursions.manage_waitlist"):
                ...
                return 403, {"error": "Permission denied"}
            with transaction.atomic():
                w = get_object_or_404(Waitlist, pk=payload.waitlist_id)
                w.is_open = payload.open
                w.save(update_fields=["is_open"])
            sse_payload = WaitlistUpdateSchema(waitlist_id=w.pk).model_dump()
            sse_client.submit(
