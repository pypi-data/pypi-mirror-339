...
class SuperPage:
    ...
    def create_notification_animation(self):
        self.notif_image = ImageTk.PhotoImage(Image.open(
            self.settings.imgsysdir + 'kpp_record_already_close_win.png'))
        start_pos = -100
    ...
    def rotate_gate_arrow(self, arrow_attr, pos, course, step=1, endpos=85,
        ...
        init_page_obg = self.operator.currentPage
        ...
        while arrow_info['pos'] != endpos:
            ...
            if self.operator.current in allow_pages:
                self.draw_set_arrow(arrow_attr, spec_tag="rotate")
            arrow_info['pos'] += step
            sleep(sleeptime)
        #if init_page != self.operator.current:
        if self.operator.current in allow_pages:
            self.draw_set_arrow(arrow_attr)
        else:
            self.can.delete("rotate")
       #     init_page_obg.can.delete(arrow_attr, "arrow_in_move")
        #if init_page != self.operator.current or self.blockImgDrawn:
            #init_page.can.delete(arrow_attr)
           # self.can.delete(arrow_attr)
    ...
    def draw_set_arrow(self, arrow_attr, spec_tag: str = None):
        ...
        try:
            ...
            self.kpp_barrier_arrows_tag[arrow_attr] = \
            self.kpp_barrier_arrows_tag[arrow_attr][:2]
        except IndexError:
            pass
        self.can.create_image(obj[1], obj[2], image=tkimage, tags=tags)
