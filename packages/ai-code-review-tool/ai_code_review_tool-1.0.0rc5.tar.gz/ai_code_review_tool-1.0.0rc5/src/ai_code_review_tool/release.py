...
import sys
import typer
from src.ai_code_review_tool.version_utils import bump_version, get_project_version, set_project_version
from src.ai_code_review_tool.git_utils import get_changed_files, generate_commit_message, commit_and_push
...
def prerelease_workflow(bump: str = typer.Option("rc", help="Bump type for prerelease")):
    ...
    subprocess.run([sys.executable, "main.py", "generate-docs"], check=True)
    subprocess.run([sys.executable, "main.py", "update-requirements"], check=True)
    subprocess.run([sys.executable, "main.py", "take-snapshot"], check=True)
