"""
The Bi-Objective Dynamic Systems Identification (BODSI) toolkit contains
...
The BODSI class has the following associated functions (methods):
    ...
    For more information, use help BODSI in the command prompt.
     To access the functions, use "function name", e.g.:
    >> help('getInfo')
    
    The package provides data of a DC-DC converter and an example file
    exempleBODSI.py and two other examples..
...
Bi-Objective Dynamic Systems Identification Package, Version 1.0.4
...
from numpy import (array, prod, sort, max, any, zeros_like, round, ones, mean, 
                   ...
                   isnan, full, bincount, column_stack)
...
def correlationDecisionMaker(model, PARAMETERS, uv, yv):
    ...
    r = full(c, 1000)  # Initialize correlation array with high values
    ...
    for k in range(c):
        ...
        if not isinf(PARAMETERS[:, k]).any():  # Skip invalid parameter sets
            Y = simulateModel(model, PARAMETERS[:, k], uv, yv[:lag])  # Simulate model
