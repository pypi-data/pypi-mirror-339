...
def compute_membranes_from_junctions(embryo_folder, junctionimput, reducvoxelsize=0.3, target_normalization_max=255,
    ...
    folder_contour_name = "FUSE_"+str(contour_suffix)+"_MEMBRANES"
    if flat_normalization:
        folder_contour_name = "FUSE_"+str(contour_suffix)+"_MEMBRANES_FLAT"
    folder_contour = os.path.join(embryo_folder, "FUSE/"+folder_contour_name+"/")
    ...
    for image in tqdm(res): # Process the contour creation for each image
        ...
        generate_contour(image, image_filled, vsize, folder_contour, "identity", sigma=2, connectivity=1,
                         target_normalization_max=target_normalization_max,replace_in_name="_junctions",replace_to="_fuse",flat_normalization=flat_normalization)  # Generating the contour
