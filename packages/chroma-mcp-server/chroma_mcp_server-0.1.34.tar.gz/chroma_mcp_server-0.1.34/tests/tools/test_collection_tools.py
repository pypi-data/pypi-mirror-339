...
class TestCollectionTools:
    ...
    async def test_set_collection_settings_fail_immutable(self, mock_chroma_client_collections):
        ...
        # Expect McpError because handle_chroma_error wraps ValidationError
        with pytest.raises(McpError) as exc_info: 
            await _set_collection_settings_impl("test_immutable_settings", {"new_setting": True})
        
        assert "Cannot set settings on collections with existing immutable settings" in str(exc_info.value)
    ...
    async def test_update_collection_metadata_fail_immutable(self, mock_chroma_client_collections):
        ...
        # Expect McpError because handle_chroma_error wraps ValidationError
        with pytest.raises(McpError) as exc_info: 
            await _update_collection_metadata_impl("test_immutable_metadata", {"new_custom_key": "value"})
        
        assert "Cannot update metadata on collections with immutable settings" in str(exc_info.value)
        mock_collection.modify.assert_not_called()
