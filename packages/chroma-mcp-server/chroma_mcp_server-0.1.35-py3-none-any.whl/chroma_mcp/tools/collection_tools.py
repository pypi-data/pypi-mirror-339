    ...
    try:
        ...
        if has_immutable_settings:
            ...
            return {
                ...
                "message": "Cannot set description on collections with existing immutable settings (e.g., hnsw:*). Collection settings must be finalized at creation."
            }
        # --- End: Fail Fast Logic ---
    ...
    try:
        ...
        if has_immutable_settings:
            ...
            return {
                "success": False,
                "error_code": INVALID_PARAMS,
                "message": "Cannot set settings on collections with existing immutable settings (e.g., hnsw:*). Collection settings must be finalized at creation."
            }
        # --- End: Fail Fast Logic ---

        # Proceed only if no immutable settings were detected
        # current_metadata = collection.metadata or {} # Already fetched
        if not isinstance(settings, dict):
            raise_validation_error("settings parameter must be a dictionary")
            
        # Filter out existing settings before adding new ones
        metadata_without_settings = {k: v for k, v in current_metadata.items() if not k.startswith("chroma:setting:")}
        # Apply new settings, flattening them
...
async def _update_collection_metadata_impl(collection_name: str, metadata_update: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(metadata_update, dict):
            # Revert to raising ValidationError for reserved keys check
        ...
        if has_immutable_settings:
            ...
            return {
                "success": False,
                "error_code": INVALID_PARAMS,
                "message": "Cannot update metadata on collections with immutable settings (e.g., hnsw:*). Set all custom metadata during collection creation if needed."
            }
        # --- End: Fail Fast Logic ---
        ...
        # Return success dictionary (implicitly, by calling _get_collection_impl)
        return await _get_collection_impl(collection_name)
    except Exception as e:
        # Let handle_chroma_error deal with unexpected/ChromaDB errors
        # It will raise McpError, which the framework *should* handle.
        # If this *also* fails, the issue is deeper in fastmcp.
