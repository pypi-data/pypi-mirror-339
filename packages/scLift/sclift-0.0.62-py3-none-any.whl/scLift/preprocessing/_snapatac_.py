...
def _get_handler_sc_atac_(
    ...
    if h5ad_file is not None and os.path.exists(h5ad_file):
        ...
        # import the fragment files and handler them
        adata, features = handler_sc_atac(
...
def merge_sc_atac(
    ...
    if str(file_list[0]).endswith(".h5ad"):
        ...
        # import the fragment files and handler them
        adatas, features = handler_sc_atac(
...
def get_gene_expression(
    ...
    # import the fragment files and handler them
    adata, selected_list, h5ad_file = _get_handler_sc_atac_(
        fragment_file=fragment_file,
        genome_anno=genome_anno,
        h5ad_file=h5ad_file,
        min_num_fragments=min_num_fragments,
...
def get_tf_data(
    ...
    tf_adata = _handler_info_to_adata_(
        adata=adata,
...
def get_difference_peaks(
    ...
    diff_peaks_adata = _handler_info_to_adata_(
        adata=adata,
        selected_adata=selected_adata,
        cluster=cluster,
