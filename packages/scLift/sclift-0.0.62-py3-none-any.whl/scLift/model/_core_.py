...
from ..util import path, enrichment_optional, to_dense, collection, to_sparse

__name__: str = "model_core"

_Method = Optional[Literal['mean', 'sum', 'median']]
    ...
    if chunk_size > 1:
        ...
        for chunk in range(chunk_size):
            ...
            if _chunk_overlap_is_read_:
                ...
                if _chunk_overlap_adata_.var.shape[0] != _trait_count_:
                    ul.log(__name__).warning(
                        f"The number of diseases read from file `{_chunk_atac_overlap_save_file_}` are inconsistent with the input ({_chunk_overlap_adata_.var.shape[0]} != {_trait_count_}) (chunk: {chunk}). "
            ...
            if _chunk_init_score_is_read_:
                ...
                if _chunk_init_score_.var.shape[0] != _trait_count_:
                    ul.log(__name__).warning(
                        f"The number of diseases read from file `{_chunk_init_score_save_file_}` are inconsistent with the input ({_chunk_init_score_.var.shape[0]} != {_trait_count_}) (chunk: {chunk}). "
...
def relevance_level(
    ...
    if method == "mean":
        _relevance_ = trs_source.mean(axis=axis).flatten().tolist()
    elif method == "sum":
        _relevance_ = trs_source.sum(axis=axis).flatten().tolist()
    elif method == "median":
        _relevance_ = np.median(trs_source, axis=axis).flatten().tolist()
    else:
        ul.log(__name__).error("method not")
        raise ValueError("The `axis` parameter supports only -1, 0, and 1")

    if axis == 0:
        adata.var["relevance_level"] = _relevance_
        ul.log(__name__).info("View this related result `adata.var[\"relevance_level\"]`.")
    elif axis == 1:
        adata.obs["relevance_level"] = _relevance_
    ...
    sorted_indices = np.argsort(_relevance_)
    ranks = np.zeros_like(_relevance_).tolist()
...
def knock(
    ...
    # RUN
    knock_random_walk.run_knock(trs, knock_trait)
    ...
    knock_trs.uns["params"] = {
        ...
        "is_add_negative": is_add_negative,
