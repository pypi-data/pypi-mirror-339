...
def adjustment_tf_idf(data: matrix_data, ri_sparse: bool = True) -> matrix_data:
    ...
    """
    ul.log(__name__).info("Start adjustment TF-IDF transformer")
    ...
    ul.log(__name__).info("End adjustment TF-IDF transformer")
    return to_sparse(tfidf) if ri_sparse else to_dense(tfidf)
...
class RandomWalk:
    ...
    def __init__(
        ...
        self.weight, self.cell_weight = self._get_weight_(self.cc_adata.X)

        if not is_simple and self.is_ablation:
            self.weight_m_knn, self.cell_weight_m_knn = self._get_weight_(self.cc_adata.layers["cell_mutual_knn"])
        ...
        if not is_simple and self.is_ablation:
            (
                self.seed_cell_threshold_m_knn, _,
                self.seed_cell_weight_m_knn, _, _,
                self.seed_cell_weight_en_m_knn
            ) = self._get_seed_cell_(init_data=init_status, cell_weight=self.cell_weight_m_knn, info="ablation")
    ...
    def _get_weight_(self, cell_cell_matrix: matrix_data) -> Tuple[matrix_data, matrix_data]:
        """
    ...
    def _get_seed_cell_size_(self) -> int:
        ...
        for cluster in cluster_types:
            ...
            self.cluster_size_factor.update({cluster: count})
    ...
    def _get_seed_cell_clustering_weight_(self, seed_cell_index: collection) -> Tuple[collection, dict]:
        ...
        """
        cell_anno: DataFrame = self.trs_adata.obs.copy()
        cell_anno = cell_anno[["clusters"]]
        seed_cell_cell_anno: DataFrame = cell_anno.iloc[seed_cell_index]
        seed_cell_cell_anno_clusters: DataFrame = seed_cell_cell_anno.groupby("clusters", as_index=False).size()
        seed_cell_cell_anno_clusters.insert(0, "count", seed_cell_cell_anno_clusters["clusters"].map(self.cluster_size_factor))
        seed_cell_cell_anno_clusters.insert(0, "rate", seed_cell_cell_anno_clusters["size"] / seed_cell_cell_anno_clusters["count"])
        # Obtain mapping relationship
        seed_cell_clusters_rate_map: dict = dict(zip(seed_cell_cell_anno_clusters["clusters"], seed_cell_cell_anno_clusters["rate"]))
        seed_cell_cluster_weight = seed_cell_cell_anno["clusters"].map(seed_cell_clusters_rate_map)
        # Na value means that there is almost no possibility of enriching this cell type
        seed_cell_cluster_weight[np.isnan(seed_cell_cluster_weight)] = 0
        return mean_symmetric_scale(seed_cell_cluster_weight, is_verbose=False), seed_cell_clusters_rate_map

    def _get_seed_cell_weight_(
        self,
        seed_cell_index: collection,
        value: collection,
        cell_weight: matrix_data = None,
        ...
        seed_cell_mutual_knn = np.array(cell_weight[seed_cell_index, :][:, seed_cell_index])
        seed_weight_degree: collection = seed_cell_mutual_knn.sum(axis=0)
        degree_sum = seed_weight_degree.sum()
        # Calculate weight
        seed_weight_value = seed_weight_degree / (1 if degree_sum == 0 else degree_sum)

        # Calculate the initialization score weight
        seed_cell_value = value[seed_cell_index_enrichment]
        seed_cell_value_weight = sigmoid(mean_symmetric_scale(seed_cell_value, is_verbose=False))
        seed_weight_value *= seed_cell_value_weight * self._get_seed_cell_clustering_weight_(seed_cell_index)[0]
    ...
    def _get_seed_cell_(
        ...
        cell_weight: matrix_data = None,
        info: str = None
    ) -> Tuple[collection, matrix_data, matrix_data, matrix_data, matrix_data, matrix_data]:
        """
        Obtain information related to seed cells
        :param init_data: Initial TRS data
        :param cell_weight: Cell weight
        ...
        for i in tqdm(self.trait_range):
            ...
            # Obtain information on trait `i`
            init_trait_adata: AnnData = self.trs_adata[:, i]
            init_trait_value: collection = to_dense(init_trait_adata.layers["init_trs"], is_array=True).flatten()
            ...
            if trait_value_min != trait_value_max:
                ...
                _seed_cell_index_ = trait_value_sort_index[0:self.seed_cell_size]
                seed_cell_index[:, i][_seed_cell_index_] = 1
                seed_cell_weight[:, i][_seed_cell_index_] = self._get_seed_cell_weight_(_seed_cell_index_, init_trait_value, cell_weight=cell_weight)

                seed_cell_en_index = trait_value_sort_index[self.seed_cell_size:2 * self.seed_cell_size if self.cell_size > 2 * self.seed_cell_size else self.seed_cell_size - 1]
                _seed_cell_en_weight_ = self._get_seed_cell_weight_(_seed_cell_index_, init_trait_value, cell_weight=cell_weight, seed_cell_index_enrichment=seed_cell_en_index)
                seed_cell_weight_en[:, i][seed_cell_en_index] = _seed_cell_en_weight_
        ...
        return seed_cell_threshold, seed_cell_matrix, seed_cell_weight, seed_cell_index, seed_cell_matrix_en, seed_cell_weight_en
    ...
    def run_benchmark(self) -> None:
        ...
        ul.log(__name__).info(f"Calculate {len(self.trait_list)} traits/diseases for process `run_benchmark`. (Randomly perturb seed cells. ===> `benchmark`)")
        ...
        for i in tqdm(self.trait_range):
            ...
            self.random_seed_cell[:, i] = cell_value
    ...
    def _get_label_description_(label: str) -> Tuple[str, str]:
        ...
        elif label == "run_knock":
            return "Run knockout or knockdown by random walk with weight.", "knock_effect"
        elif label == "run_ablation_m_knn":
    ...
    def run_ablation_m_knn(self) -> None:
        ...
        """
        self._simple_error_()
        self.trs_adata.layers["seed_cell_weight_m_knn"] = self.seed_cell_weight_m_knn

        if "seed_cell_threshold_m_knn" not in self.trs_adata.var.columns:
            self.trs_adata.var["seed_cell_threshold_m_knn"] = self.seed_cell_threshold_m_knn
    ...
    def _run_enrichment_(self, seed_cell_en_weight: matrix_data, label: str) -> None:
        ...
        for i in tqdm(self.trait_range):
            # Random walk
            cell_value = self._random_walk_(seed_cell_en_weight[:, i], weight=self.weight if label == "run_en_ablation_m_knn" else self.weight_m_knn, gamma=self.enrichment_gamma)
            # separate
            cell_value_credible = mean_symmetric_scale(np.array(source_value[:, i]).flatten() - np.array(cell_value).flatten(), is_verbose=False)

            if label == "run_en" or label == "run_en_ablation_nsw":
                ...
                _clustering_weight_[np.isnan(_clustering_weight_)] = 0
    ...
    def run_en_ablation_m_knn(self) -> None:
        ...
        """
        self._simple_error_()
        self._run_enrichment_(self.seed_cell_weight_en_ncw, "run_en_ablation_m_knn")
    ...
    def run_knock(self, trs: AnnData, knock_trait: str, is_negative: bool = False) -> None:
        ...
        seed_cell_index = to_dense(trs[:, knock_trait].layers["seed_cell_index"], is_array=True).flatten()
        seed_cell_index = np.where(seed_cell_index == 1)[0]

        # seed cell threshold
        seed_cell_weight: matrix_data = np.zeros(self.trs_adata.shape)

        ul.log(__name__).info(f"Calculate {len(self.trait_list)} for seed cells information. (Knock)")
        for i in tqdm(self.trait_range):
            ...
            if is_negative:
                init_trait_value = perturb_data(init_trait_value, 1)
            ...
            if trait_value_min != trait_value_max:
                seed_cell_weight[:, i][seed_cell_index] = self._get_seed_cell_weight_(seed_cell_index, init_trait_value)

        self.trs_adata.var["seed_cell_threshold"] = self.seed_cell_threshold

        # Obtain the result after random walk
        self.trs_source = self._run_(seed_cell_weight, "run_knock")

        self.trs_adata.layers["trs_source"] = to_sparse(self.trs_source)

        # Obtain results before knocking out or knocking down
        trs_source = to_dense(trs[:, knock_trait].layers["trs_source"])

        # Obtain the effect result
        knock_effect_value = to_sparse(trs_source - self.trs_source)
        self.trs_adata.layers["knock_effect_negative" if is_negative else "knock_effect"] = knock_effect_value
...
def calculate_fragment_weighted_accessibility(
    ...
    matrix = to_sparse(input_data["data"])
    del input_data["data"]
...
def calculate_init_score_weight(
    ...
    if "is_overlap" not in overlap_adata.uns:
        ul.log(__name__).warning("The description of traits/diseases `is_overlap` is not in `overlap_data.uns`. (Need to execute function `tl.overlap_sum`)")

    # Processing data
    fragments = to_dense(adata.layers["fragments"])
    overlap_matrix = to_dense(overlap_adata.X)
    _weight_bf_ = to_dense(da_peaks_adata.X)
    _weight_ep_ = to_dense(da_peaks_adata.layers["emp_prob1"])

    ul.log(__name__).info("Calculate cell type weight ===> (bayes factor)")
    _cluster_weight_bf_: matrix_data = matrix_dot_block_storage(_weight_bf_, overlap_matrix, block_size=block_size)
    _cluster_weight_bf_ = sigmoid(mean_symmetric_scale(_cluster_weight_bf_, axis=0, is_verbose=False))
    ul.log(__name__).info("Calculate cell type weight ===> (emp_prob1)")
    _cluster_weight_ep_: matrix_data = matrix_dot_block_storage(_weight_ep_, overlap_matrix, block_size=block_size)
    _cluster_weight_ep_ = sigmoid(mean_symmetric_scale(_cluster_weight_ep_, axis=0, is_verbose=False))
    ul.log(__name__).info("Calculate cell type weight")
    _cluster_weight_: matrix_data = _cluster_weight_bf_ + _cluster_weight_ep_
    del _weight_bf_, _weight_ep_, _cluster_weight_bf_, _cluster_weight_ep_
    ...
    # Correcting the degree of dispersion through coefficient of variation to prevent excessive correction of initial values
    factor_cw = coefficient_of_variation(_cluster_weight_, axis=0, default=0)
    factor_init_trs_ncw = coefficient_of_variation(_init_trs_ncw_, axis=0, default=1)
    _factor_weight_ = (factor_cw / factor_init_trs_ncw) + np.zeros(_cluster_weight_.shape)
    _cluster_weight_ = matrix_multiply_block_storage(_cluster_weight_, _factor_weight_, block_size=block_size, data=_cluster_weight_)
    del _factor_weight_

    cluster_size: int = adata.uns["poisson_vi"]["cluster_size"]
    _cluster_weight_ = _cluster_weight_ / cluster_size
    del cluster_size
    ...
    init_trs_adata.uns["factor_cw"] = factor_cw
    init_trs_adata.uns["factor_init_trs_ncw"] = factor_init_trs_ncw
...
def obtain_cell_cell_network(
    ...
    # euclidean distances
    _ed_data_ = euclidean_distances(latent, block_size=block_size)
    _ed_scale_data_ = mean_symmetric_scale(_ed_data_, axis=1, is_verbose=False)
    del _ed_data_

    ul.log(__name__).info("Start affinity matrix")
    _distances_ = to_dense(_ed_scale_data_, is_array=True)
    del _ed_scale_data_
    _affinity_data_ = stats.gamma.pdf(_distances_, a=1, scale=1)
    del _distances_
    cell_affinity = mean_symmetric_scale(_affinity_data_, axis=1, is_verbose=False)
    del _affinity_data_

    # Define KNN network
    cell_mutual_knn_weight, cell_mutual_knn = semi_mutual_knn_weight(cell_affinity, neighbors=k, or_neighbors=or_k, weight=weight, is_mknn_fully_connected=True)
...
def perturb_data(data: collection, percentage: float) -> collection:
    ...
    if percentage <= 0 or percentage >= 1:
        raise ValueError("The value of the `percentage` parameter must be greater than 0 and less than 1.")
