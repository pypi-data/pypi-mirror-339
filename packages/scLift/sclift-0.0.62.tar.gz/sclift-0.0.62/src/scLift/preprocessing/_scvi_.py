...
def poisson_vi(
    ...
    if "clusters" in adata.obs.columns:
        ul.log(__name__).warning("Due to the original inclusion of the `clusters` column, the original `cluster` column name has been changed to `clusters_x`.")
    ...
    da_peaks_adata = AnnData(matrix_bf, obs=obs, var=adata.var)
