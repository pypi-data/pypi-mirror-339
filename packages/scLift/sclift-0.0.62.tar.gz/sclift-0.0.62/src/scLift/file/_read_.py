...
def read_sc_atac_10x_h5(
    ...
    # format peaks information
    _, chr_list, start_list, end_list = handler_peaks(peaks.index, peak_split_character)
    peaks["chr"] = chr_list
    ...
    if is_metadata:
        ...
        if str(resource).endswith(".h5"):
            ...
            if "chr" not in peak_col or "start" not in peak_col or "end" not in peak_col:
                _, chr_list, start_list, end_list = handler_peaks(list(sc_atac.var_names), peak_split_character)
            ...
            # format peaks information
            features, _, _, _ = handler_peaks(data.columns, peak_split_character)
