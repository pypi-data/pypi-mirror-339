    ...
    try:
        ...
        if not len(data):
            ...
            if media_type == 'photo':
                ...
                res = await bot.send_video(chat_id=chat_id, video=media, width=width, height=height)
                result = res.video.file_id
...
async def post_save(bot, data_user, data_web, MEDIA_D, BASE_P, KEYS_JSON, PROJECT_USERNAME, PROJECT_TYPE):
    ...
    try:
        ...
        if PROJECT_USERNAME == 'FereyPostBot':
            ...
            web_val = f"pst-{str(ENT_TID).replace('-', '')}-{POST_TID}"
            print(f"{web_val=}")
            HASH_VAL = hashlib.blake2b(web_val.encode('utf-8'), digest_size=8).hexdigest()
            POST_WEB = f"https://t.me/{PROJECT_USERNAME}/web?startapp=pst-{HASH_VAL}-{str(ENT_TID).replace('-', '')}&mode=fullscreen"
        ...
        for button in POST_BUTTONS:
            try:
                if button['knd'] == 'web':
                    web_val = f"pst-{str(ENT_TID).replace('-', '')}-{button['lnk']}"
        ...
        sql = f"DELETE FROM {schema_prefix}_{str(ENT_TID).replace('-', '')}.LANG WHERE POST_TID=$1"
        await db_change_pg(sql, (POST_TID,), BASE_P)

        sql = f"SELECT POST_TID, POST_MEDIA FROM {schema_prefix}_{str(ENT_TID).replace('-', '')}.POST WHERE POST_TID=$1"
        ...
        sql = f""" 
            INSERT INTO {schema_prefix}_{str(ENT_TID).replace('-', '')}.POST (
