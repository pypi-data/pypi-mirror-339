...
async def get_link_for_media(bot, chat_id, file_path, KEYS_JSON, is_del=True):
    ...
    try:
        ...
        base_name = f"r{random_suffix}_{base_name}"
...
async def get_user_vars(PROJECT_USERNAME, BASE_P, ENT_TID, chat_id, lc, username, full_name, is_premium, usr_hash='',
                        page=''):
    USER_TID = chat_id
    ...
    try:
        ...
        sql = f"SELECT USER_TID, USER_GAMES, USER_VARS, USER_LSTS FROM {schema_name}_{ENT_TID}.USER WHERE USER_TID=$1"
        ...
        if len(data_user):
            ...
            if page in ['msg', 'pst']: USER_GAMES = await ch_games(USER_GAMES, 'web', True)
        ...
        sql = f""" 
        INSERT INTO {schema_name}_{ENT_TID}.USER (
...
async def post_save(bot, data_user, data_web, MEDIA_D, BASE_P, KEYS_JSON, PROJECT_USERNAME, PROJECT_TYPE):
    ...
    try:
        ...
        BOT_TOKEN = data_web['BOT_TOKEN'] if 'BOT_TOKEN' in data_web else None
        ...
        # POST_PAY = ''
        POST_ISPRIVATE = POST_CHKBOX['POST_ISPRIVATE'] if 'POST_ISPRIVATE' in POST_CHKBOX else ''
        ...
        print(f"{POST_DT=}, {POST_TARGET=}, {BOT_TOKEN=}")
        ...
        if BOT_TOKEN and len(POST_MEDIA):
            # region need start
            ...
            print(f"before {POST_ISWATER=}, {POST_WATER=}, {BOT_TOKEN=}, {POST_MEDIA=}")
            if POST_TYPE == 'photo' and POST_ISWATER and POST_WATER and BOT_TOKEN:
        ...
        # region pay
        if len(POST_BUTTONS) and BOT_TOKEN:
            try:
                extra_bot = Bot(token=BOT_TOKEN)
                for button in POST_BUTTONS:
                    try:
                        ...
                        pay_lnk = await extra_bot.create_invoice_link(title=ENT_FIRSTNAME,
        ...
        POST_PAY = ''
        if POST_ISPAY and POST_STARS and BOT_TOKEN:
            web_val = f"ppay-{ENT_TID}-{POST_TID}-{POST_STARS}-{ENT_TID}"
            HASH_VAL = hashlib.blake2b(web_val.encode('utf-8'), digest_size=8).hexdigest()
            POST_PAY = f"https://t.me/{PROJECT_USERNAME}/web?startapp=ppay-{HASH_VAL}-{ENT_TID}"
            ...
            try:
                ...
                try:
                    ...
                    amount = 1
                prices = [LabeledPrice(label="XTR", amount=amount)]
                POST_PAY = await extra_bot.create_invoice_link(title=ENT_FIRSTNAME,
                                                               description=f"@{ENT_USERNAME}",
        ...
        if BOT_TOKEN:
            ...
            if PROJECT_USERNAME == 'FereyChannelBot':
                ...
                await db_change_pg(sql, (BOT_TOKEN, ENT_TID,), BASE_P)
            elif PROJECT_USERNAME == 'FereyGroupBot':
        ...
        sql = f"DELETE FROM {schema_prefix}_{ENT_TID}.LANG WHERE POST_TID=$1"
        await db_change_pg(sql, (POST_TID,), BASE_P)

        sql = f"SELECT POST_TID, POST_MEDIA FROM {schema_prefix}_{ENT_TID}.POST WHERE POST_TID=$1"
        ...
        result['post'] = {
            ...
            'POST_WEB': POST_WEB, 'POST_PAY': POST_PAY, 'POST_BLOG': POST_BLOG,
        ...
        sql = f"""
            INSERT INTO {schema_prefix}_{ENT_TID}.POST (
                ...
                POST_WEB, POST_PAY, POST_BLOG, POST_ISPRIVATE, 
                POST_LZ, POST_TZ, POST_DT, POST_TR
            )
        ...
        await db_change_pg(sql, (
            ...
            POST_TYPE, POST_TEXT, POST_MEDIA, POST_BUTTONS, POST_CHKBOX,
            POST_WEB, POST_PAY or None, POST_BLOG, POST_ISPRIVATE or None,
            POST_LZ or 'en', POST_TZ or None, POST_DT or None, POST_TR or None
        ), BASE_P)
        # endregion
    except TelegramRetryAfter as e:
        logger.info(log_ % f"TelegramRetryAfter {e.retry_after}")
...
async def post_pub(bot, lz, chat_id, ENT_TID, post, MEDIA_D, BASE_S, BASE_D, PROJECT_USERNAME, is_private=True, cur_=1):
    result = False
    try:
        POST_ID, POST_TID, POST_MSGID, POST_CHATTID, POST_USERTID, POST_USERTUN, \
            ...
            POST_CHKBOX, POST_WEB, POST_NFT, POST_PAY, POST_BLOG, POST_ISMINTED, \
        ...
        if len(POST_MEDIA) > 1 and POST_ISGALLERY:
            len_ = len(POST_MEDIA)
        ...
        print(f"{POST_ISPAY=}, {POST_TYPE=}")
        if POST_ISPAY and POST_TYPE not in ['photo', 'video']:
                ...
                if 'filew_id' not in POST_MEDIA[index]:
                    ...
                    if BOT_TOKEN:
                        ...
                        try:
                            ...
                            sql = f"UPDATE {schema_name}_{ENT_TID}.POST SET POST_MEDIA=$1 WHERE POST_TID=$2"
                            await db_change_pg(sql, (
        ...
        if result and (int(chat_id) == int(ENT_TID) or PROJECT_USERNAME in ['FereyChannelBot', 'FereyGroupBot']):
            print('indeid')
            ...
            sql = f"UPDATE {schema_name}_{ENT_TID}.POST SET POST_MSGID=$1 WHERE POST_TID=$2"
            await db_change_pg(sql, (POST_MSGID, POST_TID,), BASE_D)
        # endregion

        # region story
        try:
            if result and POST_TYPE == 'text' and POST_CHKBOX.get('POST_ISSTORY', False) and int(chat_id) == int(
                    ENT_TID):
        ...
        # region podcast
        try:
            if result and result.chat.username and POST_CHKBOX.get('POST_ISPODCAST', False) and int(chat_id) == int(
        ...
        for button in sorted_buttons:
            try:
                ...
                if button['knd'] == 'button_in':
                    ...
                    post_fix = '' if PROJECT_USERNAME == 'FereyPostBot' else f"-{ENT_TID}"
