...
async def post_pub(bot, lz, chat_id, ENT_TID, post, MEDIA_D, BASE_S, BASE_D, PROJECT_USERNAME, is_private=True, is_paid=False):
    ...
    try:
        ...
        if not is_paid and POST_ISPAY and POST_PAY and POST_TYPE not in ['photo', 'video'] and POST_TEXT:
            print(f"{POST_PAY=}")
        ...
        # region send
        if not is_paid and POST_ISPAY and POST_TYPE in ['photo', 'video'] and not POST_TEXT:
            if PROJECT_USERNAME == 'FereyChannelBot':
