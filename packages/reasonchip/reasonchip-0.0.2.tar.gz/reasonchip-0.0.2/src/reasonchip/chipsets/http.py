...
class HttpRequestRequest(BaseModel):
    ...
    """
    method: typing.Literal['GET', 'OPTIONS', 'HEAD', 'POST', 'PUT', 'DELETE', 'PATCH'] = Field(
...
async def http_request(req: HttpRequestRequest) -> HttpRequestResponse:
    ...
    """
    MAX_CONTENT_LENGTH = 16 * (1024 * 1024)  # Maximum allowed response content size (16MB)
...
@Registry.register
async def post_json_object(req: PostJsonObjectRequest) -> PostJsonObjectResponse:
    """
    Executes an HTTP request asynchronously using `httpx.AsyncClient` and
