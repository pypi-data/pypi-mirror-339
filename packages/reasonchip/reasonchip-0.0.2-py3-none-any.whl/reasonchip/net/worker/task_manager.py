...
class TaskManager:
    ...
    async def wait(self, timeout: typing.Optional[float] = None) -> bool:
        logging.info(f"Waiting for TaskManager to finish: timeout=[{timeout}] ...")
    ...
    async def _multiplexing(self):
        ...
        while wl:
            ...
            for t in self._tasks.values():
                ...
                wl.append(asyncio.create_task(
                    self._wait_for_engine(t.task, t.cookie)
                ))
    ...
    async def _process_server_packet(
        ...
        handler = handlers.get(packet.packet_type, self._srv_unsupported_packet_type)
    ...
    async def _srv_run(self, packet: SocketPacket) -> bool:
        ...
        if len(self._tasks) >= self._max_capacity:
            logging.fatal("Capacity reached on worker. We should never have been asked.")
            return False

        # Check if the packet is fine
        if not packet.cookie or not packet.pipeline:
            logging.fatal("Missing cookie or pipeline in packet. Should have been checked beforehand.")
            return False

        # Check for collisions
        if packet.cookie in self._tasks:
            logging.fatal("Cookie collision has occurred. Should never have been allowed.")
            return False
        ...
        task_info.task = asyncio.create_task(self._run_engine(
            task_info,
    ...
    async def _srv_cancel(self, packet: SocketPacket) -> bool:
        ...
        if not packet.cookie:
            logging.fatal("Missing cookie. Should have been checked beforehand.")
            return False

        # Check for the task
        if packet.cookie not in self._tasks:
            logging.warning(f"Cookie not found trying to cancel. Could be a race condition: [{packet.cookie}]")
            return True
    ...
    async def _run_engine(
        ...
        try:
            ...
            await self._transport.send_packet(SocketPacket(
            ...
            logging.exception(f"Exception occurred during engine run: [{task_info.cookie}] [{pipeline}]")
        ...
        logging.info(f"Engine task completed: [{task_info.cookie}] [{pipeline}] [{elapsed:.2f}us] [{time_ms:.2f}ms] [{time_s:.2f}s]")
