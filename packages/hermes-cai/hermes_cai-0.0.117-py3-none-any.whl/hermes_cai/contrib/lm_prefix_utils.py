...
MAX_USED_SCENE_DEFINITION_LEN: int = 3200
...
def _truncate_scene_messages(parsed_messages: list[str], max_chars: int = MAX_USED_SCENE_DEFINITION_LEN) -> list[str]:
    """
...
def get_scene_definition(character: dict, username: str, scene_info: dict) -> list[str]:
    ...
    for message in character_definition_messages:
        parsed_messages.insert(0, message.get("src", "") + ":" + message.get("text", ""))

    return _truncate_scene_messages(parsed_messages)
