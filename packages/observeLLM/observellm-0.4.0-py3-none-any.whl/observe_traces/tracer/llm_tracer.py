            ...
            try:
                async for response_line in func(self, **params):
                    if response_line.startswith("data: "):
                        ...
                        # Handle the specific format from the service
                        if response_data["type"] == "data":
                            collected_response += response_data["data"]
                        yield response_line

                    elif response_line.startswith("tokens: "):
                        tokens_data = json.loads(response_line[8:])
                        total_input_tokens = tokens_data["data"]["input"]
                        total_output_tokens = tokens_data["data"]["output"]
                    else:
                        yield response_line
