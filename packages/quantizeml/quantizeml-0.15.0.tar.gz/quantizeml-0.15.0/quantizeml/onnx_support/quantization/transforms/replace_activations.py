...
def _rewrite_unfolded_ops(model):
    ...
    return rewriter.rewrite(model, pattern_rewrite_rules=rewrite_rule_set)
...
def replace_activations(model):
    ...
    model.model = _rewrite_unfolded_ops(model.model)

    # Rewrite equivalent activations
    model.model = _rewrite_prelu_to_leaky_relu(model.model)
