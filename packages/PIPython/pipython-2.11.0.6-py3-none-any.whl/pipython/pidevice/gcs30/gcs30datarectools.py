...
__signature__ = 0x4f882b93f11ac60bd48abacaf7b43c88
...
class GCS30Datarecorder(GCSBaseDatarecorder):
    ...
    def number_of_values(self, value):
        ...
        if value > self.max_number_of_values_per_trace:
            raise ValueError(f'number_of_values must be < {self.max_number_of_values_per_trace}')
    ...
    def offset(self, value):
        ...
        if value > self.max_number_of_values_per_trace:
            raise ValueError(f'offset must be < {self.max_number_of_values_per_trace}')
    ...
    def _wait(self):
        ...
        while self._gcs.qREC_NUM(self._recorder_id)[self._recorder_id] < self.number_of_values:
            if self.timeout and (time() - start_time) > self.timeout:
                raise SystemError(f'timeout after {self.timeout:.1f} secs while waiting on data recorder')
            sleep(0.005)
    ...
    def _check_number_of_traces(self, collection):
        if len(collection) > self.max_number_of_traces:
            raise ValueError(
                f'Number of traces ({len(collection)}) is to high. '
                f'Maximum number of traces is {self.max_number_of_traces}!')

    def _check_all_trace_ids(self, trace_ids):
        for entry in trace_ids:
            if entry < 1:
                raise ValueError(f'Invalid trace ID: {entry}. Must be >= 1!')
            if entry > self.max_number_of_traces:
                raise ValueError(f'Invalid trace ID: {entry}. Must be <= {self.max_number_of_traces}!')
    ...
    def _check_single_parameter_address(cls, parameter_address):
        ...
        if len(parameter_address) != 3:
            raise ValueError(f'Invalid parameter address {parameter_address}. Must contain 3 elements!')
        for entry in parameter_address:
            if not isinstance(entry, str):
                raise ValueError(f'Invalid parameter address {parameter_address}. Each entry must be a string!')
    ...
    def _normalize_trigger_list_or_tuple(cls, trigger_configuration):
        if len(trigger_configuration) != 3:
            raise ValueError(
                f'Invalid number of entries in trigger configuration ({len(trigger_configuration)}). '
                f'Must contain 3 elements!!')
        for entry in trigger_configuration:
            if not isinstance(entry, str):
                raise ValueError(
                    f'Invalid trigger configuration {trigger_configuration}. '
                    f'Each entry must be a string!')
