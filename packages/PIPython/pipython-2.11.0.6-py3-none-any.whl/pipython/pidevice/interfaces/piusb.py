...
__signature__ = 0xa3bd8de7268373aafde74180c606fbcf


class PIUSB(PIGateway):
    """Provide a PIUSB, can be used as context manager.
    Windows: 'PIUSB' ist not supported.
             If your PI USB controller supports a virtual COM port, use 'ConnectRS232()' of 'PISerial'.
    Linux:   Not all PI USB controllers support a native USB connection using 'PIUSB'.
             If your PI USB controller does not support 'PIUSB', connect it using a virtual COM port via
             'ConnectRS232()' of 'PISerial'.
             Find out by trial and error how the connection can be established.
    """
    ...
    def enumerate(vid=0x1a72, pid=None):
        ...
        @param vid : USB Vendor ID as integer. The default ist the PI vendor id '0x1a72'
        @param pid : USB Product ID as integer or 'None' to list all devices of the vendor.
    ...
    def connect(self, serialnumber, pid, vid=0x1a72):
        ...
        try:
            ...
            descriptor = usb.util.find_descriptor(cfg, bInterfaceNumber=self._ifnum)
