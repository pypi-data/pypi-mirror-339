...
# Using variable 'unicode' before assignment pylint: disable=E0601
try:
    unicode
except NameError:
    unicode = str

__signature__ = 0x522364ae56a7694c844d5ec7ab28a88e
...
def isinstance_of_magicmock(device, objects):
    ...
    for supported_object in objects:
        if type(device).__name__ == 'MagicMock':
            # Unnecessarily calls dunder method __str__. Use str built-in function. pylint: disable=C2801
            if type(device).__str__().find(str(supported_object).rsplit('.', maxsplit=1)[-1].replace("'>", "")) >= 0:
                return True
    return False
...
def gethexstr(paramids):
    ...
    return '[' + ', '.join([f'0x{x:x}' for x in paramids]) + ']'
...
def getsupportedfunctions(qhlp, dostrip=True):
    ...
    for line in qhlp:
        ...
        if funcname.endswith('?'):
            funcname = f'q{funcname[:-1]}'
        if funcname == 'MAC':
            for item in ('BEG', 'DEF', 'DEF?', 'DEL', 'END', 'ERR?', 'NSTART', 'START', 'FREE?', 'STOP'):
                if line.find(item) >= 0:
                    if item.endswith('?'):
                        item = f'q{item[:-1]}'
                    funcs.append(f'MAC_{item}')
        elif funcname == 'REC':
            sub_methods = ['START', 'STOP', 'TRG', 'TRACE', 'RATE']
            sub_methods = [f'REC_{cmd}' for cmd in sub_methods]
            funcs += sub_methods
        elif funcname == 'qREC':
            sub_methods = ['TRG', 'TRACE', 'RATE', 'DAT', 'STATE', 'NUM']
            sub_methods = [f'qREC_{cmd}' for cmd in sub_methods]
            funcs += sub_methods
        else:
...
def convertvalue(value, totype):
    ...
    if not isinstance(value, (basestring, int, float, bool)):
        raise TypeError(f'parameter is of unexpected type: {value}')
    ...
    if totype is bool:
        ...
        if value in ['0', 'False']:
            return False
        raise ValueError(f'unexpected response {value} for bool conversion')
    if totype is int:
        try:
...
def checksize(sizes, *args):
    ...
    for i, arg in enumerate(args):
        ...
        if size is True:
            ...
            size = listsize
        if size != len(arg):
            raise TypeError(f'parameter size mismatch: {arg}')


def logsysinfo():
    """Log which processor, operating system and python is used, useful for support."""
    PIDebug('Processor: %s', platform.processor())
    PIDebug('OS: %s, Architecture: %s', platform.platform(), platform.machine())
    pyver = f'{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro} ' \
            f'{sys.version_info.releaselevel}'
    PIDebug('Python: %s %s', pyver, platform.architecture()[0])
