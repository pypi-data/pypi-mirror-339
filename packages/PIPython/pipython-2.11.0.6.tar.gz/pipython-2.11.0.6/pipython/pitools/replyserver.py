...
# Using variable 'basestring' before assignment pylint: disable=E0601
try:
    basestring
except NameError:
    basestring = str

__signature__ = 0xc24c5da59aa91eacbb2dd86a110c3930
...
class ReplyHandler(BaseRequestHandler):
    ...
    def handle(self):
        ...
        PIDebug('ReplyHandler: shutting down')
    ...
    def __verify_answer(self, rcv):
        ...
        @return : Empty string.
        """
        PIDebug('ReplyHandler.receive: %r', rcv)
        if self.__send_from_queue(rcv):
            return ''
        if self.__send_from_static(rcv):
            return ''
        if not self.queue:
            self.__seterror(f'svr queue is empty but received {rcv}')
        else:
            self.__seterror(f'svr expected {self.queue[0]["cmd"]} but received {rcv}')
        if rcv.find('?') > -1:
            self.request.sendall(f'{self.server.RequestHandlerClass.errorbuf}\n')  # to prevent GCS timeout error
        return ''
    ...
    def __send(self, tosend, source):
        ...
        for line in tosend.splitlines(True):
            if self.delay:
                PIDebug('ReplyHandler.send: delay answer for %d ms', self.delay)
                sleep(self.delay / 1000.0)
            PIDebug('ReplyHandler.send(%s): %r', source, line)
...
class ReplyServer(object):
    ...
    def __init__(self, host='localhost', port=50000):
        ...
        @param port : Port to connect to as integer.
        """
        PIDebug('create an instance of ReplyServer(host=%r, port=%s)', host, port)
    ...
    def queue(self, cmd, answer=''):
        ...
        PIDebug('ReplyServer.queue: %r -> %r (%d)', cmd, answer, len(self.__handler.queue))
    ...
    def append(self, cmd, answer=''):
        ...
        PIDebug('ReplyServer.append: %r -> %r (%d)', cmd, answer, len(self.__handler.static))

    def clear(self):
        """Clear "queue" and "static" if they are not empty."""
        PIDebug('ReplyServer.clear')
    ...
    def delay(self, delay):
        ...
        if self.__handler.delay != int(delay):
            ...
            PIDebug('ReplyServer.delay: set to %d ms', delay)
    ...
    def rotate(self, mode):
        ...
        if self.__handler.rotate != mode:
            ...
            PIDebug('ReplyServer.rotate: set to %s', mode)
    ...
    def check(self):
        ...
        if self.__handler.queue:
            ...
            for item in self.__handler.queue:
                msg += f'{item["cmd"]} -> {item["answer"]}\n'
            assert 0, msg.strip()
        self.__handler.static = []
        PIDebug('ReplyServer.check: ok')
        return True
