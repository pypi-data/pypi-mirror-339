...
__signature__ = 0xd829cad0c4611c8a77a8b9370d79f439
...
class GCS30DeviceStartup(GCSBaseDeviceStartup):  # Too many instance attributes pylint: disable=R0902
    ...
    def __init__(self, gcs30pitools, **kwargs):
        ...
        if not isdeviceavailable([GCS30Tools, ], gcs30pitools):
            raise TypeError(f'Type {type(gcs30pitools).__name__} of pidevice is not supported!')
    ...
    def referencewait(self):
        ...
        for axis, controlmode in control_modes_before_ref.items():
            self.pidevice.SAM(axis, controlmode)
...
class GCS30Tools(GCSBaseTools):  # Too  public methods pylint: disable=R0903
    ...
    def getmaxtravelrange(self, axes):
        ...
        answer = {}
        if axes is None:
            axes = self._pidevice.axes

        if not isinstance(axes, (list, tuple)):
            axes = [axes]

        for axis in axes:
            answer[axis] = self._pidevice.qSPV("RAM",
                                               axis,
                                               "-",
                                               self.PAMID_POSITIVE_AXIS_LIMIT
                                               )["RAM"][axis.upper()]["-"][self.PAMID_POSITIVE_AXIS_LIMIT]

        return answer

    def getmintravelrange(self, axes):
        ...
        for axis in axes:
            answer[axis] = self._pidevice.qSPV("RAM",
                                               ...
                                               )["RAM"][axis.upper()]["-"][self.PAMID_NEGATIVE_AXIS_LIMIT]
    ...
    def _wait_on_stopall(self, timeout=300, polldelay=0.1):
        ...
        while any(self._read_axis_status_flag(axes=[],
            ...
            if time() > maxtime:
                raise SystemError(f'waitonready() timed out after {timeout:.1f} seconds')
            sleep(polldelay)
    ...
    def _get_servo_state(self, axes):
        ...
        # Consider iterating with .items() pylint: disable=C0206
