...
__signature__ = 0xf291cba06b4e0d2a898310fd148fefc4
...
def get_dll_name(devname):
    ...
    if not devname:
        ...
        dllname = ''
        for dll, devices in DLLDEVICES.items():
...
def modify_dll_name(dllname):
    ...
    if sys.platform in ('linux', 'linux2', 'darwin'):
        ...
        if unixdll is None:
            raise NotImplementedError(f'{dllname} is not available for this operating system')
...
class GCSDll(PIGateway):
    ...
    def unload(self):
        ...
        if self._dllhandle is not None:
            ...
            if sys.platform in ('win32', 'cygwin'):
                ...
                ctypes.cdll.LoadLibrary(str(self._dllpath)).dlclose(ctypes.c_void_p(self._dllhandle._handle))
    ...
    def _handle(self):
        ...
        if self._dllhandle is None:
            if not os.path.isfile(self._dllpath):
                raise IOError(f'{self._dllpath} not found')
    ...
    def _prefix(self):
        ...
        if self._dllprefix is None:
            ...
            for winname, unixlib in UNIXDLL.items():
                if dllname == unixlib:
                    dllname = winname
                    break
            if dllname.find('PI_MERCURY_GCS_DLL') > -1:
                ...
                self._dllprefix = f'{dllname.split("_")[0]}_'
    ...
    def ConnectRS232(self, comport, baudrate):
        ...
        @param comport: Port to use as integer (1 means "COM1") on Windows,
                        or e,g, "/dev/ttyS0" as string on Linux.
                        or e,g, "/dev/ttyUSB0" as string on Linux, if a virtual com port for USB is used.
        ...
        if self._id < 0:
            raise GCSError(self._error)
        self._ifdescription = f'RS-232 port {comport}, {baudrate} Baud'
        self.flush()
        self.call_connection_status_changed_callback(self)
    ...
    def ConnectTCPIP(self, ipaddress, ipport=50000):
        ...
        if self._id < 0:
            raise GCSError(self._error)
        self._ifdescription = f'TCPIP {ipaddress}:{ipport}'
        self.flush()
        self.call_connection_status_changed_callback(self)
    ...
    def ConnectTCPIPByDescription(self, description):
        ...
        if self._id < 0:
            raise GCSError(self._error)
        self._ifdescription = f'TCPIP {description}'
        self.flush()
        self.call_connection_status_changed_callback(self)

    def ConnectUSB(self, serialnum):
        """Open an USB connection to a device.
        Windows: All PI USB controllers support a native USB connection using 'ConnectUSB()'.
                 If your PI USB controller also supports a virtual COM port, you can connect this COM port
                 via 'ConnectRS232()'.
        Linux:   Not all PI USB controllers support a native USB connection using 'ConnectUSB()'.
                 Use 'EnumerateUSB()' to find out if your PI USB controller is supported by 'ConnectUSB()'.
                 If your controller is not listed by 'EnumerateUSB()', connect it using a virtual COM port
                 via 'ConnectRS232()'.
        ...
        if self._id < 0:
            raise GCSError(self._error)
        self._ifdescription = f'USB {serialnum}'
        self.flush()
        self.call_connection_status_changed_callback(self)
    ...
    def ConnectNIgpib(self, board, device):
        ...
        if self._id < 0:
            raise GCSError(self._error)
        self._ifdescription = f'GPIB board {board}, device {device}'
        self.flush()
        self.call_connection_status_changed_callback(self)
    ...
    def ConnectPciBoard(self, board):
        ...
        if self._id < 0:
            raise GCSError(self._error)
        self._ifdescription = f'PCI board {board}'
        self.flush()
        self.call_connection_status_changed_callback(self)
    ...
    def OpenRS232DaisyChain(self, comport, baudrate):
        ...
        self._ifdescription = f'RS-232 daisy chain at COM{comport}, {baudrate} Baud'
        self.call_connection_status_changed_callback(self)
        return devlist
    ...
    def OpenUSBDaisyChain(self, description):
        ...
        self._ifdescription = f'USB daisy chain at SN {description}'
        self.call_connection_status_changed_callback(self)
        return devlist
    ...
    def OpenTCPIPDaisyChain(self, ipaddress, ipport=50000):
        ...
        self._ifdescription = f'TCPIP daisy chain at {ipaddress}:{ipport}'
        self.call_connection_status_changed_callback(self)
        return devlist
    ...
    def ConnectDaisyChainDevice(self, deviceid, daisychainid=None):
        ...
        if self._ifdescription:
            self._ifdescription += '; '
        self._ifdescription += f'daisy chain {daisychainid}, device {deviceid}'
        self.call_connection_status_changed_callback(self)
        self.flush()
