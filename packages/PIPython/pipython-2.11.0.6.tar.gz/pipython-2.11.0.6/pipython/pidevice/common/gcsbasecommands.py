...
__signature__ = 0x5421744532bdb630e1b66664cc5cee95
...
class GCSBaseCommands(object):
    ...
    def update_paramconv_if_necessary(self):
        """
        updates the parameter convert dict if the current user level his higher the the user level at the last
        update of the parameter convert dict.
        """

    def clarfuncs(self):
        """Reset list of supported GCS functions."""
        PIDebug('GCSBaseCommands.clarfuncs')
        self._funcs = None

    def cleardevname(self):
        """Reset the device name."""
        PIDebug('GCSBaseCommands.cleardevname')
        self._name = None
    ...
    def FDG(self, routinename, scanaxis, stepaxis, minlevel=None, aligninputchannel=None, minampl=None, maxampl=None,
        ...
        cmdstr += '' if minlevel is None else f' {self.getcmdstr("ML", minlevel)}'
        cmdstr += '' if aligninputchannel is None else f' {self.getcmdstr("A", aligninputchannel)}'
        cmdstr += '' if minampl is None else f' {self.getcmdstr("MIA", minampl)}'
        cmdstr += '' if maxampl is None else f' {self.getcmdstr("MAA", maxampl)}'
        cmdstr += '' if frequency is None else f' {self.getcmdstr("F", frequency)}'
        cmdstr += '' if speedfactor is None else f' {self.getcmdstr("SP", speedfactor)}'
        cmdstr += '' if maxvelocity is None else f' {self.getcmdstr("V", maxvelocity)}'
        cmdstr += '' if maxdirectionchanges is None else f' {self.getcmdstr("MDC", maxdirectionchanges)}'
        cmdstr += '' if speedoffset is None else f' {self.getcmdstr("SPO", speedoffset)}'
    ...
    def FDR(self, routinename, scanaxis, scanrange, stepaxis, steprange, threshold=None, aligninputchannel=None,
        ...
        cmdstr += '' if threshold is None else f' {self.getcmdstr("L", threshold)}'
        cmdstr += '' if aligninputchannel is None else f' {self.getcmdstr("A", aligninputchannel)}'
        cmdstr += '' if frequency is None else f' {self.getcmdstr("F", frequency)}'
        cmdstr += '' if velocity is None else f' {self.getcmdstr("V", velocity)}'
        cmdstr += '' if scanmiddlepos is None else f' {self.getcmdstr("MP1", scanmiddlepos)}'
        cmdstr += '' if stepmiddlepos is None else f' {self.getcmdstr("MP2", stepmiddlepos)}'
        cmdstr += '' if targettype is None else f' {self.getcmdstr("TT", targettype)}'
        cmdstr += '' if estimationmethod is None else f' {self.getcmdstr("CM", estimationmethod)}'
        cmdstr += '' if mininput is None else f' {self.getcmdstr("MIIL", mininput)}'
        cmdstr += '' if maxinput is None else f' {self.getcmdstr("MAIL", maxinput)}'
        cmdstr += '' if stopoption is None else f' {self.getcmdstr("ST", stopoption)}'
        self._msgs.send(cmdstr)

    # Too many local variables pylint: disable=R0914
    def FDL(self, routinename, scanaxis, scanrange, stepaxis, steprange, thresholdsigma=None, thresholdcalculation=None,
            aligninputchannel=None, maximumsearchtime=None, scanmiddlepos=None, stepmiddlepos=None,
            scancalculationpos=None, stepcalculationpos=None):
        """Defines a fast alignment scan routine for first light search.
        Use FRS to start the process and FRR? to read out the results of the process.
        Optional parameters need to be scalar, i.e. not list, tuple, dict.
        @param routinename : Name of the fast alignment routine to define, must be unique and != 0.
        @param scanaxis : Axis that is defined to be the master axis of the scan process.
        ...
        @param stepaxis : Axis that is defined to be the second axis of the scan process.
        @param steprange : Scan range for step axis.
        @param thresholdsigma : Number of standard deviations the threshold value is to be set above the noise level,
                                ≥ 1. Used to calculate the threshold value at the calculation position.
        @param thresholdcalculation : Activation state of the threshold calculation option.
        @param aligninputchannel : Identifier of the analog input channel for which the first light position is sought.
        @param maximumsearchtime :>: Gives one stop criterion for the PILightning scan routine:
                                     When the PILightning scan routine finds the first light position within the
                                     maximum search time, the routine stops, and FRR? will report “successful”.
                                     After stopping as “successful”, the scan axis and step axis will stay at the
                                     first light position.
                                     When the PILightning scan routine does not find the first light position within
                                     the maximum search time, the routine stops, and FRR? will report “not successful”.
                                     After stopping as “not successful”, the scan axis and step axis will stay at the current position.
                                     <maximumsearchtime> is to be specified in seconds; must be > 0 s;
                                     default value is 5 s.
        @param scanmiddlepos : >: Middle position of the scan range for the scan axis. The value is used to calculate
                                  the minimum and maximum position for the scan axis, see description
                                  of <steprange> above.
        @param stepmiddlepos :>: Middle position of the scan range for the step axis. The value is used to calculate
                                 the minimum and maximum position for the step axis, see description
                                 of <steprange> above.
        @param scancalculationpos :Threshold calculation position for the scan axis. Only used
                                       if C has the value 1 (on).
        @param stepcalculationpos :Threshold calculation position for the step axis. Only used
                                       if C has the value 1 (on).
        """
        PIDebug(
            'GCSBaseCommands.FDL(routinename=%r, scanaxis=%r, scanrange=%r, stepaxis=%r, steprange=%r,'
            ' thresholdsigma=%r, thresholdcalculation=%r, aligninputchannel=%r, maximumsearchtime=%r,'
            ' scanmiddlepos=%r, stepmiddlepos=%r, scancalculationpos=%r,'
            ' stepcalculationpos=%r)',
            routinename, scanaxis, scanrange, stepaxis, steprange, thresholdsigma, thresholdcalculation,
            aligninputchannel,
            maximumsearchtime, scanmiddlepos, stepmiddlepos, scancalculationpos,
            stepcalculationpos)
        checksize((1, 1, 1, 1, 1), routinename, scanaxis, scanrange, stepaxis, steprange)
        cmdstr = self.getcmdstr('FDL', routinename, scanaxis, scanrange, stepaxis, steprange)
        cmdstr += '' if thresholdsigma is None else f' {self.getcmdstr("S", thresholdsigma)}'
        cmdstr += '' if thresholdcalculation is None else f' {self.getcmdstr("C", thresholdcalculation)}'
        cmdstr += '' if aligninputchannel is None else f' {self.getcmdstr("A", aligninputchannel)}'
        cmdstr += '' if maximumsearchtime is None else f' {self.getcmdstr("MT", maximumsearchtime)}'
        cmdstr += '' if scanmiddlepos is None else f' {self.getcmdstr("MP1", scanmiddlepos)}'
        cmdstr += '' if stepmiddlepos is None else f' {self.getcmdstr("MP2", stepmiddlepos)}'
        cmdstr += '' if scancalculationpos is None else \
            f' {self.getcmdstr("CP1", scancalculationpos)}'
        cmdstr += '' if stepcalculationpos is None else \
            f' {self.getcmdstr("CP2", stepcalculationpos)}'
        self._msgs.send(cmdstr)
    ...
    def FLM(self, axis, length, threshold=None, line=None, direction=None):
        ...
        cmdstr += '' if threshold is None else f' {self.getcmdstr("L", threshold)}'
        cmdstr += '' if line is None else f' {self.getcmdstr("A", line)}'
        cmdstr += '' if direction is None else f' {self.getcmdstr("D", direction)}'
        self._msgs.send(cmdstr)
    ...
    def AAP(self, axis1, length1, axis2, length2, stepsize=None, checks=None, line=None):
        ...
        cmdstr += '' if stepsize is None else f' {self.getcmdstr("SA", stepsize)}'
        cmdstr += '' if checks is None else f' {self.getcmdstr("N", checks)}'
        cmdstr += '' if line is None else f' {self.getcmdstr("A", line)}'
        self._msgs.send(cmdstr)
    ...
    def FIO(self, axis1, length1, axis2, length2, threshold=None, stepsize=None, angle=None, line=None):
        ...
        cmdstr += '' if threshold is None else f' {self.getcmdstr("S", threshold)}'
        cmdstr += '' if stepsize is None else f' {self.getcmdstr("AR", stepsize)}'
        cmdstr += '' if angle is None else f' {self.getcmdstr("L", angle)}'
        cmdstr += '' if line is None else f' {self.getcmdstr("A", line)}'
        self._msgs.send(cmdstr)
    ...
    def FSA(self, axis1, length1, axis2, length2, threshold=None, distance=None, stepsize=None, line=None):
        ...
        cmdstr += '' if threshold is None else f' {self.getcmdstr("L", threshold)}'
        cmdstr += '' if distance is None else f' {self.getcmdstr("S", distance)}'
        cmdstr += '' if stepsize is None else f' {self.getcmdstr("SA", stepsize)}'
        cmdstr += '' if line is None else f' {self.getcmdstr("A", line)}'
        self._msgs.send(cmdstr)
    ...
    def HasFDL(self):
        """Return True if FDL() is available."""
        return self._has('FDL')
