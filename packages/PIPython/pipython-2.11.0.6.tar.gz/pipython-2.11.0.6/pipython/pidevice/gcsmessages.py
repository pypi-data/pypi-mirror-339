...
__signature__ = 0x66422354dc2f5e829a74557e9be45097
...
class GCSMessages(object):
    ...
    def __init__(self, interface):
        ...
        """
        PIDebug('create an instance of GCSMessages(interface=%s)', interface)
        self._lock = RLock()
    ...
    def __str__(self):
        return f'GCSMessages(interface={self._interface}), id={self.connectionid}'
    ...
    def bufstate(self):
        ...
        if self._databuffer['index'] == self._databuffer['lastindex']:
            if time() - float(self._databuffer['lastupdate']) > self.timeout / 1000.:
                self._stopthread = True
                raise GCSError(gcserror.COM_TIMEOUT__7, f'GCSMessages.bufstate after {(self.timeout / 1000.):.1f} s')
        else:
            self._databuffer['lastupdate'] = time()
    ...
    def bufdata(self):
        ...
        PIDebug('GCSMessages.bufdata: %d datasets', self._databuffer['index'])
        ...
        with self._lock:
            ...
            if gcsdata != 0:
                ...
                if splitpos < 0:
                    ...
                    raise GCSError(err, f'GCSMessages.read, no {stopon} in {answer}')
                stopon += ' \n'
                splitpos += len(stopon)
    ...
    def _savelog(self, msg):
        ...
        with open(self.logfile, 'a', encoding='utf-8') as fobj:
            fobj.write(f'{msg}\n')
        ...
        for i, char in enumerate(answer[:-1]):
            ...
            if char == '\n' or char == '\r':
                if i > 0 and answer[i - 1] != ' ':
                    msg = f'GCSMessages._check_no_eol: LF/CR at {answer[max(0, i - 10):min(i + 10, len(answer))]:r}'
    ...
    def _readgcsdata(self, strbuf):
        ...
        PIDebug('GCSMessages: start background task to query GCS data')
        self._stopthread = False
            ...
            while True:
                ...
                for line in lines:
                    ...
                    if self._endofdata(line):
                        PIDebug('GCSMessages: end background task to query GCS data')
                ...
                try:
                    ...
                    PIError('GCSMessages: end background task with GCSError: %s', exc)
                    self._databuffer['size'] = True
                    return
                if stop():
                    PIError('GCSMessages: stop background task to query GCS data')
                    return
    ...
    def _convertfloats(self, line):
        ...
        msg = f'cannot convert to float: {line}'
        try:
            values = [float(x) for x in line.split()]
            if numcolumns != len(values):
                msg = f'expected {numcolumns:d}, got {len(values):d} columns: {line:r}'
                raise ValueError()
        except ValueError:
            exc = GCSError(gcserror.E_1004_PI_UNEXPECTED_RESPONSE, msg)
            self._databuffer['error'] = exc
            PIError('GCSMessages: GCSError: %s', exc)
        else:
    ...
    def _endofdata(self, line):
        ...
        if eol(line) and self._databuffer['size'] and self._databuffer['index'] < self._databuffer['size']:
            msg = f'{self._databuffer["size"]} expected, {self._databuffer["index"]:d} received'
    ...
    def _checkerror(self, senderr=True, doraise=True):
        ...
        try:
            ...
            exc = GCSError(gcserror.E_1004_PI_UNEXPECTED_RESPONSE, f'invalid answer on "ERR?": {answer:r}')
        else:
            if err:
