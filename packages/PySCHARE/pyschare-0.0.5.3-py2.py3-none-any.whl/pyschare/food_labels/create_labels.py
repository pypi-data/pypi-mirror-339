...
from PySCHARE.pyschare.helpers.create_widgets import create_helper, get_text_fields, get_label_buttons, get_uploader, get_date_picker, get_text_area,get_variable_text_area

from PySCHARE.pyschare.helpers.constants import datafacts_helper, metatable_helper, provenance_helper, dictionary_helper, stats_helper, bar_plot_helper, plot_helper, correlation_helper, metadata, output_names, button_labels, field_names
...
meta_input_area = wd.VBox([fields['filename'], fields['format'], fields['url'], fields['domain'], fields['keywords'], fields['type'],
                      fields['geography'], fields['data_collection_method'],fields['time_method'], fields['rows'], fields['columns'], fields['cdes'],
...
pro_input_area = wd.VBox([fields['source_name'], fields['source_url'], fields['source_email'], fields['author_name'], fields['author_url'], fields['author_email'], save_button['pro']])
...
def get_meta():
    return display( wd.VBox([metatable_help], layout=vbox_layout), wd.VBox([meta_input_area], layout=vbox_layout),  outputs['meta'])

def get_provenance():
    return display( wd.VBox([provenance_help], layout=vbox_layout), wd.VBox([pro_input_area], layout=vbox_layout),  outputs['pro'])

def get_dictionary():
    return display(wd.VBox([dictionary_help]), wd.VBox([dictionary_uploader]), dropdown_container1, outputs['var'])

def get_data_summary():
    return display(wd.VBox([stats_help]),wd.VBox([data_uploader], layout=vbox_layout),wd.VBox([show_data_button], layout=vbox_layout), wd.VBox([outputs['show_data']]))
