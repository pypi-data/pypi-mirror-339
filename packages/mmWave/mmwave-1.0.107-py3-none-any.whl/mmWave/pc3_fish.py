...
# hardware:(Batman-501/601)ISK  AOP type
# config file:
...
# v1     : 2023/08/11 v6 structure: readsxyz
# 
# v1.0.1 : xyzreadsf
#.         add timeTag in recording_v2() 
#          modified  readFile_v2(self,fileName)
#          
#
import serial
import time
...
class Fish:
	...
	def __init__(self,port):
		self.port = port
		print("(jb)PC3 fish initial")
		print("(jb)vsersion:v0.1.0")
		print("(jb)For Hardware:Batman-501/601(ISK)")
		print("(jb)Hardware: IWR-6843 ES2.0")
		print("(jb)Firmware: PC3-fish")
...
#
		...
		while True:
			...
			if lstate == 'idle':
				...
				if (idx%dataBytes == 0):
					try:
						#print(":".join("{:02x}".format(c) for c in sbuf))
						#(e,a,d,r,s) = struct.unpack('2bh2H', sbuf)
						...
						if (df == 'DataFrame'):
							...
							v6.append((sx,sy,sz,ran,elv,azi,dop,snr,self.hdr.frameNumber))
	...
	#
	def recording_v2(self,writer= None, v6=None,fn = None,time_tag = None):
		if writer is not None:
			#ts = datetime.now()
			if v6 is not None:  #ok
				if len(v6) > 0:
					for i in v6: 
						if time_tag != None:
							item = [fn,'v6'] + list(i)  + [time_tag]
						else:
							item = [fn,'v6'] + list(i)
						writer.writerow(item)
		
	def readFile_v2(self,fileName):
		#fileName = "project-19-10-02-17.csv"  
		...
		self.v6simo = df[df['type'] == 'v6'].rename(columns={ 'i0': 'X', 
															  'i1': 'Y', 
															  'i2': 'Z', 
															  'i3': 'range', 
															  'i4': 'elv', 
		...
		return (self.sim_startFN,self.sim_stopFN,self.v6simo)
	...
	def getRecordData_v2(self,frameNum):
		...
		#print(f'(getRecordData_v2:v6simo) = {self.v6simo}')
		v6d = self.v6simo[self.v6simo['fN'] == s_fn]
		#print(f'pc3_fish(v6d) = {v6d}')
		chk = 0
		if v6d.count != 0:
			chk = 1
		return (chk,v6d)
