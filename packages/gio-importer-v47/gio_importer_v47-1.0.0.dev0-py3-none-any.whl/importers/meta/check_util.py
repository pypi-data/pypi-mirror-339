...
def check_item_key(key):
    """
     校验主体标识符KEY，不能为空，长度小于100，只可以由字母数字下���线组成，且只能字母开头
    :param key: 字符串
    :return:
    """
    if key == '':
...
def check_item_key_variables(key):
    ...
    if key == '':
        ...
        logging.error("主体���段<{}>不合法，只可以由字母数字下划线组成，且只能字母开头~".format(key))
        exit(-1)
...
def check_event_var_exsit(key,precision):
    """
     校验事件属性标识符KEY是否存在
     :param key: 标识符
     :return:
    ...
    for var in event_variables:
        if var['key'] == key:
            # 如果传递了 precision 参数，检查是否与 var 中的 precision 匹配
            if precision is not None:
                if 'precision' in var and var['precision'] == precision:
                    return True, var  # precision 匹配，返回 True 和 var
                else:
                    return False, ''  # precision 不匹配，返回 False
            else:
                return True, var  # 未传递 precision 参数，直接返回 True 和 var

    # 如果未找到对应的 key
    return False, ''


def check_user_var_exsit(key,precision):
    """
     校验用户属性标识符KEY是否存在
     :param key: 标识符
     :return:
