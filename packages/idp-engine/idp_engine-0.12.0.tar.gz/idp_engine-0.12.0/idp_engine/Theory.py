...
class Theory(object):
    ...
    from idp_engine.Relevance import determine_relevance
    from idp_engine.EN import EN_theory as EN
    from idp_engine.Propagate import (
        _set_consequences_get_changed_choices,
        _directional_todo,
        _batch_propagate,
        _propagate_inner,
        _first_propagate,
        _propagate_using_models,
        _propagate_ignored,
        _propagate,
        _z3_propagate,
    )
    ...
    def __init__(
        self, *theories: Union[TheoryBlock, Structure, Theory], extended: bool = False
    ) -> None:
        ...
        self.def_constraints: dict[
            Tuple[SymbolDeclaration, Definition], List[Expression]
        ] = {}
        self.interpretations: dict[
            str, SymbolInterpretation
        ] = {}  # interpretations given by user
        self.extensions: dict[
            str, Extension
        ] = {}  # computed extension of types and predicates
        self.name: str = ""

        self._contraintz: Optional[List[BoolRef]] = None
        self._formula: Optional[BoolRef] = (
            None  # the problem expressed in one logic formula
        )
        self.co_constraints: Optional[OrderedSet] = (
            None  # Constraints attached to subformula. (see also docs/zettlr/Glossary.md)
        )
        ...
        self.expl_reifs: dict[
            BoolRef, Tuple[BoolRef, Expression]
        ] = {}  # {reified: (constraint, original)}
    ...
    def solver(self) -> Solver:
        ...
        if self._slvr is None:
            ...
            assignment_forms = [
                a.formula().translate(self)
                for a in self.assignments.values()
                if a.value is not None and a.status == S.UNIVERSAL
            ]
    ...
    def solver_reified(self) -> Solver:
        if self._reif is None:
            ...
            for defin in self.definitions:
                def_constraints.update(
                    defin.get_def_constraints(self, for_explain=True)
                )

            for constraint in chain(
                self.constraints, self.co_constraints, chain(*def_constraints.values())
            ):
    ...
    def copy(self) -> Theory:
        ...
        out.definitions = deepcopy(self.definitions)
    ...
    def add(self, *theories: Union[TheoryBlock, Structure, Theory]) -> Theory:
        ...
        for block in theories:
            ...
            for decl in self.declarations.values():
                if type(decl) == TypeDeclaration:
                    decl.interpretation = (  # TODO side-effects ? issue #81
                        None
                        if decl.name not in [INT, REAL, DATE, CONCEPT]
                        else decl.interpretation
                    )

            # process block.interpretations
            for name, interpret in block.interpretations.items():
                assert (
                    name not in self.interpretations
            ...
            if isinstance(block, TheoryBlock) or isinstance(block, Theory):
                ...
                self.definitions += [deepcopy(x) for x in block.definitions]
        ...
        if symbol_interpretation:
            for t in symbol_interpretation.enumeration.tuples:
                ...
                relevant = self.declarations[RELEVANT].symbol_expr
        ...
        self.constraints = OrderedSet([v.interpret(self, {}) for v in self.constraints])
        ...
        self.co_constraints = OrderedSet(
            [c.interpret(self, {}) for c in self.co_constraints]
        )
    ...
    def enable_law(self, code: str):
        ...
        assert any(
            e.original.code == code for _, e in self.expl_reifs.values()
    ...
    def constraintz(self) -> List[BoolRef]:
        ...
        if self._constraintz is None:
            ...
            self._constraintz += [
                s.translate(self) for s in chain(*self.def_constraints.values())
            ]
    ...
    def get_core_atoms(self, statuses: List[S]) -> List[Assignment]:
        return [
            a
            for a in self.assignments.values()
            if (self.extended or not a.sentence.is_reified()) and a.status in statuses
        ]
    ...
    def _is_defined(self, model, q):
        ...
        if type(q) == AppliedSymbol:
            if any(type(T.decl) != TypeDeclaration for T in q.decl.domains):
                in_domain = q.decl.has_in_domain(
                    q.sub_exprs, self.interpretations, self.extensions
                )
                if in_domain.same_as(FALSE):
                    ...
                    z3_val = model.eval(in_domain.translate(self))
                    if str(z3_val) == str(in_domain).replace("()", ""):
    ...
    def _new_questions_from_model(self, model, ass: Assignments) -> List[Expression]:
        ...
        for decl in self.declarations.values():
            if (
                type(decl) == SymbolDeclaration
                and decl.arity == 1
                and decl.instances is None
                and not decl.name in RESERVED_SYMBOLS
                and decl.name in self.z3
            ):  # declared but not used in theory
                interp = model[self.z3[decl.name]]
                collect_questions(interp, decl, ass, out)
        return out

    def _from_model(
        self, solver: Solver, todo: List[Expression], complete: bool
    ) -> Assignments:
        ...
        for q in todo:
            ...
            if not self._is_defined(model, q):
                ...
                if (
                    ...
                    # Don't use the original code `q.code` as this symbol
                    # might have interpreted arguments. E.g., `hole(wall())`
                    # might have been simplified to `hole(Tile)`; using the
                    # original may result in incorrect results.
                    val = maps.get(q.simplified_code(), _else)
                else:
                    val = None
                if val is None:
    ...
    def optimize(self, term: str, minimize: bool = True) -> Theory:
    ...
    def get_range(self, term: str, complete: bool = False) -> List[str]:
