...
from typing import TYPE_CHECKING, Tuple, Dict, List, Generator, Optional
from z3 import (
    Solver,
    sat,
    unsat,
...
def _set_consequences_get_changed_choices(self: Theory, tag: S) -> Tuple[Dict, List]:
    ...
    for a in self.assignments.values():
        if a.status in [S.GIVEN, S.DEFAULT, S.EXPANDED]:
            if a.sentence.code in removed_choices and removed_choices[
                a.sentence.code
            ].value.same_as(a.value):
    ...
    if not removed_choices:
        for a in self.previous_assignments.values():
            if a.status == tag and self.assignments[a.sentence.code].status not in [
                S.GIVEN,
                S.EXPANDED,
                S.DEFAULT,
            ]:
...
def _directional_todo(
    self: Theory, removed_choices: Dict = {}, added_choices: List = []
) -> Dict:
    ...
    * if choices are added, all unknown atoms should be checked
    ...
    if removed_choices:
        ...
        for a in self.previous_assignments.values():
            if a.status in [S.CONSEQUENCE, S.ENV_CONSQ] or a.is_certainly_undefined:
                todo[a.sentence.code] = a.sentence
...
def _batch_propagate(self: Theory, tag: S = S.CONSEQUENCE) -> Generator:
    ...
    if todo:
        ...
        if result == sat:
            ...
            for q in todo:
                solver.add(
                    q.reified(self) == q.translate(self)
                )  # in case todo contains complex formula
                if solver.check() != sat:
                    # print("Falling back !")
                    yield from self._propagate(tag=tag)
                test = Not(
                    q.reified(self) == solver.model().eval(q.reified(self))
                )  # TODO compute model once
                tests.append(test)
                lookup[str(test)] = q
            solver.push()
            while True:
                ...
                if result == sat:
                    tests = [
                        t for t in tests if is_false(solver.model().eval(t))
                    ]  # TODO compute model once
...
def _propagate_inner(self: Theory, tag: S, solver: Solver, todo: Dict) -> Generator:
    ...
    if res1 == sat:
        ...
        for q in new_todo:
            if (
                isinstance(q, Expr.AppliedSymbol)
                ...
                val = maps.get(q.simplified_code(), _else)  # val may be None
            else:
                val = None
            if val is None:
        ...
        if cons[0] == unknown:
            ...
            yield from self._propagate_using_models(solver, valqs, tag)
            yield "No more consequences."
            return
...
def _first_propagate(self: Theory, solver: Solver, complete: bool = False) -> Generator:
    ...
    if cons[0] == unknown:
        ...
        # back on the "old" propagation. For some reason, working with models
        # does not have this limitation (but it is generally much slower).
        yield from self._propagate_using_models(solver, valqs, S.UNIVERSAL)
        solver.pop()
        return
    assert cons[0] == sat, "Incorrect solver behavior"
...
def _propagate_using_models(
    self: Theory, solver: Solver, valqs: List, tag: S
) -> Generator:
...
def _propagate_ignored(
    self: Theory, tag: S = S.CONSEQUENCE, given_todo: Optional[Dict] = None
) -> Generator:
    ...
    todo = (
        ...
        if given_todo
            ...
            for a in self.assignments.values()
            if a.status not in [S.GIVEN, S.DEFAULT, S.EXPANDED]
            and (a.status != S.STRUCTURE or a.sentence.code in self.ignored_laws)
        }
    )
...
def _propagate(
    ...
    if not removed_choices and not added_choices:
        ...
        for a in todo.values():
            if a.code in self.assignments and self.assignments[a.code].status not in [
                S.UNKNOWN
            ]:
                to_remove.append(a)
...
def _z3_propagate(self: Theory, tag: S = S.CONSEQUENCE) -> Generator:
    """generator of new propagated assignments.  Update self.assignments too.
    ...
    if todo:
        ...
        if result == sat:
            for consq in consqs:
                ...
                if is_not(consq):
                    value, atom = Expr.FALSE, atom.arg(0)
                else:
                    value = Expr.TRUE
                # try to unreify it
                if atom in unreify:
                    ...
                    assert value == Expr.TRUE, f"Internal error in z3_propagate"
