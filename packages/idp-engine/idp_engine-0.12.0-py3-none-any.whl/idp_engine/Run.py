...
def model_expand(
    *theories: Union[TheoryBlock, Structure, Theory],
    ...
    ms = list(
        problem.expand(max=max, timeout_seconds=timeout_seconds, complete=complete)
    )
...
def model_propagate(
    *theories: Union[TheoryBlock, Structure, Theory],
    ...
    solve_start = time.time()
    if sort:
        ms = [str(m) for m in problem._propagate(tag=S.CONSEQUENCE, complete=complete)]
        ms = sorted(ms[:-1]) + [ms[-1]]
        out = ""
...
def maximize(*theories: Union[TheoryBlock, Structure, Theory], term: str) -> Theory:
...
def minimize(*theories: Union[TheoryBlock, Structure, Theory], term: str) -> Theory:
...
def decision_table(
    *theories: Union[TheoryBlock, Structure, Theory],
    ...
    solve_start = time.time()
    models, timeout_hit = problem.decision_table(
        goal_string, timeout_seconds, max_rows, first_hit, verify
    )
    PROCESS_TIMINGS["solve"] += time.time() - solve_start
    for model in models:
        row = f"{NEWL}∧ ".join(str(a) for a in model if a.sentence.code != goal_string)
        has_goal = model[-1].sentence.code == goal_string
        yield (
            f"{(f'  {row}{NEWL}') if row else ''}"
...
def even_safer_getattr(object, name, default=None, getattr=getattr):
    """
    Even safer getattr implementation, as wrapper around
    `RestrictedPython.Guards.safer_getattr` (which is also used internally)
    We also add a check on `.get` to prevent e.g.
    `os = globals().get("__builtins__").get("__loader__").load_module("os")`
    """
    if name in ["get", "load_module"]:
        raise AttributeError(f"'{name}' attr is not allowed")
    return RestrictedPython.Guards.safer_getattr(object, name, default, getattr)


def visit_Name(self, node):
    """
    We override restrictedPython's visit_Name to ensure it does not
    replace our print statements.
    """
    self.check_name(node, node.id)
    return node


# We have to monkey-patch the visit_Name function and the forbidden
# set of keywords to allow normal printing.
RestrictedPython.transformer.RestrictingNodeTransformer.visit_Name = visit_Name
forbidden = set(RestrictedPython.transformer.FORBIDDEN_FUNC_NAMES)
forbidden.remove("print")
RestrictedPython.transformer.FORBIDDEN_FUNC_NAMES = forbidden
...
def execute(self: IDP, capture_print: bool = False) -> Optional[str]:
    ...
    # By default, RestrictedPython requires explicitly (re)defining possibly
    # unsafe functions. See:
    ...
    mybuiltins = {
        "_getattr_": even_safer_getattr,
        "__builtins__": RestrictedPython.Guards.safe_builtins,
        'print': myprint  # default print
    }
    ...
    try:
        byte_code = RestrictedPython.compile_restricted(
            main, filename="internal execution", mode="exec"
        )
        exec(byte_code, mybuiltins, mylocals)
