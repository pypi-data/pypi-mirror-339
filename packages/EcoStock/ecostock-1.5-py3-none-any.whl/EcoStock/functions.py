...
def get_stock_data(ticker, start_date, end_date):
    ...
    # Check if the DataFrame has a multi-index column and drop the top level if necessary
...
def plot_candlestick(ticker, start_date, end_date):
    ...
    fig.add_trace(go.Bar(x=stock_data.index, 
                         y=stock_data['Volume'], 
                         marker=dict(color=np.where(stock_data['Close'] >= stock_data['Open'], 'green', 'red')), 
                         yaxis='y2'))
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            ...
            domain=[0.3, 1],  # Occupy the top 70% of the chart
            showgrid=True  # Show gridlines for better readability
        ),
        yaxis2=dict(
            title='Volume',  # Add a title for the volume axis
            domain=[0, 0.2],  # Occupy the bottom 20% of the chart
            anchor='x',
            showgrid=False  # Hide gridlines for the volume chart
        ),
        xaxis=dict(
            rangeselector=dict(
                buttons=list([
        ...
        margin=dict(
            t=80,  # Increased top margin to avoid overlap with the title
            b=50   # Bottom margin to avoid overlap with the x-axis labels
        ),
        height=600  # Set the height of the chart for better readability
    )
...
def annual_returns(tickers, start_year, end_year):
    ...
    def calculate_annual_return(ticker, start_date, end_date):
        ...
        Returns:
        float: Annual return of the stock, or NaN if data is missing.
        """
        # Fetch the stock data for the given ticker and date range
        ...
        if 'Adj Close' in stock_data.columns:
            ...
            return np.nan  # Return NaN if no valid price column is found

        # Check if there are enough data points
        if prices.empty:
            return np.nan  # Return NaN if no data is available for the given period

        # Calculate the annual return by comparing the start and end prices
    ...
    annual_returns = {ticker: [calculate_annual_return(ticker, f"{year}-01-01", f"{year}-12-31") for year in years] for ticker in tickers}

    # Debug: Print the calculated annual returns
    print("Annual Returns (raw):", annual_returns)

    # Convert annual returns to percentages
    annual_returns_percent = {ticker: [ret * 100 if pd.notna(ret) else None for ret in returns] for ticker, returns in annual_returns.items()}

    # Debug: Print the annual returns in percentage
    print("Annual Returns (percent):", annual_returns_percent)

    # Create a bar chart for the annual returns of each stock
    traces = [go.Bar(x=years, y=returns, name=ticker) for ticker, returns in annual_returns_percent.items()]

    # Define the layout for the plot
    layout = go.Layout(
        title=f'Annual Returns of Portfolio from {start_year} to {end_year}',
        xaxis=dict(title='Year'),
        yaxis=dict(title='Return (%)', tickformat=".2f"),  # Format the y-axis as a percentage
        showlegend=True
    )
...
def rolling_volatility(ticker, start_date, end_date, window):
    ...
    # Calculate the rolling volatility by taking the standard deviation of the percentage change in the prices
    rolling_volatility = prices.pct_change().rolling(window).std()
...
def lm_prediction(ticker, start_date, end_date, window_size):
    ...
    for i in range(window_size, len(data)):
        ...
        y_train = data['Close'].iloc[i-window_size:i].values.reshape(-1, 1)  # Dependent variable (adjusted close price)
