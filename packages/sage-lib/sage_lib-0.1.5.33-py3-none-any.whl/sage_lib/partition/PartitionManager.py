...
class PartitionManager(FileManager, AtomicProperties): 
    ...
    def add_empty_container(self, ):
        ...
        Parameters:
            container (object): The container object to be added.
        """
        self.containers.append( SingleRun() )
    ...
    def apply_sorting_order(self, order: list) -> bool:
        """
        Applies a sorting order to the containers.

        Parameters
        ----------
        order : list
            A list of indices specifying the new order of the containers.

        Returns
        -------
        bool
            True if the sorting was successfully applied.
        """
        # Reordenar los contenedores según el índice de orden proporcionado
        self._containers = [self.containers[i] for i in order]
        return True
