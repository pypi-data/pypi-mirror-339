...
class SMK:
    def __init__(self, n_clusters=3, kernel="linear", C=1.0, p=2, degree=3,
        ...
        self.param_grid = param_grid if param_grid is not None else {
            ...
            'degree': [2, 3]  # only relevant when kernel='poly'
        }
    ...
    def fit(self, X, y=None):
        ...
        if y is None:
            ...
            if self.enable_grid_search:
                ...
                grid_search = GridSearchCV(SVC(), self.param_grid, cv=3, n_jobs=-1, verbose=0)
                grid_search.fit(X_scaled, y)
                self.svm = grid_search.best_estimator_
                self.kernel = self.svm.kernel
                self.C = self.svm.C
                self.degree = self.svm.degree if hasattr(self.svm, 'degree') else self.degree
    ...
    def visualize_clusters(self, X):
        ...
        plt.scatter(reduced[:, 0], reduced[:, 1], c=labels, cmap='viridis', edgecolor='k')
