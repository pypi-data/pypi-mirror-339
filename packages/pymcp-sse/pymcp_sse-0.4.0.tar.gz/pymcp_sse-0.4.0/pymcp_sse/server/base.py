...
from typing import Any, Dict, Optional, Callable, Awaitable, List, AsyncIterator
...
class BaseMCPServer:
    ...
    def __init__(
        ...
        Args:
            ...
            lifespan: An asynchronous context manager for managing startup and shutdown events.
        """
        self.server_name = server_name
        self.ping_interval = ping_interval
        self._user_lifespan = lifespan # Store user-provided lifespan
    ...
    def _create_app(self) -> FastAPI:
        ...
        async def lifespan_manager(app: FastAPI):
            """Manage server lifecycle, incorporating user lifespan if provided."""
            user_cm = None
            try:
                # --- User Setup ---
                if self._user_lifespan:
                    logger.info(f"[{self.server_name}] Running user-defined startup logic...")
                    user_cm = self._user_lifespan(app)
                    await user_cm.__aenter__()
                    logger.info(f"[{self.server_name}] User-defined startup logic complete.")

                # --- Internal Setup ---
                logger.info(f"Server '{self.server_name}' starting up internal components...")
                # (Add any other internal setup here if needed in the future)
                logger.info(f"Server '{self.server_name}' internal components started.")

                yield # Application runs here

            finally:
                # --- Internal Teardown ---
                logger.info(f"Server '{self.server_name}' shutting down internal components...")
                # Gracefully stop pings for any remaining connections
                active_conns = list(self.active_connections.values())
                if active_conns:
                    logger.info(f"Stopping ping tasks for {len(active_conns)} connections...")
                    await asyncio.gather(*(conn.stop_ping() for conn in active_conns), return_exceptions=True)
                    self.active_connections.clear()
                    logger.info(f"Ping tasks stopped and active connections cleared.")
                else:
                    logger.info("No active connections to clean up.")
                logger.info(f"Server '{self.server_name}' internal components shut down.")

                # --- User Teardown ---
                if user_cm:
                    logger.info(f"[{self.server_name}] Running user-defined shutdown logic...")
                    # Determine exception details to pass to __aexit__
                    exc_type, exc_value, traceback = sys.exc_info() # We might need to import sys for this
                    try:
                        await user_cm.__aexit__(exc_type, exc_value, traceback)
                        logger.info(f"[{self.server_name}] User-defined shutdown logic complete.")
                    except Exception as e:
                         logger.error(f"[{self.server_name}] Error during user-defined shutdown: {e}", exc_info=True)
    ...
    def run(self, host: Optional[str] = None, port: Optional[int] = None, **kwargs):
        ...
        # Read MCP_PORT from environment *first* as it's specific to this library
        env_port_str = os.environ.get("MCP_PORT")
        # Explicit port argument takes highest precedence
        if port is not None:
            final_port = port
            logger.debug(f"Using explicitly provided port: {final_port}")
        # Then check environment variable
        elif env_port_str:
            try:
                final_port = int(env_port_str)
                logger.debug(f"Using port {final_port} from MCP_PORT environment variable.")
            except ValueError:
                logger.warning(f"Invalid MCP_PORT environment variable '{env_port_str}'. Using default 8000.")
                final_port = 8000
        else:
            final_port = 8000 # Fallback default
            
        # Use determined host/port in log message and uvicorn.run
        logger.info(f"Starting {self.server_name} on {final_host}:{final_port}")
        # Ensure log_level is passed down if present in kwargs
        if 'log_level' not in kwargs:
             kwargs['log_level'] = get_logger("uvicorn").level # Match our logger level by default
             if isinstance(kwargs['log_level'], int):
                 kwargs['log_level'] = logging.getLevelName(kwargs['log_level']).lower()
    ...
    async def run_with_tasks(
        ...
        **uvicorn_kwargs
        ...
        try:
            ...
            logger.debug(f"Uvicorn server task created for {final_host}:{final_port}") # Add log
