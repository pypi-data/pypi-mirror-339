...
from prophecy_lineage_extractor.constants import (
    ...
    SLEEP_TIME,
    CATALOG_COL,
    PROCESS_NAME_COL, INBOUND_COL, RUN_FOR_ALL_PIPELINES, RECURSIVE_EXTRACT
)
...
from prophecy_lineage_extractor.utils import (
    ...
    append_to_csv,
    format_sql, debug,
    KEEP_RUNNING, get_safe_datasetId, get_prophecy_name
)


class WorkflowMessageHandler:

    def __init__(self, project_id, pipeline_ids):
    ...
    def _get_lineage_for_given_pipeline(self, json_msg, datasets):
        ...
        if (safe_env_variable(RUN_FOR_ALL_PIPELINES) != "false"):
            ...
            os.environ[RECURSIVE_EXTRACT] = "true"
    ...
    def _get_lineage_from_detailed_view(self, data):
        ...
        for pipeline_id in self.pipeline_ids:
            ...
            for column in columns:
                ...
                safe_env_variable(RECURSIVE_EXTRACT) != "true"):
                    lineage_data.append(
                        {
                ...
                for upstream in column.get("upstreamTransformations", []) :
                    ...
                    for transformation in transformations:
                        ...
                        if trnsf_pipeline_id != pipeline_id or safe_env_variable(RUN_FOR_ALL_PIPELINES) != "false":
            ...
            append_to_csv(df, f"lineage_{self.project_id}_{get_prophecy_name(pipeline_id)}_{get_safe_datasetId(datasetId)}.csv")
            self.datasets_processed.append(datasetId)
            # import pdb; pdb.set_trace()
