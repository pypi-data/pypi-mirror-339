...
class BaseApiUserLogModel(BaseModel):
    ...
    def user_created_id(cls):   # type: ignore
        return mapped_column(PG_UUID(as_uuid=True), db.ForeignKey('api_user.id'), nullable=False, comment="Идентификатор пользователя, создавшего запись")

    @declared_attr
    def user_modified_id(cls):  # type: ignore
        return mapped_column(PG_UUID(as_uuid=True), db.ForeignKey('api_user.id'), nullable=False, comment="Идентификатор пользователя, изменившего запись")
