...
class MarkdownTranslator:
    ...
    def _split_into_chunks(self, content: str, max_chunk_size: int = 2000) -> List[str]:
        ...
        This method ensures that special elements like frontmatter, code blocks,
        ...
        # First, handle frontmatter if present
        if lines and lines[0].strip() == "---":
            frontmatter_chunk.append(lines[0])
            i = 1
            while i < len(lines):
                line = lines[i]
                frontmatter_chunk.append(line)
                if line.strip() == "---":
                    # Add frontmatter as a special chunk with a marker
                    chunks.append(
                        "__FRONTMATTER_START__\n"
                        + "\n".join(frontmatter_chunk)
                        + "\n__FRONTMATTER_END__"
                    )
                    i += 1
                    break
    ...
    async def _translate_single_chunk(self, chunk: str, target_language: str) -> str:
        ...
        # Check if this is a frontmatter chunk
        if chunk.startswith("__FRONTMATTER_START__") and chunk.endswith(
            "__FRONTMATTER_END__"
        ):
            # Remove the markers and return the frontmatter unchanged
            return chunk[len("__FRONTMATTER_START__\n") : -len("\n__FRONTMATTER_END__")]
        ...
        The text contains HTML tags that MUST be preserved exactly as they appear.
        Each HTML tag has been replaced with a placeholder (e.g., __HTML_0__, __HTML_1__, etc.).
        DO NOT translate or modify these placeholders in any way.

        HTML Tag Preservation Rules:
        1. Count and preserve the EXACT number of each HTML tag type from the source text
        2. Each opening tag (<tag>) must have a corresponding closing tag (</tag>)
        3. Never remove or add HTML tags during translation
        4. Tag types must remain the same (e.g., <i> stays as <i>, <b> stays as <b>)

        Tag Positioning Rules:
        1. You may reposition tags to maintain natural translation flow
        2. Tags can be moved to wrap different parts of the translated text
        3. Ensure the semantic meaning of the tagged content is preserved

        Verification Requirements:
        Before submitting the translation:
        1. Count opening and closing tags in both source and translation
        2. Ensure the counts match exactly
        3. Verify all tags are properly paired

        Common Mistakes to Avoid:
        1. Don't merge multiple tagged sections into one
        2. Don't split a single tagged section into multiple parts
        3. Don't forget to close tags that were opened
        4. Don't add new tags that weren't in the source
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "   - All placeholders in the format __HTML_123__ exactly as they appear\n"
                        "   - All image paths exactly as they appear\n"
                        "5. CRITICAL: DO NOT modify, translate, or remove placeholders like __HTML_123__\n"
                        "6. NEVER insert or add any HTML tags that aren't in the original text\n"
                    ),
                },
    ...
    def _protect_html_content(self, text: str) -> Tuple[str, Dict[str, str]]:
        ...
        """
        # Instead of matching individual tags, we'll find all HTML content as a single unit
        # This ensures that opening and closing tags stay together
        html_map = {}
        counter = 0
        ...
        for i, (start, end, tag, tag_name, tag_type) in enumerate(html_tags):
            if tag_type == "opening":
                ...
                if matching_closing:
                    ...
                    html_content = text[element_start:element_end]
                    ...
                    protected_text = (
                        protected_text[: element_start + offset]
                        + placeholder
                        + protected_text[element_end + offset :]
                    )
        ...
        return protected_text, html_map
