...
class BappFrameworkApiClient:
    ...
    def __init__(self, base_url: str = None, token: str = None, bearer: str = None):
        self.headers = {}
        if base_url:
    ...
    def api_call(self, method: Literal['get', 'post', 'put', 'patch', 'delete'], path: str, params: dict = None, json: dict = None, files: dict = None, data: dict = None):
        ...
        response = requests.request(method, url, headers=self.headers, params=params, files=files, data=data, json=json)
        return response

    def prepare_multipart(self, data=None, files=None):
        """Helper method to prepare data for multipart form submission"""
        multipart_data = {}

        # Convert data to multipart-encoded tuples
        if data:
            multipart_data.update({key: (None, value) for key, value in data.items()})
    ...
    def call_task(self, task_code: str, data: dict = None, files: dict = None):
        ...
        if files:
            payload = self.prepare_multipart(final_payload, files)
        else:
            payload = final_payload
        # Call the API
        return self.handle_response(self.api_call('post', 'tasks', files=payload if files else None, json=None if files else payload))
    ...
    def create(self, content_type:str, data: dict, files: dict = None):
        if files:
            return self.handle_response(self.api_call('post', f'content-type/{content_type}/', files=self.prepare_multipart(data, files)))
    ...
    def update(self, content_type:str, pk: str, data: dict, files: dict = None):
        if files:
            return self.handle_response(self.api_call('put', f'content-type/{content_type}/{pk}', files=self.prepare_multipart(data, files)))
    ...
    def patch(self, content_type:str, pk: str, data: dict, files: dict = None):
        if files:
            return self.handle_response(self.api_call('patch', f'content-type/{content_type}/{pk}/', files=self.prepare_multipart(data, files)))
