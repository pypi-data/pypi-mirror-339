...
class TaskLogic:
    ...
    def __init__(self,vnc,vnc_port,queue_handle):
        ...
        self.role_sect=self.ini_data_dict .get("role_sect") # 获取角色门派
        ...
        self.vnc_sync = VNC_recognition(self.vnc_ip, self.vnc_port)  # 实例化 VNC 识别对象,同步操作
    ...
    def find_image_scope(self,image_path,x1,y1,x2,y2,con:float=0.8,model:int=0):
        """
        局域图片识别,可以选用去除背景色的功能
        ...
        try:
            # 使用切片裁剪数组
            if self.data_numpy.any():
                if model == 0 : # 如果模型类型为0且启用状态为True
                    sub_images_dict.update({image_path:(x1, y1, x2, y2, con)})

                    # 查找小图在大图中的位置
                    results = self.image_finder.find(self.data_numpy, sub_images_dict)
                    # logger.success(f"图片识别结果:{sub_images_dict},{results}")
                    return results
                if model == 1 : # 如果模型类型为1且启用状态为True
                    sub_images_dict_ac.update({image_path:(x1, y1, x2, y2, con)})
                    results_ac=self.image_matcher.match_sub_images(self.data_numpy, sub_images_dict_ac)
                    # logger.success(f"图片识别结果:{results_ac},{results_ac}")
                    return results_ac
            return {}
        except Exception as e:
            logger.error(f"find_image_scope函数出错:{e}")
            return {}
    ...
    def queue_screenshot_sync(self, delay_time:float=1.5):
        """
        :param delay_time: 延迟时间
        """
        numpy_data = self.vnc_sync.capture_full_screen_as_numpy() #同步截图
    ...
    def data_info(self):
        ...
        if self.diff_time<-1:
            logger.error(f"数据不可信,休息{abs(self.diff_time)}秒")
            time.sleep(abs(self.diff_time))
            return False
    ...
    def interface_closes(self):
        ...
        try:
            ...
            elif self.find_data_from_keys_list_click(["resource/images_info/other/侠店关闭.png"], self.image_data,
    ...
    def role_info(self):
        ...
        if self.node_counter >= 5 or (self.role_scoring !="-1" and not self.role_name):
    ...
    def find_map_npc(self,npc_name:list,class_name:str="",drag_flg:bool=False,delay_time=15):
        """
        快捷搜索寻找npc
        ...
        x1, y1, x2, y2 = -1, -1, -1, -1
        if class_name in ["商店"]:
            x1,y1,x2,y2=974, 224, 1015, 233
        elif class_name in ["功能"]:
            x1,y1,x2,y2=987, 249, 1013, 258
        elif class_name in ["传送"]:
            x1,y1,x2,y2=978, 273, 1015, 283
        elif class_name in ["副本"]:
            x1,y1,x2,y2=1054, 222, 1091, 233
        elif class_name in ["任务"]:
            x1,y1,x2,y2=1054, 249, 1090, 257
        elif class_name in ["活动"]:
            x1,y1,x2,y2=1052, 272, 1090, 283
                    ...
                    if not self.clear_flag:
                        ...
                        if res_dict:
                            ...
                            for scope in scope_list:
                                ...
                                self.mouse_left_click_scope(x1,y1,x2,y2,delay_time=1.1)
                            self.mouse_move_scope(587, 323, 835, 482,delay_time=2)
                            ...
                            res_bool=self.find_color_exists_in_array(x1,y1,x2,y2,"#fdf758",50)
                            logger.error(f"颜色识别{res_bool}")
                            if not res_bool:
                                logger.error("功能未勾选")
                                self.mouse_left_click_scope(x1,y1,x2,y2,delay_time=2)
                                self.mouse_move_scope(587, 323, 835, 482, delay_time=2)
                                return True
                            elif res_bool:
                                if drag_flg:
    ...
    def task_flow(self):
        ...
        任务流程
        """
        self.data_info() # 获取数据信息

        if self.last_map_numpy.size==0: # 如果地图数据不存在
            self.last_map_numpy=self.data_numpy

        if self.last_map_numpy.size: # 如果地图数据存在
            # 创建图像差异查找器
        ...
        if self.update_role_basic_info_flag :
