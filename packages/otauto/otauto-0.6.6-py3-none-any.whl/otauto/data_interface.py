...
def main(vnc: VNC_recognition, parameter_dict, queue_handle, interval: float = 0.5, image_w: int = 1440,image_h: int = 900, screen_num=1):
    """
    主函数
        ...
        while True:
            ...
            if screen_flag:
                updated_image = vnc.capture_full_screen_as_numpy(image_w=image_w, image_h=image_h,
                                                                 screen_num=screen_num)  # 使用同步捕获方法
            ...
            if not queue_handle.empty():
                ...
                if "screen_flag" in queue_msg:
                    ...
                    if result_queue.empty():
                       pass
                    logger.error("使用队列中的numpy_data")
                    updated_image = queue_msg["numpy_data"]

                elif "image" not in queue_msg and "word" not in queue_msg and "mutil_colors" not in queue_msg:
...
def run_sync(vnc: VNC_recognition, parameter_dict, queue_handle, interval: float = 0.6, image_w: int = 1440,image_h: int = 900, screen_num=1):
    """
    运行同步函数
    :param vnc: VNC_recognition对象
    :param parameter_dict: 参数字典
    ...
    :param screen_num: 屏幕标识
    """
    main(vnc, parameter_dict, queue_handle, interval, image_w, image_h, screen_num)
...
def run_in_process(vnc,parameter_dict, task_instance,queue_handle,loop_num:int=20,delay_time=0.5,interval: float = 0.6,image_w: int = 1440, image_h: int = 900,screen_num:int=1):
    """
    在进程中运行任务
    :param vnc: VNC_recognition对象
    :param parameter_dict: 参数字典
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        # 提交任务
        executor.submit(run_sync, vnc, parameter_dict, queue_handle,interval,image_w, image_h,screen_num)  # 提交第一个任务,用于捕获图像,识别图像,处理数据
