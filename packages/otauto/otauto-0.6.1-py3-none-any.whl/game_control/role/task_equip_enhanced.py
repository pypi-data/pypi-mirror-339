...
class TaskEquipEnhanced(TaskLogic):
    ...
    def _process_装备强化(self, gear_type, next_task):
        if self.node_flag:
            ...
            logger.info(f"装备点位:{gear_name_point_dict[gear_type]}")
        ...
        if self.equip_position:
            ...
            if self.node_flag:
                ...
                if "装备强化界面" in self.interface_info:
                    ...
                    elif self.find_data_from_keys_list_click(["resource/images_info/equipment_strengthening/无材料.png"],self.image_data,x3=208,y3=82,delay_time=1):
                    ...
                    if self.find_data_from_keys_list_click(["resource/images_info/equipment_strengthening/强化图标.png"],self.image_data,x3=10,y3=15,delay_time=1):
                        self.key_press("B", delay_time=1) #关闭背包
                        self.key_press("B",delay_time=1) #背包界面前置显示
                        self.mouse_right_click(*self.equip_position, delay_time=1) #丢入装备
                        self.interface_closes() #关闭背包
                        time.sleep(2)
                        self.node_counter = 0  # 重置节点计数器
                        return True
            ...
            if "背包界面" in self.interface_info:
                ...
                if differences:
                    self.equip_position=differences[0][:2]
