...
from resource.parameters_info.basic_parameter_info import (skill_arrangement_少林,skill_arrangement_天煞,
                                                           skill_arrangement_凌云,skill_arrangement_蜀山,shortcut_bar_dict)
from otauto.opencv_image.gray_color import gray_ratio
...
class TaskSkillConfig(TaskLogic):
    ...
    def find_entry(self,data, specified_name):
        ...
        if numerical_entries:
            # 找出带数字条目中数字最大的一条
            max_numerical_entry = max(numerical_entries, key=lambda x: int(''.join(filter(str.isdigit, x[0]))))
            ...
            if res_dict["ocr_dict"] and res_dict["set_dict"]:#说明技能栏的技能和配置文件不匹配
                ...
                if res_shortcut_dict: #如果存在技能错乱的情况
                    for coord in list(res_shortcut_dict.values()):
                        ...
                        self.mouse_drag(coord[0]+random_num,coord[1]+random_num,coord[0]+random_num,coord[1]-random_num-60,110)
                        time.sleep(0.5)
                    ...
                    for shortcut_number in self.skill_allocation:
                        ...
                        if shortcut_number in self.skill_rage_num:
                            ...
                            if res_coor_tuple:#怒气技能设置
                                ...
                                self.mouse_drag(res_coor_tuple[0], res_coor_tuple[1], shortcut_coord[0], shortcut_coord[1]+10, 60)
                                time.sleep(2)
...
#怒气技能位置
skill_rage_num_少林=["2","4","7"]
skill_rage_num_凌云=["8",]
skill_rage_num_蜀山=["2","8"]
skill_rage_num_天煞=["6","7"]

#技能界面中的技能位置
skill_config_少林_scope={
    "1": ["resource/images_info/role_skill/龙形棍.png",478, 435, 521, 478],
    "2": ["resource/images_info/role_skill/沾字决.png",478, 251, 935, 294], #怒气技能
    "3": ["resource/images_info/role_skill/明王震.png",790, 591, 837, 634],
    "4": ["resource/images_info/role_skill/舍身卫道.png",458, 350, 913, 405], #怒气技能
    "5": ["resource/images_info/role_skill/豹尾脚.png",790, 434, 833, 478],
    "6": ["resource/images_info/role_skill/虎形棍.png",635, 435, 676, 478],
    "7": ["resource/images_info/role_skill/缠字决.png",478, 303, 935, 345], #怒气技能
