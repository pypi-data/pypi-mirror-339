...
from otauto import CoordinateConverter
...
class TaskAstar(TaskLogic):
    ...
    def __init__(self,vnc,vnc_port,queue_handle):
        ...
        self.target_colors_hex = ["6ec8fa", "70c8fa", "e02a35", "d23d4d", "9b8aad"]  # 允许通行的颜色
        self.route_color = ["e02a35", "d23d4d", "9b8aad"]  # 惩罚值最低的颜色
        ...
        if not self.moving_flag and self.goal_list: # 移动标识为False且节点列表不为空
            ...
            if traits_dict is not None and traits_dict["role_position"] != (-1,-1) :
                ...
                if abs(start[0] - self.goal[0]) <= 2 and abs(start[1] - self.goal[1]) <= 2:
                    ...
                    # 4 进行路径规划,A星算法
                    res_list = self.path_finder.find_path(self.map_path, start, self.goal, self.target_colors_hex, self.route_color)
                    """
