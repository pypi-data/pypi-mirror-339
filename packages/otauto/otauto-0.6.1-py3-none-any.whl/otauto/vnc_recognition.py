...
class VNC_recognition:
    ...
    def capture_full_screen_as_numpy(self, debug: bool = False, retries: int = 3, delay: float = 0.5) -> np.ndarray:
        ...
        for attempt in range(retries):
            try:
                numpy_image = self.vnc.image_numpy_locality() # 调用 vnc.image_numpy_locality 方法
    ...
    def capture_region_as_numpy(self, x1: int, y1: int, x2: int, y2: int, debug: bool = False):
        ...
        try:
            numpy_image = self.vnc.image_numpy_locality(x1, y1, x2, y2)
            if debug:
                # 将 NumPy 数组转换为图像
                image = Image.fromarray(numpy_image)
                # 拼接保存路径
    ...
    async def async_capture_full_screen_as_numpy(self, interval: float = 0.3, debug: bool = False):
        ...
        while True:
            try:
                # 捕获当前屏幕截图
                current_image = self.vnc.image_numpy_locality(debug=debug)
    ...
    def mouse_move(self,x,y,delay_time):# 鼠标移动
        """
        鼠标移动
        :param x: int:x坐标
        :param y: int:y坐标
