...
from queue import Empty  # 从标准库的 queue 模块导入 Empty
...
#模式导入
# from pattern.task_production import * # 生产模式
# from pattern.task_setting import task_setting # 游戏设置
from pattern.task_test import task_test # 任务测试
# from pattern.test_vnt_tools import vnc_tool_test # vnc_tools函数测试
...
def task_op(row, queue, win_title, running_mode:str="测试模式",log_debug: bool = True):
    ...
    if running_mode=="测试模式": #todo,测试模式代码入口
        ...
        # task_res=task_起号(int(win_hwnd),queue)#todo:暂时只有起号
        """
        参数说明
        py: pyautogui对象
        win_hwnd: 窗口句柄
        ...
        logger.info("进入设置模式")
        # task_res=task_setting( int(win_hwnd), queue)
        """
        参数说明
        py: pyautogui对象
        win_hwnd: 窗口句柄
...
class MainApp:
    ...
    def create_widgets(self):
        ...
        # 创建单选按钮
        modes = ["测试模式", "生产模式", "vnc_tool模式", "设置模式"]
        for mode in modes:
    ...
    def process_queue(self, queue, win_hand, item):
        ...
        while True:
            ...
            try:
                ...
                if isinstance(message, dict):#todo,改成读取Redis数据库的信息
                    logger.warning(f"进程队列信息: {message}")

                    # 获取当前选中行的 win_hand
                    selected_item = self.tree.selection()
                    if selected_item:
                        selected_item_values = self.tree.item(selected_item, 'values')
                        current_win_hand = selected_item_values[4]  # 获取当前行的 vnc_port

                        # 检查消息中的 win_hand 是否与当前选中的行的 win_hand 相同
                        if message.get('vnc_port') == int(current_win_hand):
                            logger.info("消息中的 win_hand 与当前选中的行的 win_hand 相同，更新表格数据。")
                            task_name = message.get('task_name', '')
                            task_info = message.get('task_message', '')
                            position_info = message.get('position_info', '')
                            health_degree = message.get('health_degree', '')
                            team_info=message.get('team_info', '')
                            combat_info=message.get('combat_info', '')

                            # 更新表格中对应的行，保持列0和列1的数据不变
                            new_values = (
                                selected_item_values[0],  # 保持 win_num 不变
                                selected_item_values[1],  # 保持 running_status 不变
                                selected_item_values[2],  # 保持 win_hand 不变
                                selected_item_values[3],
                                selected_item_values[4],
                                task_name,  # 更新列5
                                task_info,  # 更新列6
                                position_info,  # 更新列7
                                health_degree, # 更新列8
    ...
    def find_windows(self):
        """查找窗口"""
        target_class_name = "VNCMDI_Window"
        valid_titles = [f"{i:03}" for i in range(1, 21)]
        all_windows = Desktop(backend="uia").windows()
        found_windows = [
            (window.class_name(), window.window_text().strip(), window.handle)
            for window in all_windows
            if window.class_name() == target_class_name and window.window_text().strip() in valid_titles
        ]
        found_windows.sort(key=lambda x: int(x[1]))
