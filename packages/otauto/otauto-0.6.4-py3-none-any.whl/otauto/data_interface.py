...
async def main(vnc: VNC_recognition, parameter_dict, queue_handle, interval: float = 0.6,image_w: int = 1440, image_h: int = 900,screen_num=2):
    """
    主函数
    ...
    try:
        async for updated_image in vnc.async_capture_full_screen_as_numpy(interval, debug=False,image_w=image_w, image_h=image_h,screen_num=screen_num):
            ...
            if not queue_handle.empty():
                ...
                if "numpy_data" in queue_msg:
                    while not result_queue.empty():  # 清空队列
                        result_queue.get()
                    if result_queue.empty():
                        logger.error(f"result_queue队列已清空")
                    logger.error(f"使用队列中的numpy_data")
                    updated_image=queue_msg["numpy_data"]
                    # data_treating_debug=True
...
def run_asyncio_in_thread(vnc: VNC_recognition, parameter_dict,queue_handle, interval: float = 0.6,image_w: int = 1440, image_h: int = 900,screen_num=2):
    """
    在线程中运行异步函数
    :param vnc: VNC_recognition对象
    :param parameter_dict: 参数字典
    ...
    :param screen_num: 屏幕标识
    """
    asyncio.run(main(vnc, parameter_dict, queue_handle, interval,image_w, image_h,screen_num))
...
def run_in_process(vnc,parameter_dict, task_instance,queue_handle,loop_num:int=20,delay_time=0.5,interval: float = 0.6,image_w: int = 1440, image_h: int = 900,screen_num:int=2):
    """
    在进程中运行任务
    :param vnc: VNC_recognition对象
    :param parameter_dict: 参数字典
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        # 提交任务
        executor.submit(run_asyncio_in_thread, vnc, parameter_dict, queue_handle,interval,image_w, image_h,screen_num)  # 提交第一个任务,用于捕获图像,识别图像,处理数据
