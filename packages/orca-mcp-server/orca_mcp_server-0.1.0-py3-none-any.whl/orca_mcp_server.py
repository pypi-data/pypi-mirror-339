...
ORCA_REQUEST_TIMEOUT = float(os.environ.get("ORCA_REQUEST_TIMEOUT", "30.0"))
COMMON_HEADERS = {"Content-Type": "application/json", "User-Agent": "orca"}
...
async def app_lifespan(server: FastMCP) -> AsyncIterator[AppContext]:
    ...
    # Skip schema for now
    # schema_resp = await client.get(f"{ORCA_API_HOST}/api/sonar/schema")
    # schema: Dict[str, Any] = (
    #     schema_resp.json().get("data", {}) if schema_resp.status_code == 200 else {}
    # )
    schema: Dict[str, Any] = {}
    try:
        yield AppContext(client=client, schema=schema)
    finally:
        # Cleanup on shutdown
        await client.aclose()
...
async def ask_orca(ctx: Context, question: str) -> dict:
    ...
    try:
        ...
        query_resp = await client.post(
            ...
            json={"query": query, "start_at_index": 0, "limit": 10},
        )
        query_resp.raise_for_status()
        query_results = query_resp.json().get("data", {})

        if not query_results:
            return {"status": "error", "error": "No results found for your query."}

        return {"status": "success", "results": query_results}
