...
def logstash_http_server() -> Generator[int, None, None]:
    ...
    logstash_server = HTTPServer(("0.0.0.0", logstash_port), LogstashHttpServer)
...
@mark.asyncio
async def test_logstash_logs() -> None:
    loop = get_running_loop()
    log_future = loop.create_future()

    def on_log_recieved(log: LogstashLogRecord) -> SenderResponse:
...
@mark.asyncio
async def test_logstash_log_retries() -> None:
    loop = get_running_loop()
    log_future = loop.create_future()
...
@mark.asyncio
async def test_logstash_log_retries_with_exceptions() -> None:
    loop = get_running_loop()
    log_future = loop.create_future()
...
def test_logstash_log_record_properties() -> None:
...
def test_logstash_log_exception_causes() -> None:
...
def test_logstash_log_exception_contexts() -> None:
...
def test_logstash_log_record_dict() -> None:
    log = LogstashLogRecord(
...
@mark.asyncio
async def test_logstash_too_many_log_retries() -> None:
    loop = get_running_loop()
    log_retries_done = loop.create_future()

    def logger(log: LogstashLogRecord) -> SenderResponse:
...
@mark.asyncio
async def test_logstash_send_exception() -> None:
    loop = get_running_loop()
    log_retries_done = loop.create_future()
...
@mark.asyncio
async def test_logstash_exception_logger() -> None:
    loop = get_running_loop()
...
@mark.asyncio
async def test_logstash_stop_hook_exception() -> None:
    class MockLogstashSender(BaseLogstashSender):
...
@mark.asyncio
async def test_logstash_stop_hook_async_exception() -> None:
    class MockLogstashSender(BaseLogstashSender):
        def __init__(self) -> None:
            super().__init__(service_name="test")
...
@mark.asyncio
async def test_logstash_closed_failed_log_queue() -> None:
    loop = get_running_loop()
