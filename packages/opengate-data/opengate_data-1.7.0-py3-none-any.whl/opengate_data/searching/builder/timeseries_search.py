...
class TimeseriesSearchBuilder(SearchBuilder):
    ...
    def _validate_builds(self):
        ...
        if "with_organization_name" not in self.method_calls or "with_identifier" not in self.method_calls:
            raise Exception("It is mandatory to use 'with_organization_name()' and 'with_identifier()' methods")
    ...
    def execute(self):
        ...
        Example:
            ~~~python
                new_timeseries_search_builder.with_format("csv").with_organization_name(organization).build().execute()
                new_timeseries_search_builder.with_filter(filter).with_organization_name(organization).with_format("pandas").build().execute()
                new_timeseries_search_builder.with_filter(filter).with_organization_name(organization).with_format("dict").build().execute()
            ~~~
        """
