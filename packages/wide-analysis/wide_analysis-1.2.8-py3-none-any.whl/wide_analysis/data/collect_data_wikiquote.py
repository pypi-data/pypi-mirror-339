...
# def collect_wikiquote_date(year, month=None, base_url='https://en.wikiquote.org/wiki/Wikiquote:Votes_for_deletion/Log/'):
#     titles = []
#     text_urls = []
#     labels = []
#     deletion_discussions = []
    
#     # If no month is provided, use common month names to iterate through the year
#     if month is None:
...
#         url = f"{base_url}{year}_{month_name}"
...
#             no_divs = len(divs)
#             print(f"Found {no_divs} div(s) with the expected classes for {month_name} {year}.")
            
#             if no_divs >= 1:
#                 for div in divs:
#                     heading_div = div.find('div', class_="mw-heading mw-heading2 ext-discussiontools-init-section")
#                     if heading_div:
#                         heading = heading_div.find('h2')
#                         if heading:
#                             found_title = heading.text.strip()
#                             if '[edit]' in found_title:
#                                 found_title = found_title.replace('[edit]', '')
#                         else:
#                             # Try to get the title from the heading div directly
#                             found_title = heading_div.text.strip()
#                             if '[edit]' in found_title:
#                                 found_title = found_title.replace('[edit]', '')
                        
#                         titles.append(found_title)
#                         text_url = url + '#' + found_title.replace(' ', '_')
#                         text_urls.append(text_url)
#                         label = extract_outcome_from_div(div)
#                         labels.append(label)
#                         deletion_discussions.append(div.prettify())
...
#                         print("No heading div found with the expected classes.")
#             else:
#                 print(f"No divs found for {month_name} {year}.")
#                 continue
#         else:
#             print(f"Failed to retrieve the page for {month_name} {year}. Status code: {response.status_code}")
#             continue
    
#     if titles:  # Check if we've found any data
#         df = pd.DataFrame({'title': titles, 'text_url': text_urls, 'label': labels, 'discussion': deletion_discussions})
#         df = process_html_to_plaintext(df)
#         df = process_split_text_into_sentences(df)
#         df['label'] = df['label'].replace({
...
#         return df
#     else:
#         print(f"No deletion discussions found for {year} {month if month else '(all months)'}.")
#         return None

# def collect_wikiquote(mode='title', title='all', url='', year=None, month=None):
#     if mode not in ['title', 'url', 'date']:
#         raise ValueError("mode must be either 'title', 'url', or 'date'.")
    
#     if mode == 'title':
#         base_url = 'https://en.wikiquote.org/wiki/Wikiquote:Votes_for_deletion_archive'
#         df = collect_wikiquote_title(title, base_url)
#         if df is not None and 'discussion_cleaned' in df.columns:
#             df = df[['title', 'text_url', 'label', 'discussion_cleaned']]
#             df = df.rename(columns={'discussion_cleaned': 'discussion'})
#         return df
...
#     elif mode == 'url':
#         if not url:
#             raise ValueError("URL must be provided for 'url' mode")
#         df = collect_wikiquote_title('all', url)
#         title = url.split('#')[-1].replace('_', ' ')
#         df = df[df['title'].str.lower() == title.lower()].reset_index(drop=True)
#         if not df.empty and 'discussion_cleaned' in df.columns:
#             df = df[['title', 'text_url', 'label', 'discussion_cleaned']]
#             df = df.rename(columns={'discussion_cleaned': 'discussion'})
#         return df
...
#     elif mode == 'date':
#         if not year:
#             raise ValueError("Year must be provided for 'date' mode")
#         return collect_wikiquote_date(year, month)
...
def filter_label(label):
    standard = ['delete', 'keep', 'redirect', 'merge', 'no-consensus']
    words = label.split()
    if len(words) > 2:
        lower = label.lower()
        for token in standard:
            if token in lower:
                return token
    return label
...
def process_split_text_into_sentences(df):
    if df.empty:
        return df
    df['discussion_cleaned'] = df['discussion_cleaned'].apply(split_text_into_sentences)
    df['discussion_cleaned'] = df['discussion_cleaned'].apply(lambda x: x.replace(
...
def collect_wikiquote_title(title='all', base_url='https://en.wikiquote.org/wiki/Wikiquote:Votes_for_deletion_archive'):
    ...
    labels = []
    deletion_discussions = []
    url = base_url if title == 'all' else base_url + '#' + title.replace(' ', '_')
    ...
    if response.status_code == 200:
        ...
        print(f"Found {len(divs)} div(s) with the expected classes.")
        if divs:
            for div in divs:
                heading_div = div.find('div', class_="mw-heading mw-heading2 ext-discussiontools-init-section")
                if heading_div:
                    found_title = heading_div.text.strip().replace('[edit]', '')
                    titles.append(found_title)
                    text_url = base_url + '#' + found_title.replace(' ', '_') 
                    text_urls.append(text_url)
                    label = extract_outcome_from_div(div)
            ...
            df = process_split_text_into_sentences(df)
            df['label'] = df['label'].replace({
                'Deleted':'delete', 'Delete':'delete', 'delete':'delete', 'deleted':'delete', 'deleted.':'delete', 
                'speedy deleted test page':'delete', 'Deleted and protected with a message':'delete',
                'delete both':'delete', 'delete everything':'delete', 'Deleted due to copyvio':'delete', 
                'delete after various merges':'delete', 'delete 3 quoteless, keep 1 redirect':'delete',
                'Consensus to remove from Wikiquote, but only if it is not merged into another article':'delete',
                'Consensus to remove from Wikiquote, but not how':'delete', 'delete, pending technical fix':'delete', 
                'delete all':'delete', 'delete Portal:portal, no consensus/keep Template:Wikimedia':'delete',
                'Speedy-deleted':'delete', 'Speedy deleted':'delete', 'Speedy-deleted, no meaningful content':'delete',
                'kept':'keep', 'Kept.':'keep', 'Keep':'keep', 'keep':'keep', 'Kept':'keep', 
                'No consensus/keep':'keep', 'kept/no consensus':'keep', 'Kept; lack of consensus':'keep', 
                'kept after copyvio removal':'keep', 'Speedy-kept':'keep', 'Speedy kept':'keep',
                'merge':'merge', 'Merge':'merge', 'merged':'merge', 'Merged':'merge', 
                'merged into Azerbaijani proverbs':'merge', 'Merge with Stephen Covey':'merge', 
                'Merge with Lyrics':'merge', 'merge and redirect':'merge', 
                'merge with Crusade (TV series)':'merge', 'Merged to Health':'merge', 
                'merge with 3rd Rock from the Sun':'merge',
                'redirect to List of proverbs':'redirect', 'keep as redirect':'redirect', 
                'Redirect to Inuyasha':'redirect', 'Redirected to Humor':'redirect', 
                'Redirected to Doctor Who':'redirect', 'Redirect without text':'redirect', 
                'Proverbs turned to redirect to List of proverbs':'redirect', 'redirect to Drugs':'redirect', 
                'redirect to Advertising slogans':'redirect',
                'redirect to Jalal al-Din Muhammad Rumi':'redirect', 'redirect':'redirect', 
                'Redirected':'redirect', 'move to Category:United States Marines':'redirect', 
                'move to Die Hard: With a Vengeance':'redirect',
                'move to Star Wars Jedi Knight: Jedi Academy':'redirect', 
                'move to Lucien Lévy-Bruhl':'redirect', 'move to Dave Finlay':'redirect', 
                'move to User:Quenzer':'redirect', 'moved':'redirect', 
                'moved to Monument inscriptions':'redirect', 'transwiki to Wikipedia, then delete':'redirect', 
                'Transwiki to Wikipedia':'redirect',
                'delete His Holiness the Dalai Lama, redirect Dalai Lama to Tenzin Gyatso, 14th Dalai Lama':'redirect',
                'move':'redirect', 'keep Just war theory, redirect Just war, delete Just War Theory':'no_consensus',
                'move to Wikisource':'redirect', 'kept.':'keep', 'Keep as Redirect':'redirect', 
                'Deleted.':'delete', '1 delete, 1 redirect':'redirect', 'moved to User:Quenzer':'redirect',
                'transwiki, then delete':'delete', 'merge with Lyrics':'redirect','Deleted all three images':'delete',
                'No consensus':'no_consensus', 'no consensus':'no_consensus', 
                'inconclusive; no action taken.':'no_consensus', 'UNIDENTIFIED':'no_consensus'
            })
            df['label'] = df['label'].apply(filter_label)
            if 'discussion_cleaned' in df.columns:
                df = df[['title', 'text_url', 'label', 'discussion_cleaned']]
                df = df.rename(columns={'discussion_cleaned': 'discussion'})
            return df
...
def collect_wikiquote_date(year, month=None, base_url='https://en.wikiquote.org/wiki/Wikiquote:Votes_for_deletion/Log/'):
    ...
    for month_name in months:
        ...
        if response.status_code == 200:
            soup = BeautifulSoup(response.text, 'html.parser')
            divs = soup.find_all('div', class_='boilerplate metadata vfd')
            print(f"Found {len(divs)} div(s) with the expected classes for {month_name} {year}.")
            if divs:
                for div in divs:
                    heading_div = div.find('div', class_="mw-heading mw-heading2 ext-discussiontools-init-section")
                    if heading_div:
                        heading = heading_div.find('h2')
                        if heading:
                            found_title = heading.text.strip().replace('[edit]', '')
                        else:
                            found_title = heading_div.text.strip().replace('[edit]', '')
                        titles.append(found_title)
                        text_url = url + '#' + found_title.replace(' ', '_')
    ...
    if titles:
        ...
        df['label'] = df['label'].replace({
            ...
            'delete both':'delete', 'delete everything':'delete', 'Deleted due to copyvio':'delete', 
            'delete after various merges':'delete', 'delete 3 quoteless, keep 1 redirect':'delete',
            'Consensus to remove from Wikiquote, but if it is not merged into another article':'delete',
            'Consensus to remove from Wikiquote, but not how':'delete', 'delete, pending technical fix':'delete', 
            'delete all':'delete', 'delete Portal:portal, no consensus/keep Template:Wikimedia':'delete',
            'Speedy-deleted':'delete', 'Speedy deleted':'delete', 'Speedy-deleted, no meaningful content':'delete',
            'kept':'keep', 'Kept.':'keep', 'Keep':'keep', 'keep':'keep', 'Kept':'keep', 
...
def collect_wikiquote(mode='title', title='all', url='', year=None, month=None):
    ...
    if mode == 'title':
        ...
        if not url:
            raise ValueError("URL must be provided for 'url' mode")
        df = collect_wikiquote_title('all', url)
        title_from_url = url.split('#')[-1].replace('_', ' ')
        df = df[df['title'].str.lower() == title_from_url.lower()].reset_index(drop=True)
        if not df.empty and 'discussion_cleaned' in df.columns:
            df = df[['title', 'text_url', 'label', 'discussion_cleaned']]
            df = df.rename(columns={'discussion_cleaned': 'discussion'})
        return df
