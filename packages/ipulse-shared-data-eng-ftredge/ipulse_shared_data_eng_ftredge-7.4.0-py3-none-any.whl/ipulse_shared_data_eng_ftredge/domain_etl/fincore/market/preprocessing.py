...
def _get_provider_processor(records_origin_short_ref: str) -> Optional[Callable]:
    ...
    # def process_eodhd_eod_historic_bulk_single_symbol(record):
    #     # Extract date value
    #     date_value = record.pop('date', None)
    #     # Ensure it's a proper DATE format (YYYY-MM-DD) with no time component
    #     if date_value:
    #         # Split by 'T' (ISO format) or space (other formats) and take first part
    #         date_value = str(date_value).split('T')[0].split(' ')[0]
    #     return {'date_id': date_value}
    
    # def process_eodhd_eod_last_day_multiple_symbols(record):
    #     # Extract date value
    #     date_value = record.pop('date', None)
    #     # Ensure it's a proper DATE format (YYYY-MM-DD) with no time component
    #     if date_value:
    ...
    # processors = {
    #     "eodhd__eod_historic_bulk_single_symbol": process_eodhd_eod_historic_bulk_single_symbol,
    #     "eodhd__eod_last_day_multiple_symbols": process_eodhd_eod_last_day_multiple_symbols
    # }
    ...
    try:
        ...
        for record in processed_records:
            ...
            record.update({
                ...
                "exchange_code_pulse": exchange_code_pulse,
                "asset_category_pulse": asset_category_pulse,
                "change_id": change_id,
    ...
    Args:
        ...
        assets_records: Dictionary where:
            ...
            - values are dictionaries containing:
                ...
                - asset_category_pulse: Asset category
    ...
    try:
        ...
        for asset_id, asset_data in assets_records.items():
            ...
            required_fields = ["records_input", "asset_symbol_pulse", "exchange_id", 
                              "exchange_code_pulse", "asset_category_pulse", "event_time", "change_id"]
            ...
            for record in processed_records:
                # First, remove all fields that aren't in our schema
                unwanted_fields = ['code', 'exchange_short_name', 'prev_close', 'change', 'change_p']
                for field in unwanted_fields:
                    if field in record:
                        record.pop(field, None)
                
                # Apply processor to transform date to date_id
                processed_fields = processor(record)
                record.update(processed_fields)
                ...
                record.update({
                    ...
                    "asset_category_pulse": asset_data["asset_category_pulse"],
                    "change_id": asset_data["change_id"],
                    "updated_at": event_time,
                    # Add missing fields required by the schema
                    "values_history": asset_data.get("values_history", None),
                    "changelog_history": asset_data.get("changelog_history", None)
                })
        ...
        result.add_metadata(
            processed_description='Batch provider-specific preprocessing completed',
        ...
        if pipelinemon:
            pipelinemon.add_log(StructLog(
                ...
                description=f"Batch processed {len(assets_records)} assets with {total_processed_records} total records"
            ))
...
def market_multi_symbol_common_preproc(
    ...
    try:
        ...
        result.data = processed_results
        result.add_metadata(
            processed_description='Batch common preprocessing completed' + 
                                 (' with price rounding' if round_prices else ''),
            total_processed_records=total_processed_records
        )
        result.final()
        ...
        if pipelinemon:
            pipelinemon.add_log(StructLog(
                ...
                progress_status=result.progress_status,
                q=total_processed_records,
                description=f"Batch common processed {len(assets_records)} assets with {total_processed_records} total records"
            ))
