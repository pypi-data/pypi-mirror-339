from core.utils import verify, generate
...
class TestGenerate:
    def test_invalid_generation_type(self):
        with pytest.raises(ValueError) as excinfo:
            generate(
                generation_type = 'human', properties = ImageGenProperties(engine = 'imagen',
                delination = "test"),
                api_key = valid_api_key
            )
            
        assert 'is an invalid value for generation_type; valid choices are' in str(excinfo)
    ...
    def test_invalid_properties_type(self):
        ...
        human_properties = HumanGenProperties(
            name = 'John Doe',
            age = 25,
            sex = 'male'
        )
