...
def _get_executable_from_repo(repo_path_: str,
    ...
    potential_functions_with_inputs = [t[1] for t in getmembers(python_module, isfunction) if hasattr(t[1], 'inputs')]

    if len(potential_functions_with_inputs) != 1:
        raise Exception('potential_functions_with_inputs != 1')
    final_function = potential_functions_with_inputs[0]
    inputs_provided_by_kawa = ['df', 'kawa', 'data_preview']
    ...
    if len(missing_arguments) != 0:
        raise InterpreterError(f'Some arguments defined in the main function: {final_function.__name__} '
                               f'are not defined. The list : {missing_arguments}')
...
def _run_script(script_runner_inputs: ScriptRunnerInputs):
    ...
    try:
        if script_runner_inputs.callback:
