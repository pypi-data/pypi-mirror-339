...
async def get_images_from_pagination(url: str, headers: Optional[dict] = None) -> List[str]:
    pages_count_query = (
        "nav[aria-label='Pagination Navigation'] > div > span > a:not([aria-label='Next &raquo;'])"
    )
    ...
    [source_urls.add(a.attrs["href"]) for a in soup.select(pages_count_query)]

    return list(source_urls)


async def get_sources_for_ugirls(
    sources: List[str],
    entity: str,
