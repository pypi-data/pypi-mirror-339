    ...
    for source_url in tqdm_sources_iterable:
        ...
        if entity == "nudecosplaygirls.com":
            image_tags = [
                ...
                if "emoji" not in tag.attrs.get("src", "")
                and "emoji" not in tag.attrs.get("data-lazy-src", "")
            ]
            ...
            if iframe_element:
                ...
                for idx, video_tag in enumerate(video_tags):
                    ...
                    unique_media_urls.add((f"{video_name}", video_src))
        elif entity == "imgcup.com":
            if not image_tags:
                query = (
                    "div.penci-post-gallery-container div.inner-gallery-masonry-container "
                    "div a div img"
                )
                src_attr = "data-src"
        ...
        if unique_media_urls:
            ordered_unique_img_urls.update(unique_media_urls)
