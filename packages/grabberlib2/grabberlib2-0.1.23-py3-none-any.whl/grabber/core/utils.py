...
from typing import Any, AsyncGenerator, Coroutine, Dict, Iterable, List, Optional, Tuple, cast
...
async def get_tags(
    ...
    query: str,
    headers: Optional[Dict[str, Any]] = None,
    uses_js: Optional[bool] = False,
    should_retry: Optional[bool] = True,
    bypass_cloudflare: Optional[bool] = False,
...
async def create_html_template(image_tags: IndexedSet, entity: str | None = "") -> str:
    ...
    video_html_template = '<video src="{video_path}" preload="metadata" controls="controls" poster="{video_poster}" muted></video>'
...
async def build_unique_img_urls(
    ...
    for idx, img_tag in enumerate(image_tags):
        ...
        if not img_src or "gif" in img_src:
            img_src = (
                img_tag.attrs.get(secondary_src_attr, "")
                .strip()
                .rstrip()
                .replace("\r", "")
                .replace("\n", "")
    ...
    ordered_unique_img_urls = IndexedSet(
        list(sorted(unique_img_urls, key=lambda x: list(x).pop(0)))
    )
...
async def build_unique_video_urls(
    ...
    ordered_unique_video_urls = IndexedSet(
        list(sorted(unique_video_urls, key=lambda x: list(x).pop(0)))
    )
...
async def send_post_to_telegram(
    ...
    if len(ordered_unique_img_urls) >= MAX_IMAGES_PER_POST:
        async for chunk in split_every(chunk_size, ordered_unique_img_urls):
            was_successful, channels_sent = await telegraph_uploader(
    ...
    if source_url not in all_sources and was_successful:
        for channel in channels_sent:
            ...
            if not await repository.was_already_posted_in_channel(url=source_url, channel=channel):
                extracted_page = await repository.create(attributes=data)
                all_sources.append(extracted_page.url)
                tqdm_sources_iterable.set_description(f"Saved {page_title} post sent to {channel} in database")

    return all_sources


async def get_all_posts_for_url(repository: ExtractedPageRepository, url: str) -> list[str]:
    all_posts_objects = await repository.filter_by(url=url)
    all_urls = [page.url for page in all_posts_objects]
