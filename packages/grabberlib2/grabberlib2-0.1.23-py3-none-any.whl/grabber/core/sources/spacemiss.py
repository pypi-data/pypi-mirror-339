...
async def get_pages_from_pagination(
    ...
    pagination_pages_query = "div.page-nav.td-pb-padding-side a.page"
    posts_query = "div.td_block_inner.tdb-block-inner.td-fix-index div.td-module-meta-info > h3.entry-title.td-module-title > a"
    all_posts = set()
    soup = await get_soup(url, headers=headers)
...
async def get_sources_for_spacemiss(
    sources: List[str],
    entity: str,
