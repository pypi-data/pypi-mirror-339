...
async def get_pages_from_pagination(
    ...
    pagination_query = "div.pagination div.pagination-holder ul li.next a"
    images_query = "div.box div.list-videos div#list_videos_common_videos_list_items div.item a div.img img"
    ...
    while has_pagination:
        link_tag = has_pagination[0]
        target_url = link_tag.attrs["href"]
        page_response = httpx.get(url=target_url, headers=headers)
        page_content = page_response.content.decode("utf-8")
        soup = BeautifulSoup(page_content, features="lxml")
        image_tags = soup.select(images_query)
        images_tag.update(
            *[tag for tag in image_tags if ".svg" not in tag.attrs["src"]]
        )
...
async def get_sources_for_nudostar(
    sources: list[str],
    entity: str,
    ...
    **kwargs: Any,
    ...
    for source_url in tqdm_sources_iterable:
        ...
        page_title = (
            page_title.split("- NudoStar.TV!")[0].split("Page")[0].split("Nude")[0].replace("https:", "")
        )
