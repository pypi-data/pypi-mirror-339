...
async def get_sources_for_nsfw24(
    sources: List[str],
    entity: str,
    ...
    **kwargs,
    ...
    downloaded_videos: list[tuple[pathlib.Path, str]] = []
    ...
    for video, page_title in downloaded_videos:
        video_input = FSInputFile(path=video.as_posix())
        builder.add_video(media=video_input, caption=page_title)
        channel = "@backupcos0000"
        # channel = "@backprn0099"
        tqdm_sources_iterable.set_description(f"Sending video {video.name} to channel {channel}")
