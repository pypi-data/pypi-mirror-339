...
async def get_images_from_pagination(url: str, headers: dict[str, str] | None = None) -> list[str]:
    pagination_query = "nav.pagination.group div.wp-pagenavi"
    first_page_query = f"{pagination_query} span.current"
    last_page_query = f"{pagination_query} a.last"
    ...
    last_page = soup.select(last_page_query)
    if last_page:
        last_page = last_page[0]
        last_page = last_page.attrs["href"].split("/", maxsplit=4)[-1].split("/")[0]
    ...
    for index in range(first_page_number, last_page_number + 1):
        ...
        target_url = urljoin(url, f"{index}/")
