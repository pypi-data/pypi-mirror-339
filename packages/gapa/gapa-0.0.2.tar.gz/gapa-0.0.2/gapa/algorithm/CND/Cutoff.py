        ...
        with tqdm(total=pop_num * 100) as cbar:
            for _i in range(pop_num):
                ...
                for _generation in range(100):
                    ...
                    mutation_population = self._remove_repeat(mutation_population)
                    new_fitness_list = evaluator(mutation_population)
                    temp_population, temp_fitness = body.elitism(temp_population, mutation_population, temp_fitness, new_fitness_list)
                    cbar.update(1)
...
def Cutoff(mode, max_generation, data_loader, controller: CutoffController, evaluator, world_size, verbose=True):
