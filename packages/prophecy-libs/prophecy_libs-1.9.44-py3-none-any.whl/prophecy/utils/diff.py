...
class DataFrameDiff:
    ...
    def add_column_comparison_results(cls, joined_df):
        ...
        for field in left_struct_fields:
            ...
            if needs_complex_comparison(field.dataType):
                ...
                expr_str = (
                    f"{left_struct}.`{field_name}` <=> {right_struct}.`{field_name}`"
                )
                comparison_expr = (
    ...
    def get_diff_summary_dict(cls, diff_key):
        ...
        match_percentage = (__builtins__.round((rows_matching / total_rows * 100)) if total_rows > 0 else 0)
            ...
            "data": {
                "summaryTiles": [
                    ...
                    },
                    {
                        ...
                        "badgeContent": f"{__builtins__.round(((perfect_val_column_matches + key_columns_match_count)/(len(value_columns) + len(key_columns)))*100)}",
                        "isPositive": perfect_val_column_matches == len(value_columns) and key_columns_match,
                        "order": 1,
