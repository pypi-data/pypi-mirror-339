...
code={}
code['Marine Starter']='''
# 使用autobf 需事先引入下列package
import time
import autobf as bf
from autobf import Yc,Xc
# 滑鼠點擊指定座標，重複repeat次，每次間隔delay秒
bf.click(Xc,Yc,repeat=1,delay=0)
# 自訂義函式 Sum，傳入 a, b 回傳 a+b
def resetUntilOK():
  missNotFinished = bf.checkColorByXY(233,519,color=(255,255,255),color_tolorance=25,XY_tolorance=8)
  # 重複無限次
  while True:
    # 重置遊戲
    bf.reset()
    # 滑鼠滾輪 滾動量：-500 ， 重複repeat次
    bf.zoom(scroll=(-500),repeat=10)
    bf.zoom(scroll=200,repeat=10)
    check = bf.checkColorByXY(857,341,color=(242,230,15),color_tolorance=25,XY_tolorance=8)
    if check:
      print(1)
      bf.zoom2max()
      # 按往Q鍵滑行指定次數，上:''下左右
      bf.slide('d',1);bf.slide('w',3);bf.slide('a',1)
      # 接近指定顏色，offsetX:X方向位移  ; offsetY:Y方向位移
      bf.close2Color(((241,234,41)),offsetX=0,offsetY=45)
      if (not missNotFinished):
        # 取得任務(預設開啟第一個任務)
        bf.getMission(mission=1)
      bf.slide('w',3);bf.press('w',1);bf.press('a',5)
      break
    check = bf.checkColorByXY(777,413,color=(241,225,10),color_tolorance=25,XY_tolorance=8)
    if check:
      print(4)
      bf.zoom2max()
      # 按往Q鍵滑行指定次數，上:''下左右
      ...
      break
    #其它重生點...未完成
    check = bf.checkColorByXY(679,415,color=(241,227,13),color_tolorance=25,XY_tolorance=8)
    if check:
      print(2)
      # 繼續迴圈，跳過下方敘述
      continue
    check = bf.checkColorByXY(775,343,color=(243,232,20),color_tolorance=25,XY_tolorance=8)
    if check:
      print(3)
      # 繼續迴圈，跳過下方敘述
      continue
    check = bf.checkColorByXY(171,440,color=(243,235,26),color_tolorance=25,XY_tolorance=8)
    if check:
      print(5)
      # 繼續迴圈，跳過下方敘述
      continue
...
while True:
  ...
  for i in range(16):
    bf.click(repeat=30)
    if bf.isComplete(targetImgPath='goal.png'):
      # 跳出迴圈
      break
'''
code['沙漠']='''
