...
from saas_base.security.ip import get_client_ip
...
from ..models import Setting, Thread, Comment, Reaction
...
class ThreadItemEndpoint(BrowserEndpoint):
    ...
    def get_tenant_members(tenant_id: int):
        members = Member.objects.filter(tenant_id=tenant_id).values_list('user_id', flat=True).all()
...
class CommentsEndpoint(BrowserEndpoint):
    def get(self, request, *args, **kwargs):
        ...
        queryset = queryset.select_related('webmention')
        if root_id:
    ...
    def post(self, request, *args, **kwargs):
        ...
        check_security_rules(comment_settings.COMMENT_SECURITY_RULES, request)
        thread_id = kwargs['thread_id']
