...
class icelines:
    ...
    def find_icelines(self, super):
        ...
        for temperature, molecule in zip( iceline_molecules_and_temps.values(), molecule_array[1:] ):
            try:
                iceline = np.max( np.where( super.T[0,:] >= temperature ) )
            except ValueError:
                iceline = 0
            print(f'Target temp: {temperature} K')
            print(f'\t{molecule} -> {super.r[iceline]}')
            print(f'\t\tT-> {super.T[0,iceline]}\n')
