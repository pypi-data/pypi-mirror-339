...
import taurus
from taurus.core import TaurusEventType, TaurusDevice, TaurusDevState
from taurus.external.qt import Qt, compat
from taurus.qt.qtgui.base.taurusbase import _DEFAULT
from taurus.qt.qtgui.container import TaurusWidget
from taurus.qt.qtgui.base import TaurusBaseWidget
from taurus.qt.qtgui.button import TaurusCommandButton
from taurus.qt.qtgui.dialog import ProtectTaurusMessageBox, TaurusMessageBox
...
class MacroButton(TaurusWidget):
    ...
    def setModel(self, model):
        ...
        if self.door is not None:
            try:
                ...
                self.debug('Could not disconnect macroStatusUpdated and resultUpdated.')
        ...
        #This block can be removed after taurus version requirement increases
        #to a version that fixes issue #1223
        ###
        try:
            self.door.stateObj.read().rvalue
        except Exception:
            if self.door.getDeviceProxy() is None:
                doorState = TaurusDevState.Undefined
            else:
                doorState = TaurusDevState.NotReady
        ###
        if doorState == TaurusDevState.Undefined:
    ...
    def _updateToolTip(self):
        try:
            ...
            self.setToolTip(self.macro_name)
            return
    ...
    def _statusUpdated(self, *args):
        ...
        # Update progress bar
        self.ui.progress.setRange(int(step_range[0]), int(step_range[1]))
        self.ui.progress.setValue(int(step))

        if state in ['stop', 'abort', 'finish', 'exception']:
            self.ui.button.setChecked(False)

        if state == 'exception':
            exceptionDialog = TaurusMessageBox(MacroRunException,
            status_dict['exc_value'], status_dict['exc_stack'])
...
if __name__ == '__main__':
    ...
    class TestWidget(Qt.QWidget):
        ...
        def getMacroInfo(self, macro_name):
            ...
            try:
                door.stateObj.read().rvalue
            except Exception:
                if door.getDeviceProxy() is None:
                    print("Door {} does not exist!".format(
                        self.door_name))
                else:
                    print("Not possible to read door ({}) state! "
                          "Hint: try to (re)start MacroServer.".format(
                          self.door_name))
                return param_names, default_values
            try:
                pars = door.macro_server.getMacroInfoObj(macro_name).parameters
            except AttributeError:
                print("Macro {} does not exist!".format(macro_name))
                return param_names, default_values
