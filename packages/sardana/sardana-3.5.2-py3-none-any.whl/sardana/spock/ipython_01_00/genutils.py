...
__all__ = ['page', 'arg_split', 'get_gui_mode', 'get_pylab_mode',
           ...
           'is_executing_multiline_cell',
           'print_dev_from_class', 'from_name_to_tango', 'clean_up',
           'get_door', 'get_macro_server',
...
def is_terminal_interactive_shell():
    return isinstance(get_shell(), TerminalInteractiveShell)
...
def is_executing_multiline_cell(ignore_comments=True):
    """Walk Python stack and check if IPython executes a multiline code.

    For example, this returns True::

        In [4]: is_executing_multiline_cell()  # returns True
           ...: foo()

        In [5]: for i in range(1):
           ...:     is_executing_multiline_cell()  # returns True
        ...
        In [7]: # %load foo.py
           ...: is_executing_multiline_cell(ingore_comments=False)  # returns True

    and this returns False::

        In [8]: is_executing_multiline_cell()  # returns False

        In [9]: # %load foo.py
           ...: is_executing_multiline_cell()  # returns False

    If it is not possible to check it then raise RuntimeError.
    """
    is_multiline_cell = False
    for frame_info in inspect.stack(context=1000):
        if (frame_info.filename.startswith("<ipython-input")  # normal spock
                or "ipykernel_" in frame_info.filename):  # qtspock
            break
    else:
        raise RuntimeError(
            "could not determine if IPython "
            "is executing a multiline cell"
        )
    relevant_lines = 0
    for line in frame_info.code_context:
        ...
        relevant_lines += 1
        if relevant_lines > 1:
            is_multiline_cell = True
            break
    return is_multiline_cell
...
def check_for_upgrade(ipy_profile_dir):
    """Check if the current profile is compatible
    ...
    spocklib_ver = Version(spock_lib_ver_str)
    spock_profile_ver = Version(spock_profile_ver_str)
    # Spock profile hasn't changed since 2.1.0 (7f0cfe7e24)
    min_spock_version = Version("2.1.0")

    if (
        spock_profile_ver >= min_spock_version and spocklib_ver >= min_spock_version
    ) or spocklib_ver == spock_profile_ver:
        # Nothing to do - spock profile is compatible
        return
...
def load_ipython_extension(ipython):
    ...
    if "get_ipython" in user_ns.keys():
        ip = user_ns["get_ipython"]()
        if hasattr(ip, "alias_manager"):
            am = ip.alias_manager
            if hasattr(am, "default_aliases"):
                da = am.default_aliases
                if isinstance(da, list) and ('mv', 'mv') in da:
                    da.remove(('mv', 'mv'))
...
"""
    ...
    if ipy_ver < Version("6.0.0"):
        i_shell.deep_reload = True
    i_shell.confirm_exit = False

    if ipy_ver >= Version("5.0.0"):
        # Change color for ipy_ver >= 5.0.0 due to
    ...
    # autocall > 0 reads Tango attributes (on DeviceProxy object) twice
    #  e.g. mot01.position would be called twice
    term_i_shell.autocall = 0
