...
class SpockBaseDoor(BaseDoor):
    ...
    def _runMacro(self, xml, **kwargs):
        ...
        try:
            ...
            self._handle_stop()
            try:
                reraise_keyboard_interrupt = \
                    genutils.is_executing_multiline_cell()
    ...
    def _updateState(self, old_sw_state, new_sw_state, silent=False):
        ...
        # In parallel spock leave one empty line before printing prompt
        # so the output looks the same is in the active spock.
        if (genutils.is_terminal_interactive_shell()  # not QtSpock 
                and self._running_macros is None  # parallel spock
                # end of macro
                and self._old_door_state in (BaseDoor.On, BaseDoor.Alarm)
                # when macro is stopped by keyboard interrupt
                # self._running_macros is None in active spock as well
                # we need to distinguish it using a special flag
                and not self._stopped_by_keyboard_interrupt):
            print("\n", end="", flush=True)
...
def split_macro_parameters(parameters_s: str, params_def) -> List[Any]:
