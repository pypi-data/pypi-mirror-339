...
#        self.sendRecordData(data, codec='bz2_pickle_plot')
    ...
    def reloadLibrary(self, lib_name: str) -> Any:
        ...
        :raises: ImportError in case the library does not exist

        :param lib_name: library(=module) name

        :return:
            the reloaded python module object"""
        lib = self.door.reload_lib(lib_name)
        if lib is None:
            raise ImportError("Library with name {0} does not exist. "
                        "(Hint: if {0} is not a typo, "
                        "make sure {0} is importable from python on "
                        "server side)".format(lib_name))
        return lib

    @mAPI
    def reloadMacro(self, macro_name: str) -> Any:
    ...
    @mAPI
    def reloadMacros(self, macro_names: Sequence[str]) -> None:
    ...
    @mAPI
    def reloadMacroLibrary(self, lib_name: str) -> MacroLibrary:
        """**Macro API**. Reloads the given library(=module) names
    ...
    @mAPI
    def reloadMacroLibraries(self, lib_names: Sequence[str]) -> Sequence[MacroLibrary]:
        """**Macro API**. Reloads the given library(=module) names
    ...
    def isAnyAncestorStopped(self):
        next_ancestor = self.parent_macro
        while next_ancestor is not None:
            if next_ancestor.isStopped():
                return True
            next_ancestor = next_ancestor.parent_macro
        return False

    def isAnyAncestorExecutingOnStop(self):
        next_ancestor = self.parent_macro
        while next_ancestor is not None:
            if next_ancestor._on_stop_started:
                return True
            next_ancestor = next_ancestor.parent_macro
        return False
