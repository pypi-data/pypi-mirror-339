...
import traceback
import itertools
from typing import Any, List, Tuple, Sequence, Callable, Dict
from packaging.version import Version

import PyTango
from PyTango import Util, Database, WAttribute, DbDevInfo, DevFailed, \
    DevVoid, DevLong64, DevBoolean, DevString, DevDouble, DevEncoded, \
    DevState, SCALAR, SPECTRUM, IMAGE, FMT_UNKNOWN, \
    READ_WRITE, READ, Attr, SpectrumAttr, ImageAttr, \
    DeviceClass, Except, DevLong, DevShort, CmdArgType
...
def _is_maybe_memorized_attribute(attr):
    if attr.get_writable() == READ:
        return False
    if attr.get_data_format() != SCALAR:
        return False
    if attr.get_data_type() == DevEncoded:
        return False
    return True


def _from_memorized_value(attr, value):
    data_type = attr.get_data_type()
    if data_type == DevDouble:
        return float(value)
    elif data_type in (DevShort, DevLong, DevLong64):
        return int(value)
    elif data_type == DevBoolean:
        return value.lower() == "true"
    elif data_type == DevString:
        return value
    else:
        raise ValueError(
            "{} attributes can not be memorized".format(
                CmdArgType.values[data_type].name)
        )
...
def memorize_write_attribute(write_attr_func: Callable) -> Callable:
...
def to_tango_access(access: sardana.DataAccess) -> PyTango.AttrWriteType:
...
def from_tango_access(access: PyTango.AttrWriteType) -> sardana.DataAccess:
...
def to_tango_type_format(dtype_or_info: sardana.DataType, dformat: sardana.DataFormat = None) -> Tuple[PyTango._tango.CmdArgType, PyTango._tango.AttrDataFormat]:
...
def from_tango_type_format(dtype: PyTango.CmdArgType, dformat: PyTango.AttrDataFormat = PyTango.SCALAR) -> Tuple[ sardana.DataType, sardana.DataFormat ]:
...
def to_tango_quality(quality: Any) -> PyTango.AttrQuality:
...
def prepare_cmdline(parser: Any = None, args: Any = None) -> Tuple[Any, List[str], List[str]]:
...
def register_server_with_devices(db: PyTango.Database, server_name: str, server_instance: str, devices: Dict[str, Sequence[Tuple[str, str, Dict]]]) -> None:
...
def get_full_name(dev_name: str) -> str:
    ...
    full_name = "{0}/{1}".format(db_name, dev_name)
    return full_name
...
def get_free_alias(db: PyTango.Database, prefix: str, start_from: int = 1) -> str:
...
def get_server_name() -> str:
    ...
    :return: Tango DS name either Sardana, Pool or MacroServer
    """
    return Util.instance().get_ds_exec_name()

def get_server_version(dev_proxy):
    """Get Tango server version

    :return: Tango server version multiplied by 100 (i.e. 933 for Tango-9.3.3)
    """
    adm = PyTango.DeviceProxy(dev_proxy.adm_name())
    argout = adm.ZmqEventSubscriptionChange(["info"])
    version = argout[0][0]
    return version
...
def prepare_logstash(args: List[str]) -> List[str]:
...
def run(prepare_func, args=None, tango_util=None, start_time=None, mode=None,
    ...
    if tango_version < Version("8.0.5"):
