...
from sardana.tango.macroserver.util import get_macroserver_for_door
...
class TaurusSequencerWidget(TaurusWidget):
    ...
    def onSaveSequence(self):
        ...
        sequencesPath = os.path.join(sequencesPath, "Sequence.xml")
...
def createSequencer(door=None, file=None):
    sequencer = TaurusSequencer()
    sequencer.doorChanged.connect(sequencer.onDoorChanged)
    load_settings = True
    if door is not None:
        macroserver = get_macroserver_for_door(door)
        sequencer.setModel(macroserver)
        sequencer.doorChanged.emit(door)
        ...
        if taurus_config_raw is not None:
            ...
            if macroserver == oldmodel:
                load_settings = False
    if load_settings:
        sequencer.loadSettings()
    if file is not None:
        sequencer.taurusSequencerWidget.loadFile(file)
    return sequencer


def _main():
    taurus.warning(
        "`sequencer` script is deprecated since 3.4.0, "
        "use `sardanactl sequencer` instead."
    )
    ...
    sequencer = createSequencer(door, file_name)
    sequencer.show()
    sys.exit(app.exec_())


@click.command("sequencer")
@click.argument("door", required=False)
@click.option("-f", "--file",type=click.Path(),
              help="load macro sequence from a file (XML or spock syntax)")
def sequencer_cmd(door, file):
    """GUI for executing macro sequences.

    Allows creation of macro sequences with intuitive parameter editors
    and their execution. Sequences can be stored and loaded from XML files.
    """
    from taurus.qt.qtgui.application import TaurusApplication

    app = TaurusApplication(app_name="sequencer",
                            app_version=sardana.__version__,
                            org_name="sardana",
                            org_domain="sardana-controls.org",
    )
    sequencer = createSequencer(door, file)
    sequencer.show()
    sys.exit(app.exec_())
