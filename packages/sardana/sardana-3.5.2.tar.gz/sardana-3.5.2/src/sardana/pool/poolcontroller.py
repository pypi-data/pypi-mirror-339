...
class PoolBaseController(PoolBaseElement):
    ...
    def add_element(self, elem, propagate=1, handle_ctrl_err=True):
        name, axis, eid = elem.get_name(), elem.get_axis(), elem.get_id()
        if self.is_online():
            try:
                self._ctrl.AddDevice(axis)
            except Exception as e:
                msg = "Unable to add %s(%s)" % (name, axis)
                if not handle_ctrl_err:
    ...
    def read_axis_states(self, axes: Optional[Sequence[int]] = None) -> Dict["sardana.pool.poolelement.PoolElement", Any]:
    ...
    def read_axis_values(self, axes: Optional[Sequence[int]] = None) -> Dict["sardana.pool.poolelement.PoolElement", Any]:
    ...
    def calculate_state_info(self, status_info: Optional[Tuple[State, str]] = None) -> Tuple[State, str]:
...
class PoolController(PoolBaseController):
    ...
    def _create_ctrl_args(self):
        ...
        props = {}
        for info in self._ctrl_info.ctrl_properties.values():
            if info.default_value is None:
                continue
            props[info.name] = info.default_value
    ...
    def re_init(self):
        ...
        elem_axis = {**elem_axis, **self._pending_element_axis}
    ...
    def init_attribute_values(self, attr_values: Optional[Dict[str, Any]] = None) -> None:
        ...
        if attr_values is None:
            attr_values = {}
        log_level = attr_values.pop("LogLevel", None)
        if log_level is not None:
            self.log_level = log_level
        attrs = self.ctrl_info.ctrl_attributes
        for attr_info in attrs.values():
            attr_name = attr_info.name
            value = None
            if attr_info.memorized == MemorizedNoInit:
                continue
            if attr_info.memorized == Memorized:
            ...
            if value is None:
                continue
            try:
                self.set_ctrl_attr(attr_name, value)
            except:
    ...
    def set_ctrl_attr(self, name, value):
        ...
        if hasattr(self.ctrl, attr_info.fset):
            ...
            # return self.ctrl.SetCtrlPar(name.lower(), value)
    ...
    def raw_read_axis_states(self, axes: Optional[Sequence[int]] = None, ctrl_states: Any = None) -> Tuple[Dict["sardana.pool.poolelement.PoolElement", Any], bool]:
    ...
    def raw_read_axis_values(self, axes: Optional[Sequence[int]] = None, ctrl_values: Any = None) -> Dict["sardana.pool.poolelement.PoolElement", SardanaValue]:
    ...
    @check_ctrl
    def read_axis_values(self, axes: Optional[Sequence[int]] = None) -> Dict["sardana.pool.poolelement.PoolElement", SardanaValue]:
        """Reads the value for the given axes. If axes is None, reads the
        value of all active axes.
    ...
    def raw_read_axis_value_refs(self, axes: Optional[Sequence[int]] = None, ctrl_values: Any = None) -> Dict["sardana.pool.poolelement.PoolElement", sardana.sardanavalue.SardanaValue]:
    ...
    def stop_axes(self, axes: List[int]) -> List[int]:
    ...
    @check_ctrl
    def stop_element(self, element: "sardana.pool.poolelement.PoolElement") -> None:
    ...
    @check_ctrl
    def stop_elements(self, elements: Optional[Sequence["sardana.pool.poolelement.PoolElement"]] = None) -> List["sardana.pool.poolelement.PoolElement"]:
    ...
    @check_ctrl
    def abort_axes(self, axes: List[int]) -> List[int]:
    ...
    @check_ctrl
    def abort_element(self, element: "sardana.pool.poolelement.PoolElement") -> None:
    ...
    @check_ctrl
    def abort_elements(self, elements: Optional[Sequence["sardana.pool.poolelement.PoolElement"]] = None) -> List["sardana.pool.poolelement.PoolElement"]:
    ...
    @check_ctrl
    def emergency_break(self, elements: Any = None) -> List["sardana.pool.poolelement.PoolElement"]:
    ...
    def get_default_timer(self) -> Optional[int]:
