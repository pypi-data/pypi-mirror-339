...
class Door(SardanaDevice):
    ...
    def macro_server(self):
        return self.macro_server_device.macro_server
    ...
    def get_id(self) -> Union[str, int]:
        """Returns the door id from database 
        (name or numeric id).

        :return: the door id
        """
        if self.macro_server.use_numeric_element_ids:
            return self.Id
        else:
            return self.alias
    ...
    def init_device(self):
        ...
        if not macro_server.use_numeric_element_ids:
            if len(db.get_device_property(self.get_name(), "id")["id"]) > 0:
                ...
                macro_server.use_numeric_element_ids = True

        # support for old doors which didn't have ID
        if macro_server.use_numeric_element_ids and self.Id == InvalidId:
        ...
        if door is None:
            ...
            self.door = door = \
                macro_server.create_element(type="Door", name=name,
                                            full_name=full_name, id=self.get_id())
    ...
    def dev_status(self):
        return self.macro_executor.calculate_status_string()
    ...
    def read_ReservedElements(self, attr):
        try:
            reserved_macro_objects_list = self.door.reserved_elements
            codec = CodecFactory().getCodec('utf8_json')
            data = codec.encode(('', reserved_macro_objects_list))
        except MacroServerException as mse:
            throw_sardana_exception(mse)
...
class DoorClass(SardanaDeviceClass):
    ...
    attr_list = {
        ...
        'MacroStatus': [[DevEncoded, SCALAR, READ],
                        {'label': 'Macro Status', }],
        'ReservedElements': [[DevEncoded, SCALAR, READ],
                        {'label': 'Reserved macro elements', }],
    }
