...
class MacroManager(MacroServerManager):
    ...
    def getOrCreateMacroLib(self, lib_name: str, macro_name: Optional[str] = None) -> Tuple[str, str, int]:
...
#        self._xml_stack = None
        ...
        for macro, objs in list(self._reserved_macro_objs.items()):
            ...
            for obj in objs:
                ...
                try:
                    ...
                    self.output("Stopping %s was interrupted" % obj)
                except:
                    self.warning("Stopping %s failed" % obj)
                    self.debug("Details:", exc_info=1)
                else:
        ...
        for macro, objs in list(self._reserved_macro_objs.items()):
            ...
            for obj in objs:
                ...
                try:
                    ...
                    self.warning("Aborting {} was interrupted. "
                                 "Releasing it.".format(obj))
                    obj.release()
                except:
                    self.warning("Aborting %s failed" % obj)
                    self.debug("Details:", exc_info=1)
                else:
    ...
    def sendMacroStatus(self, data):
        ...
        ret = self.door.set_macro_status(data)
        self.updateDoorStatus()
        return ret
