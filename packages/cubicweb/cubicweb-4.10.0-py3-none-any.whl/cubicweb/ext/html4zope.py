...
class HTMLTranslator(CSS1HTMLTranslator):
    ...
    def visit_title(self, node):
        ...
        if isinstance(node.parent, nodes.topic):
            ...
            # document title
            self.head.append(f"<title>{self.encode(node.astext())}</title>\n")
            self.body.append(
            ...
            self.body.append(
                self.starttag(node, f"h{default_level + self.section_level - 1}", "")
            )
            atts = {}
            if node.hasattr("refid"):
                atts["class"] = "toc-backref"
                atts["href"] = f"{self.base_url}#{node['refid']}"
            self.body.append(self.starttag({}, "a", "", **atts))
            self.context.append(f"</a></h{default_level + self.section_level - 1}>\n")
    ...
    def visit_subtitle(self, node):
        ...
        if isinstance(node.parent, nodes.sidebar):
            ...
            self.body.append(
                self.starttag(node, f"h{default_level + 1}", "", CLASS="subtitle")
            )
            self.context.append(f"</h{default_level + 1}>\n")
    ...
    def visit_reference(self, node):
        ...
        if "refuri" in node:
            ...
            href = f"{self.base_url}#{node['refid']}"
        ...
        if len(node["backrefs"]):
            ...
            if len(backrefs) == 1:
                ...
                for backref in backrefs:
                    ...
                    i += 1
                backref_text = f"; <em>backlinks: {', '.join(backlinks)}</em>"
