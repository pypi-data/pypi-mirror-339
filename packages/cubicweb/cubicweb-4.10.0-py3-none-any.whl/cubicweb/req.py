...
from cubicweb import REVERSE_INDICATOR, Unauthorized, NoSelectableObject, uilib
...
class RequestSessionAndConnectionBase:
    ...
    def etype_rset(self, etype, size=1):
        ...
        rset = ResultSet([("A",)] * size, f"{etype} X", description=[(etype,)] * size)
    ...
    def create_entity(self, etype, **kwargs):
        ...
        >>> c = req.create_entity('Company', name='Logilab')
        >>> req.create_entity('Person', firstname='John', surname='Doe',
        ...                   works_for=c)
    ...
    def _build_rql_request(self, start, etype, **kwargs):
        parts = [f"{start:s} WHERE X is {etype:s}"]
        varmaker = rqlvar_maker(defined="X")
        eschema = self.vreg.schema.entity_schema_for(etype)
        for attr, value in kwargs.items():
            if isinstance(value, (list, tuple)):
                raise NotImplementedError(
                    f"{attr}: collection of values is not supported"
                )
            if hasattr(value, "eid"):
                kwargs[attr] = value.eid
            if attr.startswith(REVERSE_INDICATOR):
                attr = attr[len(REVERSE_INDICATOR) :]
                if attr not in eschema.object_relations:
                    raise KeyError(f"{attr:s} not in {eschema} object relations")
                parts.append(
                    "{var} {attr} X, {var} eid %({reverse_ind:s}{attr})s"
                    "".format(
                        var=next(varmaker),
                        attr=attr,
                        reverse_ind=REVERSE_INDICATOR,
                    )
                )
            else:
                rel = eschema.subject_relations.get(attr)
                if rel is None:
                    raise KeyError(f"{attr} not in {eschema} subject relations")
                if rel.final:
    ...
    def build_url(self, *args, **kwargs):
        ...
        return f"{base_url}{path}?{self.build_url_params(**kwargs)}"
    ...
    def build_url_params(self, **kwargs):
        ...
        for param, values in kwargs.items():
            ...
            for value in values:
                ...
                args.append(f"{param}={self.url_quote(value)}")
        return "&".join(args)
...
RequestSessionBase = class_renamed(
    "RequestSessionBase", RequestSessionAndConnectionBase, version="3.38"
)
