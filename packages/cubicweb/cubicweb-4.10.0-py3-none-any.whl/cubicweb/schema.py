...
from yams import BadSchemaDefinition, ValidationError, buildobjs as ybo
...
def guess_rrqlexpr_mainvars(expression):
    ...
    if not mainvars:
        raise BadSchemaDefinition(
            f"unable to guess selection variables in {expression!r}"
        )
    return mainvars
...
def normalize_expression(rqlstring):
    ...
    """
    union = parse(f"Any 1 WHERE {rqlstring}").as_string()
...
class RQLExpression:
    ...
    def __init__(self, expression, mainvars, eid):
        ...
        assert mainvars, f"bad mainvars {mainvars}"
    ...
    def _check(self, _cw, **kwargs):
        ...
        if has_perm_defs is None:
            ...
            get_eschema = _cw.vreg.schema.entity_schema_for
            try:
    ...
    def minimal_rql(self):
        return f"Any {','.join(sorted(self.mainvars))} WHERE {self.expression}"
...
def order_eschemas(eschemas):
    ...
    if cycles:
        ...
        raise Exception(f"cycles in entity schema specialization: {cycles}")
    eschemas = []
    while graph:
...
def check_perm(self, _cw, action, **kwargs):
    ...
    if _cw.user.matching_groups(groups):
        if DBG:
            print(f"check_perm: {action!r} {_self_str!r}: user matches {groups}")
        return
    ...
    if "owners" in groups and (
        ...
        if DBG:
            print(
                f"check_perm: {action!r} {_self_str!r}: user is owner or creation time"
            )
        return
...
class CubicWebEntitySchema(EntitySchema):
    ...
    def targets(self, role):
        ...
        if role == "subject":
            return self.subject_relations.values()
        return self.object_relations.values()
    ...
    def is_subobject(self, strict=False, skiprels=None):
        if skiprels is None:
            ...
            skiprels += SKIP_COMPOSITE_RELS
        return super(CubicWebEntitySchema, self).is_subobject(
            strict, skip_relations=skiprels
        )
    ...
    def add_subject_relation(self, rschema):
        ...
        if rschema.final:
            if self.relation_definition(rschema).get("fulltextindexed"):
    ...
    def del_subject_relation(self, rtype):
        ...
        if "has_text" in self.subject_relations:
    ...
    def _update_has_text(self, deletion=False):
        ...
        need_has_text = None
        for rschema in list(self.subject_relations.values()):
            if rschema.final:
                if rschema == "has_text":
                    has_has_text = True
                elif self.relation_definition(rschema).get("fulltextindexed"):
                ...
                if rschema.fulltext_container == "subject":
                    ...
                    need_has_text = False
        for rschema in list(self.object_relations.values()):
            if rschema.fulltext_container:
...
class CubicWebRelationSchema(PermissionMixIn, RelationSchema):
    ...
    def may_have_permission(self, action, req, eschema=None, role=None):
        if eschema is not None:
            for tschema in self.targets(eschema, role):
                rdef = self.role_relation_definition(eschema, tschema, role)
                if rdef.may_have_permission(action, req):
                    return True
        else:
            for rdef in self.relation_definitions.values():
                if rdef.may_have_permission(action, req):
                    return True
        return False
    ...
    def has_perm(self, _cw, action, **kwargs):
        ...
        if objtype and subjtype:
            return self.relation_definition(subjtype, objtype).has_perm(
                _cw, action, **kwargs
            )
        elif subjtype:
            for tschema in self.targets(subjtype, "subject"):
                rdef = self.relation_definition(subjtype, tschema)
                if not rdef.has_perm(_cw, action, **kwargs):
                    return False
        elif objtype:
            for tschema in self.targets(objtype, "object"):
                rdef = self.relation_definition(tschema, objtype)
                if not rdef.has_perm(_cw, action, **kwargs):
                    return False
        else:
            for rdef in self.relation_definitions.values():
...
class CubicWebSchema(Schema):
    ...
    def add_entity_type(self, edef: yams.types.EntityType) -> CubicWebEntitySchema:
        ...
        eschema = super().add_entity_type(edef)
    ...
    def del_relation_type(self, rtype):
        rschema = self.relation_schema_for(rtype)
    ...
    def del_relation_def(self, subjtype, rtype, objtype):
        for k, v in self._eid_index.items():
            ...
            if (
                v.subject == subjtype
                and v.relation_type == rtype
                and v.object == objtype
            ):
    ...
    def del_entity_type(self, etype):
        eschema = self.entity_schema_for(etype)
        ...
        if "has_text" in list(eschema.subject_relations.values()):
    ...
    def iter_computed_attributes(self):
        for relation in self.relations():
            for rdef in relation.relation_definitions.values():
...
class RepoEnforcedRQLConstraintMixIn:
    ...
    def repo_check(self, session, eidfrom, rtype, eidto=None):
        ...
        if not self.match_condition(session, eidfrom, eidto):
            ...
            if self.msg:
                ...
                msg = f"{session._(self.type())} {self.expression} failed"
    ...
    def exec_query(self, _cw, eidfrom, eidto):
        ...
        if "U" in self.snippet_rqlst.defined_vars:
            ...
            args["u"] = _cw.user.eid
        rql = f"Any {','.join(sorted(self.mainvars))} WHERE {expression}"
        if self.distinct_query:
            rql = "DISTINCT " + rql
