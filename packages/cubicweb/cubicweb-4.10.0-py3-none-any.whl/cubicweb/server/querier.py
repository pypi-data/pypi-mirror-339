...
class InsertPlan(ExecutionPlan):
    ...
    def add_entity_def(self, edef) -> None:
        """add an entity definition to build"""
        self.e_defs[-1].append(edef)

    def add_relation_def(self, rdef) -> None:
...
class QuerierHelper:
    ...
    def clear_caches(self, eids=None, etypes=None) -> None:
        if eids is None:
    ...
    def _apply_function_on_results(self, cnx, results, selection, descr, rql):
        if len(results) == 0:
            return results
        functions_to_apply = [
            (i, varref)
            for (i, varref) in enumerate(selection)
        ...
        for row_index, result in enumerate(new_results):
            for i, varref in functions_to_apply:
                eid = result[i]
                if eid is None:
                    continue
                entity = cnx.entity_from_eid(eid, descr[row_index][i]).cw_adapt_to(
                    "IRQLInterface"
                )
                if isinstance(varref, VariableRefMethodCall):
                    try:
                        ...
                        raise BadRQLQuery(
                            ...
                            f"EntityType {descr[row_index][i]} \n"
                            f"    RQL: {rql}"
                        )
                else:  # then isinstance(varref, VariableRefAttributeAccess):
                    try:
    ...
    def execute(self, cnx, rql: str, args: Optional[dict] = None, build_descr=True):
        ...
        * `build_descr` is a boolean flag indicating if the description should
        ...
        if rqlst.TYPE != "select":
            ...
            if args:
                ...
                # when some argument is None or not (IS NULL).
                # This should be considered when computing sql cache key
        ...
        if build_descr:
            if rqlst.TYPE == "select":
                ...
                if len(rqlst.children) == 1:
                    # We have a single query, it means that we can associate a
                    # unique name to each column of the result set
                    variables = [
                ...
                results = self._apply_function_on_results(
                    cnx, results, selected, descr, rql
                )
                ...
                # one solution
                variables = [var.name for var in rqlst.selection]
    ...
    @staticmethod
    def _get_projected_name(projected, stinfo) -> str:
        if isinstance(projected, VariableRefMethodCall):
            strargs = ",".join(
                [
                    QuerierHelper._get_projected_name(arg, stinfo)
                    for arg in projected.args_for_method_to_call
                ]
            )
            return f"{projected.name}.{projected.method_to_call}({strargs})"
        if isinstance(projected, VariableRefAttributeAccess):
            return f"{projected.variable}.{projected.attribute_to_access}"
        if isinstance(projected, VariableRef):
            return projected.name
        if isinstance(projected, Constant):
...
class RQLCache:
    def __init__(self, repo, schema):
        ...
        def parse(rql: str, annotate=False, parse=rqlhelper.parse):
...
def _rql_cache_key(cnx, rql: str, args, eidkeys):
    cachekey = [rql]
