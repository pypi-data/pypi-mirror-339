...
class QuerierTC(BaseQuerierTC):
    ...
    def tearDown(self):
        super().tearDown()
        with self.admin_access.cnx() as cnx:
            cnx.execute(f"DELETE Any X WHERE X eid > {self.maxeid}")
            cnx.commit()
    ...
    def test_select_complex_groupby(self):
        self.qexecute("Any N GROUPBY N WHERE X name N")
        self.qexecute(
    ...
    def test_select_inlined_groupby(self):
        ...
        self.qexecute(
            f"Any U,L,S GROUPBY U,L,S WHERE X in_state S, U login L, S eid {seid}"
        )
    ...
    def test_select_get_attribute_and_function(self):
        affaire = self.qexecute("INSERT Affaire X: X ref '42', X sujet 'UN SUJET'")[0][
            0
        ]
        rset = self.qexecute("Any A, A.sujet, A.ref_sujet_interface() where A ref '42'")
        self.assertEqual(rset[0], [affaire, "UN SUJET", "42 -- UN SUJET"])

    def test_select_get_attribute_from_eid(self):
        affaire = self.qexecute("INSERT Affaire X: X ref '42', X sujet 'UN SUJET'")[0][
            0
        ]
        rset = self.qexecute(f"Any A.sujet where A eid {affaire}")
        self.assertEqual(rset[0], ["UN SUJET"])

    def test_select_get_attribute_error(self):
        affaire = self.qexecute("INSERT Affaire X: X ref '42', X sujet 'UN SUJET'")[0][
            0
        ]
        with pytest.raises(BadRQLQuery) as exc:
            self.qexecute(f"Any A.sujet_not_exists where A eid {affaire}")
        self.assertEqual(
            exc.value.args[0],
            "Attribute sujet_not_exists does not exists for EntityType Affaire \n"
            f"    RQL: Any A.sujet_not_exists where A eid {affaire}",
        )

    def test_select_apply_function_error(self):
        affaire = self.qexecute("INSERT Affaire X: X ref '42', X sujet 'UN SUJET'")[0][
            0
        ]
        with pytest.raises(BadRQLQuery) as exc:
            self.qexecute(f"Any A.ref_sujet() where A eid {affaire}")
        # ref_sujet does not exists since the method on the adapter is ref_sujet_interface
        # and rql can not access to entites method/attribute directly
        self.assertEqual(
            exc.value.args[0],
            "Function ref_sujet does not exists for EntityType Affaire \n"
            f"    RQL: Any A.ref_sujet() where A eid {affaire}",
        )
    ...
    def test_select_date_extraction(self):
        ...
        for funcname, result in test_data:
            rset = self.qexecute(
                f"Any {funcname}(D) WHERE X is Personne, X datenaiss D"
            )
    ...
    def test_regexp_based_pattern_matching(self):
        peid1 = self.qexecute("INSERT Personne X: X nom 'bidule'")[0][0]
        self.qexecute("INSERT Personne X: X nom 'cidule'")
    ...
    def test_select_upper(self):
        ...
        eid = rset.rows[0][0]
        rset = self.qexecute(f"Any UPPER(L) WHERE X eid {eid}, X login L")
    ...
    def test_boolean_is_a_boolean(self):
        """Test that sqlite return True or False when asking a boolean, and not 0 or 1."""
        with self.admin_access.cnx() as cnx:
            cnx.create_entity("Personne", nom="louis", test=True)
            rset = cnx.execute("Any T WHERE P test T")
            self.assertIs(rset[0][0], True)
    ...
    def test_exists(self):
        self.qexecute("INSERT CWGroup X: X name 'lulufanclub'")
...
Any P1,B,E WHERE P1 identity P2 WITH
  ...
  P2,E BEING (Any P,COUNT(T) GROUPBY P WHERE P is Workflow, T is Transition,
    ...
    def test_insert_is(self):
        (eid,) = self.qexecute("INSERT Personne X: X nom 'bidule'")[0]
        (etype,) = self.qexecute(f"Any TN WHERE X is T, X eid {eid}, T name TN")[0]
    ...
    def test_insert_query_error(self):
        ...
        self.assertRaises(
            QueryError,
            self.qexecute,
            f"INSERT CWUser X: X login 'toto', X eid {cnx.user.eid}",
        )
    ...
    def test_delete_2(self):
        ...
        self.assertEqual(
            self.qexecute(f"Any N WHERE X nom N, X eid {rset[0][0]}")[0][0], "syt"
        )
    ...
    def test_nonregr_delete_cache(self):
        ...
        with self.admin_access.cnx() as cnx:
            ...
            sqlc.execute(f"SELECT * FROM owned_by_relation WHERE eid_from={eeid}")
            self.assertEqual(len(sqlc.fetchall()), 0)
    ...
    def test_nonregr_delete_cache2(self):
        ...
        self.qexecute(f"Any X WHERE X eid {eid}")
        self.qexecute("Folder X WHERE X eid %(x)s", {"x": eid})
        self.qexecute(f"Folder X WHERE X eid {eid}")
        self.qexecute(f"DELETE Folder T WHERE T eid {eid}")
        rset = self.qexecute("Any X WHERE X eid %(x)s", {"x": eid})
        self.assertEqual(rset.rows, [])
        rset = self.qexecute(f"Any X WHERE X eid {eid}")
        self.assertEqual(rset.rows, [])
        rset = self.qexecute("Folder X WHERE X eid %(x)s", {"x": eid})
        self.assertEqual(rset.rows, [])
        rset = self.qexecute(f"Folder X WHERE X eid {eid}")
        self.assertEqual(rset.rows, [])
    ...
    def test_update_query_error(self):
        ...
        self.assertRaises(
            QueryError, self.qexecute, f"SET X login 'tutu', X eid {cnx.user.eid}"
        )
    ...
    def test_update_having(self):
        ...
        self.qexecute("INSERT Personne Y: Y nom 'hop', Y tel 2")
    ...
    def test_sqlite_encoding(self):
        ...
        self.qexecute(
            ...
            {
                "name": "éname0",
                "modification_date": datetime(2003, 3, 12, 11, tzinfo=timezone.utc),
                "creation_date": datetime(2003, 7, 3, 11, tzinfo=timezone.utc),
            },
        )
        rset = self.qexecute(
    ...
    def test_nonregr_description(self):
        ...
        rset = self.qexecute(f"Any X WHERE X in_basket B, B eid {beid}")
        self.assertEqual(len(rset), 2)
    ...
    def test_nonregr_extra_joins(self):
        ...
        self.qexecute(f"SET X todo_by Y WHERE X is Note, Y eid {ueid}")
        rset = self.qexecute(
    ...
    def test_nonregr_set_datetime(self):
        ...
        self.qexecute(
            "SET X creation_date %(date)s WHERE X eid 1",
    ...
    def test_nonregr_is(self):
        ...
        self.qexecute(f"Any X, ET WHERE X is ET, ET eid {uteid}")
    ...
    def test_nonregr_has_text_ambiguity_1(self):
        ...
        self.qexecute("INSERT Affaire X: X ref 'bidule'")
    ...
    def test_variables_in_rset_for_select(self):
        ...
        rset = self.qexecute("Any X, Y WHERE X cwuri Y")
    ...
    def test_variables_in_rset_for_insert(self):
        rset = self.qexecute("Insert Personne P: P nom 'John Doe'")
        self.assertEqual(rset.variables, ["P"])
...
class NonRegressionTC(CubicWebTC):
    ...
    def test_set_relations_eid(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(f'SET U use_email A WHERE U login "admin", A eid >= {a2.eid}')
            self.assertEqual(
                [[a2.eid], [a3.eid]],
                cnx.execute(
            ...
            # DELETE
            cnx.execute(f'DELETE U use_email A WHERE U login "admin", A eid > {a2.eid}')
            self.assertEqual(
                [[a2.eid]],
                cnx.execute(
                    'Any A ORDERBY A WHERE U use_email A, U login "admin"'
