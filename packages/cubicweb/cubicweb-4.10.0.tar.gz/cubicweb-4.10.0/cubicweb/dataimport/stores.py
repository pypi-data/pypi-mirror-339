...
from cubicweb import REVERSE_INDICATOR
...
class NullStore:
    ...
    Stores can also be used as context manager. If no exception is raised during
    the import, a final flush and the finish method are called. On the contrary,
    is something went wrong, we roll everything back.
    """
...
class RQLObjectStore(NullStore):
    ...
    def prepare_update_entity(self, etype, eid, **kwargs):
        ...
        assert entity.cw_etype == etype, f"Trying to update with wrong type {etype}"
...
class NoHookRQLObjectStore(RQLObjectStore):
    ...
    def __init__(self, cnx, metagen=None):
        ...
        self._rschema = cnx.repo.schema.relation_schema_for
    ...
    def prepare_insert_relation(self, eid_from, rtype, eid_to, **kwargs):
        ...
        and ``eid_to``.
        """
        assert not rtype.startswith(REVERSE_INDICATOR)
...
class MetadataGenerator:
    ...
    * `source`: optional source to be used as `cw_source` for imported entities
    ...
    def __init__(self, cnx, baseurl=None, source=None, meta_skipped=()):
        ...
        relation_schema_for = cnx.vreg.schema.relation_schema_for
        ...
        for rtype in self.meta_relations:
            ...
            if cnx.user.eid == -1 and rtype in ("owned_by", "created_by"):
                continue
            if relation_schema_for(rtype).final:
...
class _MetaGeneratorBWCompatWrapper:
...
class MetaGenerator(object, metaclass=class_deprecated):
    ...
    * `source`: optional source to be used as `cw_source` for imported entities
    ...
    def gen_cwuri(self, entity):
        ...
        return f"{self.baseurl}{entity.eid}"
