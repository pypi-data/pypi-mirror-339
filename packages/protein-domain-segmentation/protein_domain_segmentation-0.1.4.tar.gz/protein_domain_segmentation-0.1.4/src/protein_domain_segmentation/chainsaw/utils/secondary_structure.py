...
def calculate_ss(pdbfile, chain, stride_path, ssfile="pdb_ss"):
    ...
    with open(ssfile, "w") as ssout_file:
        ...
        try:
            subprocess.run(
                args, stdout=ssout_file, stderr=subprocess.DEVNULL, check=True
            )
        except subprocess.CalledProcessError as e:
            LOG.warning(f"Stride failed on {pdbfile}, creating empty file, error : {e}")
            pass
...
def renum_pdb_file(pdb_path, output_pdb_path):
    ...
    with open(output_pdb_path, "w") as output_file:
        ...
        subprocess.run(
            [sys.executable, str(pdb_reres_path), pdb_path],
            stdout=output_file,
            check=True,
            text=True,
        )
...
def main(
    chain_ids, pdb_dir, feature_dir, stride_path, reres_path, savedir, job_index=0
):
    ...
    for chain_id in chain_ids:
        try:
            ...
            if os.path.exists(pdb_path):
                features = np.load(os.path.join(feature_dir, chain_id + ".npz"))[
                    "arr_0"
                ]
                ...
                output_pdb_path = os.path.join(
                    savedir, f"{job_index}.pdb"
                )  # this gets overwritten to save memory
                ...
                ss_filepath = os.path.join(
                    savedir, f"pdb_ss{job_index}.txt"
                )  # this gets overwritten to save memory
