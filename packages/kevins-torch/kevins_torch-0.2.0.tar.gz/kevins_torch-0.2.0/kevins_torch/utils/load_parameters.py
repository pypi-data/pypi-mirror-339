...
if __name__ == "__main__":
    ...
    if not os.path.exists(config_dir):
        logger.error(f"配置目錄 {config_dir} 不存在")
        sys.exit(1)

    yaml_files = [f for f in os.listdir(config_dir) if f.endswith(".yaml")]
    if not yaml_files:
        logger.error(f"在 {config_dir} 中沒有找到 YAML 文件")
        sys.exit(1)
    ...
    for yaml_file in yaml_files:
        ...
        try:
            ...
            if not isinstance(config, dict):
                logger.warning(f"警告: {yaml_file} 不是有效的配置文件")
                continue

            logger.debug("訓練參數:", config['training'])
            logger.debug("轉換配置:", config['transforms'])
            logger.debug("優化器配置:", config['optimizer'])
            if 'scheduler' in config:
                logger.debug("調度器配置:", config['scheduler'])
        except Exception as e:
            logger.error(f"處理 {yaml_file} 時發生錯誤: {str(e)}")
