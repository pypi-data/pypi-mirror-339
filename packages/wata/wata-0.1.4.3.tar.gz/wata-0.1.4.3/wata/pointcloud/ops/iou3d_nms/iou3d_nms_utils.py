...
def nms_cpu(boxes, scores, thresh, pre_maxsize=None, **kwargs):
    ...
    survived_inds = order
    keep = []
    while survived_inds.numel()  > 0:
        i = survived_inds[0]
        keep.append(i)
        if survived_inds.numel()  == 1:
            break
        rem_inds = survived_inds[1:]
        iou = boxes_bev_iou_cpu(boxes[i].unsqueeze(0), boxes[rem_inds])
        print(i,rem_inds)
        print(iou)
        mask = iou.squeeze()  <= thresh
        survived_inds = rem_inds[mask]  # 更新全局索引
