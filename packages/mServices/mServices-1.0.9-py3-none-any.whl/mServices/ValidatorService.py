...
class ValidatorService:
    ...
    def validate(data, rules, custom_messages=None):
        ...
        for field, rule in rules.items():
            ...
            if field not in data and 'required' not in rules_list:
            ...
            for r in rules_list:
                if r == 'required':
                    ...
                    # Extract the other field from 'required_without:other_field'
                    other_field = r.split(':')[1]
                    # Check if this field is required because the other field is missing
                    if not value and not data.get(other_field):
                        # If a custom message exists, use it. Otherwise, create a default one
                        custom_message = custom_messages.get(f'{field}.required_without', f'{field} is required if {other_field} is missing.')
                        ValidatorService._add_error(
                            errors, field, 'required_without', {'_attribute': field, 'other_field': other_field, 'message': custom_message}
                        )
                        ...
                        if ref_field in data:
                            ...
                            if value and ref_value:
                                ...
                                if input_date < ref_date:
                                    ValidatorService._add_error(
                                        errors, field, 'after_or_equal', {'_attribute': field, '_date': ref_field}
                                    )
