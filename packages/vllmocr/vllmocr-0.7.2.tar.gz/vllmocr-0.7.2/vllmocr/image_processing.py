...
def preprocess_image(
    ...
    try:
        ...
        denoised = cv2.fastNlMeansDenoising(
            contrast_enhanced, h=5, templateWindowSize=7, searchWindowSize=21
        )
        ...
        (
            os.path.join(
                "/Users/nealcaren/Dropbox/",
                f"{os.path.basename(image_path)}_denoised.png",
            ),
        )  # TODO: This looks like a leftover debug path, should it be removed?
        ...
        while current_size > MAX_FILE_SIZE_BYTES:
            ...
            scale_factor = math.sqrt(
                MAX_FILE_SIZE_BYTES / current_size
            )  # Estimate needed reduction
            scale_factor = min(
                scale_factor, 0.95
            )  # Ensure at least 5% reduction to avoid stalling
            ...
            if new_width == width and new_height == height:
                logging.warning(
                    f"Cannot resize {output_path} further to meet size limit. Stopping resize attempt."
                )
                break  # Avoid infinite loop if dimensions are already minimal or scale factor is ~1

            if new_width < MIN_DIMENSION or new_height < MIN_DIMENSION:
                logging.warning(
                    f"Resizing {output_path} stopped: reached minimum dimension {MIN_DIMENSION}px."
                )
                break
            ...
            logging.info(
                f"Resized {output_path} to {new_width}x{new_height}, new size: {current_size / (1024 * 1024):.2f} MB"
            )
