...
class ManualService:
    ...
    async def publish_manual(
        ...
        try:
            ...
            # If export_path points to a specific file, use its directory as source_path
            # or update source_path to point to the specific file
            if os.path.isfile(export_path):
                if export_path.startswith(source_path):
                    # If the file is already in the correct solution directory, we can just use that specific file
                    source_path = export_path
                else:
                    # Otherwise use the directory containing the file
                    logger.info(
                        f"Leaf node export: Using specific file at {export_path}"
                    )
                    # We need to make sure the file is properly placed in the solution directory
                    # so use the file name but in the solution directory
                    file_name = os.path.basename(export_path)
                    source_path = os.path.join(source_path, file_name)
                    # In case the file doesn't exist in the target location, make sure it gets there
                    if not os.path.exists(source_path) and os.path.exists(export_path):
                        os.makedirs(os.path.dirname(source_path), exist_ok=True)
                        logger.info(f"Copying {export_path} to {source_path}")
                        shutil.copy2(export_path, source_path)
            ...
            logger.info(
                f"Resetting repository path from {self.repo_path} to {original_repo_path}"
            )
            self.repo_path = original_repo_path

            # Optionally clean up the job-specific repository to save space
            # This can be uncommented if you want to automatically delete job repositories after completion
            # if hasattr(self, "_current_job_context") and self._current_job_context.get("job_id"):
            #     job_id = self._current_job_context["job_id"]
            #     job_repo_path = os.path.join(original_repo_path, job_id)
            #     if os.path.exists(job_repo_path):
            #         logger.info(f"Cleaning up job repository at {job_repo_path}")
            #         try:
            #             shutil.rmtree(job_repo_path)
            #         except Exception as e:
            #             logger.warning(f"Failed to clean up job repository: {str(e)}")
    ...
    async def _cleanup_old_files(self, selected_solution: SolutionType) -> None:
        ...
        Only removes the specific file or directory that matches the current operation,
        preserving other files in the repository.
        ...
        try:
            if not hasattr(
                ...
                logger.warning("No job context found, skipping selective cleanup")
                return

            job_id = self._current_job_context["job_id"]
            if job_id not in self.active_jobs:
                logger.warning(
                    f"Job {job_id} not found in active jobs, skipping selective cleanup"
                )
                return
            ...
            if not notion_page_id:
                logger.warning(
                    "No Notion page ID found in job, skipping selective cleanup"
                )
                return

            # Log what we're trying to clean up
            logger.info(
                f"Starting selective cleanup for Notion page ID: {notion_page_id}"
            )

            # Get additional information from the job
            file_title = job.title
            is_directory = job.is_directory
            parent_id = job.parent_id

            if file_title:
                logger.info(f"Using title for cleanup: {file_title}")
            if is_directory is not None:
                logger.info(f"Page is a directory: {is_directory}")
            if parent_id:
                logger.info(f"Parent ID: {parent_id}")

            # Create multiple possible name variations to match files in the repo
            possible_names = []

            # 1. If we have a title, this is most reliable for matching
            if file_title:
                # Kebab case (common format for filenames)
                kebab_title = file_title.lower().replace(" ", "-")
                possible_names.append(kebab_title)

            logger.info(f"Potential file name patterns: {possible_names}")

            # Define a function to perform selective cleanup operations
            def cleanup_item(directory, possible_names):
                if not os.path.exists(directory):
                    logger.info(f"Directory does not exist, skipping: {directory}")
                    return 0

                # List all files in the directory to help with debugging and matching
                try:
                    dir_contents = os.listdir(directory)
                    logger.info(f"Found {len(dir_contents)} items in {directory}")

                    # Log some directory contents (limited to 10 items to avoid huge logs)
                    log_items = dir_contents[:10]
                    if len(dir_contents) > 10:
                        log_items.append("... and more")
                    logger.info(f"Sample items in {directory}: {log_items}")

                    # Add potential matches from directory scanning
                    actual_matches = []

                    # For each name we're looking for, find potential matches in the directory
                    for name in possible_names:
                        # Look for exact matches first (full filename or with extensions)
                        exact_matches = [
                            item
                            for item in dir_contents
                            if item == name  # Exact match
                            or item.startswith(f"{name}.")  # With extension
                            or item == f"{name}"
                        ]  # With trailing slash (for directories)
                        actual_matches.extend(exact_matches)

                        # If we didn't find exact matches, try with common extensions
                        if not exact_matches:
                            for ext in [".md", ".mdx", ".json"]:
                                ext_match = f"{name}{ext}"
                                if ext_match in dir_contents:
                                    actual_matches.append(ext_match)

                        # If still no matches and name is longer than 3 chars, try partial matches
                        # This helps with cases where filenames might have slight variations
                        if not actual_matches and len(name) > 3:
                            contained_matches = [
                                item
                                for item in dir_contents
                                if name in item  # Name is part of item
                                or (len(item) > 3 and item in name)
                            ]  # Item is part of name (if both substantial)
                            actual_matches.extend(contained_matches)

                    # Remove duplicates while preserving order
                    seen = set()
                    actual_matches = [
                        x for x in actual_matches if not (x in seen or seen.add(x))
                    ]

                    if actual_matches:
                        logger.info(
                            f"Found matching items in {directory}: {actual_matches}"
                        )
                    else:
                        logger.info(
                            f"No matching items found in {directory} for {possible_names}"
                        )
                        return 0
                except Exception as e:
                ...
                # Remove the matched files/directories
                files_removed = 0
                for item_name in actual_matches:
                    ...
                    try:
                        if os.path.isfile(item_path):
                            ...
                            # If we know this is a directory page, or item is a directory with matching name
                            if is_directory or (
                                file_title
                                and item_name.lower()
                                == file_title.lower().replace(" ", "-")
                            ):
                                shutil.rmtree(item_path)
                                logger.info(f"Removed directory: {item_path}")
                                files_removed += 1
                            else:
                                # For directories, only remove if we're confident it's the right one
                                # Check if directory contains index.mdx or has exactly the name we expect
                                if (
                                    os.path.exists(os.path.join(item_path, "index.mdx"))
                                    or item_name in possible_names
                                ):
                                    shutil.rmtree(item_path)
                                    logger.info(f"Removed directory: {item_path}")
                                    files_removed += 1
                                    ...
                                    logger.info(
                                        f"Skipping directory {item_path} - not confident it matches our target"
                                    )
                    except Exception as e:
            ...
            files_removed = await asyncio.to_thread(
                cleanup_item, source_dir, possible_names
            )
            ...
            files_removed = await asyncio.to_thread(
                cleanup_item, static_img_dir, possible_names
            )
            total_files_removed += files_removed
            ...
            for lang in self.target_languages:
                ...
                files_removed = await asyncio.to_thread(
                    cleanup_item, target_dir, possible_names
                )
                total_files_removed += files_removed

            logger.info(
                f"Completed selective cleanup for Notion page: {notion_page_id}, removed {total_files_removed} items"
            )
        except Exception as e:
            logger.error(f"Error during selective cleanup: {str(e)}")
            raise
    ...
    async def export_repository(
        ...
        try:
            ...
            if os.path.isfile(actual_path):
                ...
                # Ensure source directory exists for this solution
                source_dir = os.path.join(
                    self.repo_path, self.source_dir, selected_solution.value.lower()
                )
                os.makedirs(source_dir, exist_ok=True)

                # For single file exports, make sure it's in the correct directory structure
                ...
                if actual_path != target_path:
                    ...
                    logger.info(f"Copying {actual_path} to {target_path}")
