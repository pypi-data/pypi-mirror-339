...
@router.post("/webhook/notion/{solution}")
async def notion_webhook_solution(
    solution: str,
    background_tasks: BackgroundTasks,
    manual_service: ManualService = Depends(get_manual_service),
    notion_page_id: Optional[str] = Query(
        None, description="Notion page ID that was created or updated"
    ),
):
    """Webhook endpoint for Notion to trigger when a specific page in a solution is updated.

    This endpoint doesn't require authentication and allows Notion to send updates
    about specific pages that have been created or updated. When a webhook event
    is received, the service updates both in-memory and filesystem caches to ensure
    they stay in sync and reflect the latest changes.

    Args:
        solution: The solution type (zcp, apim, amdp)
        notion_page_id: The ID of the Notion page that was created or updated
    """
    try:
        # Check if notion_page_id is provided
        if not notion_page_id:
            raise HTTPException(
                status_code=400, detail="notion_page_id query parameter is required"
            )

        logger.info(
            f"Received webhook call from Notion for solution: {solution}, page: {notion_page_id}"
        )

        # Convert string to SolutionType
        try:
            solution_type = SolutionType(solution.lower())
        except ValueError:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid solution type: {solution}. Must be one of: {', '.join([s.value for s in SolutionType])}",
            )

        # Check if this solution has a root page ID configured
        if solution_type not in manual_service.root_page_ids:
            raise HTTPException(
                status_code=404,
                detail=f"No root page ID configured for solution: {solution}",
            )

        # Verify the notion_page_id format
        try:
            # Use existing format page ID method if available
        ...
        # Update the specified solution's cache in the background
        ...
        return {
            "status": "accepted",
            "message": f"Cache update scheduled for {solution} due to changes in page {notion_page_id}",
            "timestamp": datetime.now().isoformat(),
        }
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error processing Notion webhook for {solution}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
