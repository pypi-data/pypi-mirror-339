...
class TestAnnotationFile:
    def test_init_cbin_notmat_pairs(self, a_cbin_notmat_pair):
        ...
        audio_file = biosound.AudioFile(path=audio_path)
        annot_file = biosound.AnnotationFile(path=annot_path, annotates=audio_file)
        assert isinstance(annot_file, biosound.AnnotationFile)
        for attr_name, attr_type, attr_val in zip(
            ("path", "annotates"), (pathlib.Path, biosound.AudioFile), (annot_path, audio_file)
        ):
    ...
    def test_init_list_of_wav(self, annot_file_koumura, birdsongrec_wav_list):
        audio_files = [biosound.AudioFile(path=wav_path) for wav_path in birdsongrec_wav_list]
        annot_file = biosound.AnnotationFile(path=annot_file_koumura, annotates=audio_files)
        assert isinstance(annot_file, biosound.AnnotationFile)
        for attr_name, attr_type, attr_val in zip(
    ...
    def test_init_list_of_spect(self, annot_file_yarden, spect_list_mat):
        ...
        spect_files = [biosound.SpectrogramFile(path=mat_spect_path) for mat_spect_path in spect_list_mat]
        annot_file = biosound.AnnotationFile(path=annot_file_yarden, annotates=spect_files)
        assert isinstance(annot_file, biosound.AnnotationFile)
        for attr_name, attr_type, attr_val in zip(
    ...
    def test_raises(self, annot_file_koumura, birdsongrec_wav_list, spect_list_mat):
        ...
        with pytest.raises(TypeError):
            biosound.AnnotationFile(path=annot_file_koumura, annotates=mixed_list)
