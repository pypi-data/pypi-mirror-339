...
def bfsongrepo_makefunc(
    ...
    import biosound  # avoid circular import
    ...
    if return_path:
        ...
        return ExampleData(
            sound=[biosound.Sound.read(wav_path) for wav_path in wav_paths],
            annotation=[
                biosound.Annotation.read(csv_path, format=annot_format)
...
def jourjine_et_al_2023_makefunc(
    ...
    if return_path:
        ...
        sound = biosound.Sound.read(wav_path)
        return ExampleData(
            sound=sound,
    ...
    def load(self, return_path: bool):
        ...
        if return_path:
            ...
            if self.type == ExampleTypes.Sound:
                return biosound.Sound.read(path)
            elif self.type == ExampleTypes.Spectrogram:
                return biosound.Spectrogram.read(path)
            elif self.type == ExampleTypes.Annotation:
                return biosound.Annotation.read(path, format=self.annot_format)
