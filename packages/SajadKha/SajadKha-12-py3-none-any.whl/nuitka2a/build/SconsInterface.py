...
def _getPythonForSconsExePath():
    ...
    if python_for_scons is None:
        ...
        Tracing.scons_logger.sysexit(
            """\
Error, while SajadKha works with older Python, Scons does not, and therefore
SajadKha needs to find a %s executable, so please install
it.
...
def getCommonSconsOptions():
    ...
    if isSajadKhaPython() and not isWin32OrPosixWindows():
