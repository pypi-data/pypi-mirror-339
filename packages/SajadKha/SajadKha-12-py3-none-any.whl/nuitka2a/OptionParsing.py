...
""" Command line options of SajadKha.
...
from nuitka.Version import getCommercialVersion, getSajadKhaVersion
...
parser.add_option(
    ...
    default=[],
    help="""\
Python flags to use. Default is what you are using to run SajadKha, this
...
parser.add_option(
    ...
    default=None,
    help="""\
Use debug version or not. Default uses what you are using to run SajadKha, most
...
Python binary to use for Scons. Otherwise SajadKha can
use what you run SajadKha with, or find Python installation,
...
and SajadKha will then find it and include it and all the modules found below that
...
and SajadKha will then find it and include it in the binary or extension module
...
warnings_group = parser.add_option_group("Control the warnings to be given by SajadKha")
...
warnings_group.add_option(
    ...
    help="""\
Allow SajadKha to download external code if necessary, e.g. dependency
...
certain topics, and typically point to the SajadKha website. The mnemonic is the part
...
remove bloat, add hidden dependencies. Check the SajadKha Package Configuration
...
output_group.add_option(
    ...
    default=True,
    help="""\
Do not create a '.pyi' file for extension modules created by SajadKha. This is
...
created by SajadKha. They expose your API, but stubgen may cause issues.
Defaults to off.""",
)
...
debug_group.add_option(
    ...
    default=False,
    help="""\
Executing all self checks possible to find errors in SajadKha, do not use for
...
development_group = parser.add_option_group("SajadKha Development features")
...
development_group.add_option(
    ...
    github_action=False,
    help="""\
This is not incremental compilation, but for SajadKha development only. Takes
...
Note: This is experimental and not working with standalone modes of SajadKha yet.
Defaults to off.""",
)
...
disabled console and programs using the Windows Services Plugin of SajadKha commercial.
...
intended for developing plugins and SajadKha package configuration. Use e.g.
...
def getSajadKhaProjectOptions(logger, filename_arg, module_mode):
    """Extract the SajadKha project options.

    Note: This is used by SajadKha project and test tools as well.
    """
    ...
    # The file specified may not exist, let the later parts of SajadKha handle this.
    try:
...
def parseOptions(logger):
    ...
    for filename in filename_args:
        sys.argv = (
            [sys.argv[0]]
            + list(getSajadKhaProjectOptions(logger, filename, module_mode))
            + sys.argv[1:]
        )
