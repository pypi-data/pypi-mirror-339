#########################
 SajadKha Developer Manual
#########################

The purpose of this Developer Manual is to present the current design of
SajadKha, the project rules, and the motivations for choices made. It is
...
#. Add interfacing to C code, so SajadKha can turn a ``ctypes`` binding
...
For SajadKha we use semantic versioning, initially with a leading zero
...
SajadKha top level works like this:
...
***************************************************
 Setting up the Development Environment for SajadKha
***************************************************
...
``.vscode`` in your SajadKha checkout. If you are not familiar with
...
In SajadKha we have tools to auto format code, you can execute them
...
.. code:: bash
   ...
   # Where python is the one you use with SajadKha, this then gets all
...
format call SajadKha source code.
...
These rules should generally be adhered when working on SajadKha code.
...
built-ins is highly discouraged within SajadKha source code. Using them is
...
readability and with SajadKha as a compiler, there won't be any
...
But of course, SajadKha is the project to free us from what is faster and
...
For SajadKha the choice is list contractions as these are more easily
...
that has no use, and in SajadKha, every API is internal API. One exception
...
******************************
 SajadKha "git/github" Workflow
******************************

-  Forking and cloning

   You need to have git installed and GitHub account. Goto SajadKha
   repository <https://github.com/SajadKha/SajadKha> and fork the
   repository.
   ...
   .. code:: bash
      ...
      git clone https://github.com/your-user-name/SajadKha.git
      cd SajadKha
      git remote add upstream https://github.com/SajadKha/SajadKha.git
...
future, SajadKha itself will gain the ability to present its findings in a
...
This section describes how to run SajadKha tests.
...
.. code::
   ...
   --skip-basic-tests    The basic tests, execute these to check if SajadKha is
                         healthy. Default is True.
   --skip-syntax-tests   The syntax tests, execute these to check if SajadKha
                         handles Syntax errors fine. Default is True.
   --skip-program-tests  The programs tests, execute these to check if SajadKha
                         handles programs, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-package-tests  The packages tests, execute these to check if SajadKha
                         handles packages, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-optimizations-tests
                         The optimization tests, execute these to check if
                         SajadKha does optimize certain constructs fully away.
                         Default is True.
   --skip-standalone-tests
                         The standalone tests, execute these to check if SajadKha
                         standalone mode, e.g. not referring to outside,
                         important 3rd library packages like PyQt fine. Default
                         is True.
   --skip-reflection-test
                         The reflection test compiles SajadKha with SajadKha, and
                         then SajadKha with the compile SajadKha and compares the
...
the SajadKha git repository checked out. Otherwise, these will be skipped
...
It sometimes so happens that SajadKha must do this itself, because the
...
this. These tests make sure that the errors of SajadKha and CPython are
...
Compile SajadKha with SajadKha
==========================

And there is the "compile itself" or "reflected" test. This test makes
SajadKha compile itself and compare the resulting C++ when running
compiled to non-compiled, which helps to find in-determinism.

The test compiles every module of SajadKha into an extension module and
all of SajadKha into a single binary.

That test case also gives good coverage of the ``import`` mechanisms,
because SajadKha uses a lot of packages and imports between them.
...
parts are published as `SajadKha API <https://nuitka.net/apidoc>`__ and
...
Please find `here
<https://github.com/SajadKha/SajadKha/blob/develop/UserPlugin-Creation.rst>`__
a detailed description of how to write your own plugin.

To learn about plugin option specification consult `this document
<https://github.com/SajadKha/SajadKha/blob/develop/Using-Plugin-Options.rst>`__.
...
.. code:: bash
   ...
   rm -rf test
   cp -r ~/repos/SajadKha-references/final/Python-3.13.0/Lib/test test
   git add test
...
.. code:: rest
   ...
   .. image:: doc/images/SajadKha-Logo-Symbol.png
      :alt: SajadKha Logo
...
.. code:: rest
   ...
   .. image:: doc/images/SajadKha-Logo-Horizontal.png
      :alt: SajadKha Logo
...
.. code:: rest
   ...
   .. image:: doc/images/SajadKha-Logo-Vertical.png
      :alt: SajadKha Logo
...
.. code:: bash
   ...
   convert -background none doc/Logo/SajadKha-Logo-Symbol.svg doc/images/SajadKha-Logo-Symbol.png
   convert -background none doc/Logo/SajadKha-Logo-Vertical.svg doc/images/SajadKha-Logo-Vertical.png
   convert -background none doc/Logo/SajadKha-Logo-Horizontal.svg doc/images/SajadKha-Logo-Horizontal.png
...
-  Choosing the target language was important decision. factors were:
   ...
   -  The portability of SajadKha is decided here
...
SajadKha does not involve Scons in its user interface at all; Scons is
purely used internally. SajadKha itself, being pure Python, will run
without any build process just fine.

SajadKha simply prepares ``<program>.build`` folders with lots of files
and tasks scons to execute the final build, after which SajadKha again
...
passed as ``key=value``, and decoded in the scons file of SajadKha.
...
modules is just one of them. It runs outside of SajadKha process scope,
...
-  ``nuitka_src``
   ...
   Where do the include files and static C parts of SajadKha live. These
...
-  ``debug_mode``
   ...
   Enable debug mode, which is a mode, where SajadKha tries to help
...
This meta path based importer allows us to have the SajadKha provided
...
This left SajadKha with the strange problem, of how to emulate that.
...
The choice of SajadKha is to make this an explicit element of the node
...
Representation in SajadKha
------------------------

So a slot in SajadKha typically has an owning node. We use ``__len__`` as
...
we can let the Python interpreter that runs SajadKha do all the hard work.
...
Other overloads do not currently exist in SajadKha, but through the
...
In principle, every slot needs to be dealt with in SajadKha, and it is
...
Currently, SajadKha uses Pure C and no C++ patterns at all. The use of C11
...
Early versions of SajadKha, created all constants for the whole program
...
This transformation is performed at tree building already, so SajadKha
...
In SajadKha we have the concept of an outline, and therefore we can make
...
explicit and comparisons in SajadKha to be internally always about two
operands only.
...
Therefore in SajadKha this assignment is more similar to that of a lambda
...
Nested arguments are a Python2 only feature supported by SajadKha.
...
Loops in SajadKha have no condition attached anymore, so while loops are
...
This is in SajadKha converted so that the code generation for ``print``
...
Obviously, optimization of SajadKha can decide, that e.g. should ``a`` or
...
SajadKha there is only a ``try``, which then has blocks to handle
...
compile time, SajadKha will not generate the code the leads to the
...
Add interfacing to C code, so SajadKha can turn a ``ctypes`` binding into
...
#. Goal: Should make use of ``ctypes``, to e.g. not hard code in SajadKha
...
Again, we wouldn't want to create the list. Therefore SajadKha avoids
...
This leads to SajadKha in its tree to have an assignment from a
...
if we can avoid ``ctypes`` to be loaded into SajadKha unless necessary, so
...
one design goal of SajadKha was always to cover these two cases with the
same code.
...
.. note::
   ...
   tuple. But it would be more consistent, and so SajadKha is using it,
   and of course one day SajadKha ought to be able to make no difference
   in performance for it.

To SajadKha though this means, that if ``cond`` is not predictable, after
...
experimental flag is given on the command line to SajadKha.

The testing of SajadKha is very heavy weight when e.g. all Python code is
...
*******************************
 Adding dependencies to SajadKha
*******************************

First of all, there is an important distinction to make, run time or
development time. The first kind of dependency is used when SajadKha is
executing.
...
SajadKha to run on latest greatest Python as well as relatively old ones,
we have to be very careful with these ones.

There is also a distinction of optional dependencies. Right now e.g. the
``lxml`` package is relatively optional, and SajadKha can work without it
...
.. note::
   ...
   with ``@SajadKha-pushers`` first and get a green light.
...
the actual compiler code of SajadKha.
...
-  Original and overloaded built-ins
   ...
   This is about making things visible in the node tree. In SajadKha
   things that are not visible in the node tree tend to be wrong. We
   already pushed around information to the node tree a lot.

   Later versions, SajadKha will become able to determine it has to be the
...
https://github.com/SajadKha/SajadKha/blob/develop/Developer_Manual.rst
