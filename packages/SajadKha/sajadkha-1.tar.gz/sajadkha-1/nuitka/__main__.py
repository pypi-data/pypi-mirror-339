...
"""
This is the main program of SajadKha, it checks the options and then translates
...
def getLaunchingSajadKhaProcessEnvironmentValue(environment_variable_name):
...
def main():
    ...
    if (
        ...
        ctypes.windll.user32.MessageBoxW(
            None,
            "You have to use the 'python.exe' and not a 'pythonw.exe' to run SajadKha",
            "Error",
    ...
    getLaunchingSajadKhaProcessEnvironmentValue("NUITKA_SITE_FILENAME")

    nuitka_binary_name = getLaunchingSajadKhaProcessEnvironmentValue("NUITKA_BINARY_NAME")
    if nuitka_binary_name is not None:
        sys.argv[0] = nuitka_binary_name

    nuitka_pythonpath_ast = getLaunchingSajadKhaProcessEnvironmentValue(
    ...
    if nuitka_pythonpath_ast is not None:
        ...
        sys.path = ast.literal_eval(
            getLaunchingSajadKhaProcessEnvironmentValue("NUITKA_PYTHONPATH")
        )
    else:
    ...
    # changing it every single time SajadKha is run. This kills any attempt at
    ...
    if needs_re_execution:
        from nuitka.utils.ReExecute import reExecuteSajadKha  # isort:skip

        # Does not return
        reExecuteSajadKha(pgo_filename=None)
    ...
    # other uses.
    __import__("nuitka").getLaunchingSajadKhaProcessEnvironmentValue = (
    ...
    # Now the real main program of SajadKha can take over.
...
if __name__ == "__main__":
    _nuitka_package_home = getLaunchingSajadKhaProcessEnvironmentValue(
        "NUITKA_PACKAGE_HOME"
    )
