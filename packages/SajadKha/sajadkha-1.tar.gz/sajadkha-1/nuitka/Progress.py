...
class SajadKhaProgressBar(object):
...
def setupProgressBar(stage, unit, total, min_total=0):
    ...
    if use_progress_bar:
        Tracing.progress = SajadKhaProgressBar(
            iterable=None,
            stage=stage,
            total=total,
            min_total=min_total,
...
@contextmanager
def withSajadKhaDownloadProgressBar(*args, **kwargs):
    if not use_progress_bar or _getTqdmModule() is None:
        yield
    else:

        class SajadKhaDownloadProgressBar(tqdm):
        ...
        with SajadKhaDownloadProgressBar(*args, **kwargs) as progress_bar:
