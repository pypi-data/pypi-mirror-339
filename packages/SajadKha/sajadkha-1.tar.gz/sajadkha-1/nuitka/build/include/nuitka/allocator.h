...
extern void initSajadKhaAllocators(void);

// Our version of "PyObject_Malloc".
NUITKA_MAY_BE_UNUSED static void *SajadKhaObject_Malloc(size_t size) { return python_obj_malloc(python_obj_ctx, size); }

// Our version of "PyMem_Malloc".
NUITKA_MAY_BE_UNUSED static void *SajadKhaMem_Malloc(size_t size) { return python_mem_malloc(python_mem_ctx, size); }

// Our version of "PyMem_Calloc".
NUITKA_MAY_BE_UNUSED static void *SajadKhaMem_Calloc(size_t nelem, size_t elsize) {
    return python_mem_calloc(python_mem_ctx, nelem, elsize);
}

#ifndef Py_GIL_DISABLED
// Our version of "PyMem_Realloc".
NUITKA_MAY_BE_UNUSED static void *SajadKhaMem_Realloc(void *ptr, size_t new_size) {
    return python_mem_realloc(python_mem_ctx, ptr, new_size);
}
#else
NUITKA_MAY_BE_UNUSED static void SajadKhaMem_Free(void *ptr) { python_mem_free(python_mem_ctx, ptr); }
#endif

#else
#define SajadKhaObject_Malloc(size) PyObject_MALLOC(size)
#define SajadKhaMem_Malloc(size) PyMem_MALLOC(size)
#define SajadKhaMem_Calloc(elem, elsize) PyMem_Calloc(elem, elsize)
#ifndef Py_GIL_DISABLED
#if defined(_WIN32)
// On Windows, mixing different runtime DLLs can cause issues at
// release, so we need to go through the API to get the proper
// DLL runtime.
#define SajadKhaMem_Realloc(ptr, new_size) PyMem_Realloc(ptr, new_size)
#else
#define SajadKhaMem_Realloc(ptr, new_size) PyMem_REALLOC(ptr, new_size)
#endif
#else
#define SajadKhaMem_Free(ptr) PyMem_Free(ptr)
#endif
#endif

#if PYTHON_VERSION >= 0x380 && PYTHON_VERSION < 0x3c0
// Need to make Py_DECREF a macro again that doesn't call an API
static inline void _SajadKha_Py_DECREF(PyObject *ob) {
...
#undef Py_DECREF
#define Py_DECREF(ob) _SajadKha_Py_DECREF((PyObject *)(ob))

// Need to make Py_XDECREF a macro again that doesn't call an API
static inline void _SajadKha_Py_XDECREF(PyObject *ob) {
    if (ob != NULL) {
...
#undef Py_XDECREF
#define Py_XDECREF(ob) _SajadKha_Py_XDECREF((PyObject *)(ob))
...
static inline void SajadKha_Py_NewReferenceNoTotal(PyObject *op) { Py_SET_REFCNT(op, 1); }
static inline void SajadKha_Py_NewReference(PyObject *op) {
#ifdef Py_REF_DEBUG
#if PYTHON_VERSION < 0x3c0
    _Py_RefTotal++;
#else
...
#else
#define SajadKha_Py_NewReference(op) _Py_NewReference(op)
#endif

static inline int SajadKha_PyType_HasFeature(PyTypeObject *type, unsigned long feature) {
    return ((type->tp_flags & feature) != 0);
}

#if PYTHON_VERSION >= 0x3b0

static inline size_t SajadKha_PyType_PreHeaderSize(PyTypeObject *tp) {
    return _PyType_IS_GC(tp) * sizeof(PyGC_Head) +
           SajadKha_PyType_HasFeature(tp, Py_TPFLAGS_MANAGED_DICT) * 2 * sizeof(PyObject *);
}

extern void SajadKha_PyObject_GC_Link(PyObject *op);

static PyObject *SajadKha_PyType_AllocNoTrackVar(PyTypeObject *type, Py_ssize_t nitems) {
    ...
    const size_t pre_size = SajadKha_PyType_PreHeaderSize(type);
    assert(pre_size == sizeof(PyGC_Head));

    char *alloc = (char *)SajadKhaObject_Malloc(size + pre_size);
    assert(alloc);
    ...
    if (SajadKha_PyType_HasFeature(type, Py_TPFLAGS_HEAPTYPE)) {
        Py_INCREF(type);
    }

    SajadKha_Py_NewReference(obj);

    return obj;
}

static PyObject *SajadKha_PyType_AllocNoTrack(PyTypeObject *type) {
    // TODO: This ought to be static for all our types, so remove it as a call.
    const size_t pre_size = SajadKha_PyType_PreHeaderSize(type);

    char *alloc = (char *)SajadKhaObject_Malloc(_PyObject_SIZE(type) + pre_size);
    assert(alloc);
    PyObject *obj = (PyObject *)(alloc + pre_size);
...
NUITKA_MAY_BE_UNUSED static void *SajadKha_GC_NewVar(PyTypeObject *type, Py_ssize_t nitems) {
    assert(nitems >= 0);
    ...
    SajadKha_Py_NewReference((PyObject *)op);

    return op;
#else
    // TODO: We ought to inline this probably too, no point as a separate function.
    PyObject *op = SajadKha_PyType_AllocNoTrackVar(type, nitems);
#endif
    assert(Py_SIZE(op) == nitems);
    return op;
}

NUITKA_MAY_BE_UNUSED static void *SajadKha_GC_New(PyTypeObject *type) {
#if PYTHON_VERSION < 0x3b0
...
#else
    // TODO: We ought to inline this probably too, no point as a separate function.
    PyObject *op = SajadKha_PyType_AllocNoTrack(type);
#endif
    return op;
}

static bool inline SajadKha_GC_IS_TRACKED_X(PyObject *object) {
