...
// SajadKha frames.
#if PYTHON_VERSION < 0x3b0
typedef PyFrameObject SajadKha_ThreadStateFrameType;
#else
typedef _PyInterpreterFrame SajadKha_ThreadStateFrameType;
#endif
...
extern void PRINT_COMPILED_FRAME(char const *prefix, struct SajadKha_FrameObject *frame);
extern void PRINT_INTERPRETER_FRAME(char const *prefix, SajadKha_ThreadStateFrameType *frame);
#else
...
extern struct SajadKha_FrameObject *MAKE_MODULE_FRAME(PyCodeObject *code, PyObject *module);
extern struct SajadKha_FrameObject *MAKE_FUNCTION_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
                                                      Py_ssize_t locals_size);
extern struct SajadKha_FrameObject *MAKE_CLASS_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
...
extern PyTypeObject SajadKha_Frame_Type;

static inline bool SajadKha_Frame_CheckExact(PyObject *object) {
    CHECK_OBJECT(object);
    return Py_TYPE(object) == &SajadKha_Frame_Type;
}

static inline bool SajadKha_Frame_Check(PyObject *object) {
    assert(object);
    ...
    if (SajadKha_Frame_CheckExact(object)) {
        return true;
    }
...
NUITKA_MAY_BE_UNUSED static inline bool isFrameUnusable(struct SajadKha_FrameObject *frame_object) {
...
#if PYTHON_VERSION >= 0x3b0
inline static PyCodeObject *SajadKha_InterpreterFrame_GetCodeObject(_PyInterpreterFrame *frame) {
...
inline static PyCodeObject *SajadKha_Frame_GetCodeObject(PyFrameObject *frame) {
#if PYTHON_VERSION >= 0x3b0
    assert(frame->f_frame);
    return SajadKha_InterpreterFrame_GetCodeObject(frame->f_frame);
#else
    return frame->f_code;
#endif
}
...
inline static void assertPythonFrameObject(PyFrameObject *frame_object) {
    ...
    CHECK_CODE_OBJECT(SajadKha_Frame_GetCodeObject(frame_object));
}

inline static void assertFrameObject(struct SajadKha_FrameObject *frame_object) {
    CHECK_OBJECT(frame_object);
...
inline static void assertThreadFrameObject(SajadKha_ThreadStateFrameType *frame) {
#if PYTHON_VERSION < 0x3b0
...
static inline void SajadKha_PythonFrame_MarkAsExecuting(PyFrameObject *frame) {
...
static inline void SajadKha_Frame_MarkAsExecuting(struct SajadKha_FrameObject *frame) {
    CHECK_OBJECT(frame);
#if PYTHON_VERSION >= 0x3b0
...
#else
#define SajadKha_Frame_MarkAsExecuting(frame) ;
#endif

#if PYTHON_VERSION >= 0x300
static inline void SajadKha_Frame_MarkAsNotExecuting(struct SajadKha_FrameObject *frame) {
    CHECK_OBJECT(frame);
#if PYTHON_VERSION >= 0x3b0
...
#else
#define SajadKha_Frame_MarkAsNotExecuting(frame) ;
#define SajadKha_PythonFrame_MarkAsExecuting(frame) ;
#endif

#if PYTHON_VERSION >= 0x300
static inline bool SajadKha_Frame_IsExecuting(struct SajadKha_FrameObject *frame) {
    CHECK_OBJECT(frame);
#if PYTHON_VERSION >= 0x3b0
...
NUITKA_MAY_BE_UNUSED inline static void pushFrameStackPythonFrame(PyThreadState *tstate, PyFrameObject *frame_object) {
    ...
    if (old) {
        ...
        CHECK_CODE_OBJECT(SajadKha_Frame_GetCodeObject(old));
    }
    ...
    SajadKha_PythonFrame_MarkAsExecuting(frame_object);
...
NUITKA_MAY_BE_UNUSED inline static void pushFrameStackCompiledFrame(PyThreadState *tstate,
    ...
    SajadKha_Frame_MarkAsExecuting(frame_object);
    Py_INCREF(frame_object);
#endif
}
...
NUITKA_MAY_BE_UNUSED inline static void popFrameStack(PyThreadState *tstate) {
...
#if PYTHON_VERSION < 0x3b0
    struct SajadKha_FrameObject *frame_object = (struct SajadKha_FrameObject *)(tstate->frame);
    CHECK_OBJECT(frame_object);

#if _DEBUG_FRAME
    printf("Taking off frame %s %s\n", SajadKha_String_AsString(PyObject_Str((PyObject *)frame_object)),
    ...
    SajadKha_Frame_MarkAsNotExecuting(frame_object);
    Py_DECREF(frame_object);
#else
    assert(CURRENT_TSTATE_INTERPRETER_FRAME(tstate));

    struct SajadKha_FrameObject *frame_object =
        (struct SajadKha_FrameObject *)CURRENT_TSTATE_INTERPRETER_FRAME(tstate)->frame_obj;
    CHECK_OBJECT(frame_object);
...
#if PYTHON_VERSION >= 0x300
NUITKA_MAY_BE_UNUSED static void SajadKha_SetFrameGenerator(struct SajadKha_FrameObject *nuitka_frame,
                                                          PyObject *generator) {
#if PYTHON_VERSION < 0x3b0
    ...
    if (generator) {
        SajadKha_Frame_MarkAsExecuting(nuitka_frame);
    }
}
#endif

NUITKA_MAY_BE_UNUSED static PyCodeObject *SajadKha_GetFrameCodeObject(struct SajadKha_FrameObject *nuitka_frame) {
#if PYTHON_VERSION < 0x3b0
    return nuitka_frame->m_frame.f_code;
#else
    return SajadKha_InterpreterFrame_GetCodeObject(&nuitka_frame->m_interpreter_frame);
#endif
}

NUITKA_MAY_BE_UNUSED static int SajadKha_GetFrameLineNumber(struct SajadKha_FrameObject *nuitka_frame) {
    return nuitka_frame->m_frame.f_lineno;
}

NUITKA_MAY_BE_UNUSED static PyObject **SajadKha_GetCodeVarNames(PyCodeObject *code_object) {
#if PYTHON_VERSION < 0x3b0
...
extern void SajadKha_Frame_AttachLocals(struct SajadKha_FrameObject *frame, char const *type_description, ...);

NUITKA_MAY_BE_UNUSED static SajadKha_ThreadStateFrameType *_SajadKha_GetThreadStateFrame(PyThreadState *tstate) {
#if PYTHON_VERSION < 0x3b0
    return tstate->frame;
#else
...
NUITKA_MAY_BE_UNUSED inline static void pushFrameStackGenerator(PyThreadState *tstate,
                                                                SajadKha_ThreadStateFrameType *frame_object) {
#if PYTHON_VERSION < 0x3b0
...
#if _DEBUG_REFCOUNTS
extern int count_active_SajadKha_Frame_Type;
extern int count_allocated_SajadKha_Frame_Type;
extern int count_released_SajadKha_Frame_Type;
#endif
