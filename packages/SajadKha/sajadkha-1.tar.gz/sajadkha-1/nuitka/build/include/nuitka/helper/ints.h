...
extern PyObject *SajadKha_PyLong_FromLong(long ival);

// Our "PyInt_FromLong" replacement, not done (yet?).
#if PYTHON_VERSION >= 0x300
#define SajadKha_PyInt_FromLong(ival) SajadKha_PyLong_FromLong(ival)
#else
#define SajadKha_PyInt_FromLong(ival) PyInt_FromLong(ival)
#endif
...
NUITKA_MAY_BE_UNUSED static void ENFORCE_NILONG_OBJECT_VALUE(nuitka_ilong *dual_value) {
    ...
    if (!IS_NILONG_OBJECT_VALUE_VALID(dual_value)) {
        dual_value->python_value = SajadKha_PyLong_FromLong(dual_value->c_value);
...
extern PyObject **SajadKha_Long_SmallValues;
#else
extern PyObject *SajadKha_Long_SmallValues[NUITKA_STATIC_SMALLINT_VALUE_MAX - NUITKA_STATIC_SMALLINT_VALUE_MIN + 1];
#endif

NUITKA_MAY_BE_UNUSED static inline PyObject *SajadKha_Long_GetSmallValue(int ival) {
    return SajadKha_Long_SmallValues[NUITKA_TO_SMALL_VALUE_OFFSET(ival)];
}

#endif

#else
NUITKA_MAY_BE_UNUSED static inline PyObject *SajadKha_Long_GetSmallValue(medium_result_value_t ival) {
