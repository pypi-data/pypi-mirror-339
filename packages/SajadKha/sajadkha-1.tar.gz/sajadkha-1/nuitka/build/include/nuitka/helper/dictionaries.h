...
typedef PyDictEntry *SajadKha_DictEntryHandle;

static PyDictEntry *GET_STRING_DICT_ENTRY(PyDictObject *dict, SajadKha_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(SajadKha_String_CheckExact(key));
...
NUITKA_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(SajadKha_DictEntryHandle handle) { return handle->me_value; }

NUITKA_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(SajadKha_DictEntryHandle handle, PyObject *value) {
    handle->me_value = value;
}

static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, SajadKha_StringObject *key) {
...
typedef PyObject **SajadKha_DictEntryHandle;

#if PYTHON_VERSION >= 0x3b0
extern Py_ssize_t SajadKha_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash);

extern Py_ssize_t SajadKha_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
extern Py_ssize_t SajadKha_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
#endif

static SajadKha_DictEntryHandle GET_STRING_DICT_ENTRY(PyDictObject *dict, SajadKha_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(SajadKha_String_CheckExact(key));
    ...
    // We cannot use SajadKha_PyDictLookupStr in all cases, some modules misbehave
    // and put non-strings in module dictionaries.
    if (unlikely(dict->ma_keys->dk_kind == DICT_KEYS_GENERAL)) {
        found = SajadKha_PyDictLookup(dict, (PyObject *)key, hash, &value);
    } else {
        found = SajadKha_PyDictLookupStr(dict, (PyObject *)key, hash, &value);
    }
...
NUITKA_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(SajadKha_DictEntryHandle handle) { return *handle; }

NUITKA_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(SajadKha_DictEntryHandle handle, PyObject *value) {
    *handle = value;
}

NUITKA_MAY_BE_UNUSED static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, SajadKha_StringObject *key) {
    SajadKha_DictEntryHandle handle = GET_STRING_DICT_ENTRY(dict, key);
...
NUITKA_MAY_BE_UNUSED static void UPDATE_STRING_DICT0(PyDictObject *dict, SajadKha_StringObject *key, PyObject *value) {
    CHECK_OBJECT(value);

    SajadKha_DictEntryHandle entry = GET_STRING_DICT_ENTRY(dict, key);
...
NUITKA_MAY_BE_UNUSED static void UPDATE_STRING_DICT_INPLACE(PyDictObject *dict, SajadKha_StringObject *key,
                                                            PyObject *value) {
    CHECK_OBJECT(value);
...
NUITKA_MAY_BE_UNUSED static void UPDATE_STRING_DICT1(PyDictObject *dict, SajadKha_StringObject *key, PyObject *value) {
    CHECK_OBJECT(key);
    CHECK_OBJECT(value);
...
extern bool SajadKha_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr);
