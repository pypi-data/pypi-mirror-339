    ...
    if (free_list != NULL) {                                                                                           \
        ...
        if (Py_SIZE(result) < size) {                                                                                  \
            ...
            assert(result != NULL);                                                                                    \
                                                                                                                       ...
                                                                                                                       \
        SajadKha_Py_NewReference((PyObject *)result);                                                                    \
    } else {                                                                                                           \
        result = (object_type *)SajadKha_GC_NewVar(&type_type, size);                                                    \
    }                                                                                                                  \
    CHECK_OBJECT(result);
...
#define allocateFromFreeListFixed(free_list, object_type, type_type)                                                   \
    if (free_list != NULL) {                                                                                           \
        ...
        result = (object_type *)SajadKha_GC_New(&type_type);                                                             \
    }                                                                                                                  \
    CHECK_OBJECT(result);
...
NUITKA_MAY_BE_UNUSED static inline struct _Py_object_freelists *_SajadKha_object_freelists_GET(PyThreadState *tstate) {
