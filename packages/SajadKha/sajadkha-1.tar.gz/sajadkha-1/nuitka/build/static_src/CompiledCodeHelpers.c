...
static PyObject *_BUILTIN_RANGE_INT3(long low, long high, long step) {
    ...
    for (int i = 0; i < size; i++) {
        PyList_SET_ITEM(result, i, SajadKha_PyInt_FromLong(current));
        current += step;
    }
...
static PyObject *_BUILTIN_RANGE_INT(long boundary) {
    ...
    for (int i = 0; i < boundary; i++) {
        PyList_SET_ITEM(result, i, SajadKha_PyInt_FromLong(i));
    }
...
static PyObject *MAKE_XRANGE(PyThreadState *tstate, PyObject *start, PyObject *stop, PyObject *step) {
    start = SajadKha_Number_IndexAsLong(start);
    if (unlikely(start == NULL)) {
        return NULL;
    }
    stop = SajadKha_Number_IndexAsLong(stop);
    if (unlikely(stop == NULL)) {
        return NULL;
    }
    step = SajadKha_Number_IndexAsLong(step);
    if (unlikely(step == NULL)) {
        return NULL;
    }
...
#else
    PyObject *stop = SajadKha_Number_IndexAsLong(high);
...
PyObject *BUILTIN_LEN(PyThreadState *tstate, PyObject *value) {
    ...
    Py_ssize_t res = SajadKha_PyObject_Size(value);
...
bool PRINT_STRING_W(wchar_t const *str) {
    if (str) {
        PyObject *tmp = SajadKhaUnicode_FromWideChar(str, -1);
        bool res = PRINT_ITEM(tmp);
        Py_DECREF(tmp);
        return res;
    } else {
...
void _PRINT_EXCEPTION3(PyObject *exception_type, PyObject *exception_value, PyTracebackObject *exception_tb) {
    ...
    if (exception_value != NULL && PyExceptionInstance_Check(exception_value)) {
        ...
        PyObject *context = SajadKha_Exception_GetContext(exception_value);
        PRINT_REPR(context);
    }
#endif
    PRINT_STRING("|");
...
void PRINT_TRACEBACK(PyTracebackObject *traceback) {
    ...
    while (traceback != NULL) {
        ...
        while (frame != NULL) {
            printf("  Frame at %s\n", PyString_AsString(PyObject_Str((PyObject *)SajadKha_Frame_GetCodeObject(frame))));
...
PyObject *GET_STDOUT(void) {
    PyObject *result = SajadKha_SysGetObject("stdout");
...
PyObject *GET_STDERR(void) {
    PyObject *result = SajadKha_SysGetObject("stderr");
...
static PyObject *nuitka_class_getattr(PyClassObject *class_object, PyObject *attr_name) {
    ...
    descrgetfunc tp_descr_get = SajadKhaType_HasFeatureClass(type) ? type->tp_descr_get : NULL;
...
static bool MAKE_QUICK_ITERATOR(PyThreadState *tstate, PyObject *sequence, struct SajadKha_QuickIterator *qiter) {
    if (SajadKha_Generator_Check(sequence)) {
        qiter->iterator_mode = ITERATOR_COMPILED_GENERATOR;
        qiter->iterator_data.generator = (struct SajadKha_GeneratorObject *)sequence;
...
static PyObject *QUICK_ITERATOR_NEXT(PyThreadState *tstate, struct SajadKha_QuickIterator *qiter, bool *finished) {
    PyObject *result;
    ...
    case ITERATOR_COMPILED_GENERATOR:
        result = SajadKha_Generator_qiter(tstate, qiter->iterator_data.generator, finished);
...
PyObject *BUILTIN_SUM1(PyThreadState *tstate, PyObject *sequence) {
    struct SajadKha_QuickIterator qiter;
    ...
    for (;;) {
        ...
        if (finished) {
            return SajadKha_PyInt_FromLong(int_result);
        } else if (item == NULL) {
            return NULL;
        }
...
// For Python2 long, Python3 int objects
    ...
    result = SajadKha_PyInt_FromLong(int_result);
    CHECK_OBJECT(result);
...
static PyTypeObject SajadKha_BuiltinModule_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_module", // tp_name
    sizeof(PyModuleObject),                           // tp_size
};

int SajadKha_BuiltinModule_SetAttr(PyModuleObject *module, PyObject *name, PyObject *value) {
    CHECK_OBJECT(module);
    CHECK_OBJECT(name);
...
static PyObject *getPathSeparatorStringObject(void) {
    ...
    if (sep_str == NULL) {
        sep_str = SajadKha_String_FromString(sep);
    }
...
// there e.g. code objects expect Python2 strings.
    ...
    *binary_object_target = SajadKhaUnicode_FromWideChar(exe_filename, -1);
#else
...
// there e.g. code objects expect Python2 strings.
    ...
    *binary_object_target = SajadKhaUnicode_FromWideChar(bin_directory, -1);
#else
...
static PyObject *getDllDirectoryObject(void) {
    ...
    if (dll_directory == NULL) {
        ...
        dll_directory = SajadKha_String_FromFilename(dll_directory_filename);
...
PyObject *getDllFilenameObject(void) {
    ...
    if (dll_filename == NULL) {
        ...
        dll_filename = SajadKha_String_FromFilename(dll_filename_str);
...
PyObject *getContainingDirectoryObject(bool resolve_symlinks) {
    ...
    if (onefile_directory != NULL) {
        PyObject *result = SajadKha_String_FromFilename(onefile_directory);
...
void _initBuiltinModule(void) {
    ...
    // init SajadKha_BuiltinModule_Type, PyType_Ready won't copy all member from
    ...
    // tp_setattro to SajadKha_BuiltinModule_SetAttr and we can detect value
    // change. Set tp_base to PyModule_Type and PyModule_Check will pass.
    SajadKha_BuiltinModule_Type.tp_dealloc = PyModule_Type.tp_dealloc;
    SajadKha_BuiltinModule_Type.tp_repr = PyModule_Type.tp_repr;
    SajadKha_BuiltinModule_Type.tp_setattro = (setattrofunc)SajadKha_BuiltinModule_SetAttr;
    SajadKha_BuiltinModule_Type.tp_getattro = PyModule_Type.tp_getattro;
    SajadKha_BuiltinModule_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC | Py_TPFLAGS_BASETYPE;
    SajadKha_BuiltinModule_Type.tp_doc = PyModule_Type.tp_doc;
    SajadKha_BuiltinModule_Type.tp_traverse = PyModule_Type.tp_traverse;
    SajadKha_BuiltinModule_Type.tp_members = PyModule_Type.tp_members;
    SajadKha_BuiltinModule_Type.tp_base = &PyModule_Type;
    SajadKha_BuiltinModule_Type.tp_dictoffset = PyModule_Type.tp_dictoffset;
    SajadKha_BuiltinModule_Type.tp_init = PyModule_Type.tp_init;
    SajadKha_BuiltinModule_Type.tp_alloc = PyModule_Type.tp_alloc;
    SajadKha_BuiltinModule_Type.tp_new = PyModule_Type.tp_new;
    SajadKha_BuiltinModule_Type.tp_free = PyModule_Type.tp_free;
    NUITKA_MAY_BE_UNUSED int res2 = PyType_Ready(&SajadKha_BuiltinModule_Type);
    assert(res2 >= 0);

    // Replace type of builtin module to take over.
    ((PyObject *)builtin_module)->ob_type = &SajadKha_BuiltinModule_Type;
