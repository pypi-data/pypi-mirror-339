...
#else
    PyObject **value_addr;
    Py_ssize_t ix = SajadKha_PyDictLookup(dict_object, key, hash, &value_addr);
#endif
...
static PyObject *SajadKha_CreateKeyError(PyThreadState *tstate, PyObject *key) {
    return (PyObject *)SajadKha_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_KeyError, key);
}
#endif
...
#else
    struct SajadKha_ExceptionPreservationItem exception_state = {SajadKha_CreateKeyError(tstate, key)};
...
static inline PyObject *_MAKE_DICT_ITERATOR(PyThreadState *tstate, PyDictObject *dict, PyTypeObject *type,
...
#if PYTHON_VERSION < 0x300
    dictiterobject *di = (dictiterobject *)SajadKha_GC_New(type);
    CHECK_OBJECT(di);
    Py_INCREF(dict);
    di->di_dict = dict;
    ...
    SajadKha_GC_Track(di);
    return (PyObject *)di;
#else
    _PyDictViewObject *dv = (_PyDictViewObject *)SajadKha_GC_New(type);
    CHECK_OBJECT(dv);

    Py_INCREF(dict);
    dv->dv_dict = dict;

    SajadKha_GC_Track(dv);
    return (PyObject *)dv;
#endif
}
...
static PyObject *_MAKE_DICT_VIEW(PyDictObject *dict, PyTypeObject *type) {
    ...
    dictviewobject *dv = (dictviewobject *)SajadKha_GC_New(type);
...
#if PYTHON_VERSION >= 0x300
static PyDictObject *_SajadKha_AllocatePyDictObject(PyThreadState *tstate) {
...
#else
    struct _Py_object_freelists *freelists = _SajadKha_object_freelists_GET(tstate);
    ...
    if (*numfree) {
        ...
        SajadKha_Py_NewReference((PyObject *)result_mp);
...
#endif
    {
        result_mp = (PyDictObject *)SajadKha_GC_New(&PyDict_Type);
    }
...
#if PYTHON_VERSION >= 0x360
static PyDictKeysObject *_SajadKha_AllocatePyDictKeysObject(PyThreadState *tstate, Py_ssize_t keys_size) {
...
// seems it can only be used for the smallest keys type.
    ...
    {
#if PYTHON_VERSION < 0x3d0
        dk = (PyDictKeysObject *)SajadKhaObject_Malloc(keys_size);
#else
        dk = (PyDictKeysObject *)SajadKhaMem_Malloc(keys_size);
#endif
    }
...
static Py_ssize_t _SajadKha_Py_PyDict_KeysSize(PyDictKeysObject *keys) {
#if PYTHON_VERSION < 0x360
...
#if PYTHON_VERSION >= 0x3b0
static inline int SajadKha_py_get_index_from_order(PyDictObject *mp, Py_ssize_t i) {
...
static inline Py_ssize_t SajadKha_Py_dictkeys_get_index(const PyDictKeysObject *keys, Py_ssize_t i) {
...
static inline Py_hash_t SajadKha_Py_unicode_get_hash(PyObject *o) {
#if PYTHON_VERSION < 0x3d0
...
static Py_ssize_t SajadKha_Py_unicodekeys_lookup_generic(PyDictObject *mp, PyDictKeysObject *dk, PyObject *key,
                                                       Py_hash_t hash) {
    ...
    while (1) {
        Py_ssize_t ix = SajadKha_Py_dictkeys_get_index(dk, i);
        ...
        if (ix >= 0) {
            ...
            if (SajadKha_Py_unicode_get_hash(ep->me_key) == hash) {
    ...
    NUITKA_CANNOT_GET_HERE("SajadKha_Py_unicodekeys_lookup_generic failed");
}

Py_ssize_t SajadKha_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash) {
    ...
    while (true) {
        ...
        if (ix >= 0) {
            ...
            if (ep->me_key == key || (SajadKha_Py_unicode_get_hash(ep->me_key) == hash &&
        ...
        i = mask & (i * 5 + perturb + 1);
        ix = SajadKha_Py_dictkeys_get_index(dk, i);
    ...
    NUITKA_CANNOT_GET_HERE("SajadKha_Py_unicodekeys_lookup_unicode failed");
}

// Search key from Generic table.
static Py_ssize_t SajadKha_Py_dictkeys_generic_lookup(PyDictObject *mp, PyDictKeysObject *dk, PyObject *key,
                                                    Py_hash_t hash) {
...
Py_ssize_t SajadKha_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr) {
    PyDictKeysObject *dk;
    DictKeysKind kind;
    Py_ssize_t ix;
...
restart:
    ...
    if (kind != DICT_KEYS_GENERAL) {
        if (PyUnicode_CheckExact(key)) {
            ix = SajadKha_Py_unicodekeys_lookup_unicode(dk, key, hash);
        } else {
            ix = SajadKha_Py_unicodekeys_lookup_generic(mp, dk, key, hash);
        ...
        ix = SajadKha_Py_dictkeys_generic_lookup(mp, dk, key, hash);
...
Py_ssize_t SajadKha_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr) {
    assert(PyUnicode_CheckExact(key));

    PyDictKeysObject *dk = mp->ma_keys;
    assert(dk->dk_kind != DICT_KEYS_GENERAL);

    Py_ssize_t ix = SajadKha_Py_unicodekeys_lookup_unicode(dk, key, hash);
...
bool SajadKha_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr) {
    CHECK_OBJECT(dict);
    assert(PyDict_CheckExact(dict));
    assert(key_ptr != NULL);
    ...
    *value_ptr = value;
    ...
    if (mp->ma_values) {
        ...
        int index = SajadKha_py_get_index_from_order(mp, i);
...
PyObject *MAKE_DICT_EMPTY(PyThreadState *tstate) {
    ...
    PyDictObject *result_mp = _SajadKha_AllocatePyDictObject(tstate);
    ...
    // No SajadKha_GC_Track for the empty dictionary.
