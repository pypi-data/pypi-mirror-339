...
#if _DEBUG_REFCOUNTS
int count_active_SajadKha_Frame_Type = 0;
int count_allocated_SajadKha_Frame_Type = 0;
int count_released_SajadKha_Frame_Type = 0;
#endif
...
static PyMemberDef SajadKha_Frame_members[] = {
...
#else
#define SajadKha_Frame_members 0
#endif

#if PYTHON_VERSION < 0x300

static PyObject *_SajadKha_Frame_get_exc_traceback(PyObject *self, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct SajadKha_FrameObject *frame = (struct SajadKha_FrameObject *)self;
...
static int _SajadKha_Frame_set_exc_traceback(PyObject *self, PyObject *traceback, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_SajadKha_Frame_get_exc_type(PyObject *self, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _SajadKha_Frame_set_exc_type(PyObject *self, PyObject *exception_type, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_SajadKha_Frame_get_exc_value(PyObject *self, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _SajadKha_Frame_set_exc_value(PyObject *self, PyObject *exception_value, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_SajadKha_Frame_get_restricted(PyObject *self, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_SajadKha_Frame_get_locals(PyObject *self, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    struct SajadKha_FrameObject *nuitka_frame = (struct SajadKha_FrameObject *)self;
    if (nuitka_frame->m_type_description == NULL) {
        ...
        PyObject **var_names = SajadKha_GetCodeVarNames(SajadKha_GetFrameCodeObject(nuitka_frame));
        ...
        while (*w != 0) {
            ...
            case NUITKA_TYPE_DESCRIPTION_CELL: {
                struct SajadKha_CellObject *value = *(struct SajadKha_CellObject **)t;
                assert(SajadKha_Cell_Check((PyObject *)value));
                CHECK_OBJECT(value);
                ...
                t += sizeof(struct SajadKha_CellObject *);
...
static PyObject *_SajadKha_Frame_get_lineno(PyObject *self, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct SajadKha_FrameObject *frame = (struct SajadKha_FrameObject *)self;
    return SajadKha_PyInt_FromLong(frame->m_frame.f_lineno);
}

static PyObject *_SajadKha_Frame_get_trace(PyObject *self, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _SajadKha_Frame_set_trace(PyObject *self, PyObject *value, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    if (value == Py_None) {
        ...
        SET_CURRENT_EXCEPTION_TYPE0_STR(
            ...
            "f_trace is not writable in SajadKha, ignore with '--no-deployment-flag=frame-useless-set-trace'");
        return -1;
    }
#else
    return 0;
...
static PyObject *_SajadKha_Frame_get_trace_lines(PyObject *self, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _SajadKha_Frame_set_trace_lines(PyObject *self, PyObject *value, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_lines is not writable in SajadKha");
    return -1;
}

static PyObject *_SajadKha_Frame_get_trace_opcodes(PyObject *self, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _SajadKha_Frame_set_trace_opcodes(PyObject *self, PyObject *value, void *data) {
    assert(SajadKha_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_opcodes is not writable in SajadKha");
    return -1;
}
#endif

static PyGetSetDef SajadKha_Frame_tp_getset[] = {
    {(char *)"f_locals", _SajadKha_Frame_get_locals, NULL, NULL},
    {(char *)"f_lineno", _SajadKha_Frame_get_lineno, NULL, NULL},
    {(char *)"f_trace", _SajadKha_Frame_get_trace, _SajadKha_Frame_set_trace, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"f_restricted", _SajadKha_Frame_get_restricted, NULL, NULL},
    {(char *)"f_exc_traceback", _SajadKha_Frame_get_exc_traceback, _SajadKha_Frame_set_exc_traceback, NULL},
    {(char *)"f_exc_type", _SajadKha_Frame_get_exc_type, _SajadKha_Frame_set_exc_type, NULL},
    {(char *)"f_exc_value", _SajadKha_Frame_get_exc_value, _SajadKha_Frame_set_exc_value, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {(char *)"f_trace_lines", _SajadKha_Frame_get_trace_lines, _SajadKha_Frame_set_trace_lines, NULL},
    {(char *)"f_trace_opcodes", _SajadKha_Frame_get_trace_opcodes, _SajadKha_Frame_set_trace_opcodes, NULL},
#endif
    {NULL}};

// tp_repr slot, decide how a function shall be output
static PyObject *SajadKha_Frame_tp_repr(struct SajadKha_FrameObject *nuitka_frame) {
    assert(SajadKha_Frame_CheckExact((PyObject *)nuitka_frame));
    CHECK_OBJECT((PyObject *)nuitka_frame);
    assert(_PyObject_GC_IS_TRACKED(nuitka_frame));

#if PYTHON_VERSION >= 0x370
    PyCodeObject *code_object = SajadKha_GetFrameCodeObject(nuitka_frame);
    return SajadKha_String_FromFormat("<compiled_frame at %p, file %R, line %d, code %S>", nuitka_frame,
                                    code_object->co_filename, SajadKha_GetFrameLineNumber(nuitka_frame),
                                    code_object->co_name);
#elif _DEBUG_FRAME || _DEBUG_REFRAME || _DEBUG_EXCEPTIONS
    PyCodeObject *code_object = SajadKha_GetFrameCodeObject(nuitka_frame);
    return SajadKha_String_FromFormat("<compiled_frame object for %s at %p>",
                                    SajadKha_String_AsString(code_object->co_name), nuitka_frame);
#else
    return SajadKha_String_FromFormat("<compiled_frame object at %p>", nuitka_frame);
#endif
}

static void SajadKha_Frame_tp_clear(struct SajadKha_FrameObject *frame) {
...
static struct SajadKha_FrameObject *free_list_frames = NULL;
static int free_list_frames_count = 0;

static void SajadKha_Frame_tp_dealloc(struct SajadKha_FrameObject *nuitka_frame) {
#if _DEBUG_REFCOUNTS
    count_active_SajadKha_Frame_Type -= 1;
    count_released_SajadKha_Frame_Type += 1;
#endif
    ...
    SajadKha_GC_UnTrack(nuitka_frame);
    ...
    assert(SajadKha_GC_IS_TRACKED_X((PyObject *)frame->f_back));
    ...
    SajadKha_Frame_tp_clear(nuitka_frame);
...
static int SajadKha_Frame_tp_traverse(struct SajadKha_FrameObject *frame, visitproc visit, void *arg) {
    assert(SajadKha_Frame_CheckExact((PyObject *)frame));
...
static PyObject *SajadKha_GetFrameGenerator(struct SajadKha_FrameObject *nuitka_frame) {
#if PYTHON_VERSION < 0x3b0
...
static PyObject *SajadKha_Frame_clear(struct SajadKha_FrameObject *frame, PyObject *unused) {
    assert(SajadKha_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
    ...
    if (SajadKha_Frame_IsExecuting(frame)) {
    ...
    PyObject *f_gen = SajadKha_GetFrameGenerator(frame);
    if (f_gen != NULL) {
        CHECK_OBJECT(f_gen);
        ...
        if (SajadKha_Generator_Check(f_gen)) {
            struct SajadKha_GeneratorObject *generator = (struct SajadKha_GeneratorObject *)f_gen;
            SajadKha_SetFrameGenerator(frame, NULL);

            close_exception = !_SajadKha_Generator_close(tstate, generator);
        }
#if PYTHON_VERSION >= 0x350
        else if (SajadKha_Coroutine_Check(f_gen)) {
            struct SajadKha_CoroutineObject *coroutine = (struct SajadKha_CoroutineObject *)f_gen;
            SajadKha_SetFrameGenerator(frame, NULL);

            close_exception = !_SajadKha_Coroutine_close(tstate, coroutine);
        }
#endif
#if PYTHON_VERSION >= 0x360
        else if (SajadKha_Asyncgen_Check(f_gen)) {
            struct SajadKha_AsyncgenObject *asyncgen = (struct SajadKha_AsyncgenObject *)f_gen;
            SajadKha_SetFrameGenerator(frame, NULL);

            close_exception = !_SajadKha_Asyncgen_close(tstate, asyncgen);
        }
#endif
        else {
...
static inline Py_ssize_t SajadKha_Frame_GetSize(struct SajadKha_FrameObject *frame) {
    assert(SajadKha_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
...
static PyObject *SajadKha_Frame_sizeof(struct SajadKha_FrameObject *frame, PyObject *unused) {
    assert(SajadKha_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));

    return PyInt_FromSsize_t(sizeof(struct SajadKha_FrameObject) + Py_SIZE(frame));
}

static PyMethodDef SajadKha_Frame_methods[] = {
#if PYTHON_VERSION >= 0x300
    {"clear", (PyCFunction)SajadKha_Frame_clear, METH_NOARGS, "F.clear(): clear most references held by the frame"},
#endif
    {"__sizeof__", (PyCFunction)SajadKha_Frame_sizeof, METH_NOARGS, "F.__sizeof__() -> size of F in memory, in bytes"},
    {NULL, NULL}};

PyTypeObject SajadKha_Frame_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_frame",
    sizeof(struct SajadKha_FrameObject),
    1,
    (destructor)SajadKha_Frame_tp_dealloc,     // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_compare
    (reprfunc)SajadKha_Frame_tp_repr,          // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)SajadKha_Frame_tp_traverse,  // tp_traverse
    (inquiry)SajadKha_Frame_tp_clear,          // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    SajadKha_Frame_methods,                    // tp_methods
    SajadKha_Frame_members,                    // tp_members
    SajadKha_Frame_tp_getset,                  // tp_getset
    0,                                       // tp_base
    0,                                       // tp_dict
};

void _initCompiledFrameType(void) {
    assert(SajadKha_Frame_Type.tp_doc != PyFrame_Type.tp_doc || PyFrame_Type.tp_doc == NULL);
    assert(SajadKha_Frame_Type.tp_traverse != PyFrame_Type.tp_traverse);
    assert(SajadKha_Frame_Type.tp_clear != PyFrame_Type.tp_clear || PyFrame_Type.tp_clear == NULL);
    assert(SajadKha_Frame_Type.tp_richcompare != PyFrame_Type.tp_richcompare || PyFrame_Type.tp_richcompare == NULL);
    assert(SajadKha_Frame_Type.tp_weaklistoffset != PyFrame_Type.tp_weaklistoffset ||
           PyFrame_Type.tp_weaklistoffset == 0);
    assert(SajadKha_Frame_Type.tp_iter != PyFrame_Type.tp_iter || PyFrame_Type.tp_iter == NULL);
    assert(SajadKha_Frame_Type.tp_iternext != PyFrame_Type.tp_iternext || PyFrame_Type.tp_iternext == NULL);
    assert(SajadKha_Frame_Type.tp_methods != PyFrame_Type.tp_methods);
    assert(SajadKha_Frame_Type.tp_members != PyFrame_Type.tp_members);
    assert(SajadKha_Frame_Type.tp_getset != PyFrame_Type.tp_getset);

    assert(SajadKha_Frame_Type.tp_descr_get != PyFrame_Type.tp_descr_get || PyFrame_Type.tp_descr_get == NULL);

    assert(SajadKha_Frame_Type.tp_descr_set != PyFrame_Type.tp_descr_set || PyFrame_Type.tp_descr_set == NULL);
    assert(SajadKha_Frame_Type.tp_dictoffset != PyFrame_Type.tp_dictoffset || PyFrame_Type.tp_dictoffset == 0);
    // TODO: These get changed and into the same thing, not sure what to compare against, project something
    // assert(SajadKha_Frame_Type.tp_dict != PyFrame_Type.tp_dict);
    // assert(SajadKha_Frame_Type.tp_init != PyFrame_Type.tp_init || PyFrame_Type.tp_init == NULL);
    // assert(SajadKha_Frame_Type.tp_alloc != PyFrame_Type.tp_alloc || PyFrame_Type.tp_alloc == NULL);
    // assert(SajadKha_Frame_Type.tp_new != PyFrame_Type.tp_new || PyFrame_Type.tp_new == NULL);
    // assert(SajadKha_Frame_Type.tp_free != PyFrame_Type.tp_free || PyFrame_Type.tp_free == NULL);
    // assert(SajadKha_Frame_Type.tp_weaklist != PyFrame_Type.tp_weaklist);
    assert(SajadKha_Frame_Type.tp_bases != PyFrame_Type.tp_bases);
    assert(SajadKha_Frame_Type.tp_mro != PyFrame_Type.tp_mro);
    assert(SajadKha_Frame_Type.tp_cache != PyFrame_Type.tp_cache || PyFrame_Type.tp_cache == NULL);
    assert(SajadKha_Frame_Type.tp_subclasses != PyFrame_Type.tp_subclasses || PyFrame_Type.tp_cache == NULL);
    assert(SajadKha_Frame_Type.tp_del != PyFrame_Type.tp_del || PyFrame_Type.tp_del == NULL);
#if PYTHON_VERSION >= 0x300
    assert(SajadKha_Frame_Type.tp_finalize != PyFrame_Type.tp_finalize || PyFrame_Type.tp_finalize == NULL);
#endif
    SajadKha_PyType_Ready(&SajadKha_Frame_Type, &PyFrame_Type, true, true, false, false, false);

    // These are to be used interchangeably. Make sure that's true.
    assert(offsetof(struct SajadKha_FrameObject, m_frame) == 0);
}

static struct SajadKha_FrameObject *_MAKE_COMPILED_FRAME(PyCodeObject *code, PyObject *module, PyObject *f_locals,
...
#if _DEBUG_REFCOUNTS
    count_active_SajadKha_Frame_Type += 1;
    count_allocated_SajadKha_Frame_Type += 1;
#endif
    ...
    struct SajadKha_FrameObject *result;

    // Macro to assign result memory from GC or free list.
    allocateFromFreeList(free_list_frames, struct SajadKha_FrameObject, SajadKha_Frame_Type, locals_size);
...
#if PYTHON_VERSION >= 0x300
    SajadKha_SetFrameGenerator(result, NULL);

    SajadKha_Frame_MarkAsNotExecuting(result);
#endif
...
struct SajadKha_FrameObject *MAKE_MODULE_FRAME(PyCodeObject *code, PyObject *module) {
...
struct SajadKha_FrameObject *MAKE_FUNCTION_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
                                               Py_ssize_t locals_size) {
    PyObject *f_locals;
...
struct SajadKha_FrameObject *MAKE_CLASS_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
...
PyCodeObject *makeCodeObject(PyObject *filename, int line, int flags, PyObject *function_name,
    ...
    assert(SajadKha_StringOrUnicode_CheckExact(filename));

    CHECK_OBJECT(function_name);
    assert(SajadKha_String_CheckExact(function_name));
    ...
    if (empty_code == NULL) {
        ...
        NUITKA_MAY_BE_UNUSED bool bool_res = SajadKha_DelModuleString(tstate, "nuitka_empty_function");
...
void SajadKha_Frame_AttachLocals(struct SajadKha_FrameObject *frame_object, char const *type_description, ...) {
    assert(SajadKha_Frame_CheckExact((PyObject *)frame_object));
    ...
    PRINT_FORMAT("Attaching to frame 0x%lx %s\n", frame_object,
                 SajadKha_String_AsString(PyObject_Repr((PyObject *)SajadKha_Frame_GetCodeObject(&frame_object->m_frame))));
#endif
    ...
    while (*w != 0) {
        ...
        case NUITKA_TYPE_DESCRIPTION_CELL: {
            struct SajadKha_CellObject *value = va_arg(ap, struct SajadKha_CellObject *);
            assert(SajadKha_Cell_Check((PyObject *)value));
            CHECK_OBJECT(value);
            CHECK_OBJECT_X(value->ob_ref);

            memcpy(t, &value, sizeof(struct SajadKha_CellObject *));
            ...
            // Py_SET_REFCNT((struct SajadKha_CellObject *)t, 1);
            Py_INCREF(value);
        ...
        case NUITKA_TYPE_DESCRIPTION_NULL: {
            NUITKA_MAY_BE_UNUSED void *value = va_arg(ap, struct SajadKha_CellObject *);
    ...
    assert(t - frame_object->m_locals_storage <= SajadKha_Frame_GetSize(frame_object));
}
...
void dumpFrameStack(void) {
    ...
    while (current) {
        ...
        PyObject *code_repr = PyObject_Str((PyObject *)SajadKha_InterpreterFrame_GetCodeObject(current->current_frame));
#endif

        PRINT_FORMAT("Frame stack %d: %s %d %s\n", total--, SajadKha_String_AsString(current_repr), Py_REFCNT(current),
                     SajadKha_String_AsString(code_repr));
...
static void PRINT_UNCOMPILED_FRAME(char const *prefix, PyFrameObject *frame) {
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)SajadKha_Frame_GetCodeObject(frame));
...
void PRINT_COMPILED_FRAME(char const *prefix, struct SajadKha_FrameObject *frame) {
    return PRINT_UNCOMPILED_FRAME(prefix, &frame->m_frame);
}

void PRINT_INTERPRETER_FRAME(char const *prefix, SajadKha_ThreadStateFrameType *frame) {
#if PYTHON_VERSION < 0x3b0
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)SajadKha_InterpreterFrame_GetCodeObject(frame));
        PRINT_ITEM(code_object_str);
        Py_DECREF(code_object_str);
    } else {
        PRINT_STRING("<NULL> no frame");
