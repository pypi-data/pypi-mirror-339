 ...
 * normally should be avoided to exist in SajadKha calls.
 *
 */
...
static PyObject *SajadKha_Method_get__doc__(PyObject *self, void *data) {
    struct SajadKha_MethodObject *method = (struct SajadKha_MethodObject *)self;
...
static PyGetSetDef SajadKha_Method_tp_getset[] = {{(char *)"__doc__", SajadKha_Method_get__doc__, NULL, NULL}, {NULL}};

#define OFF(x) offsetof(struct SajadKha_MethodObject, x)

static PyMemberDef SajadKha_Method_members[] = {
...
static PyObject *SajadKha_Method_reduce(struct SajadKha_MethodObject *method, PyObject *unused) {
...
static PyObject *SajadKha_Method_reduce_ex(struct SajadKha_MethodObject *method, PyObject *args) {
    int proto;
    ...
    PyObject *type_tuple = MAKE_TUPLE1(tstate, (PyObject *)&SajadKha_Method_Type);
    ...
    return result;
#else
    return SajadKha_Method_reduce(method, NULL);
#endif
}

static PyObject *SajadKha_Method_deepcopy(struct SajadKha_MethodObject *method, PyObject *memo) {
    assert(SajadKha_Method_Check((PyObject *)method));
    ...
    PyObject *result = SajadKha_Method_New(method->m_function, object, method->m_class);
    // SajadKha_Method_New took a reference to the object.
    Py_DECREF(object);
    return result;
}

static PyMethodDef SajadKha_Method_methods[] = {
    {"__reduce__", (PyCFunction)SajadKha_Method_reduce, METH_NOARGS, NULL},
    {"__reduce_ex__", (PyCFunction)SajadKha_Method_reduce_ex, METH_VARARGS, NULL},
    {"__deepcopy__", (PyCFunction)SajadKha_Method_deepcopy, METH_O, NULL},
    {NULL}};

#if PYTHON_VERSION >= 0x380 && !defined(_NUITKA_EXPERIMENTAL_DISABLE_VECTORCALL_SLOT)
static PyObject *SajadKha_Method_tp_vectorcall(struct SajadKha_MethodObject *method, PyObject *const *stack, size_t nargsf,
                                             PyObject *kwnames) {
    assert(SajadKha_Method_Check((PyObject *)method));
    ...
    if (totalargs == 0) {
        return SajadKha_CallMethodFunctionNoArgs(PyThreadState_GET(), method->m_function, method->m_object);
    }
    ...
    if (nargsf & PY_VECTORCALL_ARGUMENTS_OFFSET) {
        ...
        result = SajadKha_CallFunctionVectorcall(PyThreadState_GET(), method->m_function, new_args, nargs + 1,
...
static PyObject *SajadKha_Method_tp_call(struct SajadKha_MethodObject *method, PyObject *args, PyObject *kw) {
    ...
    if (method->m_object == NULL) {
        ...
        if (kw == NULL) {
            if (arg_count == 0) {
                return SajadKha_CallMethodFunctionNoArgs(tstate, method->m_function, method->m_object);
            } else {
                return SajadKha_CallMethodFunctionPosArgs(tstate, method->m_function, method->m_object,
                                                        &PyTuple_GET_ITEM(args, 0), arg_count);
            }
        } else {
            return SajadKha_CallMethodFunctionPosArgsKwArgs(tstate, method->m_function, method->m_object,
                                                          &PyTuple_GET_ITEM(args, 0), arg_count, kw);
        }
    }
}

static PyObject *SajadKha_Method_tp_descr_get(struct SajadKha_MethodObject *method, PyObject *object,
    ...
    return SajadKha_Method_New(method->m_function, object, class_object);
}

static PyObject *SajadKha_Method_tp_getattro(struct SajadKha_MethodObject *method, PyObject *name) {
    PyObject *descr = SajadKha_TypeLookup(&SajadKha_Method_Type, name);

    if (descr != NULL) {
        if (SajadKhaType_HasFeatureClass(Py_TYPE(descr)) && (Py_TYPE(descr)->tp_descr_get != NULL)) {
...
static long SajadKha_Method_tp_traverse(struct SajadKha_MethodObject *method, visitproc visit, void *arg) {
...
static PyObject *SajadKha_Method_tp_repr(struct SajadKha_MethodObject *method) {
    if (method->m_object == NULL) {
#if PYTHON_VERSION < 0x300
        return PyString_FromFormat("<unbound compiled_method %s.%s>", GET_CLASS_NAME(method->m_class),
                                   SajadKha_String_AsString(method->m_function->m_name));
#else
        return PyUnicode_FromFormat("<compiled_function %s at %p>", SajadKha_String_AsString(method->m_function->m_name),
                                    method->m_function);
#endif
    } else {
        ...
        PyObject *result =
            SajadKha_String_FromFormat("<bound compiled_method %s.%s of %s>", GET_CLASS_NAME(method->m_class),
                                     SajadKha_String_AsString_Unchecked(method->m_function->m_name),
                                     SajadKha_String_AsString_Unchecked(object_repr));
#else
        PyObject *result =
...
#if PYTHON_VERSION < 0x300
static int SajadKha_Method_tp_compare(struct SajadKha_MethodObject *a, struct SajadKha_MethodObject *b) {
...
static PyObject *SajadKha_Method_tp_richcompare(struct SajadKha_MethodObject *a, struct SajadKha_MethodObject *b, int op) {
    if (op != Py_EQ && op != Py_NE) {
        Py_INCREF(Py_NotImplemented);
        return Py_NotImplemented;
    }

    if (SajadKha_Method_Check((PyObject *)a) == false || SajadKha_Method_Check((PyObject *)b) == false) {
        Py_INCREF(Py_NotImplemented);
        return Py_NotImplemented;
    }
...
static long SajadKha_Method_tp_hash(struct SajadKha_MethodObject *method) {
...
static struct SajadKha_MethodObject *free_list_methods = NULL;
static int free_list_methods_count = 0;

static void SajadKha_Method_tp_dealloc(struct SajadKha_MethodObject *method) {
...
static PyObject *SajadKha_Method_tp_new(PyTypeObject *type, PyObject *args, PyObject *kw) {
    PyObject *func;
    PyObject *self;
    ...
    if (SajadKha_Method_Check(func)) {
        return SajadKha_Method_New(((struct SajadKha_MethodObject *)func)->m_function, self, class_object);
    }

    if (SajadKha_Function_Check(func) == false) {
        SET_CURRENT_EXCEPTION_TYPE_COMPLAINT_NICE("Cannot create compiled_ method from type '%s'", func);
        return NULL;
    }

    return SajadKha_Method_New((struct SajadKha_FunctionObject *)func, self, class_object);
}

PyTypeObject SajadKha_Method_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_method",
    sizeof(struct SajadKha_MethodObject),
    0,
    (destructor)SajadKha_Method_tp_dealloc, // tp_dealloc
#if PYTHON_VERSION < 0x380 || defined(_NUITKA_EXPERIMENTAL_DISABLE_VECTORCALL_SLOT)
    0, // tp_print
#else
    offsetof(struct SajadKha_MethodObject, m_vectorcall), // tp_vectorcall_offset
#endif
    0, // tp_getattr
    0, // tp_setattr
#if PYTHON_VERSION < 0x300
    (cmpfunc)SajadKha_Method_tp_compare, // tp_compare
#else
    0,
#endif
    (reprfunc)SajadKha_Method_tp_repr,         // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    (hashfunc)SajadKha_Method_tp_hash,         // tp_hash
    (ternaryfunc)SajadKha_Method_tp_call,      // tp_call
    0,                                       // tp_str
    (getattrofunc)SajadKha_Method_tp_getattro, // tp_getattro
    ...
    Py_TPFLAGS_DEFAULT |                     // tp_flags
        ...
        Py_TPFLAGS_HAVE_GC,
    0,                                                // tp_doc
    (traverseproc)SajadKha_Method_tp_traverse,          // tp_traverse
    0,                                                // tp_clear
    (richcmpfunc)SajadKha_Method_tp_richcompare,        // tp_richcompare
    offsetof(struct SajadKha_MethodObject, m_weakrefs), // tp_weaklistoffset
    0,                                                // tp_iter
    0,                                                // tp_iternext
    SajadKha_Method_methods,                            // tp_methods
    SajadKha_Method_members,                            // tp_members
    SajadKha_Method_tp_getset,                          // tp_getset
    0,                                                // tp_base
    0,                                                // tp_dict
    (descrgetfunc)SajadKha_Method_tp_descr_get,         // tp_descr_get
    0,                                                // tp_descr_set
    0,                                                // tp_dictoffset
    0,                                                // tp_init
    0,                                                // tp_alloc
    SajadKha_Method_tp_new,                             // tp_new
    0,                                                // tp_free
    0,                                                // tp_is_gc
    0,                                                // tp_bases
    0,                                                // tp_mro
...
void _initCompiledMethodType(void) {
    SajadKha_PyType_Ready(&SajadKha_Method_Type, &PyMethod_Type, false, true, false, false, false);
}

PyObject *SajadKha_Method_New(struct SajadKha_FunctionObject *function, PyObject *object, PyObject *class_object) {
    struct SajadKha_MethodObject *result;

    CHECK_OBJECT((PyObject *)function);
    assert(SajadKha_Function_Check((PyObject *)function));
    assert(_PyObject_GC_IS_TRACKED(function));

    allocateFromFreeListFixed(free_list_methods, struct SajadKha_MethodObject, SajadKha_Method_Type);

    if (unlikely(result == NULL)) {
        PyErr_Format(PyExc_RuntimeError, "cannot create method %s", SajadKha_String_AsString(function->m_name));
...
#if PYTHON_VERSION >= 0x380 && !defined(_NUITKA_EXPERIMENTAL_DISABLE_VECTORCALL_SLOT)
    result->m_vectorcall = (vectorcallfunc)SajadKha_Method_tp_vectorcall;
#endif
