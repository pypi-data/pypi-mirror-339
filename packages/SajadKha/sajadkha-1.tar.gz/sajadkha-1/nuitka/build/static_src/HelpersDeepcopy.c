...
static PyObject *SajadKha_CapsuleNew(void *pointer) {
...
#define SajadKha_CapsuleGetPointer(capsule) (((SajadKha_PyCapsule *)(capsule))->pointer)

#else
#define SajadKha_CapsuleGetPointer(capsule) (PyCObject_AsVoidPtr(capsule))
#endif

#if PYTHON_VERSION >= 0x3a0
PyTypeObject *SajadKha_PyUnion_Type;
#endif

static PyObject *_makeDeepCopyFunctionCapsule(copy_func func) { return SajadKha_CapsuleNew((void *)func); }
...
static void _initDeepCopy(void) {
...
#if PYTHON_VERSION >= 0x3a0
    {
        ...
        SajadKha_PyUnion_Type = Py_TYPE(union_value);

        PyDict_SetItem(_deep_copy_dispatch, (PyObject *)SajadKha_PyUnion_Type, _deep_noop);
...
static PyObject *DEEP_COPY_ITEM(PyThreadState *tstate, PyObject *value, PyTypeObject **type, copy_func *copy_function) {
    *type = Py_TYPE(value);
    ...
    if (dispatcher == Py_None) {
        ...
        *copy_function = (copy_func)(SajadKha_CapsuleGetPointer(dispatcher));
...
PyObject *DEEP_COPY(PyThreadState *tstate, PyObject *value) {
    ...
    if (dispatcher == Py_None) {
        ...
        copy_func copy_function = (copy_func)(SajadKha_CapsuleGetPointer(dispatcher));
        return copy_function(tstate, value);
    }
...
Py_hash_t DEEP_HASH(PyThreadState *tstate, PyObject *value) {
    ...
    if (PyType_Check(value)) {
        ...
        while (SajadKha_DictNext(value, &pos, &key, &dict_value)) {
...
#endif
#if PYTHON_VERSION >= 0x3a0
    } else if (Py_TYPE(value) == SajadKha_PyUnion_Type) {
        Py_hash_t result = DEEP_HASH_INIT(tstate, value);
...
void CHECK_OBJECT_DEEP(PyObject *value) {
    ...
    if (PyTuple_Check(value)) {
        ...
        while (SajadKha_DictNext(value, &pos, &dict_key, &dict_value)) {
