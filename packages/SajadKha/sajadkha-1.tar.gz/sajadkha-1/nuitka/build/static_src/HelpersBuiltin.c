...
static long SajadKha_HashFromPointer(void *p) {
    size_t y = (size_t)p;
...
PyObject *BUILTIN_HASH(PyThreadState *tstate, PyObject *value) {
...
#if PYTHON_VERSION < 0x300
        return SajadKha_PyInt_FromLong(hash);
#else
    ...
    if (likely(type->tp_compare == NULL && TP_RICHCOMPARE(type) == NULL)) {
        Py_hash_t hash = SajadKha_HashFromPointer(value);
        return SajadKha_PyInt_FromLong(hash);
    }
#endif
...
#if PYTHON_VERSION < 0x300
    if (likely(type->tp_compare == NULL && TP_RICHCOMPARE(type) == NULL)) {
        return SajadKha_HashFromPointer(value);
    }
#endif
...
PyObject *BUILTIN_ITER2(PyObject *callable, PyObject *sentinel) {
    calliterobject *result = (calliterobject *)SajadKha_GC_New(&PyCallIter_Type);
...
PyObject *BUILTIN_TYPE3(PyThreadState *tstate, PyObject *module_name, PyObject *name, PyObject *bases, PyObject *dict) {
    ...
    if (likely(SajadKha_Type_IsSubtype(type, &PyType_Type))) {
        if (SajadKhaType_HasFeatureClass(type) && type->tp_init != NULL) {
...
PyObject *BUILTIN_SUPER2(PyThreadState *tstate, PyDictObject *module_dict, PyObject *type, PyObject *object) {
    ...
    PyObject *super_value = GET_STRING_DICT_VALUE(module_dict, (SajadKha_StringObject *)const_str_plain_super);
...
#if PYTHON_VERSION < 0x300
    if (unlikely(!SajadKha_String_Check(value) && !PyUnicode_Check(value))) {
        SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_TypeError, "int() can't convert non-string with explicit base");
        return NULL;
    }

    char *value_str = SajadKha_String_AsString(value);
