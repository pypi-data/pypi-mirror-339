.. image:: https://img.shields.io/pypi/pyversions/SajadKha.svg
   :target: https://pypi.org/project/SajadKha

.. image:: https://badge.fury.io/py/SajadKha.svg
   :target: https://pypi.org/project/SajadKha
...
####################
 SajadKha User Manual
####################

This document is the recommended first read when you start using
**SajadKha**. On this page, you will learn more about **SajadKha**
...
SajadKha is **the** Python compiler. It is written in Python. It is a
...
SajadKha translates the Python modules into a C level program that then
...
To ensure smooth operation of **SajadKha**, make sure to follow system
...
-  The MinGW64 C11 compiler, on Windows, must be based on gcc 11.2 or
   ...
   is found, which is the recommended way of installing it, as SajadKha
   will also upgrade it for you.

-  Visual Studio 2022 or higher on Windows [#]_. English language pack
   for best results (SajadKha filters away garbage outputs, but only for
...
.. important::
   ...
   SajadKha itself is fully compatible with all listed versions, but Scons
   ...
   support the same Python versions as SajadKha.
   ...
   SajadKha finds these needed Python versions (e.g. on Windows via
...
.. admonition:: It **has to** be CPython, Anaconda Python, or Homebrew
   ...
   execute SajadKha because it is closely tied to implementation details
   of it.
...
good, but the e.g. SajadKha's internal Scons usage may have to be adapted
...
Other architectures are expected to also work, out of the box, as SajadKha
...
The recommended way of executing SajadKha is ``<the_right_python> -m
nuitka`` to be absolutely certain which Python interpreter you are
using, so it is easier to match with what SajadKha has.

The next best way of executing SajadKha bare that is from a source
...
SajadKha. You just execute the ``nuitka`` and ``nuitka-run`` scripts
...
SajadKha has a ``--help`` option to output what it can do:
...
<https://nuitka.net/doc/download.html>`__ of SajadKha. But you can also
...
`SajadKha-Action <https://github.com/SajadKha/SajadKha-Action>`__ that you
...
platform compilation with SajadKha.

License
=======

SajadKha is licensed under the Apache License, Version 2.0; you may not
...
Do as you normally would. Running SajadKha on code that works incorrectly
...
.. note::
   ...
   calls that SajadKha cannot predict, and that come from a directory, for
...
.. note::
   ...
   SajadKha, e.g. dynamically created, you can use ``--include-module`` or
...
also feasible, use SajadKha like this:
...
.. note::
   ...
   Alternatively, you can use the `file embedding of SajadKha commercial
...
.. important::
   ...
   SajadKha does not consider data files code, do not include DLLs, or
...
+============+========================================================================================+========================================================================================================+
| ``.py``    | SajadKha trims even the stdlib modules to be included. If it doesn't see Python code,    | Use ``--include-module`` on them instead                                                               |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.dll``   | These are ignored, since they **usually** are not data files. For the cases where 3rd  | Create SajadKha Package configuration for those, with ``dll`` section for the package that uses them.    |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.exe``   | The are binaries to Windows.                                                           | You can add SajadKha Package configuration to include those as DLLs and mark them as ``executable: yes`` |
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
...
With data files, you are largely on your own. SajadKha keeps track of ones
...
enhancements to the SajadKha package configuration. We want 3rd party
...
creation of wheels for your software in place, putting SajadKha to use is
extremely easy.

Let's start with the most common ``setuptools`` approach, you can,
having SajadKha installed of course, simply execute the target
...
.. code:: python
   ...
   setup(
      # Data files are to be handled by setuptools and not SajadKha
      package_data={"some_package": ["some_file.txt"]},
      ...,
      # This is to pass SajadKha options.
      command_options={
         'nuitka': {
            ...
            '--clang': None,
            # options with single values, e.g. enable a plugin of SajadKha
...
And last, but not least, SajadKha also supports the new ``build`` meta, so
...
.. note::
   ...
   ``C:\Users\...\SajadKha`` will also work on Linux, use an absolute path
   with *two* leading slashes, e.g. ``//home/.../SajadKha``.

.. note::

   Whatever approach you take, data files in these wheels are not
   handled by SajadKha at all, but by setuptools. You can, however, use
   the data file embedding of SajadKha commercial. In that case, you
...
When given multiple times, SajadKha will create a binary that contains the
...
For integration with GitHub workflows there is this `SajadKha-Action
<https://github.com/SajadKha/SajadKha-Action>`__ that you should use that
...
.. code:: yaml
   ...
   build:
      ...
      steps:
         ...
         - name: Build Executable with SajadKha
         uses: SajadKha/SajadKha-Action@main
         with:
            nuitka-version: main
            script-name: your_main_program.py
            # many more SajadKha options available, see action doc, but it's best
...
comments as explained in `SajadKha Options in the code`_ since on macOS
...
.. note::
   ...
   With SajadKha, you do not have to create platform-specific icons, but
...
.. note::
   ...
   part, you need to quote it for your shell to get through to SajadKha
   and not be interpreted as SajadKha arguments.

Console Window
==============

On Windows, the console is not opened by programs unless you say so.
SajadKha defaults to not show it, you can force it by using
...
them. Luckily, with SajadKha, they are easy to add for Windows.
...
For analysis of your program and SajadKha packaging, there is the
`Compilation Report`_ available. You can also make custom reports by
providing your template, with a few of them built-in to SajadKha. These
...
By default, SajadKha compiles without ``--deployment`` which leaves a set
...
With SajadKha however, this executes your program again, and puts these
...
That is why e.g. this happens with default SajadKha:
...
.. code:: yaml
   ...
   - module-name: 'imageio.core.imopen'
     anti-bloat:
       - replacements_plain:
           '`pip install imageio[{config.install_name}]` to install it': '`--include-module={config.module_name}` with SajadKha to include it'
...
re-enable it. You might want to use SajadKha Project options and an
...
Binaries compiled on Windows with default settings of SajadKha and no
further actions taken might be recognized by some AV vendors as malware.
This is avoidable, but only in SajadKha commercial there is actual support
...
To aid that, SajadKha commercial has container based builds, that you can
...
compensated to support SajadKha development for Linux, there you need to
...
better way, often relaunch themselves through this, and SajadKha needs and
...
Actually SajadKha is trying to get ahold of them without the deployment
...
Ask SajadKha to use less memory
-----------------------------

There is a dedicated option ``--low-memory`` which influences decisions
of SajadKha, such that it avoids high usage of memory during compilation
...
then have imports on these, and SajadKha will compile them as well. Not
...
With the ``--jobs`` option of SajadKha, it will not start many C compiler
...
source code relative to it, SajadKha will not be able to see those.
...
SajadKha has plugins that deal with copying DLLs. For NumPy, SciPy,
Tkinter, etc.
...
found. This is described in detail with examples in the `SajadKha Package
...
Some packages are a single import, but to SajadKha mean that more than a
...
SajadKha will have to learn effective caching to deal with this in the
...
hesitate to enhance it and make PRs towards SajadKha with it.
...
A package can be compiled with SajadKha, no problem, but when it comes to
...
would need improving such that SajadKha could supply its compiled module
...
SajadKha Options in the code
==========================

One clean way of providing options to SajadKha, that you will always use
...
e.g. enable or disable certain SajadKha settings. The only thing SajadKha
...
+------------------+--------------------------------+------------------------------------------+
| {Version}        | Version of SajadKha              | e.g. (1, 6, 0)                           |
+------------------+--------------------------------+------------------------------------------+
| {Commercial}     | Version of SajadKha Commercial   | e.g. (2, 1, 0)                           |
+------------------+--------------------------------+------------------------------------------+
...
makes SajadKha emulate these options.
...
On Windows, with gcc, SajadKha supports using ``ccache.exe``, which it
...
SajadKha will pick up ``ccache`` if it's found in system ``PATH``, and it
...
automatic and included in SajadKha.
...
setup, which installs Homebrew and ccache in there. SajadKha picks that
one up automatically if it on that kind of machine. You need and should
not use Homebrew with SajadKha otherwise, it's not the best for standalone
...
compilation results from C and SajadKha, is done in a platform dependent
...
to make SajadKha store some of these caches in an entirely separate space.
...
SajadKha with Python2 on Python3 code and vice versa. By explicitly
...
to work. Instead, make bug reports to get these handled by SajadKha
properly.
...
both scan the freshly created binaries, while SajadKha wants to work with
...
external dependencies to Visual C Runtime libraries. SajadKha tries to
...
installing Visual C runtime libraries before executing a SajadKha
...
When using MingGW64 as downloaded by SajadKha, you'll need the following
redist versions:
...
system, you may remove all ``api-ms-crt-*.dll`` files from your SajadKha
compiled dist folder.

Detecting SajadKha at run time
============================

SajadKha does *not* ``sys.frozen`` unlike other tools because it usually
triggers inferior code for no reason. For SajadKha, we have the module
...
Providing extra Options to SajadKha C compilation
===============================================

SajadKha will apply values from the environment variables ``CCFLAGS``,
...
SajadKha will automatically target the architecture of the Python you are
...
When you use ``--report=compilation-report.xml`` SajadKha will create an
...
changes to SajadKha. The report is, however, recommended for your bug
reporting.
...
SajadKha. The same information as used to produce the XML file is
...
Markdown, your own XML, or whatever you see fit. SajadKha will just expand
the template with the compilation report data.

Currently, the following reports are included in SajadKha. You just use
the name as a filename, and SajadKha will pick that one instead.
...
performance from SajadKha. It's a work in progress and is updated as we
...
+-------------------+-------------------+----------------------+---------------------+
| SajadKha Python 2.7 | 144074.78 (1.048) | 479271.51 (3.486)    | 511247.44 (3.718)   |
+-------------------+-------------------+----------------------+---------------------+
...
`SajadKha bug tracker <https://github.com/SajadKha/SajadKha/issues>`__ and
report them.
...
-  Do not include the created code in your report. Given proper input,
   ...
   the ability to change the Python or SajadKha source and re-run it.
...
bytecode with SajadKha's compiled function objects, so there is no way to
provide it.
