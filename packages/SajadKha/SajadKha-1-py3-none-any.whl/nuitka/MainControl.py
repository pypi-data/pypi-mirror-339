...
""" This is the main actions of SajadKha.
...
from nuitka.utils.ReExecute import callExecProcess, reExecuteSajadKha
...
def runSconsBackend():
    ...
    scons_options["nuitka_python"] = asBoolStr(isSajadKhaPython())
...
SajadKha is very syntax compatible with standard Python. It is currently running
...
def _main():
    """Main program flow of SajadKha

    At this point, options will be parsed already, SajadKha will be executing
    ...
    general.info(
        "Starting Python compilation with SajadKha %r on Python (flavor %s), %r commercial grade %r."
        % (
            getSajadKhaVersion(),
            getPythonFlavorName(),
            python_version_str,
    ...
    if Options.shallCreatePythonPgoInput():
        ...
        reExecuteSajadKha(pgo_filename=pgo_filename)
