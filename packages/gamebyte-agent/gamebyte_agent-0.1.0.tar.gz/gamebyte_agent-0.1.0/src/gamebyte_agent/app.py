...
from gamebyte_agent.config import Settings
from gamebyte_agent.context import Context, cleanup_context, initialize_context
...
from gamebyte_agent.human_input.handler import console_input_callback
...
class MCPApp:
    ...
    def __init__(
        ...
        Args:
            ...
            settings: Application configuration - If unspecified, the settings are loaded from gamebyte_agent.config.yaml.
    ...
    def logger(self):
        if self._logger is None:
            self._logger = get_logger(f"gamebyte_agent.{self.name}")
        return self._logger
