        ...
        for s in sequences:
            ...
            for p in s.passes:
                ...
                for layer in p.layers:
                    ...
                    if has_program:
                        if layer_type in hrc_layers or nps is None:
                            row.append('N/A')
                        else:
                            row.append(len(nps))
                    table.append(row)
                    ...
                    if layer_type == LayerType.SeparableConvolutional:
                        ...
                        table.append(row)
                        new_splits.append(False)
                    elif layer_type == LayerType.VitEncoderBlock:
                        # Add details on constitutive layers of the block
                        sub_layers = ['norm_mha', 'query', 'key', 'value', 'attention',
                                      'attention_projection', 'skip_connection_1', 'norm_mlp',
                                      'mlp_1', 'mlp_2', 'skip_connection_2']
                        if layer.parameters.tokens_to_extract:
                            sub_layers += ['batch_norm', 'extract_token']
                            if layer.parameters.num_classes:
                                sub_layers += ['head']
                                if layer.parameters.tokens_to_extract == 2:
                                    sub_layers += ['extract_token', 'dist_head', 'add_heads']
                        for sub_layer in sub_layers:
                            if sub_layer + '_weights' in layer.get_variable_names():
                                sub_layer_kernel = layer.get_variable(sub_layer + "_weights").shape
                            else:
                                sub_layer_kernel = 'N/A'
                            if has_program:
                                row.append('')
                            row = ["  " + sub_layer, '', sub_layer_kernel]
                            table.append(row)
                            new_splits.append(-1)
