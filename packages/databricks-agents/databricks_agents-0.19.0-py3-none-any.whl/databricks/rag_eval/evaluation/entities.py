...
class EvalResult:
    ...
    def assessments(self) -> List[mlflow_entities.Assessment]:
        ...
        overall_assessment = (
            [
                PerRequestAssessmentResult(
                    assessment_name=schemas.OVERALL_ASSESSMENT,
                    assessment_type=assessment_config.AssessmentType.ANSWER,
                    assessment_source=AssessmentSource.builtin(),
                    rating=self.overall_assessment,
                ).to_mlflow_assessment_v3(trace_id=trace_id, span_id=span_id)
            ]
