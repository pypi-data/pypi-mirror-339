...
class AssessmentsSuiteConfig:
    ...
    def get_metrics(self) -> list[str] | None:
        """Get a list of metric names from the assessments, or None if no assessments are defined."""
        if self.assessments is None:
            return None
        metrics = []
        for assessment in self.assessments:
            if isinstance(assessment, BuiltinJudge):
                ...
                metrics.append(AI_JUDGE_GUIDELINE_ADHERENCE)
        return metrics
