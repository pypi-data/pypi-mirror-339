...
def create_video_clip_list(ws_config: WebServiceConfig, device_name: str, create_video_clips: List[CreateVideoClip]) -> tuple[int, str]:

    try:
        # Clean and encode device_name
        device_name = device_name.replace(" ", "")
        device_name = urllib.parse.quote(device_name)

        method = "POST"
        path = 'v1/videoclip/createlist/' + device_name
        url = ws_config.base_url + path
        headers = {"Content-Type": "application/json"}
        parameters = ''
        
        # Convert the list of CreateVideoClip objects to a list of dictionaries for JSON serialization
        clips_data = [clip.__dict__ for clip in create_video_clips]
