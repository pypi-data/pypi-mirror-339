...
@pytest.mark.asyncio
async def test_get_models(registry_client: AsyncClient) -> None:
    """Test retrieving all models."""
    response = await registry_client.get(MODELS_URL)
...
@pytest.mark.asyncio
async def test_get_model_by_id(registry_client: AsyncClient) -> None:
    ...
    response = await registry_client.get(f"{MODELS_URL}/{test_model['id']}")
    assert response.status_code == 200
    ...
    response = await registry_client.get(f"{MODELS_URL}/non-existent-model")
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_model_schema(registry_client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_models_api_works_without_env_file(registry_client: AsyncClient) -> None:
    ...
    with mock.patch.object(Path, "exists") as mock_exists:
        ...
        response = await registry_client.get(f"{MODELS_URL}/{model_id}")
        assert response.status_code == 200
