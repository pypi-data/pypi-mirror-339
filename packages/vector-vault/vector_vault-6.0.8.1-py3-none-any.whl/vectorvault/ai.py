...
class LLMClient:
    def __init__(
        ...
        personality_message: str = None, 
        main_prompt: str = None, 
        main_prompt_with_context: str = None, 
        verbose: bool = False, 
        timeout: int = 300, 
        fine_tuned_context_window=128000
    ):
        ...
        # Insert history into your default prompts
        self.main_prompt = (
            main_prompt if main_prompt 
            else "Question: {content}\n\nHistory: {history}"
        )
        self.main_prompt_with_context = (
            main_prompt_with_context if main_prompt_with_context else 
...
"""
        ...
        # We add personality at the bottom of each prompt
        self.context_prompt = (
    ...
    def within_context_window(self, text: str = None, model=None):
        if model not in self.platform.model_token_limits.keys():
            # If no explicit limit for the model, fall back to self.fine_tuned_context_window
    ...
    def truncate_text(self, text, history='', prompt='', context='', max_tokens=128000):
        ...
        max_tokens -> limit
        """
        def cut(str_to_cut, tokens_to_remove):
            # a rough approach to dropping characters
            return str_to_cut[(tokens_to_remove * 4):]

        def get_tokes(t, h, p, c):
            t_tokens = self.platform.get_tokens(t) if t else 0
            h_tokens = self.platform.get_tokens(h) if h else 0
            p_tokens = self.platform.get_tokens(p) if p else 0
            c_tokens = self.platform.get_tokens(c) if c else 0
            return t_tokens + h_tokens + p_tokens + c_tokens

        total_tokens = get_tokes(text, history, prompt, context)
        if total_tokens > max_tokens:
            excess = total_tokens - max_tokens
            # Try cutting history first (most expendable)
            if len(history) > excess * 4:
                history = cut(history, excess)

        total_tokens = get_tokes(text, history, prompt, context)
        if total_tokens > max_tokens:
            excess = total_tokens - max_tokens
            # Then cut context if still too large
            if len(context) > excess * 4:
                context = cut(context, excess)

        total_tokens = get_tokes(text, history, prompt, context)
        if total_tokens > max_tokens:
            excess = total_tokens - max_tokens
            # Finally cut user input (text)
            if len(text) > excess * 4:
                text = cut(text, excess)
    ...
    def llm(self, user_input: str = '', history: str = '', model=None, custom_prompt=False, temperature=0, timeout=None, max_retries=5):
        """
        Basic call without separate context param. 
        History is now inserted into the same prompt as user_input.
        """
        ...
        # We'll combine user_input + history + prompt_template for token counting
        token_count = self.platform.get_tokens(str(user_input) + str(history) + str(prompt_template))
        model = self.platform.model_check(token_count, model)
        max_tokens = (
            self.platform.model_token_limits.get(model) 
            if model in self.platform.model_token_limits.keys() 
            else self.fine_tuned_context_window
        )

        if user_input:
            # Truncate if needed
            new_texts = self.truncate_text(
                text=user_input,
                history=str(history),
            ...
            # Build final prompt, injecting history
        ...
        # Single user message (history inlined above)
    ...
    def llm_instruct(self, content: str, instructions: str, system_message="You are an AI assistant that excels at following instructions exactly.", model=None, temperature=0):
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": f"""Follow these instructions to properly process the following content:
Content: {content}
Instructions: {instructions}"""
    ...
    def llm_w_context(self, user_input='', context='', history='', model=None, custom_prompt=False, temperature=0, timeout=None, max_retries=5):
        """
        Combines user_input + history + context in the same final prompt.
        """
        timeout = self.timeout if not timeout else timeout
        prompt_template = custom_prompt if custom_prompt else self.context_prompt

        model = model if model else self.default_model
        token_count = self.platform.get_tokens(
            str(user_input) + str(history) + str(context) + str(prompt_template)
        )
        model = self.platform.model_check(token_count, model)
        max_tokens = (
            self.platform.model_token_limits.get(model)
        ...
        prompt = prompt_template.format(context=context, content=user_input, history=history)

        if self.verbose:
            print('Full input prompt:', prompt)

        # Single user message with everything included
        messages = [{"role": "user", "content": prompt}]
    ...
    def llm_w_context_stream(self, user_input='', context='', history='', model=None, custom_prompt=False, temperature=0):
        """
        Streaming version of llm_w_context.
        """
        prompt_template = custom_prompt if custom_prompt else self.context_prompt
        model = model if model else self.default_model
        token_count = self.platform.get_tokens(
    ...
    def smart_summary(self, text, previous_summary, model=None, custom_prompt=False, temperature=0):
        prompt_template = (
            custom_prompt if custom_prompt else 
            """Given the previous summary: {previous_summary}
