...
class _EmissionTimeIOC(PVGroup):
    '''
    ...
    async def _get_harp_pvs(self, hpref, ch):
        ...
        return {k:p for k,p in zip(pv_def.keys(),await cli.get_pvs(*[i[1]for i in  pv_def.items()])) }
    ...
    async def DIODE_T0(self, inst, async_lib):
        ...
        pv = await self._get_harp_pvs(self.harp_prefix, self.harp_channel)
