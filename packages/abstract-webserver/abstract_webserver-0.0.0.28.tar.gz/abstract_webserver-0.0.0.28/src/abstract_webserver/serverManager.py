...
def find_spec_dir(substr='src', directory=None):
    ...
    for path in base_dir.rglob('*'):
        if is_valid_path(path) and substr in path.name:
            return path
    return None
def find_src_dir(directory=None):
    src_dir = find_spec_dir(substr="src", directory=directory)
    return src_dir

def get_main_dir(directory=None):
    src_dir = find_src_dir(directory=directory)
    main_dir = os.path.dirname(src_dir)
    return main_dir

def get_public_dir(directory=None):
    main_dir = get_main_dir(directory=directory)
    public_dir = find_spec_dir(substr="public", directory=main_dir)
    return public_dir
...
def make_directory_links(directory,domain=None,directory_links=None):
    ...
    for item in os.listdir(directory):
        ...
        if os.path.isdir(dir_path):
            link = os.path.join(directory,item)
            directory_links[item]={"directory":dir_path,"link":link}
    return directory_links

def create_directory_links(public_dir=None,pages_dir=None,json_dir=None,domain=None,main_dir=None,src_dir=None,contents_dir=None,directory_links=None):
    ...
    domain = get_domain_dirs(get_domain,dir_str=domain,directory=public_dir)
    directory_links= directory_links or {}
    directory_links["pages"] = {"directory":pages_dir,"link":domain}
    directory_links["json"] = {"directory":json_dir,"link":domain}
    directory_links["contents"] = {"directory":json_dir,"link":domain}
    directory_links = make_directory_links(public_dir,domain=domain=directory_links)
    return directory_links

class serverManager(metaclass=SingletonMeta):
    def __init__(self, src_dir=None,main_dir=None,domain=None,imgs_dir=None,pages_dir=None,json_dir=None,contents_dir=None,fb_id=None,directory=None,page_layout_dir=None):
        ...
        self.public_dir = get_domain_dirs(get_public_dir,dir_str=public_dir,directory=self.main_dir)
        self.imgs_dir = get_domain_dirs(find_imgs_dir,dir_str=imgs_dir,directory=self.public_dir)
        ...
        self.page_layout_dir = get_domain_dirs(get_page_layout_dir,dir_str=page_layout_dir,directory=self.pages_dir)
        self.domain = get_domain_dirs(get_domain,dir_str=domain,directory=self.src_dir)
        self.fb_id = fb_id
