...
import asyncio
import sys
from unittest.mock import AsyncMock, MagicMock, patch, Mock, create_autospec
...
async def test_run_demo_single_server(mock_llm_backend, mock_config_file):
    ...
    with patch('sys.argv', ['script.py', '--config', mock_config_file]), \
...
@pytest.mark.asyncio
async def test_run_demo_all_servers_connection_failure(mock_llm_backend, mock_config_file):
...
async def test_run_demo_missing_config(mock_llm_backend, tmp_path):
    ...
    with patch('sys.argv', ['script.py', '--config', nonexistent_config]), \
