...
class ModelSettings(BaseSettings):
    ...
    split_mode: int = Field(
        default=llama_core.LLAMA_SPLIT_LAYER,
    ...
    use_mmap: bool = Field(
        default=llama_core.llama_mmap_supported(),
        description="Use mmap.",
    )
    use_mlock: bool = Field(
        default=llama_core.llama_mlock_supported(),
    ...
    # Context Params
    seed: int = Field(
        default=llama_core.LLAMA_DEFAULT_SEED, description="Random seed. -1 for random."
    )
    ...
    rope_scaling_type: int = Field(default=llama_core.LLAMA_ROPE_SCALING_UNSPECIFIED)
