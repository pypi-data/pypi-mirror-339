...
import llama_core.llama_cpp as llama_core
...
class LlamaModel:
    ...
    def __init__(
        ...
        *,
        path_model: str,
        params: llama_core.llama_model_params,
        verbose: bool = True,
    ):
        ...
        with suppress_stdout_stderr(disable=verbose):
            model = llama_core.llama_load_model_from_file(
        ...
        vocab = llama_core.llama_model_get_vocab(model)
        ...
        def free_model():
            if self.model is None:
                return
            llama_core.llama_free_model(self.model)
            self.model = None
    ...
    def vocab_type(self) -> int:
        return llama_core.llama_vocab_type(self.model)

    def n_vocab(self) -> int:
        return llama_core.llama_n_vocab(self.vocab)

    def n_ctx_train(self) -> int:
        return llama_core.llama_n_ctx_train(self.model)

    def n_embd(self) -> int:
        return llama_core.llama_n_embd(self.model)

    def rope_freq_scale_train(self) -> float:
        return llama_core.llama_model_rope_freq_scale_train(self.model)

    def desc(self) -> str:
        buf = ctypes.create_string_buffer(1024)
        llama_core.llama_model_desc(self.model, buf, 1024)
        return buf.value.decode("utf-8")

    def size(self) -> int:
        return llama_core.llama_model_size(self.model)

    def n_params(self) -> int:
        return llama_core.llama_model_n_params(self.model)
    ...
    def token_get_text(self, token: int) -> str:
        return llama_core.llama_token_get_text(self.vocab, token).decode("utf-8")

    def token_get_score(self, token: int) -> float:
        return llama_core.llama_token_get_score(self.vocab, token)

    def token_get_attr(self, token: int) -> int:
        return llama_core.llama_token_get_attr(self.vocab, token)

    # Special tokens

    def token_bos(self) -> int:
        return llama_core.llama_token_bos(self.vocab)

    def token_eos(self) -> int:
        return llama_core.llama_token_eos(self.vocab)

    def token_cls(self) -> int:
        return llama_core.llama_token_cls(self.vocab)

    def token_sep(self) -> int:
        return llama_core.llama_token_sep(self.vocab)

    def token_nl(self) -> int:
        return llama_core.llama_token_nl(self.vocab)
    ...
    def token_eot(self) -> int:
        return llama_core.llama_token_eot(self.vocab)

    def add_bos_token(self) -> bool:
        return llama_core.llama_add_bos_token(self.vocab)

    def add_eos_token(self) -> bool:
        return llama_core.llama_add_eos_token(self.vocab)

    # Tokenization

    def tokenize(self, text: bytes, add_bos: bool, special: bool):
        n_ctx = self.n_ctx_train()
        tokens = (llama_core.llama_token * n_ctx)()
        n_tokens = llama_core.llama_tokenize(
            self.vocab, text, len(text), tokens, n_ctx, add_bos, special
        )
        if n_tokens < 0:
            n_tokens = abs(n_tokens)
            tokens = (llama_core.llama_token * n_tokens)()
            n_tokens = llama_core.llama_tokenize(
    ...
    def token_to_piece(self, token: int, special: bool = False) -> bytes:
        ...
        llama_core.llama_token_to_piece(self.vocab, token, buf, 32, 0, special)
        return bytes(buf)
    ...
    def detokenize(self, tokens: List[int], special: bool = False) -> bytes:
        ...
        for token in tokens:
            n = llama_core.llama_token_to_piece(
                self.vocab, llama_core.llama_token(token), buffer, size, 0, special
            )
            assert n <= size
    ...
    def metadata(self) -> Dict[str, str]:
        ...
        for i in range(llama_core.llama_model_meta_count(self.model)):
            nbytes = llama_core.llama_model_meta_key_by_index(
            ...
            nbytes = llama_core.llama_model_meta_val_str_by_index(
                self.model, i, buffer, buffer_size
            )
            if nbytes > buffer_size:
                buffer_size = nbytes + 1
    ...
    def default_params():
        ...
        return llama_core.llama_model_default_params()
...
class LlamaContext:
    ...
    def __init__(
        ...
        *,
        model: LlamaModel,
        params: llama_core.llama_context_params,
        verbose: bool = True,
    ):
        self.model = model
        self.params = params
        ...
        ctx = llama_core.llama_new_context_with_model(self.model.model, self.params)
        ...
        def free_ctx():
            if self.ctx is None:
                return
            llama_core.llama_free(self.ctx)
            self.ctx = None
    ...
    def n_ctx(self) -> int:
        return llama_core.llama_n_ctx(self.ctx)

    def pooling_type(self) -> int:
        return llama_core.llama_pooling_type(self.ctx)

    def kv_cache_clear(self):
        llama_core.llama_kv_cache_clear(self.ctx)

    def kv_cache_seq_rm(self, seq_id: int, p0: int, p1: int):
        llama_core.llama_kv_cache_seq_rm(self.ctx, seq_id, p0, p1)

    def kv_cache_seq_cp(self, seq_id_src: int, seq_id_dst: int, p0: int, p1: int):
        llama_core.llama_kv_cache_seq_cp(self.ctx, seq_id_src, seq_id_dst, p0, p1)

    def kv_cache_seq_keep(self, seq_id: int):
        llama_core.llama_kv_cache_seq_keep(self.ctx, seq_id)

    def kv_cache_seq_shift(self, seq_id: int, p0: int, p1: int, shift: int):
        llama_core.llama_kv_cache_seq_add(self.ctx, seq_id, p0, p1, shift)

    def get_state_size(self) -> int:
        return llama_core.llama_get_state_size(self.ctx)
    ...
    def decode(self, batch: LlamaBatch):
        return_code = llama_core.llama_decode(
            self.ctx,
            batch.batch,
        )
        if return_code != 0:
    ...
    def set_n_threads(self, n_threads: int, n_threads_batch: int):
        llama_core.llama_set_n_threads(self.ctx, n_threads, n_threads_batch)

    def get_logits(self):
        return llama_core.llama_get_logits(self.ctx)

    def get_logits_ith(self, i: int):
        return llama_core.llama_get_logits_ith(self.ctx, i)

    def get_embeddings(self):
        return llama_core.llama_get_embeddings(self.ctx)
    ...
    def sample_repetition_penalties(
        ...
        last_tokens_data: "llama_core.Array[llama_core.llama_token]",
        penalty_last_n: int,
    ...
    def sample_token_mirostat(
        ...
        eta: float,
        m: int,
        mu: llama_core.CtypesPointerOrRef[ctypes.c_float],
    ) -> int:
    ...
    def reset_timings(self):
        llama_core.llama_perf_context_reset(self.ctx)

    def print_timings(self):
        llama_core.llama_perf_context_print(self.ctx)

    # Utility functions
    @staticmethod
    def default_params():
        """Get the default llama_context_params."""
        return llama_core.llama_context_default_params()
...
class LlamaBatch:
    def __init__(
        ...
        batch = llama_core.llama_batch_init(self._n_tokens, self.embd, self.n_seq_max)
        ...
        def free_batch():
            if self.batch is None:
                return
            llama_core.llama_batch_free(self.batch)
            self.batch = None
...
class LlamaTokenDataArray:
    def __init__(self, *, n_vocab: int):
        ...
        self.candidates = llama_core.llama_token_data_array(
            data=self.candidates_data.ctypes.data_as(llama_core.llama_token_data_p),
            size=self.n_vocab,
            sorted=False,
        )
...
class LlamaSamplingContext:
    ...
    cur: list[llama_core.llama_token_data] = field(default_factory=list)
    ...
    def sample(
        ...
        if len(self.prev) > 0:
            ...
            if last_tokens_size > 0:
                last_tokens_p = (llama_core.llama_token * len(last_tokens))(*last_tokens)
...
class CustomSampler:
    def __init__(
        self, apply_func: typing.Callable[[llama_core.llama_token_data_array], None]
    ):
        self.apply_func = apply_func

        def apply_wrapper(
            sampler: llama_core.llama_sampler_p,
            cur_p: llama_core.llama_token_data_array_p,
        ):
            self.apply_func(cur_p)

        def free_wrapper(sampler: llama_core.llama_sampler_p):
            pass

        sampler_i = llama_core.llama_sampler_i()
        sampler_i.apply = llama_core.llama_sampler_i_apply(apply_wrapper)
        self._apply_wrapper_ref = apply_wrapper

        sampler_i.name = llama_core.llama_sampler_i_name(0)
        sampler_i.accept = llama_core.llama_sampler_i_accept(0)
        sampler_i.reset = llama_core.llama_sampler_i_reset(0)
        sampler_i.clone = llama_core.llama_sampler_i_clone(0)
        sampler_i.free = llama_core.llama_sampler_i_free(0)

        self.sampler = llama_core.llama_sampler()
        self.sampler.iface = ctypes.pointer(sampler_i)
        self.sampler.ctx = None

    def get_sampler(self) -> llama_core.llama_sampler_p:
        return ctypes.pointer(self.sampler)


class LlamaSampler:
    def __init__(self):
        params = llama_core.llama_sampler_chain_params()
        self.sampler = llama_core.llama_sampler_chain_init(params)
        self.samplers: List[llama_core.llama_sampler_p] = []
        self.custom_samplers: List[Tuple[int, CustomSampler]] = []

    def add_greedy(self):
        sampler = llama_core.llama_sampler_init_greedy()
        self._add_sampler(sampler)

    def add_dist(self, seed: int):
        sampler = llama_core.llama_sampler_init_dist(seed)
        self._add_sampler(sampler)

    def add_softmax(self):
        sampler = llama_core.llama_sampler_init_softmax()
        self._add_sampler(sampler)

    def add_top_k(self, k: int):
        sampler = llama_core.llama_sampler_init_top_k(k)
        self._add_sampler(sampler)

    def add_top_p(self, p: float, min_keep: int):
        sampler = llama_core.llama_sampler_init_top_p(p, min_keep)
        self._add_sampler(sampler)

    def add_min_p(self, p: float, min_keep: int):
        sampler = llama_core.llama_sampler_init_min_p(p, min_keep)
        self._add_sampler(sampler)

    def add_typical(self, p: float, min_keep: int):
        sampler = llama_core.llama_sampler_init_typical(p, min_keep)
        self._add_sampler(sampler)

    def add_temp(self, temp: float):
        sampler = llama_core.llama_sampler_init_temp(temp)
        self._add_sampler(sampler)

    def add_temp_ext(self, t: float, delta: float, exponent: float):
        sampler = llama_core.llama_sampler_init_temp_ext(t, delta, exponent)
        self._add_sampler(sampler)

    def add_mirostat(self, n_vocab: int, seed: int, tau: float, eta: float, m: int):
        sampler = llama_core.llama_sampler_init_mirostat(n_vocab, seed, tau, eta, m)
        self._add_sampler(sampler)

    def add_mirostat_v2(self, seed: int, tau: float, eta: float):
        sampler = llama_core.llama_sampler_init_mirostat_v2(seed, tau, eta)
        self._add_sampler(sampler)

    def add_grammar(self, model: LlamaModel, grammar: LlamaGrammar):
        sampler = llama_core.llama_sampler_init_grammar(
    ...
    def add_penalties(
        ...
        sampler = llama_core.llama_sampler_init_penalties(
            penalty_last_n,
            penalty_repeat,
            penalty_freq,
            penalty_present,
    ...
    def init_logit_bias(
        self, n_vocab: int, n_logit_bias, logit_bias: llama_core.llama_logit_bias_p
    ):
        sampler = llama_core.llama_sampler_init_logit_bias(
            n_vocab, n_logit_bias, logit_bias
        )
        self._add_sampler(sampler)

    def add_custom(
        self, apply_func: Callable[[llama_core.llama_token_data_array], None]
    ):
        ...
        self.custom_samplers.append(
            (llama_core.llama_sampler_chain_n(self.sampler) - 1, custom_sampler)
        )

    def _add_sampler(self, sampler: llama_core.llama_sampler_p):
        assert self.sampler is not None
        llama_core.llama_sampler_chain_add(self.sampler, sampler)
        self.samplers.append(sampler)

    def get_seed(self) -> int:
        assert self.sampler is not None
        return llama_core.llama_sampler_get_seed(self.sampler)

    def sample(self, ctx: LlamaContext, idx: int) -> int:
        assert self.sampler is not None
        assert ctx.ctx is not None
        return llama_core.llama_sampler_sample(self.sampler, ctx.ctx, idx)

    def close(self):
        if self.sampler:
            # NOTE: Must remove custom samplers before free or llama.cpp will try to free them
            for i, _ in reversed(self.custom_samplers):
                llama_core.llama_sampler_chain_remove(self.sampler, i)
            llama_core.llama_sampler_free(self.sampler)
            self.sampler = None
        self.samplers.clear()
