...
class ScreenArea(QObject):
    ...
    def physicalRectF(self, rectf):
        ...
        return QRectF(
            rectf.x() * self._pixelRatio,
            rectf.y() * self._pixelRatio,
            rectf.width() * self._pixelRatio,
            rectf.height() * self._pixelRatio,
        )
    ...
    def logicalRectF(self, physicalRectF):
        ...
        return QRectF(
            physicalRectF.x() / self._pixelRatio,
            physicalRectF.y() / self._pixelRatio,
            physicalRectF.width() / self._pixelRatio,
            physicalRectF.height() / self._pixelRatio,
        )
    ...
    def screenLogicalSizeF(self):
        return QSizeF(
            self._screenPixmap.width() / self._pixelRatio,
            self._screenPixmap.height() / self._pixelRatio,
        )
    ...
    def remakeNightArea(self):
        ...
        self._pt_centerTopMid = (
            self._rt_center.topLeft() + self._rt_center.topRight()
        ) / 2
        self._pt_centerBottomMid = (
            self._rt_center.bottomLeft() + self._rt_center.bottomRight()
        ) / 2
        self._pt_centerLeftMid = (
            self._rt_center.topLeft() + self._rt_center.bottomLeft()
        ) / 2
        self._pt_centerRightMid = (
            self._rt_center.topRight() + self._rt_center.bottomRight()
        ) / 2
        ...
        self._square_bottomRight = self.squareAreaByCenter(
            self._rt_center.bottomRight()
        )

        # 挪到四侧边缘操作检测区域，而不是采用边缘中心点的方式
        self._square_topMid = self.squareAreaByHorizontal(
            self._pt_centerTopMid, self._rt_center.width()
        )
        self._square_bottomMid = self.squareAreaByHorizontal(
            self._pt_centerBottomMid, self._rt_center.width()
        )
        self._square_leftMid = self.squareAreaByVertical(
            self._pt_centerLeftMid, self._rt_center.height()
        )
        self._square_rightMid = self.squareAreaByVertical(
            self._pt_centerRightMid, self._rt_center.height()
        )

        # 除中央截图区域外的8个区域
        self._rt_topLeft = QRectF(self._rt_screen.topLeft(), self._rt_center.topLeft())
        self._rt_top = QRectF(
            QPointF(self._rt_center.topLeft().x(), 0), self._rt_center.topRight()
        )
        self._rt_topRight = QRectF(
            QPointF(self._rt_center.topRight().x(), 0),
            QPointF(self._rt_screen.width(), self._rt_center.topRight().y()),
        )
        self._rt_left = QRectF(
            QPointF(0, self._rt_center.topLeft().y()), self._rt_center.bottomLeft()
        )
        self._rt_right = QRectF(
            self._rt_center.topRight(),
            QPointF(self._rt_screen.width(), self._rt_center.bottomRight().y()),
        )
        self._rt_bottomLeft = QRectF(
            QPointF(0, self._rt_center.bottomLeft().y()),
            QPointF(self._rt_center.bottomLeft().x(), self._rt_screen.height()),
        )
        self._rt_bottom = QRectF(
            self._rt_center.bottomLeft(),
            QPointF(self._rt_center.bottomRight().x(), self._rt_screen.height()),
        )
        self._rt_bottomRight = QRectF(
            self._rt_center.bottomRight(), self._rt_screen.bottomRight()
        )
    ...
    def aroundAreaIn4Direction(self):
        ...
        return [
            QRectF(0, 0, screenSizeF.width(), pt_topRight.y()),
            QRectF(
                0,
                pt_bottomLeft.y(),
                screenSizeF.width(),
                screenSizeF.height() - pt_bottomLeft.y(),
            ),
            QRectF(0, 0, pt_bottomLeft.x(), screenSizeF.height()),
            QRectF(
                pt_topRight.x(),
                0,
                screenSizeF.width() - pt_topRight.x(),
                screenSizeF.height(),
            ),
        ]
    ...
    def moveCenterAreaTo(self, pointf):
        ...
        if endPointF.y() > screenSizeF.height():
            self._rt_center.moveBottomRight(
                QPointF(endPointF.x(), screenSizeF.height())
            )
    ...
    def getMouseShapeBy(self, pointf):
        """根据鼠标位置返回对应的鼠标样式"""
        if self._square_topLeft.contains(pointf) or self._square_bottomRight.contains(
            pointf
        ):
            return Qt.CursorShape.SizeFDiagCursor  # ↖↘
        elif self._square_topMid.contains(pointf) or self._square_bottomMid.contains(
            pointf
        ):
            return Qt.CursorShape.SizeVerCursor  # ↑↓
        elif self._square_topRight.contains(pointf) or self._square_bottomLeft.contains(
            pointf
        ):
            return Qt.CursorShape.SizeBDiagCursor  # ↙↗
        elif self._square_leftMid.contains(pointf) or self._square_rightMid.contains(
            pointf
        ):
    ...
    def paintMagnifyingGlassPixmap(self, pos, glassSize):
        ...
        glassPixmap = glassPixmap.scaled(
            glassSize, glassSize, Qt.AspectRatioMode.KeepAspectRatio
        )
...
class ScreenShotWindow(QWidget):
    ...
    def initPainterTool(self):
        ...
        self.color_lightBlue = QColor(
            30, 120, 255, 255
        )  # 浅蓝色。深蓝色Qt.GlobalColor.blue
        self.color_lightWhite = QColor(
            255, 255, 255, 255
        )  # 浅蓝色。深蓝色Qt.GlobalColor.blue
        self.font_normal = QtGui.QFont("Times New Roman", 11, QtGui.QFont.Weight.Normal)
        self.font_textInput = QtGui.QFont(
            "微软雅黑", 16, QtGui.QFont.Weight.Normal
        )  # 工具条文字工具默认字体
        ...
        self.pen_SolidLine_lightBlue.setStyle(
        ...
        self.pen_SolidLine_lightWhite.setStyle(
            Qt.PenStyle.SolidLine
        )  # 实线SolidLine，虚线DashLine，点线DotLine
    ...
    def paintCenterArea(self, centerRectF: QRectF):
        ...
        if centerRectF.width() >= 100 and centerRectF.height() >= 100:
            points = [  # 点坐标
                centerRectF.topLeft(),
                centerRectF.topRight(),
                centerRectF.bottomLeft(),
                centerRectF.bottomRight(),
                self.screenArea.centerLeftMid(),
                self.screenArea.centerRightMid(),
                self.screenArea.centerTopMid(),
                self.screenArea.centerBottomMid(),
            ]
        ...
        self.painter.drawText(
            labelPos,
            "%s x %s" % (centerPhysicalRect.width(), centerPhysicalRect.height()),
        )
    ...
    def paintMagnifyingGlass(
        self, screenSizeF: QSizeF, glassSize=150, offset=30, labelHeight=60
    ):
        ...
        glassPixmap, screenColor = self.screenArea.paintMagnifyingGlassPixmap(
            pos, glassSize
        )  # 画好纵横十字线后的放大镜内QPixmap
        ...
        labelRectF = QRectF(
            glassRect.bottomLeft().x(),
            glassRect.bottomLeft().y(),
            glassSize,
            labelHeight,
        )
