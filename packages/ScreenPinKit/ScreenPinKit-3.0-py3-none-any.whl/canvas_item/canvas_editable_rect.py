...
class CanvasEllipseItem(CanvasBaseItem):
    ...
    def getControllerPosition(self, rect: QRectF, radius, posType: EnumPosType):
        if posType == EnumPosType.ControllerPosTL:
            ...
            # 底端居中
            return (rect.bottomLeft() + rect.bottomRight()) / 2 - QPointF(
                radius, radius
            )
...
class CanvasEditablePath(QGraphicsObject):
    ...
    def __init__(self, parent: QGraphicsItem = None, isClosePath: bool = True) -> None:
        super().__init__(parent)
        self.editMode = (
            CanvasEditablePath.RoiEditableMode
            | CanvasEditablePath.BorderEditableMode
            | CanvasEditablePath.HitTestMode
            | CanvasEditablePath.AdvanceSelectMode
        )
    ...
    def insertPoint(
        self, insertIndex: int, point: QPointF, cursor: QCursor = Qt.SizeAllCursor
    ) -> CanvasROI:
        self.m_instId += 1
        id = self.m_instId
    ...
    def calcOffset(
        self, startPoint: QPointF, endPoint: QPointF, dbRadious: float
    ) -> QPointF:
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            def hitTest(startIndex, endIndex, polygonPath: QPainterPath):
                if polygonPath.contains(event.pos()):
                    self.insertPoint(
                        endIndex, event.pos(), Qt.CursorShape.SizeBDiagCursor
                    )
                    return True
                return False
    ...
    def getStretchableRect(self) -> QRect:
        return self.polygon.boundingRect() + QMarginsF(
            self.roiRadius, self.roiRadius, self.roiRadius, self.roiRadius
        )
    ...
    def endResizeOld(self, localPos: QPointF) -> None:
        ...
        p2.setX(
            origin.x()
            + math.cos(angle) * (p1.x() - origin.x())
            - math.sin(angle) * (p1.y() - origin.y())
        )
        p2.setY(
            origin.y()
            + math.sin(angle) * (p1.x() - origin.x())
            + math.cos(angle) * (p1.y() - origin.y())
        )
