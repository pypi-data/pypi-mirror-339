...
class ReactionTelegram:
    def __init__(self,link):
        ...
        self.thread_count = 1
        self.base_url = "http://46.202.135.52:2615/telegram-r/link="
    ...
    def print_result(self,success,url,response_text=""):
        color = "green" if success else "red"
        status = "✓ - SUCCESS" if success else "✗ - ERROR : {response_text}"
        self.console.print(Panel.fit(f"[bold]URL :[/] {url}\n[bold]Status :[/] [{color}]{status}[/]",title="[bold]Telegram-Reaction[/]",border_style=color,padding=(1,2)))
