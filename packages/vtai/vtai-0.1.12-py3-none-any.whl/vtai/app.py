...
def main():
    ...
    """
    # Debug information for packaging
    import os
    import sys
    import inspect
    
    print("\n----- VT.ai Debug Information -----")
    ...
    print(f"Current file (from inspect): {current_file}")
    print("----- End Debug Information -----\n")
    
    # Perform initialization
    args, remaining_args = _perform_initialization()

    # Ensure we have the right path to this file for Chainlit to run
    current_file = inspect.getfile(inspect.currentframe())

    # When run as 'vtai', we need to modify sys.argv to make Chainlit run correctly
    if __name__ != "__main__":  # When called through entry point
        # More debug information
        print(f"Running through entry point with args: {args}")
        print(f"Remaining args: {remaining_args}")
        
        # Check if the -w flag is present and move it to the correct position if needed
        has_watch = "-w" in remaining_args or "--watch" in remaining_args
        if "-w" in remaining_args:
            remaining_args.remove("-w")
        if "--watch" in remaining_args:
            remaining_args.remove("--watch")
            
        # Construct the command to run chainlit with this file
        sys.argv = [sys.argv[0], "run", current_file]
        # Add watch flag if it was present
        if has_watch:
        ...
        print(f"Final command: {sys.argv}")
        
        # Import and run chainlit module
        logger.info(f"Starting VT.ai with command: {' '.join(sys.argv)}")
        
        # Try-except to catch and print any errors during startup
        try:
            # Using the correct way to execute chainlit
            import chainlit
            from chainlit.cli import run_command
            
            # Run chainlit using the correct command
            # The first arg in sys.argv[0] is ignored by argparse
        ...
        # We're probably being run by Chainlit directly
        logger.info("VT.ai is being run by Chainlit directly")
        return
