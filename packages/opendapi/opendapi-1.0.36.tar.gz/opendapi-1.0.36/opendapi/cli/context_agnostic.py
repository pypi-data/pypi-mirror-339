...
def repo_runner_cli(
    ...
    _ = get_opendapi_config_from_root(
...
def minimal_schemas_for_generate(kwargs: dict) -> Schemas:
...
def cicd_get_s3_upload_data(
    ...
    """
    Get the s3 upload data for the given woven cicd id and integration
    """
    dapi_server_config = construct_dapi_server_config(kwargs)
    dapi_requests = DAPIRequests(
        dapi_server_config=dapi_server_config,
...
def cicd_init(
    call_cicd_init: Callable[[DAPIRequests], Tuple[str, dict]],
    change_trigger_event: ChangeTriggerEvent,
    kwargs: dict,
) -> Tuple[str, dict]:
    """
    ...
    woven_cicd_id, s3_upload_data = call_cicd_init(dapi_requests)
    print_cli_output(
        f"Finished initializing DAPI server-driven CICD with Woven CICD ID: {woven_cicd_id}",
        color="green",
    )
    return woven_cicd_id, s3_upload_data
...
def cicd_start(
    ...
    dapi_requests.cicd_start(
        integration=cicd_integration,
        metadata_file=metadata_file,
        woven_cicd_id=woven_cicd_id,
    )
    print_cli_output(
        "Successfully initiated DAPI server-driven CICD",
        color="green",
    )
    cleanup_tmp_state(opendapi_config)
