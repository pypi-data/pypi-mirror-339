...
class GCPStorage:
    ...
    def load_parquet_file(self, bucket_name, object_key, environment, project_root):
        ...
        try:
            if environment == "local":
                file_path = f'{project_root}/{object_key}'
    ...
    def save_json_metadata(self, processing_info, trident_changes_metadata_df, hr_metadata_df, mr_metadata_df, json_metadata_path):
        ...
        trident_changes_gcp_path = f"{summary_version}/trident_changes/{person_id}_trident-changes.parquet"
        hr_gcp_path = f"{summary_version}/hr_metadata_info/{person_id}_hr-metadata.parquet"
        mr_gcp_path = f"{summary_version}/mr_metadata_info/{person_id}_mr-metadata.parquet"
    ...
    def save_file_to_storage(self, df=None, bucket_name=None, key=None, file_type='parquet', compression=None,
        ...
        Example Usage:
            gcp_service.save_file_to_storage(df=df, bucket_name='my-bucket', key='path/to/file.parquet', file_type='parquet')
        """
        ...
        try:
            ...
            # print(f"File successfully saved to Google Cloud Storage: {key}")
