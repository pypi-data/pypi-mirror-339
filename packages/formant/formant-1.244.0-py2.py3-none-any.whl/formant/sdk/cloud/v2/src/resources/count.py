...
class Count(Resources):
    ...
    def active_devices_async(self, active_devices_query: ActiveDevicesQuery):
        """Gets all the active devices during the timestamp"""
        client = self._get_client()
        response = count_controller_active_devices.asyncio_detailed(client=client, json_body=active_devices_query)
        return response