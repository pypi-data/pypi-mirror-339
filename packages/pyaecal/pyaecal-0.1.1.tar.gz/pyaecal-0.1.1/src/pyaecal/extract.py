...
class Extract(Thread):
    ...
    def extract(self):
        self.run()
        self.files.join()
        # TODO return the data in a list ?
    ...
    def get_data(self, filename):
        ...
        if self.mdf.MDFVersionNumber <= 300:
            ...
            timestamp = datetime.fromisoformat("%s-%s-%s %s" % (s[2], s[1], s[0], time))
        ...
        for rename in self.rename.keys():
            ...
            if data is None:
                if self.optional[rename] == False:
                    logging.info("%s not in file" % rename)
            ...
            if rename not in df.columns:
                if df.is_empty():
                    ...
                    df = df.join(data, on="offset")  # , how='outer')

        # ffill or interpolate
        for rename in self.rename.keys():
            if rename not in df.columns:
                continue
            if self.interpolate[rename] is True:
                df = df.with_columns(polars.pl(rename).interpolate())
            else:
        ...
        df = df.drop_nans()
        rows, cols = df.shape
        logging.info("%s: %i rows" % (filename, rows))
        df=df.sort(['offset'])
        return df
