...
class BuildProject:
    ...
    manages file consistency, and initializes the basic implementation files.
    
    The generated project includes operators.py, semantic.py, examples.py, and 
    README.md files that provide the foundation for implementing a new modal logic theory.
    
    Attributes:
        theory (str): Name of the new theory to create
        source_dir (str): Directory path for the source theory template
        project_name (str): Name of the new project
        destination_dir (str): Directory path for the new project
        created_files (dict): Tracks which files were successfully created
    """

    # Standard file templates with consistent naming, required status, and fallbacks
    DEFAULT_TEMPLATES = {
        "examples.py": {
            "required": True,
            "description": "Example model checking cases",
            "fallback": """# Example cases for {project_name}
# Generated from {theory} theory template
...
# Define semantic theory components
semantic_theory = {{
    "semantics": SemanticDefaults,
    "model": ModelDefaults,
    "proposition": PropositionDefaults,
    "operators": operators,
}}
...
general_settings = {{
    ...
    "maximize": False,
        ...
        "operators.py": {
            ...
            "fallback": """# Operator definitions for {project_name}
# Generated from {theory} theory template

from model_checker.syntactic import OperatorCollection

# Define the operator collection for this semantic theory
operators = OperatorCollection()

# Define basic operators
operators.add_operator("\\\\neg", 1)  # Negation
operators.add_operator("\\\\and", 2)  # Conjunction
operators.add_operator("\\\\or", 2)   # Disjunction
operators.add_operator("\\\\imp", 2)  # Material implication
operators.add_operator("\\\\iff", 2)  # Material equivalence

# Add your custom operators here

# Export the collection for use in other modules
__all__ = ['operators']"""
        },
        "semantic.py": {  # Note: consistent semantic.py naming (not semantics.py)
            "required": True,
            "description": "Semantic theory implementation",
            "fallback": """# Semantic implementation for {project_name}
# Generated from {theory} theory template

import z3
from model_checker.model import SemanticDefaults, ModelDefaults, PropositionDefaults

class Semantic(SemanticDefaults):
    \"\"\"Implements the semantic theory for {project_name}.\"\"\"
    
    NAME = "{project_name}"
    
    DEFAULT_EXAMPLE_SETTINGS = {{
        "N": 3,                # Number of worlds
        "max_time": 30,        # Maximum time in seconds
        "model": True,         # Whether model should exist
        "iterate": 1,          # Number of models to find
    }}
    
    def __init__(self, N: int = 3):
        \"\"\"Initialize semantic theory with N worlds.\"\"\"
        super().__init__(N)
        
        # Define your semantic theory here
        
# Export the components for use in other modules
__all__ = ['Semantic', 'ModelDefaults', 'PropositionDefaults']"""
        },
        "README.md": {
            "required": True,
...
Modal logic theory implementation created with the model-checker tool.

## Overview

This project implements a semantic theory for modal logic, generated from the '{theory}' theory template.

## Files

- `examples.py`: Example model checking cases
- `operators.py`: Logical operator definitions
- `semantic.py`: Semantic theory implementation

## Usage

To run the example cases:

```
model-checker {project_name}/examples.py
```

## Customization

Edit the files to implement your custom semantic theory:

1. Define custom operators in `operators.py`
2. Implement semantic rules in `semantic.py`
3. Create test cases in `examples.py`
"""
        }
    }
    ...
    def __init__(self, theory: str = 'default'):
        ...
        self.DEFAULT_FILES = {
            ...
            "semantic.py": "semantic.py",  # Fixed to use semantic.py, not semantics.py
            "README.md": "README.md",
        }
    ...
    def generate(self, name):
        ...
        try:
            ...
            # Customize file content with project-specific values
            self._customize_file_content()
            
            # Report success and handle example script
    ...
    def _process_template_files(self):
        """Process all template files using a reliable sequence."""
        self.created_files = {}
        
        # First copy everything from the source directory
        try:
            ...
            if sys.version_info >= (3, 8):
                ...
                # For Python 3.7 and earlier, copy files individually
            ...
            print(f"Warning: Could not copy all files from source directory: {e}")
        
        # Then process each template individually
        for template_name, config in self.DEFAULT_TEMPLATES.items():
            ...
            self.created_files[template_name] = {
                "success": success,
    ...
    def _process_single_template(self, template_name, config):
        ...
        destination_path = os.path.join(self.destination_dir, template_name)
        source_path = os.path.join(self.source_dir, template_name)
        
        # Check if the file already exists (from copytree)
        if os.path.exists(destination_path):
            return True, {"path": destination_path, "source": "copied"}
        
        # Special case for semantic.py/semantics.py mismatch
        if template_name == "semantic.py" and not os.path.exists(source_path):
            semantics_path = os.path.join(self.source_dir, "semantics.py")
            if os.path.exists(semantics_path):
                try:
                    shutil.copy(semantics_path, destination_path)
                    return True, {"path": destination_path, "source": "renamed"}
                except Exception as e:
                    print(f"Warning: Could not copy semantics.py to semantic.py: {e}")
        
        # If file is required but doesn't exist, use fallback
        if config["required"] and not os.path.exists(destination_path):
            try:
                fallback_content = config["fallback"].format(
                    project_name=self.project_name,
                    theory=self.theory
                )
                with open(destination_path, 'w', encoding='utf-8') as f:
                    f.write(fallback_content)
                return True, {"path": destination_path, "source": "fallback"}
            except Exception as e:
                print(f"Warning: Could not create fallback {template_name}: {e}")
                return False, {"error": str(e)}
        
        # If we got this far and the file still doesn't exist
        if not os.path.exists(destination_path):
            if config["required"]:
                print(f"Warning: Required file {template_name} could not be created")
                return False, {"error": "File could not be created"}
            else:
                return False, {"error": "File not required and not found"}
                
        return True, {"path": destination_path, "source": "unknown"}
            
    def _customize_file_content(self):
    ...
    def _get_package_version(self):
        ...
        try:
            # Try importing version directly if installed
            try:
                from model_checker import __version__
                return __version__
            except ImportError:
    ...
    def _verify_project_completion(self):
        """Verify all required files were properly created."""
        missing_files = []
        for template_name, config in self.DEFAULT_TEMPLATES.items():
            if config["required"]:
                file_path = os.path.join(self.destination_dir, template_name)
    ...
    def _print_success_message(self):
        ...
        is_complete, missing_files = self._verify_project_completion()
        ...
        if not is_complete:
            print("\nWarning: The following required files could not be created:")
