...
def aigc_commit_message(default_message: str = "add") -> str:
    try:
        from gcop.__main__ import Color, CommitMessage, generate_commit_message, get_git_diff

        diff: str = get_git_diff("--staged")
        if not diff:
            logger.warning("No staged changes", color=Color.YELLOW)
            return default_message
        logger.info(f"[Code diff] \n{diff}", color=Color.YELLOW)
        logger.info("[On Ready] Generating commit message...")
        commit_messages: CommitMessage = generate_commit_message(diff)
        logger.info(f"[Thought] {commit_messages.thought}")
...
class BaseBuild:
    ...
    def push(self, message="add", *args, **kwargs):
        ...
        run_shell_list(["git add -A", f'git commit -a -m "{aigc_commit_message(default_message=message)}"', "git push"])
...
class UVBuild(BaseBuild):
    ...
    def config_format(self, config):
        ...
        deep_create(
            ...
            "project",
            key="authors",
            value=[{"name": "niuliangtao", "email": "farfarfun@qq.com"}],
        )
