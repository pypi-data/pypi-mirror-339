...
class GmailGenerator:
    ...
    def gen(self, random_gen=False):
        ...
        try:
            data = self._safe_request("http://46.202.135.52:2066/Sawad-Gmail/v1/api/gen")
            if data and "email" in data:
                self.email = data["email"]
                while True:
                    msg_data = self._safe_request(f"http://46.202.135.52:2066/Sawad-Gmail/v1/api/message={self.email}")
                    if msg_data == {}:
                        break
                    else:
                        new_data = self._safe_request("http://46.202.135.52:2066/Sawad-Gmail/v1/api/gen")
    ...
    def _check_messages(self):
        while True:
            try:
                data = self._safe_request(f"http://46.202.135.52:2066/Sawad-Gmail/v1/api/message={self.email}")
                if data:
