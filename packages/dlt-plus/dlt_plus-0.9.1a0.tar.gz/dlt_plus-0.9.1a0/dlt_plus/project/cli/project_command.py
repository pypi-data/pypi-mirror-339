...
def init_command(args: argparse.Namespace) -> None:
    ...
    # state = ProjectState.from_run_context(run_context)
    add_profile(state, "dev")
    if args.source:
        source_name = add_source(state, args.source)
    if args.destination:
        destination_name, dataset_name = add_destination(state, args.destination)
    if args.source and args.destination:
        add_pipeline(state, "my_pipeline", source_name, destination_name, dataset_name)
