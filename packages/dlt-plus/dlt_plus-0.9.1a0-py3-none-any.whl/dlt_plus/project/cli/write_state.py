...
class ProjectWriteState:
    ...
    def check_file_conflicts(self) -> None:
        ...
        for file in self.new_files:
            ...
            if not file["accept_existing"] and os.path.exists(full_path):
                fmt.error(
                    """
                File Conflit: <%s> already exists in project. Use --force to overwrite this
    ...
    def commit(self, allow_overwrite: bool = False) -> None:
        ...
        if self.pending_secrets:
            write_values(
                self.secrets_provider._config_toml,
    ...
    def update_secrets_toml(self, updates: str) -> None:
        """Updates the secrets.toml content"""
        update_dict = tomlkit.loads(updates)
        update_dict_nested(self.secrets_provider._config_toml, update_dict)

    def update_config_toml(self, update: str) -> None:
        """Updates the config.toml content"""
        update_dict = tomlkit.loads(update)
        update_dict_nested(self.config_provider._config_toml, update_dict)
