# Python Client For Nutanix Objects Storage Management APIs

The Python client for Nutanix Objects Storage Management APIs is designed for Python client application developers offering them simple and flexible access to APIs that manage Petabytes of Unstructured and Machine-generated data using a software-defined Object Store Service.
## Features
...
```sh
<my-env>/bin/pip install ntnx-objects-py-client
```
...
```sh
<your-env>\Scripts\pip.exe install ntnx-objects-py-client
```
...
## Configuration
The python client for Nutanix Objects Storage Management APIs can be configured with the following parameters
...
```python
# Initialize the API
object_stores_api_instance = ObjectStoresApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Get an Object store
try:
    api_response = object_stores_api_instance.get_objectstore_by_id(extId)
...
try:
    ...
    api_response = object_stores_api_instance.update_objectstore_by_id(body, extId, if_match=etag_value) # Use the extracted etag value
except ApiException as e:
```
...
try:
    api_response = object_stores_api_instance.list_objectstores(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=objects&version=v4.0&language=python). This documentation is auto-generated, and the location may change.
