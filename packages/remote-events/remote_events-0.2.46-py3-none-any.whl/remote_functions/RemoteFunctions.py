...
class RemoteFunctions:
    ...
    def get_output(self, omit_incomplete_last_line=True) -> str:
        ...
        Returns:
            str: The concatenated contents of the output file with complete lines only.

        Raises:
            FileNotFoundError: If the output file is not found or if the "TOOLBOX_OUTPUT_NAME" environment variable is not set.
        """
        output_filename = os.environ.get("TOOLBOX_OUTPUT_NAME")

        if not output_filename or not os.path.exists(output_filename):
            raise FileNotFoundError("Cannot find output file. Make sure to call 'run_self_with_output_filename()' as the first line of your main.py")
