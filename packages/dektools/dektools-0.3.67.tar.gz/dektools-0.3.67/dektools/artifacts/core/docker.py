...
import tempfile
import subprocess
from ...shell import shell_wrapper, shell_with_input_once, shell_result, shell_exitcode, shell_output
from ...file import read_text, write_file, read_lines, sure_dir, remove_path
...
class DockerArtifact(ArtifactBase):
    ...
    @classmethod
    def exist(cls, image):
        return shell_wrapper(f'{cls.cli} inspect {image} > /dev/null 2>&1', check=False) == 0
    ...
    def build_fast(
            cls,
            path, images, basic=None, step=None, base=None, args=None, options=None, push=True,
