...
@tool
def reasoning_llm( # Function name from YAML
    connection: Union[CustomConnection, AzureOpenAIConnection, OpenAIConnection],
    deployment_name: str,
    # ---> Argument MUST be named 'prompt' and type PromptTemplate <---
    prompt: PromptTemplate,
    max_completion_tokens: int = 5000,
    reasoning_effort: str = "low",
    # ---> Keep **kwargs to receive other inputs (like 'question' from flow) <---
    **kwargs
) -> str:
    """
    Calls the specified Azure OpenAI deployment using a PromptTemplate.
    Parses rendered prompt for '# system:'/'# user:'. Configurable parameters.
    ...
    try:
        # Use the 'prompt' argument directly (it holds the template string)
        # Pass kwargs from flow inputs for rendering
        rendered_prompt = Template(prompt, trim_blocks=True, keep_trailing_newline=True).render(**kwargs)
        print(f"Rendered Prompt:\n{rendered_prompt}")
    except Exception as e:
         raise ValueError(f"Failed to render prompt template: {e}") from e

    # --- Parsing Logic (using rendered_prompt) - Remains the same ---
    developer_content = ""
    user_content = ""
    try:
        ...
        if user_match:
            ...
            raise ValueError("Could not find '# user:' marker in the rendered prompt.") # This error check is still valid
    ...
    try:
        # Use the explicit inputs passed directly, not from kwargs here
