...
class AgentClient(BaseAgentClient, AuthClient):
    ...
    async def order_ad2epydin(self, ad: models.Ad, amount: float, cred_id: int = None) -> OrderEpydIn:
    ...
    async def cred_new(self, cred: models.Cred) -> models.CredEx:
        ...
        vals = (
            [{"name": "PHONE", "value": cred.detail}, {"name": "BANKS", "value": banks}]
    ...
    async def ad_f2ein(
        ...
        if ad_ein.type == "SALE":
            ...
            pmexs = await models.Pmex.filter(ex=self.agent.actor.ex, pm__pmcurs__id__in=[c.pmcur_id for c in creds])
...
async def main():
    ...
    taker: models.Agent
    maker, taker = (
        await models.Agent.filter(actor__ex_id=34, auth__isnull=False, actor__person__user__status__gt=UserStatus.SLEEP)
        # .order_by("-actor__ads")
        .limit(2)
