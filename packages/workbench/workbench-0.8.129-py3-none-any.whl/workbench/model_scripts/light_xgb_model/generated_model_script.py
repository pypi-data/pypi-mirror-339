...
TEMPLATE_PARAMS = {
    ...
    "feature_list": ['height', 'weight', 'salary', 'age', 'iq_score', 'likes_dogs', 'food'],
    "model_metrics_s3_path": "s3://sandbox-sageworks-artifacts/models/training/test-regression",
...
def convert_categorical_types(df: pd.DataFrame, category_mappings=None) -> tuple:
    """
    ...
    """
    # Initialize mappings dict if None
    if category_mappings is None:
        category_mappings = {}

    # Training mode (empty dictionary provided)
    ...
    # Inference mode (populated dictionary provided)
    else:
        for col, categories in category_mappings.items():
            if col in df.columns:
                print(f"Inference mode: Applying categorical mapping for {col}")
                # Apply consistent categorical mapping
