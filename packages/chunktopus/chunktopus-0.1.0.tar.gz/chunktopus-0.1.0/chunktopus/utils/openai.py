...
def get_openai_client(api_key=None):
    ...
    Args:
        api_key: Optional API key. If not provided, will attempt to get from environment.
        
    Returns:
        An initialized OpenAI client or None if initialization failed.
    """
    try:
        # Use provided API key or get from environment
        api_key = api_key or os.environ.get("OPENAI_API_KEY")
        if not api_key:
            logger.error("OpenAI API key not provided and OPENAI_API_KEY environment variable is not set")
...
def semantic_chunk_with_structured_output(text: str, api_key=None) -> List[Dict[str, Any]]:
    """
    Use OpenAI API to create semantic chunks from text using JSON mode.

    Args:
        text: The text to chunk
        api_key: Optional OpenAI API key. If not provided, will be retrieved from environment.
    ...
    if len(text) > 25000:
        ...
        return process_long_text_semantically(text, api_key=api_key)
...
def process_long_text_semantically(text: str, api_key=None) -> List[Dict[str, Any]]:
    """
