...
def mp4(filename, d, fps=2, triplet=True, mn = None, cmap=cmap, clamp=1.0):
    ...
    if triplet:
        ...
        print(frames.shape)
        r = np.max(np.abs(frames[:,-1]), axis=(0,2,3))[None,None,:,None,None] # B, Y, C, H, W
        ...
        frames = d
        r = np.max(np.abs(frames), axis=(0,2,3))[None,:,None,None] # B, C, H, W
        nframes = frames / (2 * r * clamp) + 0.5
