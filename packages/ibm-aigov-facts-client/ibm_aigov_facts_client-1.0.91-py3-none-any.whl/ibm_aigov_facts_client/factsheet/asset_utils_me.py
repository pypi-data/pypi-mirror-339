...
class ModelUsecaseUtilities:
    ...
    def get_tracked_models(self) -> list:
        ...
        if assets_data:
            get_facts_global = assets_data.json(
            )['entity'].get('modelfacts_global')
    ...
    def set_attachment_fact(self,
        ...
        if self._is_cp4d and self._cp4d_version < "5.1.0":
            ...
            if get_factid_attachment:
                ...
                try:
                        ...
                        attachment_url = self._get_url_attachments_new(
                            asset_id=model_asset_id,container_id=model_container_id, container_type=model_container_type, name=file_name, description=description, mimetype=mimetype, fact_id=fact_id)
    ...
    def _get_headers(self):
        token = self._facts_client._authenticator.token_manager.get_token() if (
                isinstance(self._facts_client._authenticator, IAMAuthenticator) or (
            isinstance(self._facts_client._authenticator,
                       CloudPakForDataAuthenticator))) else self._facts_client._authenticator.bearer_token
        iam_headers = {
    ...
    def list_associated_workspaces_by_phase(self, phase_name: Phases = None) -> None:
        """
