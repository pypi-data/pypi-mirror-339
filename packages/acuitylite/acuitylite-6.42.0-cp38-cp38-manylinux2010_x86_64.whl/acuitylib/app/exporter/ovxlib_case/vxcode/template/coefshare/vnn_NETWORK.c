...
#define NEW_CONST_TENSOR(_id, _attr, _dtype, _ofst, _size, _save_quant_param_to_export_data) do {\
        ...
        _id = vsi_nn_AddTensor( graph, VSI_NN_TENSOR_ID_AUTO,\
                & _attr, data );\
        free( data );\
        if( TRUE == _save_quant_param_to_export_data ){\
            if( (void*) _attr.dtype.scales ){\
                free( (void*) _attr.dtype.scales );\
                _attr.dtype.scales = NULL;\
            }\
            if( (void*) _attr.dtype.zero_points ){\
                free( (void*) _attr.dtype.zero_points );\
                _attr.dtype.zero_points = NULL;\
            }\
        }\
        if( VSI_NN_TENSOR_ID_NA == _id ) {\
            goto error;\
...
#if defined(VX_GRAPH_ENV_SUPPORT) && VX_GRAPH_ENV_SUPPORT
    if(env_count) {
        for(i = 0; i < env_count; i++) {
            printf("Set ENV: %s=%s\n", envs[i].name, envs[i].value);
            vsi_nn_SetRunTimeVariable(graph, envs[i].name, envs[i].value);
        }
    }
    #else
    (void)env_count;(void)envs;
#endif
