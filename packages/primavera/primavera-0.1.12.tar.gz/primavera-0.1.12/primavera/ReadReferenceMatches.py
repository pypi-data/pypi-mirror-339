...
class SequencingRead:
    ...
    def __init__(
        ...
        self.average_quality = (
            None if read_qualities is None else np.mean(read_qualities)
        )
...
class ReadReferenceMatchesSet:
    ...
    def from_reads(
        ...
        """ """
        reference_sequence = (
            reference if isinstance(reference, str) else str(reference.seq)
        )
        ...
        reads_primer_sequences = {
            ...
            for read in reads
            if (read.primer is not None) and (read.primer.sequence is not None)
        }
                ...
                for hsp in al.hsps:
                    ...
                    if len(primer_matches) == 1:
                        ...
                        if m.strand == 1:
                            ...
                            if not any(
                                [
                                    (mstart < start < mstart + max_read_length)
                            ...
                            if not any(
                                [
                                    (mstart > end > mstart - max_read_length)
                                    for mstart in mstarts
                                ]
                            ):
                                continue
    ...
    def plot(
        ...
        Useful to get a general overview of the sequencing (coverage, mutations etc.)
        ...
        if plot_reference:
            ...
            reference_ax.set_ylim(bottom=-3)
