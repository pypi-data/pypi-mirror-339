...
class PrimerSelectorLogger(TqdmProgressBarLogger):
    ...
    def __init__(self, bars=("record", "primer"), notebook="default"):
        ...
        TqdmProgressBarLogger.__init__(
            self,
            bars=bars,
            notebook=notebook,
...
class PrimerSelector:
    ...
    primer_reuse_bonus
      ...
      solutions where fewer new primers have to be ordered, but more
    ...
    def __init__(
        ...
        self.primer_reuse_bonus = primer_reuse_bonus
        if logger == "bars":
    ...
    def select_primers(
        ...
        records
          ...
          indicated by a feature of type ``misc_feature`` and label ``no_primer``.
        ...
        if strand == "both":
            ...
            return [
                forward + rev for forward, rev in zip(forward_primers, reverse_primers)
            ]
        ...
        indices_to_cover = {}
        primers_coverages = defaultdict(
            lambda *a: {"primary": set(), "extended": set()}
        )
        ...
        for record in self.logger.iter_bar(record=records):
            ...
            coverages = self.compute_all_primers_coverage_on_record(
                record,
                available_primers=available_primers_seqs,
        ...
        primers_cover = minimal_cover(
            elements_set, subsets=subsets, heuristic=heuristic
        )
        ...
        for primer_seq in primers_cover:
            if primer_seq in available_primers_dict:
                ...
                primer = Primer(
                    name, primer_seq, metadata={"available": False, "infos": infos}
                )
        ...
        for record in self.logger.iter_bar(record=records):
            ...
            subcovers = {
                prim_seq: {
                    "primary": primers_coverages[prim_seq]["primary"].intersection(
                        elements
                    ),
                    "extended": primers_coverages[prim_seq]["extended"].intersection(
                        elements
                    ),
                }
            ...
            sub_primers_cover = minimal_cover(
                elements, subsets=subsets, heuristic=heuristic
            )
    ...
    def compute_forbidden_patterns_locations(self, record):
        ...
        pattern_constraints = [AvoidPattern(HomopolymerPattern(c, 5)) for c in "ATGC"]
        kmer_constraints = [
            AvoidPattern(RepeatedKmerPattern(n_repeats, kmer_size))
            for n_repeats, kmer_size in [(4, 2), (3, 3), (2, 4)]
        ]
        problem = DnaOptimizationProblem(
            sequence=record, constraints=pattern_constraints + kmer_constraints
        )
    ...
    def compute_sequence_primers(self, record, strand="any"):
        ...
        locations = find_best_primer_locations(
            sequence, tm_range=self.tm_range, size_range=self.size_range
        )
    ...
    def compute_all_primers_coverage_on_record(
        self, record, indices_to_cover, available_primers, strand="any"
    ):
        ...
        reusable_primers_sequences = [
            ...
            and (
                (strand == "any")
                or ((location[2] == 1) and (strand == "5-3"))
                or ((location[2] == -1) and (strand == "3-5"))
            )
        ]
        ...
        for primer, (start, end, strand) in iter_bar(primer=primers):
            ...
            primers_coverages[primer]["extended"] = set(
                [indices_to_cover[ind] for ind in primers_coverages[primer]["extended"]]
            )
            # print(len(primers_coverages[primer]['primary']))

        return primers_coverages

    def generate_primer_name(self, prefix="P", available_primers_names=(), n_digits=6):
        ...
        starting with P in the list of available primer names.

        Parameters
        ----------
        prefix
          The prefix for the primer name.
    ...
    def find_part_name_in_record(self, record, index):
        ...
        record
          A single Biopython record where to find the sequence. The parts
        ...
        for f in record.features:
            ...
            start, end = sorted([int(e) for e in [f.location.start, f.location.end]])
            if start <= index <= end:
    ...
    def compute_coverage_records(self, records, selected_primers, target_dir=None):
        result = [
            simulate_sequencing(
                record,
                primers=primers,
    ...
    def write_records_primers_table(
        self, selected_primers, records, sep="|", csv_path=None
    ):
