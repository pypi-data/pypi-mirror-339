...
TEventixTaskStatusLiteral = Literal[
    "scheduled",
    "processing",
    "done",
    "error",
...
class TEventixTask(BackendModel):
    ...
    max_retries: int | None = (
        None  # None forever, positive value gets decremented until 0
    )
    ...
    class Config:
        ...
        backend_indexes = [
            ...
            SortingIndex(
                "status_scheduled", [{"status": "desc"}, {"scheduled": "desc"}]
            ),
            ...
            AggregationIndex(
                "task_namespace_sum", {"task": "_sum", "namespace": "_sum"}
            ),
            AggregationIndex(
                "status_namespace_sum", {"status": "_sum", "namespace": "_sum"}
            ),
        ]
