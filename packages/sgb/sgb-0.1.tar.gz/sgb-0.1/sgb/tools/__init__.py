...
def nl(
    value: str = "", count: int = 1, reversed: nbool = False, normal: bool = True
) -> str:
    nl_text: str = ["<br>", "\n"][int(normal)] * count
...
def j_nl(
    ...
    return j(value, nl("", count, reversed, normal))
...
class DataTool:
    ...
    def filter_by_string(
        ...
        return DataTool.filter(
            lambda item: (
                ...
                else search_function(item, string)
            ),
            value,
        )
    ...
    @staticmethod
    def triple_bool(
        value: nbool, false_result: Any, true_result: Any, none_result: Any
    ) -> Any:
        if n(value):
            return none_result
...
class ResultTool:
    ...
    def map(
        ...
        result_data: list[T] = result.data or []
        result_data = DataTool.as_list(result_data)
...
class ListTool:
    ...
    @staticmethod
    def list_str_diff(
        a: list[str], b: list[str], two_way_comparing: bool = False
    ) -> list[str]:
        def compare_function(value1: str, value2: list[str]) -> bool:
            for item in value2:
                if StringTool.contains(value1, item, True, two_way_comparing):
                    return True
            return False
    ...
    def list_str_intersection_ratio(
        ...
        return len(ListTool.list_str_diff(a, b, two_way_comparing))
...
class StringTool:
    ...
    def _comparation_ratio_by_tokens(
        ...
        return (
            ListTool.list_str_intersection_ratio(a_list, b_list, two_way_comparing),
            a_list,
            b_list,
        )
    ...
    def contains(
        ...
        return (
            eq(value2, value1)
            if two_way_comparing and len(value2) > len(value1)
...
def while_not_do(
    ...
    # delay_time_on_success: int | None = None,
) -> Any:
    result: Any = None
...
class OSTool:
    ...
    def host(alias: nstr = None) -> str:
        ...
        return OSTool.host_name()  # platform.node()
