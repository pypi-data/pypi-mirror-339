...
from sgb.service_interfaces.executor import IResultExecutorClient, ExecutorClient
...
from sgb.service_interfaces.ad import (
    ...
    IActionWorkstationClient,
)
from sgb.service_interfaces.skype import IResultSkypeBusinessClient, SkypeBusinessClient
from sgb.service_interfaces.dameware import IActionDamewareClient, DamewareClient
...
class RootSuppport(IRootSupporter):
    ...
    def __init__(self):
        self._domain_dns = AD.DOMAIN_DNS

    def error_detect_handler(self, exception: BaseException) -> None:
...
class MarkOutput(MarkOutputBase):
    ...
    def by_any(self, value: str) -> None:
        try:
            self.result(SGB.RESULT.MARK.by_any(value), "Найденные карты доступа:", True)
...
class OutputBase(OutputAbstract, OutputExtendedAbstract):
    def __init__(self, user_output: UserOutputBase | None = None):
...
class Session(SessionBase, ArgumentParserTool):
    ...
    def get_user(self) -> User:
        if n(self.user):
            self.user = one(SGB.RESULT.USER.by_login(self.get_login()))
        return self.user
    ...
    def authenticate(self, exit_on_fail: bool = True, once: bool = True) -> bool:
        try:
            ...
            if SGB.SERVICE.is_available(SERVICE_ROLE.AD):
...
class Output(OutputBase, OutputStub):
    ...
    def free_marks(
        ...
        self.table_with_caption_first_title_is_centered(
            ResultTool.sort(sort_function, SGB.RESULT.MARK.free_list(show_guest_marks)),
            "Свободные карты доступа:",
            use_index,
        )

    def guest_marks(self, use_index: bool = False) -> None:
        mark_list_result: Result[list[Mark]] = SGB.RESULT.MARK.free_list(True)
    ...
    def free_marks_group_statistics(
        ...
        self.free_marks_group_statistics_for_result(
            SGB.RESULT.MARK.free_marks_group_statistics(show_guest_marks), use_index
        )

    def free_marks_by_group(self, group: dict, use_index: bool = False) -> None:
        self.free_marks_by_group_for_result(
            SGB.RESULT.MARK.free_marks_by_group_id(group), group, use_index
        )
    ...
    def temporary_marks(
        ...
        self.table_with_caption(
            SGB.RESULT.MARK.temporary_list(),
...
class Input(InputBase):
    ...
    def index(
        ...
        search_function: Callable[[Any, str], bool] | None = None,
    ) -> nint | str:
        NO_INDEX: int = -1
        length: int = len(data)
        if length == 0:
        ...
        if allow_choose_all:
            data.insert(0, j_nl(("Выбрать всё", "...")))
            ...
            while True:
                ...
                if selected_index == NO_INDEX:
                    if use_free_input:
                        ...
                        data_filtered_list: list[Any] = SGB.DATA.filter_by_string(
                            value,
                            data_label_list if n(search_function) else data,
                            search_function=search_function,
                        )
                        data_label_filtered_list: list[str] | None = None
                        if n(search_function):
                            data_label_filtered_list = data_filtered_list
                        else:
                            data_label_filtered_list = DataTool.map(
                                lambda item: (
                                    item
                                    if n(label_function)
                                    else label_function(item, -1)
                                ),
                                data_filtered_list,
                            )
    ...
    def tab_number(self, check: bool = True) -> str:
        ...
        while True:
            ...
            if check:
                if SGB.CHECK.MARK.tab_number(tab_number):
                    return tab_number
                else:
...
class MarkInput(MarkInputBase):
        ...
        while True:
            if n(group):
                if self.parent.confirm(
                    ...
                    free_mark_list_result = SGB.RESULT.MARK.by_name(self.parent.name())
                    ...
                    free_mark_list_result = SGB.RESULT.MARK.free_marks_group_statistics(
                        False
                    )
                    free_mark_list: list[Mark] = free_mark_list_result.data
                ...
                free_mark_list_result = SGB.RESULT.MARK.free_marks_by_group_id(
                    group.GroupID
                )
                length = len(free_mark_list_result)
                if length > 0:
    ...
    def person_division(self, use_empty_division: bool = True) -> PersonDivision:
        division_list: list[PersonDivision] = SGB.RESULT.MARK.person_divisions().data
    ...
    def by_name(self) -> Mark:
        ...
        while result is None:
            try:
                result = SGB.RESULT.MARK.by_name(self.parent.name())
            except NotFound as error:
    ...
    def by_any(self, value: nstr = None) -> Mark:
        ...
        while result is None:
            try:
                result = SGB.RESULT.MARK.by_any(value or self.any())
            except NotFound as error:
                self.parent.output.error(error.get_details())
        self.parent.output.mark.result(result, "Карты доступа", True)
        return self.parent.item_by_index(
...
class WorkstationInput(WorkstationInputBase):
    ...
    def select(
        ...
        if show_select_all and len(result) > 1:
            result.data.append(None)
        result.sort(key=lambda item: j(([1, 0][item.accessable]), item.name))
        workstation: Workstation
        result.data = DataTool.as_list(
            self.parent.item_by_index(
                "Выберите компьютер, введя индекс",
                result.data,
                lambda workstation, _: (
                    "Все"
                    if n(workstation)
                    else j(
                        (
                            (
                                self.parent.output.green_str(A.CT_V.BULLET)
                                if workstation.accessable
                                else self.parent.output.red_str(A.CT_V.BULLET)
                            ),
                            " ",
                            b(workstation.name),
                            ": ",
...
class SGB:
    ...
    class EXECUTOR:
        ...
        def execute(
            ...
            if (
                ...
                if SGB.SERVICE.is_available(SERVICE_ROLE.EXECUTOR):
        ...
        def python_for_result(
            ...
            return one(SGB.RESULT.SSH.execute_python(command, host))
    ...
    class EVENT:
        ...
        def send(
            ...
            if send_on_accessibility:
                block = False
            elif not SGB.SERVICE.is_available(SERVICE_ROLE.EVENT):
                return False
            ...
            def internal_send(
                ...
                if send_on_accessibility:
                    while_not_do(
                        lambda: SGB.SERVICE.is_available(SERVICE_ROLE.EVENT),
                        sleep_time=1,
                    )
                try:
        ...
        def it_notify_about_create_mark(
            ...
            mark: Mark = SGB.RESULT.MARK.by_name(name, True).data
            telephone_number: str = SGB.DATA.FORMAT.telephone_number(
                ...
                class DB_DUMP:
                    ...
                    def creation_start(
                        ...
                        SGB.EVENT.send(
                            event,
                            flags=(
                                None if save else SGB.DATA.EVENT.remove_save_flag(event)
                            ),
                        )
                    ...
                    def archive_creation_start() -> None:
                        SGB.EVENT.send(Events.POLIBASE_DB_DUMP_ARCHIVE_CREATION_START)
        ...
        def it_notify_about_create_new_mark(full_name: str | FullName) -> None:
            SGB.EVENT.it_notify_about_create_mark(full_name, False)

        @staticmethod
        def it_notify_about_create_temporary_mark(
            full_name: str | FullName, tab_number: str
        ) -> None:
            SGB.EVENT.it_notify_about_create_mark(full_name, True, tab_number)
    ...
    class SERVICE(ServiceTool):
        ...
        def create_support_service_or_master_service_description(
            ...
            if SGB.SERVICE.is_available(master_service_desctiption):
        ...
        def start(
            ...
            if SGB.SYS.python_exists(host):
                ...
                if sgb_install:
                    ...
                    sgb_last_version: str = one(sgb_version_list)
                ...
                try:
                    ...
                    if check_if_started:
                        if SGB.SERVICE.is_available(service_description):
                            return None
                    command_list: strlist = []
                    file_path: nstr = None
                    if as_standalone:
            ...
            if SGB.SERVICE.check_on_host_chanchable(service_description) or isolate:
                ...
                with A.ER.detect_interruption("Выход"):
                    SGB.SERVICE.client.create_service().serve(
                        ...
                        as_standalone,
                        show_output,
                        SGB.SERVICE.is_available(SERVICE_ROLE.SERVICE_ADMIN),
                    )
            else:
        ...
        def stop(
            ...
            if not check_if_started or SGB.SERVICE.is_available(description):
        ...
        def validate(
            ...
            if ne(service_information_list):
                ...
                with ErrorableThreadPoolExecutor(max_workers=length) as executor:
                    future_to_bool = {
                        executor.submit(
                            SGB.ERROR.wrap(SGB.SERVICE.is_available),
                            service_descroption,
                        ): service_descroption
        ...
        def is_available(
            ...
            return SGB.SERVICE.client.check_on_accessibility(service_object, cached)
    ...
    class DATA(
        ...
        class IOTDevices:
            ...
            def status_value(device_id: str, status_value_code: str) -> Any:
                status: IOTDeviceStatus | None = SGB.RESULT.IOTDevices.status(
                    device_id
                ).data
                if n(status):
                    return None
        ...
        class USER:
            ...
            @staticmethod
            def admin_for(user: User) -> list[User] | None:
                result: list[User] = ResultTool.filter(
                    lambda user_item: user_item.description.find(user.name) != -1,
            ...
            def by_workstation_name(value: str) -> str:
                return SGB.EXECUTOR.get_logged_user(value)
        ...
        class FORMAT(FormatTool):
            ...
            def domain(value: str) -> str:
                return FormatTool.domain(AD.DOMAIN_DNS, value)
            ...
            def user_principal_name(login: str) -> str:
                return j((login, AD.DOMAIN), CONST.DOMAIN_SPLITTER)
            ...
            def host(
                ...
                if use_default_domain and not value.endswith(AD.DOMAIN):
                    value = j_p((value, AD.DOMAIN))
        ...
        class TELEPHONE_NUMBER:
            ...
            def it_administrator() -> str:
                return SGB.DATA.TELEPHONE_NUMBER.by_login(
                    SGB.DATA.VARIABLE.ENVIRONMENT.value(LINK.ADMINISTRATOR_LOGIN, False)
                )
            ...
            def by_mark_tab_number(value: str, format: bool = True) -> str:
                result: str = SGB.DATA.MARK.by_tab_number(value).telephoneNumber
                return SGB.DATA.FORMAT.telephone_number(result) if format else result

            @staticmethod
            def by_polibase_person_pin(value: int, format: bool = True) -> bool:
                result: str = SGB.DATA.POLIBASE.person_by_pin(value).telephoneNumber
                return SGB.DATA.FORMAT.telephone_number(result) if format else result
        ...
        class POLIBASE:
            ...
            def person_by_pin(value: int, test: nbool = None) -> PolibasePerson:
                return SGB.RESULT.POLIBASE.person_by_pin(value, test).data
            ...
            def duplicate_persons_by_person(
                ...
                return ResultTool.get_first_item(
                    ResultTool.filter(
                        ...
                        SGB.RESULT.POLIBASE.persons_by_name(person.FullName),
                    )
                )
            ...
            def unique_by_telephone(value: str) -> PolibasePerson:
                ...
                def check_function(check_person: PolibasePerson) -> bool:
                    return (
                        SGB.DATA.FORMAT.telephone_number(check_person.telephoneNumber)
                        == value
                    )
                ...
                return ResultTool.get_first_item(
                    ResultTool.filter(
                        lambda item: check_function(item),
                        SGB.RESULT.POLIBASE.person_by_telephone_number(value),
                    )
                )
            ...
            def duplicate_persons_by_person_pin(
                ...
                try:
                    return SGB.DATA.POLIBASE.duplicate_persons_by_person(
                        SGB.RESULT.POLIBASE.person_by_pin(value).data, check_birth
                    )
                except NotFound as _:
                    return None
    ...
    class SYS(OSTool):
        ...
        def stop_windows_service(name: str, computer_name: str) -> nbool:
            accessable: nbool = SGB.CHECK.COMPUTER.accessibility(computer_name)
            if n(accessable):
                return None
            return SGB.EXECUTOR.stop_windows_service(name, computer_name)
        ...
        def start_windows_service(
            ...
            accessable: nbool = (
                ...
                or SGB.CHECK.COMPUTER.accessibility(computer_name)
            )
            if n(accessable):
                return None
            return SGB.EXECUTOR.start_windows_service(name, computer_name)

        @staticmethod
        def start_windows_service_if_stopped(name: str, computer_name: str) -> nbool:
            windows_service_information = name, computer_name
            if not SGB.CHECK.COMPUTER.windows_service_running(
                *windows_service_information
            ):
                return SGB.SYS.start_windows_service(*windows_service_information)
            return None
        ...
        @staticmethod
        def domain_dns() -> str:
            return SGB.SYS.environment_variable("USERDNSDOMAIN")

        @staticmethod
        def domain() -> str:
            return SGB.SYS.environment_variable("USERDOMAIN")
    ...
    class RESULT(ResultTool):
        ...
        COMPUTER: IResultComputerClient | None = None

        def __init__(self):
            SGB.RESULT.COMPUTER = ComputerClient.RESULT()
        ...
        class NOTES:
            ...
            def find(value: str, label: nstr = None) -> Result[list[File]]:
                ...
                if nn(value):
                    result = ResultTool.filter(search_function, result, True)
        ...
        class FILES:
            ...
            def find(
                ...
                result = (
                    ...
                    if n(value)
                    else ResultTool.filter(search_function, result, True)
                )
                result = (
                    result
                    if n(command_type)
            ...
            def execute(
                ...
                file: File | None = one(SGB.RESULT.FILES.by_name(file_search_request))
                if n(file):
                    return None
        ...
        class SSH:
            ...
            def execute(
                ...
                return DataTool.to_result(
                    SGB.SERVICE.call_command(
                        SERVICE_COMMAND.execute_ssh_command,
                        (
                            command,
                            EnumTool.get(host),
                            username,
            ...
            def get_certificate_information(
                ...
                return DataTool.to_result(
                    SGB.SERVICE.call_command(
                        SERVICE_COMMAND.get_certificate_information,
            ...
            def get_unix_free_space_information_by_drive_name(
                ...
                return DataTool.to_result(
                    SGB.SERVICE.call_command(
                        SERVICE_COMMAND.get_unix_free_space_information_by_drive_name,
        ...
        class MARK:
            ...
            def free_list(show_with_guest_marks: bool = False) -> Result[list[Mark]]:
                result: Result[list[Mark]] = DataTool.to_result(
                    SGB.SERVICE.call_command(SERVICE_COMMAND.get_free_mark_list), Mark
                )
            ...
            def free_marks_by_group_id(value: int) -> Result[list[Mark]]:
                return DataTool.to_result(
                    SGB.SERVICE.call_command(
                        SERVICE_COMMAND.get_free_mark_list_by_group_id, value
                    ),
                    Mark,
                )
            ...
            def free_marks_group_statistics(
                ...
                return DataTool.to_result(
                    SGB.SERVICE.call_command(
                        SERVICE_COMMAND.get_free_mark_group_statistics_list,
                        show_guest_marks,
                    ),
                    MarkGroupStatistics,
                )
    ...
    class CHECK:
        ...
        class FILES:
            ...
            @staticmethod
            def exists(name: str) -> bool:
                return ne(SGB.RESULT.FILES.find(name))
        ...
        class MARK:
            ...
            def free(tab_number: str) -> bool:
                return DataTool.rpc_decode(
                    SGB.SERVICE.call_command(
                        SERVICE_COMMAND.check_mark_free, tab_number
                    )
                )

            @staticmethod
            def exists_by_full_name(full_name: FullName) -> bool:
                result: Result[list[Mark]] = SGB.RESULT.MARK.by_name(
                    FullNameTool.fullname_to_string(full_name)
                )
                return e(result)
            ...
            def accessibility() -> bool:
                return SGB.SERVICE.is_available(SERVICE_ROLE.MARK)
        ...
        class TIME_TRACKING:
            @staticmethod
            def accessibility() -> bool:
                return SGB.CHECK.ACCESS.action_for_group(AD.Groups.TimeTrackingReport)
        ...
        class INVENTORY:
            ...
            def is_report_file(file_path: str) -> bool:
                return DataTool.rpc_decode(
                    SGB.SERVICE.call_command(
                        SERVICE_COMMAND.check_inventory_report, file_path
                    )
                )
            ...
            def accessibility() -> bool:
                return (
                    SGB.SERVICE.is_available(SERVICE_ROLE.DOCS)
                    and SGB.CHECK.ACCESS.inventory()
                )
        ...
        class WORKSTATION:
            ...
            def accessibility(name: str) -> nbool:
                try:
                    return SGB.RESULT.WORKSTATION.by_name(name).data.accessable
                except NotFound as _:
                    return None
    ...
    class ACTION:
        ...
        class SSH:
            ...
            def mount_facade_for_linux_host(value: nstr) -> nbool:
                if n(value) and not SGB.SYS.is_linux():
                    return None
                return DataTool.rpc_decode(
                    SGB.SERVICE.call_command(
                        SERVICE_COMMAND.mount_facade_for_linux_host,
                        (value or SGB.SYS.host(),),
                    )
                )
...
def thread_on_accessibility(
    ...
    checked_threaded(
        value,
        lambda: A.SRV.is_available(service_object or RPC.service_description),
    )
