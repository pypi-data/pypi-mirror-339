...
class SGB:
    ...
    class CHECK:
        ...
        class WORKSTATION:
            ...
            def accessibility(name: str) -> nbool:
                try:
                    return one(SGB.RESULT.WORKSTATION.by_name(name)).accessable
                except NotFound as _:
                    return None
        ...
        class FILE:
            ...
            def write_content_by_computer(
                ...
                if isinstance(content, (tuple, list)):
                    content = j_nl(content)
                try:
                    with open(
                        A.PTH.resolve(file_path, computer.name), "a", encoding=encoding
                    ) as file:
                        file.write(content)
                except Exception as error:
                    return False
