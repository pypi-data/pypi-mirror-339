        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if self.downloader.download_file(
                    DISTRIBUTION_FILE, os.path.join(self.data_dir, DISTRIBUTION_FILE), retries=1, log_error_on_failure=False
                ):
                    with open(os.path.join(self.data_dir, DISTRIBUTION_FILE), "r") as f:
                        distribution_content = f.read()
                else:
                    if os.path.exists(os.path.join(self.data_dir, DISTRIBUTION_FILE)):
                        logger.warning("Failed to download distribution file, attempting to delete")
                        os.unlink(os.path.join(self.data_dir, DISTRIBUTION_FILE))
