...
def get_fn(api_key: str, model: str):
    def chat_fn(message, history):
        ...
        response = client.chat.completions.create(
            ...
            stream=True,
            extra_headers={
                "HTTP-Referer": "http://localhost:7860",  # Default Gradio URL
...
def registry(name: str, token: str | None = None, twilio_sid: str | None = None, twilio_token: str | None = None, enable_voice: bool = False, coder: bool = False, **kwargs):
    api_key = token or os.environ.get("OPENROUTER_API_KEY")
    if not api_key:
                            ...
                            with antd.Flex(vertical=True, gap="middle", wrap=True):
                                header = gr.HTML("""
                                    <div class="left_header">
                                        <h1>OpenRouter Code Generator</h1>
                                    </div>
                                """)
...
def generate_code(query, image, setting, history, model):
    ...
    model_to_use = "openai/gpt-4-vision-preview" if image else model
    
    # Add the content to messages
    messages.append({"role": "user", "content": content if image else query})
    
    # Create completion with OpenRouter parameters
    completion_params = {
        "model": model_to_use,
        "messages": messages,
        "stream": True,
    ...
    # Add specific parameters for o3 models if needed
    if "o3" in model_to_use:
        completion_params.update({
            "response_format": {"type": "text"},
            "reasoning_effort": setting.get("reasoning_effort", "high")
        })
