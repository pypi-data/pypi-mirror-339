...
static int _byte_get_bit(Byte_t* byte, ll bit_index){
...
bool check_index(ull num, ull start, ull end){
    return (num >= start && num < end) && (start > 0 && end > 0);
}


static void _bitmap_set_bit(BitmapObject *bitmap, ll index, bool on){
    ll byte_index = index / 8;
    ll bit_index = index % 8;
    _byte_set_bit(&(bitmap->data[byte_index]), bit_index, on);
}

static int _bitmap_get_bit(BitmapObject *bitmap, ll index){
    ll byte_index = index / 8;
    ll bit_index = index % 8;
    return _byte_get_bit(&(bitmap->data[byte_index]), bit_index);
}

static void _bitmap_set_bits(BitmapObject *bitmap, int on, ll start, ll end, ll step){
    for (ll i = start; i < end; i += step){
        _bitmap_set_bit(bitmap, i, on);
    }
}

ull _get_range_length(ll start, ll end, ll step){
...
int* _bitmap_get_bits(BitmapObject *bitmap, ll start, ll end, ll step){
    if (start == end){
        return NULL;
    }
...
int _bitmap_get_byte(BitmapObject *bitmap, ll index){
    ll byte_index = index / 8;
