...
class Index:
    ...
    def upsert(self, input_array):
        ...
        for item in input_array:
            ...
            # Prepare metadata as binary data
            if isinstance(item.get('meta', {}), dict):
                meta_data = json_zip(dict=item.get('meta', {}))
            else:
                meta_data = item.get('meta', b'')
                
            # Ensure meta_data is bytes for std::vector<uint8_t>
            if not isinstance(meta_data, bytes):
                meta_data = b'' if meta_data is None else bytes(meta_data)
            
            # Convert filter to JSON string to match std::string filter
            filter_str = json.dumps(item.get('filter', {}))
            ...
            # Create vector object as an array matching the exact order in MSGPACK_DEFINE
            vector_obj = [
                str(item.get('id', '')),  # std::string id
                meta_data,                # std::vector<uint8_t> meta
                filter_str,               # std::string filter
                float(norm),              # float norm
                vector_list               # std::vector<float> vector
            ]
            
            vector_batch.append(vector_obj)
        
        # Serialize batch using msgpack with binary type support
