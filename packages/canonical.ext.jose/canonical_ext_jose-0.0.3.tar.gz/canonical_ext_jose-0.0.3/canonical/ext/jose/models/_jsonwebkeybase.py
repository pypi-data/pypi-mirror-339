...
from canonical.ext.jose.types import KeyOperationType
from canonical.ext.jose.types import KeyUseType
...
class JSONWebKeyBase(pydantic.BaseModel, Generic[K, O, U]): # pragma: no cover
    ...
    key_ops: set[O] | None = pydantic.Field(
        default=None,
    ...
    def get_hash(self, alg: JSONWebAlgorithm):
        assert alg.config.dig is not None
