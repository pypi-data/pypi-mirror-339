...
import torchbiosound.datapipes.parametric_umap
...
class TestDatapipe:
    ...
    def test_from_dataset_path(self, config_type, model_name, audio_format, spect_format, annot_format,
        ...
        dataset = torchbiosound.datapipes.parametric_umap.Datapipe.from_dataset_path(
            dataset_path=cfg_command.dataset.path,
            split=split,
        )
        assert isinstance(dataset, torchbiosound.datapipes.parametric_umap.Datapipe)
