...
import torchbiosound.datapipes.frame_classification
...
class TestMetadata:
    ...
    def test_metadata_init(self, dataset_csv_filename, input_type, frame_dur):
        metadata = torchbiosound.datapipes.frame_classification.Metadata(dataset_csv_filename, input_type, frame_dur)
        assert isinstance(metadata, torchbiosound.datapipes.frame_classification.Metadata)
    ...
    def test_metadata_from_path(self, dataset_csv_filename, input_type, frame_dur, tmp_path):
        ...
        metadata_json_path = tmp_path / torchbiosound.datapipes.frame_classification.Metadata.METADATA_JSON_FILENAME
        with metadata_json_path.open('w') as fp:
            json.dump(metadata_dict, fp, indent=4)

        metadata = torchbiosound.datapipes.frame_classification.Metadata.from_path(metadata_json_path)
        assert isinstance(metadata, torchbiosound.datapipes.frame_classification.Metadata)
        for attr_name, attr_val in zip(
            ('dataset_csv_filename', 'input_type', 'frame_dur'),
            (dataset_csv_filename, input_type, frame_dur),
    ...
    def test_metadata_to_json(self, dataset_csv_filename, input_type, frame_dur, tmp_path):
        metadata_to_json = torchbiosound.datapipes.frame_classification.Metadata(dataset_csv_filename, input_type, frame_dur)
        mock_dataset_path = tmp_path / 'mock_dataset'
        mock_dataset_path.mkdir()

        metadata_to_json.to_json(dataset_path=mock_dataset_path)
        expected_json_path = mock_dataset_path / torchbiosound.datapipes.frame_classification.Metadata.METADATA_JSON_FILENAME
        assert expected_json_path.exists()

        metadata_from_json = torchbiosound.datapipes.frame_classification.Metadata.from_path(expected_json_path)
