...
import torchbiosound.learncurve
...
def assert_learncurve_output_matches_expected(cfg, model_name, results_path):
    ...
    for train_set_dur in cfg.prep.train_set_durs:
        train_set_dur_root = results_path / torchbiosound.learncurve.dirname.train_dur_dirname(train_set_dur)
        assert train_set_dur_root.exists()

        for replicate_num in range(1, cfg.prep.num_replicates + 1):
            replicate_path = train_set_dur_root / torchbiosound.learncurve.dirname.replicate_dirname(replicate_num)
...
def test_learning_curve_for_frame_classification_model(
    ...
    results_path = torchbiosound.common.paths.generate_results_dir_name_as_path(tmp_path)
    results_path.mkdir()

    torchbiosound.learncurve.frame_classification.learning_curve_for_frame_classification_model(
