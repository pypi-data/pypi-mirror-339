...
import torchbiosound
import torchbiosound.common  # for constants
...
class TestFromSegments:
    def test_init(self):
        from_segments_tfm = torchbiosound.transforms.frame_labels.FromSegments()
        assert isinstance(from_segments_tfm, torchbiosound.transforms.frame_labels.FromSegments)
    ...
    def test_call(self, annot, spect_path, labelset):
        labelset = torchbiosound.common.converters.labelset_to_set(labelset)
        labelmap = torchbiosound.common.labels.to_map(labelset, True)

        spect_dict = torchbiosound.common.files.spect.load(spect_path)
        ...
        from_segments_tfm = torchbiosound.transforms.frame_labels.FromSegments(background_label=labelmap[torchbiosound.common.constants.DEFAULT_BACKGROUND_LABEL])
...
class TestToLabels:
    ...
    def test_init(self, labelset):
        ...
        labelset = torchbiosound.common.converters.labelset_to_set(labelset)
        labelmap = torchbiosound.common.labels.to_map(labelset, map_background=True)

        to_labels_tfm = torchbiosound.transforms.frame_labels.ToLabels(
            labelmap=labelmap,
        )
        assert isinstance(to_labels_tfm, torchbiosound.transforms.frame_labels.ToLabels)

    @pytest.mark.parametrize(
        "lbl_tb, labelmap, labels_expected_int",
        [
            (np.array([0, 0, 1, 1, 0, 0, 2, 2, 0, 0]), {torchbiosound.common.constants.DEFAULT_BACKGROUND_LABEL: 0, 'a': 1, 'b': 2}, [1, 2]),
            (np.array([0, 0, 1, 1, 0, 0, 2, 2, 0, 0]), {torchbiosound.common.constants.DEFAULT_BACKGROUND_LABEL: 0, '1': 1, '2': 2}, [1, 2]),
            (np.array([0, 0, 21, 21, 0, 0, 22, 22, 0, 0]), {torchbiosound.common.constants.DEFAULT_BACKGROUND_LABEL: 0, '21': 21, '22': 22}, [21, 22]),
            (np.array([0, 0, 11, 11, 0, 0, 12, 12, 0, 0]), {torchbiosound.common.constants.DEFAULT_BACKGROUND_LABEL: 0, '11': 11, '12': 12}, [11, 12]),
        ]
    )
    def test_call(self, lbl_tb, labelmap, labels_expected_int):
        # Note that we add an vak.common.constants.DEFAULT_BACKGROUND_LABEL class because post-processing transforms *require* it
        # This is default, just making it explicit
        labelmap = torchbiosound.common.labels.multi_char_labels_to_single_char(
            labelmap, skip=(torchbiosound.common.constants.DEFAULT_BACKGROUND_LABEL,)
        )
    ...
    def test_call_real_data(
        ...
        labelset = torchbiosound.common.converters.labelset_to_set(labelset)
        labelmap = torchbiosound.common.labels.to_map(labelset)
        ...
        timebins = torchbiosound.common.files.spect.load(spect_path)[TIMEBINS_KEY]

        lbl_tb = torchbiosound.transforms.frame_labels.from_segments(
            lbls_int,
            annot.seq.onsets_s,
            annot.seq.offsets_s,
            timebins,
            background_label=labelmap[torchbiosound.common.constants.DEFAULT_BACKGROUND_LABEL],
        )
...
class TestToSegments:
    ...
    def test_init(self, labelset):
        ...
        to_segments_tfm = torchbiosound.transforms.frame_labels.ToSegments(
            labelmap=labelmap,
        )
        assert isinstance(to_segments_tfm, torchbiosound.transforms.frame_labels.ToSegments)
...
class TestPostprocess:
    ...
    def test_init(self, min_segment_dur, majority_vote, timebin_dur):
        ...
        # This is default, just making it explicit
        to_labels_tfm = torchbiosound.transforms.frame_labels.PostProcess(
            min_segment_dur=min_segment_dur,
            majority_vote=majority_vote,
            timebin_dur=timebin_dur,
        )
        assert isinstance(to_labels_tfm, torchbiosound.transforms.frame_labels.PostProcess)
    ...
    def test_call(self, lbl_tb, timebin_dur, background_label, min_segment_dur, majority_vote, lbl_tb_expected):
        # Note that we add an vak.common.constants.DEFAULT_BACKGROUND_LABEL class because post-processing transforms *require* it
        # This is default, just making it explicit
        postprocess_tfm = torchbiosound.transforms.frame_labels.PostProcess(
            min_segment_dur=min_segment_dur,
            majority_vote=majority_vote,
            timebin_dur=timebin_dur,
        )
