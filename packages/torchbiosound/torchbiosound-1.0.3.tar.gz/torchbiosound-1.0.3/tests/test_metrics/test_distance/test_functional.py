...
import torchbiosound.metrics.distance.functional
...
def test_levenshtein(source, target, expected_distance):
    distance = torchbiosound.metrics.distance.functional.levenshtein(source, target)
...
def test_character_error_rate(y_pred, y_true, expected_distance):
    if len(y_true) == 0 and len(y_pred) == 0:
        expected_rate = 0
        rate = torchbiosound.metrics.distance.functional.character_error_rate(y_pred, y_true)
        assert rate == expected_rate
    elif len(y_true) == 0 and len(y_pred) != 0:
        with pytest.raises(ValueError):
            torchbiosound.metrics.distance.functional.character_error_rate(y_pred, y_true)
    else:
