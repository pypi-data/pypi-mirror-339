...
import torchbiosound.common.annotation


def test_files_from_dir(annot_dir_notmat, annot_files_notmat):
    annot_files_from_dir = torchbiosound.common.annotation.files_from_dir(
...
def test_audio_stem_from_path(path, audio_ext, expected_stem):
    stem = torchbiosound.common.annotation.audio_filename_from_path(path, audio_ext)
...
def test_audio_stem_from_path_raises(path, audio_ext):
    with pytest.raises(torchbiosound.common.annotation.AudioFilenameNotFoundError):
        torchbiosound.common.annotation.audio_filename_from_path(path, audio_ext)
...
def test__map_using_notated_path(
    ...
    annotated_annot_map = torchbiosound.common.annotation._map_using_notated_path(
    ...
    for source_path, annot in list(annotated_annot_map.items()):
        assert torchbiosound.common.annotation.audio_filename_from_path(
            annot.notated_path
        ) == torchbiosound.common.annotation.audio_filename_from_path(source_path)
...
def test__map_using_ext(
    ...
    annotated_annot_map = torchbiosound.common.annotation._map_using_ext(
...
def test_map_annotated_to_annot(
    ...
    if source_type == "audio":
        ...
        annotated_files = spect_list_mat
    annot_list = specific_annot_list(annot_format)
    annotated_annot_map = torchbiosound.common.annotation.map_annotated_to_annot(
        annotated_files=annotated_files, annot_list=annot_list, annot_format=annot_format
    )
...
def test_has_unlabeled(annot, duration, expected_result):
    ...
    """
    has_unlabeled = torchbiosound.common.annotation.has_unlabeled(annot, duration)
...
def test_has_unlabeled_annotation_with_no_segments(annotated_annot_no_segments):
    ...
    assert torchbiosound.common.annotation.has_unlabeled(annot, dur) is True
