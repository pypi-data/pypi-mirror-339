...
def results_path_matches_expected(
    ...
    assert results_path.name.startswith(torchbiosound.common.constants.RESULTS_DIR_PREFIX)
...
def test_generate_results_dir_name_as_path(tmp_path):
    ...
    results_path = torchbiosound.common.paths.generate_results_dir_name_as_path(
...
def test_generate_results_dir_name_as_path_no_root_results_dir(tmp_path):
    ...
    home = os.getcwd()
    os.chdir(tmp_path)
    results_path = torchbiosound.common.paths.generate_results_dir_name_as_path(root_results_dir=None)
    after_timestamp = time.mktime(datetime.now().timetuple())
...
def test_generate_results_dir_name_as_path_nonexistent_root_raises():
    with pytest.raises(NotADirectoryError):
        torchbiosound.common.paths.generate_results_dir_name_as_path(
