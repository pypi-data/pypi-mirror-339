...
import torchbiosound.common.files.files
...
def test_find_fname(fname, find_ext, spect_ext):
    ...
    out = torchbiosound.common.files.files.find_fname(fname, find_ext)
...
def test_files_from_dir_with_mat(spect_dir_mat, spect_list_mat):
    files = torchbiosound.common.files.files.from_dir(spect_dir_mat, "mat")
...
def test_files_from_dir_with_cbin(audio_dir_cbin, audio_list_cbin):
    files = torchbiosound.common.files.files.from_dir(audio_dir_cbin, "cbin")
    # files.from_dir returns str not Path, need to convert fixture
