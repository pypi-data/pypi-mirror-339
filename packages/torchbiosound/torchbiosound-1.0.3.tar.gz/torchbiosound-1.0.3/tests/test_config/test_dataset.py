...
import torchbiosound.config.dataset
...
class TestDatasetConfig:
    ...
    def test_init(self, path, splits_path, name):
        if name is None and splits_path is None:
            dataset_config = torchbiosound.config.dataset.DatasetConfig(
                path=path
            )
        elif name is None:
            dataset_config = torchbiosound.config.dataset.DatasetConfig(
        ...
        assert isinstance(dataset_config, torchbiosound.config.dataset.DatasetConfig)
        assert dataset_config.path == torchbiosound.common.converters.expanded_user_path(path)
        if splits_path is not None:
            assert dataset_config.splits_path == torchbiosound.common.converters.expanded_user_path(splits_path)
        else:
    ...
    def test_from_config_dict(self, config_dict):
        dataset_config = torchbiosound.config.dataset.DatasetConfig.from_config_dict(config_dict)
        assert isinstance(dataset_config, torchbiosound.config.dataset.DatasetConfig)
        assert dataset_config.path == torchbiosound.common.converters.expanded_user_path(config_dict['path'])
        if 'splits_path' in config_dict:
            assert dataset_config.splits_path == torchbiosound.common.converters.expanded_user_path(config_dict['splits_path'])
        else:
            assert dataset_config.splits_path is None
    ...
    def test_asdict(self, config_dict):
        ...
        for key in ('name', 'path', 'splits_path', 'params'):
            if key in config_dict:
                if 'path' in key:
                    assert dataset_config_as_dict[key] == torchbiosound.common.converters.expanded_user_path(config_dict[key])
                else:
