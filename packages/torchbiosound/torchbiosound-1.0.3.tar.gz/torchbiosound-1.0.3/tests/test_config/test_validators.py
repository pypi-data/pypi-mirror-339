...
import torchbiosound.config.validators


def test_are_tables_valid(invalid_table_config_path):
    """test that invalid table name raises a ValueError"""
    config_dict = torchbiosound.config.load._load_toml_from_path(invalid_table_config_path)
    with pytest.raises(ValueError):
        torchbiosound.config.validators.are_tables_valid(
...
def test_are_keys_valid(invalid_key_config_path):
    ...
    config_dict = torchbiosound.config.load._load_toml_from_path(invalid_key_config_path)
    with pytest.raises(ValueError):
        torchbiosound.config.validators.are_keys_valid(
