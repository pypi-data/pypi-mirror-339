...
import torchbiosound.config.trainer
...
class TestTrainerConfig:
    ...
    def test_init(self, config_dict):
        trainer_config = torchbiosound.config.trainer.TrainerConfig(**config_dict)

        assert isinstance(trainer_config, torchbiosound.config.trainer.TrainerConfig)
        if 'accelerator' in config_dict:
            ...
            assert getattr(trainer_config, 'accelerator') == torchbiosound.common.accelerator.get_default()
    ...
    def test_init_raises(self, config_dict, expected_exception):
        with pytest.raises(expected_exception):
            torchbiosound.config.trainer.TrainerConfig(**config_dict)
    ...
    def test_asdict(self, config_dict):
        ...
        if 'accelerator' in config_dict:
            ...
            # TODO: mock `accelerator.get_default` here, return either 'cpu' or 'gpu'
            assert trainer_config_asdict['accelerator'] == torchbiosound.common.accelerator.get_default()
        if 'devices' in config_dict:
