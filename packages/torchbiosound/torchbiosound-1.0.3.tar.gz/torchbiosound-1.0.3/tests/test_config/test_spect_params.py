...
import torchbiosound.config.spect_params
...
def test_freq_cutoffs_not_two_values_raises():
    ...
    with pytest.raises(ValueError):
        torchbiosound.config.spect_params.freq_cutoffs_validator(
...
def test_invalid_transform_type_raises():
    ...
    with pytest.raises(ValueError):
        torchbiosound.config.spect_params.is_valid_transform_type(
            instance=None,
...
class TestSpectParamsConfig:
    ...
    def test_init(self, config_dict):
        spect_params_config = torchbiosound.config.SpectParamsConfig(**config_dict)
        assert isinstance(spect_params_config, torchbiosound.config.spect_params.SpectParamsConfig)
    ...
    def test_with_real_config(self, a_generated_config_dict):
        ...
        spect_params_config = torchbiosound.config.spect_params.SpectParamsConfig(**spect_config_dict)
        assert isinstance(spect_params_config, torchbiosound.config.spect_params.SpectParamsConfig)
