...
import torchbiosound.config.prep
...
class TestPrepConfig:
    ...
    def test_init(self, config_dict):
        config_dict['spect_params'] = torchbiosound.config.SpectParamsConfig(**config_dict['spect_params'])

        prep_config = torchbiosound.config.PrepConfig(**config_dict)

        assert isinstance(prep_config, torchbiosound.config.prep.PrepConfig)
        for key, val in config_dict.items():
            assert hasattr(prep_config, key)
            if key == 'data_dir' or key == 'output_dir':
                assert getattr(prep_config, key) == torchbiosound.common.converters.expanded_user_path(val)
            elif key == 'labelset':
                assert getattr(prep_config, key) == torchbiosound.common.converters.labelset_to_set(val)
            else:
    ...
    def test_from_config_dict(self, config_dict):
        ...
        prep_config = torchbiosound.config.prep.PrepConfig.from_config_dict(config_dict_copy)
        ...
        for key, val in config_dict.items():
            ...
            if key == 'data_dir' or key == 'output_dir':
                ...
                assert getattr(prep_config, key) == torchbiosound.config.SpectParamsConfig(**val)
            else:
                assert getattr(prep_config, key) == val
    ...
    def test_from_config_dict_real_config(
        ...
        prep_config = torchbiosound.config.prep.PrepConfig.from_config_dict(a_generated_config_dict['prep'])
        assert isinstance(prep_config, torchbiosound.config.prep.PrepConfig)
