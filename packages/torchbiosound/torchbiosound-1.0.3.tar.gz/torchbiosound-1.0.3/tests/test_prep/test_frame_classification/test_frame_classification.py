...
def assert_prep_output_matches_expected(dataset_path, df_returned_by_prep):
    ...
    meta_json_path = dataset_path / torchbiosound.datapipes.frame_classification.Metadata.METADATA_JSON_FILENAME
    ...
    for column in torchbiosound.prep.spectrogram_dataset.spect_helper.DF_COLUMNS:
        if column == "duration":
            check_exact = False
        else:
            check_exact = True
    ...
    if torchbiosound.datapipes.frame_classification.constants.FRAMES_PATH_COL_NAME in df_returned_by_prep.columns:
    ...
    if torchbiosound.datapipes.frame_classification.constants.MULTI_FRAME_LABELS_PATH_COL_NAME in df_returned_by_prep.columns:
...
def test_prep_frame_classification_dataset(
    ...
    dataset_df, dataset_path = torchbiosound.prep.frame_classification.frame_classification.prep_frame_classification_dataset(
...
def test_prep_frame_classification_dataset_raises_when_labelset_required_but_is_none(
    ...
    with pytest.raises(ValueError):
        torchbiosound.prep.frame_classification.frame_classification.prep_frame_classification_dataset(
            data_dir=cfg.prep.data_dir,
            input_type=cfg.prep.input_type,
            purpose=purpose,
            audio_format=cfg.prep.audio_format,
