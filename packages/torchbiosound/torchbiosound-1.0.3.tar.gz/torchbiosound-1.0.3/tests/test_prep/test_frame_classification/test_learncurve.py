...
import torchbiosound.common.converters
import torchbiosound.common.labels
import torchbiosound.common.paths
import torchbiosound.prep.frame_classification
...
def test_make_index_vectors_for_each_subsets(
    ...
    metadata = torchbiosound.datapipes.frame_classification.Metadata.from_dataset_path(dataset_path)
    ...
    for train_dur in cfg.prep.train_set_durs:
        for replicate_num in range(1, cfg.prep.num_replicates + 1):
            train_dur_replicate_subset_name = torchbiosound.common.learncurve.get_train_dur_replicate_subset_name(
                    train_dur, replicate_num
                )
            sample_id_vec_path = (tmp_dataset_path / "train" /
                                  torchbiosound.datapipes.frame_classification.helper.sample_ids_array_filename_for_subset(
                                      train_dur_replicate_subset_name)
                                  )
            sample_id_vec_path.unlink()
            inds_in_sample_vec_path = (tmp_dataset_path / "train" /
                                       torchbiosound.datapipes.frame_classification.helper.inds_in_sample_array_filename_for_subset(
                                       train_dur_replicate_subset_name)
                                  )
            inds_in_sample_vec_path.unlink()

    torchbiosound.prep.frame_classification.learncurve.make_index_vectors_for_each_subset(
        subsets_df,
        tmp_dataset_path,
        input_type,
    )
    ...
    for train_dur in cfg.prep.train_set_durs:
        ...
        for replicate_num in range(1, cfg.prep.num_replicates + 1):
            subset_name = torchbiosound.common.learncurve.get_train_dur_replicate_subset_name(
                    train_dur, replicate_num
                )
            ...
            for sample_id, frames_path in enumerate(frames_paths):
                ...
                frames = torchbiosound.datapipes.frame_classification.helper.load_frames(tmp_dataset_path / frames_path,
                                                                              input_type)
...
def test_make_subsets_from_dataset_df(
    ...
    with mock.patch('vak.prep.frame_classification.learncurve.make_index_vectors_for_each_subset') as mock_idx_vectors:
        out = torchbiosound.prep.frame_classification.learncurve.make_subsets_from_dataset_df(
            dataset_df,
            input_type,
