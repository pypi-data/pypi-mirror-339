...
import torchbiosound.prep.frame_classification.make_splits
...
def test_argsort_by_label_freq(annots, expected_sort_inds):
    out = torchbiosound.prep.frame_classification.make_splits.argsort_by_label_freq(annots)
...
def test_make_splits(config_type, model_name, audio_format, spect_format, annot_format,
    ...
    cfg = torchbiosound.config.Config.from_toml_path(toml_path)
    ...
    if purpose != "predict":
        ...
        map_unlabeled_segments = torchbiosound.prep.sequence_dataset.has_unlabeled_segments(
            dataset_df
        )
        labelmap = torchbiosound.common.labels.to_map(
            cfg.prep.labelset, map_background=map_unlabeled_segments
        )
    else:
        labelmap = None

    dataset_df_with_splits = torchbiosound.prep.frame_classification.make_splits.make_splits(
        dataset_df,
        tmp_dataset_path,
        cfg.prep.input_type,
        purpose,
    ...
    for split in splits:
        ...
        assert torchbiosound.datapipes.frame_classification.constants.FRAMES_PATH_COL_NAME in split_df.columns

        frames_paths = split_df[
            torchbiosound.datapipes.frame_classification.constants.FRAMES_PATH_COL_NAME
        ].values

        if purpose != "predict":
            assert torchbiosound.datapipes.frame_classification.constants.MULTI_FRAME_LABELS_PATH_COL_NAME in split_df.columns

            frame_labels_paths = split_df[
                torchbiosound.datapipes.frame_classification.constants.MULTI_FRAME_LABELS_PATH_COL_NAME
            ].values

            annots = torchbiosound.common.annotation.from_df(split_df)
        ...
        for sample_id, frames_tuple in enumerate(frames_tuples):
            ...
            # see next code block
            frames = torchbiosound.datapipes.frame_classification.helper.load_frames(tmp_dataset_path / frames_path, input_type)
            ...
            if frame_labels_path is not None and annot is not None:
                ...
                if input_type == "audio":
                    _, samplefreq = torchbiosound.common.constants.AUDIO_FORMAT_FUNC_MAP[
                        audio_format
                    ](tmp_dataset_path / frames_path)
                    frame_times = np.arange(frames.shape[-1]) / samplefreq
                elif input_type == "spect":
                    spect_dict = torchbiosound.common.files.spect.load(tmp_dataset_path / frames_path, "npz")
                    frame_times = spect_dict[torchbiosound.common.constants.TIMEBINS_KEY]

                lbls_int = [labelmap[lbl] for lbl in annot.seq.labels]
                expected_frame_labels = torchbiosound.transforms.frame_labels.from_segments(
                    lbls_int,
                    annot.seq.onsets_s,
                    annot.seq.offsets_s,
                    frame_times,
        ...
        spect_npz_files_not_in_split = sorted(
            tmp_dataset_path.glob(f'*{torchbiosound.common.constants.SPECT_NPZ_EXTENSION}')
        )
        assert len(spect_npz_files_not_in_split) == 0

        sample_id_vec_path = (
            split_subdir /
            torchbiosound.datapipes.frame_classification.constants.SAMPLE_IDS_ARRAY_FILENAME
        )
        ...
        inds_in_sample_vec_path = (
            split_subdir /
            torchbiosound.datapipes.frame_classification.constants.INDS_IN_SAMPLE_ARRAY_FILENAME
        )
