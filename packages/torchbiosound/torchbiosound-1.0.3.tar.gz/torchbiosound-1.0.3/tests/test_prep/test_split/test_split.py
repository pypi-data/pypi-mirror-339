...
import torchbiosound.prep.spectrogram_dataset.spect_helper
import torchbiosound.prep.split.split
...
def test_train_test_dur_split_inds_fake_data(
    ...
    for _ in range(NUM_SAMPLES):
        train_inds, val_inds, test_inds = torchbiosound.prep.split.split.train_test_dur_split_inds(
...
def test_train_test_dur_split_inds_fake_data_impossible():
    ...
    with pytest.raises(ValueError):
        torchbiosound.prep.split.split.train_test_dur_split_inds(
            durs, labels, labelset, train_dur, test_dur, val_dur
        )
...
def test_split_frame_classification_dataframe(
    ...
    labelset = set(key for key in labelmap.keys() if key != torchbiosound.common.constants.DEFAULT_BACKGROUND_LABEL)

    dataset_df_split = torchbiosound.prep.split.split.frame_classification_dataframe(
...
def test_split_unit_dataframe(
    ...
    metadata = torchbiosound.datapipes.parametric_umap.Metadata.from_dataset_path(dataset_path)
    dataset_csv_path = dataset_path / metadata.dataset_csv_filename
    dataset_df = pd.read_csv(dataset_csv_path)
    dataset_df = dataset_df.drop(columns=('split'))
    labelmap_path = dataset_path / "labelmap.json"
    ...
    dataset_df_split = torchbiosound.prep.split.split.unit_dataframe(
        dataset_df, dataset_path, labelset=labelset, train_dur=train_dur, val_dur=val_dur, test_dur=test_dur
    )
