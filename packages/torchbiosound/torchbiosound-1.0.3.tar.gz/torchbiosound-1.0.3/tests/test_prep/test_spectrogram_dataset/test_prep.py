...
import torchbiosound.prep.spectrogram_dataset.prep
...
def assert_returned_dataframe_matches_expected(
    ...
    assert df_returned.columns.values.tolist() == torchbiosound.prep.spectrogram_dataset.spect_helper.DF_COLUMNS
    ...
    if annot_format is not None:
        ...
        annot_list = torchbiosound.common.annotation.from_df(df_returned)
    else:
        # if no annotation format specified, check that `annot_format` is `none`
        assert annot_format_from_df == torchbiosound.common.constants.NO_ANNOTATION_FORMAT
        annot_list = None
...
def test_prep_spectrogram_dataset(
    ...
    dataset_df = torchbiosound.prep.spectrogram_dataset.prep.prep_spectrogram_dataset(
        data_dir=data_dir,
