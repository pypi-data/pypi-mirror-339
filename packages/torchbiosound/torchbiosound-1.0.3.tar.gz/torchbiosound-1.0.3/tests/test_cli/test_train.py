...
import torchbiosound.cli.train
...
def test_train(
    ...
    with mock.patch('vak.train.train', autospec=True) as mock_core_train:
        torchbiosound.cli.train.train(toml_path)
        assert mock_core_train.called

    results_path = sorted(root_results_dir.glob(f"{torchbiosound.common.constants.RESULTS_DIR_PREFIX}*"))
