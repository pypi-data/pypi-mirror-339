...
class TestFrameClassificationModel:
    ...
    def test_load_state_dict_from_path(self,
        ...
        train_cfg = torchbiosound.config.Config.from_toml_path(train_toml_path)

        # stuff we need just to be able to instantiate network
        labelmap = torchbiosound.common.labels.to_map(train_cfg.prep.labelset, map_background=True)
        train_dataset = torchbiosound.datapipes.frame_classification.TrainDatapipe.from_dataset_path(
        ...
        cfg = torchbiosound.config.Config.from_toml_path(train_toml_path)
        ...
        eval_cfg = torchbiosound.config.Config.from_toml_path(eval_toml_path)
