...
class ConvEncoderUMAPDefinition:
    network = {"encoder": torchbiosound.nets.ConvEncoder}
    loss = torchbiosound.nn.UmapLoss
    optimizer = torch.optim.AdamW
    metrics = {
        "acc": torchbiosound.metrics.Accuracy,
        "levenshtein": torchbiosound.metrics.Levenshtein,
        "character_error_rate": torchbiosound.metrics.CharacterErrorRate,
...
class TestModelFactory:
    def test_init_no_definition_raises(self):
        ...
        with pytest.raises(TypeError):
            torchbiosound.models.factory.ModelFactory()
    ...
    def test_init_invalid_definition_raises(self):
        ...
        with pytest.raises(torchbiosound.models.decorator.ModelDefinitionValidationError):
            torchbiosound.models.factory.ModelFactory(
    ...
    def test_validate_instances_or_get_default(self, definition, kwargs):
        model = torchbiosound.models.factory.ModelFactory(
            definition,
            MockModelFamily,
        )
    ...
    def test_from_config_frame_classification(self, labelmap, input_shape, definition):
        model_factory = torchbiosound.models.factory.ModelFactory(
            definition,
            torchbiosound.models.FrameClassificationModel,
        )
        ...
        assert isinstance(model, torchbiosound.models.FrameClassificationModel)
        for attr in ('network', 'loss', 'optimizer', 'metrics'):
    ...
    def test_from_config_with_frame_classification(self, definition, specific_config_toml_path):
        ...
        labelmap = torchbiosound.common.labels.to_map(cfg.prep.labelset, map_background=True)
    ...
    def test_from_instances_parametric_umap(
        ...
        network = {'encoder': torchbiosound.nets.ConvEncoder(input_shape)}

        model_factory = torchbiosound.models.ModelFactory(
            definition,
            torchbiosound.models.ParametricUMAPModel,
        )
        model = model_factory.from_instances(network=network)

        # now test that attributes are what we expect
        assert isinstance(model, torchbiosound.models.ParametricUMAPModel)
        for attr in ('network', 'loss', 'optimizer', 'metrics'):
            assert hasattr(model, attr)
            model_attr = getattr(model, attr)
            definition_attr = getattr(definition, attr)
