...
class TestED_TCN:
    ...
    def test_init(self, labelmap, input_shape):
        ...
        network = torchbiosound.models.ED_TCN.definition.network(len(labelmap), num_input_channels, num_freqbins)
        model = torchbiosound.models.ED_TCN.from_instances(network=network, labelmap=labelmap)
        ...
        for attr in ('network', 'loss', 'optimizer'):
            ...
            assert isinstance(getattr(model, attr),
                              getattr(torchbiosound.models.ed_tcn.ED_TCN.definition, attr))
        ...
        for metric_name, metric_callable in model.metrics.items():
            assert isinstance(metric_callable,
                              torchbiosound.models.ed_tcn.ED_TCN.definition.metrics[metric_name])
