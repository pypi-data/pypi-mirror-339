...
import torchbiosound.models.registry
...
def test_model_family():
    ...
    assert ModelFamilyCopy.__name__ not in torchbiosound.models.registry.MODEL_FAMILY_REGISTRY
    torchbiosound.models.registry.model_family(ModelFamilyCopy)
    assert ModelFamilyCopy.__name__ in torchbiosound.models.registry.MODEL_FAMILY_REGISTRY
    assert torchbiosound.models.registry.MODEL_FAMILY_REGISTRY[ModelFamilyCopy.__name__] == ModelFamilyCopy
...
def test_register_model(family, definition):
    ...
    model_factory = torchbiosound.models.factory.ModelFactory(
        definition,
        family
    )
    model_factory.__name__ = model_name

    assert model_name not in torchbiosound.models.registry.MODEL_REGISTRY
    torchbiosound.models.registry.register_model(model_factory)
    assert model_name in torchbiosound.models.registry.MODEL_REGISTRY
    assert torchbiosound.models.registry.MODEL_REGISTRY[model_name] == model_factory
    del torchbiosound.models.registry.MODEL_REGISTRY[model_name]  # so this test doesn't fail for the second case
...
def test_register_model_raises_family():
    ...
    model_factory = torchbiosound.models.ModelFactory(
        MockModel,
        UnregisteredMockModelFamily,
    )
...
@pytest.mark.parametrize(
    'family, definition',
    [
        (torchbiosound.models.FrameClassificationModel, TweetyNetDefinition),
    ]
)
...
def test___get_attr__MODEL_FAMILY_FROM_NAME():
    assert hasattr(torchbiosound.models.registry, 'MODEL_FAMILY_FROM_NAME')

    model_family_from_name_dict = getattr(torchbiosound.models.registry, 'MODEL_FAMILY_FROM_NAME')
    assert isinstance(model_family_from_name_dict, dict)

    for model_name, model_factory in torchbiosound.models.registry.MODEL_REGISTRY.items():
...
def test___get_attr__MODEL_NAMES():
    assert hasattr(torchbiosound.models.registry, 'MODEL_NAMES')
    model_names_list = getattr(torchbiosound.models.registry, 'MODEL_NAMES')
    assert isinstance(model_names_list, list)
    for model_name in torchbiosound.models.registry.MODEL_REGISTRY.keys():
