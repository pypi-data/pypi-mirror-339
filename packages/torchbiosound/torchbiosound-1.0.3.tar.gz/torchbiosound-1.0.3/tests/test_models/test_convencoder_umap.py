...
class TestConvEncoderUMAP:
    ...
    def test_from_instances(self, input_shape):
        network = {
            'encoder': torchbiosound.models.ConvEncoderUMAP.definition.network['encoder'](input_shape=input_shape)
        }
        model = torchbiosound.models.ConvEncoderUMAP.from_instances(network=network)
        assert isinstance(model, torchbiosound.models.ParametricUMAPModel)
        for attr in ('network', 'loss', 'optimizer'):
            assert hasattr(model, attr)
            attr_from_definition = getattr(torchbiosound.models.convencoder_umap.ConvEncoderUMAP.definition, attr)
            if isinstance(attr_from_definition, dict):
                ...
                assert isinstance(getattr(model, attr),
                                  getattr(torchbiosound.models.convencoder_umap.ConvEncoderUMAP.definition, attr))
        ...
        for metric_name, metric_callable in model.metrics.items():
            assert isinstance(metric_callable,
                              torchbiosound.models.convencoder_umap.ConvEncoderUMAP.definition.metrics[metric_name])
