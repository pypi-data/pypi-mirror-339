...
def test_predict_with_frame_classification_model(
    ...
    torchbiosound.predict.frame_classification.predict_with_frame_classification_model(
    ...
    if save_net_outputs:
        net_outputs = sorted(
            Path(output_dir).glob(f"*{torchbiosound.common.constants.NET_OUTPUT_SUFFIX}")
        )

        metadata = torchbiosound.datapipes.frame_classification.Metadata.from_dataset_path(cfg.predict.dataset.path)
