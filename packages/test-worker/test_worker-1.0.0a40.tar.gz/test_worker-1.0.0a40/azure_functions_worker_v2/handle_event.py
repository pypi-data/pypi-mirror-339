...
async def worker_init_request(request):
    ...
    try:
        ...
        logger.info("HTTP server init error has occurred")
        ...
        metadata_exception = ex
        logger.info("An exception in WorkerInitRequest has occurred: %s", ex)
...
async def invocation_request(request):
    ...
    try:
        ...
        logger.info("An exception in WorkerInvocationRequest has occurred: %s", ex)
        if http_v2_enabled:
...
async def function_environment_reload_request(request):
    ...
    try:
        ...
        logger.info("An exception in WorkerEnvReloadRequest has occurred: %s", ex)
        metadata_exception = ex
        return protos.FunctionEnvironmentReloadResponse(
            result=protos.StatusResult(
                status=protos.StatusResult.Failure,
