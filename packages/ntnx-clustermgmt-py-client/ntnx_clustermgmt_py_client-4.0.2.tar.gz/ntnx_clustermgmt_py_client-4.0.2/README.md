# Python Client For Nutanix Cluster Management APIs

The Python client for Nutanix Cluster Management APIs is designed for Python client application developers offering them simple and flexible access to APIs that manage Hosts, Clusters and other Infrastructure.
## Features
...
## Configuration
The python client for Nutanix Cluster Management APIs can be configured with the following parameters
...
```python
# Initialize the API
cluster_profiles_api_instance = ClusterProfilesApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Get cluster profile
try:
    api_response = cluster_profiles_api_instance.get_cluster_profile_by_id(extId)
...
try:
    ...
    api_response = cluster_profiles_api_instance.update_cluster_profile_by_id(body, extId, if_match=etag_value) # Use the extracted etag value
except ApiException as e:
```
...
try:
    api_response = cluster_profiles_api_instance.list_cluster_profiles(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=clustermgmt&version=v4.0&language=python). This documentation is auto-generated, and the location may change.
